...
type cls = Help | With | Comp of bool

module Key = struct
  type t = string [@@deriving sexp]

  let normalize = String.map ~f:(function '_' | ' ' -> '-' | c -> c)
  let sexp_of_t x = sexp_of_string @@ String.lowercase @@ normalize x
  ...
  include Comparable.Make (struct
    ...
    let compare x y = String.Caseless.compare (normalize x) (normalize y)
  end)
end
...
let () =
  Cfg.main ~args ~name:"bap-configurator" @@ fun self ->
  let ocamlc ?name var =
    (Option.value name ~default:var, Cfg.ocaml_config_var_exn self var)
  in

  let prefix =
    let stdlib = Cfg.ocaml_config_var_exn self "standard_library" in
    String.chop_suffix_exn stdlib ~suffix:"/lib/ocaml"
  in

  let build_id =
    try
      Cfg.Process.run_capture_exn self "git"
  ...
  let site name =
    ...
    ("site_" ^ name, List.map sites ~f:(sprintf "%S") |> String.concat ~sep:"; ")
  in

  if init.contents then (
    let vars =
    ...
    let add_variable spec =
      ...
      | [ key; data ] -> Hashtbl.set vars ~key ~data
      | _ -> Cfg.die "An invalid variable specification %S\n" spec
    in

    let classify arg =
      List.find_map classes ~f:(fun ((prefix, _) as cls) ->
          Option.some_if (String.is_prefix ~prefix arg) cls)
    in
  ...
  let vars =
    cfg_of_sexp @@ Sexp.of_string @@ In_channel.read_all "config.status"
  in
  ...
  let rec substitute str =
    ...
    if String.equal res1 res2 then res2 else substitute res2
  in
