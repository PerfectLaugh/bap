let ( / ) = Filename.concat
let header = "common"
let has_extension name = String.contains name '.'
let enabled = "--enable-"
...
let feature enable arg =
  ...
  try
    if String.sub arg 0 prefix = enable then
      Some (String.sub arg prefix (length - prefix))
    else None
  with exn -> None
...
let features args =
  ...
  if List.(mem "--enable-everything" args || mem "--help" args) then
    header :: everything_except disabled_args
  else header :: selected enabled args

let special_args =
  List.concat
  @@ List.map
       (fun f -> [ enabled ^ f; disabled ^ f ])
      ...
      (fun arg ->
        ...
        || not
             (mem arg
                ...
                @ map (fun f -> disabled ^ f) disabled_args)))
      args)
