...
let destdir_arg =
  ( "-destdir",
    ...
    sprintf "DIR Set the destination directory (default: %s)" !destdir )

let field_modified field = Hash_set.add modified (Field.name field)
let is_field_modified field = Hash_set.mem modified (Field.name field)
...
let chop_extension name =
  if String.mem name '.' then Filename.chop_extension name else name

let name_of_path path = Filename.basename path |> chop_extension
let fields = String.split ~on:','

let named_resource r =
...
let open_bundle () =
  match normalized target.contents with
...
module Update = struct
  let resources = ref []
  let add_res r = resources := r :: !resources
  let set_list s f = String.split ~on:',' s |> List.map ~f:String.strip |> set f
  let set_name s = set name @@ String.map ~f:(function '_' -> '-' | c -> c) s
  ...
  let args =
    common_args
    @ Arg.[ ("-add-resources", String add_res, "<list> Add resource files") ]

  let update m field =
    if is_field_modified field then
      Field.fset field m (Field.get field !manifest)
    else m
...
module Pack = struct
  ...
  let add_resources rs = List.map (fields rs) ~f:named_resource |> bundle_files
  ...
  let args =
    ...
    @ Arg.
        [
          ...
          ( "-provides",
            String add_provides,
            "<list> List of provided interfaces" );
          ( "-resources",
            String add_resources,
            "<list> List of embedded resources" );
        ]

  let main () =
    let manifest = !manifest in
    let target =
      if String.is_empty target.contents then Manifest.name manifest
...
module Show = struct
  ...
  let show_manifest m = printf "%s@." @@ Manifest.to_string m
  ...
  let show_files k m b =
    ...
    in
    let is_library name =
      Filename.check_suffix name ".cmxs" || Filename.check_suffix name ".cma"
    in
    List.iter (Bundle.list b) ~f:(fun name ->
        if choose (is_library name) then printf "%s@." name)
...
module Install = struct
  ...
  let main () =
    ...
    if not (Sys.file_exists_exn !destdir) then
      FileUtil.mkdir ~parent:true !destdir;
    if not (Sys.is_directory_exn !destdir) then raise Destdir_is_not_a_dir;
    if Sys.file_exists_exn !target then
      if Sys.is_directory_exn !target then raise Target_is_directory
...
module Remove = struct
  ...
  let main () =
    ...
    if Sys.file_exists_exn file then FileUtil.rm ~recurse:true [ file ]
...
let usage fmt =
  kfprintf
    (fun ppf ->
      pp_print_newline ppf ();
      pp_print_newline ppf ();
      pp_print_string ppf @@ Arg.usage_string !args usage_msg;
      pp_print_newline ppf ();
      exit 1)
    err_formatter fmt
...
let () =
  ...
  | Target_unspecified -> usage "Please, specify the bundle name"
  | Target_is_directory -> error "The specified bundle is a directory"
  ...
  | Target_not_installed -> error "The specified plugin is not installed"
  | Sys_error err -> error "%s" err
  | exn -> error "Unexpected error: %s\nPlease report" @@ Exn.to_string exn
