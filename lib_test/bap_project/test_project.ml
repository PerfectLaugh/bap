...
let tag =
  Value.Tag.register
    (module String)
    ~name:"test" ~uuid:"6b3dab52-f3f5-493d-9db7-ad3eed33add8"
...
let test_substitute case =
  ...
  let symbolizer =
    Symbolizer.create @@ fun addr -> Option.some_if Addr.(base = addr) sub_name
  in
  let agent =
    let name = sprintf "test-project-symbolizer-for-%s" case.name in
    KB.Agent.register name
  in
  ...
  let input =
    let mem =
      Memory.create LittleEndian base (Bigstring.of_string case.code) |> ok_exn
    in
    let code =
      Memmap.add Memmap.empty mem (Value.create Image.section "bap.test")
    in
    let data = Memmap.empty in
    Project.Input.create case.arch file ~code ~data
  in
  ...
  let test expect s =
    ...
    let s =
      Option.value_exn (Project.memory p |> Memmap.find_map ~f:(Value.get tag))
    in
    expect >:: assert_normalized ~expect s
  in
  let has_filename =
    "filename-is-provided" >:: fun ctxt ->
    match Project.get p filename with
    | None -> assert_failure "filename is not set"
    | Some file' -> assert_equal ~ctxt ~printer:Fn.id file file'
  in
  [
    test case.asm "$asm";
    test case.bil "$bil";
...
let suite () =
  "Project"
  >::: [ "ARM" >::: test_substitute arm; "386" >::: test_substitute x86 ]
