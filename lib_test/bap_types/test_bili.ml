...
let assert_exp value exp ctxt = assert_equal ~ctxt ~printer value (Exp.eval exp)
...
let suite () =
  ...
  >::: [
         ...
         >:: assert_exp (r8 word 0xBE)
               Bil.(
                 load ~mem:(store ~mem ~addr data el `r32) ~addr:addr1 be `r8);
         "(mem with [0,el]:u32 <- 0xDEADBEEF)[1,el]:u8 ~> 0xBE"
         >:: assert_exp (r8 word 0xBE)
               Bil.(
                 load ~mem:(store ~mem ~addr data el `r32) ~addr:addr1 el `r8);
         "(mem with [0,el]:u32 <- 0xDEADBEEF)[1,be]:u16 ~> 0xBEAD"
         >:: assert_exp (r16 word 0xBEAD)
               Bil.(
                 load ~mem:(store ~mem ~addr data el `r32) ~addr:addr1 be `r16);
         "(mem with [0,el]:u32 <- 0xDEADBEEF)[1,el]:u16 ~> 0xADBE"
         >:: assert_exp (r16 word 0xADBE)
               Bil.(
                 load ~mem:(store ~mem ~addr data el `r32) ~addr:addr1 el `r16);
         "(3 * 8 + 3)/3 ~> 9"
         >:: assert_exp (r8 word 9)
         ...
         \     delta[result] |-> 5"
         >:: assert_prg (r32 word 5l)
               Bil.[ a := r32 int 2l; b := r32 int 3l; r := var a + var b ];
         "\n\
         \     a := 3; b := 0; while (a > 0) { a := a - 1; b := b + 1}; result \
          := b\n\
         \     ------------------------------------------------------------\n\
         \     delta[result] |-> 3 "
         >:: assert_prg (r32 word 3l)
               Bil.
                 [
                   a := r32 int 3l;
                   b := r32 int 0l;
                   while_
                     (var a > r32 int 0l)
                     [ a := var a - r32 int 1l; b := var b + r32 int 1l ];
                   r := var b;
                 ];
         "collatz"
         >::
         (* Starting with 17, we take 12 steps in the `3x+1` problem (OEIS). *)
         assert_prg (r32 word 12l)
           Bil.
             [
               a := r32 int 17l;
               r := r32 int 0l;
               while_
                 ...
                 [
                   if_
                     (r32 int 0l = var a mod r32 int 2l)
                     [ a := var a / r32 int 2l ]
