...
module type Graph_for_algo =
  Graph with type Node.label = int and type Edge.label = unit
...
module Test_algo (Gl : Graph_for_algo) = struct
  ...
  type node_info = { enter : int; leave : int; rpost : int; pre : int }

  let empty = { enter = -1; leave = -1; rpost = -1; pre = -1 }
  ...
  let timestamps ty gr : int * ('n, node_info, _) Map.t =
    Graphlib.depth_first_search ty gr ~init:(0, Node.Map.empty)
      ~enter_node:(fun pre node (time, stamps) ->
        ( time + 1,
          Map.change stamps node ~f:(function
  ...
  module Span = struct
    type t = { children : Node.Set.t Node.Map.t; iparents : Node.t Node.Map.t }

    let empty = { children = Node.Map.empty; iparents = Node.Map.empty }

    let add_relation t ~parent ~child =
      {
        children =
          Map.change t.children parent ~f:(function
    ...
    let ancestors t child =
      let rec loop parents child =
        match parent t child with
        | None -> parents
        | Some parent -> loop (Set.add parents parent) parent
      in
  ...
  let build_tree g =
    Graphlib.depth_first_search
      (module Gl)
      g ~init:Span.empty
      ~enter_edge:(fun kind edge tree ->
        match kind with
        | `Tree ->
            Span.add_relation tree ~parent:(Edge.src edge)
              ~child:(Edge.dst edge)
        | _ -> tree)
  ...
  *)
  ...
  let theorem_1_5 stamps graph ctxt : bool =
    Graphlib.depth_first_search
      ...
      ~enter_edge:(fun kind e -> function
        ...
        | true -> (
            let x =
              match Map.find stamps (Gl.Edge.src e) with
              | None -> assert_failure "edge with unstamped src"
              | Some x -> x
            in
            let y =
              match Map.find stamps (Gl.Edge.dst e) with
  ...
  let random_flowgraph size =
    let g =
      Seq.range 0 size
      |> Seq.fold ~init:Gl.empty ~f:(fun g i -> Gl.Node.insert (node i) g)
    in
    let g =
      Seq.range 0 (size - 1)
      |> Seq.fold ~init:g ~f:(fun g i -> Gl.Edge.insert (edge i (i + 1)) g)
    in
    Seq.range 0 size
  ...
  let compare_dom_frontier gr ctxt =
    ...
    let dfrt = Graphlib.dom_frontier (module Gl) gr idom in
    let exp_idom = Dom.compute_idom gr (node 0) in
    let dom_tree = Dom.idom_to_dom_tree gr exp_idom in
    let exp_dfrt = Dom.compute_dom_frontier gr dom_tree exp_idom in
    let set_of_frontier n = Frontier.enum dfrt n |> Seq.to_list_rev in
  ...
  let equal_sets s1 s2 ctxt = assert_equal ~ctxt ~cmp:Set.equal s1 s2
...
module Construction (Factory : Factory) = struct
  ...
  let make_edges (type g)
    ...
    G.edges g
    |> Seq.map ~f:(fun e -> (G.Edge.src e, G.Edge.dst e, G.Edge.src e))
  ...
  let compare e g : int =
    ...
    | 0 -> 0
    | n ->
        let set enum g = enum g |> Seq.fold ~init:Nodes.empty ~f:Set.add in
        Nodes.compare (set E.nodes e) (set G.nodes g)

  let random_elt number enum e =
    let n = number e in
    if n > 0 then Seq.nth (enum e) (Random.int n) else None
  ...
  let insert_node (e, g) : graphs =
    ...
    (E.Node.insert x e, G.Node.insert x g)
  ...
  let constructive_scheme : (graphs -> graphs) list =
    List.concat [ constructives; constructives; destructives ]

  let validate (e, g) ctxt =
    if compare e g <> 0 then eprintf "expected:\n%a\ngot:\n%a\n%!" E.pp e G.pp g;
  ...
  let suite : test = "constructive" >:: run constructive_scheme 1000
end

module ODIU = Graphlib.Of_ocamlgraph (Graph.Persistent.Digraph.Concrete (Int))

module OBIU =
  Graphlib.Of_ocamlgraph (Graph.Persistent.Digraph.ConcreteBidirectional (Int))
...
module Test_IR = struct
  ...
  let goto dst src =
    Term.append jmp_t src @@ Jmp.create_goto (Label.direct (Term.tid dst))

  let entry = entry |> goto b1
  let b1 = b1 |> def k _F |> def i _1 |> def j _2 |> goto b2
  ...
  let b5 =
    let call =
      Call.create ()
        ...
        ~target:(Label.indirect (Bil.var j))
    in
  ...
  let insert blks =
    let g =
      List.fold blks ~init:G.empty ~f:(fun g blk ->
          G.Node.insert (G.Node.create blk) g)
    in
    let etalon =
      G.edges g
      |> Seq.fold ~init:etalon ~f:(fun etalon e -> G.Edge.remove e etalon)
    in
    [
      ( "all edges inserted" >:: fun ctxt ->
        assert_equal ~ctxt ~cmp:Int.equal ~printer:Int.to_string 0
          (G.number_of_edges etalon) );
  ...
  let hasn't = not
  let nil = G.empty
  let edges n g ctxt = assert_bool "failed" (G.number_of_edges g = n)
  let nodes n g ctxt = assert_bool "failed" (G.number_of_nodes g = n)

  let edge has g x y ctxt =
    assert_bool "failed" (has G.Node.(has_edge (create x) (create y) g))
...
module Test_partition = struct
  ...
  let refine p equiv _ =
    assert_bool "failed"
      (Seq.for_all (P.groups p) ~f:(fun g ->
