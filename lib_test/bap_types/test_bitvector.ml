...
let normalized ~n ~add ~width ctxt =
  assert_equal ~ctxt ~printer:Word.to_string (Word.of_int n ~width)
...
let of_string ~str v ctxt =
  assert_equal ~ctxt ~printer:Word.to_string v (Word.of_string str)
...
let to_chars ~expect endian v ctxt =
  assert_equal ~ctxt ~printer:print_int_list expect
    Word.(enum_chars endian v |> Seq.map ~f:Char.to_int |> Sequence.to_list)

let concat ~expect v1 v2 ctxt =
  assert_equal ~ctxt ~printer:Word.to_string expect Word.(v1 @. v2)

let is_zero word ctxt = assert_bool Word.(to_string word) Word.(is_zero b0)
...
let binary op ~width ~expect x y ctxt =
  ...
  assert_equal ~ctxt ~printer:Word.to_string ~cmp:Word.equal !$expect
    (op !$x !$y)
...
let binary3 op ~width ~expect x y ctxt =
  ...
  let equal (a, b, c) (a', b', c') =
    Word.equal a a' && Word.equal b b' && Word.equal c c'
  in
  let to_string (a, b, c) =
    Printf.sprintf "(%s,%s,%s)" (Word.to_string a) (Word.to_string b)
      (Word.to_string c)
  in
  assert_equal ~ctxt ~printer:to_string ~cmp:equal !$$$expect (op !$x !$y)
...
let bitsub ?hi ?lo ~expect v ctxt =
  ...
  assert_equal ~ctxt ~printer:Word.to_string !$expect
...
let suite () =
  ...
  >::: [
         ...
         "to_string:-1|3"
         >:: to_string ~str:"-1:3s" Word.(signed @@ of_int (-1) ~width:3);
         ...
         "chars:0|32le"
         >:: to_chars
               Word.(of_int 0 ~width:32)
               LittleEndian ~expect:[ 0x0; 0x0; 0x0; 0x0 ];
         "chars:0|32be"
         >:: to_chars
               Word.(of_int 0 ~width:32)
               BigEndian ~expect:[ 0x0; 0x0; 0x0; 0x0 ];
         "chars:1|128le"
         >:: to_chars
               Word.(of_int 1 ~width:128)
               LittleEndian
               ~expect:
                 [
                   0x1;
         ...
         "chars:1|32le"
         >:: to_chars
               Word.(of_int 1 ~width:32)
               LittleEndian ~expect:[ 0x1; 0x0; 0x0; 0x0 ];
         "chars:1|32be"
         >:: to_chars
               Word.(of_int 1 ~width:32)
               BigEndian ~expect:[ 0x0; 0x0; 0x0; 0x1 ];
         "chars:1|16le"
         >:: to_chars
               Word.(of_int 1 ~width:16)
               LittleEndian ~expect:[ 0x1; 0x0 ];
         "chars:1|16be"
         >:: to_chars Word.(of_int 1 ~width:16) BigEndian ~expect:[ 0x0; 0x1 ];
         "chars:1|8le"
         >:: to_chars Word.(of_int 1 ~width:8) LittleEndian ~expect:[ 0x1 ];
         "chars:1|8be"
         >:: to_chars Word.(of_int 1 ~width:8) BigEndian ~expect:[ 0x1 ];
         "chars:1|4le"
         >:: to_chars Word.(of_int 1 ~width:4) LittleEndian ~expect:[ 0x1 ];
         "chars:1|4be"
         >:: to_chars Word.(of_int 1 ~width:4) BigEndian ~expect:[ 0x1 ];
         "chars:1|1le"
         >:: to_chars Word.(of_int 1 ~width:1) LittleEndian ~expect:[ 0x1 ];
         "chars:1|1be"
         >:: to_chars Word.(of_int 1 ~width:1) BigEndian ~expect:[ 0x1 ];
         "chars:1|7le"
         >:: to_chars Word.(of_int 1 ~width:7) LittleEndian ~expect:[ 0x1 ];
         "chars:1|7be"
         >:: to_chars Word.(of_int 1 ~width:7) BigEndian ~expect:[ 0x1 ];
         "chars:1|13le"
         >:: to_chars
               Word.(of_int 1 ~width:13)
               LittleEndian ~expect:[ 0x1; 0x0 ];
         "chars:1|13be"
         >:: to_chars Word.(of_int 1 ~width:13) BigEndian ~expect:[ 0x0; 0x1 ];
         "chars:1|71le"
         >:: to_chars
               Word.(of_int 1 ~width:71)
               LittleEndian
         ...
         "chars:DEADBEEF|32"
         >:: to_chars deadbeef LittleEndian ~expect:[ 0xEF; 0xBE; 0xAD; 0xDE ];
         "chars:DEADBEEF|32"
         >:: to_chars deadbeef BigEndian ~expect:[ 0xDE; 0xAD; 0xBE; 0xEF ];
         "of_binary:be"
         >:: of_binary BigEndian "\xDE\xAD\xBE\xEF" ~expect:deadbeef;
         "of_binary:le"
         >:: of_binary LittleEndian "\xEF\xBE\xAD\xDE" ~expect:deadbeef;
         "concat:DEADBEEF|16+16"
         >:: concat ~expect:deadbeef
               Word.(of_int 0xDEAD ~width:16)
               Word.(of_int 0xBEEF ~width:16);
         "concat:DEADBEEF|8+24"
         >:: concat ~expect:deadbeef
               Word.(of_int 0xDE ~width:8)
               Word.(of_int 0xADBEEF ~width:24);
         "concat:DEADBEEF|4+28"
         >:: concat ~expect:deadbeef
               Word.(of_int 0x0D ~width:4)
               Word.(of_int 0x0EADBEEF ~width:28);
         "concat:D7|3+5"
         >:: concat
               ~expect:Word.(of_int 0xD7 ~width:8)
         ...
         "with_zero:255+1"
         >:: is_zero Word.(Int_exn.(of_int 255 ~width:8 + of_int 1 ~width:8));
         "validate_positive:1" >:: validate Word.validate_positive Word.b1;
         "validate_non_negative:1"
         >:: validate Word.validate_non_negative Word.b1;
         "validate_non_negative:0"
         >:: validate Word.validate_non_negative Word.b0;
         "validate_non_positive:0"
         >:: validate Word.validate_non_positive Word.b0;
         "validate_positive:-1u"
         >::
         ...
         "validate_negative:1s"
         >::
         (* yes, in 2-complement there is no +1 *)
         validate Word.validate_negative Word.(signed Int_exn.(b1));
         "validate_positive:deadbeef"
         >:: validate Word.validate_positive deadbeef;
         "validate_positive:-deadbeef"
         >:: validate Word.validate_positive Word.Int_exn.(~-deadbeef);
         "validate_positive:-deadbeefs"
         >::
         (*  not enough bits! *)
         ...
         "is_positive:-deadbeefs"
         >:: is Word.(signed Int_exn.(~-deadbeef) > zero_32);
         "is_positive:DEADBEEF" >:: is Word.(_DEADBEEF > zero_64);
         "is_positive:-DEADBEEF" >:: is Word.(Int_exn.(~-_DEADBEEF) > zero_64);
         "is_negative:-DEADBEEFs"
         >:: is Word.(signed Int_exn.(~-_DEADBEEF) < zero_64);
