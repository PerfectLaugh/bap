...
let full_pipeline bil =
  Bil.fixpoint
    (fun b ->
      Bil.fold_consts b |> Bil.propagate_consts |> Bil.prune_dead_virtuals)
    bil
...
let eval bil bil' =
  let init =
    Hash_set.fold all_vars ~init:[] ~f:(fun bil v ->
        Bil.(v := random ()) :: bil)
  in
      ...
      if Var.is_physical v then
        let res =
          ...
          | Some r1, Some r2 -> (
              ...
              | Bil.Bot, Bil.Bot | Bil.Mem _, Bil.Mem _ -> true
              | _ -> false)
        in
...
let check f bil expected (_ : test_ctxt) =
  ...
  eval bil bil';
  assert_bool
    (error_of_bil bil bil' (Some expected))
...
let kill_virtual =
  ...
  ==> Bil.[ always_alive := zero; always_alive := one ]
...
let trivial_dead_1 =
  Bil.[ always_alive := one; could_be_dead := two ]
  ==> Bil.[ always_alive := one ]

let trivial_dead_2 =
  Bil.[ could_be_dead := one; always_alive := two ]
...
let trivial_dead_3 =
  ...
  ==> Bil.[ will_survive := one; always_alive := var will_survive + two ]
...
let assigned_twice =
  ...
  ==> Bil.[ could_be_dead := one; always_alive := var could_be_dead + two ]

let survive_anyway_1 =
  let bil = Bil.[ will_survive := one; if_ (var will_survive > zero) [] [] ] in
  bil ==> bil
          ...
          [
            if_
              ...
              [
                while_
                  (var z > var y)
                  [ if_ (var z > four) [] [ will_survive := one ] ];
              ];
          ]
          [];
      ]
...
let preserves_special =
  Bil.[ could_be_dead := one; special "I will survive!" ]
  ==> Bil.[ special "I will survive!" ]

let preserves_cpuexn =
  Bil.[ could_be_dead := one; cpuexn 42 ] ==> Bil.[ cpuexn 42 ]
...
let preserves_loop_1 =
  let bil =
    Bil.
      [
        will_survive := one;
        while_ (var will_survive < four) [ will_survive := five ];
      ]
  in
  bil ==> bil
...
let preserves_loop_2 =
  Bil.
    [
      will_survive := two;
      while_
        (var will_survive < four)
        [ could_be_dead := one; will_survive := five ];
      could_be_dead := two;
    ]
  ==> Bil.
        [
  ...
  let bil =
    Bil.
      [
        v1 := zero;
        v := four;
        while_ (var v > zero) [ y := var v1 + one; v1 := one; v := var v - one ];
      ]
  in
  bil ==> bil
    ...
    Bil.
      [
        ...
        if_
          ...
          [
            while_
              (var will_survive = two)
              [
                if_
                  (var always_alive > zero)
                  [ will_survive := one ]
                  [ could_be_dead := x42 ];
              ];
          ]
...
let with_jmp =
  let bil = Bil.[ will_survive := x42; jmp (var will_survive) ] in
  bil ==> bil
  ...
  let bil =
    Bil.
      [
        always_alive := one;
        will_survive := x42;
        if_ (var always_alive > zero) [ jmp (var will_survive) ] [];
      ]
  in
  bil ==> bil
...
let trivial_propagate_1 =
  Bil.[ v := x42; x := var v + one ] $==> Bil.[ v := x42; x := x42 + one ]

let trivial_propagate_2 =
  Bil.[ x := x42; y := var x + one ] $==> Bil.[ x := x42; y := x42 + one ]

let trivial_propagate_3 =
  Bil.[ x := x42; jmp (var x) ] $==> Bil.[ x := x42; jmp x42 ]

let trivial_propagate_4 =
  Bil.[ x := x42; jmp (var y); z := var x ]
  $==> Bil.[ x := x42; jmp (var y); z := x42 ]

let trivial_propagate_5 =
  Bil.[ x := undefined; z := var x ] $==> Bil.[ x := undefined; z := undefined ]

let trivial_propagate_6 =
  Bil.
    [
      x := one;
      if_ (var x < zero) [ y := undefined ] [ y := undefined ];
      z := var y;
    ]
  $==> Bil.
         [
           x := one;
           if_ (one < zero) [ y := undefined ] [ y := undefined ];
           z := undefined;
         ]

let merging_cond_branches_1 =
  Bil.[ x := x42; if_ (var x > zero) [ y := one ] [ y := two ]; z := var y ]
  $==> Bil.[ x := x42; if_ (x42 > zero) [ y := one ] [ y := two ]; z := var y ]

let merging_cond_branches_2 =
  Bil.[ x := x42; if_ (var x > zero) [ y := one ] [ y := one ]; z := var y ]
  $==> Bil.[ x := x42; if_ (x42 > zero) [ y := one ] [ y := one ]; z := one ]

let dont_merge_cond_branches_1 =
  Bil.
    [
      x := one;
      if_ (var x < zero) [ y := two; jmp (var y) ] [ y := four ];
      z := var y;
    ]
  $==> Bil.
         [
           x := one;
           if_ (one < zero) [ y := two; jmp two ] [ y := four ];
           z := four;
         ]

let dont_merge_cond_branches_2 =
  Bil.
    [
      x := one;
      if_ (var x > zero) [ y := two ] [ y := four; jmp (var y) ];
      z := var y;
    ]
  $==> Bil.
         [
           x := one;
           if_ (one > zero) [ y := two ] [ y := four; jmp four ];
           z := two;
         ]

let dont_merge_cond_branches_3 =
  Bil.
    [
      x := one;
      y := two;
      while_ (var x < zero) [ y := one; jmp (var y) ];
      z := var y;
    ]
  $==> Bil.
         [
           x := one;
           y := two;
           while_ (one < zero) [ y := one; jmp one ];
           z := two;
         ]

let merging_cond_branches_3 =
  Bil.
    [
      x := one;
      y := two;
      if_ (var x > zero) [ y := one; if_ (var y > zero) [ jmp (var y) ] [] ] [];
      z := var y;
    ]
  $==> Bil.
         [
           x := one;
           y := two;
           if_ (one > zero) [ y := one; if_ (one > zero) [ jmp one ] [] ] [];
           z := var y;
         ]

let merging_cond_branches_4 =
  Bil.
    [
      x := one;
      y := two;
      while_ (var x < zero) [ y := one; if_ (var y < zero) [ jmp (var y) ] [] ];
      z := var y;
    ]
  $==> Bil.
         [
           x := one;
           y := two;
           while_ (one < zero) [ y := one; if_ (one < zero) [ jmp one ] [] ];
           z := var y;
         ]

let propagate_in_while_1 =
  let bil =
    Bil.[ x := four; while_ (var x < five) [ y := var x + one; x := x42 ] ]
  in
  bil $==> bil
...
let propagate_in_while_2 =
  let bil =
    Bil.
      [
        x := x42;
        while_ (var x > two) [ y := var x + one; x := one; z := var x + two ];
      ]
  in
  bil
  $==> Bil.
         [
           x := x42;
           while_ (var x > two) [ y := var x + one; x := one; z := one + two ];
         ]
    ...
    Bil.
      [
        ...
        while_
          (var z > four)
          [
            if_ (var z > one) [ y := one ] [ y := one ];
            if_
              (var x + var y > one)
              [ (* the only that could be computed  *) y := zero ]
              [ y := two ];
            z := var y;
          ];
      ]
...
let all_together_trivial =
  Bil.
    [
      v := one; v := var v + one; v := var v + one; v := var v + one; x := var v;
    ]
  => Bil.[ x := four ]

let vars_not_dead =
  Bil.[ x := two; y := var x + two ] => Bil.[ x := two; y := four ]

let with_computable_cond =
  Bil.
    [
      x := one;
      v := var x + one;
      y := var v * two;
      if_ (var y < two) [ z := one ] [ z := two ];
      y := var z;
    ]
  => Bil.[ x := one; y := four; z := two; y := two ]

let dead_unused_with_cond =
  Bil.
    [
      x := one;
      v := one;
...
let with_various_input input expected _ctxt =
  let bil =
    Bil.
      [
        ...
        if_
          (var x > two)
          [ while_ (var x > zero) [ y := var x * two; x := var x - one ] ]
          [
            if_
              (var x = zero)
              [ y := one ]
              [ if_ (var x = one) [ y := two ] [ y := four ] ];
          ];
        z := var y;
      ]
  in
  ...
  let z_value =
    Option.value_exn
      ((object
          inherit [exp option] Stmt.visitor

          method! enter_move var e x =
            if String.equal (Var.name var) "z" then Some e else x
       end)
         #run
         bil' None)
  in
  let error =
    sprintf "error with various input %s\n" (Exp.to_string expected)
  in
...
let all_together =
  Bil.
    [
      ...
      if_
        (var v1 > zero)
        [ v2 := var v1 + one; x := var v2 + one ]
        [ v2 := var v1 + two; x := var v2 ];
      v2 := four;
      y := var x - var v2;
      if_ (var y < zero) [ z := four; jmp (var z + two - var y) ] [ z := two ];
      x := var z;
    ]
  => Bil.[ x := eight; y := four; z := two; x := two ]
