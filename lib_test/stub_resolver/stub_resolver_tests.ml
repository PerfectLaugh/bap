(** In order to test stub resolver we generate a dummy programs like in the
    example below:

    0000000b: program 00000008: sub a() 00000007:

    0000000a: sub b() 00000009:

    ...

    and provide a knowledge about each symbol: if it's a stub and if it has
    aliases. *)
...
type sym = { is_stub : bool; aliases : string list }

let run dis mem =
  Or_error.ok_exn
  @@ Dis.run dis mem ~init:[] ~return:Result.return ~stop_on:[ `Valid ]
...
let block_of_bytes addr b =
  ...
  let mem = Or_error.ok_exn @@ Memory.create LittleEndian addr code in
  let dis =
    Or_error.ok_exn @@ Dis.create ~backend:"llvm" (Arch.to_string `x86_64)
  in
...
let add_symbol symtab name addr bytes =
  ...
  Symtab.add_symbol symtab (name, block, Cfg.Node.insert block Cfg.empty)

let collect_stubs syms =
  Map.fold syms
    ~init:(Set.empty (module String))
    ~f:(fun ~key:name ~data:{ is_stub } stubs ->
      if is_stub then Set.add stubs name else stubs)

let tag_stubs syms prog =
  let stubs = collect_stubs syms in
  let make_addr sub =
    Addr.of_int64 @@ Int63.to_int64 @@ KB.Object.id (Term.tid sub)
  in
  Term.map sub_t prog ~f:(fun sub ->
      if Set.mem stubs (Sub.name sub) then Term.set_attr sub Sub.stub ()
      else
...
let create_program syms =
  ...
  let rec loop symtab addr = function
    ...
    | name :: names ->
        ...
        loop symtab Addr.(addr + step) names
  in
...
let run ?(skip = false) name symbols expected should_fail _ctxt =
  let syms =
    List.fold symbols
      ...
      ~f:(fun syms (name, data) -> Map.add_exn syms ~key:name ~data)
  in
    ...
    List.fold expected
      ...
      ~f:(fun tids (stub, impl) ->
        Map.add_exn tids
          ...
          ~data:(tid_for_name_exn prog impl))
  in
  ...
  let msg =
    "the mappings shall " ^ if should_fail then "differ" else "be the same"
  in
  OUnit.skip_if skip "To be fixed";
  assert_equal expected pairs ~cmp:equal ~msg ~printer:string_of_tids
...
let suite =
  ...
  >::: [
         ...
         test "simple case: still no pairs"
           [ real "c0" []; stub "c1" [ "c2" ] ]
           ~expected:[];
         test "stubs only" [ stub "d0" []; stub "d1" [ "d0" ] ] ~expected:[];
         test "impl only" [ real "e0" []; real "e1" [ "e0" ] ] ~expected:[];
         test "impl can be aliased as well"
           [ real "f0" [ "f1" ]; stub "f1" [] ]
           ~expected:[ ("f1", "f0") ];
         test "many aliases"
           [ real "g0" [ "g1"; "g2" ]; stub "g1" [] ]
           ~expected:[ ("g1", "g0") ];
         test "ambiguous impl"
           [ real "h0" [ "h1"; "h2" ]; stub "h1" []; stub "h2" [] ]
           ~expected:[ ("h1", "h0"); ("h2", "h0") ];
         test "ambiguous stubs"
           [ real "i0" []; stub "i1" [ "i0" ]; stub "i2" [ "i0" ] ]
           ~expected:[ ("i1", "i0"); ("i2", "i0") ];
         test "crossreference"
           [ real "j0" [ "j1" ]; stub "j1" [ "j0" ] ]
           ~expected:[ ("j1", "j0") ];
         test "many pairs"
           [
             real "k0" [];
