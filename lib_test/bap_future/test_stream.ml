...
let unfold_until ctxt =
  let f x = if x > 7 then None else Some (x, x + 1) in
...
let watch ctxt =
  ...
  let buf = Queue.create () in
  let once id x =
    if x > 3 then Stream.unsubscribe ss id else Queue.enqueue buf x
  in
...
let has_subscribers ctxt =
  let assert_no_subscribers ss =
    assert_false "no subscribers" (Stream.has_subscribers ss)
  in
  let assert_exists_subscribers ss =
    assert_bool "exists subscribers" (Stream.has_subscribers ss)
  in
  let ss, _, _ = Stream.of_list values in
  let f x = () in
...
let parse ctxt =
  ...
  let f state x = if is_even x then (Some x, x + 1) else (None, x + 1) in
...
let hd values ctxt =
  ...
  | _ -> assert_equal ~ctxt (List.hd_exn values) (Future.peek_exn f)
...
let base_find values ~find f expect ctxt =
  ...
  match expect with
  | None ->
      assert_false "find: future mustn't be decided" (Future.is_decided future)
  | Some v -> assert_equal ~ctxt v (Future.peek_exn future)
...
let nth values ~expect ~n ctxt =
  ...
  match expect with
  | None -> assert_false "nth: future mustn't be decided" (Future.is_decided f)
  | Some v -> assert_equal ~ctxt v (Future.peek_exn f)
...
let upon values ~expect ~after ctxt =
  ...
  match expect with
  | None -> assert_false "upon: future mustn't be decided" (Future.is_decided f)
  | Some v -> assert_equal ~ctxt v (Future.peek_exn f)
...
let suite =
  ...
  >::: [
         ...
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[ 1 ] ~stride:0 ~width:0 ~values:[] ~times:10 ~expect:[];
         "foldw: stride = 0; width = 0; size > 0"
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[ 1 ] ~stride:0 ~width:0 ~values:[ 0; 1; 2 ] ~times:4
               ~expect:[ [ 1 ]; [ 1 ]; [ 1 ] ];
         "foldw: stride = 0; width > 0; size = 0"
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[ 1 ] ~stride:0 ~width:3 ~values:[] ~times:10 ~expect:[];
         "foldw: stride = 0; 0 < width < size; size > 0"
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[] ~stride:0 ~width:2 ~values ~times:10
               ~expect:
                 [
                   [ 0; 1 ];
                   [ 0; 1 ];
         ...
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[] ~stride:0 ~width:(List.length values) ~values
               ~times:(List.length values + 1)
               ~expect:[ values ];
         "foldw: stride = 0; width > size ; size > 0"
         >:: foldw
         ...
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[ 1 ] ~stride:3 ~width:0 ~values:[] ~times:10 ~expect:[];
         "foldw: stride > 0; width > 0; size = 0"
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[ 1 ] ~stride:3 ~width:3 ~values:[] ~times:10 ~expect:[];
         "foldw: 0 < stride < size; width = size ; size > 0"
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[] ~stride:2 ~width:(List.length values) ~values
               ~times:(List.length values + 1)
               ~expect:[ values ];
         "foldw: 0 < stride < size; 0 < width < size; size > 0"
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[] ~stride:3 ~width:3 ~values
               ~times:(List.length values + 1)
               ~expect:[ [ 0; 1; 2 ]; [ 3; 4; 5 ] ];
         "foldw: 0 < stride < size; width = 0; size > 0"
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[ 1 ] ~stride:3 ~width:0 ~values
               ~times:(List.length values + 1)
               ~expect:[ [ 1 ]; [ 1 ]; [ 1 ] ];
         ...
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[] ~stride:2 ~width:4 ~values
               ~times:(List.length values + 1)
               ~expect:[ [ 0; 1; 2; 3 ]; [ 2; 3; 4; 5 ]; [ 4; 5; 6; 7 ] ];
         "foldw: stride = size; width > size ; size > 0"
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[] ~stride:(List.length values)
               ~width:(List.length values + 1)
               ~values
               ~times:(List.length values + 1)
               ~expect:[];
         ...
         >:: foldw
               ~f:(fun xs x -> xs @ [ x ])
               ~init:[] ~stride:(List.length values) ~width:(List.length values)
               ~values
               ~times:(List.length values + 1)
               ~expect:[ values ];
         ...
         >:: foldw_range
               ~f:(fun xs x -> x :: xs)
               ~init:[] ~max_stride:10 ~max_width:10;
         "frame\n      clk:  T T    T\n     data: 0    12  34567 "
         ...
         "sample"
         >:: sample values ~expect:[ Some 0; None; Some 1 ]
               ~time_line:[ 1; 1; 3 ];
         "sample"
         >:: sample values
               ~expect:[ None; None; Some 0; Some 1 ]
               ~time_line:[ 0; 0; 1; 2 ];
         "find_map"
         >:: find_map values
               ~f:(fun x -> if x = 3 then Some (Int.succ x) else None)
               ~expect:(Some 4);
         "find_map"
         >:: find_map values
               ~f:(fun x -> if x = 10 then Some (Int.succ x) else None)
               ~expect:None;
