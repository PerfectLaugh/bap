...
let abc =
  ...
  Term.append jmp_t abc (Jmp.create_ret (Label.indirect Bil.(var lr)))
...
let program =
  Term.append sub_t (Term.append sub_t (Program.create ()) sub1) sub2

let equal_var ctxt x y =
  assert_equal ~ctxt ~cmp:Var.equal ~printer:Var.to_string x y
...
let seq f ?rev def = assert_vars (fun t blk -> Term.(f t ?rev blk (tid def)))
...
let triplicate =
  transform Term.concat_map (fun v d ->
...
let remove_with_none =
  transform Term.filter_map (fun v d ->
      if Var.(Def.lhs d = v) then None else Some d)

let rename_to_s =
  transform Term.map (fun v d -> if Var.(Def.lhs d = v) then def_s else d)
...
module Example = struct
  ...
  let b3_with_jump_to_b5 =
    Term.map jmp_t ~f:(fun _ -> Jmp.create_goto (Label.direct (Term.tid b5))) b3
  ...
  let phi_node sub var var_ver blk blk_ver _ =
    ...
    |> Seq.find_map ~f:(fun blk ->
        ...
        |> Seq.find ~f:(fun phi ->
            ...
            Var.same v var && Var.index v = var_ver))
    |> function
    | None -> assert_string "no such phi-node"
    | Some phi -> (
        Phi.values phi
        |> Seq.find ~f:(fun (tid, exp) ->
            Tid.equal (Term.tid blk) tid
            &&
            match exp with
  ...
  let live_vars_ssa vars blk _ctxt =
    ...
    List.iter vars ~f:(fun (v, i) ->
        ...
        let msg = Format.asprintf "%a must be live at %a" Var.pp v Tid.pp b in
  ...
  let tests = [ check_free_vars; check_live_vars; check_phi_nodes ]
end
...
let suite () =
  ...
  >::: [
         ...
         "split_before(xyOz)"
         >:: split (before_def def_o) [ x; y ] [ o; z ] xyoz;
         "split_after(xyoZ)" >:: split (after_def def_z) [ x; y; o; z ] [] xyoz;
         "split_before(xyoZ)"
         >:: split (before_def def_z) [ x; y; o ] [ z ] xyoz;
         "split_after(Xyoz)" >:: split (after_def def_x) [ x ] [ y; o; z ] xyoz;
         "split_before(Xyoz)"
         >:: split (before_def def_x) [] [ x; y; o; z ] xyoz;
