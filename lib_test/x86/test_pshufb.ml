(** # rappel (don't forget -x option) ## mov to xmm0 register a 128-bit value
    "50 4F 4E 4D 4C 4B 4A 49 48 47 46 45 44 43 42 41"

    mov rax, 0x4847464544434241 mov rbx, 0x504F4E4D4C4B4A49 pinsrq xmm0, rax, 0
    pinsrq xmm0, rbx, 1

    So, xmm0 will be used for original value, xmm1 - for mask, and xmm2 for our
    tests.

    Cases: 1) mov to xmm1 register a 128 bit value with permutation indexes,
    just bytes numbers as they are, i.e "0F 0E 0D 0C 0B 0A 09 08 07 06 05 04 03
    02 01 00" expected: xmm2 value should be the same as before

    mov rbx, 0x0706050403020100 mov rcx, 0x0f0e0d0c0b0a0908 pinsrq xmm1, rbx, 0
    pinsrq xmm1, rcx, 1 pshufb xmm2, xmm1
    ...
    movaps xmm2, xmm0 mov rbx, 0x0706050403010101 mov rcx, 0x0f0e0d0c0b0a0908
    pinsrq xmm1, rbx, 0 pinsrq xmm1, rcx, 1 pshufb xmm2, xmm1
    ...
    movaps xmm2, xmm0 mov rbx, 0x0706050403020100 mov rcx, 0x0101010c0b0a0908
    pinsrq xmm1, rbx, 0 pinsrq xmm1, rcx, 1 pshufb xmm2, xmm1

    4) permutation mask = "0F 0E 0D 0C 0B 0A 09 08 07 06 05 04 03 80 80 80",
    i.e. last three bytes of destination shold be eqaul to zero. expected: xmm2
    = "50 4F 4E 4D 4C 4B 4A 49 48 47 46 45 44 00 00 00"

    movaps xmm2, xmm0 mov rbx, 0x0706050403808080 mov rcx, 0x0f0e0d0c0b0a0908
    pinsrq xmm1, rbx, 0 pinsrq xmm1, rcx, 1 pshufb xmm2, xmm1 *)
...
let test_rr (mask, expected) ctxt =
  ...
  let bil =
    Bil.[ move xmm0 (int origin); move xmm1 (int mask) ] @ insn_bil pshufb_rr
  in
...
(** tests that memory alignment check is performed, i.e. cpuexn could be raise
  ...
  let bil =
    Bil.
      [
        move rax (int addr);
        move mem
          (store ~mem:(var mem) ~addr:(int addr) (int ident_mask) LittleEndian
             `r256);
        move xmm0 (int origin);
      ]
    @ insn_bil pshufb_rm
  in
  assert_bool "pshufb: memory alignment" @@ (new exn_visitor)#run bil false
