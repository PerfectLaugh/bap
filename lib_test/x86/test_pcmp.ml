(** # rappel (don't forget -x option)
    ...
    move to xmm0 register a 128-bit value "0f 0e 0d 0c 0b 0a 09 08 07 06 05 04
    03 02 01 00" move to xmm1 register a 128-bit value "0f 00 00 00 00 00 00 00
    00 00 00 00 00 00 00 00"

    mov rax, 0x0706050403020100 mov rbx, 0x0f0e0d0c0b0a0908 pinsrq xmm0, rax, 0
    pinsrq xmm0, rbx, 1

    mov rax, 0x0000000000000000 mov rbx, 0x0f00000000000000 pinsrq xmm1, rax, 0
    pinsrq xmm1, rbx, 1

    Cases: 1) pcmpeqb xmm1, xmm0 expected: first and last bytes in xmm1 should
    be 0xFF

    2) pcmpgtb xmm1, xmm0 expected: first and last bytes in xmm1 should be 0x00,
    all others bytes should be 0xFF

    ## pminsb, pminub, pmaxsb, pmaxub, pminsw ... instructions

    move to xmm0 register a 128-bit value "0f 0e 0d 0c 0b 0a 09 08 07 06 05 04
    03 02 01 00" move to xmm1 register a 128-bit value "ff 00 00 00 00 00 00 00
    00 00 00 00 00 00 00 ff"

    mov rax, 0x0706050403020100 mov rbx, 0x0f0e0d0c0b0a0908 pinsrq xmm0, rax, 0
    pinsrq xmm0, rbx, 1

    mov rax, 0x00000000000000ff mov rbx, 0xff00000000000000 pinsrq xmm1, rax, 0
    pinsrq xmm1, rbx, 1

    Cases: 1) pminub xmm1, xmm0 expected: first byte in xmm1 should be 0x0f 2)
    pminsb xmm1, xmm0 expected: xmm1 stay unchanged 3) pmaxub xmm1, xmm0
    expected: all bytes in xmm1 should be equal to corresponded bytes in xmm0,
    except first and last bytes, which stay unchanchged 4) pmaxsb xmm1, xmm0
    expected: xmm1 = xmm0 *)
...
let insn_bil x =
  ...
  let mem =
    Or_error.ok_exn @@ Memory.create LittleEndian (Word.zero 32) bytes
  in
  let mem, insn =
    Or_error.ok_exn
...
let test insn_name bytes x y ~expected _ctxt =
  ...
  let bil = Bil.[ move xmm0 (int x); move xmm1 (int y) ] @ insn_bil bytes in
  ...
  | Some r -> (
      ...
      | Bil.Imm word ->
          if not (Word.equal word expected) then
            printf "%s: got %s, expected %s\n" insn_name (Word.to_string word)
              (Word.to_string expected);
          Word.equal word expected
      | _ -> false)

let test_eqb =
  let pcmpeqb = "\x66\x0f\x74\xc8" in
  (* pcmpeqb %xmm1, %xmm0 *)
...
let test_gtb =
  let pcmpgtb = "\x66\x0f\x64\xc8" in
  (* pcmpgtb %xmm1, %xmm0 *)
...
let test_pminuw =
  let bytes = "\x66\x0f\x38\x3a\xc8" in
  (* pminuw %xmm0, %xmm1  *)
  let x = of_bytes "8f 0e 00 00 0b 0a 09 08 05 06 05 04 03 04 01 00" in
  let y = of_bytes "0f 00 0d 0c 00 00 00 00 07 06 05 05 03 02 00 00" in
...
let test_pminud =
  let bytes = "\x66\x0f\x38\x3b\xc8" in
  (* pminud %xmm0, %xmm1  *)
  let x = of_bytes "8f 0e 00 00 0b 0a 09 08 05 06 05 04 03 04 01 00" in
  let y = of_bytes "0f 00 0d 0c 00 00 00 00 07 06 05 05 03 02 00 00" in
...
let test_pminsd =
  let bytes = "\x66\x0f\x38\x39\xc8" in
  (* pminsd %xmm0, %xmm1  *)
  let x = of_bytes "8f 0e 00 00 0b 0a 09 08 05 06 05 04 03 04 01 00" in
  let y = of_bytes "0f 00 0d 0c 00 00 00 00 07 06 05 05 03 02 00 00" in
  let z = of_bytes "8f 0e 00 00 00 00 00 00 05 06 05 04 03 02 00 00" in
...
let test_pmaxud =
  let bytes = "\x66\x0f\x38\x3f\xc8" in
  (* pmaxud %xmm0, %xmm1  *)
  let x = of_bytes "8f 0e 00 00 0b 0a 09 08 05 06 05 04 03 04 01 00" in
  let y = of_bytes "0f 00 0d 0c 00 00 00 00 07 06 05 05 03 02 00 00" in
...
let test_pmaxsd =
  let bytes = "\x66\x0f\x38\x3d\xc8" in
  (* pmaxsd %xmm0, %xmm1  *)
  let x = of_bytes "8f 0e 00 00 0b 0a 09 08 05 06 05 04 03 04 01 00" in
  let y = of_bytes "0f 00 0d 0c 00 00 00 00 07 06 05 05 03 02 00 00" in
  let z = of_bytes "0f 00 0d 0c 0b 0a 09 08 07 06 05 05 03 04 01 00" in
