...
let check_pc name c expected =
  ...
  | Bil.Imm a ->
      assert_bool
        (sprintf "%s fail: addr is not equal to expected: %s <> %s" name
           (Addr.to_string a) (Addr.to_string expected))
        (Addr.equal a expected)
  | _ -> assert_bool (sprintf "%s fail: pc is not a word" name) false

let is_bot = function Bil.Bot -> true | _ -> false
...
let branch name aa lk arch offs (_ctxt : test_ctxt) =
  ...
  let imm = Word.(word_of_int arch offs lsl word_of_int arch 2) in
  let expected = if aa = 1 then imm else Word.(addr + imm) in
...
let make_bo ?ctr_before ?ctr_after ?cond_reg0 ?(expect_jump = false) bo_field
  ...
  let bits = Size.in_bits @@ Arch.addr_size arch in
  let ctr_before =
    match ctr_before with
    | None -> Word.zero bits
    | Some x -> Word.of_int ~width:bits x
  in
  let ctr_after =
    Option.map ~f:(fun x -> Word.of_int ~width:bits x) ctr_after
  in
  let cond_reg0 =
    match cond_reg0 with None -> Word.b0 | Some x -> Word.of_bool (x = 1)
  in
  { bo_field; ctr_before; ctr_after; cond_reg0; expect_jump }

let counter_unchanged =
  make_bo ~ctr_before:42 ~cond_reg0:1 ~ctr_after:42 0b00100

let counter_decremented =
  make_bo ~ctr_before:42 ~cond_reg0:1 ~ctr_after:41 0b00000
...
let cond_ctr_ok_2 =
  make_bo ~ctr_before:42 ~cond_reg0:0 ~expect_jump:true 0b00000

(** cond: bo3 = CR0 = 1; ctr: bo1 = 0 and ctr <> 0 *)
let cond_ctr_ok_3 =
  make_bo ~ctr_before:42 ~cond_reg0:1 ~expect_jump:true 0b01000
...
let bcx name aa lk arch imm case (_ctxt : test_ctxt) =
  ...
  let bits = Size.in_bits @@ Arch.addr_size arch in
  let bytes = make_insn ~name ~arch `B [ 16; case.bo_field; 0; imm; aa; lk ] in
  let cr0 = cr_bit 0 in
  let ctr = ctr arch in
  let init = Bil.[ cr0 := int case.cond_reg0; ctr := int case.ctr_before ] in
  let addr = addr_of_arch arch in
  ...
  let () =
    match case.ctr_after with
    | None -> ()
    | Some x ->
        assert_bool (sprintf "%s failed" name)
        @@ is_equal_words x (lookup_var c ctr)
  in
  if lk = 1 then
...
let bcXrX name opt_opcode lk reg arch case (_ctxt : test_ctxt) =
  ...
  let cr_num = 31 in
  let bytes =
    make_insn ~name ~arch `XL
      [ 19; case.bo_field; cr_num; 0; 0; opt_opcode; lk ]
  in
  ...
  let () =
    ...
    | None -> ()
    | Some x ->
        assert_bool (sprintf "%s ctr check failed" name)
        @@ is_equal_words x (lookup_var c ctr)
  in
  if lk = 1 then
    let next = Word.(of_int ~width:bits 4 + addr) in
...
let bdnz ~jmp arch (_ctxt : test_ctxt) =
  ...
  let ctr_val = if jmp then Word.of_int ~width:bits 42 else Word.one bits in
  let ctr = ctr arch in
