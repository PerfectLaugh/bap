  ...
  List.fold ~init:String.Map.empty
    ~f:(fun regs i ->
      ...
      | Some a ->
          ...
          Map.set regs ~key:name ~data:var)
    range
  ...
  let cri =
    let _, bits =
      List.fold (List.rev cr_bits) ~init:(0, Int.Map.empty)
        ~f:(fun (num, bits) bit -> (num + 1, Map.set bits ~key:num ~data:bit))
    in
    bits

  let crn =
    Map.fold cri ~init:String.Map.empty ~f:(fun ~key:_ ~data:var acc ->
...
let raise_arch () = failwith "powerpc arch is the only expected"
let cr_bit n = Map.find_exn Any_ppc.cri n
let nf = cr_bit 0
let pf = cr_bit 1
let zf = cr_bit 2
let ca = Any_ppc.ca
let ca32 = Any_ppc.ca32
let lr = function `ppc -> P32.lr | `ppc64 -> P64.lr | _ -> raise_arch ()
let ctr = function `ppc -> P32.ctr | `ppc64 -> P64.ctr | _ -> raise_arch ()
let tar = function `ppc -> P32.tar | `ppc64 -> P64.tar | _ -> raise_arch ()
let cri = Any_ppc.cri
let mem = function `ppc -> P32.mem | `ppc64 -> P64.mem | _ -> raise_arch ()
...
let create_dis arch =
  ...
  |> Or_error.ok_exn |> Dis.store_kinds |> Dis.store_asm

let create_memory arch s addr =
  let endian = Arch.endian arch in
  Memory.create endian addr @@ Bigstring.of_string s |> function
  | Ok r -> r
...
let get_insn ?addr arch bytes =
  ...
  let addr = match addr with None -> Addr.of_int ~width 0 | Some a -> a in
  ...
  | Ok (mem, Some insn, _) ->
      ...
      (mem, insn, insn_name)
  | Ok (mem, None, _) ->
      failwithf "Failed to find an instruction in: %a" Memory.pps mem ()
  | Error err ->
      failwithf "Diassembler failed with: %s" (Error.to_string_hum err) ()

let lookup_var c var =
...
let find_gpr arch name =
  ...
  with _ -> sprintf "gpr %s not" name |> failwith
  ...
  | Error te ->
      let err =
        sprintf "The lifted code is not well-typed: %s"
          (Type.Error.to_string te)
      in
      failwith err
  | Ok () -> ()

(** [check_gpr ?addr init_bil bytes var expected arch ctxt] - tests if a result
    bound to the [var] is equal to [exptected]. Evaluates bil, that is a
    concatenation of [init_bil] and code, obtained from lifting [bytes]. [addr]
    is an instruction address, 0 by default. *)
let check_gpr ?addr init bytes var expected arch _ctxt =
  ...
  | Some w ->
      if
        (not (Word.equal w expected))
        || Word.bitwidth w <> Word.bitwidth expected
      then
        printf "\n%s: check failed for %s: expected %s <> %s\n" insn_name
          (Var.name var) (Word.to_string expected) (Word.to_string w);
      assert_equal ~cmp:Word.equal w expected

(** [eval ?addr init_bil bytes arch] - evaluates bil, that is a concatenation of
    [init_bil] and code, obtained from lifting [bytes]. [addr] is an instruction
...
let load_word ctxt mem addr endian size =
  ...
  let bil = Bil.[ tmp := load ~mem:(var mem) ~addr:(int addr) endian size ] in
  check_bil bil;
  let ctxt = Stmt.eval bil ctxt in
  lookup_var ctxt tmp

let check_mem init bytes mem ~addr ~size expected ?(endian = BigEndian) arch
    _ctxt =
  ...
  | Some w ->
      if not (Word.equal w expected) then
        printf "\n%s: check failed for %s: expected %s <> %s\n" insn_name
          (Addr.to_string addr) (Word.to_string expected) (Word.to_string w);
      assert_equal ~cmp:Word.equal w expected

let concat_words ws =
  match ws with
  | [] -> failwith "words list is empty!"
  | w :: ws -> List.fold ~init:w ~f:(fun ws w -> Word.concat ws w) ws
...
let is_equal_words w = function None -> false | Some w' -> Word.equal w w'

let string_of_bytes bytes =
  String.fold ~init:""
    ~f:(fun acc b -> sprintf "%s%02X " acc (Char.to_int b))
    bytes

let arch_width a = Arch.addr_size a |> Size.in_bits

type form =
  [ `B | `D | `I | `M | `MD | `MDS | `VA | `X | `XL | `XFX | `XO | `XS ]
[@@deriving sexp]
...
let make_insn ?name ?(arch = `ppc) form fields =
  ...
  let bytes =
    ...
    | _ -> failwith "unexpected argument set for given insn form"
  in
  let () =
    match name with
    | None -> ()
    | Some name ->
        ...
        if not (String.equal name insn_name) then
          let err =
            sprintf "error: failed to construct %s insn, got a %s" name
              insn_name
          in
          failwith err
  in
