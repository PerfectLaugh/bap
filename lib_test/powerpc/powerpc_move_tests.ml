...
let mtspr arch ctxt =
  ...
  let init = Bil.[ r1 := int data ] in
  let expected =
    match arch with `ppc -> Word.extract_exn ~hi:31 data | _ -> data
  in
...
let mfspr arch ctxt =
  ...
  let lr = lr arch in
  let init = Bil.[ lr := int x ] in
  let expected = match arch with `ppc -> Word.extract_exn ~hi:31 x | _ -> x in
...
let mtcrf arch _ctxt =
  ...
  let init_cr =
    Map.fold cri ~init:[] ~f:(fun ~key:_ ~data:bit acc ->
        Bil.(bit := b0) :: acc)
  in
  ...
  List.iter range ~f:(fun i ->
      if List.exists set_bits ~f:(fun j -> j * 4 <= i && i < (j + 1) * 4) then
        let expected = if r1_bits.(i) then Word.b1 else Word.b0 in
...
let mfcr arch ctxt =
  ...
  let init =
    List.foldi bits ~init:[] ~f:(fun i acc bit ->
