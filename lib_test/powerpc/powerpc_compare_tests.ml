...
let print_bits lt gt eq =
  let str = function None -> "unknown" | Some x -> Word.to_string x in
...
let check ctxt expected =
  ...
  match expected with
  | LT ->
      is_equal_words Word.b1 lt && is_equal_words Word.b0 gt
      && is_equal_words Word.b0 eq
  | GT ->
      is_equal_words Word.b0 lt && is_equal_words Word.b1 gt
      && is_equal_words Word.b0 eq
  | EQ ->
      is_equal_words Word.b0 lt && is_equal_words Word.b0 gt
...
let cmpi name ~opcode l_field arch reg_value value expected (_ctxt : test_ctxt)
  ...
  let bytes = make_bytes [ opcode; cr_field; Word.b0; l_field; reg; value ] in
...
let cmp name ~opt_opcode l_field arch reg1_value reg2_value expected
    (_ctxt : test_ctxt) =
  let reg1_num = 6 in
  let reg2_num = 7 in
  ...
  let bytes =
    make_bytes
      [ opcode; cr_field; Word.b0; l_field; reg1; reg2; opt_opcode; fin ]
  in
