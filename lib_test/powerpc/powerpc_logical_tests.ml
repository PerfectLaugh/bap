...
let andc arch ctxt =
  ...
  let init =
    Bil.[ r7 := int (Word.of_int ~width x); r10 := int (Word.of_int ~width y) ]
  in
  let expected = Word.of_int ~width r in
  check_gpr init bytes r10 expected arch ctxt

let andc_dot arch ctxt =
  let bytes = "\x7c\xea\x50\x79" in
  (* andc. r10 r7 r10 *)
  let r10 = find_gpr arch "R10" in
  let r7 = find_gpr arch "R7" in
  let width = arch_width arch in
...
let ori arch ctxt =
  let bytes = "\x60\xc6\x51\xc1" in
  (* ori     r6,r6,20929 *)
  let r6 = find_gpr arch "R6" in
  let width = arch_width arch in
  let x = 62 in
  let r = x lor 20929 in
  let init = Bil.[ r6 := int (Word.of_int ~width x) ] in
  let expected = Word.of_int ~width r in
  check_gpr init bytes r6 expected arch ctxt

let oris arch ctxt =
  let bytes = "\x65\x4a\x00\x0F" in
  (* oris    r10,r10,15  *)
  let r10 = find_gpr arch "R10" in
  let width = arch_width arch in
  let x = 61440 in
  ...
  let init = Bil.[ r10 := int (Word.of_int ~width x) ] in
  let expected = Word.of_int ~width r in
  check_gpr init bytes r10 expected arch ctxt

let or_ arch ctxt =
  let bytes = "\x7f\x38\xc3\x78" in
  (* or r24,r25,r24  *)
  ...
  let r = x lor y in
  let init =
    Bil.[ r24 := int (Word.of_int ~width x); r25 := int (Word.of_int ~width y) ]
  in
  let expected = Word.of_int ~width r in
  check_gpr init bytes r24 expected arch ctxt

let or_dot arch ctxt =
  let bytes = "\x7f\x38\xc3\x79" in
  (* or r24,r25,r24  *)
  let r24 = find_gpr arch "R24" in
  let r25 = find_gpr arch "R25" in
  let width = arch_width arch in
...
let orc arch ctxt =
  ...
  let init =
    Bil.[ r4 := int (Word.of_int ~width x); r10 := int (Word.of_int ~width y) ]
  in
  let expected = Word.of_int ~width r in
  check_gpr init bytes r10 expected arch ctxt

let orc_dot arch ctxt =
  let bytes = "\x7c\x8a\x53\x39" in
  (* orc.     r10,r4,r10 *)
  let r4 = find_gpr arch "R4" in
  let r10 = find_gpr arch "R10" in
  let width = arch_width arch in
...
let xori arch ctxt =
  let bytes = "\x68\x63\x00\x0B" in
  (* xori    r3,r3,11   *)
  let r3 = find_gpr arch "R3" in
  let width = arch_width arch in
  let x = 16 in
  let r = x lxor 11 in
  let init = Bil.[ r3 := int (Word.of_int ~width x) ] in
  let expected = Word.of_int ~width r in
  check_gpr init bytes r3 expected arch ctxt

let xoris arch ctxt =
  let bytes = "\x6d\x2a\x00\x0f" in
  (* xoris r10,r9,15 *)
  let r9 = find_gpr arch "R9" in
  let r10 = find_gpr arch "R10" in
  let width = arch_width arch in
  ...
  let init = Bil.[ r9 := int (Word.of_int ~width x) ] in
  let expected = Word.of_int ~width r in
  check_gpr init bytes r10 expected arch ctxt

let xor_ arch ctxt =
  let bytes = "\x7c\x6a\x52\x78" in
  (* xor     r10,r3,r10 *)
  let r3 = find_gpr arch "R3" in
  let r10 = find_gpr arch "R10" in
  let width = arch_width arch in
  ...
  let r = x lxor y in
  let init =
    Bil.[ r3 := int (Word.of_int ~width x); r10 := int (Word.of_int ~width y) ]
  in
  let expected = Word.of_int ~width r in
  check_gpr init bytes r10 expected arch ctxt

let xor_dot arch ctxt =
  let bytes = "\x7c\x6a\x52\x79" in
  (* xor.     r10,r3,r10 *)
  let r3 = find_gpr arch "R3" in
  let r10 = find_gpr arch "R10" in
  let width = arch_width arch in
...
let nand arch ctxt =
  ...
  let init =
    Bil.[ r3 := int (Word.of_int ~width x); r4 := int (Word.of_int ~width y) ]
  in
  let expected = Word.of_int ~width r in
  check_gpr init bytes r3 expected arch ctxt

let nand_dot arch ctxt =
  let bytes = "\x7c\x63\x23\xb9" in
  (* nand.    r3,r3,r4 *)
  let r3 = find_gpr arch "R3" in
  let r4 = find_gpr arch "R4" in
  let width = arch_width arch in
...
let nor arch ctxt =
  ...
  let init =
    Bil.[ r8 := int (Word.of_int ~width x); r9 := int (Word.of_int ~width y) ]
  in
  let expected = Word.of_int ~width r in
  check_gpr init bytes r9 expected arch ctxt

let nor_dot arch ctxt =
  let bytes = "\x7d\x09\x48\xf9" in
  (* nor.     r9,r8,r9 *)
  let r8 = find_gpr arch "R8" in
  let r9 = find_gpr arch "R9" in
  let width = arch_width arch in
...
let extsw arch ctxt =
  let bytes = "\x7d\x25\x07\xb4" in
  (* extsw   r5,r9 *)
  let r5 = find_gpr arch "R5" in
  let r9 = find_gpr arch "R9" in
  let width = arch_width arch in
  let init = Bil.[ r9 := int (Word.of_int ~width 0xC0000000) ] in
  let expected = Word.of_int64 ~width 0xFFFFFFFFC0000000L in
  check_gpr init bytes r5 expected arch ctxt

let extsw_dot arch ctxt =
  let bytes = "\x7d\x25\x07\xb5" in
  (* extsw.  r5,r9 *)
  let r5 = find_gpr arch "R5" in
  let r9 = find_gpr arch "R9" in
  let width = arch_width arch in
...
let cntlzw arch value zeros ctxt =
  ...
  let width = arch_width arch in
  let init = Bil.[ r := int (Word.of_int ~width value) ] in
...
let cmpb arch ~bytes_cnt x y expected ctxt =
  ...
  let r10 = find_gpr arch "R10" in
  let r4 = find_gpr arch "R4" in
  let init = Bil.[ r10 := int x; r4 := int y ] in
...
let popcntw arch ctxt =
  ...
  let value, expected =
    match arch with
    | `ppc ->
        (Word.of_int64 ~width 0x10000001L, Word.of_int64 ~width 0x000000002L)
    | _ -> (Word.of_int64 0xA0200040_10000001L, Word.of_int64 0x400000002L)
  in
  (* 4 bits set in first word, and 2 in second  *)
  let init = Bil.[ r2 := int value ] in
  check_gpr init bytes r4 expected arch ctxt

let popcntd ctxt =
  let arch = `ppc64 in
  let bytes = "\x7c\x44\x03\xf4" in
  (* popcntw r4, r2 *)
  let r4 = find_gpr arch "R4" in
  let r2 = find_gpr arch "R2" in
  let width = arch_width arch in
...
let bperm ctxt =
  ...
  let init = Bil.[ r5 := int index_bits; r9 := int value ] in
...
let suite =
  ...
  >::: [
         ...
         "cnttzd" >:: cnttzd;
         "cmpb64: 1"
         >:: cmpb `ppc64 ~bytes_cnt:3 0x31_42_45 0x34_42_AD 0x00_FF_00;
         "cmpb64: 2" >:: cmpb `ppc64 ~bytes_cnt:3 0 0x34_42_AD 0x0;
         "cmpb64: 3"
         >:: cmpb `ppc64 ~bytes_cnt:3 0x34_42_AD 0x34_42_AD 0xFF_FF_FF;
