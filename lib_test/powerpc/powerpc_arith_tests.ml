...
let neg arch ctxt =
  ...
  let x =
    ...
    | _ -> Word.of_int64 0x80000000_00000000L
  in
  let init = Bil.[ r2 := int x ] in
...
let subf arch ctxt =
  ...
  let x =
    ...
    | _ -> Word.of_int64 0xABCDEF42_12345678L
  in
  let y = Word.of_int ~width 0x42 in
  let r1 = find_gpr arch "R1" in
  let r2 = find_gpr arch "R2" in
  let r3 = find_gpr arch "R3" in
  let init = Bil.[ r3 := int x; r2 := int y ] in
...
let subfe arch ctxt =
  ...
  let r2 = find_gpr arch "R2" in
  let r3 = find_gpr arch "R3" in
  let init = Bil.[ r3 := int x; r2 := int y; ca := int Word.b1 ] in
...
let mulhw arch ctxt =
  ...
  let r2 = find_gpr arch "R2" in
  let r3 = find_gpr arch "R3" in
  let init = Bil.[ r1 := int x; r2 := int y; r3 := int z ] in
  let expected =
    let y = extend (Word.signed y) in
    let z = extend (Word.signed z) in
    Word.signed (high Word.(y * z))
  in
  let expected = match arch with `ppc -> expected | _ -> extend expected in
  check_gpr init bytes r1 expected arch ctxt
...
let mullw arch ctxt =
  ...
  let r2 = find_gpr arch "R2" in
  let r3 = find_gpr arch "R3" in
  let init = Bil.[ r2 := int x; r3 := int y ] in
  let expected =
    match arch with
...
let divw arch ctxt =
  ...
  let expected = match arch with `ppc -> r | _ -> extend r in
  check_gpr init bytes r1 expected arch ctxt
...
let divwe arch ctxt =
  ...
  let x =
    match arch with
    | `ppc -> Word.(signed (concat x (zero 32)))
    | _ -> Word.(signed (x lsl Word.of_int64 32L))
  in
  let y = Word.signed (low y) in
...
let divweu arch ctxt =
  ...
  let x =
    ...
    | _ -> Word.(x lsl Word.of_int64 32L)
  in
  let y = low y in
