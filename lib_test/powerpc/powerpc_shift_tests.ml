...
let slw arch sh ctxt =
  ...
  if sh >= 32 then check_gpr init bytes r10 (Word.zero width) arch ctxt
  else
    let x = Word.extract_exn ~hi:31 Word.(x lsl shw) in
    let expected =
      match arch with `ppc -> x | _ -> Word.concat (Word.zero 32) x
    in
...
let srawi arch sh x ?(carry = false) ctxt =
  ...
  let left_bits_num = match arch with `ppc -> sh | _ -> 32 + sh in
  let left_bits =
    if Word.(equal (extract_exn ~hi:31 ~lo:31 x) b1) then
      Word.ones left_bits_num
    else Word.zero left_bits_num
  in
...
let sraw arch sh x ?(carry = false) ctxt =
  ...
  let expected =
    if right_bits_num = 0 then left_bits
      ...
      let right_bits =
        Word.extract_exn ~hi:(right_bits_num - 1) Word.(x lsr shw)
      in
      Word.concat left_bits right_bits
  in
  let ca_expected = if carry then Word.b1 else Word.b0 in
  let ctxt = eval init bytes arch in
  let value = lookup_var ctxt r10 in
...
let sradi sh x ?(carry = false) ctxt =
  ...
  let expected =
    let mask =
      if Word.(equal (extract_exn ~hi:63 ~lo:63 x) b1) then Word.ones 64
      else Word.zero 64
    in
    let y = Word.(x lsr shw) in
    let shm = Word.of_int ~width:64 (64 - sh) in
    Word.((mask lsl shm) lor y)
  in
  let ca_expected = if carry then Word.b1 else Word.b0 in
  let ctxt = eval init bytes arch in
  let value = lookup_var ctxt r10 in
...
let suite =
  ...
  >::: [
         ...
         "sraw64 42, neg, ca"
         >:: sraw `ppc64 42 0x00000000_80000000L ~carry:true;
