...
let addis arch ctxt =
  ...
  let width = arch_width arch in
  let init = Bil.[ r30 := int (Word.of_int ~width 42) ] in
...
let addis_zero_op arch ctxt =
  let bytes = "\x3f\xc0\x00\x02" in
  (* addis r30, 0, 2 OR lis r30,1 *)
  let r30 = find_gpr arch "R30" in
  let width = arch_width arch in
  let r = 2 lsl 16 in
  let expected = Word.of_int ~width r in
...
let add arch ctxt =
  ...
  let init = Bil.[ r2 := int x; r11 := int y ] in
  let expected = Word.of_int ~width 42 in
  check_gpr init bytes r11 expected arch ctxt

let addic arch _ctxt =
  let bytes = "\x30\x21\x00\x10" in
  (* addic r1, r1, 16  *)
  let r1 = find_gpr arch "R1" in
  let width = arch_width arch in
  let x = Word.of_int ~width 0x0fffffff0 in
  let init = Bil.[ r1 := int x ] in
  let expected =
    match arch with
    | `ppc -> Word.zero width
    | _ -> Word.of_int ~width 0x00000001_00000000
  in
  ...
  let ca_expected = match arch with `ppc -> Word.b1 | _ -> Word.b0 in
...
let addc arch _ctxt =
  let bytes = "\x7d\x62\x58\x14" in
  (* addc r11, r2, r11  *)
  let r2 = find_gpr arch "R2" in
  let r11 = find_gpr arch "R11" in
  let width = arch_width arch in
  let x = Word.of_int ~width 0x0fffffff0 in
  let init = Bil.[ r2 := int x; r11 := int (Word.of_int ~width 16) ] in
  let expected =
    match arch with
    | `ppc -> Word.zero width
    | _ -> Word.of_int ~width 0x00000001_00000000
...
let adde arch _ctxt =
  ...
  let init =
    Bil.[ r1 := int x; r16 := int (Word.of_int ~width 15); ca := int Word.b1 ]
  in
  let ctxt = eval init bytes arch in
  let r1_value = lookup_var ctxt r1 in
  let ca_value = lookup_var ctxt ca in
  let ca32_value = lookup_var ctxt ca32 in
  let expected, ca_expected =
    match arch with
    | `ppc -> (Word.zero width, Word.b1)
    | _ -> (Word.of_int ~width:64 0x00000001_00000000, Word.b0)
  in
...
let addme arch _ctxt =
  let bytes = "\x7c\x22\x01\xd4" in
  (* addme r1, r2   *)
  let r1 = find_gpr arch "R1" in
  let r2 = find_gpr arch "R2" in
  let width = arch_width arch in
  let x = Word.of_int64 ~width 0xffffffffL in
  let init = Bil.[ r2 := int x; ca := int Word.b1 ] in
  let ctxt = eval init bytes arch in
  let r1_value = lookup_var ctxt r1 in
  let ca_value = lookup_var ctxt ca in
  let ca32_value = lookup_var ctxt ca32 in
...
let addze arch _ctxt =
  ...
  let x =
    ...
    | _ -> Word.of_int64 0xffffffff_ffffffffL
  in
  let init = Bil.[ r2 := int x; ca := int Word.b1 ] in
  let ctxt = eval init bytes arch in
  let r1_value = lookup_var ctxt r1 in
...
let addic_dot arch _ctxt =
  ...
  let x =
    ...
    | _ -> Word.of_int64 0xffffffff_fffffff0L
  in
  let init = Bil.[ r1 := int x ] in
  ...
  let x, y, expected =
    ...
    | _ ->
        ( Word.of_int64 0x0fffffff_fffffff0L,
          ...
          Word.of_int64 0xffffffff_ffffffffL )
  in
  let init = Bil.[ r2 := int x; r11 := int y ] in
  let ctxt = eval init bytes arch in
  let r11_value = lookup_var ctxt r11 in
...
let adde_dot arch _ctxt =
  ...
  let init = Bil.[ r1 := int x; r16 := int y; ca := int Word.b1 ] in
  let ctxt = eval init bytes arch in
  let r1_value = lookup_var ctxt r1 in
  let ca_value = lookup_var ctxt ca in
  let ca32_value = lookup_var ctxt ca32 in
  let expected = Word.of_int64 ~width 0xffffffffL in
  let nf_expected, pf_expected =
    match arch with
    | `ppc -> (Word.b1, Word.b0)
    | `ppc64 -> (Word.b0, Word.b1)
    | _ -> failwith "PowerPC only expected"
  in
  let nf_value = lookup_var ctxt nf in
  let pf_value = lookup_var ctxt pf in
  let zf_value = lookup_var ctxt zf in
...
let addme_dot arch _ctxt =
  let bytes = "\x7c\x22\x01\xd5" in
  (* addme. r1, r2  *)
  let r1 = find_gpr arch "R1" in
  let r2 = find_gpr arch "R2" in
  let width = arch_width arch in
  let x = Word.one width in
  let init = Bil.[ r2 := int x; ca := int Word.b0 ] in
  let ctxt = eval init bytes arch in
  let r1_value = lookup_var ctxt r1 in
  let ca_value = lookup_var ctxt ca in
  let ca32_value = lookup_var ctxt ca32 in
  let expected = Word.zero width in
  let cf_expected = match arch with `ppc -> Word.b1 | _ -> Word.b1 in
  let nf_value = lookup_var ctxt nf in
  let pf_value = lookup_var ctxt pf in
  let zf_value = lookup_var ctxt zf in
  assert_bool "addme. failed: result" (is_equal_words expected r1_value);
