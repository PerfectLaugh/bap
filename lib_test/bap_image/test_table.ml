...
let tab =
  let t =
    Table.add empty_table mem_low "lo" >>= fun t ->
    Table.add t mem_high "hi" >>= fun t -> return t
  in
  match t with
  | Ok t -> t
  | Error err -> assert_failure (Error.to_string_hum err)
...
let test_one_to_one ctxt =
  let tab2 =
    ...
    let t =
      Table.add empty_table mem_low "reg1" >>= fun t ->
      Table.add t mem_high "reg2" >>= fun t ->
      Table.add t extra "reg3" >>= fun t -> return t
    in
    match t with
    | Ok t -> t
    | Error err -> assert_failure (Error.to_string_hum err)
...
let suite () =
  ...
  >::: [
         ...
         "mem" >:: test_mem;
         "intersections1"
         >:: test_intersections ~start_addr:0x10 ~size:0x30
               ~expect:[ "hi"; "lo" ];
         "intersections2"
         >:: test_intersections ~start_addr:0x1F ~size:0x30
               ~expect:[ "hi"; "lo" ];
         "intersections3"
