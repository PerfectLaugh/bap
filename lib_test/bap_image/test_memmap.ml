...
let random_chunk () =
  ...
  let offset = if max_size = size then 0 else Random.int (max_size - size) in
  chunk offset size

let random_list size = List.init size ~f:(fun v -> (random_chunk (), v))
...
let add_lookup chunk ctxt =
  ...
  let check x =
    match Memmap.lookup map x |> Seq.to_list with
    | [ (mem, v) ] ->
        assert_memory ~ctxt chunk mem;
        assert_equal ~ctxt ~printer:Int.to_string 42 v
    | [] -> assert_string "can't find just added chunk"
    | _ :: _ -> assert_string "found more that one chunk"
  in
  let rec loop x =
    if Addr.(x <= Memory.max_addr chunk) then (
      check x;
      loop Addr.(succ x))
  in
...
let min_max cons x y z ctxt =
  ...
  let map =
    List.foldi [ c1; c2; c3 ] ~init:empty ~f:(fun n map c -> Memmap.add map c n)
  in
...
let dominate cons size ctxt =
  ...
  List.iter seq ~f:(fun (k, v) ->
      let msg =
        sprintf "(%d,[%a,%a]) is not dominated by [%a,%a]" v Addr.pps
          (Memory.min_addr k) Addr.pps (Memory.max_addr k) Addr.pps
          (Option.value_exn (Memmap.min_addr map))
          Addr.pps
          (Option.value_exn (Memmap.max_addr map))
      in
      assert_bool msg @@ Memmap.dominates map k)

let contains k v = Seq.exists ~f:(fun (mem, w) -> mem_equal k mem && v = w)

let find_all cons size ctxt =
  let lst = cons size in
  let map = map_of_list lst in
  List.iter lst ~f:(fun (k, v) ->
      assert_bool "find_dominators" @@ contains k v @@ Memmap.dominators map k;
...
let three_equal cons size ctxt =
  ...
  let map = map_of_list lst in
  let map =
    List.foldi ~init:map [ c3; c3; c3 ] ~f:(fun i map key ->
        Memmap.add map key i)
  in
  let ins = Memmap.intersections map c3 in
  List.iter eqs ~f:(fun (k, v) ->
      assert_bool "find_dominators" @@ contains k v @@ ins);
  let map = Memmap.remove_intersections map c3 in
  assert_bool "C3 must be removed" @@ not (Memmap.intersects map c3)
...
let run_through cons size ctxt =
  ...
  let rec loop = function
    ...
    | n ->
        ...
        let got =
          Memmap.lookup map a |> Seq.map ~f:snd |> Seq.to_list |> sort_ints
        in
...
let intersections cons size ctxt =
  ...
  let sort_ints = List.sort ~compare:Int.compare in
  let map =
    List.fold (cons size) ~init:map ~f:(fun map (k, _) ->
        Memmap.remove_intersections map k)
  in
  let map =
    List.fold jam ~init:map ~f:(fun map (k, _) ->
        Memmap.remove_dominators map k)
  in
  let map = List.fold lst ~init:map ~f:(fun map (k, v) -> Memmap.add map k v) in
  let is_in mem mem' =
    Memory.(contains mem (min_addr mem') || contains mem (max_addr mem'))
  in
  let collect mem' =
    List.filter_map lst ~f:(fun (mem, x) ->
  ...
  List.iter (cons size) ~f:(fun (mem, _) ->
      ...
      let got =
        Memmap.intersections map mem
        |> Seq.map ~f:snd |> Seq.to_list |> sort_ints
