...
let round t x ctxt =
  try
    match t.enc ~signed:t.signed x |> t.dec with
    | Ok y -> assert_equal ~ctxt ~printer:t.show x y
    | Error err -> assert_failure @@ Error.to_string_hum err
  with exn ->
    assert_failure
    @@ sprintf "failed on %s with %s" (t.show x) (Exn.to_string exn)

let rounds seq t ctxt = Sequence.iter seq ~f:(fun x -> round t x ctxt)

let int ~signed =
  { signed; dec = Leb128.to_int; enc = Leb128.of_int; show = Int.to_string }
...
let leb128_printer t =
  Sexp.to_string_hum @@ Or_error.sexp_of_t Leb128.sexp_of_t t

let read ~expect buffer t ctxt =
  assert_equal ~ctxt ~printer:leb128_printer
...
let size ~expect value t ctxt =
  let leb = t.enc value in
  assert_equal ~ctxt ~printer:Int.to_string (Leb128.size leb) expect
...
let suite =
  ...
  >::: [
         ...
         "unsigned write"
         >:: write ~expect:"\xE5\x8E\x26" 624485 @@ int ~signed:false;
         "signed read"
         >:: read ~expect:(-624485) "\x9b\xf1\x59" @@ int ~signed:true;
         "signed write"
         >:: write ~expect:"\xE5\x8E\x26" 624485 @@ int ~signed:true;
         "read int"
         >:: read ~expect:624485l "\xE5\x8E\x26" @@ int32 ~signed:false;
         "unsigned write"
         >:: write ~expect:"\xE5\x8E\x26" 624485l @@ int32 ~signed:false;
         "signed read"
         >:: read ~expect:(-624485l) "\x9b\xf1\x59" @@ int32 ~signed:true;
         "signed write"
         >:: write ~expect:"\xE5\x8E\x26" 624485l @@ int32 ~signed:true;
         "read int"
         >:: read ~expect:624485L "\xE5\x8E\x26" @@ int64 ~signed:false;
         "unsigned write"
         >:: write ~expect:"\xE5\x8E\x26" 624485L @@ int64 ~signed:false;
         "signed read"
         >:: read ~expect:(-624485L) "\x9b\xf1\x59" @@ int64 ~signed:true;
         "signed write"
         >:: write ~expect:"\xE5\x8E\x26" 624485L @@ int64 ~signed:true;
