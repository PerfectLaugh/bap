...
let x86_64 =
  ( "x86_64",
    [
      ...
      (* callq 942040 *)
      (call, [ "CALL64pcrel32"; "-0x11b9" ], [ `Call; `May_affect_control_flow ]);
      (* mov 0x10(%rax),%eax *)
      ( mov,
...
let assert_memory = assert_equal ~printer:Memory.to_string ~cmp:mem_equal

let memory_of_string ?(start = 0) ?(width = 64) data =
  Memory.create LittleEndian (Addr.of_int start ~width)
    (Bigstring.of_string data)
  |> Or_error.ok_exn

let string_of_strings insn = Sexp.to_string (sexp_of_list sexp_of_string insn)

let printer r =
  Or_error.sexp_of_t (sexp_of_list sexp_of_string) r |> Sexp.to_string

let strings_of_insn insn =
  let name = Dis.Insn.name insn in
  let ops = Dis.Insn.ops insn |> Array.to_list |> List.map ~f:Op.to_string in
  name :: ops
...
let insn_of_mem arch data _ctxt =
  ...
  Dis.with_disasm ~backend:"llvm" arch ~f:(fun dis ->
      ...
      | mem, Some insn, `finished ->
          assert_equal ~printer:string_of_int (String.length data)
            (Memory.length mem);
          Result.return (strings_of_insn insn))

let assert_equal_regexp s s' =
  let pat = sprintf "%s.?" s in
  let msg = sprintf "failed to find %s by pattern %s in %s" s' pat s in
...
let test_insn_of_mem (arch, samples) ctxt =
  let test (data, expect, _) =
    ...
    | Ok strs -> assert_strings_equal ctxt expect strs
...
let test_run_all (arch, samples) ctxt =
  Toplevel.reset ();
  let mem = samples |> List.map ~f:fst3 |> String.concat |> memory_of_string in
  Dis.with_disasm ~backend:"llvm" arch ~f:(fun dis ->
      let dis = Dis.store_kinds dis in
      Dis.run ~return:Or_error.return ~init:()
          ...
          @@ List.iter2_exn samples (Dis.insns s)
               ...
               | mem, Some r ->
                   ...
                   List.iter kinds ~f:(fun expected ->
                       let name = string_of_sexp @@ sexp_of_kind expected in
...
let test_run_all data ctxt =
  ...
  assert_equal ~ctxt ~printer (Ok ()) (test_run_all data ctxt)
...
let block3 = [ "\x10\x30\x1B\xE5"; "\x00\x00\x53\xE3"; "\xF6\xFF\xFF\x1A" ]
...
let strlen = List.concat [ block1; block2; block3; block4 ]
...
type graph = (int * int list * (int * dest_kind) list) list [@@deriving sexp_of]
...
let create_block addr blk =
  ...
  Memory.create LittleEndian (Addr.of_int32 addr) (Bigstring.of_string data)
  |> Or_error.ok_exn
...
let amount dis ctxt =
  assert_equal ~ctxt ~printer:Int.to_string ~msg:"Expected 4 basic blocks" 4
    (Cfg.number_of_nodes (Rec.cfg dis))
...
let structure cfg ctxt =
  ...
  let deepsort graph =
    List.map graph ~f:(fun (id, preds, succs) -> (id, sort preds, sort succs))
    |> sort
  in
...
let test_micro_cfg insn ctxt =
  ...
  | [ (mem, _) ] ->
      ...
      assert_equal ~printer:Addr.to_string ~ctxt (Addr.of_int64 0L)
        (Memory.min_addr mem);
      assert_equal ~printer:Addr.to_string ~ctxt max_addr (Memory.max_addr mem)
...
let sort_by_addr =
  List.sort ~compare:(fun x y -> Addr.compare (Block.addr x) (Block.addr y))

let call1_3ret _ctxt =
  Toplevel.reset ();
  let mem = String.concat [ call1; ret; ret; ret ] |> memory_of_string in
  ...
  | [ b1; b2; b3; b4 ] ->
      ...
      assert_bool "b2 has no succs" @@ Seq.is_empty (Cfg.Node.succs b2 cfg);
      assert_bool "b3 has no succs" @@ Seq.is_empty (Cfg.Node.succs b3 cfg)
  | _ -> assert false
