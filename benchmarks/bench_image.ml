...
let tab =
  Memory.With_error.foldi mem ~init:Table.empty ~f:(fun addr word tab ->
      Memory.view ~from:addr ~words:1 mem >>= fun mem -> Table.add tab mem word)
...
let sum_foldm () =
  let (_ : word Or_error.t) =
    Memory.With_error.fold mem
      ~f:(fun w1 w2 -> Word.Int_err.(!$w1 + !$w2))
      ~init:zero
  in
  ()
...
let test =
  Bench.Test.create_group ~name:"image"
    [
