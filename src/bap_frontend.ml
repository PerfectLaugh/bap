...
|}
       pp_info (commands ctxt) pp_info (plugins ctxt)
...
|}
  in
  Command.declare ~doc "." Command.args @@ fun _ -> Ok ()
...
let matches default set str =
  ...
  | Some set ->
      ...
      Set.mem set str || Set.mem set (String.lowercase str)

let strings = Option.map ~f:(Set.of_list (module String))
let pp_sep ppf () = Format.fprintf ppf ",@ "
let pp_strings ppf xs = Format.(pp_print_list ~pp_sep pp_print_string) ppf xs
...
let _list_command_declaration : unit =
  ...
  let filter =
    Command.parameter
      ...
      ~doc:
        ...
        \      then entities with that name will not be printed."
  in
  let doc = "explores various BAP facilities" in
  let push x xs =
    match xs with None -> Some [ x ] | Some xs -> Some (x :: xs)
  in
  let parse_filter =
    List.fold ~init:(None, None) ~f:(fun (features, exclude) elt ->
        match String.chop_prefix elt ~prefix:"-" with
        | Some elt -> (features, push elt exclude)
        | None ->
            let elt =
              String.chop_prefix elt ~prefix:"+" |> Option.value ~default:elt
            in
            (push elt features, exclude))
  in
  ...
  let requested = strings requires and excluded = strings exclude in
  let selected name =
    matches true requested name && not (matches false excluded name)
  in
  match what with
  | `Recipes ->
...
let _config_command_declaration : unit =
  ...
  Command.declare ~doc "config" Command.(args $ entity) @@ fun entity _ctxt ->
  match entity with
  | Some entity ->
...
let () =
  ...
  let () =
    try
      if String.(Sys.getenv "BAP_DEBUG" <> "0") then
  ...
  match
    Bap_main.init ~default:print_info ~default_command:"disassemble" ~name:"bap"
      ~man ~argv:Sys.argv ()
  with
  | Ok () -> ()
