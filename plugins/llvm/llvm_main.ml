let provides = [ "disassembler"; "loader"; "elf"; "macho"; "coff"; "llvm" ]
...
let x86_syntax =
  let typ =
    Type.define
      ~parse:(fun s ->
        try x86_syntax_of_sexp (Sexp.of_string s)
        with _ -> raise (Invalid_argument s))
      ~print:(fun x -> Sexp.to_string (sexp_of_x86_syntax x))
      `att
  in
  let doc = "Choose style of code for x86 syntax between att and intel" in
...
let base_addr =
  let doc =
    "Replace image base address. If not set, a reasonable default corresponded\n\
     ...
     For relocatable files a default image base is equal to 0xC0000000."
  in
...
let pdb_path =
  let doc =
    ...
    \     to a current working directory."
  in
