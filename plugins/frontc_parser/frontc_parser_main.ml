...
let int size sign : C.Type.basic =
...
let char sign : C.Type.basic =
  match sign with NO_SIGN -> `char | SIGNED -> `schar | UNSIGNED -> `uchar

let cv = C.Type.Qualifier.{ const = false; volatile = false; restrict = () }
...
let func variadic return args =
  let args = match args with [ (_, `Void) ] -> [] | args -> args in
  `Function (spec no_qualifier C.Type.Proto.{ return; args; variadic })
...
and gnu_attrs_args =
  List.filter_map ~f:(function
    | GNU_ID s
    | GNU_CST (CONST_INT s | CONST_FLOAT s | CONST_CHAR s | CONST_STRING s) ->
        Some s
    | _ -> None)
    ...
    apply =
      (fun t ->
        C.Type.Spec.
          {
            t with
            qualifier = C.Type.Qualifier.{ t.qualifier with const = true };
          });
  }
    ...
    apply =
      (fun t ->
        C.Type.Spec.
          {
            t with
            qualifier = C.Type.Qualifier.{ t.qualifier with volatile = true };
          });
  }

let rec qualify f : C.Type.t -> C.Type.t = function
              ...
              C.Type.Compound.
                {
                  s.t with
                  fields =
                    List.map s.t.fields ~f:(fun (n, t) -> (n, qualify f t));
                };
          }
  | x -> x
...
let ctype gamma { lookup } t =
  let rec ctype : base_type -> C.Type.t = function
    | NO_TYPE | TYPE_LINE _ | OLD_PROTO _ | BITFIELD _ | BUILTIN_TYPE _ | VOID
      ->
        `Void
    | BOOL -> basic `bool
  ...
  and fields xs =
    List.map xs ~f:(fun (name, t, a) ->
        (name, with_attrs (gnu_attrs a) @@ ctype t))
  in
  ctype t
...
let is_signed = function #C.Type.signed -> true | #C.Type.unsigned -> false
...
let resolver lookup =
  object (self)
    ...
    method private resolve t =
      match t with { C.Type.Compound.fields = [] } -> self#lookup t | _ -> t

    method private lookup { C.Type.Compound.fields; name } =
      match lookup name with
      | Some (`Structure { C.Type.Spec.t }) | Some (`Union { C.Type.Spec.t }) ->
          C.Type.Compound.{ t with name }
...
let parse (size : C.Size.base) parse lexbuf =
  ...
  let gamma name =
    match Hashtbl.find env name with Some t -> t | None -> `Void
  in
  let lookup what name =
    match Hashtbl.find tags name with Some t -> t | None -> what name []
  in
  ...
  let typedef_int sz t =
    ...
    add name (basic (t :> C.Type.basic))
  in
  List.iter [ `r8; `r16; `r32; `r64 ] ~f:(fun sz ->
      let def types = Option.iter (repr size sz types) ~f:(typedef_int sz) in
...
let parser cpp size name =
  ...
  try Ok (parse size parser lexbuf) with exn -> Or_error.of_exn exn

let cpp =
  Config.param
    Config.(some string)
    ~as_flag:(Some "cpp")
    ~doc:"Preprocess headers with the specified preprocessor." "preprocess"
    ~synonyms:[ "pp" ]

let () =
  Config.declare_extension ~doc:"provides a C parser based on FrontC"
    ~provides:[ "api"; "c"; "parser" ]
  @@ fun { get } -> C.Parser.provide (parser (get cpp))
