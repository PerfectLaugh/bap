...
let print_meta trace =
  Trace.meta trace |> Dict.data |> Seq.to_list
...
let try_dump uri =
  Result.map (Trace.load uri) ~f:(fun trace ->
      print_meta trace;
      printf "@[<v2>events {@\n";
      Trace.read_events trace |> Sequence.iter ~f:(printf "%a@\n" Value.pp);
      printf "@]@\n}")

let dump uri =
  match try_dump uri with Error err -> Error err | Ok () -> Ok `Exit

let rec load = function
  | [] -> Ok `Done
  | uri :: uris ->
      Trace.load uri >>= fun trace ->
      Traces.add trace;
      load uris
...
module Cmdline = struct
  let () =
    Config.manpage
      [
        ...
        `S "DESCRIPTION";
        `P
          "Loads and prints traces. The plugin can be used in two\n\
          \       modes. When called as $(b,--)$(mname)$(b,-dump) it will just \
           dump the\n\
  ...
  let load : Uri.t list Config.param =
    let doc =
      "Load trace from the specified $(docv). The option maybe\n\
      \    used many times to load several traces"
    in
  ...
  let () =
    Config.declare_extension ~doc:"manages execution traces"
      ~provides:[ "dynamic"; "pass"; "printer"; "trace" ]
      (fun { Config.get = ( ! ) } ->
        Future.upon Plugins.loaded (fun () ->
            ...
            | Error e -> (
                match e with
                | `Protocol_error err -> exitf "Protocol error: %a" Error.pp err
                | `System_error err ->
                    exitf "System error: %s" @@ Unix.error_message err
                | `No_provider -> exitf "No provider for the given URI"
                | `Ambiguous_uri ->
