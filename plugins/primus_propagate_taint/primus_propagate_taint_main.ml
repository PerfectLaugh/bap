...
type mapper = { regs : taints; ptrs : taints; tids : tid Taint.Object.Map.t }
[@@deriving fields]

let mapper =
  Primus.Machine.State.declare ~name:"primus-taint-mapper"
    ~uuid:"12511d75-e626-4b0d-8dcd-2a3b597992cf" (fun _ ->
      {
        regs = Tid.Map.empty;
        ptrs = Tid.Map.empty;
...
let is_mem v = match Var.typ v with Type.Mem _ -> true | _ -> false
...
module Intro (Machine : Primus.Machine.S) = struct
  ...
  let kind t = Kind.create (sprintf "user-%s" (Tid.name t))

  let connect tid taint =
    Machine.Local.update mapper ~f:(fun m ->
        { m with tids = Map.set m.tids ~key:taint ~data:tid })
  ...
  let gentaint t =
    ...
    connect t taint >>| fun () -> Taint.Object.Set.singleton taint

  let introduces def =
    List.find_map translation ~f:(fun (k, s, r) ->
        match Term.get_attr def k with None -> None | Some t -> Some (t, s, r))

  let taint_ptr taints ptr = Tracker.attach ptr Taint.Rel.indirect taints

  let taint_var k taints var =
    if is_mem var then Machine.return ()
    else Env.get var >>= fun v -> Tracker.attach v k taints
  ...
  let intro def =
    ...
    | Some (taint, kind, rel) -> (
        ...
        | Bil.Load (_, addr, ed, sz) | Bil.Store (_, addr, _, ed, sz) ->
            Eval.exp addr >>= fun ptr ->
            if [%compare.equal: [ `Reg | `Ptr ]] kind `Reg then taint_ptr t ptr
            else Eval.load ptr ed sz >>= taint_ptr t
        | exp ->
            Exp.free_vars exp |> Set.to_sequence
            |> Machine.Seq.iter ~f:(taint_var rel t))

  let init () = Primus.Interpreter.leave_def >>> intro
end
...
module Mapper (Machine : Primus.Machine.S) = struct
  ...
  let remap tids objs = Tid.Set.filter_map objs ~f:(Map.find tids)
  ...
  let init () =
    Machine.sequence
      Primus.Interpreter.[ leave_def >>> update_def; leave_jmp >>> update_jmp ]
end
...
module Marker (Machine : Primus.Machine.S) = struct
  ...
  let init () = Primus.Interpreter.leave_blk >>> mark
end
...
manpage
  [
    `S "DESCRIPTION";
    `P
      "This plugin implements a compatibility layer between the new\n\
    ...
    `P
      "The translation is achieved by mapping the $(b,tainted-ptr) and\n\
...
let injection_enabled =
  flag "from-attributes"
    ~doc:
      "Introduces taint in terms that are marked with the\n\
...
let enabled =
  flag "run"
    ~doc:
      ("Enables propagating taint from term attributes and back to attributes, \
        unless the latter is disabled with the\n\
       \           $(b,no-marks) option. " ^ soft_deprecation_notice)

let marking_disabled =
  flag "no-marks"
    ~doc:
...
let () =
  declare_extension
    ...
    ~provides:[ "dataflow"; "pass"; "analysis"; "taint"; "primus" ]
  @@ fun { get = is } ->
  if is injection_enabled || is projection_enabled then
    if is enabled || is marking_disabled then
      invalid_arg "Incorrect mix of old and new parameters";
  if is marking_disabled && not (is enabled) then
    invalid_arg
      "The no-marks option is only valid if the run option is specified";

  List.iter markers ~f:(fun (name, comp, desc) ->
      Primus.Components.register_generic name comp ~package:"bap" ~desc);

  Primus.Components.register_generic "taint-intro"
    (module Intro)
    ~package:"bap"
    ~desc:
      "Introduces a taint when a term marked with tainted-reg or tainted-ptr \
       is entered.";

  (* Modern interface *)
  if is injection_enabled then enable_injection ();
  if is projection_enabled then enable_projection ();

  (* Legacy interface *)
  if is enabled then (
    enable_injection ();
    if not (is marking_disabled) then enable_projection ())
