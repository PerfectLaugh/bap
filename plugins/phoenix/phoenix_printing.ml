...
struct
  ...
  let pp_blk_name fmt blk =
    ...
    let pos =
      Symtab.owners (Project.symbols project) addr |> List.hd |> function
      ...
      | Some (name, entry, cfg) ->
          ...
          else sprintf "%s_%s" name (Word.string_of_value off)
    in
    fprintf fmt "%s" pos
  ...
  (** [pp_blk fmt blk] creates a basic block printer. The block is printed
      inside a 2 space indented vertical box *)
  let pp_blk proj pp fmt blk =
    let xs = proj blk in
    fprintf fmt "@{<(a (id %a))>@{<(pre (style %S))>%a@;@}@}" pp_blk_name blk
      "margin : 0" pp xs

  (** [pp_sym base cfg symtab] creates a printer for a symbol. The symbol is
      dentoned by its memory region, i.e., the printer has
      [formatter -> mem -> unit] type. *)
  let pp_sym pp_blk fmt (name, entry, cfg) =
    let blks =
      Graphlib.reverse_postorder_traverse (module Graphs.Cfg) ~start:entry cfg
    in
    fprintf fmt "@;@[<v2>@{<(div (id %s))>%a@}@]" name (pp_seq pp_blk) blks

  (** [pp_sym base cfg] return a printer that will print a symtab. Each symbol
      would be printed as a set of basic blocks that has intersections with the
      memory occupied by the symbol. Block will be printed in an order of their
      starting addresses. *)

  let pp_syms pp_blk fmt syms =
    pp_seq (pp_sym pp_blk) fmt (Symtab.to_sequence (Project.symbols project))
  ...
  let pp_code pp fmt v =
    ...
    fprintf fmt
      ...
      \       (rel stylesheet)\n\
      \       (type css)\n\
      \       (href ../../../css/code-panel.css))>@}@}@{<body>%a@;\
       @}@}"
      pp v;
