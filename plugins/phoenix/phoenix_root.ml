...
let mkdir dir =
  ...
  else if not (Sys.is_directory dir) then raise Target_directory_is_a_file

let touch_bases names = List.iter names ~f:(fun p -> mkdir (Filename.dirname p))
...
let create ?root fname =
  ...
  let base =
    ...
    else base / Filename.basename fname
  in
  mkdir base;
...
let with_files_in_folders folders (base, _) name ~f =
  let names =
    List.map folders ~f:(fun (p, ext) -> (base / p / name) ^ "." ^ ext)
  in
  touch_bases names;
  List.iter names ~f:(with_file ~f)

let with_cfg_file = with_files_in_folders [ ("cfg", "cfg"); ("sacfg", "sacfg") ]
let with_bil_file = with_files_in_folders [ ("bil", "html"); ("hil", "html") ]
let with_index_file (base, name) ~f = with_file ((base / name) ^ ".json") ~f

let with_funcs_file (base, name) ~f =
  with_file ((base / name) ^ ".functionlist") ~f
