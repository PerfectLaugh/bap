...
struct
  ...
  (** maps immediates to symbols. For any given value, if it belongs to some
      basic block, then substitute it with [base + off], where [base] is a start
      of basic block and [off] is the offset from the [base]. *)
  let resolve_to_symbols =
    let jump_type =
      match Arch.addr_size arch with `r32 -> reg32_t | `r64 -> reg64_t
    in
    let make_var name = Bil.var (Var.create name jump_type) in
    (object
       inherit Stmt.mapper
       ...
       method! map_int addr =
         ...
         | Some (sym, entry, _) ->
             ...
             if Addr.(start = addr) then make_var sym
             else
  ...
  (** This optimization will inline temporary variables that occurres inside the
      instruction definition if the right hand side of the variable definition
      is either side-effect free, or another variable, that is not changed in
      the scope of the variable definition. *)
  let inline_variables stmt =
    let rec loop ss = function
      ...
      | (Bil.Move (x, Bil.Var y) as s) :: xs when Var.is_virtual x ->
          if Bil.is_assigned y xs || Bil.is_assigned x xs then loop (s :: ss) xs
          else
            let xs = Bil.substitute (Bil.var x) (Bil.var y) xs in
            loop ss xs
      | (Bil.Move (x, y) as s) :: xs when Var.is_virtual x ->
          if has_side_effect y xs || Bil.is_assigned x xs then loop (s :: ss) xs
  ...
  let bil_of_insns insns = List.(insns >>| bil_of_insn |> concat)
  let bil_of_block blk : bil = bil_of_insns (Block.insns blk)
end
