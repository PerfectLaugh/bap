...
struct
  ...
  let store_cfg ((name, entry, cfg) as fn) =
    with_cfg_file root name ~f:(fun out -> Dot.fprint_graph out fn)

  let store_index () =
    let func ?(children = []) ?parent is_func id =
      Ezjsonm.(
        dict
          [
            ("text", string id);
            ( "id",
              string
                (Option.value_map parent ~default:id ~f:(fun p -> p ^ "*" ^ id))
            );
            ("expanded", bool false);
            ("function", bool is_func);
            (if List.is_empty children then ("leaf", bool true)
             else ("children", `A children));
          ])
    in
    let funcs =
    ...
    in
    let document =
      `O Ezjsonm.[ ("text", string "."); ("children", `A [ children ]) ]
    in
    with_index_file root ~f:(fun out ->
        Ezjsonm.to_string ~minify:false document |> pp_print_string out)

  let store_functions () =
    let address sel mem = Ezjsonm.string (Addr.string_of_value (sel mem)) in
    let func min max name =
      Ezjsonm.(
        dict
          [
            ("name", string name);
  ...
  let store_asm ((name, _, _) as fn) =
    ...
    with_bil_file root name ~f:(fun out -> pp_code (pp_sym pp_blk_bil) out fn)

  let store_dump () =
    let pp_blk_asm = pp_blk Block.insns pp_insns in
    let pr fmt cfg =
      fprintf fmt "@{<(pre (class %S))>%a@}" "prettyprint linenums"
        (pp_syms pp_blk_asm) symbols
    in
