...
struct
  ...
  module Dottable = struct
    ...
    let block_name blk = Format.asprintf "%a" pp_blk_name blk
    ...
    let iter_edges_e f mem =
      iter_vertex
        (fun (src, _) ->
          ...
          let arity =
            if Seq.length_is_bounded_by ~min:1 ~max:1 dests then `Mono
            else `Many
          in
    ...
    let graph_attributes _ = [ `Fontsize 14 ]
    ...
    let default_edge_attributes _ = [ `Penwidth 1.0 ]
