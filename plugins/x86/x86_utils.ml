...
let tmp ?(name = "v") ty = Var.create ~fresh:true ~is_virtual:true name ty
...
let concat_explist elist = List.reduce_exn ~f:Bil.( ^ ) elist
...
(** [ints_mem xs x] is [true] if [x] is a member of the list of integers [xs] *)
...
let min_symbolic ~is_signed e1 e2 =
  let open Bil in
  let cond = match is_signed with true -> e1 <$ e2 | false -> e1 < e2 in
...
let type_of_mode = function X86 -> Type.imm 32 | X8664 -> Type.imm 64
let width_of_mode = function X86 -> 32 | X8664 -> 64

let sig_to_mask =
  let open Pcmpstr in
  function LSB -> Bitmask | MSB -> Bytemask
...
let disfailwith m s =
  let a = match m with X86 -> `x86 | X8664 -> `x86_64 in
...
let df_to_offset mode e =
  ...
  | Type.Imm t ->
      let open Exp in
      Bil.(
        ite ~if_:(e = exp_false) ~then_:(int_exp 1 t) ~else_:(int_exp (-1) t))

let bap_to_rflags =
  let undefined d =
    Bil.unknown (Printf.sprintf "Undefined RFLAGS bit %d" d) bool_t
  in
...
let assns_rflags_to_bap =
  List.map
    ~f:(function
      | None -> fun _ -> [] | Some (v, f) -> fun e -> [ Bil.move v (f e) ])
    rflags_to_bap
...
let int64_of_mode m i =
  match m with
  | X86 -> BV.of_int64 i ~width:32
  | X8664 -> BV.of_int64 i ~width:64

let int_of_mode m i =
  match m with X86 -> BV.of_int i ~width:32 | X8664 -> BV.of_int i ~width:64
...
let bits2ymm mode b =
  let ymms = match mode with X86 -> R32.ymms | X8664 -> R64.ymms in
  ymms.(b)

let bits2ymme mode b = bits2ymm mode b |> Bil.var
let bits2ymm128e mode b = bits2ymme mode b |> Bil.(cast low !!reg128_t)
let bits2ymm64e mode b = bits2ymme mode b |> Bil.(cast low !!reg64_t)
let bits2ymm32e mode b = bits2ymme mode b |> Bil.(cast low !!reg32_t)
let bits2xmm = bits2ymm128e
let bits2xmm64e = bits2ymm64e
let bits2xmm32e = bits2ymm32e
let bits2reg64e mode b = Bil.var (bits2genreg mode b)

let bits2reg32e mode b =
  bits2genreg mode b |> Bil.var |> Bil.(cast low !!reg32_t)

let bits2reg16e mode b = bits2reg32e mode b |> Bil.(cast low !!reg16_t)

let bits2reg8e mode ?(has_rex = false) b =
  if b < 4 || has_rex then bits2reg32e mode b |> Bil.(cast low !!reg8_t)
  else
    b land 3 |> bits2reg32e mode
    |> Bil.(cast low !!reg16_t)
    |> Bil.(cast high !!reg8_t)
...
let pp_insn ppf (mem, insn) =
  Format.fprintf ppf "%a %s" Memory.pp mem (Disasm_expert.Basic.Insn.asm insn)
