...
let to_sbig_int (i, t) =
  ...
  if bi_is_zero sign then (* positive *) final
  else (* negative *) minus_big_int (power_of_two (bits - 1) -% final)

let t_div dividend divisor = Z.div dividend divisor
let t_mod dividend divisor = Z.rem dividend divisor
...
let toshift shiftedt v =
  let max = bits_of_width shiftedt and i = to_big_int v in
  assert (i >=% bi0);
  let max_bi = big_int_of_int max in
  if i <=% max_bi then int_of_big_int i
  else
    let error_str =
      Stdlib.Printf.sprintf "shifting %d-bit value by %s" max
        (string_of_big_int i)
    in
    let () = failwith error_str in
    max

(* "cast" an int64 to a value *)
let to_val t i = (to_big_int (i, t), t)

let exp_bool =
  let t = (unit_big_int, Reg 1) and f = (zero_big_int, Reg 1) in
...
let cast ct ((_, t) as v) t2 =
  let bits1 = bits_of_width t and bits = bits_of_width t2 in
  match ct with
  | CAST_UNSIGNED -> to_val t2 (to_big_int v)
  | CAST_SIGNED -> to_val t2 (to_sbig_int v)
  | CAST_HIGH -> to_val t2 (shift_right_big_int (to_big_int v) (bits1 - bits))
  | CAST_LOW -> to_val t2 (to_big_int v)
