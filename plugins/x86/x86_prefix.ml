...
let deref mem =
  Seq.unfold_step ~init:(Memory.min_addr mem) ~f:(fun addr ->
...
let get memory =
  Seq.(
    deref memory >>| Word.to_int >>| ok_exn >>| to_known_prefix
    |> take_while ~f:Option.is_some
    |> filter_opt |> to_list)

let exists pref mem = List.mem ~equal:[%compare.equal: t] (get mem) pref
