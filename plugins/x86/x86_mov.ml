...
module Make (Tools : X86_tools.S) (Backend : X86_backend.S) = struct
  ...
  let mov_mi (op : mov_mi) =
    X86_operands.mi ~f:(fun mem ~seg ~base ~scale ~index ~disp imm ->
        ...
        let size =
          ...
          | `MOV64mi32 -> `r64
        in
        Ok [ IM.get ~width:size imm |> MM.store mem ~size ])
  ...
  let mov_rs (op : mov_rs) =
    X86_operands.rr ~f:(fun _mem reg seg ->
        ...
        let bil =
          match (op, RR.size) with
          | `MOV16rs, _ | `MOV32rs, `r64 -> RR.get seg |> RR.set reg
          | `MOV32rs, `r32 | `MOV64rs, _ ->
              let size = RR.size |> Size.in_bits in
              RR.get seg |> Bil.(cast unsigned size) |> RR.set reg
        in
        Ok [ bil ])
    ...
    X86_operands.rr ~f:(fun _mem seg reg ->
        ...
        let bil =
          ...
          | `MOV32sr | `MOV64sr ->
              ...
              Bil.(cast low width (var reg)) |> RR.set seg
        in
        Ok [ bil ])
  ...
  let register what =
    ...
    List.iter
      (what :> mov list)
      ~f:(fun op -> Backend.register (name op) (mov op))
end
