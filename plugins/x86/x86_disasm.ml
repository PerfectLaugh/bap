...
let parse_instr mode mem addr =
  ...
  let g a =
    ...
    Memory.get ~addr:a mem >>= Word.to_int |> ok_exn |> Char.of_int_exn
  in
  ...
  let get_prefixes a =
    let rex a =
      ...
      | X8664 ->
          ...
          if i >= 0x40 && i <= 0x4f then (Some i, s a) else (None, a)
    in
    let rec f l a =
      if Addr.(a > Memory.max_addr mem) then (l, a)
      else
        match get_prefix (g a) with Some p -> f (p :: l) (s a) | None -> (l, a)
    in
    (* Legacy prefixes *)
    let leg, a = f [] a in
    (* Add rex *)
  ...
  in
  let get_vex a =
    if [%compare.equal: mode] mode X86 then (None, a)
    else
  ...
  let parse_modrmbits a =
    let b = Char.to_int (g a) and na = s a in
    let r = (b lsr 3) land 7 and m = b lsr 6 and rm = b land 7 in
    (b, r, m, rm, na)
  in
  ...
  let parse_modrm16int _a _b r modb rm na =
    (* ISR 2.1.5 Table 2-1 *)
    match modb land 7 with
    (* MOD *)
    | 0 -> (
        match rm land 7 with
        | 6 ->
            let disp, na = parse_disp16 na in
            (r, Oaddr (Bil.Int (bm disp)), na)
        | n when n < 8 ->
            (r, Oaddr Bil.(cast unsigned !!tm (eaddr16 mode rm)), na)
  ...
  let parse_modrm3264int rex at _ia _a _b r modb rm naoffset na =
    ...
    let bits2rege =
      ...
      | Imm 32 ->
          fun b ->
            Bil.(cast unsigned !!(type_of_mode mode) (bits2reg32e mode b))
    ...
    | 0 -> (
        ...
        | 4 ->
            let sib, na = parse_sib rex modb na in
            (r, Oaddr sib, na)
        | 5 -> (
          ...
          *)
            match mode with
            | X86 ->
                let disp, na = parse_disp32 na in
                (r, Oaddr (b32 disp), na)
            | X8664 ->
                let immoff =
                  match naoffset with
                ...
                in
                let disp, na = parse_disp32 na in
                (r, Oaddr Bil.(b64 disp + b64 na + immoff), na))
        | _ -> (r, Oaddr (bits2rege rm), na))
    | 1 | 2 ->
        let base, na =
          (* rm & 7 ignores the extended rex bit *)
          if 4 = rm land 7 then parse_sib rex modb na else (bits2rege rm, na)
        in
        let disp, na =
          if modb = 1 then parse_disp8 na else (*2*) parse_disp32 na
        in
  ...
  let tovec op = match op with Oreg r -> Ovec r | _ -> op in
  let toreg op = match op with Ovec r -> Oreg r | _ -> op in
  ...
  let expanded_jump_type opsize =
    match mode with X86 -> opsize | X8664 -> reg32_t
  in
  let get_opcode _pref ({ rex; vex; rm_extend; addrsize; _ } as prefix) a =
    let ( parse_disp_addr,
      ...
      | Imm 16 ->
          ( parse_disp16,
            parse_modrm16 rex,
            parse_modrm16seg rex,
            parse_modrm16ext rex )
      | Imm 32 ->
          ( parse_disp32,
            parse_modrm3264 rex vex addrsize a,
            parse_modrm3264seg rex vex addrsize a,
            parse_modrm3264ext rex vex addrsize a )
      | Imm 64 ->
          ( parse_disp64,
            parse_modrm3264 rex vex addrsize a,
            parse_modrm3264seg rex vex addrsize a,
    ...
    let b1, na =
      if Option.is_some vex then (0x0f, a) else (Char.to_int (g a), s a)
    in
    match b1 with
    (* Table A-2 *)
    ...
    | 0x68 | 0x6a ->
        ...
        in
        let size = match mode with X86 -> prefix.opsize | X8664 -> reg64_t in
        (Push (size, o), na)
    | 0x69 | 0x6b ->
        let it =
          if b1 = 0x6b then reg8_t
    ...
    | 0x80 | 0x81 | 0x82 | 0x83 -> (
        let it =
          match b1 with
          | 0x81 ->
              if Type.equal prefix.opsize reg64_t then reg32_t
              else prefix.opsize
          | _ -> reg8_t
        in
        ...
        | _ ->
            disfailwith (Printf.sprintf "impossible Grp 1 opcode: %02x/%d" b1 r)
        )
    | 0x84 | 0x85 ->
        let r, rm, na = parse_modrm_addr None na in
    ...
    | 0x8c ->
        let r, rm, na = parse_modrmseg_addr None na in
        let extend =
          if Type.equal prefix.opsize reg64_t then reg64_t else reg16_t
        in
    ...
    | 0xa9 ->
        let it =
          if Type.equal prefix.opsize reg64_t then reg32_t else prefix.opsize
        in
    ...
    | 0xd8 | 0xd9 | 0xda | 0xdb | 0xdc | 0xdd | 0xde | 0xdf -> (
        ...
        | _, _ ->
            unimplemented
              (Printf.sprintf "unsupported opcode: %02x %s" b1
                 (Word.string_of_value b2)))
    | 0xe8 ->
    ...
    | 0xc0 | 0xc1 | 0xd0 | 0xd1 | 0xd2 | 0xd3 -> (
        let immoff = if b1 land 0xfe = 0xc0 then Some reg8_t else None in
        let r, rm, na = parse_modrmext_addr immoff na in
        let opsize = if b1 land 1 = 0 then reg8_t else prefix.opsize in
        let amt, na =
          match b1 land 0xfe with
          | 0xc0 -> parse_imm8 na
          | 0xd0 -> (Oimm Addr.b1, na)
          | 0xd2 -> (o_rcx, na)
          | _ -> disfailwith (Printf.sprintf "impossible opcode: %02x/%d" b1 r)
        in
        let open Bil in
        match r with
        (* Grp 2 *)
    ...
    | 0xe3 ->
        ...
        ( Jcc
            ( Jrel (resize_word na t, resize_word i t),
              Bil.(rcx_e = (mi 0 |> int)) ),
          na )
    | 0xf4 -> (Hlt, na)
    | 0xf6 | 0xf7 -> (
        let t = if b1 = 0xf6 then reg8_t else prefix.opsize in
    ...
    | 0xff -> (
        ...
        | 6 ->
            let size =
              match mode with X86 -> prefix.opsize | X8664 -> reg64_t
            in
            (Push (size, rm), na)
        | _ -> disfailwith (Printf.sprintf "impossible opcode: %02x/%d" b1 r))
    (*** 00 to 3e ***)
    ...
    | 0x0f -> (
        ...
        let b2, na =
          ...
          | Some { vex_map_select; _ } ->
              disfailwith
                (Printf.sprintf "reserved mmmmmm vex value: %d" vex_map_select)
        in
        match b2 with
        (* Table A-3 *)
        | 0x01 -> (
        ...
        | (0x10 | 0x11) when prefix.repeat || prefix.nrepeat -> (
            ...
            let d, s, td =
              if b2 = 0x10 then (r, rm, reg128_t) else (rm, r, t)
            in
            match (rm, rv) with
            | Ovec _, Some rv ->
                let nt = !!t in
                ( Movoffset
                  ...
                  na )
            | Ovec _, None -> (Movdq (t, s, t, d, false), na)
            | Oaddr _, _ -> (Movdq (t, s, td, d, false), na)
            | _ -> disfailwith "impossible")
        | 0x12 | 0x13 | 0x16 | 0x17 ->
            (* MOVLPS, MOVLPD, MOVHPS, MOVHPD, MOVHLPS, MOVHLPD, MOVLHPS, MOVLHPD *)
            let r, rm, rv, na = parse_modrm_vec None na in
            let tdst, dst, telt, tsrc1, src1, off_src1, off_dst1, src2 =
              match b2 with
              | (0x12 | 0x16) when Option.is_some rv ->
                  let offs1, offs2, offd1, offd2 =
                    match (b2, rm) with
                  ...
                  [
                    {
                      ...
                      offtyp = tsrc1;
                      offop = src1;
                      offsrcoffset = off_src1;
                      offdstoffset = off_dst1;
                    };
                  ]
                  @ src2 ),
              na )
        | 0x10 | 0x11 | 0x28 | 0x29 | 0x6e | 0x7e | 0x6f | 0x7f | 0xd6 ->
            ...
            let src, dst, tsrc, tdst, align =
              ...
              | 0x10 | 0x11 | 0x28 | 0x29 ->
                  ...
                  in
                  let t =
                    if Type.equal prefix.mopsize reg256_t then reg256_t
                    else reg128_t
                  in
                  (s, d, t, t, align)
              | 0x6e | 0x7e ->
                  (* MOVD, MOVQ *)
                  let t =
                    if Type.equal prefix.opsize reg64_t then reg64_t
                    else reg32_t
                  in
                  let s, d, ts, td =
                    match b2 with
              ...
              | 0x6f | 0x7f ->
                  ...
                  in
                  let size =
                    if prefix.repeat && Option.is_none prefix.vex then reg128_t
                    else prefix.mopsize
                  in
        ...
        | 0x38 -> (
            ...
            | 0x29 when prefix.opsize_override ->
                let r, rm, rv, na = parse_modrm_vec None na in
                ( Pcmp (prefix.mopsize, Type.imm 64, Bil.EQ, "pcmpeq", r, rm, rv),
                  na )
            | 0x20 | 0x21 | 0x22 | 0x23 | 0x24 | 0x25 | 0x30 | 0x31 | 0x32
            | 0x33 | 0x34 | 0x35
              when prefix.opsize_override ->
                (* pmovsx and pmovzx *)
                let r, rm, _, na = parse_modrm_vec None na in
                (* determine sign/zero extension *)
                let ext, name =
                  match b3 land 0xf0 with
                  | 0x20 -> (Bil.signed, "pmovsx")
                  | 0x30 -> (Bil.unsigned, "pmovzx")
                  | _ -> disfailwith "impossible"
                in
                (* determine dest/src element size *)
                let dstet, srcet, fullname =
                  match b3 land 0x0f with
            ...
            | 0x37 when prefix.opsize_override ->
                let r, rm, rv, na = parse_modrm_vec None na in
                ( Pcmp
                    (prefix.mopsize, Type.imm 64, Bil.SLT, "pcmpgt", r, rm, rv),
                  na )
            ...
            | 0x3f when prefix.opsize_override ->
                ...
                ( Ppackedbinop
                  ...
                  na )
            | _ ->
                disfailwith (Printf.sprintf "opcode unsupported: 0f 38 %02x" b3)
            )
        | 0x3a -> (
            let b3 = Char.to_int (g na) and na = s na in
            match b3 with
            ...
            | 0x60 | 0x61 | 0x62 | 0x63 -> (
                ...
                | Oimm imm ->
                    ...
                    let pcmp =
                      {
                        out = (if b3 land 0x1 = 0x1 then Index else Mask);
                        len = (if b3 land 0x2 = 0x2 then Implicit else Explicit);
                      }
                    in
        ...
        | 0x71 | 0x72 | 0x73 ->
            ...
            let fbop, str, et, i =
              ...
              | 0x73, Ovec 3, Oimm i when prefix.opsize_override ->
                  (Bil.binop RSHIFT, "psrldq", t, Oimm Addr.(i * bi8))
              | 0x73, Ovec 7, Oimm i when prefix.opsize_override ->
                  (Bil.binop LSHIFT, "pslldq", t, Oimm Addr.(i * bi8))
              | _, Oreg i, _ ->
                  disfailwith
                    (Printf.sprintf "invalid psrl/psll encoding b2=%#x r=%#x" b2
                       i)
              | _ -> disfailwith "impossible"
            in
        ...
        | 0xb6 | 0xb7 ->
            let st = if b2 = 0xb6 then reg8_t else reg16_t in
            let r, rm, na = parse_modrm_addr None na in
        ...
        | 0xbe | 0xbf ->
            let st = if b2 = 0xbe then reg8_t else reg16_t in
            let r, rm, na = parse_modrm_addr None na in
        ...
        | 0xf0 ->
            let r, rm, _, na = parse_modrm_vec None na in
            let t =
              if Type.equal prefix.mopsize reg256_t then reg256_t else reg128_t
            in
            (Movdq (t, rm, t, r, false), na)
        | 0xf8 | 0xf9 | 0xfa | 0xfb ->
            let r, rm, rv, na = parse_modrm_vec None na in
            let eltsize =
              match b2 land 7 with
              | 0 -> reg8_t
              | 1 -> reg16_t
              | 2 -> reg32_t
              | 3 -> reg64_t
            ...
            ( Ppackedbinop
                (prefix.mopsize, eltsize, Bil.( - ), "psub", r, rm, rv),
              na )
        | 0xfc | 0xfd | 0xfe | 0xd4 ->
            let r, rm, rv, na = parse_modrm_vec None na in
            let eltsize =
            ...
            in
            ( Ppackedbinop
                (prefix.mopsize, eltsize, Bil.( + ), "padd", r, rm, rv),
              na )
        | _ ->
            unimplemented (Printf.sprintf "unuspported opcode: %02x %02x" b1 b2)
        )
  ...
  let pref =
    ...
    | _ -> pref
  in
  let rex = match rex with Some v -> Some (parse_rex v) | None -> None in
  ...
  let opsize, bopsize, mopsize =
    if ints_mem pref pref_opsize then (reg16_t, reg16_t, reg128_t)
    else (reg32_t, modesize, reg64_t)
  in
  let opsize =
    match rex with
    | Some { rex_w = true; _ } ->
        reg64_t
        (* See Table 3-4: Effective Operand-
                                         and Address-Size Attributes in 64-Bit Mode *)
    | Some { rex_w = false; _ } | None -> opsize
  in
  let opsize =
    match vex with Some { vex_we = true; _ } -> reg64_t | _ -> opsize
  in
  let mopsize =
    match vex with
  ...
  let r_extend, rm_extend, sib_extend =
    let e b = (if b then 1 else 0) lsl 3 in
    match rex with
    | Some { rex_r; rex_b; rex_x; _ } -> (e rex_r, e rex_b, e rex_x)
    | None -> (
        match vex with
        | Some { vex_nr; vex_nb; vex_nx; _ } ->
            (e (not vex_nr), e (not vex_nb), e (not vex_nx))
        | None -> (0, 0, 0))
  in
...
let parse_prefixes mode pref _ =
  ...
  let rec f s r = function
    | [] -> ((match s with Some v -> Some v | None -> None), List.rev r)
    | 0x2e :: p -> f seg_cs r p
    | 0x36 :: p -> f seg_ss r p
    | 0x3e :: p -> f seg_ds r p
    | 0x26 :: p -> f seg_es r p
