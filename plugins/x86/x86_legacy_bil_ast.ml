...
(** The Abstract Syntax Tree. This IL allows nested expressions, making it
    closer to VEX and the concrete syntax than our SSA form. However, in most
    cases, this makes analysis harder, so you will generally want to convert to
    SSA for analysis.
...
let sexp_of_big_int bi = Big_int_Z.string_of_big_int bi |> Core.String.sexp_of_t
...
type stmt =
  ...
  | Special of (string * defuse option * attrs)
...
(** If possible, make a label that would be refered to by the given expression.
*)
let lab_of_exp =
...
(** False constant. (If convenient, refer to this rather than building your
    own.) *)
