...
let funop_to_string = function
  ...
  | FBVTOUF { exp_bits; sig_bits } ->
      Printf.sprintf "bvtouf(%d, %d)" exp_bits sig_bits
  | FBVTOSF { exp_bits; sig_bits } ->
      Printf.sprintf "bvtosf(%d, %d)" exp_bits sig_bits
  | FFTOF { exp_bits; sig_bits } ->
      Printf.sprintf "ftof(%d, %d)" exp_bits sig_bits
  | FIEEEBVTOF { exp_bits; sig_bits } ->
      Printf.sprintf "fIEEEtof(%d, %d)" exp_bits sig_bits
  | FNAN { exp_bits; sig_bits } ->
      Printf.sprintf "fnan(%d, %d)" exp_bits sig_bits
...
let option_may ~f = function Some x -> f x | None -> ()
...
let var_to_string ?ctx (Var.V (id, name, t) as v) =
  match ctx with
  | None -> name ^ "_" ^ string_of_int id ^ ":" ^ typ_to_string t
  | Some (vars, names) -> (
      if
        (not !printed_varctx_warning)
        && (Hashtbl.length names > reasonable_size_varctx
           || Hashtbl.length vars > reasonable_size_varctx)
      then printed_varctx_warning := true;
      (*        Logs.tbd_s "varctx is getting very large"); *)
      match Hashtbl.find vars v with
      | Some s -> s
      | None ->
          let rec trystring (s, `F next) =
            if Hashtbl.mem names s then trystring (next s)
            else
          ...
          trystring
            (name, `F (fun _ -> (name ^ "_" ^ string_of_int id, `F more))))
...
class pp ft =
  ...
  let vctx =
    ( VH.create ~size:100 (),
      Hashtbl.Using_hashable.create ~hashable:Hashtbl.Poly.hashable ~size:100 ()
    )
  in
  ...
  object (self)
    method var v =
      if !output_varnums then pp (var_to_string v)
      else pp (var_to_string ~ctx:vctx v)
    ...
    method attr =
      ...
      | Context
          ...
          let ustr = match u with RD -> "rd" | RW -> "rw" | WR -> "wr" in
          ...
          pc '"';
          pp
            (" = 0x" ^ big_int_to_hex v ^ ", " ^ ts ^ ", u" ^ string_of_int bits
           ^ ", " ^ ustr)
      | Context _ -> failwith "Contexts only specify register types"
      | ThreadId i ->
          pp "@tid \"";
          pp (string_of_int i);
          pp "\""
      ...
      | TaintIntro (tid, src_name, offset) ->
          pp "@taint_intro ";
          pp (string_of_int tid);
          pp ", ";
          pc '"';
          pp src_name;
          pp "\", ";
          pp (string_of_int offset)
    ...
    method int i t =
      let is, i =
        (Arithmetic.to_sbig_int (i, t), Arithmetic.to_big_int (i, t))
      in
      match (is, t) with
      | bi, Reg 1 when bi_is_zero bi -> pp "false"
      | bi, Reg 1 when bi_is_minusone bi -> pp "true"
      | bi, t ->
          if abs_big_int bi <% bia then pp (string_of_big_int bi)
    ...
    *)
    method ast_exp ?(prec = 0) e =
      ...
      | Ast.UnOp (u, x) ->
          lparen 100;
          pp (unop_to_string u);
          self#ast_exp ~prec:100 x;
          rparen 100
      | Ast.Var v -> self#var v
      | Ast.Lab s ->
          (* FIXME: quote s? *)
          pp "\"";
          pp s;
          pp "\""
      | Ast.Int (i, t) -> self#int i t
      | Ast.Cast (ct, t, e) ->
          pp (ct_to_string ct);
          pp ":";
          self#typ t;
    ...
    method ast_endian =
      function
      | Ast.Int (bi, Reg 1) when bi_is_zero bi -> pp "e_little"
      | Ast.Int (bi, Reg 1) when bi_is_one bi -> pp "e_big"
      | x -> self#ast_exp x
    ...
    method close = Format.pp_print_newline ft ()
  end
