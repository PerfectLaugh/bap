...
(** Utility functions for ASTs. It's useful to have these in a separate file so
    it can use functions from Typecheck and elsewhere. *)
...
let cjmp c t = CJmp (c, t, [])

(* exp helpers *)
let unknown t s = Unknown (s, t)
...
let rec _maybe_float = function
  ...
  | Unknown (_, Float _) ->
      true
  | Let (_, _, x) | Ite (_, x, _) -> _maybe_float x
  | _ -> false
...
let ieeebvtof ?(rm = RNE) ~float_size a =
  unop (FP (FIEEEBVTOF float_size, rm)) a
...
let cast ct tnew = function
  | Int (i, t) ->
      let i', t' = Arithmetic.cast ct (i, t) tnew in
      Int (i', t')
...
let it i t =
  match t with
  | Reg n -> Int (int_to_big_uint ~width:n ~value:i, t)
...
let exp_ite ?t b e1 e2 =
  ...
  (match t with None -> () | Some t -> assert (Poly.( = ) t t1));
