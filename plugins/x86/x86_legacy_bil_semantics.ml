...
module Binary = struct
  ...
  let of_bv s n e k =
    Bil.UnOp (FP (FIEEEBVTOF s, rm_unused), Concat (n, Concat (e, k)))
  ...
  let fextract ?bias { Theory.IEEE754.w; p; k } x ~hi ~lo =
    ...
    match bias with None -> x | Some b -> BinOp (MINUS, x, b)

  let unbiased_exponent ({ Theory.IEEE754.bias; t; w } as p) x =
    fextract p x ~hi:(t + w) ~lo:t ~bias:(Int (Z.of_int (bias + t), Reg w))

  let unbiased_exponent ({ Theory.IEEE754.t; w } as p) x =
    fextract p x ~hi:(t + w) ~lo:t

  let fraction ({ Theory.IEEE754.t } as p) x = fextract p x ~hi:(t - 1) ~lo:0
  ...
  let is_nan ({ Theory.IEEE754.w; t } as p) x =
    ...
    Bil.BinOp (AND, BinOp (EQ, uexp, ones), BinOp (NEQ, frac, zero))
end

let bits_of_var (_, _, t) =
  match t with Type.Reg x -> x | _ -> invalid_arg "expects a bitvector"
...
module Semantics = struct
  ...
  let fresh_label = Knowledge.Object.create lbl >>= Knowledge.Object.repr lbl

  module Domain = struct
    let stmt =
      Knowledge.Domain.flat "stmt" ~empty:[]
        ~inspect:(function
          | [] -> Sexp.List [] | prog -> Sexp.Atom (ast_prog_to_string prog))
        ~equal:Poly.( = )

    let exp =
      Knowledge.Domain.optional "exp"
        ~inspect:(fun exp -> Sexp.Atom (ast_exp_to_string exp))
        ~equal:Poly.( = )
  end

  let stmt =
    Knowledge.Class.property Theory.Semantics.cls "legacy-bil-stmt" Domain.stmt
      ~public:true ~package

  let exp =
    Knowledge.Class.property Theory.Value.cls "legacy-bil-exp" Domain.exp
      ~public:true ~package
  ...
  module Base = struct
    ...
    let bool_exp : _ -> Bil.exp =
     fun x ->
      match value x with None -> Unknown ("bool", bool_t) | Some x -> x

    let bitv_exp : _ -> Bil.exp =
     fun x ->
      match value x with
    ...
    let type_of_sort s =
      ...
      | None -> (
          ...
          | None -> (
              ...
              | None -> None
              | Some ms ->
                  let ks = Theory.Mem.keys ms and vs = Theory.Mem.vals ms in
    ...
    let var r =
      ...
      | None -> unk s
      | Some t -> create_var (Var.name r) t >>= fun v -> exp s (Var v)

    let zeros = Bil.Int (Z.of_int 0, Reg 1)
    let ones = Bil.Int (Z.of_int 1, Reg 1)
    let b0 = bit zeros
    let b1 = bit ones
    let int s w = exp s @@ Bil.Int (Bitvec.to_bigint w, Type.Reg (size s))
    let vsort x = sort x
    ...
    let lift1 s f v =
      v >>-> fun sort -> function
      | None -> unk (s sort) | Some x -> exp (s sort) (f x)
    ...
    let branch cnd yes nay =
      ...
      eff
        ...
        @ effect_ yes
        @ Bil.[ Jmp (Lab bot, []); Label (Name rhs, []) ]
        @ nay
        @ Bil.[ Label (Name bot, []) ])

    let make_cast s t x =
      x >>-> fun _ -> function
      | Some x -> exp s Bil.(Cast (t, Reg (size s), x)) | None -> unk s
    ...
    let append s ex ey =
      ...
      | _ ->
          if size s < size sx then exp s (Cast (CAST_LOW, Reg (size s), x))
    ...
    let concat s vs =
      ...
      | _ ->
          ...
          let sz = List.fold ~init:0 vs ~f:(fun sz x -> sz + size (vsort x)) in
          let x =
            List.reduce_exn ~f:(fun x y -> Bil.Concat (x, y))
            @@ List.map vs ~f:bitv_exp
          in
    ...
    let seq x y =
      x >>= fun x ->
      y >>= fun y -> eff (effect_ x @ effect_ y)

    let blk lbl x y =
      x >>:= fun x ->
      y >>:= fun y ->
      if KB.Object.is_null lbl then eff (x @ y)
      else
    ...
    let set var rhs =
      ...
      | _ ->
          data [ Bil.Special (sprintf "(set %s <?>)" (Var.name var), None, []) ]
    ...
    let repeat cnd body =
      ...
      data
        (Bil.
           [ Label (Name head, []); Jmp (Lab tail, []); Label (Name loop, []) ]
        @ effect_ body
        @ Bil.[ Label (Name tail, []); CJmp (bool_exp cnd, Lab head, []) ])

    let jmp dst = dst >>= fun dst -> ctrl [ Bil.Jmp (bitv_exp dst, []) ]
    ...
    let goto lbl : _ knowledge =
      ...
      | Some bitv ->
          make_addr lbl bitv >>= fun dst -> ctrl Bil.[ Jmp (dst, []) ]
      | None -> (
              ...
              let dst =
                ...
                | None ->
                    Format.asprintf "%%%08Lx"
                      (Int63.to_int64 (KB.Object.id lbl))
              in
  ...
  let loadw rs cnd mem key =
    ...
    mem >>-> fun _ -> function
    | None -> unk rs | Some mem -> exp rs @@ Load (mem, key, dir, Reg (size rs))
  ...
  let of_bv s n e k =
    Bil.UnOp (FP (FIEEEBVTOF (fsize s), RNE), Concat (n, Concat (e, k)))

  let known_ieee754_formats =
    Theory.IEEE754.[ binary16; binary32; binary64; binary80; binary128 ]
  ...
  let with_ieee x ~ieee754 ~default =
    x >>-> fun xs x ->
    match (x, ieee754_of_sort xs) with
  ...
  let is_fzero x =
    with_ieee x
      ...
      ~ieee754:(fun ({ w; t } as p) x ->
        ...
        exp bool
        @@ BinOp (AND, BinOp (EQ, uexp, zero w), BinOp (EQ, frac, zero t)))
  ...
  let is_inf x =
    with_ieee x
      ...
      ~ieee754:(fun ({ w; t } as p) x ->
        ...
        let ones = Bil.Int (Z.minus_one, Reg w) in
        let zero = Bil.Int (Z.zero, Reg t) in
        exp bool @@ BinOp (AND, BinOp (EQ, uexp, ones), BinOp (EQ, frac, zero)))
  ...
  let fbits x =
    ...
    | _ -> exp bs @@ Unknown (sortname fs, Reg (size bs))

  type 'a t = 'a knowledge

  let float_bitv_uop : type a.
      (Type.float_size -> Type.funop_type) -> _ sort -> _ -> _ t -> _ t =
   fun op s m x ->
    match ieee754_of_sort s with
    | Some { Theory.IEEE754.base = 2; w; p } ->
        let fs = { Type.sig_bits = p; exp_bits = w } in
        x >>| bitv_exp >>= fun x -> exp s @@ UnOp (FP (op fs, RNE), x)
    | _ -> ret (None %: s)

  let bitv_float_uop : type a.
      (int -> Type.funop_type) -> _ sort -> _ -> _ t -> _ t =
   fun op s m x ->
    x >>-> fun xs x ->
    match (ieee754_of_sort s, x) with
  ...
  let fuop op x =
    x >>-> fun xs x ->
    match (ieee754_of_sort xs, x) with
  ...
  let exp _ x = x >>-> fun xs _ -> ret (None %: xs)
end
...
let init () =
  ...
  Theory.declare ~package ~desc:"semantics in legacy AST" ~name:"legacy-bil"
