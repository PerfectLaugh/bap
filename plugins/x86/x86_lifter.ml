...
module ToIR = struct
  ...
  (** doubts here *)
  let size_of_typ s = Size.of_int_exn !!s
  ...
  let store_s mode s t a e =
    let mem = match mode with X86 -> R32.mem | X8664 -> R64.mem in
    let sz = size_of_typ t in
    match s with
    | None -> Bil.Move (mem, Bil.(Store (Var mem, a, e, LittleEndian, sz)))
    | Some v ->
        Bil.Move (mem, Bil.(Store (Var mem, Var v + a, e, LittleEndian, sz)))
  ...
  let op2e_s_keep_width mode ss has_rex t operand =
    ...
    | Ovec _ ->
        let i = match t with Type.Imm n -> ": " ^ string_of_int n | _ -> "" in
    ...
    | Oseg r when t = reg64_t ->
        Bil.(Cast (UNSIGNED, !!reg64_t, bits2segrege r), t)
  ...
  let op2e_s mode ss has_rex t operand =
    ...
    | Oreg _ -> unimplemented mode "unknown register"
    | Oseg r when t = reg64_t ->
        Bil.(Cast (UNSIGNED, !!reg64_t, bits2segrege r))
    (* There is no 32-bit extension of segment selectors; this is not a bug *)
  ...
  let assn_s mode s has_rex has_vex t v e =
    ...
    let sub_assn ?(off = 0) t v e =
      ...
      if ubh > ubl then
        concat_exps := Bil.(Extract (ubh, ubl, Var v)) :: !concat_exps;
      ...
      let final_e =
        List.reduce_exn ~f:(fun big_e e -> Bil.Concat (e, big_e)) !concat_exps
      in
      Bil.Move (v, final_e)
    in
    ...
    | Oreg r, Type.Imm (64 | 32 | 16) ->
        ...
        sub_assn t v e
    | Oreg r, Type.Imm 8
      when r < 4 || ([%compare.equal: mode] mode X8664 && has_rex) ->
        let v = bits2genreg mode r in
        sub_assn t v e
    | Oreg r, Type.Imm 8 ->
    ...
    | Oseg r, _ when Type.equal t reg16_t ->
        ...
        Bil.Move (v, e)
    | Oseg _, _ ->
        disfailwith mode "Can't assign to non 16 bit segment register"
    | Oaddr a, _ -> store_s mode s t a e
    | Oimm _, _ ->
        disfailwith mode "disasm x86: Can't assign to an immediate value"
  ...
  let op2e_dbl_s mode ss has_rex t =
    ...
    List.reduce_exn ~f:(fun bige little -> Bil.(bige ^ little)) ol

  (* Double width assignments, as used by multiplication *)
  let assn_dbl_s mode s has_rex has_vex t e =
    match op_dbl t with
    | (t, o) :: [] ->
        ([ assn_s mode s has_rex has_vex t o e ], op2e_s mode s has_rex t o)
    | l ->
        let tmp = tmp (Type.Imm (!!t * 2)) in
        let f (stmts, off) (ct, o) =
          let newoff = off + !!ct in
          ( assn_s mode s has_rex has_vex ct o
              (Bil.Extract (newoff - 1, off, Bil.Var tmp))
            :: stmts,
            newoff )
        in
        ( List.rev
            (fst
               (List.fold_left ~f ~init:([ Bil.Move (tmp, e) ], 0) (List.rev l))),
          Bil.Var tmp )
    ...
    exp_not
      (Cast
         ( LOW,
           ...
           Let
             ( acc,
               (r lsr int_exp 4 t') lxor r,
               Let
                 ( acc,
                   (var_acc lsr int_exp 2 t') lxor var_acc,
                   (var_acc lsr int_exp 1 t') lxor var_acc ) ) ))
  ...
  let set_pszf t r =
    let t' = !!t in
    [ set_pf t r; set_sf r; set_zf t' r ]
  ...
  *)
  ...
  let set_apszf t s1 s2 r =
    ...
    let t = Type.Imm t in
    Bil.Move (af, Bil.(bit4 = bit4 land (r lxor s1 lxor s2))) :: set_pszf t r
  ...
  let set_flags_add t s1 s2 r =
    Bil.Move (cf, Bil.(r < s1)) :: set_aopszf_add t s1 s2 r
  ...
  let set_flags_sub t s1 s2 r =
    Bil.Move (cf, Bil.(s2 > s1)) :: set_aopszf_sub t s1 s2 r

  let popcount x ~one ~two ~four ~sh ~m1 ~m2 ~m4 ~h01 =
    Bil.
      [
  ...
  let ctz16 x =
    Bil.(x := (var x land unop NEG (var x)) - int Word.(one 16)) :: popcnt16 x

  let ctz32 x =
    Bil.(x := (var x land unop NEG (var x)) - int Word.(one 32)) :: popcnt32 x

  let ctz64 x =
    Bil.(x := (var x land unop NEG (var x)) - int Word.(one 64)) :: popcnt64 x
  ...
  let bitscan_flags is_zero_count =
    ...
    in
    if is_zero_count then l else Bil.(cf := unknown "bits" bool_t) :: l

  let rec to_ir mode addr next ss pref has_rex has_vex =
    let module R = (val vars_of_mode mode) in
    let open R in
    let load = load_s mode ss in
    (* Need to change this if we want seg_ds <> None *)
    ...
    let sign_extend_imm op t =
      let op_typ =
        ...
        | _ -> disfailwith "imm operand expected"
      in
      Bil.(cast signed (bitwidth_of_type t) (op2e op_typ op))
    in
    let is_repz = function [ x ] -> x = repz | _ -> false in
    let is_repnz = function [ x ] -> x = repnz | _ -> false in
    function
    | Nop -> []
    | Bswap (t, op) ->
        let e =
          match t with
          | Type.Imm 32 | Type.Imm 64 ->
              let op', t' = op2e_keep_width t op in
              reverse_bytes op' t'
          | _ -> disfailwith "bswap: Expected 32 or 64 bit type"
        in
        [ assn t op e ]
    | Retn (op, far_ret)
      when List.is_empty pref || is_repz pref || is_repnz pref ->
        let temp = tmp mt in
        let load_stmt =
          if far_ret then (* TODO Mess with segment selectors here *)
        ...
        let rsp_stmts =
          ...
          (match op with
          | None -> []
          | Some (t, src) -> [ Bil.Move (rsp, Bil.(rsp_e + op2e t src)) ])
        in
        (load_stmt :: rsp_stmts) @ [ Bil.Jmp (Bil.Var temp) ]
    | Mov (t, dst, src, condition) ->
        ...
        let base_e e =
          ...
          let index =
            Bil.(
              Cast (UNSIGNED, !!mt, Extract (15, 4, e) lsl Int (mi entry_shift)))
          in
          (* Load the table entry *)
          let table_entry =
            load_s mode None (size_of_typ entry_size) Bil.(base + index)
          in
          (* Extract the base *)
          concat_explist
            ((match mode with
        ...
        let bs =
          ...
          if [%compare.equal: operand] dst o_fs && !compute_segment_bases then
            [ Bil.Move (fs_base, base_e dst_e) ]
    ...
    | Movs (Type.Imm _bits as t) ->
        let stmts =
          [
            store_s mode seg_es t rdi_e
              (load_s mode seg_es (size_of_typ t) rsi_e);
            string_incr mode t rsi;
            string_incr mode t rdi;
          ]
        in
        if List.is_empty pref then stmts
        ...
        else unimplemented "unsupported prefix for movs"
    | Movzx (t, dst, ts, src) ->
        ...
        let al =
          if align then
            List.map
              ~f:(fun a ->
                Bil.If
                  ( Bil.(a land im 15 = im 0),
                    [],
                    [ Cpu_exceptions.general_protection ] ))
              al
          else []
        in
        d :: al
    | Movoffset ((tdst, dst), offsets) ->
        ...
        let offsets =
          List.sort
            ~compare:(fun { offdstoffset = o1; _ } { offdstoffset = o2; _ } ->
              Int.compare o1 o2)
            offsets
        in
        let add_exp (elist, nextbit)
            { offlen; offtyp; offop; offsrcoffset; offdstoffset } =
          ( Bil.Extract
              (!!offlen + offsrcoffset - 1, offsrcoffset, op2e offtyp offop)
    ...
    | Punpck (t, et, o, d, s, vs) ->
        ...
        let mape i =
          [
            extract_element et' (Bil.Var st) i;
            extract_element et' (Bil.Var dt) i;
          ]
        in
        let e =
          concat_explist
            (List.concat
               (List.map ~f:mape
                  (List.range ~stride:(-1) ~stop:`inclusive (nelem_per_src - 1)
                     0)))
        in
        let dest = match vs with None -> d | Some vdst -> vdst in
        [ Bil.Move (st, se); Bil.Move (dt, de); assn t dest e ]
            ...
            [
              ...
              foreach_size (fun i ->
                  [
                    ...
                    tmp_dst :=
                      var tmp_dst
                      lor cast unsigned t_width (var elt)
                          lsl (var iv * int bits);
                  ]);
    ...
    | Pmov (t, dstet, srcet, dst, src, ext, _) ->
        ...
        let e =
          concat_explist
            (List.map ~f:extend
               (List.range ~stride:(-1) ~stop:`inclusive (nelem - 1) 0))
        in
        [ assn t dst e ]
    | Pmovmskb (t, dst, src) ->
        ...
        let byte_indices = List.init ~f:(fun i -> i + 1) nbytes in
        (* list 1-nbytes *)
        let all_bits = List.rev (List.map ~f:get_bit byte_indices) in
        (* could also be done with shifts *)
        let padt = Type.Imm (32 - nbytes) in
        let or_together_bits =
          List.fold_left
            ~f:(fun acc i -> Bil.Concat (acc, i))
            ~init:(int_exp 0 !!padt) all_bits
        in
    ...
    | Palignr (t, dst, src, vsrc, imm) -> (
        ...
        let shift =
          Bil.(conct lsr Cast (UNSIGNED, !!t_concat, imm lsl int_exp 3 !!t))
        in
        let high, low =
          match t with
          | Type.Imm 256 -> (255, 0)
          ...
          | _ ->
              disfailwith
                "impossible: used non 64/128/256-bit operand in palignr"
        in
        let result = Bil.Extract (high, low, shift) in
        let im i = Bil.Int (int_of_mode mode i) in
        let addresses =
          List.fold
            ~f:(fun acc -> function Oaddr a -> a :: acc | _ -> acc)
            ~init:[] [ src; dst ]
        in
    ...
    | Pcmpstr (t, xmm1, xmm2m128, _imm, imm8cb, pcmpinfo) ->
        ...
        let nelem, _nbits, elemt =
          match imm8cb.ssize with
          | Bytes -> (16, 8, Type.imm 8)
          | Words -> (8, 16, Type.imm 16)
        in
        ...
        let implicit_check is_valid_xmm_i get_xmm_i =
          let f acc i =
            let previous_valid =
              if i = 0 then exp_true else Bil.var (is_valid_xmm_i (i - 1))
            in
            let current_valid = Bil.(get_xmm_i i <> int_exp 0 !!elemt) in
            let x = is_valid_xmm_i i in
            Bil.(x := previous_valid land current_valid) :: acc
          in
          List.fold ~f ~init:[] nelem_range
        in
        ...
        let explicit_check is_valid_xmm_i sizee =
          ...
          let f acc i =
            ...
            let x = is_valid_xmm_i i in
            Bil.(x := current_valid) :: acc
          in
          List.fold_left ~f ~init nelem_range
        in
        ...
        let is_valid =
          ...
          fun xmmnum index ->
            match Hashtbl.find vh (xmmnum, index) with
            | Some v -> v
            | None ->
        ...
        let xmm1_checks, xmm2_checks =
          ...
          | Explicit ->
              ( explicit_check is_valid_xmm1 rax_e,
                explicit_check is_valid_xmm2 rdx_e )
        in
        ...
        let get_bit index =
          ...
          | EqualAny ->
              ...
              Bil.(
                ...
                land List.fold ~f:check_char ~init:exp_false nelem_range)
          | Ranges ->
             ...
             xmm1[j+1]? *)
              let check_char acc j =
                ...
                let ind1 = ind0 + 1 in
                let rangevalid =
                  Bil.(is_valid_xmm1_e ind0 land is_valid_xmm1_e ind1)
                in
                let ( <= ) =
                  match imm8cb.ssign with
                  | Unsigned -> Bil.( <= )
                  | Signed -> Bil.( <=$ )
                in
                let ( land ) = Bil.( land ) in
                let inrange =
                  (get_xmm1 ind0 <= get_xmm2 index)
                  land (get_xmm2 index <= get_xmm1 ind1)
                in
                Bil.(rangevalid land (inrange lor acc))
              in
              Bil.(
                is_valid_xmm2_e index
                land List.fold_left ~f:check_char ~init:exp_false
                       (List.range ~stride:(-1) ~stop:`inclusive
                          Stdlib.((nelem / 2) - 1)
                          0))
          | EqualEach ->
          ...
          | EqualOrdered ->
              ...
              let check_char acc j =
               ...
               byte same => keep going  *)
                Bil.(
                  substrended lor (UnOp (NOT, bigstrended) land (equal land acc)))
              in
              (* Is xmm1[j] equal to xmm2[index+j]? *)
              List.fold_left ~f:check_char ~init:exp_true
                (List.range ~stride:(-1) ~stop:`inclusive (nelem - index - 1) 0)
        in
        ...
        let contains_null e =
          let elts =
            List.init nelem ~f:(fun i -> Bil.(get_elem e i = int_exp 0 !!elemt))
          in
          List.fold elts ~init:exp_false ~f:(fun acc elt -> Bil.(acc lor elt))
        in
        ...
        let sb exp =
          List.fold_left
            ~f:(fun acc i ->
              Bil.Ite
                (Bil.(exp_true = Extract (i, i, exp)), int_exp i !!regm, acc))
            ~init:(int_exp nelem !!regm)
            (match imm8cb.outselectsig with
            | LSB ->
                List.range ~stride:(-1) ~start:`exclusive ~stop:`inclusive nelem
                  0
        ...
        let mask e =
          ...
          | Bytemask ->
              let get_element i =
                Bil.(cast unsigned !!elemt (extract ~hi:i ~lo:i e))
              in
              let range =
                List.range ~stride:(-1) ~start:`exclusive ~stop:`inclusive nelem
                  0
              in
              concat_explist (List.map ~f:get_element range)
        in
        ...
        let res_of_cb =
          ...
          | { negintres1 = true; maskintres1 = true; _ } ->
             ...
             valid. *)
              let validvector =
                let range =
                  List.range ~stride:(-1) ~start:`exclusive ~stop:`inclusive
                    nelem 0
                in
                let bits = List.map ~f:is_valid_xmm2_e range in
                Bil.(cast unsigned 16 (concat_explist bits))
              in
              Bil.(int_res_2 := validvector lxor var int_res_1)
        in

        let res_of_pcmpinfo =
          match pcmpinfo.out with
          | Index -> Bil.(rcx := sb (var int_res_2))
          (* FIXME: ymms should be used instead of xmms here *)
          | Mask -> Bil.(ymms.(0) := mask (var int_res_2))
        in
    ...
    | Pshufd (t, dst, src, vsrc, imm) -> (
        ...
        let get_dword ndword =
          ...
          let index =
            Bil.(Cast (UNSIGNED, !!t, Extract (high_b, low_b, imm_e)))
          in
          let t' = !!t in
         ...
         * going wrong later. *)
          let index, index_width =
            if t' = 256 && ndword > 3 then
              (Bil.(index + Int (BV.of_int ~width:!!t 4)), 256)
            else (index, t')
          in
          extract_element_symbolic_with_width (Type.imm 32) src_e index
            index_width
        in
        let topdword = match t with Type.Imm 128 -> 3 | _ -> 7 in
        let dwords =
          concat_explist
            (List.map ~f:get_dword
               (List.range ~stride:(-1) ~stop:`inclusive topdword 0))
        in
        match vsrc with
            ...
            foreach_byte (fun i ->
                Bil.
                  [
                    ...
                    if_
                      (msb_one land var mask_byte_i = msb_one)
                      [ tmp_byte := zero ]
                      (* else *)
                      [
                        ind :=
                          cast unsigned 8
                            (extract ~hi:index_bits ~lo:0 (var mask_byte_i));
                        tmp_byte :=
                          cast unsigned op_size
                            (extract ~hi:7 ~lo:0 (dst lsr (var ind * byte)));
                      ];
                    (tmp_dst :=
                       Bil.(var tmp_dst lor (var tmp_byte lsl (var iv * byte))));
                  ]);
    ...
    | Jump o -> [ Bil.Jmp (jump_target mode ss has_rex o) ]
    | Jcc (o, c) ->
        [ Bil.If (c, [ Bil.Jmp (jump_target mode ss has_rex o) ], []) ]
    | Setcc (t, o1, c) -> [ assn t o1 Bil.(Cast (UNSIGNED, !!t, c)) ]
    ...
    | Shiftd (st, s, dst, fill, count) ->
        ...
        let unk_of =
          Bil.Unknown ("OF undefined after shiftd of more then 1 bit", bool_t)
        in
        let ret1 =
          match st with
        ...
        Bil.
          [
            ...
            if_
             ...
             is undefined. *)
                if_ (e_count = int_exp 1 s') [ oF := new_of ] [ oF := unk_of ];
              ]
              [];
          ]
              ...
              if_ (count = zero)
                ...
                [
                  if_ (count = one)
                    ...
                    [
                      oF :=
                        unknown "OF undefined after rotate of more then 1 bit"
                          bool_t;
                    ];
                ];
            ]
                ...
                [
                  if_ (count = one)
                    [
                      oF :=
                        cast high 1 dst
                        lxor cast high 1 (dst lsl int_exp 1 word_size);
                    ]
                    (* else  *)
                    [
                      oF :=
    ...
    | Bt (t, bitoffset, bitbase) ->
        ...
        let value, shift =
          ...
          | Oaddr a ->
              ...
              let byte =
                load (size_of_typ reg8_t) Bil.(a + (offset lsr int_exp 3 t'))
              in
    ...
    | Bs (t, dst, src, dir) ->
        ...
        let width = !!t in
        let src_e = op2e t src in
        let is_fwd = match dir with Backward -> false | Forward -> true in
        let res = tmp t in
        ...
        let assn_result =
          ...
          assn t dst
          @@
          if is_zero_count || is_fwd then Bil.var res
          else Bil.(var res lxor int Word.(of_int n1 ~width))
        in
        ...
        let bil =
          if is_zero_count then
            ...
            Bil.
              [
                if_
                  (src_e = int (Word.zero width))
                  [ zf := int Word.b1; assn t dst @@ unknown "bits" t ]
    ...
    | Fld _src -> unimplemented "unsupported FPU register stack"
    | Fst (_dst, _pop) -> unimplemented "unsupported FPU flags"
    | Cmps (Type.Imm _bits as t) -> (
        ...
        let stmts =
          ...
          :: string_incr mode t rsi :: string_incr mode t rdi
        ...
        | _ -> unimplemented "unsupported flags in cmps")
    | Scas (Type.Imm _bits as t) -> (
        ...
        | [ single ] when single = repz || single = repnz ->
            rep_wrap ~mode ~check_zf:single ~addr ~next stmts
        | _ -> unimplemented "unsupported flags in scas")
    | Stos (Type.Imm _bits as t) -> (
        let stmts =
          [ store_s mode seg_es t rdi_e (op2e t o_rax); string_incr mode t rdi ]
        in
        match pref with
        | [] -> stmts
        | [ single ] when single = repz -> rep_wrap ~mode ~addr ~next stmts
        | _ -> unimplemented "unsupported prefix for stos")
    | Push (t, o) ->
        let o = if is_small_imm o t then sign_extend_imm o t else op2e t o in
        let tmp = tmp t in
        (* only really needed when o involves esp *)
        [
          Bil.Move (tmp, o);
          Bil.Move (rsp, Bil.(rsp_e - Int (mi (bytes_of_width t))));
          store_s mode seg_ss t rsp_e (Bil.var tmp)
          (* FIXME: can ss be overridden? *);
        ]
    | Pop (t, o) ->
        (* From the manual:
    ...
    | Popf t ->
        ...
        let extractlist =
          List.map
            ~f:(fun i -> Bil.(Extract (i, i, Var tmp)))
    ...
    | Popcnt (t, s, d) ->
        ...
        let cnt =
          ...
          | 32 -> popcnt32 res
          | 64 -> popcnt64 res
          | _ -> disfailwith "Invalid popcnt width"
        in
        let bil = (assn_src :: cnt) @ Bil.[ assn t d (var res) ] in
        let flags =
          List.map
            ~f:(fun r -> Bil.Move (r, int_exp 0 1))
            [ cf; oF; sf; af; pf ]
        in
    ...
    | Sahf ->
        ...
        let extractlist =
          List.map
            ~f:(fun i -> Bil.(Extract (i, i, Var tah)))
    ...
    | Add (t, o1, o2) ->
        ...
        ::
        (let s1 = Bil.Var tmp in
         let s2 = Bil.Var tmp2 in
         let r = op2e t o1 in
    ...
    | Adc (t, o1, o2) ->
        ...
        let s1 = Bil.Var orig1 in
        let s2 = Bil.Var orig2 in
        let r = op2e t o1 in
    ...
    | Dec (t, o) (* o = o - 1 *) ->
        ...
        Bil.Move (tmp, op2e t o)
        :: assn t o Bil.(op2e t o - int_exp 1 t')
        :: set_aopszf_sub t' (Bil.Var tmp) (int_exp 1 t') (op2e t o)
        (* CF is maintained *)
    | Sub (t, o1, o2) (* o1 = o1 - o2 *) ->
        ...
        let op2 =
          if is_small_imm o2 t then sign_extend_imm o2 t else op2e t o2
        in
        Bil.(
          [ oldo1 := op1; oldo2 := op2; assn t o1 (op1 - op2) ]
    ...
    | Sbb (t, o1, o2) ->
        ...
        :: Bil.Move
             ( oF,
               Bil.(
                 Cast (HIGH, !!bool_t, orig_s lxor orig_d land (orig_d lxor d)))
             )
    ...
    | Not (t, o) -> [ assn t o (exp_not (op2e t o)) ]
    | Neg (t, o) ->
        let t' = !!t in
        let tmp = tmp t in
        let min_int = Bil.BinOp (LSHIFT, int_exp 1 t', int_exp (t' - 1) t') in
        Bil.Move (tmp, op2e t o)
        :: assn t o Bil.(int_exp 0 t' - op2e t o)
        :: Bil.Move
             (cf, Bil.(Ite (Var tmp = int_exp 0 t', int_exp 0 1, int_exp 1 1)))
    ...
    | Mul (t, src) ->
        ...
        let assnstmts, assne =
          Bil.(
            assn_dbl t
              (Cast (UNSIGNED, !!new_t, op2e t o_rax)
              * Cast (UNSIGNED, !!new_t, op2e t src)))
        in
        let flag =
        ...
        let mul_stmts =
          ...
          | true ->
              ...
              let assnstmts, assne =
                assn_dbl t
                  Bil.(
                    Cast (SIGNED, !!new_t, op2e t src1)
                    * Cast (SIGNED, !!new_t, op2e t src2))
              in
              let flag =
                ...
                Bil.(Extract (highbit, lowbit, assne) <> int_exp 0 !!t)
              in
              assnstmts @ [ Bil.Move (oF, flag); Bil.Move (cf, flag) ]
          | false ->
          ...
          [
            ...
            if_
              ...
              [
                ...
                if_
                  (cast high !!t (var tdiv) = int_exp 0 !!t)
                  apply_result (* else *)
                  [ Cpu_exceptions.divide_by_zero ];
              ];
          ]
    ...
    | Idiv (t, src) ->
        ...
        let lbound =
          let dtm1 = dt' - 1 in
          Word.(one dt' lsl of_int ~width:dt' dtm1)
        in
          ...
          [
            ...
            if_
              ...
              [
                ...
                if_
                  ((var tdiv >$ int ubound) lor (var tdiv <$ int lbound))
                  [ Cpu_exceptions.divide_by_zero ] (* else *)
                  apply_result;
              ];
          ]
        @ undefine [ cf; oF; sf; zf; af; pf ]
    | Cld -> [ Bil.Move (df, exp_false) ]
    | Leave t when List.is_empty pref ->
    ...
    | Sysenter | Syscall -> [ Bil.(encode call "syscall") ]
