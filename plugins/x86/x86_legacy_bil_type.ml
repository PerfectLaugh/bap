...
let addr_of_sexp sexp = String.t_of_sexp sexp |> Big_int_Z.big_int_of_string

let sexp_of_addr bi = Big_int_Z.string_of_big_int bi |> String.sexp_of_t
...
(** [Array] memories can only be updated or accessed in terms of their element
    type, which is usually [Reg 8]. [TMem] memories can be updated or accessed
    by any type, for instance both [Reg 8] and [Reg 32]. Native code is
    generally lifted with [TMem] memories for simplicity, and converted to
    [Array] type when needed. SMT solvers require [Array] type memories. *)
...
(** Visitors are a systematic method for exploring and changing objects of type
    ['a]. In BAP, ['a] can be [stmt], [exp], etc. The children of statements are
    expressions; the children of expressions are subexpressions and variables.
*)
(*type 'a visit_action =
...
(** Specifies whether generated VCs will be evaluated for satisfiability or
    validity. Alternatively, quantifiers can be used. *)
