...
let check_eq t1 t2 f = check eq f t1 t2
let check_mult t1 t2 f = check mult f t1 t2
...
let is_mem_type = function TMem _ | Array _ -> true | Reg _ | Float _ -> false
...
let bits_of_width = function Reg n -> n | _ -> invalid_arg "bits_of_width"
...
let rec infer_ast_internal check e =
  ...
  | Var v ->
      (* FIXME: Check context *)
      Var.typ v
  | UnOp (FP ((FFTOUBV bv | FFTOSBV bv | FFTOIEEEBV bv), _), _) -> Reg bv
  | UnOp
      ( FP ((FISNORM | FISSUB | FISZERO | FISINF | FISNAN | FISNEG | FISPOS), _),
        _ ) ->
      Reg 1
  | UnOp
      (FP ((FFTOF bv | FIEEEBVTOF bv | FBVTOSF bv | FBVTOUF bv | FNAN bv), _), _)
    ->
      Float bv
  | UnOp (_, e) ->
      (if check then
  ...
  | Extract (h, l, e) ->
      ...
      (if check then
         ...
         | Reg oldn ->
             ...
             if h >% big_int_of_int oldn -% bi1 then
               terror
                 "Upper bit index must be at most one less than the size of \
  ...
  | Cast (ct, t, e) ->
      ...
      if check then (
        ...
        match ct with
        | CAST_UNSIGNED | CAST_SIGNED ->
            if bitst < bitse then
              terror
                (Printf.sprintf
                   "Cast type %s is a widening case, but it was used to narrow \
                    %s to %s"
                   (Pp.ct_to_string ct) (Pp.typ_to_string te)
                   (Pp.typ_to_string t))
        | CAST_HIGH | CAST_LOW ->
            if bitst > bitse then
              terror
                (Printf.sprintf
                   "Cast type %s is a narrowing case, but it was used to widen \
                    %s to %s"
                   (Pp.ct_to_string ct) (Pp.typ_to_string te)
                   (Pp.typ_to_string t)));
      t
  ...
  | Store (arr, idx, vl, endian, t) ->
      if check then (
        ...
        check_eq tv t
          "Store of value with type %s performed using a Store of type %s");
...
and check_same ?e ?s t1 t2 =
  if t1 <> t2 then
    ...
    in
    terror
      ("Similar types expected: " ^ Pp.typ_to_string t1 ^ " <> "
     ^ Pp.typ_to_string t2 ^ probs)

and check_reg t =
  if not (is_integer_type t) then
    terror
      (Printf.sprintf "Expected integer type, but got %s" (Pp.typ_to_string t))

and check_bool t =
  if t <> Reg 1 then
    terror
      (Printf.sprintf "Expected bool type, but got %s" (Pp.typ_to_string t))
...
and check_mem arr idx endian t =
  ...
  | Array (i, e) ->
      check_eq ti i
        "Index type not suitable for indexing into this array. Index has type \
         %s, but array has type %s.";
      check_eq t e "Can't get/put a %s from array with element type of %s"
  | TMem (i, e) ->
      check_eq ti i
        "Index type not suitable for indexing into this array. Index has type \
         %s, but the memory has type %s.";
...
and check_cjmp_direct e =
  if Ast.lab_of_exp e = None then
    terror
      "Conditional jump targets must be direct (to a constant address or label)"

let infer_ast = infer_ast_internal false
let typecheck_expression e = ignore (infer_ast_internal true e)

(** Check that conversions between fps and bvs are well-formed. Assumes labelled
    types are accurate without recursing as those will later be checked by
    infer_ast_internal. *)
let rec infer_fp = function
  | Load (arr, idx, endian, t) | Store (arr, idx, _, endian, t) -> (
      match t with
      | Reg _ -> `bv
      | TMem _ | Array _ -> failwith "unimplemented"
      | Float _ -> `fp)
  | BinOp (FP ((FEQ | FLT), _), e1, e2) -> (
      match (infer_fp e1, infer_fp e2) with
  ...
  | UnOp
      ( FP
          ( ( FISNORM | FISSUB | FISZERO | FISINF | FISNAN | FISNEG | FISPOS
            | FFTOUBV _ | FFTOSBV _ | FFTOIEEEBV _ ),
            _ ),
        e ) -> (
      match infer_fp e with
      | `fp | `any -> `bv
  ...
  | Ite (e1, e2, e3) -> (
      ...
      match (infer_fp e2, infer_fp e3) with
      | `bv, `bv | `bv, `any | `any, `bv -> `bv
      | `fp, `fp | `fp, `any | `any, `fp -> `fp
...
let typecheck_stmt =
  ...
  | Halt (e, _) ->
      ...
      check_reg et
  | Assert (e, _) | Assume (e, _) ->
      let et = infer_te e in
      check_bool et
  | Label _ | Comment _ | Special _ -> ()

let typecheck_prog prog =
  let last_label = ref None in
  let is_asm = function Asm _ -> true | _ -> false in
  let ts = function
    | Label (_, attrs) as s when List.exists is_asm attrs ->
        last_label := Some s
    | _ -> ()
  in
  List.iter
    (fun stmt ->
      ...
      with TypeError s -> (
        ...
        | Some ls -> raise (TypeError (s ^ "\nat " ^ Pp.ast_stmt_to_string ls))
        | None -> raise (TypeError s)))
    prog
