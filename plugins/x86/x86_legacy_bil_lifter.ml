...
let var_name _ v =
  match String.chop_prefix ~prefix:"T_" v with
  | Some v -> v
  | None -> (
      match String.chop_prefix ~prefix:"R_" v with
      | Some v -> v
      | None -> ( match v with "mem64" | "mem32" -> "mem" | v -> v))

let is_tmp v = String.is_prefix v ~prefix:"T_"

let type_error exp ctxt =
  warning "Ill-formed expression in the %s context: %s" ctxt
    (ast_exp_to_string exp)
...
let bitv : type t. (t, exp, rmode) Theory.Parser.bitv_parser =
 fun (module S) -> function
  ...
  | UnOp (FP (FFTOIEEEBV n, _), x) -> S.fbits x
  ...
  | ( Unknown _
    ...
    | Store (_, _, _, _, _)
    | BinOp (FP (_, _), _, _)
    | Var _ | Load _ | UnOp _ | Lab _ | Cast _ | Int _ | Let _ ) as exp ->
      type_error exp "bitv";
      S.error
...
let float : type t. (t, exp, rmode) Theory.Parser.float_parser =
 fun (module S) -> function
  ...
  | Ite (c, x, y) -> S.ite c x y
  | ( Load _ | Store _ | UnOp _ | Var _ | Lab _ | Int _ | Cast _ | Let _
    | Unknown _ | Extract _ | Concat _ | BinOp _ ) as exp ->
      type_error exp "float";
      S.error
...
let bool : type t. (t, exp, rmode) Theory.Parser.bool_parser =
 fun (module S) -> function
  ...
  | ( UnOp (_, _)
    ...
    | Concat (_, _)
    | BinOp
        ( ( PLUS | MINUS | TIMES | DIVIDE | SDIVIDE | MOD | SMOD | LSHIFT
          | RSHIFT | ARSHIFT ),
          _,
          _ )
    | Var (V (_, _, Reg _))
 ...
 fun (module S) ->
  let set (Var.V (i, var, t)) x =
    ...
    if is_tmp var then (
      ...
      | Float { exp_bits = 5; sig_bits = 11 } ->
          S.set_ieee754 n Theory.IEEE754.binary16 x
      | Float { exp_bits = 8; sig_bits = 24 } ->
          S.set_ieee754 n Theory.IEEE754.binary32 x
      | Float { exp_bits = 11; sig_bits = 53 } ->
          S.set_ieee754 n Theory.IEEE754.binary64 x
      | Float { exp_bits = 15; sig_bits = 64 } ->
          S.set_ieee754 n Theory.IEEE754.binary80 x
      | Float { exp_bits = 15; sig_bits = 113 } ->
          S.set_ieee754 n Theory.IEEE754.binary128 x
      | _ ->
...
module Bap_integration = struct
  ...
  let bytes_of_mem mem = Bigsubstring.to_string (Memory.to_buffer mem)
  let addr_of_mem mem = Addr.to_int64_exn (Memory.min_addr mem)
  ...
  let fixrip next prog =
    let rec stmt : Ast.stmt -> Ast.stmt = function
      ...
      | (Label _ | Comment _ | Special _) as x -> x
    and exp = function
      ...
      | Var (V (i, p, _)) as var -> (
          match var_name i p with "RIP" -> next | _ -> var)
    in
  ...
  let lift (module Core : Theory.Core) arch mem insn =
    ...
    | Ok prog ->
        ...
        let next =
          Ast.Int (Bitvec.to_bigint @@ Word.to_bitvec pc, Reg (Word.bitwidth pc))
        in
  ...
  let provide_lifter () =
    ...
    let lifter obj =
      ...
      Theory.instance ~context:[ "floating-point" ] () >>= Theory.require
      >>= fun theory ->
      let* mem = obj -->? Memory.slot in
      let* insn = obj -->? Disasm_expert.Basic.Insn.slot in
      match arch with
      | (`x86 | `x86_64) as arch ->
          KB.catch (lift theory arch mem insn) (fun _ -> !!empty)
      | _ -> Knowledge.return empty
    in
