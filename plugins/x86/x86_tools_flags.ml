...
module Make (CPU : X86CPU) : FR = struct
  ...
  let parity width result =
    ...
    let v2 = Type.imm width |> Var.create ~is_virtual:true ~fresh:true "v" in
    let open Bil in
    lnot
      (cast low 1
         (let_ v2
            ((result lsr i 4) lxor result)
            (let_ v2
               ((var v2 lsr i 2) lxor var v2)
  ...
  let sub =
    ...
    let overflow diff op1 op2 =
      let open Bil in
      cast high 1 (op1 lxor op2 land (op1 lxor diff))
    in
    (carry, overflow)
  ...
  let add =
    ...
    let overflow sum op1 op2 =
      let open Bil in
      cast high 1 (lnot (op1 lxor op2) land (op2 lxor sum))
    in
    (carry, overflow)
