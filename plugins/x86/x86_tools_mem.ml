...
module Make (CPU : X86CPU) (RR : RR) (IM : IM) : MM = struct
  ...
  module Segment = struct
    module Base = struct
      type t = GPR of RR.t | IP of word [@@deriving variants, sexp]

      let create mem' base =
        let open Option in
        X86_asm.Reg.decode base >>= function
        | #X86_asm.Reg.gpr -> RR.of_mc_exn base |> gpr |> some
        | `IP | `EIP | `RIP -> Memory.max_addr mem' |> Word.succ |> ip |> some
        | b ->
            failwithf "invalid base %s"
    ...
    let create ?seg ?base ?scale ?index ~disp mem =
      let seg =
        ...
        in
        let open X86_asm in
        let seg = Option.value_map ~default:None ~f:X86_asm.Reg.decode seg in
        match seg with
        | None -> None
        | Some (#Reg.segment as s) -> base s
        | Some r ->
            Error.failwiths ~here "invalid segment" r X86_asm.Reg.sexp_of_t
      in
    ...
    let make_value reg =
      ...
      | #Reg.r8, _ | #Reg.r16, _ | #Reg.r32, `x86_64 ->
          RR.get reg |> Bil.(cast unsigned size)
      | #Reg.r32, `x86 | #Reg.r64, _ | #Reg.segment_base, _ -> RR.get reg
    ...
    let make_scale scale =
      let shift =
        ...
        | 4 -> Some 2
        | 8 -> Some 3
        | s -> Error.failwiths ~here "invalid memory scale" s sexp_of_int
      in
    ...
    let addr { seg; base; scale; index; disp } =
      ...
      let base =
        Option.map base ~f:(function
          | Base.GPR base -> make_value base
          | Base.IP word -> Bil.int word)
      in

      let scale = Option.value_map ~default:None ~f:make_scale scale in
  ...
  type t = Segment of Segment.t | Relative of Relative.t [@@deriving variants]

  let of_mem ?seg ?base ?scale ?index ~disp mem =
    segment @@ Segment.create mem ?seg ?base ?scale ?index ~disp

  let of_offset imm = Relative.create imm |> relative

  let addr =
    Variants.map
      ~segment:(fun _ -> Segment.addr)
