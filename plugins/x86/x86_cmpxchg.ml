...
module Make (Tools : X86_tools.S) (Backend : X86_backend.S) = struct
  ...
  let get_rax = function
    | `CMPXCHG8rr | `CMPXCHG8rm -> `AL
    | `CMPXCHG16rr | `CMPXCHG16rm -> `AX
    | `CMPXCHG32rr | `CMPXCHG32rm -> `EAX
    | `CMPXCHG64rr | `CMPXCHG64rm -> `RAX
  ...
  let cmpxchg_rr (op : cmpxchg_rr) =
    X86_operands.rr ~f:(fun mem fst snd ->
        ...
        let bil =
          ...
          let flags =
            FR.after_sub ~diff:(Bil.var v) ~op1:(RR.get rax) ~op2:(RR.get fst)
              size
          in
          let cmpxchg =
            Bil.(
              if_
                (RR.get rax = RR.get fst)
                [ RR.get snd |> RR.set fst ]
                [ RR.get fst |> RR.set rax ])
          in
          List.concat [ sub :: flags; [ cmpxchg ] ]
        in
        Ok bil)
  ...
  let cmpxchg_rm (op : cmpxchg_rm) =
    X86_operands.mr ~f:(fun mem ~seg ~base ~scale ~index ~disp reg ->
        ...
        let bil =
          ...
          let flags =
            FR.after_sub ~diff:(Bil.var v) ~op1:(RR.get rax) ~op2:(Bil.var d)
              size
          in
          let cmpxchg =
            Bil.(
              if_
                ...
                [ RR.set rax (Bil.var d) ])
          in
          List.concat [ load :: sub :: flags; [ cmpxchg ] ]
        in
        Ok bil)
  ...
  let register what =
    let name op = sexp_of_cmpxchg (op :> cmpxchg) |> Sexp.to_string in
    List.iter
      (what :> cmpxchg list)
