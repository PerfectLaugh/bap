...
type endbr = [ `ENDBR32 | `ENDBR64 ]
[@@deriving bin_io, sexp, compare, enumerate]

let lift enabled _mem _insn =
  if enabled then Ok [ Bil.(encode intrinsic "endbr") ] else Ok []

let enabled =
  Extension.Configuration.flag "cet-enabled" ~aliases:[ "endbr-enabled" ]
    ~doc:
      "When enabled, translate all endbr instruction into intrinsic calls. \
       Otherwise treat them as nops"
