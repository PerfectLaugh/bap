...
module Insn_semantics (Tools : X86_tools.S) = struct
  ...
  let apply mem seg disp reg sem =
    ...
    match sem with
    | Mem_to_reg -> Ok [ MM.load mem ~size:(RR.width reg) |> RR.set reg ]
    | Reg_to_mem -> Ok [ RR.get reg |> MM.store mem ~size:(RR.width reg) ]

  let lift asm sem allow_nil =
    let reg = RR.of_asm_exn asm in
    if allow_nil then
      X86_operands.ir ~f:(fun mem off seg -> apply mem (Some seg) off reg sem)
    else X86_operands.i ~f:(fun mem off -> apply mem None off reg sem)
end
...
module Ver_34 = struct
  ...
  module Mov_oa_ia32 = struct
    ...
    let asm_of_t t =
      match t with MOV8o8a -> `AL | MOV16o16a -> `AX | MOV32o32a -> `EAX
  ...
  module Mov_ao_ia32 = struct
    ...
    let asm_of_t t =
      match t with MOV8ao8 -> `AL | MOV16ao16 -> `AX | MOV32ao32 -> `EAX
...
let normalize ver =
  ...
  | None -> ver
  | Some i -> (
      match String.sub ver ~pos:0 ~len:(i + 2) with
      | x -> x
      | exception _ -> ver)
...
let () =
  ...
  if String.equal ver "3.4" then T_34.register () else T.register ();
  Ok ()
