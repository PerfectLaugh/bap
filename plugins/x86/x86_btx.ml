...
module Make (Tools : X86_tools.S) (Backend : X86_backend.S) = struct
  ...
  let btx_rr (op : btx_rr) =
    X86_operands.rr ~f:(fun _mem base offset ->
        ...
        let bil =
          ...
          let side =
            side_effect op (Bil.var base) (mask_reg RR.size o) (fun exp ->
                Bil.(base := exp))
          in
          List.concat [ off :: flag; side ]
        in
        Ok bil)
  ...
  let btx_ri (op : btx_ri) =
    X86_operands.ri ~f:(fun _mem base offset ->
        ...
        let size = RR.width base in
        let o =
          Option.value_exn (Imm.to_word offset ~width:(offset_width size))
        in
        let base = RR.var base in
        let bil =
          let flags = set_flags op (Bil.var base) (Bil.int o) in
          let side =
            side_effect op (Bil.var base) (mask_imm RR.size o) (fun exp ->
                Bil.(base := exp))
          in
          List.concat [ flags; side ]
        in
        Ok bil)
  ...
  let btx_mi (op : btx_mi) =
    X86_operands.mi ~f:(fun mem ~seg ~base ~scale ~index ~disp offset ->
        ...
        let size =
          ...
          | `BT16mi8 | `BTC16mi8 | `BTR16mi8 | `BTS16mi8 -> `r16
        in
        ...
        let bil =
          let load =
            Bil.[ a := MM.addr base_mem; d := MM.load_from ~size (var a) ]
          in
          let flag = set_flags op (Bil.var d) (Bil.int o) in
          let side =
            side_effect op (Bil.var d) (mask_imm size o) (fun exp ->
                MM.store_to Bil.(var a) ~size exp)
          in
          List.concat [ load; flag; side ]
        in
        Ok bil)
        ...
        let bil =
          let load =
            let bexp =
              ...
              if bsize_addr <> bsize then Bil.(cast signed bsize_addr e) else e
            in
            Bil.
              [
                a := MM.addr base_mem;
          ...
          let flags =
            ...
            :: set_flags op (Bil.var d) (Bil.var o)
          in
          let side =
            side_effect op (Bil.var d) (mask_reg size o) (fun exp ->
                MM.store_to Bil.(var a + var b) ~size exp)
          in
          List.concat [ load; flags; side ]
        in
        Ok bil)
