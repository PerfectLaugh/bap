...
let type_of_mode = function `x86 -> Reg 32 | `x86_64 -> Reg 64
...
module Pcmpstr = struct
  ...
  let ssize_to_string = function Bytes -> "Bytes" | Words -> "Words"

  type ssign = Signed | Unsigned

  let ssign_to_string = function Signed -> "Signed" | Unsigned -> "Unsigned"
  ...
  type outselectsig =
    | LSB
    | MSB (* For PCMPESTRI/PCMPISTRI, choosees LSB or MSB.  *)

  let outselectsig_to_string = function LSB -> "LSB" | MSB -> "MSB"

  type outselectmask =
    | Bitmask
    | Bytemask (* For PCMPESTRM/PCMPISTRM, represents bit mask/word mask. *)

  let outselectmask_to_string = function
    | Bitmask -> "Bitmask"
    | Bytemask -> "Bytemask"

  let sig_to_mask = function LSB -> Bitmask | MSB -> Bytemask
...
type rex = {
  ...
  rex_b : bool;
      (* Bit 0: Extension of ModR/M r/m field, SIB base
...
type prefix = {
  ...
  bopsize : typ;
      (* Operand size that defaults to machine size
                     (e.g. for pop) *)
...
let ite t b e1 e2 = X86_legacy_bil_convenience.exp_ite ~t b e1 e2
...
let gv mode { v32; v64 } = match mode with `x86 -> v32 | `x86_64 -> v64
...
let nums =
  Array.init 8 (fun i ->
      nmv "ERROR" (Reg 0) (Printf.sprintf "R_R%d" (i + 8)) r64)
...
let gymms mode =
  let regs = match mode with `x86 -> 8 | `x86_64 -> 16 in
...
let r_8, r_9, r_10, r_11, r_12, r_13, r_14, r_15 =
  match Array.to_list nums with
...
let regs_x86_64 : var list =
  ...
  @ List.map
      ~f:(fun { v64; v32 } -> v64)
      (shared_multi_regs @ Array.to_list nums)
  @ Array.to_list ymms

let regs_full : var list =
  shared_regs
  @ List.concat
      (List.map ~f:(fun { v64; v32 } -> [ v32; v64 ]) shared_multi_regs)
...
module ToIR = struct
  ...
  (** Converts from Intel 80-bit bitvector to IEEE 15,64 floating point. Ported
      from Ryan's ssemantics. *)
  let intel80tof ~rm exp =
    ...
    let xregular =
      ieeebvtof ~float_size:fp_80_bits
      @@ X86_legacy_bil_convenience.concat sign
           (X86_legacy_bil_convenience.concat expon frac)
    in
    let xinvalid = fnan ~rm ~float_size:fp_80_bits in
    let xpseudo =
      ieeebvtof ~float_size:fp_80_bits
      @@ X86_legacy_bil_convenience.concat sign
           (X86_legacy_bil_convenience.concat bitvec15_one frac)
    in
    X86_legacy_bil_convenience.exp_ite isvalid
      (X86_legacy_bil_convenience.exp_ite ispseudodenorm xpseudo xregular)
      xinvalid
  ...
  let ftointel80 ~rm exp =
    ...
    X86_legacy_bil_convenience.concat sign_exp
      (X86_legacy_bil_convenience.concat integer frac)
  ...
  (** Convert X87 bitvector (80-bit) to reduced precision version based on the
      contents of the precision-control field. This function also does the
      necessary rounding. *)
  let x87_bv_to_fp_pc ?(rm = RNE) exp =
    let cmp_pc i =
      binop EQ (X86_legacy_bil_convenience.it i (Reg 2)) (Var x87_pc)
    in
    let convert_00 =
      intel80tof ~rm exp
      |>
      (* ftof ~rm ~float_size:fp_pc_32_bits |> *)
      ftof ~rm ~float_size:fp_80_bits
    in
    let convert_10 =
      intel80tof ~rm exp
      |>
      (* ftof ~rm ~float_size:fp_pc_64_bits |> *)
      ftof ~rm ~float_size:fp_80_bits
    in
  ...
  let x87_fp_to_bv_pc ?(rm = RNE) exp =
    ...
    let convert_00 =
      (* ftof ~rm ~float_size:fp_pc_32_bits *)
      exp |> ftof ~rm ~float_size:fp_80_bits |> ftointel80 ~rm
    in
    let convert_10 =
      (* ftof ~rm ~float_size:fp_pc_64_bits *)
      exp |> ftof ~rm ~float_size:fp_80_bits |> ftointel80 ~rm
  ...
  let get_x87_stack ~index =
    Core.Array.foldi
      ~f:(fun i acc st_exp ->
        X86_legacy_bil_convenience.exp_ite
          (x87_top_exp
          ==* X86_legacy_bil_convenience.it ((i - index) mod 8) (Reg 3))
          st_exp acc)
  ...
  let set_x87_stack ~index value =
    Core.Array.mapi
      ~f:(fun i st_var ->
        ...
        [
          move st_tag.(i)
            (X86_legacy_bil_convenience.exp_ite selector tag st_tag_exps.(i));
          move st_var
            (X86_legacy_bil_convenience.exp_ite selector bv_value st_exps.(i));
        ])
      st
    |> Array.to_list |> Core.List.concat

  let empty_x87_stack ~index =
    Core.Array.mapi
      ~f:(fun i st_tag_var ->
        let selector =
          x87_top_exp ==* X86_legacy_bil_convenience.it i (Reg 3)
        in
        move st_tag_var
          (X86_legacy_bil_convenience.exp_ite selector fempty st_tag_exps.(i)))
      st_tag
    |> Array.to_list
  ...
  let pop_x87_stack = [ inc_x87_stack ] @ empty_x87_stack ~index:0
  let push_x87_stack x = set_x87_stack ~index:7 x @ [ dec_x87_stack ]
  ...
  let bv_to_fp80 typ exp =
    ...
    in
    if typ = `reg_80 then fp exp else ftof ~float_size:fp_80_bits (fp exp)
  ...
  let _storem mode t a e = move mode (Store (Var mode, a, e, little_endian, t))
  ...
  *)
  ...
  let set_apszf t s1 s2 r =
    ...
    move af (bit4 ==* (bit4 &* (r ^* s1) ^* s2)) :: set_pszf t r

  (* Helper functions to set flags for adding *)
  let set_aopszf_add t s1 s2 r =
    move oF (cast_high r1 (s1 =* s2 &* s1 ^* r)) :: set_apszf t s1 s2 r

  let set_flags_add t s1 s2 r = move cf (r <* s1) :: set_aopszf_add t s1 s2 r

  (* Helper functions to set flags for subtracting *)
  let set_apszf_sub t s1 s2 r = set_apszf t s1 s2 r

  let set_aopszf_sub t s1 s2 r =
    move oF (cast_high r1 (s1 ^* s2 &* s1 ^* r)) :: set_apszf_sub t s1 s2 r

  let set_flags_sub t s1 s2 r = move cf (s2 >* s1) :: set_aopszf_sub t s1 s2 r
end
(* ToIR *)
