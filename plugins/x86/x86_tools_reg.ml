...
module Make (CPU : X86CPU) : RR = struct
  ...
  let of_asm = function r when CPU.avaliable r -> Some r | r -> None
  ...
  let var t =
    ...
    else Error.failwiths ~here "invalid reg variable" t X86_asm.sexp_of_reg
  ...
  let set r e =
    ...
    let rhs =
      ...
      | (#Reg.r8l as r1), _ | (#Reg.r16 as r1), _ | (#Reg.r128 as r1), _ ->
      ...
      | #Reg.r256, _ ->
          e
      | #Reg.r64, `x86 ->
          Error.failwiths ~here "invalid reg" r X86_asm.sexp_of_reg
    in
    Bil.(lhs := rhs)
end
