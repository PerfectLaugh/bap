...
module Fp_lifter = struct
  ...
  type st = [ `ST0 | `ST1 | `ST2 | `ST3 | `ST4 | `ST5 | `ST6 | `ST7 ]
  [@@deriving sexp]

  type ip = [ `IP | `EIP | `RIP ] [@@deriving sexp]
  type segment = [ `CS | `DS | `ES | `FS | `GS | `SS ] [@@deriving sexp]
  type segment_base = [ `FS_BASE | `GS_BASE ] [@@deriving sexp]
  type gpr = [ r8 | r16 | r32 | r64 | r128 | r256 ] [@@deriving sexp]
  type reg_t = [ gpr | ip | st | segment | segment_base ] [@@deriving sexp]
  ...
  let var_of_reg_t mode = function
    ...
    | `ST7 -> st.(7)
    | reg ->
        failwithf "no var_of_reg_t translation yet %s"
  ...
  let st_index_of_reg_t = function
    ...
    | `ST7 -> 7
    | reg ->
        failwithf "Input is not an ST register. %s"
          (Sexp.to_string @@ sexp_of_reg_t reg)
          ()
  ...
  let mem_index ~arch ~typ ~seg ~base ~scale ~index ~disp =
    ...
    let index_typ = match arch with `x86 -> reg_32 | `x86_64 -> reg_64 in
    ...
    let seg = None and scale = None in
  ...
  let convert_int64_to_single_precision_fp integer =
    bvtosf ~float_size:fp_64_bits integer |> ftof ~float_size:fp_32_bits
  ...
  let convert_double_precision_fp_to_int32 fp = ftosbv ~bv_size:32 fp
  ...
  let lift arch mem insn =
    ...
    let ld_fm mem ~typ ~seg ~base ~scale ~index ~disp =
      ...
      in
      let mem_index =
        mem_index ~typ:exp_typ ~arch ~seg ~base ~scale ~index ~disp
      in
      let mem = load_s arch None exp_typ mem_index in
      let value = ToIR.bv_to_fp80 (typ :> ToIR.convertable_bv) mem in
      let stmts = [ ToIR.dec_x87_stack ] @ ToIR.set_x87_stack ~index:0 value in
      Result.Ok stmts
    in
    ...
    let st_fprr mem dst =
      ...
      let stmts =
        ToIR.set_x87_stack ~index:dst_index src @ ToIR.pop_x87_stack
      in
      Result.Ok stmts
    in
    ...
    let cvtsi2sd dst exp =
      ...
      Result.Ok [ ToIR.move dst (upper_part ++* fp_to_sse_bv ~bv_size double) ]
    in
    ...
    let cvttsd2si dst exp =
      ...
      Result.Ok [ ToIR.move dst integer ]
    in
    let cvttsd2si_rm arch mem dst ~seg ~base ~scale ~index ~disp =
      let dst = decode dst |> value_exn |> var_of_reg_t arch in
      let mem_index =
        mem_index ~typ:reg_64 ~arch ~seg ~base ~scale ~index ~disp
      in
    ...
    let sse_unopsd_r ~unop arch mem dst src =
      ...
      let stmts = [ ToIR.move dst (upper_part ++* double) ] in
      Result.Ok stmts
    in
    ...
    let sse_binopsd_rrr ~typ ~binop arch mem dst src1 src2 =
      ...
      let double =
        binop
          (sse_bv_to_fp ~float_size (extract (bv_size - 1) 0 src1))
    ...
    let sse_binopsd_rrm ~typ ~binop arch mem dst src1 ~seg ~base ~scale ~index
        ~disp =
      let float_size, typ, bv_size =
        match typ with
    ...
    let x87_binop style ~binop ?(reversed = false) mem sti =
      ...
      let stmts =
        ...
        | `FPrST0 ->
            ToIR.set_x87_stack ~index:sti_index (binop sti st0)
            @ ToIR.pop_x87_stack
        | `R_FPrST0 ->
            ToIR.set_x87_stack ~index:sti_index (binop st0 sti)
            @ ToIR.pop_x87_stack
      in
      Result.Ok stmts
    in
    ...
    let x87_binop_mem ~typ:atyp ~binop mem ~seg ~base ~scale ~index ~disp =
      ...
      let stmts = ToIR.set_x87_stack ~index:0 value in
      Result.Ok stmts
    in
    ...
    let ucomisx_rm ~typ arch mem src1 ~seg ~base ~scale ~index ~disp =
      let src1 = decode src1 |> value_exn |> exp_of_reg_t arch in
      let float_size, highbit, exp_typ =
        match typ with
    ...
    let fcom_r ~pop ?(pop2 = false) ~unordered mem sti =
      ...
      let has_empty =
        binop OR
          (ToIR.get_x87_tag ~index:0 ==* it 3 (Reg 2))
          (ToIR.get_x87_tag ~index:sti_index ==* it 3 (Reg 2))
      in
      let stmts =
        [
    ...
    let fidiv mem ~typ ~seg ~base ~scale ~index ~disp =
      let exp_typ = match typ with `reg_32 -> reg_32 | `reg_16 -> reg_16 in
      let mem_index =
        mem_index ~typ:exp_typ ~arch ~seg ~base ~scale ~index ~disp
      in
      let mem : Ast.exp = load_s arch None exp_typ mem_index in
    ...
    let unpcklps_rr arch mem dest src1 src2 =
      ...
      let stmts = [ ToIR.move dest (y1 ++* x1 ++* y0 ++* x0) ] in
      Result.Ok stmts
    in
    ...
    let vzeroupper arch =
      let f reg =
        ...
        ToIR.move reg
        @@ exp_and (Var reg) (Cast (Type.CAST_UNSIGNED, cast_t, mask128))
      in
    ...
    let vpbroadcast_rr dest_width src_width arch mem dst src =
      ...
      let max_vl = Typecheck.bits_of_width (Var.typ dst) in
      let src_size =
        match src_width with `B -> 8 | `W -> 16 | `D -> 32 | `Q -> 64
      in
      let dst_size =
        match dest_width with `YMM -> 256 | `XMM -> 128 | `ZMM -> 512
      in
    ...
    let vinserti128_rrri arch mem dst src1 src2 imm =
      ...
      let final_value =
        if keep_src1_low then concat packed src1_low
        else concat src1_high packed
      in
      let stmts = [ ToIR.move dst final_value ] in
      Result.Ok stmts
    in
    ...
    let packuswb_rr arch mem dst src1 src2 =
      ...
      let saturate_signed_word_to_unsigned_byte exp =
        exp_ite
          (is_saturated_negative exp)
          min_byte
          (exp_ite (is_saturated_positive exp) max_byte (extract 7 0 exp))
      in
      ...
      let upper_bytes =
        if dst_size / 2 < real_reg_size then
          Sequence.singleton
            (extract (real_reg_size - 1) (dst_size / 2) (Var src1))
        else Sequence.empty
      in
      let packed_bytes =
      ...
      let final_value =
        Sequence.fold ~init:low_byte
          ~f:(fun acc byte -> concat byte acc)
          remaining_bytes
      in
      Result.Ok [ ToIR.move dst final_value ]
    in

    let std () = [ ToIR.move df exp_true ] in

    let fnstcw mem ~seg ~base ~scale ~index ~disp =
      let mem_index =
        mem_index ~typ:reg_16 ~arch ~seg ~base ~scale ~index ~disp
      in
      let control_word =
        concat (Var x87_15) @@ concat (Var x87_14) @@ concat (Var x87_13)
        @@ concat (Var x87_x) @@ concat (Var x87_rc) @@ concat (Var x87_pc)
        @@ concat (Var x87_7) @@ concat (Var x87_6) @@ concat (Var x87_pm)
        @@ concat (Var x87_um) @@ concat (Var x87_om) @@ concat (Var x87_zm)
        @@ concat (Var x87_dm) @@ Var x87_im
      in
    ...
    let fnstsw_ax arch =
      ...
      let gax_high = match arch with `x86 -> 31 | `x86_64 -> 63 in
      (* was Target.Arch.addr_width - 1 *)
      [
    ...
    let fxam () =
      ...
      [
        ToIR.move x87_c0
        @@ switch st0 ~def:_0 ~nan:_1 ~norm:_0 ~inf:_1 ~zero:_0 ~empty:_1
             ~denorm:_0;
        ToIR.move x87_c1 @@ fisneg st0;
        ToIR.move x87_c2
        @@ switch st0 ~def:_0 ~nan:_0 ~norm:_1 ~inf:_1 ~zero:_0 ~empty:_0
             ~denorm:_1;
        ToIR.move x87_c3
        @@ switch st0 ~def:_0 ~nan:_0 ~norm:_0 ~inf:_0 ~zero:_1 ~empty:_1
             ~denorm:_1;
      ]
    in
    ...
    let decode_name insn =
      ...
      with exn -> raise (Unsupported_opcode inst_name)
    in
    match decode_name insn with
    | `LD_F0 -> ld_f0 ()
    | `LD_F1 -> ld_f1 ()
    | `LD_Frr -> Operands.r ~f:ld_fprr mem insn |> ok_exn
    | `LD_F32m -> Operands.m ~f:(ld_fm ~typ:`reg_32) mem insn |> ok_exn
    | `LD_F64m -> Operands.m ~f:(ld_fm ~typ:`reg_64) mem insn |> ok_exn
    | `LD_F80m -> Operands.m ~f:(ld_fm ~typ:`reg_80) mem insn |> ok_exn
    | `ILD_F16m -> Operands.m ~f:(ild_fm ~typ:`reg_16) mem insn |> ok_exn
    | `ILD_F32m -> Operands.m ~f:(ild_fm ~typ:`reg_32) mem insn |> ok_exn
    | `ILD_F64m -> Operands.m ~f:(ild_fm ~typ:`reg_64) mem insn |> ok_exn
    | `ABS_F -> abs_f ()
    | `SQRT_F -> sqrt_f ()
    | `SQRTSDr ->
        Operands.rr ~f:(sse_unopsd_r ~unop:fsqrt arch) mem insn |> ok_exn
    | `CHS_F -> chs_f ()
    | `ST_FPrr -> Operands.r ~f:st_fprr mem insn |> ok_exn
    | `ST_FP32m ->
    ...
    | `CVTTSD2SIrm -> Operands.rm ~f:(cvttsd2si_rm arch) mem insn |> ok_exn
    | `CVTSD2SSrr -> Operands.rr ~f:(cvtsd2ss_rr arch) mem insn |> ok_exn
    | `ADDSDrr ->
        Operands.rrr ~f:(sse_binopsd_rrr ~typ:`reg_64 ~binop:fadd arch) mem insn
        |> ok_exn
    | `ADDSDrm | `ADDSDrm_Int ->
        Operands.rrm ~f:(sse_binopsd_rrm ~typ:`reg_64 ~binop:fadd arch) mem insn
        |> ok_exn
    | `ADDSSrr ->
        Operands.rrr ~f:(sse_binopsd_rrr ~typ:`reg_32 ~binop:fadd arch) mem insn
        |> ok_exn
    | `ADDSSrm ->
        Operands.rrm ~f:(sse_binopsd_rrm ~typ:`reg_32 ~binop:fadd arch) mem insn
        |> ok_exn
    | `ADD_FrST0 ->
    ...
    | `ADD_F32m ->
        Operands.m ~f:(x87_binop_mem ~typ:`reg_32 ~binop:fadd) mem insn
        |> ok_exn
    | `ADD_F64m ->
        Operands.m ~f:(x87_binop_mem ~typ:`reg_64 ~binop:fadd) mem insn
        |> ok_exn
    | `SUBSDrr ->
        Operands.rrr ~f:(sse_binopsd_rrr ~typ:`reg_64 ~binop:fsub arch) mem insn
        |> ok_exn
    | `SUBSDrm ->
        Operands.rrm ~f:(sse_binopsd_rrm ~typ:`reg_64 ~binop:fsub arch) mem insn
        |> ok_exn
    | `SUBSSrr ->
        Operands.rrr ~f:(sse_binopsd_rrr ~typ:`reg_32 ~binop:fsub arch) mem insn
        |> ok_exn
    | `SUBSSrm ->
        Operands.rrm ~f:(sse_binopsd_rrm ~typ:`reg_32 ~binop:fsub arch) mem insn
        |> ok_exn
    | `SUB_FrST0 ->
    ...
    | `SUB_F32m ->
        Operands.m ~f:(x87_binop_mem ~typ:`reg_32 ~binop:fsub) mem insn
        |> ok_exn
    | `SUB_F64m ->
        Operands.m ~f:(x87_binop_mem ~typ:`reg_64 ~binop:fsub) mem insn
        |> ok_exn
    | `MULSDrr ->
        Operands.rrr ~f:(sse_binopsd_rrr ~typ:`reg_64 ~binop:fmul arch) mem insn
        |> ok_exn
    | `MULSDrm ->
        Operands.rrm ~f:(sse_binopsd_rrm ~typ:`reg_64 ~binop:fmul arch) mem insn
        |> ok_exn
    | `MULSSrr ->
        Operands.rrr ~f:(sse_binopsd_rrr ~typ:`reg_32 ~binop:fmul arch) mem insn
        |> ok_exn
    | `MULSSrm ->
        Operands.rrm ~f:(sse_binopsd_rrm ~typ:`reg_32 ~binop:fmul arch) mem insn
        |> ok_exn
    | `MUL_FrST0 ->
    ...
    | `MUL_F32m ->
        Operands.m ~f:(x87_binop_mem ~typ:`reg_32 ~binop:fmul) mem insn
        |> ok_exn
    | `MUL_F64m ->
        Operands.m ~f:(x87_binop_mem ~typ:`reg_64 ~binop:fmul) mem insn
        |> ok_exn
    | `DIVSDrr | `DIVSDrr_Int ->
        Operands.rrr ~f:(sse_binopsd_rrr ~typ:`reg_64 ~binop:fdiv arch) mem insn
        |> ok_exn
    | `DIVSDrm ->
        Operands.rrm ~f:(sse_binopsd_rrm ~typ:`reg_64 ~binop:fdiv arch) mem insn
        |> ok_exn
    | `DIVSSrr ->
        Operands.rrr ~f:(sse_binopsd_rrr ~typ:`reg_32 ~binop:fdiv arch) mem insn
        |> ok_exn
    | `DIVSSrm ->
        Operands.rrm ~f:(sse_binopsd_rrm ~typ:`reg_32 ~binop:fdiv arch) mem insn
        |> ok_exn
    | `DIV_FrST0 ->
    ...
    | `DIV_F32m ->
        Operands.m ~f:(x87_binop_mem ~typ:`reg_32 ~binop:fdiv) mem insn
        |> ok_exn
    | `DIV_F64m ->
        Operands.m ~f:(x87_binop_mem ~typ:`reg_64 ~binop:fdiv) mem insn
        |> ok_exn
    | `DIV_FI32m -> Operands.m ~f:(fidiv ~typ:`reg_32) mem insn |> ok_exn
    | `DIV_FI16m -> Operands.m ~f:(fidiv ~typ:`reg_16) mem insn |> ok_exn
    | `MAXSDrr ->
        Operands.rrr ~f:(sse_binopsd_rrr ~typ:`reg_64 ~binop:fmax arch) mem insn
        |> ok_exn
    | `UCOMISDrr ->
    ...
    | `UCOM_FIr -> Operands.r ~f:(ucomfi_r ~pop:false) mem insn |> ok_exn
    | `UCOM_FIPr -> Operands.r ~f:(ucomfi_r ~pop:true) mem insn |> ok_exn
    | `UCOM_Fr ->
    ...
    | `UNPCKLPSrr -> Operands.rrr ~f:(unpcklps_rr arch) mem insn |> ok_exn
    | `XCH_F -> Operands.r ~f:xch_f mem insn |> ok_exn
    (* FWAIT: NOP *)
    | `WAIT -> []
    (* YMMs *)
    | `VZEROUPPER -> vzeroupper arch
    | `VPBROADCASTBYrr ->
    ...
    | `PACKUSWBrr -> Operands.rrr ~f:(packuswb_rr arch) mem insn |> ok_exn
    (* Others *)
    | `STD -> std ()
    | `FNSTCW16m -> Operands.m ~f:fnstcw mem insn |> ok_exn
    | `FLDCW16m -> Operands.m ~f:fldcw mem insn |> ok_exn
    | `FNSTSW16m -> Operands.m ~f:fnstsw_m mem insn |> ok_exn
    | `FNSTSW16r -> fnstsw_ax arch
    | `FXAM -> fxam ()
    | `FRNDINT -> frndint ()
end
...
let run arch mem insn =
  ...
  | Typecheck.TypeError msg -> Or_error.errorf "ill-typed code: %s" msg
  | Unsupported_opcode name -> Or_error.errorf "unsupported opcode: %s" name
