...
let get_symbols =
  Command.create `python ~script:(request "symbols") ~parser:(fun name ->
...
let extract path arch =
  let id = Data.Cache.digest ~namespace:"ida" "%s" (digest path) in
  let syms =
    match Symbols.Cache.load id with
    | Some syms -> syms
    | None -> (
        match Ida.(with_file path get_symbols) with
        | [] ->
  ...
  List.map syms ~f:(fun (n, s, e) -> (n, addr s, addr e)) |> Seq.of_list

let ida_symbolizer =
  let reliability = Knowledge.Agent.reliable in
  Knowledge.Agent.register ~reliability ~package:"bap" "ida-symbolizer"
...
type section = string * perm * int * (int64 * int) [@@deriving sexp]
...
let arch_of_procname size name =
  ...
  | "80586r" | "80586p" | "80686p" | "k62" | "p2" | "p3" | "athlon" | "p4"
  | "metapc" ->
...
let read_image name =
  In_channel.with_file name ~f:(fun ch -> Sexp.input_sexp ch |> image_of_sexp)

let load_image =
  Command.create `python ~script:(request "loader") ~parser:read_image

let mapfile path : Bigstring.t =
  let fd = Unix.(openfile path [ O_RDONLY ] 0o400) in
  let size = Unix.((fstat fd).st_size) in
  let data =
    Mmap.V1.map_file fd Bigarray.char Bigarray.c_layout false [| size |]
  in
  Unix.close fd;
  Bigarray.array1_of_genarray data

let loader path =
  let id = Data.Cache.digest ~namespace:"ida-loader" "%s" (digest path) in
  ...
  let code, data =
    List.fold sections ~init:(Memmap.empty, Memmap.empty)
      ~f:(fun (code, data) (name, perm, pos, (beg, len)) ->
        ...
        | Ok mem ->
            ...
            if equal_perm perm `code then (Memmap.add code mem sec, data)
            else (code, Memmap.add data mem sec))
  in
...
type brancher_info = (int64 * int64 option * int64 list) list [@@deriving sexp]
...
module IdaBrancher : sig
  ...
  let of_brancher_info arch bf : t =
    ...
    let other_flows_to_dests flows =
      List.fold flows ~init:[] ~f:(fun acc addr -> (Some !addr, `Jump) :: acc)
    in
    let helper tab (addr, _normal_flow, other_flows) =
      Hashtbl.add_exn tab ~key:!addr ~data:(other_flows_to_dests other_flows);
      tab
    in
    List.fold bf ~init:(Addr.Table.create ()) ~f:helper

  let resolve t mem _ =
    match Hashtbl.find t (Memory.min_addr mem) with
    | Some dests -> dests
    | None -> []
end
...
let load_brancher_info arch =
  Command.create `python ~script:(request "brancher")
    ~parser:(read_brancher_info arch)

let get_resolve_fun file arch =
  let id = Data.Cache.digest ~namespace:"ida-brancher" "%s" (digest file) in
  let brancher =
    match IdaBrancher.Cache.load id with
    | Some i -> i
    | None ->
...
let find_path = function None -> Bap_ida_config.ida_path | Some p -> p
...
module Cmdline = struct
  module Headless = struct
    ...
    let printer fmt t =
      Format.fprintf fmt "%s"
        (match t with Some x -> sprintf "%b" x | None -> "auto")
  ...
  module Mode = struct
    ...
    let printer fmt t =
      Format.fprintf fmt "%s"
        (match t with Some `m32 -> "32" | Some `m64 -> "64" | None -> "auto")
  ...
  let () =
    let () =
      Config.manpage
        [
          `S "DESCRIPTION";
          `P
            "This plugin provides rooter, symbolizer and reconstuctor services.";
          `P
            "If IDA instance is found on the machine, or specified by a\n\
            \        user, it will be queried for the specified information.";
          `S "SEE ALSO";
          `P
            "$(b,bap-ida)(3), $(b,regular)(3),$(b,bap-plugin-byteweight)(1), \
             $(b,bap-plugin-objdump)(1)";
        ]
    in

    let path =
      let doc = "Path to IDA directory." in
      Config.(param (some string) "path" ~doc)
    in
    let headless =
      let doc = "Use headless curses based IDA." in
      Config.(param Headless.t "headless" ~default:None ~doc)
    in
    let mode =
      let doc = "Specify IDA mode." in
      Config.(param Mode.t "mode" ~default:None ~doc)
    in
    Config.declare_extension
      ...
      (fun { Config.get = ( ! ) } ->
        ...
        | Ok info ->
            Bap_ida_service.register info !mode;
            main ()
        | Error e ->
