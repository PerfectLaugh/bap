...
module Ida_config = struct
  type t = { ida_info : Info.t; curses : string option; debug : int }
end
...
let system cmd = if Sys.command cmd <> 0 then raise (Ida.Failed cmd)
let pread cmd = ksprintf run cmd
let shell cmd = ksprintf (fun cmd () -> system cmd) cmd

let idb ida p =
  if Filename.check_suffix ida "64" then ext p "i64" else ext p "idb"
...
let setup_headless_env path =
  ...
  let old_path, new_path =
    try (Unix.getenv var, lib ^ ":" ^ Unix.getenv var)
    with Stdlib.Not_found -> ("", lib)
  in
...
let cleanup_minidump () =
  ...
  if Sys.file_exists dump_path && Sys.is_directory dump_path then (
    FileUtil.ls dump_path |> List.filter ~f:is_dump |> function
    | [] -> ()
    | files ->
        ...
        protect
          ~f:(fun () -> List.iter files ~f:Sys.remove)
          ~finally:(fun () ->
...
let create { Ida_config.ida_info; debug; curses } target mode =
  if not (Sys.file_exists target) then
    invalid_argf "Can't find target executable" ();
  ...
  let self = { ida; exe; curses; trash = [ exe; idb exe; asm exe ]; debug } in
  if Sys.file_exists (idb target) then (
    FileUtil.cp [ idb target ] (idb exe);
    if Sys.file_exists (asm target) then FileUtil.cp [ asm target ] (asm exe))
  else (
    check_path ida_info;
...
let exec (self : ida) command =
  let ext =
    match Command.language command with `idc -> ".idc" | `python -> ".py"
  in
...
let close (self : ida) = if self.debug < 2 then FileUtil.rm self.trash
...
let register ida_info mode : unit =
  let curses = if Info.require_ncurses ida_info then find_curses () else None in
  let debug = try Int.of_string (Sys.getenv "BAP_IDA_DEBUG") with _exn -> 0 in
  let config = { Ida_config.ida_info; curses; debug } in
