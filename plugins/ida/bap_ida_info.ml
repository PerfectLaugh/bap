...
type old_kind = [ `idal | `idal64 | `idaq | `idaq64 ]
[@@deriving sexp, enumerate, equal]

type new_kind = [ `idat | `idat64 | `ida | `ida64 ]
[@@deriving sexp, enumerate, equal]
...
type t = { path : string; ida : ida; is_headless : bool }
type mode = [ `m32 | `m64 ]
...
module Check = struct
  ...
  let require_ida path =
    let open Result in
    require_path path ~error:Ida_not_exists >>= fun () -> require_dir path
  ...
  let check_integrity ida =
    let files =
      [ ida.graphical; ida.headless; ida.graphical64; ida.headless64 ]
    in
    ...
    let expected =
      match ida.headless with
      | `idal -> (all_of_old_kind :> ida_kind list)
      | _ -> (all_of_new_kind :> ida_kind list)
    in
...
let exists_kind path kind = Sys.file_exists (path / string_of_kind kind)
...
let create_ida path =
  let find_kind ~is_headless mode =
    let kinds =
      ...
      | `m64 -> [ `idaq64; `ida64 ]
    in
    List.find ~f:(exists_kind path) kinds |> function
    | Some k -> Ok k
    | None ->
        let kinds = List.map ~f:string_of_kind kinds in
        let files = String.concat ~sep:"/" kinds in
        Error (File_not_found files)
  in
  let version_of_headless = function `idal -> Vold | _ -> Vnew in
  Result.(
...
let create' path is_headless =
  ...
  Result.(
    ...
    check_integrity ida >>= fun () -> Ok { ida; path; is_headless })
...
let create path is_headless =
  ...
  | Error fail ->
      ...
      Or_error.errorf "IDA detection failed with error code %d: %s"
        (code_of_failure fail) (string_of_failure fail)

(* Note, we always launch headless ida in case of IDA Pro 7 *)
let ida32 info =
  match info.ida.version with
  | Vnew -> info.ida.headless
  | Vold -> if info.is_headless then info.ida.headless else info.ida.graphical

let ida64 info =
  match info.ida.version with
  | Vnew -> info.ida.headless64
  | Vold ->
      if info.is_headless then info.ida.headless64 else info.ida.graphical64
...
let ida_of_mode info = function `m32 -> ida32 info | `m64 -> ida64 info
