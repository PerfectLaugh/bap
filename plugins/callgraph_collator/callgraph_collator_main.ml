...
include struct
  ...
  let ignore_reserved =
    flag "ignore-reserved"
      ~doc:
        "Ignore functions that have reserved names, i.e., starting with $(b,_) \
         (underscore) or $(b,.) (dot)."

  let ignore_unresolved =
    flag "ignore-unresolved"
  ...
  let ignore_matching =
    parameter
      ...
      "ignore-matching"
      ~doc:
        "Ignore functions that partially match the speciefied PCRE regular \
         expression."

  let output =
    parameter
      (some ("FILE" %: string))
      "output" ~doc:"Outputs all information into the specified file."
end
...
let () =
  Extension.declare ~provides:[ "collator" ] ~doc @@ fun ctxt ->
  let ppf, close =
    match ctxt --> output with
    | None -> (std_formatter, ignore)
    | Some file ->
        let chan = Out_channel.create file in
        (formatter_of_out_channel chan, fun () -> Out_channel.close chan)
  in
  let match_prefix flag prefix =
    if ctxt --> flag then String.is_prefix ~prefix else Fn.const false
  in
  let match_custom =
    match ctxt --> ignore_matching with
    | None -> Fn.const false
    | Some regexp -> Re.execp @@ Re.compile @@ Re.Pcre.re regexp
  in
  let matchers =
    [
    ...
    object
      ...
      method! enter_jmp jmp (src, calls) =
        ...
        | Some dst when filter src && filter dst ->
            ( src,
              Map.update calls src ~f:(function
  ...
  let prepare proj = (calls_of_project proj, 0) in
  ...
  let collate ver (base, diffs) proj =
    ...
    let diff = Map.symmetric_diff base alt ~data_equal:String.Set.equal in
    let is_different = not (Seq.is_empty diff) in
    if is_different then fprintf ppf "Version %d differs:@\n%!" ver
    else fprintf ppf "Version %d is not different@\n%!" ver;
    Seq.iter diff ~f:(fun (src, diff) ->
        match diff with
  ...
  let print_summary = function
    | 0 ->
        fprintf ppf
          "All versions have the same callgraph as the base version\n%!"
  ...
  Project.Collator.register ~package:"bap" "callgraph" ~prepare ~collate
    ~summary ~desc:"compares binaries by their callgraphs";
