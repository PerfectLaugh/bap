...
module Param = struct
  ...
  manpage
    [
      `S "DESCRIPTION";
      `P
        "Beagle is an obfuscated string solver that uses CPU emulation\n\
  ...
  let dicts = param (list file) "dictionary" ~doc:"Add dictionary file(s)."

  let words =
    param (list string) "words" ~doc:"Add specified words to the dictionary."

  let pwords = flag "print-words" ~doc:"Print all buildable words."
  let pchars = flag "print-chars" ~doc:"Print all observed letters."
  let pstrings = flag "print-strings" ~doc:"Print static strings"

  let no_words =
    flag "no-words" ~doc:"Don't try to build words from a dictionary"

  let pchar =
    param float "noise" ~default:0.05 ~doc:"expected amount of noise characters"

  let alpha =
    param float "alpha" ~default:0.05
      ~doc:"desired probability of a false positive"

  let beta =
    param float "beta" ~default:1e-3
      ~doc:"desired probability of a false negative"

  let p1 =
    param float "text-probability" ~default:0.1
      ~doc:"a prior probability of the search data"

  let alphabet : (module Alphabet) param =
    param
      Strings.Unscrambler.(
        enum
          [
      ...
      "alphabet" ~doc:"Build words from the specified alphabet"

  let get p =
    match Future.peek (determined p) with
    | Some x -> x
...
let hunters_club =
  Primus.Machine.State.declare ~uuid:"1fa35cfe-f720-473c-a5ac-50fa8fa9f1fd"
    ~name:"beagle-hunters-club" (fun _ ->
      {
...
let beagle =
  Primus.Machine.State.declare ~uuid:"5d404352-722d-4ea1-bddb-aa4e4dc65c1d"
    ~name:"beagle" (fun _ ->
      let open Param in
      Beagle
        (Strings.Detector.create ~alpha:(get alpha) ~beta:(get beta)
           ~p1:(get p1) ~ps:(get pchar)
           (make_alphabet (get alphabet))))

let hunter =
  Primus.Machine.State.declare ~uuid:"14d5eef5-4b04-418d-8a9f-3a20f52a5e10"
    ~name:"beagle-hunter" (fun _ ->
      let ( !! ) = Param.get in
      ...
      let dict = List.fold !!Param.words ~f:Unscrambler.add_word ~init:dict in
      let build input =
        Unscrambler.build dict input
        |> Seq.fold ~init:String.Set.empty ~f:Set.add
      in
      Hunter build)
...
module Hunter (Machine : Primus.Machine.S) = struct
  ...
  let process_byte char =
    ...
    Strings.Detector.when_decided d (Machine.return ()) ~f:(fun d ->
    ...
    >>= fun () -> Machine.Local.put beagle (Beagle d)
  ...
  let process_variable (_, r) =
    ...
    | Bil.Imm w ->
        ...
        if Set.mem checked id then Machine.return ()
        else
          process_word w >>= fun () ->
  ...
  let hunt prey =
    ...
    | None ->
        ...
        Machine.Global.update hunters_club ~f:(fun club ->
            { club with cache = Map.set club.cache ~key ~data })
        >>= fun () -> Machine.Observation.make Beagle_prey.catch (prey, data)
    | Some data -> Machine.Observation.make Beagle_prey.catch (prey, data)

  let print_prey prey =
    if Param.(get pchars) then printf "chars: %s\n%!" (Beagle_prey.data prey);
    Machine.return ()

  let print_result (prey, words) =
    if Param.(get pwords) then Set.iter words ~f:(printf "word: %s\n");
    Machine.return ()
...
let main _proj = Primus.Machine.add_component (module Hunter) [@warning "-D"]

let () =
  Config.declare_extension ~doc:"microx-powered obfuscated string solver"
    ~provides:
      [ "deobfuscator"; "microx"; "primus"; "pass"; "string"; "analysis" ]
  @@ fun _ ->
  Primus.Components.register_generic "beagle-hunter"
    (module Hunter)
    ~package:"bap"
    ~desc:
      "Monitors execution and detects data that looks like words from the \
       provided dictionary.";
