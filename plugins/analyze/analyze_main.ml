...
let collect_commands () =
  ...
  |> List.fold
       ~init:(Map.empty (module Knowledge.Name))
       ~f:(fun hints info ->
...
let initial_ctxt ~history directives path =
  {
    ...
    history;
    directives =
      List.fold directives ~init:String.Map.empty
        ~f:(fun dirs (name, _, desc) -> Map.add_exn dirs ~key:name ~data:desc);
  }

let fail problem = Error (Fail problem)

let break_line str =
  String.split ~on:' ' str |> List.filter ~f:(fun s -> not (String.is_empty s))
...
let set_package args ctxt =
  match args with
  | [ pkg ] -> Ok (in_package ctxt pkg)
  | _ ->
      fail
        (Bad_directive
           { dir = "in-package"; msg = "expects exactly one argument" })

let save_base args ctxt =
  match args with
  | [] | [ _ ] ->
      let path = Option.value (List.hd args) ~default:ctxt.path in
      Knowledge.save (Toplevel.current ()) path;
      Ok ctxt
  | _ ->
      fail (Bad_directive { dir = "save"; msg = "takes at most one argument" })

let no_args dir f args ctxt =
  match args with
  | [] -> f ctxt
...
let clear =
  no_args "clear" @@ fun ctxt ->
  LNoise.clear_screen ();
  Ok ctxt

let short str =
  let len =
    min (String.length str) (Option.value (String.index str '.') ~default:40)
  in
...
let help_msg =
  "Type `commands' for the list of available commands or `directives' for the \
   list of directives. Commands, unlike directives, are properly namespaced \
   (packaged), the name of the currently opened namesapce (package) is \
   displayed in the prompt. Most of the commands accept required or optional \
   arguments. Some arguments are keyworded, i.e., them must be prefixed with \
   the specified keyword, e.g.,\n\n\
   instruction /bin/ls:0x8080 :semantics bil\n\n\
   Type `help <command> ...` for the detailed description of the <command>. If \
   more than one command is specified, then the detailed description will be \
   printed for each.\n"
...
let help args ctxt =
  ...
  | _ :: _ :: _ ->
      fail
        (Bad_directive { dir = "help"; msg = "expects zero or one argument" })
  | [ cmd ] -> (
                ...
                (Bad_directive
                   {
                     dir = "help";
                     msg =
                       sprintf "no such command %s in package %s" cmd
                         ctxt.package;
                   })
          | Some info ->
              ...
              Format.printf "SYNOPSIS\n\n%s %s\n\nDESCRIPTION\n@[%a@]\n" cmd
                (Project.Analysis.Grammar.to_string grammar)
                Format.pp_print_text desc;
              Ok ctxt))

let list_directives =
  no_args "directives" @@ fun ctxt ->
...
let find_directive dir =
  List.find_map directives ~f:(fun (s, f, _) ->
      if String.equal s dir then Some f else None)

let dispatch str ctxt =
  match break_line str with
  | [] -> Ok ctxt
  | cmd :: args -> (
      match find_directive cmd with
...
let report_error err = Format.eprintf "%a@\n%!" Extension.Error.pp err

let load_history filename =
  if Sys.file_exists filename then
    match LNoise.history_load ~filename with
    | Ok () -> ()
    | Error msg -> Format.eprintf "Failed to load the history file: %s@\n%!" msg
...
let hint { package; commands } prefix =
  ...
  | cmd :: args -> (
      ...
      | Some info -> (
          let grammar =
            break_line @@ Project.Analysis.Grammar.to_string
            @@ Project.Analysis.grammar info
          in
          match List.drop grammar inputed with
          | [] | [ "" ] -> None
          | xs ->
              Option.return
                (" " ^ String.concat ~sep:" " xs, LNoise.Yellow, false)))
...
let () =
  ...
  let knowledge =
    parameter
      ...
      "project" ~aliases:[ "k" ] ~doc:"The path to a knowledge base."
  in
  let commands =
    arguments
      Extension.Type.("command" %: string)
      ~doc:"The command to execute."
  in
  let script =
    parameter
      Type.("path" %: some file)
      "script" ~aliases:[ "s" ] ~doc:"The path to a script file with commands."
  in
  let history =
    let history_location =
      match Sys.getenv_opt "HOME" with
      | None | Some "" -> ".bap_history"
      | Some home -> Filename.concat home ".bap_history"
    in
    parameter
      Type.("path" %: file =? history_location)
      "history" ~aliases:[ "H" ]
  in
  ...
  if Sys.file_exists base then Toplevel.set @@ Knowledge.load base;
  ...
  | [], None ->
      ...
      interactive_loop ctxt
  | _ -> (
      match run_non_interactive commands script ctxt with
      | Ok _ -> Ok ()
      | Error err -> Error err)
