...
let known_stub_names = [ ".plt"; ".plt.sec"; "__stubs"; ".MIPS.stubs" ]
...
let names =
  Extension.Configuration.parameters
    ...
    ~doc:
      (sprintf
         "The list of known sections that contain function stubs. The names \
          specified with this parameter are appended to the existing list that \
          includes: %s"
      @@ String.concat ~sep:", "
...
let signatures =
  Extension.Configuration.parameters
    Extension.Type.(list path)
    "signatures"
    ~doc:
      ("A list of folders and files that contain signatures for stubs \
        identification. Each file shall have a name of the form \
        $(b,<target>.stubs) and contain a list of words each denoting a \
        possible starting sequence of a bytes for a stub. The <target> is the \
        name of the target, e.g., $(b,arm.stubs), \
        $(b,armv7-linux-gnueabi.stubs), etc. Each word denoting a signature \
        must be encoded as an ASCII number and be binary (start with $(b,0b)), \
        octal (start with $(b,0o), or hexadecimal (start with $(b,0x), e.g., \
        $(b,0xDEADBEEF). If the prefix is omitted then the hexadecimal \
        notation is assumed, e.g., $(b,DEADBEEF) is also acceptable. The \
        signature length is automatically inferred from the word, i.e., the \
        leading zeros are not discarded. By default we search in the current \
        working folder, " ^ user_signatures_folder ^ ", and in "
...
let link_only =
  Extension.Configuration.parameter
    ...
    "link-only"
    ~doc:
      "A list of subroutine names that are to be exclusively considered for \
       redirecting calls to stubs to calls to the implementations. An empty \
       list means that no stubs will be exclusively considered."
...
module Stubs : sig
  ...
  let equal xs ys = Seq.equal Interval.equal (keys xs) (keys ys)

  let inspect xs =
    Seq.sexp_of_t [%sexp_of: Interval.t * string] @@ Ranges.to_sequence xs
  ...
  let collect t stubs =
    ...
    >>| Seq.fold
          ~f:(fun intervals { info = name; addr; size } ->
            if Set.mem stubs name then
              Ranges.add intervals
                Interval.
                  {
  ...
  let stubs ctxt =
    List.fold
      (Extension.Configuration.get ctxt names)
      ~init:(Set.of_list (module String) known_stub_names)
      ~f:(fun init -> List.fold ~init ~f:Set.add)

  let create ctxt t doc =
    match Ogre.eval (collect t (stubs ctxt)) doc with
    | Ok plt -> plt
    | Error err ->
        warning "failed to find plt entries: %a" Error.pp err;
        empty

  let t = KB.Domain.flat ~empty "interval" ~equal ~inspect

  let slot =
    KB.Class.property Theory.Unit.cls "stubs-section" t ~package:"bap"
      ~public:true ~desc:"addresses of the stub sections"

  let declare () =
    let open KB.Rule in
    declare ~package:"bap" "stubs-section"
    |> require Image.Spec.slot |> provide slot
  ...
  let prepare ctxt =
    ...
    KB.collect Image.Spec.slot unit >>| create ctxt t
end
...
module Signatures : sig
  ...
  type parser_outcome = Success of word | Failure of string | Empty | Comment

  let is_prefixed s =
    String.length s > 1
    && match (s.[0], s.[1]) with '0', ('b' | 'o' | 'x') -> true | _ -> false
  ...
  let parse_word s =
    ...
    Word.create (Bitvec.of_string s)
    @@
    match (s.[0], s.[1]) with
    | '0', 'b' -> n
    | '0', 'o' -> n * 3
  ...
  let parse_line s =
    ...
    | s -> (
        let s = String.strip @@ List.hd_exn (String.split s ~on:';') in
        try Success (parse_word s) with Invalid_argument err -> Failure err)
  ...
  let paths ctxt =
    ...
    @ [ user_signatures_folder; system_signatures_folder ]

  let collect ctxt target =
    let paths = paths ctxt in
    let add_file sigs path =
      match parse_filename path with
      | Some triple
        when Sys.file_exists path && Theory.Target.matches target triple ->
          Set.union sigs (parse_file path)
      | _ -> sigs
    in
    let add_files sigs folder =
      try
        Array.fold ~init:sigs (Sys.readdir folder) ~f:(fun sigs path ->
  ...
  let slot =
    KB.Class.property Theory.Unit.cls "stub-signatures" t ~package:"bap"

  let declare ctxt =
    let open KB.Rule in
    declare ~package:"bap" "stub-signatures"
    |> dynamic (paths ctxt)
    |> require Theory.Unit.target |> provide slot
    |> comment "loads the stubs signature file for the given unit"

  let prepare ctxt =
    declare ctxt;
    KB.promise slot @@ fun unit ->
    KB.collect Theory.Unit.target unit >>| collect ctxt
end

let mark_plt_as_stub () : unit =
  KB.Rule.(
    declare ~package:"bap" "stub-resolver"
    |> dynamic [ "code" ]
    |> dynamic [ "stub-resolver:names" ]
    |> require Theory.Label.addr |> require Theory.Label.unit
...
let with_path_and_unit label f =
  KB.collect Theory.Label.unit label >>=? fun unit ->
  KB.collect Theory.Unit.path unit >>=? fun path -> f path unit
...
let detect_stubs_by_signatures () : unit =
  KB.Rule.(
    declare ~package:"bap" "stub-detector"
    |> require Theory.Label.addr |> require Theory.Label.unit
  ...
  if Set.is_empty sigs then !!None
  else
...
let mangle_name addr tid name =
  match addr with
  | Some a -> sprintf "%s@%s" name @@ Word.string_of_value ~hex:true a
...
let update prog ~link_only ~no_link =
  ...
  (object
     ...
     method! map_sub sub =
       ...
       let sub =
         if Set.mem stubs tid then Term.set_attr sub Sub.stub ()
...
let abi_pass ctxt =
  let link_only =
    String.Set.of_list @@ Extension.Configuration.get ctxt link_only
  in
  let no_link =
    String.Set.of_list @@ Extension.Configuration.get ctxt no_link
  in
  Project.map_program ~f:(update ~link_only ~no_link)

let () =
  Extension.declare ~doc @@ fun ctxt ->
