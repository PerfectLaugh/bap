...
module Class = struct
  ...
  let links =
    Knowledge.Class.property t "stub-refs" ~package
      ~desc:
        "Describes unambiguous connections between stubs and their \
         implementations"
    @@ Knowledge.Domain.mapping
         (module Tid)
         "links" ~equal:Tid.equal ~inspect:sexp_of_tid

  let stubs =
    KB.Class.property t "stub-tids" ~package ~desc:"The set of identified stubs"
...
let in_file file f =
  ...
  KB.promising Theory.Label.unit ~promise @@ fun () -> f unit

let is_stub sub =
  if Term.has_attr sub Sub.stub then !!true
  else
    match Term.(get_attr sub address, get_attr sub filename) with
    | None, _ | _, None -> !!true
    | Some addr, Some file ->
        in_file file @@ fun _unit ->
...
let aliases_of_sub s =
  ...
  | Some addr, Some file ->
      ...
      >>= KB.collect Theory.Label.aliases
      >>| Set.union aliases
...
let update_names t sub ~link_only ~no_link =
  ...
  if should_link aliases ~link_only ~no_link then
    let tid = Term.tid sub in
    let names =
      Set.fold aliases ~init:t.names ~f:(fun m a ->
...
let add t sub ~link_only ~no_link =
  ...
  update_units t sub >>= fun t -> update_names t sub ~link_only ~no_link
...
let resolve prog ~link_only ~no_link =
  ...
  Term.to_sequence sub_t prog |> Knowledge.Seq.fold ~init:empty ~f
  >>| fun state -> (state, find_pairs state)
...
let unit_path units x =
  match Map.find units x with
  | None -> !!"(none)"
  | Some unit -> (
...
let run ?(link_only = String.Set.empty) ?(no_link = String.Set.empty) prog =
  Toplevel.current ()
  |> Knowledge.run Class.t (provide prog ~link_only ~no_link)
  |> function
  | Ok (v, _) -> v
  | Error cnf ->
