  ...
  object (self)
    ...
    method! map_term cls t =
      ...
      let taint taint taints =
        ...
        map_if
          (not (Map.is_empty taints))
          ~f:(fun t -> Term.set_attr t taint taints)
      in
      ...
      |> taint Taint.regs regs |> taint Taint.ptrs ptrs
  end

let is_seeded t = Term.has_attr t Taint.reg || Term.has_attr t Taint.ptr

let contains_seed sub =
  Term.enum blk_t sub
  |> Seq.exists ~f:(fun blk -> Term.enum def_t blk |> Seq.exists ~f:is_seeded)

let seeded callgraph subs =
  let callers sub =
    Graphlib.fold_reachable
      (module Graphs.Callgraph)
      callgraph ~rev:true ~init:Tid.Set.empty ~f:Set.add (Term.tid sub)
  in
...
let tids_of_sub sub =
  let terms t p =
    Term.enum t p
    |> Seq.fold ~init:Tid.Set.empty ~f:(fun set t -> Set.add set (Term.tid t))
  in
  let ( ++ ) = Set.union in
...
module State = struct
  ...
  let keys =
    Map.fold ~init:Tid.Set.empty ~f:(fun ~key ~data set -> Set.add set key)

  let update_taints t taints =
    {
      t with
      taints = Taints.union t.taints taints;
      visited = Set.union t.visited @@ keys (Propagator.Result.visited taints);
    }

  let percent (x, y) =
    if y = 0 then 0 else Int.of_float (100. *. (float x /. float y))

  let pp_ratio ppf (x, y) = fprintf ppf "[%d/%d] %3d%%" x y (percent (x, y))
  ...
  let pp_coverage ppf { visited; terms } = pp_ratio ppf (coverage visited terms)
...
let digest_project proj =
  ...
  let add_taint tag t dst =
    match Term.get_attr t tag with
    | None -> dst
    | Some v -> Digest.add dst "%a" Tid.pp v
  in
  (object
     inherit [Digest.t] Term.visitor

     method! enter_term cls t dst =
       add_taint Taint.reg t dst |> add_taint Taint.ptr t
...
let digest_args args = sexp_of_args args |> Sexp.to_string_mach |> Digest.string

let digest args proj =
  Data.Cache.Digest.(add_sexp (digest_project proj) sexp_of_args args)

let process args proj =
  let prog = Project.program proj in
  let callgraph = Program.to_graph prog in
  let is_interesting =
    match args.interesting with
    | [] -> fun _ -> true
    | xs -> fun sub -> List.mem ~equal:String.equal xs (Sub.name sub)
  in
  let subs =
    Term.enum sub_t prog |> Seq.filter ~f:is_interesting |> seeded callgraph
  in
  Term.enum sub_t prog
  |> Seq.filter ~f:(fun sub -> Set.mem subs (Term.tid sub))
  |> Seq.fold
       ~init:(State.create (Set.length subs))
       ~f:(fun s sub ->
         State.report_progress s (Sub.name sub);
         let s = State.entered_sub s sub in
         Propagator.run ~max_steps:args.max_trace ~max_loop:args.max_loop
           ~deterministic:args.deterministic ?random_seed:args.random_seed
           ~reg_policy:args.reg_policy ~mem_policy:args.mem_policy proj
           (`Term (Term.tid sub))
...
let main args proj =
  ...
  let state =
    match State.Cache.load digest with
    | Some s -> s
    | None ->
        ...
        s
  in
  if args.print_coverage then eprintf "@.Coverage: %a@." State.pp_coverage state;
...
module Cmdline = struct
  let () =
    Config.manpage
      [
        `S "DESCRIPTION";
        `P
          "A taint propagation framework, that uses microexecution to\n\
          ...
          \    programs with an unspecified user input. Moreover, to increase \
           the\n\
          \    coverage it may take infeasible paths.";
        `P
          "The taint is propagated from a seed to its maximum extent. The\n\
          ...
          \    the result.";
        `P
          "The microexecution is performed over a lifted program using\n\
          \    bap-microx library. Memory reads are intercepted and redirected \
           to\n\
        ...
        `Pre
          "\n\
          \      - Const - all unknown values are concretized to a specified \
           constant;\n\
          \      - Random - a random value is picked from a value domain;\n\
          \      - Range - a random value is picked from a specified range.";
        `P
          "By default, the microexecution engine tries to visit all\n\
          \    program branches. During the execution, it will record missed\n\
          \    branches as checkpoints. When there is nothing more to explore, \
           it\n\
        ...
        `P
          "The maximum length of an execution path is limited with some\n\
  ...
  let max_trace =
    Config.(
      param int "max-trace" ~default:1_000_000 ~docv:"BLOCKS"
        ~doc:"Limit maximum trace length to $(docv)")

  let max_loop =
    Config.(
      param int "max-iterations" ~default:10 ~docv:"N"
        ~doc:"Limit loop to $(docv) iterations")

  let interesting =
    Config.(
      param (list string) "interesting" ~doc:"Look only at specified functions")

  let deterministic =
    Config.(
      flag "deterministic"
        ~doc:
          "Run in a deterministic mode. In this mode we will follow only one \
           execution path, without backtracking, giving a more feasible \
           result, but much less coverage")

  let print_coverage =
    Config.(flag "print-coverage" ~doc:"print coverage [debug]")
  ...
  module Policy = struct
    ...
    let interval b e = try `Ok (`Interval (num b, num e)) with exn -> error
    ...
    let t default : t Config.converter = Config.converter parser printer default
  end

  let policy key name default : policy Config.param =
    let doc =
      sprintf
        "Input generation policy. If set to a fixed value,\n\
        ...
        \      domain, defined by a type of value."
        name
    in
    Config.(param (Policy.t default) (sprintf "%s-value" key) ~doc)

  let random_seed : int option Config.param =
    let doc = "Initialize random number generator with the given seed" in
    Config.(param (some int) "random-seed" ~doc)

  let create max_trace max_loop deterministic random_seed reg_policy mem_policy
  ...
  let () =
    ...
    Config.declare_extension ~doc:"the legacy taint propagation pass"
      ~provides:[ "taint"; "pass" ] (fun { Config.get = ( ! ) } ->
        let args =
          create !max_trace !max_loop !deterministic !random_seed !reg !mem
            !interesting !print_coverage
        in
