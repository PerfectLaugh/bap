...
type taints = Tid.Set.t Var.Map.t Tid.Map.t [@@deriving bin_io, compare, sexp]
...
module Result = struct
  ...
  let of_context ctxt =
    { visited = ctxt#visited; tainted_regs = ctxt#tvs; tainted_ptrs = ctxt#tms }

  let taints_of_tid taints tid : Taint.map =
    Map.find taints tid |> function None -> Var.Map.empty | Some ts -> ts
...
type result = Result.t [@@deriving bin_io, compare, sexp]
...
class context ?max_steps ?max_loop p =
  object (self : 's)
    ...
    method propagate_mem tid v r : 's =
      ...
      | Bil.Imm a -> {<tms = propagate taints#ptr_taints tms tid v a>}

    method! merge runner =
      let self = super#merge runner in
      self#merge_taints runner

    method merge_taints runner = {<tvs = runner#tvs; tms = runner#tms>}
  end

let taint_reg ctxt x seed = ctxt#taint_reg x (Tid.Set.singleton seed)

let taint_ptr ctxt x seed =
  match Bil.Result.value x with
  | Bil.Mem _ | Bil.Bot -> ctxt
  | Bil.Imm a -> ctxt#taint_ptr a `r8 (Tid.Set.singleton seed)
...
let memory_lookup proj addr =
  ...
  | None -> None
  | Some (mem, _) -> (
      match Memory.get ~addr mem with Ok w -> Some w | _ -> None)
...
let foreach seq ~f =
  SM.get () >>= fun ctxt ->
  Seq.fold seq ~init:(SM.return ()) ~f:(fun m x -> m >>= fun () -> f x)
  >>= fun () -> SM.return ()
...
class ['a] main ?deterministic ?random_seed ?reg_policy ?mem_policy proj =
  ...
  object (self)
    ...
    inherit!
      ['a] Concretizer.main
        ~memory ~lookup ?random_seed ?reg_policy ?mem_policy () as concrete
    ...
    method! lookup v =
      ...
      | Some tid ->
          ...
          SM.put ctxt >>= fun () -> SM.return r

    method! eval_def def =
      self#taint_free_vars def >>= fun () -> super#eval_def def
...
let tid_of_ident = function `Term tid -> tid | `Name str -> tid_of_name str
let run_from_point p biri point = run_from_tid p biri (tid_of_ident point)

let run ~max_steps ~max_loop ~deterministic ?random_seed ~reg_policy ~mem_policy
    proj point =
  let p = Project.program proj in
  let ctxt = new context ~max_steps ~max_loop p in
  let biri =
    new main ~deterministic ?random_seed ~reg_policy ~mem_policy proj
  in
