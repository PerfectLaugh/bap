...
let main file proj =
  ...
  match file with
  | Some name -> Out_channel.with_file name ~f:(fun oc -> output oc syms)
...
let () =
  let () =
    Config.manpage
      [
        `S "DESCRIPTION";
        `P
          "Output symbol information. In the output file, each symbol\n\
          \          is in format of:\n\
          \          `(<symbol name> <symbol start address> <symbol end \
           address>)', e.g.,\n\
  ...
  let file =
    Config.(
      param (some string) "file" ~docv:"FILE"
        ~doc:"Dump symbols to the specified $(docv)")
  in
  Config.declare_extension ~doc:"dumps functions as a sexp-list of blocks"
    ~provides:[ "printer"; "pass" ] (fun { Config.get = ( ! ) } ->
