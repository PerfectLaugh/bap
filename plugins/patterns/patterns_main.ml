...
module Attributes = struct
  ...
  let lookup attrs attr =
    List.find_map attrs ~f:(fun (k, v) ->
  ...
  let const : 'a -> 'a arg = fun x _ -> Some x
end
...
(** The parser combinator library for XML streams.
    ...
    A parser is stateful machine that recognizes a sequence of XML tokens
    (signals). A parser combinator is a function that takes several parsers and
    produces another parser. When a parser recognizes a sequence of tokens it
    generates a value, which witnesses that the parser was used in the
    derivation. A parser may recognize an empty sequence of tokens, i.e., it
    doesn't consume any input, e.g., [return ()] produces [()] without consuming
    any tokens. Values generated by parsers could be a parsers themselves,
    therefore the generated grammar is value-dependent and can recongize
    language that are describe with context-sensitive grammars.

    The XMLM source is destructive so we keep an extra state to keep track of
    the consumed tokens and to enable better error messages. Whether a parser is
    consuming a token or not is stored in its state. It is a dynamic property,
    since a parser could be combined from parsers that either consume or not.
    For example,
    ...
    when [exp] matches a sequence of tokens it could either use the [foo] or
    [bar] deriviation and either consume ([foo]) or not ([bar]) tokens. In the
    following example,

    {[
      let start = exp >>= function `bar -> return 1 | `foo -> return 2
    ]}

    the produced parser [start] will consume tokens if [foo] is in the
    derivation, despite that the parser was created with [return 2].

    The parser may fail because it doesn't recognize the sequence of tokens, the
    soft failure, or because a runtime error happened, the hard error. A soft
    error is normal during parsing and could be backtracked with [<|>] and other
    operators. The hard error can never be backtracked and immediately halts
    parsing. The runtime error can be generated with [error msg] or result from
...
module Parser : sig
  ...
  (** [p >>= f] creates context-dependent parser.
      ...
      If [p] succeeds with [x] then uses [f x] to create a new parser that
      depends on [x]. If it fails, then [f] is not called.
  ...
  (** [p >>| f] maps with [f] the result of [p].
      ...
      The resulting grammar is [R := pq], where [q] is [return (f x)]. The [q]
      parser grammar is [ɛ], i.e., it matches with an empty string. *)

  val ( >> ) : 'a t -> 'b t -> 'b t
  (** [p >> q] composes [p] and [q].

      the generated grammar is [pq] and the result of [p >> q] is the result of
      [q] (the result of [p] is ignored). *)
  ...
  (** [fold p f x] folds over the successive derivation of [p].
      ...
      Generates the longest possible derivation of [p] by trying it as long as
      it consumes input _and_ generates values.

      The resulting parser will consume input if at least one of the derivations
      of [p] was consuming. If [plus] is true and [p] didn't consume input
      (i.e., [f] was never called) then the derivation is rejected and
      [fold ~plus p f x] doesn't generate any value.

      The [fold] function is a generic function that is used to implement
      [star], [many], and [plus]. *)
  ...
  (** [dtd] recognizes DTD or its absence.
      ...
      Matches with the [`Dtd] signal or with an empty sequence. This parser
      never rejects any input. *)
  ...
  (** [any] recognizes any XML tree.
      ...
      The recognized subtree is read (including all chidlren) and discarded. *)

  val tag : string -> attributes t
  (** [tag name] recognizes tag with the given name.

      Recognizes the start-tag of an element with the given [name]. Note, the
      empty-element tag is represented with two XML tokens (signals), the
      start-tag and the end-tag.

      Note, each start-tag should be accompanied with a corresponding end-tag,
      see {!close}. This parser only matches the start-tag. *)

  val start : Xmlm.tag t
  (** [start] recognizes a start tag with any name.

      Produces the name of the recognized tag together with the attributes. *)
  ...
  (** [tree name children] recognizes an element with [children].
      ...
      Recognizes an element with the given [name] that has contents that is
      recognized by [children]. Returns a pair composed of the attributes of the
      matched element and the result of the [children] parser. *)

  val attr : string -> string -> string t
  (** [attr tag attr] recognizes an element with attribute.

      Recognizes and element with the given name that has an attribute named
      [attr], returns the value of that attribute. *)
  ...
  type state = { input : Xmlm.input; empty : bool }
  ...
  let peek s = if Xmlm.eoi s.input then eof s else (s, Ok (Xmlm.peek s.input))

  let ( >>= ) : 'a parser -> ('a -> 'b parser) -> 'b parser =
   fun p f s -> match p s with (_, Error _) as err -> err | s, Ok r -> f r s

  let ( >>| ) p f = p >>= fun x -> return (f x)
  let ( >> ) p q = p >>= fun _ -> q
  let ignore p = p >>| ignore
  let drop = ignore read
  let consumed r = drop >>= fun () -> return r
  let dtd = peek >>= function `Dtd x -> consumed x | _ -> return None

  let start =
    peek >>= function
    | `El_start (name, attrs) -> consumed (name, attrs)
    | s -> reject s

  let close = peek >>= function `El_end -> consumed () | s -> reject s

  let tag name =
    peek >>= function
    | `El_start (tag, attrs) when Name.matches name tag -> consumed attrs
    | s -> reject s
  ...
  let tree name children =
    tag name >>= fun attrs ->
    children >>= fun child -> return (attrs, child) << close
  ...
  let choice parsers = List.reduce_exn parsers ~f:( <|> )

  let fold ?(plus = false) : 'a parser -> ('b -> 'a -> 'b) -> 'b -> 'b parser =
   fun p f xs s ->
    let finish xs empty s =
      ({ s with empty }, if empty && plus then Error Is_required else Ok xs)
    in
    let rec loop xs empty s =
      match p s with
      | ({ empty = false } as s), Ok x -> loop (f xs x) false s
  ...
  let list_rev ?plus : 'a parser -> 'a list parser =
   fun p -> fold p ?plus (fun xs x -> x :: xs) []
  ...
  let pp_error ppf ((lin, col), reject) =
    ...
    | Unexpected `El_end -> Format.fprintf ppf "An unexpected end-tag"
    | Unexpected (`Dtd _) ->
    ...
    | User_error msg -> Format.fprintf ppf "%s" msg
    | Missing_attribute { tag : string; attr : string } ->
        Format.fprintf ppf "The attribute %S of the tag '%s' is required" attr
          tag
    | Is_required -> Format.fprintf ppf "The element is required"
end
...
module Action : sig
  ...
  type action = { name : KB.Name.t; args : string Map.M(KB.Name).t }
  [@@deriving compare, equal, sexp]

  let xml_name ~default (pkg, name) =
    match pkg with
    | "" -> KB.Name.create ~package:default name
    | _ -> (
        match Uri.(host @@ of_string pkg) with
  ...
  let slot =
    KB.Class.property Theory.Value.cls "action-attributes" ~package:"bap"
    @@ KB.Domain.mapping (module KB.Name) ~equal:String.equal "attributes"

  let property =
    KB.Class.property Theory.Program.cls "patterns-action" ~package:"bap"
    @@ KB.Domain.optional ~equal:equal_action "action"

  let args { args } = KB.Value.put slot Theory.Value.Top.empty args

  let pp_arg ppf (name, value) =
    Format.fprintf ppf "%a %S" KB.Name.pp name value

  let pp_args ppf args =
    Format.pp_print_list ~pp_sep:Format.pp_print_space pp_arg ppf
      (Map.to_alist args)

  let pp ppf { name; args } =
    if Map.is_empty args then Format.fprintf ppf "(%a)" KB.Name.pp name
    else Format.fprintf ppf "@[<hv2>(%a@ %a)@]" KB.Name.pp name pp_args args
  ...
  module Library = struct
    ...
    let provide_primitive () =
      ...
      let docs =
        "(patterns-attribute ATTRS NAME) returns the attribute NAME of the \
         attributes ATTRS or NIL if no such attribute is present"
      in
      Sigma.declare ~types ~docs ~package:"bap" "patterns-attribute"
        ~body:(fun _ ->
          KB.return @@ fun _lbl args ->
          match args with
    ...
    let provide_signal () =
      ...
      let docs =
        "(patterns-action ACTION ADDR ATTRS) is signaled when ACTION matches \
         at ADDR. The ATTRS is key-value set of attributes accessible with \
         patterns-attribute"
      in
...
module Pattern : sig
  ...
  module Parser = struct
    ...
    type state = { bits : Z.t; mask : Z.t; size : int; mode : mode }

    let init = { bits = Z.zero; mask = Z.zero; size = 0; mode = Start }
    ...
    let seq s f = List.fold f ~init:s ~f:(fun s push -> push s)

    let run =
      String.fold ~init ~f:(fun s c ->
          match (s.mode, c) with
          | Start, '1' -> bit 1 s
  ...
  let nth_byte size x n =
    ...
    Z.to_int @@ Z.((x asr off) land of_int 0xff)
  ...
  let empty = { repr = ""; bits = [||]; mask = [||]; pops = 0; size = 0 }
  let bits x = x.bits
  let mask x = x.mask
  ...
  let matches { mask; bits } ~pos:n data = data land mask.(n) = bits.(n)
  let pp ppf { repr } = Format.fprintf ppf "%s" repr
end
...
module Target = struct
  ...
  type target = Name of (Theory.Target.t[@sexp.opaque]) | Spec of spec
  [@@deriving compare, sexp]

  type problem = Wrong_endianness | Unrecognized_target
  ...
  let parse str =
    ...
    | _ -> (
        match Theory.Target.lookup ~package:"bap" str with
        | None -> fail Unrecognized_target
        | Some t -> Name t)

  let field_matches equal value field =
    match field with None -> true | Some field -> equal value field
...
module Rule = struct
  ...
  let empty =
    { sizes = None; prepatterns = []; postpatterns = []; actions = [] }
  ...
  let pp ppf { prepatterns; postpatterns; actions } =
    Format.fprintf ppf
      "@[<v>@[<v2>preps {@;\
       ...
       @]"
      pp_patterns prepatterns pp_patterns postpatterns pp_actions actions
end
...
module Grammar = struct
  ...
  let file_with_compiler =
    ...
    file >>= fun file -> return (compiler, file) << close

  let with_compilers id =
    plus file_with_compiler
    >>| List.map ~f:(fun (comp, file) -> (sprintf "%s:%s" id comp, file))

  let without_compilers id = file >>= fun file -> return [ (id, file) ]

  let language =
    attr "language" "id" >>= fun id ->
    with_compilers id <|> without_compilers id << close

  let files = dtd >> tag "patternconstraints" >> plus language << close
  ...
  let patternpairs_open_tag =
    tag "patternpairs"
    >>| Attributes.(
          const (fun x y -> Rule.sizes x y) $ int "totalbits" $ int "postbits")
  ...
  let data_elt =
    data >>| fun p -> { Rule.empty with postpatterns = [ Pattern.create p ] }

  let patterns_with_actions name =
    tag name >> !*(data_elt <|> action) >>= fun matches ->
    return (Rule.collapse matches) << close

  let postpatterns = patterns_with_actions "postpatterns"

  let patternpair =
    required patternpairs_open_tag >>= fun sizes ->
    prepatterns >>= fun prepatterns ->
    postpatterns >>= fun matcher ->
    return { matcher with prepatterns; sizes = Some sizes } << close

  let singlepattern = patterns_with_actions "pattern"
  let entries = star (patternpair <|> singlepattern)
  let patterns = dtd >> tag "patternlist" >> entries << close
end
...
module Rules = struct
  ...
  let empty = { cases = []; known = Set.empty (module Action) }

  let slot =
    KB.Class.property Theory.Unit.cls "pattern-rules" ~package:"bap"
    @@ KB.Domain.flat ~empty ~equal "rules"

  let pp_actions ppf actions =
    Format.pp_print_list ~pp_sep:Format.pp_print_space Action.pp ppf
      (Set.to_list actions)

  let pp_case ppf { pattern; actions } =
    Format.fprintf ppf "@[<hv2>%a ->@ %a@]" Pattern.pp pattern pp_actions
      actions

  let pp ppf { cases } =
    Format.fprintf ppf "@[<v>%a@]" (Format.pp_print_list pp_case) cases
  ...
  let concat x y =
    { cases = x.cases @ y.cases; known = Set.union x.known y.known }

  let patterns_product pre post =
    let pre = match pre with [] -> [ Pattern.empty ] | pre -> pre in
  ...
  let apply addr case f x =
    ...
    Set.fold case.actions ~init:x ~f:(fun x action -> f addr action x)
  ...
  let search cases mem f x =
    let rec outer addr x =
      if Word.(addr < Memory.max_addr mem) then inner addr 0 cases x else x
    and inner addr pos work x =
      if Set.is_empty work.known || List.is_empty work.cases then
        outer (Addr.succ addr) x
      else
        match Memory.get ~index:pos ~addr mem with
        | Error _ -> x
        | Ok byte ->
            let byte = Word.to_int_exn byte in
            let x, work =
              List.fold work.cases
                ~init:(x, { work with cases = [] })
                ~f:(fun (x, work) case ->
                  let hits = Pattern.matches case.pattern ~pos byte in
                  if hits && Pattern.length case.pattern = pos + 1 then
                    (apply addr case f x, remove_actions work case)
                  else
                    ( x,
                      if hits then { work with cases = case :: work.cases }
                      else work ))
            in
            inner addr (pos + 1) work x
    in
...
module Repository : sig
  ...
  let parse_file file grammar =
    In_channel.with_file file ~f:(fun ch ->
        Xmlm.make_input ~strip:true (`Channel ch) |> Parser.run grammar
        |> function
        | Ok result -> result
        | Error problem ->
            warning "failed to parse file %s: %a" file Parser.pp_error problem;
            [])
  ...
  let dedup selected =
    ...
    List.fold selected ~init:(empty, empty) ~f:(fun (digests, files) file ->
        ...
        if Set.mem digests digest || Set.mem files file then (digests, files)
        else (Set.add digests digest, Set.add files file))
    |> snd |> Set.to_list
  ...
  let load roots target =
    select roots target
    |> List.concat_map ~f:(fun path -> parse_file path Grammar.patterns)
    |> Rules.create
...
module Analysis : sig
  ...
  type KB.conflict += Expected of KB.Name.t | Invalid_arity
  ...
  type outcome = { roots : Set.M(Addr).t; names : string Map.M(Addr).t }
  [@@deriving bin_io, compare, sexp, equal]

  let empty =
    { roots = Set.empty (module Addr); names = Map.empty (module Addr) }

  let slot =
    KB.Class.property Theory.Unit.cls "patterns-outcome" ~package:"bap"
      ~persistent:
  ...
  let roots = KB.Context.declare ~package:"bap" "patterns-roots" !!empty_roots
  let names = KB.Context.declare ~package:"bap" "patterns-names" !!empty_names
  ...
  let vec vec = KB.Value.put Sigma.static Theory.Value.Top.empty (Some vec)
  let accept c = c >>| fun _ -> sym "t"
  let reject c = c >>| fun _ -> sym "nil"

  let promise_root addr =
    let* addr = addr.?[Sigma.static] in
    accept @@ KB.Context.update roots (fun roots -> Set.add roots addr)
  ...
  let promise_name addr name =
    ...
    | [], [] -> accept @@ KB.Context.set names (Bap_relation.add rel addr name)
    | _ -> reject @@ KB.return ()

  let declare_promise_root () =
    let types = Primus.Lisp.Type.Spec.(tuple [ int ] @-> bool) in
    Sigma.declare ~types ~package:"bap" "promise-function-start" ~body:(fun _ ->
        !!(fun _ args ->
            match args with
  ...
  let compute_outcome =
    ...
    let+ names = KB.Context.get names in
    {
      roots;
      names =
        Bap_relation.fold names
          ~init:(Map.empty (module Addr))
          ~f:(fun addr name names -> Map.add_exn names ~key:addr ~data:name);
    }

  let reset_context =
    KB.sequence
      [ KB.Context.set roots empty_roots; KB.Context.set names empty_names ]
  ...
  let promise_outcome () =
    ...
    let actions =
      ...
      |> Seq.fold
           ~f:(fun actions (mem, tag) ->
             if Value.is Image.code_region tag then
               collect_actions rules mem actions
             else actions)
           ~init:(Map.empty (module Addr))
    in
    apply_actions unit actions >>= fun () ->
    compute_outcome >>= fun result ->
    reset_context >>| fun () -> result
  ...
  let setup_context () =
    ...
    KB.return @@ Primus.Lisp.Context.create [ ("patterns", [ "enabled" ]) ]
...
let with_rules spec k =
  ...
  | Error err -> warning "Failed to parse the spec: %a@." Parser.pp_error err
  | Ok rules ->
...
let test_on_bytes ~spec bytes =
  ...
  | Error err -> Format.printf "Wrong memory: %a@." Error.pp err
  | Ok mem -> print_matches rules mem

let paths =
  Extension.Configuration.parameters Extension.Type.dir "path"
    ~doc:"Add the specified path to the list of patterns directories."

let path = Extension.Command.argument Extension.Type.file

let spec =
  Extension.Command.parameter Extension.Type.file "specification"
    ~aliases:[ "s" ]

let provides = [ "symbolizer"; "rooter"; "function-starts" ]

let () =
  Extension.Command.(
    declare "match-patterns"
      ~doc:
        "Run the specified patterns file on the input file and prints the \
         matches"
      (args $ spec $ path))
...
let () =
  ...
  let paths =
    ...
    :: Extension.Configuration.get ctxt paths
  in
