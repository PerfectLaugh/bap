...
let run_gc_with_threshold () =
  let cfg = Cache.read_config () in
  if cfg.gc_enabled then GC.shrink ~by_threshold:true cfg

let filename_of_digest = Data.Cache.Digest.to_string

let run_and_exit cmd =
  cmd ();
  exit 0
...
let create reader writer =
  Data.Cache.create ~load:(load reader) ~save:(save writer)

let provide_service () =
  info "caching to %s" (Cache.root ());
  Data.Cache.Service.provide { Data.Cache.create }

let set_dir dir = match dir with None -> () | Some dir -> Cache.set_root dir
...
let update_config sz ov no_gc gc =
  let set f x y = match x with None -> y | Some x -> f x y in
  let is_set = Option.is_some in
  let cfg = Cache.read_config () in
  let cfg' =
    set capacity sz cfg |> set overhead ov |> set disable_gc no_gc
    |> set enable_gc gc
  in
  if is_set sz || is_set ov || is_set no_gc || is_set gc then (
...
let cache_dir =
  Configuration.parameter ~doc:"Use provided folder as a cache directory"
    Extension.Type.("DIR" %: some string)
    "dir"
...
let () =
  ...
  let clean =
    Configuration.flag
      ~doc:
        "Cleanup all caches.\n\
        \         Deprecated, use $(b, bap cache --clean) instead"
      "clean"
  in
  Bap_main.Extension.declare @@ fun ctxt ->
  init (ctxt --> cache_dir);
  if ctxt --> clean then run_and_exit GC.clean;
  provide_service ();
  Ok ()
...
let add_description ?arg name descr =
  let name =
    match arg with None -> name | Some arg -> sprintf "%s=%s" name arg
  in
  let name = "--" ^ name in
...
let enable_gc =
  ...
  Command.parameter ~as_flag:(Some true)
    ~doc:
      "enables garbage collector. The option value will persist\n\
      \          between different runs of the program"
    Extension.Type.(some bool)
    name
...
let disable_gc =
  ...
  Command.parameter ~as_flag:(Some true)
    ~doc:
      "disables garbage collector. The option value will persist\n\
      \          between different runs of the program"
    Extension.Type.(some bool)
    name
...
let capacity =
  ...
  add_description ~arg:"N" name "sets the capacity of the cached data to <N> Mb";
  Command.parameter
    ~doc:
      "Set the capacity of cached data in Mb.\n\
      \          The option value will persist\n\
      \          between different runs of the program"
    Extension.Type.("N" %: some int)
    ~aliases:[ "size" ] name

let overhead =
  let name = "overhead" in
  add_description ~arg:"P" name "sets capacity overhead to <P> percents";
  Command.parameter
    ~doc:
      "Controls the aggressiveness of the garbage collector.\n\
...
let _cmd =
  let doc =
    "provides options to control cache size and cache garbage collector."
  in
  Extension.declare ~doc ~provides:[ "cache" ] (fun _ -> Ok ());
  Extension.Command.(
    declare ~doc "cache"
      (args $ clean $ capacity $ overhead $ run_gc $ disable_gc $ enable_gc
     $ info)
      (fun clean capacity overhead run_gc disable_gc enable_gc info ctxt ->
        init (ctxt --> cache_dir);
