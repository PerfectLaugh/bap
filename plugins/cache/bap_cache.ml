...
module Cfg = struct
  ...
  let gc_threshold c = c.capacity + (c.capacity * c.overhead / 100)
  let default_root = ref None
  let set_root dir = default_root := Some dir

  let write path cfg =
    try Utils.binable_to_file (module T) path cfg
    with e -> warning "Failed to store config file: %s" (Exn.to_string e)
...
let rec mkdir path =
  ...
  if not (Sys.file_exists path) then ensure_dir_exists path
...
let mkdtemp ?(mode = 0o0700) ?tmp_dir ?(prefix = "") ?(suffix = "") () =
  ...
  let rec create name =
    ...
    in
    let path =
      String.concat ~sep:Filename.dir_sep [ tmp; prefix; name; suffix ]
    in
...
let with_temp_dir path ~f =
  ...
  protect
    ~f:(fun () ->
      ...
      f tmp_dir)
    ~finally:(fun () ->
      if dir_exists tmp_dir then FileUtil.rm ~recurse:true [ tmp_dir ])

let mkdir_from_tmp ~target ~f path =
  with_temp_dir path ~f:(fun tmp_dir ->
      f tmp_dir;
      try Unix.rename tmp_dir target with
      (* these errors occur if the destination exists and is not empty *)
      | Unix.(Unix_error (EEXIST, _, _)) | Unix.(Unix_error (ENOTEMPTY, _, _))
        ->
          ()
      | exn -> raise exn)
...
module Upgrade = struct
  ...
  let upgrade_from_index_v2 index dst =
    ...
    with e ->
      warning "can't read entries from index version 2: %s" (Exn.to_string e)

  let upgrade_from_index_v2 file =
    mkdir_from_tmp ~target:(data ()) ~f:(upgrade_from_index_v2 file) (root ())
