...
type ('b, 'e, 't, 's) fsort =
  (('b, 'e, 't) Theory.IEEE754.t, 's) Theory.format Theory.Float.t
  Theory.Value.sort
...
module Make (B : Theory.Core) = struct
  ...
  module B = struct
    ...
    let is_one x = x >>= fun v -> eq x (one (Theory.Value.sort v))
    let is_all_ones x = x >>= fun v -> eq x (ones (Theory.Value.sort v))
    ...
    let of_bool = function true -> b1 | false -> b0
  ...
  let ( >>-> ) x f = x >>= fun x -> f (Theory.Value.sort x) x

  let bind a body =
    a >>= fun a ->
    let sort = Theory.Value.sort a in
    Theory.Var.scoped sort @@ fun v -> B.let_ v !!a (body (B.var v))
  ...
  let unpack fsort x f =
    ...
    B.(ite (is_zero expn) (succ expn) expn) >>>= fun expn -> f sign expn coef

  let unpack_raw fsort x f =
    x >>>= fun x ->
    exponent fsort x >>>= fun expn ->
    raw_significand fsort x >>>= fun coef -> f (fsign x) expn coef
  ...
  let with_sign sign bitv =
    ...
    let bit = Bits.define 1 in
    ite sign
      (append s (one bit) (zero s') lor !!bitv)
  ...
  let fzero fsort sign =
    ...
    ite sign
      ( ones bits >>>= fun ones ->
        (not (ones lsr one bits)) >>>= fun one -> one lor bitv )
      bitv
  ...
  let is_pinf fsort x =
    is_inf fsort x >>>= fun inf -> B.(and_ inf (inv (msb x)))

  let is_ninf fsort x = is_inf fsort x >>>= fun inf -> B.(and_ inf (msb x))
  ...
  let with_special fsort x f =
    ...
    (is_special && msb coef) >>>= fun is_qnan -> f ~is_inf ~is_snan ~is_qnan
  ...
  let is_norml fsort x =
    unpack_raw fsort x @@ fun _ e _ -> B.(non_zero e && inv (is_all_ones e))

  let is_subnormal fsort x = unpack_raw fsort x @@ fun _ e _ -> B.is_zero e
  let is_fpos = B.is_positive
  let is_fneg = B.is_negative

  let is_zero x =
    let open B in
    x >>-> fun s x -> is_zero ((!!x lsl one s) lsr one s)
  ...
  let fsucc fsort x =
    ...
    expn + de >>>= fun expn -> pack fsort sign expn coef
  ...
  let precision fsort =
    let { IEEE754.p } = IEEE754.Sort.spec fsort in
    p

  let bias fsort =
    let { IEEE754.bias } = IEEE754.Sort.spec fsort in
    bias
  ...
  let match_ ?default cases =
    let cases, default =
      ...
      | None ->
          ...
          (List.tl_exn cases, d)
    in
    List.fold cases ~init:default ~f:(fun fin (cond, ok) -> B.ite cond ok fin)
  ...
  let round rm sign coef loss lost_bits f =
    ...
    ite (is_round_up rm sign (lsb coef) grd rnd stk) (succ coef) coef
    >>>= fun coef' ->
  ...
  let nearest_pow2 num =
    let rec find pow n =
      let n' = 2 * n in
      if n' >= num then (pow + 1, n') else find (pow + 1) n'
    in
    find 0 1
  ...
  let clz x =
    ...
    let shifts, _ =
      List.fold shifts ~init:([], 0) ~f:(fun (acc, prev) curr ->
          let total = Stdlib.(curr + prev) in
          ((total, curr) :: acc, total))
    in
    ...
    let rec loop lets x = function
      ...
      | (total, shf) :: shifts ->
          ...
          ite (is_zero (x land mask)) (x lsl of_int sort shf) x
          >>>= fun nextx ->
          ite (is_zero (x land mask)) (of_int sort shf) (zero sort)
          >>>= fun nextn -> loop (nextn :: lets) nextx shifts
    in
  ...
  let possible_lshift expn coef =
    ...
    unsigned sigs diff >>>= fun diff -> ite (clz < diff) clz diff
  ...
  let norm expn coef f =
    let open B in
    expn >>-> fun _exps expn ->
    ite (is_all_ones !!expn) (f !!expn coef) (norm_finite !!expn coef f)

  let msbn x =
    let open B in
    x >>-> fun sort x ->
    clz !!x >>>= fun clz -> of_int sort (Bits.size sort) - clz - one sort
  ...
  let fadd_finite fsort rm x y =
    ...
    guardbits' (sum < xcoef) (lsb sum) loss lost_bits
    @@ fun guard round sticky ->
    ...
    norm expn coef @@ fun expn coef -> pack fsort xsign expn coef

  let bitv_extend bitv ~addend f =
    bitv >>-> fun sort bitv ->
    let sort' = Bits.define (Bits.size sort + addend) in
    B.unsigned sort' !!bitv >>>= fun bitv -> f bitv sort'
    ...
    match_
      [
        ...
        --> ( extract_last ycoef lost_bits >>>= fun loss ->
              ...
              ycoef lsr lost_bits >>>= fun ycoef -> f loss lost_bits xcoef ycoef
            );
        (xexpn < yexpn)
        --> ( extract_last xcoef lost_bits >>>= fun loss ->
              xcoef lsr lost_bits >>>= fun xcoef ->
              ycoef lsl one sigs >>>= fun ycoef -> f loss lost_bits xcoef ycoef
            );
  ...
  let fsub_finite fsort rm x y =
    ...
    ite (is_zero coef) (min_exponent exps) (ite msbc (succ expn) expn)
    >>>= fun expn ->
    guardbits loss lost_bits @@ fun guard' round' sticky' ->
  ...
  let double bitv f =
    bitv >>-> fun sort bitv -> bitv_extend !!bitv ~addend:(Bits.size sort) f

  let normalize_coef coef f =
    let open B in
    coef >>-> fun _sort coef ->
    clz !!coef >>>= fun clz ->
    !!coef lsl clz >>>= fun coef -> f coef clz
  ...
  let fmul_finite fsort rm x y =
    ...
    ite (dexpn >=$ expn) (dexpn - expn + of_int exps' mine) (zero exps')
    >>>= fun underflow ->
  ...
  let fmul fsort rm x y =
    ...
    is_finite fsort y >>>= fun is_y_fin ->
    B.(
      ite (is_x_fin && is_y_fin) (fmul_finite fsort rm x y)
    ...
    let rec loop i bits nomin =
      if Stdlib.(i < 0) then
        ...
        bind next_nomin (fun next_nomin ->
            loop Stdlib.(i - 1) (bit :: bits) next_nomin)
    in
  ...
  let fdiv_finite fsort rm x y =
    ...
    xexpn - yexpn >>>= fun expn ->
    (xexpn < yexpn && yexpn - xexpn > dexpn + bias - mine)
    >>>= fun underflowed ->
    (xexpn < yexpn && yexpn - xexpn > prec' + dexpn + bias - mine)
    >>>= fun is_underflow ->
  ...
  let fdiv fsort rm x y =
    let open B in
    ite
      (is_finite_nonzero fsort x && is_finite_nonzero fsort y)
      (fdiv_finite fsort rm x y) (fdiv_special fsort x y)

  let ftwo fsort = fone fsort B.b0 >>>= fun one -> fadd fsort B.rne one one
  ...
  let truncate fsort x rm fsort' =
    ...
    unpack fsort x @@ fun sign expn coef ->
    if_
      (is_all_ones expn || is_zero expn)
      ~then_:
        ( low (exps fsort') expn >>>= fun expn ->
          high (sigs fsort') coef >>>= fun coef -> pack fsort' sign expn coef )
      ~else_:
        ( expn - of_int (exps fsort) d_bias >>>= fun expn ->
          if_
            (expn > of_int (exps fsort) dst_maxe)
  ...
  let convert fsort x rm fsort' =
    ...
    if size fsort = size fsort' then B.unsigned (bits fsort') x
    else if size fsort < size fsort' then extend fsort x fsort'
  ...
  let cast_int (fsort : _ fsort) outs bitv =
    ...
    unsigned outs coef >>>= fun coef -> ite sign (neg coef) coef
  ...
  let sqrt2 fsort' =
    ...
    B.int (bits fsort) (Bitvec.of_string "0x3fff6a09e667f3bcc908b2fb1366ea95")
    >>>= fun x -> convert fsort x B.rne fsort'
  ...
  let horner fsort x cfs =
    ...
    let rec sum y = function
      ...
      | c :: cs ->
          ...
          y + c >>>= fun y -> sum y cs
    in
    match cfs with [] -> assert false | [ c ] -> c | c :: cs -> sum c cs
  ...
  let coefs s =
    List.map
      ~f:(fun x -> B.int s @@ Bitvec.of_string x)
      [
