...
let normalizations = [ ("0", []); ("1", [ bnf1 ]); ("2", [ bnf2 ]) ]

let fold_consts =
  Bil.register_pass "constant-folding" ~desc:"constant-folding" Bil.fold_consts

let consts_propagation =
  Bil.register_pass "constant-propagation" ~desc:"constant-propagation"
...
let optimizations = [ ("0", []); ("1", o1) ]

let print_passes () =
  Bil.passes ()
  |> List.map ~f:Bil.Pass.to_string
  |> List.sort ~compare:String.compare
  |> List.iter ~f:(printf "%s\n");
  exit 0

let pass =
  Type.enum (List.map (Bil.passes ()) ~f:(fun p -> (Bil.Pass.to_string p, p)))
...
let ispec =
  Type.define
    ~parse:(fun s ->
      ...
      | [ ""; kw ] ->
          invalid_argf "unknown keyword `%s', expects any | unknown | special"
            kw ()
...
let () =
  let norml =
    let doc =
      "Selects a BIL normalization level.\n\
      ...
      \      description of the BIL normalized forms."
    in
    Configuration.(parameter Type.(enum normalizations) ~doc "normalization")
  in
  let optim =
    let doc =
      "Specifies an optimization level.\n\n\
      \      Level $(b,0) disables all optimizations,  and level $(b,1) performs\n\
      \      regular program simplifications, e.g., applies constant folding,\n\
      \      propagation, and elimination of dead temporary (aka virtual) \
       variables."
    in
    Configuration.(
      parameter Type.(enum optimizations =? o1) ~doc "optimization")
  in
  let list_passes =
    let doc = "List all available passes and exit" in
    Configuration.flag ~doc "list-passes"
  in
  let passes =
    let doc =
      "Selects the list and the order of analyses to be applied during\n\
      \       the lifing to BIL code."
    in
    Configuration.(parameter Type.(list pass) ~doc "passes")
  in
  let enable_fp_emu =
    Configuration.parameter
      Type.(bool =? true)
      "floating-point-emulation" ~aliases:[ "enable-fp-emulation" ]
      ~as_flag:true
      ~doc:
        "Enable/disable floating-point emulation (on by default).\n\
    ...
    Configuration.parameters
      ...
      ~doc:
        "Translate the specified instructions into calls to intrinsic \
         ...
         call to an intrinsic function. The following keywords are recognized, \
         $(b,:any) - matches with any instruction, $(b,:unknown) - matches \
         with instructions that have unknown (to our lifters) semantics, \
         $(b,:special) - matches with instructions that have special semantics \
         (expressed with the special statement by our lifters). The following \
         predicates are recognized, $(b,asm:<str>) matches with instructions \
         which assembly strings start with $(b,<str>), $(b,tag:<str>) - \
         matches with instructions that have a tag (kind) that starts with \
         $(b,<str>), $(b,<s1>:<s2>) - matches with instructions that have \
         opcodes starting with $(b,<s2>) in the encoding that starts with \
         $(b,<s1>). For predicates, all string comparisons are made \
         case-insensitive. Example, \
         $(b,:unknown,:special,asm:addsd,llvm:trap)."
  in
  ...
  if ctxt --> list_passes then print_passes ()
    ...
    Theory.declare
      !!(module Bil_semantics.Core : Theory.Core)
      ~package:"bap" ~name:"bil" ~desc:"semantics in BIL"
      ~provides:[ "bil"; "lifter" ];
    if Syntax.(ctxt --> enable_fp_emu) then
      Theory.declare
        !!(module Bil_semantics.Core_with_fp_emulation : Theory.Core)
        ~package:"bap" ~name:"bil-fp-emu" ~extends:[ "bap:bil" ]
        ~desc:"semantics in BIL, including FP emulation"
        ~provides:[ "bil"; "floating-point"; "lifter" ];
    Ok ())
