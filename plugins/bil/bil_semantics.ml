...
module Basic : Theory.Basic = struct
  ...
  module Base = struct
    ...
    let unk s' =
      ...
      | None -> (
          ...
          | Some b -> Bil.unknown "bits" (Type.imm (Theory.Bitv.size b)) %: s'
    ...
    let lift2 mk s f x y =
      x >>-> fun sx x ->
      y >>-> fun sy y -> mk (s sx sy) (f x y)

    let lift3 mk s f x y z =
      x >>-> fun sx x ->
      y >>-> fun sy y ->
      z >>-> fun sz z -> mk (s sx sy sz) (f x y z)
    ...
    let ite cnd yes nay =
      ...
      | Bil.Int w, Bil.Int w' when Word.bitwidth w = 1 ->
          if Word.is_one w && Word.is_zero w' then exp s cnd
          else if Word.is_zero w && Word.is_one w' then exp s (Bil.lnot cnd)
          else exp s (Bil.ite ~if_:cnd ~then_:yes ~else_:nay)
      | _ -> exp s (Bil.ite ~if_:cnd ~then_:yes ~else_:nay)
    ...
    let branch cnd yes nay =
      ...
      match (yes, nay) with
      | [], [] -> eff []
      | _ -> eff Bil.[ if_ (bool_exp cnd) yes nay ]

    let make_cast s t x = x >>-> fun _ x -> vec s @@ Bil.cast t (size s) x
    ...
    let append s ex ey =
      ...
      | _ ->
          if size s < size sx then vec s Bil.(cast low (size s) x)
    ...
    let concat s vs =
      ...
      | _ ->
          ...
          let sz = List.fold ~init:0 vs ~f:(fun sz x -> sz + size (sort x)) in
          let x = List.reduce_exn ~f:Bil.concat @@ List.map vs ~f:bitv_exp in
    ...
    let load mem key =
      ...
      | Some sz -> exp vals Bil.(load ~mem ~addr:key BigEndian sz)
      | None -> unk vals
    ...
    let store m k d =
      ...
      | Some rs -> exp ms Bil.(store ~mem:m ~addr:k d BigEndian rs)
      | _ -> unk ms
    ...
    let let_ var rhs body =
      ...
      body >>-> fun bs body ->
      match rhs with
      | Int _ as rhs -> exp bs @@ recursive_simpl @@ Let (v, rhs, body)
      | _ -> exp bs @@ Bil.let_ v rhs body

    let set lhs rhs = rhs >>-> fun _ rhs -> data [ Bil.(Var.reify lhs := rhs) ]

    let repeat cnd body =
      cnd >>= fun cnd ->
      body >>:= fun body -> data [ Bil.while_ (bool_exp cnd) body ]

    let jmp dst = dst >>= fun dst -> ctrl [ Bil.jmp (bitv_exp dst) ]
    let goto lbl = ctrl [ Bil.(encode goto (Tid.to_string lbl)) ]
  end
  ...
  let extract s hi lo x =
    ...
    | Int h, Int l ->
        let h = Word.to_int_exn h and l = Word.to_int_exn l in
        if h - l + 1 = size s then vec s @@ Bil.(extract ~hi:h ~lo:l (value e))
  ...
  let fbits x = x >>= fun x -> unk @@ Theory.Float.bits (sort x)
end
...
module Emulator = struct
  ...
  let fbits x = x >>| fun x -> resort (Theory.Float.bits (sort x)) x
  let float s x = x >>| fun x -> resort s x
  ...
  let fuop : type f. _ -> _ -> f Theory.float -> f Theory.float =
   fun op rm x ->
    x >>= fun x ->
    let xs = sort x in
  ...
  let classify { Theory.IEEE754.w; t } v ~fin ~inf ~nan =
    ...
    let is_pos = msb v in
    ite is_fin (fin ~is_sub)
      (ite (eq frac zero) (inf ~is_pos) (nan ~is_tss:(msb frac)))

  let tmp x f =
    x >>= fun x ->
    Theory.Var.scoped (sort x) @@ fun v -> let_ v !!x (f (var v))

  let make_cast_float cast s m v =
    match ieee754_of_sort s with
    | None -> CT.unk s
    | Some p -> cast (Theory.IEEE754.Sort.define p) m v >>| resort s
  ...
  let case f x =
    x >>= fun x ->
    with_fsort (sort x) ~unk_s:bool @@ fun bs fs -> f fs !!(resort bs x)
    ...
    | Some { Theory.IEEE754.k; w; t } ->
        ...
        List.reduce_exn ~f:and_
          [
            ...
            or_ (and_ x_is_neg y_is_pos) (ite x_is_neg (ult my mx) (ult mx my));
          ]
end
