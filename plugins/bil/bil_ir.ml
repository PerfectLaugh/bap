...
type cfg = { blks : blk list; entry : Theory.Label.t } [@@deriving bin_io]
type t = cfg

let null = KB.Object.null Theory.Program.cls
let is_null x = KB.Object.is_null x
let is_call jmp = Option.is_some (Jmp.alt jmp)
let is_empty = function { entry; blks = [] } -> is_null entry | _ -> false

let is_unconditional jmp =
  match Jmp.cond jmp with Int w when Word.(w = b1) -> true | _ -> false
...
module BIR = struct
  ...
  let add_term add (blks, b) t =
    add b t;
    (blks, b)

  let add_terms terms add (blks, b) =
    List.fold ~init:(blks, b) (List.rev terms) ~f:(add_term add)
  ...
  *)
  let make_blk { name; defs; jmps } =
    ...
    ([], b) |> add_terms defs add_def |> add_terms jmps add_jmp
    |> fun (blks, b) -> Blk.Builder.result b :: blks |> List.rev
  ...
  let dsts blk = List.filter_map blk.jmps ~f:dst
  ...
  let build_graph =
    List.fold ~init:Tid.Map.empty ~f:(fun blks blk ->
  ...
  let can_contract refs b1 b2 =
    ...
    | Some dst -> (
        ...
        && match Map.find refs dst with Some 1 -> true | _ -> false)

  (* pre: can_contract b1 b2 *)
  let join x y = { x with defs = y.defs @ x.defs; jmps = y.jmps }
  let ( // ) graph node = Map.remove graph node.name
...
let domain =
  KB.Domain.flat ~inspect "graph" ~empty:{ entry = null; blks = [] }
...
module IR = struct
  ...
  let push_to_blk (get, put) blk elt = put blk @@ (elt :: get blk)

  let push fld elt cfg : cfg =
    match cfg with
  ...
  let target =
    KB.Object.scoped Theory.Program.cls @@ fun lbl -> Theory.Label.target lbl

  let goto ?(is_call = false) ?cnd ~tid dst =
    if is_call then Jmp.reify ?cnd ~tid ~alt:(Jmp.resolved dst) ()
    else Jmp.reify ?cnd ~tid ~dst:(Jmp.resolved dst) ()

  let is_subinstruction label =
    KB.collect Insn.Seqnum.slot label >>| Option.is_some
  ...
  let relink label { entry; blks } =
    ...
    if is_null entry then
      ...
      let+ blks =
        List.fold_map blks ~init:`Unbound ~f:(fun r blk ->
            if Theory.Label.equal blk.name entry then
              if blk.keep then (`Relink blk.name, blk)
        ...
        data
          {
            entry = head;
            blks =
              (blk ~keep tail ++ goto ~tid:jmp1 ~cnd loop)
  ...
  let branch cnd yes nay =
    ...
    match (yes, nay) with
    | { entry; blks = [ ({ defs = []; jmps = [ j ] } as blk) ] }, { blks = [] }
      ->
        ret
          {
            entry;
            blks =
              [ { blk with defs = []; jmps = [ Jmp.with_guard j (Some cnd) ] } ];
          }
        ...
        ret
          {
            ...
            blks =
              blk ~keep tail
              :: (blk ~keep head ++ jump ~tid:jmp1 lhs ++ goto ~tid:jmp2 tail)
              :: (b ++ goto ~tid:jmp3 tail)
              :: blks;
          }
        ...
        ret
          {
            ...
            blks =
              blk ~keep tail
              :: (blk ~keep head ++ jump ~tid:jmp1 tail ++ goto ~tid:jmp2 rhs)
              :: (b ++ goto ~tid:jmp3 tail)
              :: blks;
          }
        ...
        ret
          {
            ...
            blks =
              blk ~keep tail
              :: (blk ~keep head ++ jump ~tid:jmp1 lhs ++ goto ~tid:jmp2 rhs)
  ...
  let fall ~tid x dst =
    ...
    | [ jmp ] when is_call jmp ->
        ...
        if is_unconditional jmp then { x with jmps = [ jmp' ] }
        else { x with jmps = [ Jmp.create_goto (Direct dst); jmp' ] }
    | [ jmp ] when is_unconditional jmp -> x
    | jmps -> { x with jmps = goto ~tid dst :: jmps }

  let appgraphs fst snd =
    if is_empty fst then ret snd
    else if is_empty snd then ret fst
    else
      match (fst, snd) with
      | _, { blks = [] } | { blks = [] }, _ -> assert false
      | ( { entry; blks = ({ jmps = [] } as x) :: xs },
          { blks = [ ({ keep = false } as y) ] } ) ->
          ret
            {
              entry;
      ...
      | { entry; blks = x :: xs }, { entry = esnd; blks = y :: ys } ->
          fresh >>= fun tid ->
          ret { entry; blks = y :: fall ~tid x esnd :: List.rev_append xs ys }

  let seq fst snd =
    fst >>-> fun fst ->
    snd >>-> fun snd -> appgraphs fst snd

  let do_goto dst =
    fresh >>= fun entry ->
    fresh >>= fun tid ->
    KB.collect Theory.Label.is_subroutine dst >>= fun is_call ->
    ctrl { entry; blks = [ blk ~keep:false entry ++ goto ?is_call ~tid dst ] }

  let blk label defs jmps =
    defs >>-> fun defs ->
    jmps >>-> fun jmps ->
    appgraphs defs jmps >>-> fun res ->
    if is_null label then ret res else relink label res >>= ret
