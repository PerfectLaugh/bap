...
module Relocations = struct
  type t = { rels : Int64.t Map.M(Int64).t; exts : string Map.M(Int64).t }
  [@@deriving equal]
  ...
  module Request = struct
    ...
    let external_symbols =
      ...
      Seq.fold s
        ~f:(fun addrs (addr, name) -> Map.set addrs ~key:addr ~data:name)
        ~init:(Map.empty (module Int64))
  end
  ...
  let references bil =
    snd @@ reference_analyzer#run bil (Var.Map.empty, Addr.Set.empty)
  ...
  let find src addr =
    match Addr.to_int64 addr with Ok addr -> Map.find src addr | _ -> None
  ...
  let relocations_slot =
    KB.Class.property Theory.Unit.cls "bil-relocations" ~package:"bap"
  ...
  let fixup obj mem bil =
    ...
    | Some unit -> (
        ...
        Theory.Label.target obj >>| Theory.Target.code_addr_size
        >>= fun width ->
        KB.collect (Value.Tag.slot Sub.stub) obj >>| Option.is_some
        >>| fun is_stub ->
        ...
        | None -> (
            ...
            | Some name -> override_external is_stub name bil
            | None -> bil))

  let fixup_agent = KB.Agent.register ~package "bil-fixup-relocator"
...
module Brancher = struct
  ...
  let jmp _ = ret Theory.Effect.Sort.jump Theory.Label.null
  ...
  let blk _ data ctrl =
    ...
    ctrl >>= fun ctrl ->
    let k =
      Theory.Effect.Sort.join
        [ Theory.Effect.sort data ]
        [ Theory.Effect.sort ctrl ]
    in
...
let create_intrinsic target mem insn =
  ...
  @ Bil.
      [
        ...
        Bil.(encode intrinsic)
        @@ sprintf "%s:%s" (Insn.encoding insn) (Insn.name insn);
      ]

type predicate = [ `tag of string | `asm of string | `insn of string * string ]
type ispec = [ `any | `unk | `special | predicate ]

let matches ~pat str =
  String.is_prefix ~prefix:pat @@ String.(uppercase @@ strip str)

let string_of_kind k = Format.asprintf "%a" Sexp.pp (sexp_of_kind k)
...
let split_specs =
  List.fold
    ~init:
      { for_all = false; for_unk = false; for_special = false; predicates = [] }
    ~f:(fun spec -> function
    | `any -> { spec with for_all = true }
    | `unk -> { spec with for_unk = true }
    | `special -> { spec with for_special = true }
    | #predicate as p -> { spec with predicates = p :: spec.predicates })
...
let lift ~enable_intrinsics:{ for_all; for_unk; for_special; predicates } target
    arch mem insn =
  if for_all || matches_spec predicates insn then
    ...
    | Ok bil ->
        if for_special && has_special bil then
          Ok (create_intrinsic target mem insn)
        else Ok bil

let provide_bil ~enable_intrinsics () =
  KB.Rule.(
    declare ~package "bil-lifter"
    |> require Memory.slot
...
let provide_lifter ~with_fp () =
  ...
  let context target =
    ...
    :: (if with_fp then "floating-point" :: base_context else base_context)
  in
  ...
  let lifter obj =
    ...
    if is_empty bil then !!Insn.empty
      ...
      Theory.instance ~context:(context target) () >>= Theory.require
      >>= fun (module Core) ->
      let module Lifter = Theory.Parser.Make (Core) in
      Lifter.run Bil.Theory.parser bil
  in
  KB.Rule.(
    declare ~package "bil-semantics"
    |> require Bil.code
    |> provide Theory.Semantics.slot
...
let init ~enable_intrinsics ~with_fp () =
  ...
  Relocations.prepare ();
  Theory.declare
    !!(module Brancher : Theory.Core)
    ~package ~name:"jump-dests" ~desc:"an approximation of jump destinations"
    ~provides:[ "brancher"; "branch-destinations" ]
