...
let enum_args t =
  let compare x y =
    Option.compare compare_intent (Arg.intent x) (Arg.intent y)
  in
...
let insert_defs prog sub =
  ...
  let insert intent blk jmp sub =
    Option.value_map
      ...
      ~default:sub ~f:(Term.update blk_t sub)
  in
...
let () =
  ...
  Config.declare_extension ~doc:"annotates callsites with subroutines arguments"
    ~provides:[ "pass" ] (fun _ -> Project.register_pass ~deps:[ "abi" ] main)
