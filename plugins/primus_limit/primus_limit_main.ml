...
type counter = Blk | Insn | Term | Exp | Clk
type bound = { counter : counter; limit : int }

module Bound = struct
  let parse_int s =
    try Ok (Int.of_string s) with _ -> Error ("Expected a number, got " ^ s)
  ...
  let make_bound counter s =
    match parse_int s with
    | Ok limit -> `Ok { counter; limit }
    | Error s -> `Error s

  let parse s =
    match counter_of_suffix s with
    | None -> make_bound Clk s
    | Some cnt -> make_bound cnt (String.subo s ~len:(String.length s - 1))
...
type state = { visited : int Name.Map.t; length : int }

let state =
  Primus.Machine.State.declare ~name:"primus-limiter"
    ~uuid:"b1b58dff-54de-4611-abf8-88ff8f6c7481" (fun _ ->
      { visited = Name.Map.empty; length = 0 })
...
module Main (Machine : Primus.Machine.S) = struct
  ...
  let check { limit; counter } _ =
    ...
    if s.length > limit then terminate (string_of_counter counter)
    else Machine.Local.put state { s with length = s.length + 1 }

  let check_max_visits name visited =
    match get Cfg.max_visited with
    | None -> Machine.return ()
    | Some max_visited -> (
        match Map.find visited name with
        | Some visits ->
            if visits > max_visited then
              terminate (sprintf "visits of the %s destination" (Tid.name name))
            else Machine.return ()
        | _ -> Machine.return ())
    ...
    Machine.Local.put state
      {
        s with
        visited =
          Map.update s.visited name ~f:(function None -> 1 | Some n -> n + 1);
      }
  ...
  let init () =
    Machine.sequence
      [ Primus.Interpreter.enter_blk >>> on_blk; register_counter ]
end
...
let () =
  ...
  @@ fun { get } ->
  if Option.is_some (get Cfg.max_length) || Option.is_some (get Cfg.max_visited)
  then Primus.Machine.add_component (module Main) [@warning "-D"];
  Primus.Components.register_generic "limit"
    (module Main)
    ~package:"bap"
    ~desc:
