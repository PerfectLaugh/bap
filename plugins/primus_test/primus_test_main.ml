...
module Location = struct
  ...
  *)
  ...
  type t = trace
  type locations = { points : Set.M(Bitvec_order).t list; traces : trace list }
  ...
  let sexp_of_point { id } addr =
    Sexp.Atom (asprintf "%s:%s" id @@ Z.format "%x" (Bitvec.to_bigint addr))

  let prefixed_trace prefix t =
    let rec build id pos switches trace acc =
      match switches with
      ...
      | _ -> (
          match trace with
          | [] -> acc
          | p :: ps -> build id (pos + 1) switches ps @@ (prefix id p :: acc))
    in
  ...
  let inspect_instance (name, key, locations) =
    Sexp.List
      (Sexp.Atom name :: sexp_of_key key :: List.map locations ~f:sexp_of_key)
  ...
  let new_instance, report_new_instance =
    Primus.Observation.provide "incident-new-instance" ~inspect:inspect_instance
      ~desc:"Occurs on a new instance of the incident class"

  let new_class, report_new_class =
    Primus.Observation.provide "incident-new-class" ~inspect:inspect_instance
  ...
  let trace =
    Primus.Machine.State.declare ~uuid:"3663b645-87a6-4561-b75f-c447cdc221bc"
      ~name:"program-trace"
    @@ fun _ ->
    { trace = []; switches = []; initial = { id = "unknown" }; length = 0 }

  let state =
    Primus.Machine.State.declare ~uuid:"22d66ae9-82ad-4185-a1f8-5eb99f5569df"
      ~name:"incident-locations"
    @@ fun _ ->
    { incidents = String.Map.empty; locations = Primus.Value.Map.empty }

  let push pc machine t =
    let pc = Word.to_bitvec pc and machine = Id.to_string machine in
    match t.trace with
    | [] ->
        {
          trace = [ pc ];
    ...
    | _ ->
        let current =
          match t.switches with [] -> t.initial.id | (_, { id }) :: _ -> id
        in
        let t =
          if Bitvec.equal (List.hd_exn t.trace) pc then t
          else { t with trace = pc :: t.trace; length = t.length + 1 }
        in
        if String.equal current machine then t
        else
          {
            t with
            trace = pc :: t.trace;
  ...
  let set_cls name id cls s =
    {
      s with
      incidents =
        Map.update s.incidents name ~f:(function
          | None -> Primus.Value.Map.singleton id cls
          | Some classes -> Map.set classes ~key:id ~data:cls);
    }

  let classify { incidents; locations } name traces ~new_representative
    ...
    | Some classes -> (
        Map.fold classes ~init:None ~f:(fun ~key ~data:cls found ->
            ...
            | Some _ -> found
            | None ->
                if is_member cls traces then Some (new_instance key)
                else if is_member cls' cls.traces then
                  Some (new_representative key cls')
                else None)
        |> function
        | None -> new_class cls'
        | Some other -> other)
  ...
  module Record (Machine : Primus.Machine.S) = struct
    ...
    let update_backtrace ~f = Machine.Local.update trace ~f

    let record_callsite _ =
      Eval.pc >>= fun addr ->
      Machine.current () >>= fun mach -> update_backtrace ~f:(push addr mach)

    let init () = Primus.Interpreter.leave_blk >>> record_callsite
  end
  ...
  module Make (Machine : Primus.Machine.S) = struct
    ...
    let next incidents =
      match Map.max_elt incidents with
    ...
    let record =
      ...
      Machine.Global.put state
        { s with locations = Map.set s.locations ~key ~data:t }
      >>= fun () ->
...
module Incident = struct
  ...
  let t, report =
    Primus.Observation.provide ~inspect "incident"
      ~desc:
        "Occurs when the incident is reported that involves the given \
         locations."
  ...
  module Report (Machine : Primus.Machine.S) = struct
    ...
    let next name incidents =
      match Map.find incidents name with
      | None -> Value.of_int 1 ~width:63
      | Some classes -> (
          match Map.max_elt classes with
          | None -> Value.of_int 1 ~width:63
          | Some (k, _) -> Value.succ k)
    ...
    let run (name :: locations) =
      ...
      Location.classify s name locations
        ~new_representative:(fun id cls ->
          ...
          Machine.Global.update Location.state ~f:(Location.set_cls name id cls))
        ~new_instance:(fun id -> !!report_new_instance (name, id, locations))
        ~new_class:(fun cls ->
...
module Lisp (Machine : Primus.Machine.S) = struct
  ...
  let def name types closure = Lisp.define ~types name closure
...
let main () =
  ...
  Primus.Components.register_generic ~package:"bap" "incident-location-recorder"
    (module Location.Record)
    ~desc:"Records tracepoints for incident reporting.";
  Primus.Components.register_generic ~package:"bap" "lisp-incidents"
    (module Lisp)
...
manpage
  [ `S "DESCRIPTION"; `P "Primus Testing and Program Verification module." ]
;;
