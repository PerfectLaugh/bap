...
let () =
  Extension.Command.declare ~doc "eval-lisp" Spec.t
...
let string_of_error = function
  | Unknown_arch name ->
      sprintf "Unknown architecture %S. List of supported architectures: %s."
        name
  ...
  | Unknown_system name ->
      sprintf
        "Unknown system %S, see `bap primus-systems` for the list of known \
         systems."
        (Knowledge.Name.to_string name)
  | Conflict err ->
      Format.asprintf "A conflict in the knowledge base: %a"
        Knowledge.Conflict.pp err
  | Primus_exn exn ->
      sprintf "An unexpected Primus exception: %s" (Primus.Exn.to_string exn)
