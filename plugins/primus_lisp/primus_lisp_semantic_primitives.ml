...
let export =
  Primus.Lisp.Type.Spec.
    [
      ( "+",
        all any @-> any,
        "(+ X Y ... Z) returns X + Y + ... + Z, performing any necessary type \
      ...
      ( "-",
        all any @-> any,
        "(- X Y ... Z) returns X - Y - ... - Z, performing any necessary type \
         conversions in the process. If no numbers are supplied returns 0. If \
         one number is supplied returns its negation." );
      ( "-.",
        (tuple [ sym ] // all any) @-> any,
        "(-. MODE X Y ... Z) performs floating-point subtraction, X - Y - ... \
         - Z, using the rounding mode MODE. (-. MODE) evaluates to 0.0." );
      ( "neg",
        one any @-> any,
        "(neg X) returns the negation (2-complement) of X. Same as (- X)." );
      ("lnot", one a @-> a, "(lnot X) returns a bitwise logical negation of X.");
      ( "*",
        all any @-> any,
        "(*  X Y ... Z) returns X * Y * ... * Z, performing any necessary type \
      ...
      ( "*.",
        (tuple [ sym ] // all any) @-> any,
        "(-. MODE X Y ... Z) performs floating-point multiplication, X * Y * \
         ... * Z, using the rounding mode MODE. (*. MODE) evaluates to 1.0." );
      ( "/",
        all any @-> any,
        "(/  X Y ... Z) returns X / Y / ... / Z, performing any necessary type \
         conversions in the process. If no numbers are supplied, 1 is \
      ...
      ( "/.",
        (tuple [ sym ] // all any) @-> any,
        "(-. MODE X Y ... Z) performs floating-point division, X / Y / ... / \
         Z, using the rounding mode MODE. (/. MODE) evaluates to 1.0." );
      ( "s/",
        all any @-> any,
        "(s/ X Y ... Z) returns signed X / Y / ... / Z, performing any \
         necessary type conversions in the process. If no numbers are \
         supplied, 1 is returned. If one number is provided returns its signed \
         reciprocal." );
      ( "mod",
        all any @-> any,
        "(/ X Y ... Z) returns mod(X,Y) Y mod ... mod Z, performing any \
         necessary type conversions in the process. Where `X mod Y` is the \
         remainder of [X / Y]. If no numbers are supplied, 1 is returned. If \
         one number is provided returns that number." );
      ( "signed-mod",
        all any @-> any,
        "(/ X Y ... Z) returns signed mod(X,Y) Y mod ... mod Z, performing any \
         necessary type conversions in the process. Where `X mod Y` is the \
         remainder of [X / Y]. If no numbers are supplied, 1 is returned. If \
         one number is provided returns that number." );
      ( "lshift",
        all any @-> any,
        "(lshift X Y ... Z) returns X << Y << ... << Z, performing any \
         necessary type conversions in the process. Where `X << Y` is logical \
         shift left of X by Y bits. If no numbers are supplied, 1 is returned. \
         If one number is provided returns that number" );
      ( "rshift",
        all any @-> any,
        "(rshift X Y ... Z) returns X >> Y >> ... >> Z, performing any \
         necessary type conversions in the process. Where `X >> Y` is logical \
         shift right of X by Y bits. If no numbers are supplied, 1 is \
         returned. If one number is provided returns that number" );
      ( "arshift",
        all any @-> any,
        "(arshift X Y ... Z) returns X ~>> Y ~>> ... ~>> Z, performing any \
         necessary type conversions in the process. Where `X ~>> Y` is \
      ...
      ( "logand",
        all any @-> any,
        "(logand X Y ... Z) returns X land Y land ... land Z, performing any \
         necessary type conversions in the process. Where `X land Y` is \
         bitwise (logical) / (AND) of X and Y. If no numbers are supplied, 1 \
         is returned. If one number is provided returns that number" );
      ( "logor",
        all any @-> any,
        "(logor X Y ... Z) returns X lor Y lor ... lor Z, performing any \
         necessary type conversions in the process. Where `X lor Y` is bitwise \
         (logical) \\/ (OR) of X and Y. If no numbers are supplied, 1 is \
         returned. If one number is provided returns that number" );
      ( "logxor",
        all any @-> any,
        "(loxgor X Y ... Z) returns X lxor Y lxor ... lxor Z, performing any \
         necessary type conversions in the process. Where `X lor Y` is bitwise \
         (logical) exclusive \\/ (XOR) of X and Y. If no numbers are supplied, \
         1 is returned. If one number is provided returns that number" );
      ( "<.",
        all any @-> any,
        "(< X Y ... Z) is true iff all floating-point numbers are in the \
         strictly increasing order. Synonym to FORDER." );
      ( "forder",
        all any @-> any,
        "(forder X Y ... Z) is true iff all floating-point numbers are in the \
         strictly increasing order." );
      ( "=.",
        all any @-> any,
        "(=. X Y ... Z) is true iff all floating-point numbers are equal in \
         value." );
      ( ">.",
        all any @-> any,
        "(>. X Y ... Z) is true iff all floating-point numbers are in the \
      ...
      ( ">=.",
        all any @-> any,
        "(>=. X Y ... Z) returns true if all floating-point numbers are in the \
      ...
      ( "is-inf",
        all any @-> any,
        "(is-inf X Y ... Z) is true iff all values represent positive or \
         negative infinities." );
      ( "is-fzero",
        all any @-> any,
      ...
      ( "<",
        all any @-> any,
        "(< X Y ... Z) is true iff all numbers are in the strictly increasing \
         order." );
      ( ">",
        all any @-> any,
        "(> X Y ... Z) is true iff all numbers are in the strictly decreasing \
         order." );
      ( "<=",
        all any @-> any,
      ...
      ( "s<",
        all any @-> any,
        "(s< X Y ... Z) is true iff all numbers are in the strictly increasing \
         signed order." );
      ( "s>",
        all any @-> any,
        "(s> X Y ... Z) is true iff all numbers are in the strictly decreasing \
         signed order." );
      ( "s<=",
        all any @-> any,
      ...
      ( "not",
        all any @-> any,
        "(not X Y ... Z) is true iff all numbers are not true. Equivalent to \
         (is-zero X Y Z)" );
      ( "is-positive",
        all any @-> any,
      ...
      ( "word-width",
        all any @-> any,
        "(word-width X Y ... Z) returns the maximum width of its arguments. If \
         no arguments provided returns the size of the machine word." );
      ( "exec-addr",
        one int @-> any,
      ...
      ( "memory-read",
        one int @-> byte,
        "(memory-read PTR) loads one byte from the address PTR (synonymous to \
         load-byte)" );
      ( "load-word",
        one int @-> int,
      ...
      ("unquote", one sym @-> any, "(unquote 'X) evaluates to X");
      ( "is-symbol",
        one any @-> bool,
      ...
      ( "cast-unsigned",
        tuple [ int; a ] @-> b,
        "(cast-unsigned S X) performs unsigned extension of X to the size of S \
         bits" );
      ( "cast-saturate",
        tuple [ sym; int; a ] @-> b,
        "(cast-saturate T S X) performs saturated downcasting\n\
        \    of X to the size of S bits. If X doesn't fit into lower or upper\n\
        \    bound of S then it is saturated to the coresponding value, the \
         keyword\n\
      ...
      ( "nth",
        one any @-> bool,
        "(nth N X) returns the Nth bit of X. N must be static. The function is \
         equivalent to (select N X)" );
      ( "empty",
        unit @-> any,
...
let illformed fmt = Format.kasprintf (fun msg -> KB.fail (Illformed msg)) fmt

let string_of_error name args err =
  Format.asprintf
    "Failed to apply primitive %a: %s@\nApplied as:@\n@[<hov2>(%a %a)@]"
    KB.Name.pp name err KB.Name.pp name
    Format.(pp_print_list ~pp_sep:pp_print_space KB.Value.pp)
    args

let () =
  KB.Conflict.register_printer (function
    | Illformed msg -> Some ("illformed lisp program " ^ msg)
    | Failed_primitive (name, args, err) -> Some (string_of_error name args err)
    | _ -> None)
...
module Saturate = struct
  let max_value bits t =
    ...
    match t with `unsigned -> r | `signed -> Bitvec.((r lsr one) mod m)
  ...
  let static_signed src dst x =
    ...
    if Bitvec.(ubits = zero || B.lnot ubits = zero) then
...
module Primitives (CT : Theory.Core) (T : Target) = struct
  ...
  let pass = seq []
  let skip = seq []
  let negone s = CT.int s @@ Bitvec.(ones mod modulus (size s))
  ...
  let set_const v x = KB.Value.put Primus.Lisp.Semantics.static v (Some x)
  let const_int s x = CT.int s x >>| fun v -> set_const v x
  let set_sym n v = KB.Value.put Primus.Lisp.Semantics.symbol v (Some n)
  ...
  let bool b =
    ...
    let b1 = const_int s Bitvec.M1.one and b0 = const_int s Bitvec.M1.zero in
    match const b with
    | Some r -> if Bitvec.(equal r zero) then b0 else b1
    | None -> CT.ite !!b b1 b0

  let intern name =
    let name = KB.Name.read name in
    KB.Symbol.intern (KB.Name.unqualified name) Theory.Value.cls
    >>| KB.Object.id >>| Int63.to_int64 >>| Bitvec.M64.int64

  let bitv x =
    match Theory.Value.resort Theory.Bitv.refine x with
    | Some x -> !!x
    | None -> (
        match Theory.Value.resort Theory.Bool.refine x with
        | Some b -> bool b
        | None -> (
            match symbol x with
  ...
  let join s xs =
    List.max_elt xs ~compare:(fun x y ->
        ...
        Theory.Bitv.(compare_int (size xs) (size ys)))
    |> function
    | None -> s
    | Some v -> sort v

  let first s = function [] -> s | x :: _ -> sort x

  let with_nbitv s cast xs f =
    match xs with [] -> f s [] | xs -> nbitv xs >>= fun xs -> f (cast s xs) xs

  type 'a bitv = 'a Theory.Bitv.t Theory.Value.sort

  let coerce s x =
    if Theory.Value.Sort.same (Theory.Value.sort x) s then !!x
    else match const x with Some x -> const_int s x | None -> CT.signed s !!x
  ...
  let monoid s cast sf df init xs =
    ...
    | x :: xs ->
        ...
        KB.List.fold ~init xs ~f:(fun res x ->
            ...
            | Some res, Some x -> const_int s Z.(sf res x mod m)
            | _ ->
                let* x = coerce s x in
                df !!res !!x)
        |> forget
  ...
  let ( &&& ) x y =
    ...
    | Some x, Some y ->
        if Bitvec.(equal x zero || equal y zero) then false_ else true_
    | _ -> CT.and_ !!x !!y
  ...
  let rec is_ordered sf df = function
    ...
    | x :: (y :: _ as rest) -> (
        ...
        | Some x, Some y ->
            ...
            is_ordered sf df rest >>= fun r' -> r &&& r'
        | _ ->
            df !!x !!y >>= fun r ->
            is_ordered sf df rest >>= fun r' -> CT.and_ !!r !!r')
  ...
  let all s cast sf df xs =
    ...
    KB.List.fold ~init xs ~f:(fun r x ->
        let r' =
          match const x with Some x -> const_bool Z.(sf x m) | None -> df !!x
        in
        r' >>= fun r' -> r &&& r')
    |> forget
  ...
  let full eff res =
    res >>= fun res ->
    eff >>| fun eff -> KB.Value.put Theory.Semantics.value eff res

  let pure res = full (seq []) res
  let ctrl eff = CT.blk null (seq []) eff
  let data eff = CT.blk null eff (seq [])
  let memory eff res = full CT.(blk null (perform eff) skip) res
  let nop () = CT.perform Theory.Effect.Sort.bot
  let loads = memory Theory.Effect.Sort.rmem
  let stores = memory Theory.Effect.Sort.wmem
  let loads = pure
  let d_is_negative x = CT.msb x
  let d_is_positive x = CT.(and_ (non_zero x) (inv (d_is_negative x)))
  let s_is_negative x m = Z.(msb x mod m)
  let s_is_positive x m = not (Z.(s_is_negative x m) && Z.equal x Z.zero)
  let s_is_zero x _ = Z.equal x Z.zero

  let word_width s xs =
    nbitv xs >>= fun xs ->
    List.max_elt xs ~compare:(fun x y -> Int.compare (bits x) (bits y))
    |> Option.value_map ~f:(fun x -> int s (bits x)) ~default:(int s (size s))
    |> forget
  ...
  let exec_addr xs =
    ...
    | None -> CT.jmp !!dst
    | Some bitv -> Theory.Label.for_addr bitv >>= fun dst -> CT.goto dst
  ...
  let goto_subinstruction lbl xs =
    ...
    | Some dst -> (
        ...
        | Some pos ->
            ...
            Bap.Std.Insn.Seqnum.label dst >>= CT.goto)
  ...
  let is_big_endian t =
    if Theory.Endianness.equal (Theory.Target.endianness t) Theory.Endianness.eb
    then CT.b1
    else CT.b0
  ...
  let word_loader f t xs =
    ...
    unary xs >>= bitv >>= fun addr -> forget @@ CT.(loadw s b mem !!addr)
  ...
  let load_bits t xs =
    ...
    binary xs @@ fun sz x ->
    to_sort sz >>= fun s ->
    bitv x >>= fun x -> forget @@ CT.(loadw s (is_big_endian t) mem !!x)
  ...
  let rec prefix p = function [] -> [] | x :: xs -> (p, x) :: prefix p xs
  ...
  let alias_base_register target v =
    ...
    | None ->
        illformed "the register %a is not an alias in %a" Theory.Var.pp r
          Theory.Target.pp target
    | Some origin -> (
        match Theory.Origin.cast_sub origin with
        | None ->
  ...
  module Intrinsic = struct
    ...
    let parse_param input =
      Option.(symbol input >>= List.Assoc.find ~equal:String.equal params)

    let parse_args args =
      List.fold args ~init:([], [], Inputs)
        ~f:(fun (parsed, current, state) v ->
          match parse_param v with
          | Some state' ->
              let parsed = (state, List.rev current) :: parsed in
              (parsed, [], state')
          | None -> (parsed, v :: current, state))
      |> fun (parsed, last, state) -> List.rev ((state, List.rev last) :: parsed)
    ...
    let get kw =
      List.filter_map ~f:(fun (k, v) -> Option.some_if (equal_param k kw) v)

    let result x = get result x
    let writes x = get writes x
    let stores x = get stores x
    let inputs x = List.concat @@ get inputs x
    let mk_var d i s = Theory.Var.define s (sprintf "intrinsic:%c%d" d i)
    let ivar = mk_var 'x'
    let ovar = mk_var 'y'
    ...
    let invoke_symbol name =
      let name =
        KB.Name.create ~package:"intrinsic" KB.Name.(unqualified @@ read name)
      in
    ...
    let store_word t = mk_store (Theory.Target.data_addr_size t) t
  ...
  let symbol_concat s syms =
    ...
    | Some syms ->
        let syms, sep =
          List.split_while syms ~f:(Fn.non @@ String.equal ":sep")
        in
        let* sep =
          match sep with
          | [] -> KB.return ""
          | [ _; sep ] -> KB.return sep
          | _ -> illformed ":sep must be last and followed by an argument"
        in
  ...
  let mksymbol s v =
    ...
    | None -> illformed "symbol requires a symbolic value"
  ...
  module Saturate = struct
    ...
    let cast xs =
      ...
      | Some x, `signed ->
          forget @@ const_int ds @@ static_signed (size ss) (size ds) x
      | Some x, `unsigned ->
          forget @@ const_int ds @@ static_unsigned (size ss) (size ds) x
  end
  ...
  let extract xs =
    ...
    | Some hi, Some lo -> (
        if hi < lo then illformed "extract: high must be no les the low"
        else
          let s = Theory.Bitv.define (hi - lo + 1) in
          forget
          @@
          match const x with
          | None -> CT.extract s (int s hi) (int s lo) !!x
          | Some x -> const_int s @@ Bitvec.extract ~hi ~lo x)
        ...
        | None -> (
            ...
            | b :: bs ->
                ...
                KB.List.fold bs ~init ~f:(fun r b ->
                    ...
                    CT.append s !!r (CT.extract b1 (int s b) (int s b) !!x))))
  ...
  module IEEE754 = struct
    module Host = struct
      ...
      let round dir x = prj64 (Float.round ~dir (inj x))
      let cast_float _ x = prj64 (Float.of_int64 (Z.to_int64 x))
      let cast_int _ x = Z.M64.int64 (Float.to_int64 (inj x))
      let zero = prj 0.0
      let one = prj 1.0
      ...
      let is cls x _ = Float.Class.compare cls (Float.classify (inj x)) = 0
    ...
    let bop op fs rm x y =
      let* x = inj fs x and* y = inj fs y in
      let* r = op rm !!x !!y in
      prj !!r
    ...
    let with_1bitv xs k =
      ...
      | [ x ] ->
          ...
          k (sort x) x
      | _ ->
          illformed "floating-point operators require mode and a single operand"

    let with_cast_ops xs k =
      match xs with
      | [ x; y ] ->
      ...
      | _ ->
          ...
          illformed
            "the cast operaton requires rmode and two operands, but %d %s \
             provided"
            n
    ...
    let fsort size =
      ...
      | None ->
          illformed "unsupported floating-point IEEE754 format: binary%d" size
    ...
    let operator_rm sf df xs =
      ...
      match const x with
      | Some x when size s = 64 -> forget @@ const_int s (sf sm x)
      | _ ->
    ...
    let operator sf df x =
      ...
      let s = sort x in
      match const x with
      | Some x when size s = 64 -> forget @@ const_int s (sf x)
      | _ ->
          let* fs = fsort (size s) in
          forget @@ df fs !!x
    ...
    let cast_int sf df xs =
      ...
      match const x with
      | Some x when ds = 64 && size sx = 64 -> forget @@ const_int s (sf sm x)
      | _ -> forget @@ df is rm (inj fs !!x)
    ...
    let monoid_rm s cast sf df (si, di) args =
      ...
      if size = 64 then monoid s cast sf (df fs rm) si xs
      else dynamic s cast (df fs rm) (di fs s rm) xs

    let lt x y =
      let* s = x >>| sort >>| size >>= fsort in
      let* x = inj s x and* y = inj s y in
      CT.forder !!x !!y
  ...
  let dispatch lbl name args =
    ...
    | "cast-sfloat", _ ->
        pure @@ F.cast_float F.Z.cast_float CT.cast_sfloat args
    ...
    | "get-program-counter", [] | "get-current-program-counter", [] ->
        pure @@ get_pc s lbl
