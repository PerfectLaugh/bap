          ...
          Map.update library package ~f:(function
            ...
            | Some cats ->
                Map.update cats cat ~f:(function
                  | None -> name %: desc
                  | Some elts -> Map.set elts ~key:name ~data:desc))))

let pp_source ppf desc =
  Format.fprintf ppf "#+BEGIN_SRC lisp\n;; %a\n%a\n#+END_SRC\n"
    Doc.Descr.pp_location desc Doc.Descr.pp_source desc

let pp_descr ppf desc =
  let pp = if Doc.Descr.has_source desc then pp_source else Doc.Descr.pp in
  ...
  List.iter index ~f:(fun (cat, elts) ->
      ...
      List.iter elts ~f:(fun (name, desc) ->
          if String.equal (KB.Name.package name) package then
            printf "** ~%s~@\n%a@\n" (KB.Name.unqualified name) pp_descr desc))

let print = function None -> print_library | Some p -> print_package p

let string_of_problem = function
  | Wrong_target s ->
      sprintf
        "Unknown target %S, see `bap list targets' for the list of known \
         targsts"
        s
  | Wrong_system s ->
      sprintf
        "Unknown system %S, see `bap primus-systems' for the list of known \
         systems"
        s
  | Conflict err ->
      sprintf "Failed to initialize Primus.@\n%s@." (KB.Conflict.to_string err)
  | Unexpected_status exn ->
      sprintf "Failed to initialize Primus.@\n%s@." (Primus.Exn.to_string exn)
...
let print_dynamic package target system =
  ...
  let init =
    ...
    generate_index >>| print package
  in
  match Primus.System.run system proj state ~init with
  | Ok (Normal, _, _) | Ok (Exn Primus.Interpreter.Halt, _, _) -> Ok ()
...
let system =
  Extension.Command.parameter
    Extension.Type.(string =? "bap:legacy-main")
    "system" ~doc:"Print the documentation for the specified system"

let target =
  Extension.Command.parameter
    Extension.Type.(string =? ":unknown")
    "target" ~doc:"Print the documenentation for the specified target"

let semantics =
  Extension.Command.flag "semantics"
    ~doc:"Print the documentation for Primus Lisp semantics lifter"

let package =
  Extension.Command.parameter
    Extension.Type.(some string)
    "package" ~doc:"Print the documentation for the specified package."
...
let () =
  ...
  | Some target -> (
      if semantics then print_static package target
        ...
        | Some system -> print_dynamic package target system)
