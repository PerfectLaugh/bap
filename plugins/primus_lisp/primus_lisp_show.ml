let doc =
  "Shows the static semantics of Primus Lisp definitions.\n\n\
   # DESCRIPTION\n\n\
  ...
  \  (set R0 (/= 1 2 3 4)))\n\
   ...
   version $(b,o)), e.g., the following will print only the BIL \
   representation,\n\n\
   ```\n\
...
module Spec = struct
  ...
  let bitvec =
    Type.define Bitvec.zero ~parse:Bitvec.of_string ~print:Bitvec.to_string

  let unknown = KB.Name.create ":unknown"
  let name = Type.define unknown ~parse:KB.Name.read ~print:KB.Name.show

  let target =
    Type.define Theory.Target.unknown ~print:Theory.Target.to_string
      ~parse:(Theory.Target.get ~package:"bap")

  let names = Command.arguments name
  let target = Command.parameter target "target" ~aliases:[ "t"; "arch" ]

  let slots =
    Command.parameters Type.(list string) "slots" ~aliases:[ "s"; "o" ]

  let addr = Command.parameter Type.(some bitvec) "address" ~aliases:[ "a" ]
  let t = args $ target $ names $ slots $ addr
end

let show target slots addr name =
  let pp =
    match List.concat slots with
    | [] -> KB.Value.pp
    | ss -> KB.Value.pp_slots ss
  in
  Toplevel.try_exec
...
let () =
  Extension.Command.declare ~doc "show-lisp" Spec.t
  @@ fun target names slots addr _ctxt ->
  List.map names ~f:(show target slots addr)
  |> Result.all_unit
  |> Result.map_error ~f:(fun err -> Failed (Conflict err))

let string_of_error = function
  | Conflict err ->
      Format.asprintf "Failed to reify the program: %a" KB.Conflict.pp err
