...
module Primitives (Machine : Primus.Machine.S) = struct
  ...
  let desc_of_op (type a) (op : a op) name =
    ...
    | Ord ->
        sprintf "returns true if all operands are ordered with the %s order"
          name
  ...
  let define (type f) (k : f op) name (f : f) =
    let module Op (Machine : Primus.Machine.S) = struct
      ...
      let if_empty ops f =
        if List.is_empty ops then f
        else Lisp.failf "%s: expects exactly three arguments" name ()
      ...
      let run = function
        ...
        | sz :: op :: ops -> (
            match Word.to_int (Value.to_word sz) with
            | Error _ -> Lisp.failf "%s: expects format as the first a" name ()
            | Ok n when n <> 32 && n <> 64 ->
            ...
            | Ok width -> (
                ...
                | Inf ->
                    ...
                    |> word_of_float width |> Value.of_word
                | Pre ->
                    if_empty ops @@ Value.of_word @@ word_of_float width (f op)
                | Cti ->
                    if_empty ops @@ Value.of_word @@ Word.of_int64 ~width (f op)
                | Ord ->
                    Value.of_word @@ Word.of_bool
                    @@ ordered f (op :: List.map ~f:value_of_word ops)
                | Cnd -> Value.of_word @@ Word.of_bool @@ f op))
    end in
    Lisp.define ("ieee754-" ^ name)
      (module Op)
      ~docs:(desc_of_op k name) ~types:(type_of_op k)

  let is_nan x = Float.Class.(compare Nan (Float.classify x) = 0)
...
let init () =
  ...
  Primus.Components.register_generic "lisp-ieee754"
    (module Primitives)
    ~package:"bap"
