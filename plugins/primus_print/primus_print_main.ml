...
module Param = struct
  ...
  let output =
    param (some string) "output"
      ~doc:
        "A name of a file in which to store the monitor output. If\n\
        \      not specified, then outputs result into stdout"

  let rules = param (list string) "rules"

  let traceback =
    param (some int) ~as_flag:(Some 16) "traceback"
      ~doc:
        "Stores and outputs a trace of execution. Takes an\n\
        \      optional argument that limits the traceback length to the\n\
        \      specified number of terms."
end

let starts_with name x = String.length name > 1 && Char.equal name.[0] x

let strip name =
  if starts_with name '+' || starts_with name '-' then String.subo ~pos:1 name
  else name
...
let monitor_provider name ps =
  Primus.Observation.list_providers () |> List.find ~f:(has_name name)
  |> function
...
let id ppf pos = fprintf ppf "%a" Tid.pp (Primus.Pos.tid pos)
...
type state = { trace : Primus.pos list }

let concat streams =
  let stream, main = Stream.create () in
  List.iter streams ~f:(fun stream ->
      Stream.observe stream (fun x -> Signal.send main x));
  (stream, main)
...
let process_rule rule =
  ...
  Stream.parse facts ~init:rule ~f:(fun rule ev ->
      ...
      match facts with [] -> (None, rule) | facts -> (Some facts, rule))

let read_rules filename =
  match Bare.Rule.from_file filename with
  | Ok rules -> rules
  | Error err ->
      let err = asprintf "%a" (Bare.Rule.report_error ~filename) err in
      invalid_arg err
...
let state =
  Primus.Machine.State.declare ~name:"primus-debug"
    ~uuid:"2fdb0758-3233-4d69-b2e6-704b303ac03a" (fun _ -> { trace = [] })

let start_monitoring { Config.get = ( ! ) } =
  let out =
    match !Param.output with
    | None -> std_formatter
    | Some name -> formatter_of_out_channel (Out_channel.create name)
  in
  ...
  let module Monitor (Machine : Primus.Machine.S) = struct
    ...
    let record_trace p =
      Machine.Local.update state ~f:(fun s -> { trace = p :: s.trace })

    let print_trace _ =
      Machine.Local.get state >>| fun { trace } -> print_trace out trace

    let setup_tracing () =
      if Option.is_some !Param.traceback then
  ...
  Primus.Components.register_generic "observation-printer"
    (module Monitor)
    ~package:"bap"
    ~desc:
      "Prints the specified set of observations. Controlled via the \
       primus-print plugin.";
