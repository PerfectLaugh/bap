...
module Make_unresolved (Machine : Primus.Machine.S) = struct
  ...
  let exec = Linker.exec (`symbol Primus.Linker.unresolved_handler)
end
...
module SetupPLT (Machine : Primus.Machine.S) = struct
  ...
  let section_memory sec_name =
    ...
    |> Memmap.to_sequence |> Seq.map ~f:fst

  let load_table =
    Machine.gets Project.target >>| Theory.Target.bits >>| Size.of_int_exn
    >>= fun word_size ->
    section_memory ".got.plt" >>| fun memory ->
    Seq.fold memory ~init:[] ~f:(fun acc mem ->
  ...
  let unresolve = load_table >>= filter_plt >>= unlink

  let correct_sp ignore sp addend (name, _) =
    if Set.mem ignore name then Machine.return ()
    else
      Interpreter.get sp >>= fun x ->
      Interpreter.binop Bil.plus x addend >>= Interpreter.set sp
  ...
  let run () =
    ...
    | `x86 -> Machine.sequence [ correct_sp ignore IA32.sp 4; unresolve ]
    | `x86_64 -> Machine.sequence [ correct_sp ignore AMD64.sp 8; unresolve ]
    | _ -> Machine.return ()
...
module InitializeRegisters (Machine : Primus.Machine.S) = struct
  ...
  let registers (module Vars : X86_env.ModeVars) =
    Vars.[ gdt; ldt; fs_base; gs_base ] @ Array.to_list Vars.ymms
...
module InitializeSpecialRegisters (Machine : Primus.Machine.S) = struct end
