...
type ref = Addr of Bitvec.t | Name of string [@@deriving compare, sexp, equal]
...
module References : sig
  ...
  let slot =
    KB.Class.property Theory.Unit.cls "refs" ~package:"bap" ~public:true
      ~desc:"external references"
    @@ KB.Domain.flat ~empty "refs" ~inspect:sexp_of_t ~equal
  ...
  let merge_name m x y =
    ...
    | Bad -> Bad
    | Ref (Addr _) as y -> y
    | Ref (Name _) as x -> if compare_value x y = 0 then y else Bad
...
let plt_agent =
  Knowledge.Agent.register ~package:"bap" "plt-symbolizer"
    ~desc:"extracts symbols from external relocations"
    ~reliability:Knowledge.Agent.reliable

open KB.Syntax

let optimize = Bil.fixpoint @@ Fn.compose Bil.propagate_consts Bil.fold_consts

let collect_insns number_of_instructions entry =
  let return bils = KB.return @@ optimize @@ List.(concat @@ rev bils) in
...
let demangle s =
  match String.chop_suffix ~suffix:":external" s with None -> s | Some s -> s

let extract_external stmt =
  match Bil.(decode call stmt) with
...
let find_references =
  List.filter_map ~f:(function
    ...
    | Bil.Jmp (Int dst)
    | Bil.Move (_, Load (_, Int dst, _, _)) ->
        Some (Addr (Word.to_bitvec dst))
...
let addresses mem =
  ...
  Seq.init len ~f:(Addr.nsucc start) |> Seq.map ~f:Word.to_bitvec

let matches refs ref mem =
  match ref with
  | Some (Name _) as ref -> ref
  | ref ->
      let addr =
        match ref with Some (Addr addr) -> Seq.singleton addr | _ -> Seq.empty
      in
