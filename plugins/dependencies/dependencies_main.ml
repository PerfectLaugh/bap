...
module Ldconfig : sig
  ...
  let load ldconfig =
    ...
    protect
      ~f:(fun () ->
        In_channel.fold_lines ldconfig
          ~f:(fun cache s ->
      ...
      ~finally:(fun () ->
        ...
        | WEXITED 0 -> ()
        | WEXITED n -> warning "ldconfig terminated with a non-zero code: %d" n
...
module Spec = struct
  ...
  let imports =
    Ogre.foreach
      Ogre.Query.(
        select
          (from external_reference $ named_symbol)
          ~join:[ [ field name ] ]
  ...
  let exports =
    Ogre.foreach
      Ogre.Query.(
        select (from named_symbol) ~where:(named_symbol.(addr) <> int 0L))
      ~f:(fun (_, name) -> name)

  let libraries = Ogre.collect Ogre.Query.(select (from require))
  let set_of_seq = Seq.fold ~f:Set.add ~init:(Set.empty (module String))

  let build name =
    imports >>| set_of_seq >>= fun imports ->
    exports >>| set_of_seq >>= fun exports ->
    libraries >>| Seq.to_list >>| fun libraries ->
    { name = Filename.basename name; libraries; imports; exports }

  let query name image =
    match Ogre.eval (build name) (Image.spec image) with
    | Error err -> failwithf "Query failed: %s" (Error.to_string_hum err) ()
    | Ok r -> r
end
...
module Unit = struct
  ...
  module Io = struct
    type t = info [@@deriving bin_io]
  end

  let reader =
    Data.Read.create () ~of_bigstring:(Binable.of_bigstring (module Io))

  let writer =
    Data.Write.create () ~to_bigstring:(fun data ->
        Binable.to_bigstring (module Io) data)

  let search_cache cache name =
    match cache with None -> None | Some cache -> Ldconfig.lookup cache name

  let reroot ?root path =
    match root with
    | Some root when Filename.is_absolute path -> Filename.concat root path
    | _ -> path
  ...
  let search ?root ?ldconfig paths file =
    ...
    if Sys.file_exists file then Some file
      ...
      match search_cache ldconfig file with
      | Some path -> Some path
      | None ->
          List.find_map paths ~f:(fun folder ->
  ...
  let load ?root ?ldconfig ~context paths name =
    ...
    | None ->
        ...
        empty name
    | Some name -> (
        let empty = Data.Cache.Digest.create ~namespace:"dependencies" in
  ...
  let print_set ~pp_nil ~pp_sep ~pp_elt ppf elts =
    ...
    | Some last ->
        Set.iter elts ~f:(fun elt ->
            pp_elt ppf elt;
            if not (String.equal elt last) then pp_sep ppf ())
  ...
  module Sexp = struct
    ...
    let pp_set =
      print_set ~pp_nil:Format.pp_print_space ~pp_sep:Format.pp_print_space
        ~pp_elt:Format.pp_print_string

    let pp_list_field ppf = function
      | _, [] -> ()
      | name, elts -> Format.fprintf ppf "@ @[<hv2>(%s@ %a)@]" name pp_list elts

    let pp_set_field ppf (name, elts) =
      if not (Set.is_empty elts) then
        Format.fprintf ppf "@ @[<hv2>(%s@ %a)@]" name pp_set elts

    let pp ppf { name; imports; exports; libraries } =
      Format.fprintf ppf "@[<hv2>(%s%a%a%a)@]" name pp_set_field
        ("imports", imports) pp_set_field ("exports", exports) pp_list_field
        ("libraries", libraries)
  end
...
@]|}
  ...
  module Yaml = struct
    ...
    let pp_list ppf xs =
      match xs with [] -> pp_nil ppf () | xs -> pp_elts ppf xs
...
module State = struct
  type t = { root : string; units : Unit.t Map.M(String).t }
  ...
  let load ?root ?ldconfig ~recursive ~context paths path =
    ...
    let init =
      { root = path; units = Map.singleton (module String) path unit }
    in
    ...
    and load_deps state unit =
      List.fold unit.libraries ~init:state ~f:closure
    in
    if recursive then load_deps init unit else init
  ...
  let pp_graph ppf g =
    Format.fprintf ppf "@[digraph %S {@\n%a}" g.root (print_units Unit.Deps.pp)
      g
  ...
  let pp_json ppf g =
    Format.fprintf ppf "@[<hv2>{%a}@]"
      (print_units ~pp_sep:pp_comma Unit.Json.pp)
      g
...
let recursive =
  Extension.Command.flag "recursive" ~aliases:[ "r" ]
    ~doc:"Calls the commmand recursively on the obtained dependencies. "

let ldconfig =
  Extension.Command.parameter
    Extension.Type.(some string)
    "ldconfig" ~as_flag:(Some "ldconfig -p")
    ~doc:
      "A command that returns an ldconfig-like cache. If the option is \
       specified without an argument then a system ldconfig is used (with the \
       $(b,-p) option) to obtain the library cache. If an argument is passed \
       it should be a command that returns a list of entries, separated with \
       $(b,=>), with the right entry being a path to a library. (See \
       $(b,ldconfig -p) for an example."
...
let root =
  Extension.Command.parameter
    ...
    "root" ~aliases:[ "R" ]
    ~doc:
      "Specify the default root directory. All absolute paths (including those \
       that are specified on the command line) are prefixed with the specified \
       root."

let formats =
  Extension.Type.enum
    [ ("yaml", `Yaml); ("sexp", `Sexp); ("json", `Json); ("graph", `Graph) ]
...
let format = Extension.Command.parameter formats "format" ~aliases:[ "o" ]
...
let () =
  ...
  @@ fun root ldconfig fmt recursive paths input ctxt ->
...
let () =
  Extension.declare ~doc ~provides:[ "depedencies"; "command" ] (fun _ -> Ok ())
