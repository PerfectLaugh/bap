...
(** Trivial Condition Form (TCF) transformation.
    ...
    In the TCF a condition expression must be either a variable or a constant.
    The transformations detect non-trivial condition expressions and bind them
    to variables whose definitions are pushed to the block definition list. *)
module TCF = struct
...
let () =
  declare_extension ~doc:"eliminates complex conditional on branches"
    ~provides:[ "pass"; "simplification" ]
  @@ fun _ -> Project.register_pass ~runonce:true main
