...
let word_of_string width str =
  Option.try_with (fun () -> Addr.of_int64 ~width (Int64.of_string str))
...
let term_name x =
  object
    inherit [bool] Term.visitor
    method! enter_term cls t _ = String.equal (Term.name t) x
  end

let def_lhs x =
  object
    inherit content_visitor
    method! enter_def t _ = String.equal (Var.name (Def.lhs t)) x
  end
...
let def_uses x =
  object
    ...
    method! enter_def t _ =
      Set.exists (Def.free_vars t) ~f:(fun y -> String.equal x (Var.name y))
  end
