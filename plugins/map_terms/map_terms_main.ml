...
module Scheme = struct
  ...
  let parse_exp1 uns nns tag v =
    match lookup1 uns nns tag with
    | Error err -> Error err
    | Ok parse_arg -> (
        try Ok (parse_arg v) with Parse_error msg -> Error msg)
  ...
  let rec parse_exps nns uns = function
    ...
    | List exps -> List.map exps ~f:(parse_exps nns uns) |> List.concat
    | s -> [ error "expected <expr> got %s" @@ Sexp.to_string s ]

  let parse_mappers s =
    parse_exps Mappers.Nullary.find Mappers.Unary.find s |> Result.all

  let parse_preds s =
    parse_exps Predicates.Nullary.find Predicates.Unary.find s |> Result.all

  let parse_marker ps ms =
    match (parse_preds ps, parse_mappers ms) with
  ...
  let parse_string s =
    try parse (Sexp.of_string s) with
    | Sexp.Parse_error err -> sexp_error err
    | exn -> Error "Malformed sexp"

  let parse_file f =
    try List.map ~f:parse (Sexp.load_sexps f) |> Result.all with
    | Sexp.Parse_error err -> sexp_error err
    | Sys_error e -> Error e
    | exn -> Error "Malformed sexp"

  let parse_arg s =
    match parse_string s with Ok r -> `Ok r | Error e -> `Error e

  let arg = Config.converter parse_arg (fun ppf _ -> ()) ([], [])
end

class marker (patts : Scheme.t) =
  object (self)
    inherit Term.mapper as super

    method! map_term cls t =
      List.fold patts ~init:t ~f:(fun t (preds, maps) ->
          if List.for_all preds ~f:(fun p -> p#enter_term cls t false) then
            List.fold maps ~init:t ~f:(fun t m -> m#map_term cls t)
          else t)
      |> super#map_term cls
  end
...
let unmarker attr =
  object
    ...
    method! map_term cls t =
      let attrs =
        Dict.filter
          ~f:(fun v -> String.(Value.tagname v <> attr))
          (Term.attrs t)
      in
...
let main patts file =
  let patts =
    ...
    | None -> patts
    | Some file -> (
        match Scheme.parse_file file with
        | Ok ps -> patts @ ps
...
module Cmdline = struct
  ...
  let attr attrs name desc =
    `I
      ( sprintf "$(b,(%s))" name,
        sprintf "%s, where $(i,ATTR) must be one of %s." desc (enum attrs) )
  ...
  module Predicates = struct
    let color attr =
      `I
        ( sprintf "$(b,(has-%s COLOR))" attr,
          sprintf
            "Is satisfied when a term's\n\
    ...
    let term_attr attr =
      `I
        ( sprintf "$(b,(term-%s VALUE))" attr,
          sprintf
            "Is satisfied when a term's\n\
            \    attribute $(b,%s) has the given value"
            attr )

    let term_field attr =
      `I
        ( sprintf "$(b,(term-%s VALUE))" attr,
          sprintf "Is satisfied when a term's\n    $(b,%s) has the given value"
            attr )

    let term_parent =
      `I
        ( sprintf "$(b,(term-parent name))",
          sprintf
            "Is satisfied when a term is a parent for term with a given name" )

    let def_lhs =
      `I
        ( sprintf "$(b,(def-lhs VAR))",
          sprintf "Is satisfied when a term defines $(b,VAR)" )

    let def_uses =
      `I
        ( sprintf "$(b,(def-uses VAR))",
    ...
    let section =
      [
        ...
        attr term "is-ATTR" "Is satisfied when a term has the given attribute";
        ...
        `I
          ( "$(b,(has-mark))",
            "Is satisfied when a term has an\n      attribute $(b,mark)." );
        color "color";
        color "foreground";
        color "background";
        term_attr "addr";
        ...
        `I
          ( "$(b,(taints))",
            "Is satisfied if a term is taint source, i.e., has\n\
            \      $(b,tainted-reg) or $(b,tainted-ptr) attributes." );
        `I
          ( "$(b,(taints-reg))",
            "Is satisfied if a term is taint source,\n\
            \      that taints a value stored in a register, i.e., has a\n\
            \      $(b,tainted-reg) attribute." );
        `I
          ( "$(b,(taints-ptr))",
            "Is satisfied if a term is taint source,\n\
            \      that taints a value pointed by a value stored in a \
             register, i.e., has a\n\
            \      $(b,tainted-ptr) attribute." );
        `I
          ( "$(b,(has-taints))",
            "Is satisfied if a term is tainted, i.e., has\n\
            \      $(b,tainted-reg) or $(b,tainted-ptr) attributes." );
        `I
          ( "$(b,(has-tainted-reg))",
            "Is satisfied if a term uses a\n\
            \      tainted value stored in a register, i.e., has a\n\
            \      $(b,tainted-regs) attribute." );
        `I
          ( "$(b,(has-tainted-reg taint))",
            "Is satisfied if a term uses a\n\
            \      value tainted with $(i,taint) and stored in a register, \
             where $(i,taint)\n\
            \      must be a valid taint identifier, e.g., $(b,%12)." );
        `I
          ( "$(b,(has-tainted-ptr))",
            "Is satisfied if a term loads a\n\
            \      value from a tainted address, i.e., has a $(b,tainted-regs) \
             attribute." );
        `I
          ( "$(b,(has-tainted-reg taint))",
            "Is satisfied if a term\n\
            \      loads a value from an address tainted by the give\n\
            \      $(i,taint). The $(i,taint) must be a valid taint \
             identifier, e.g., $(b,%42)." );
      ]
  end
  ...
  module Mappers = struct
    let color attr =
      `I
        ( sprintf "$(b,(%s COLOR))" attr,
          sprintf
            "Set term's attribute $(b,%s) to the given value,\n\
            \          where $(i,COLOR) must be one of %s"
            attr (enum colors) )
    ...
    let section =
      [
        ...
        `I
          ( "$(b,(taint-reg TID))",
            "Mark a term with the given $(b,TID)\n\
  ...
  let grammar = [ `S "LANGUAGE GRAMMAR"; `Pre grammar ]
  ...
  let example =
    [
      ...
      `P
        {|$(b,bap) exe --$(mname)$(b,-with)='((taints-ptr %12) (comment "ha ha"))'|};
      `P
        "$(b,bap) exe --$(mname)$(b,-with)='((term-name @strlen) (foreground \
         blue))'";
      `P
        "$(b,bap) exe --$(mname)$(b,-with)='((term-tid %0000042) (foreground \
         blue))'";
      `P
        "$(b,bap) exe --$(mname)$(b,-with)='((goto mem[0x42]) (foreground \
         blue))'";
    ]
  ...
  let man =
    [
      `S "SYNOPSIS";
      `P
        "$(b,bap) [$(b,--)$(mname)$(b,-with)=$(i,SCHEME)]\n\
        \                 [$(b,--)$(mname)$(b,-using)=$(i,FILE)] \
         $(b,--)$(mname)";
      `S "DESCRIPTION";
      `P
        "Transform terms using a domain specific pattern matching language.\n\
        ...
        \    represented by a pair $(b,(<condition> <action>)). The \
         $(b,<action>) specifies\n\
    ...
    @ Predicates.section @ Mappers.section @ grammar @ example @ see_also
  ...
  let () =
    ...
    Config.declare_extension
      ...
      (fun { Config.get = ( ! ) } ->
        try Project.register_pass (main !scheme !file)
        with Parse_error msg -> eprintf "Parsing error: %s\n%!" msg)
end
