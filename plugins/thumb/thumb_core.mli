...
module Make (CT : Theory.Core) : sig
  ...
  val it_set :
    'a var -> 'a pure -> ('a var -> data eff list) -> cond -> unit eff
  (** [it_set v x k] assigns [x] to [v] in the scope of IT block.

      Creates a function [cond -> unit eff] that performs two different
      operations depending on whether its applied inside of an IT block or not.
      An instruction is assumed to be in the IT block if the passed condition is
      not [AL].

      If inside an IT block, the assignment is turned into a conditional,
      expression, [v := c ? x : v].

      If outside of an IT block then a fresh variable [t] is created and bound
      to [x], it is then passed to [t] and finally, [v] is set to [t], i.e., the
      resulting compuation is [t := x; k t; v := t]. *)
  ...
  module Syntax : sig
    ...
    (** [v %=? x] conditional assignment.
        ...
        Creates a function that turns [x] into a conditional expression. If
        [cond] is [`AL], then the assignment is unconditional. *)

    val ( <--? ) : r32 bitv -> 'a bitv -> cond -> unit eff
    (** [p <--? x] conditional store.

        if [cond] is not [`AL] turns the store into a conditional expression,
        [mem := cnd ? mem[p] <- x : mem] *)
