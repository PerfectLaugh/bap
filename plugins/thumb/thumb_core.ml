...
module Make (CT : Theory.Core) = struct
  ...
  let goto dst = Theory.Label.for_addr dst >>= fun dst -> ctrl (CT.goto dst)
  ...
  module Syntax = struct
    ...
    let ( <--? ) dst src = function
      ...
      | cnd ->
          data
            [
              mem := CT.ite ~?cnd (CT.storew CT.b0 (var mem) dst src) (var mem);
            ]
  end
  ...
  let it_set dst exp rest cnd =
    ...
    | cnd -> data @@ [ dst := CT.ite ~?cnd exp (var dst) ]
