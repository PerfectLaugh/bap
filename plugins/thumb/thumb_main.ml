...
let string_of_operands ops =
  Array.map ops ~f:Op.to_string |> String.concat_array ~sep:", "

let decode_opcode op =
  try Some (opcode_of_sexp (Sexp.Atom op)) with _exn -> None

let pp_insn ppf (op, ops) =
  Format.fprintf ppf "%a(%s)" Sexp.pp (sexp_of_opcode op)
...
module Thumb (CT : Theory.Core) = struct
  ...
  let cnd x =
    match Option.value_exn (Imm.to_int x) with
    | 0 -> `EQ
    | 1 -> `NE
    | 2 -> `CS
    | 3 -> `CC
  ...
  let lift_move addr opcode insn =
    ...
    | `tADDspi, [| _; _; Imm x; Imm c; _ |] -> addspi (imm x * 4) (cnd c)
    ...
    | `tSUBspi, [| _; _; Imm x; Imm c; _ |] -> subspi (imm x * 4) (cnd c)
    | `tMOVi8, [| Reg rd; _; Imm x; Imm c; _ |] ->
        movi8 (reg rd) (imm x) (cnd c)
    | `tMOVSr, [| Reg rd; Reg rn |] -> movsr (reg rd) (reg rn)
    ...
    | `tCMPi8, [| Reg rn; Imm x; _; _ |] -> cmpi8 (reg rn) (imm x)
    | `tCMPr, [| Reg rn; Reg rm; _; _ |] -> cmpr (reg rn) (reg rm)
  ...
  let lift_mem addr opcode insn =
    ...
    | (#opmem_multi as op), ops -> (
        match (op, Array.to_list ops) with
        | `tSTMIA_UPD, Reg rd :: Imm c :: _ :: ar
        | `tSTMIA_UPD, Reg rd :: Reg _ :: Imm c :: _ :: ar ->
            stm (reg rd) (regs ar) (cnd c)
        | `tLDMIA, Reg rd :: Imm c :: _ :: ar -> ldm (reg rd) (regs ar) (cnd c)
        | `tPUSH, Imm c :: _ :: ar -> push (regs ar) (cnd c)
        | `tPOP, Imm c :: _ :: ar ->
            let regs = regs ar in
            if has_pc regs then popret (remove_pc regs) (cnd c)
            else pop regs (cnd c)
        | op, _ ->
  ...
  let unpredictable = Theory.Label.for_name "arm:unpredictable" >>= CT.goto
  ...
  let lift_insn addr opcode insn =
    match opcode with
...
module Main = struct
  ...
  let load () =
    ...
    | Some opcode -> (
        ...
        with uncaught ->
          warning
            "failed to decode a thumb instruction: uncaught exception %s\n\
             Backtrace:\n\
            \ %s\n"
...
let () =
  Bap_main.Extension.declare ~doc:"provides semantics for Thumb instructions"
