...
let cleanup ~keep r = if not keep then Recipe.close r
...
let minimize str =
  ...
  if len < 60 then str else String.subo ~len:60 str ^ "..."

let summary str =
  minimize @@ String.uncapitalize
  @@
...
let print_all_recipes ~keep () =
  ...
  List.iter recipe_paths ~f:(fun dir ->
      if Sys.file_exists dir && Sys.is_directory dir then
        Array.iter (Sys.readdir dir) ~f:(fun entry ->
            let file = dir / entry in
            if Filename.check_suffix file ".recipe" then
...
let () =
  ...
  | Recipe_error err -> Some (Stdlib.Format.asprintf "%a" Recipe.pp_error err)
  | _ -> None

let () =
  Extension.declare ~doc:"provides the recipe command"
    ~provides:[ "command"; "recipe" ] (fun _ -> Ok ())
