...
type assn = { use : tid; var : var; res : bool }
type id = Primus.Machine.id
type state = { conflicts : assn list; visited : Tid.Set.t }
...
let inspect_conflict assn = Sexp.List (List.map assn ~f:inspect_assn)
let inspect_conflicts cs = Sexp.List (List.map ~f:inspect_conflict cs)
...
let init_visited prog =
  (object
     ...
     method! enter_blk blk visited =
       if Term.has_attr blk Term.visited then Set.add visited (Term.tid blk)
       else visited
  end)
    #run
    prog Tid.Set.empty
...
let state =
  Primus.Machine.State.declare ~name:"conflicts"
    ~uuid:"58bb35f4-f259-4712-8d15-bdde1be3caa8"
  @@ fun proj ->
  { conflicts = []; visited = init_visited (Project.program proj) }
...
module Forker (Machine : Primus.Machine.S) = struct
  ...
  let assume assns =
    Machine.List.iter assns ~f:(fun assn ->
        ...
        Eval.binop op r r' >>= Eval.set assn.var)

  let unsat_assumptions blk =
    Machine.List.map (assumptions blk)
      ~f:
        (Machine.List.filter ~f:(fun { var; res } ->
  ...
  let do_fork blk ~child =
    ...
    if Set.mem t.visited (Term.tid blk) then Machine.return pid
    else
      Machine.fork () >>= fun () ->
  ...
  let assume_returns blk call =
    ...
    | Some (Direct dst) ->
        ...
        if Id.(id = pid) then Machine.return ()
          ...
          if Set.mem visited dst then Eval.halt >>= never_returns
          else Linker.exec (`tid dst) >>= fun () -> Eval.halt >>= never_returns
    | _ -> Machine.return ()

  let fork_on_calls blk jmp =
    match Jmp.kind jmp with
    | Call c -> assume_returns blk c
    | _ -> Machine.return ()

  let is_last blk def =
    match Term.last def_t blk with
    | None -> true
...
let desc =
  "When this mode is enabled the Primus Machine will venture into paths with \
...
let () =
  declare_extension ~doc:"enables the promiscuous mode of execution"
    ~provides:[ "primus"; "fuzzer" ] (fun { get = ( !! ) } ->
      Primus.Components.register_generic "promiscuous-path-explorer"
        (module Forker)
        ~package
        ~desc:
          "Forces execution of all linearly independent paths by forcefully \
           flipping the branch conditions.";
      Primus.Components.register_generic "division-by-zero-handler"
        (module EnableDivisionByZero)
        ~package ~desc:"Disables division by zero errors.";
