...
type strain = Addr of int64 | Tid of string | Var of string
[@@deriving variants]
...
module Strain = struct
  ...
  let word s =
    try Ok (Int64.of_string s) with exn -> expect "0x<hex-digits>" ~got:s
  ...
  let atom = function
    ...
    | s -> expect "<tid> | <var> | <addr>" ~got:(Sexp.to_string s)

  let sexp = function List x -> List.map ~f:atom x | x -> [ atom x ]
  let start s = Result.all (sexp s)
  let parse s = try start (Sexp.of_string s) with exn -> expect grammar ~got:s
  let to_string = function Addr a -> sprintf "%0Lx" a | Tid s | Var s -> s
  let parser s = match parse s with Ok r -> `Ok r | Error e -> `Error e

  let pp ppf ss =
    List.map ~f:to_string ss |> String.concat ~sep:" "
...
module Marker = struct
  let sats strains def =
    ...
    List.for_all strains ~f:(function
      ...
      | Addr a -> (
          ...
          | None -> false
          | Some addr -> (
              match Addr.to_int64 addr with
              | Error _ -> false
              | Ok x -> Int64.equal a x)))

  let seed t =
    match Term.get_attr t Term.origin with None -> Term.tid t | Some t -> t
  ...
  class main ~regs ~ptrs =
    object (self)
      inherit Term.mapper as super

      method! map_def def =
        super#map_def def |> mark Taint.reg regs |> mark Taint.ptr ptrs
    end
...
let main regs ptrs =
  match (regs, ptrs) with
  | [], [] -> ()
  | regs, ptrs ->
      Project.register_pass ~deps:[ "callsites" ] ~autorun:true (mark regs ptrs)
...
address. You can specify these options several times.|};
      `P
        "The full grammar specification of strain specification\n\
         language follows:";
      `Pre grammar;
      `S "SEE ALSO";
  ...
  let taints kind : strain list list Config.param =
    let doc =
      sprintf
        "Taint %s value of definition matching\n     with the specification"
        kind
    in
  ...
  let () =
    ...
    Config.manpage man;
    Config.declare_extension ~doc:"the legacy taint propagation engine"
