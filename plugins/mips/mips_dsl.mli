...
(** [extract e hi lo] extracts portion of [e] starting at bit [lo] and ending at
    bit [hi], all bounds are inclusive. Bits indexes start from the least
    significant bit. *)
...
(** [first e n] - extracts first [n] bits from [e], starting from the least
    significant bit *)

val last : exp -> int -> exp
(** [last e n] - extracts last [n] bits from [e], where the last bit is the most
    significant bit *)
...
(** [nth width e n] - extracts a portion of [e] of width [width] at index [n],
    ...
    based and started from least significant portion. E.g. [nth halfword e 1]
