...
let define t =
  ...
  let ipass =
    sequence
      [
        (always, choose [ Arg.split_with_memory iargs; Arg.memory ]);
  ...
  let return ~alignment:_ _ =
    select
      [
        (C.Type.is_floating, choose [ Arg.registers frets; Arg.reference iargs ]);
        (otherwise, choose [ Arg.register irets; Arg.reference iargs ]);
      ]
  in
  declare ~return @@ fun ~alignment:_ size ->
  select [ (C.Type.is_floating, fpass size); (otherwise, ipass) ]

let setup () =
  Theory.Target.filter ~parent:Bap_mips_target.parent () ~abi:Theory.Abi.gnu
  |> List.iter ~f:define
