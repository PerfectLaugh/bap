...
let reglist_to_map regs =
  List.fold ~init:String.Map.empty
    ~f:(fun regs var -> Map.set regs ~key:(Var.name var) ~data:var)
    regs
...
let of_vars vars = String.Map.map vars ~f:(fun v -> Exp.of_var v)
let of_vars_i vars = Int.Map.map vars ~f:(fun v -> Exp.of_var v)
...
module Make_MIPS (S : Spec) : MIPS = struct
  ...
  let gpr =
    List.fold gprs ~init:String.Map.empty ~f:(fun init (reg, alias) ->
        ...
        let names =
          if gpr_bitwidth = 64 then (name ^ "_64") :: names else names
        in
        List.fold names ~init ~f:(fun regs name ->
            Map.set regs ~key:name ~data:reg))

  let gpri =
    List.foldi ~init:Int.Map.empty
      ~f:(fun n regs (reg, _) -> Map.set regs ~key:n ~data:reg)
      gprs
...
module Make_cpu (M : MIPS) : CPU = struct
  ...
  let sp =
    Set.find_exn gpr ~f:(fun v -> String.is_prefix ~prefix:"SP" (Var.name v))

  let fp =
    Set.find_exn gpr ~f:(fun v -> String.is_prefix ~prefix:"FP" (Var.name v))
