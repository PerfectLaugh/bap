...
module Std = struct
  ...
  let register ?delay name lifter =
    Option.iter delay ~f:(fun d ->
        Hashtbl.add_exn delayed_opcodes ~key:name ~data:d);
  ...
  let lift addr_size endian mem insn =
    ...
    let lift lifter =
      try lifter cpu (Insn.ops insn) |> bil_of_rtl |> Result.return with
...
let promise_delay_slots () =
  KB.Rule.(
    declare ~package:"mips" "delay-slot"
    |> require Insn.Slot.name |> provide Insn.Slot.delay
  ...
  Hashtbl.find_and_call Std.delayed_opcodes name
    ~if_found:(fun delay -> KB.Value.put Insn.Slot.delay insn (Some delay))
