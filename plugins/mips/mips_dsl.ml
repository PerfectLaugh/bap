...
let int_of_imm = function
  | Op.Reg _ | Op.Fmm _ -> mips_fail "imm operand expected"
  | Op.Imm x -> (
      match Imm.to_int x with
      | Some x -> x
      | None -> mips_fail "failed to convert imm operand to int")
...
let var signed width = Exp.tmp (int_of_bitwidth width) |> apply_signess signed

let reg find signed op =
  match op with
  | Op.Imm _ | Op.Fmm _ -> mips_fail "reg operand expected"
...
type clause = [ `Case of exp * rtl list | `Default of rtl list ]
...
let switch exp cases =
  let default =
    List.filter_map ~f:(function `Default y -> Some y | _ -> None) cases
  in
  let default = Option.value ~default:[] (List.hd default) in
  let cases =
    List.filter_map
      ~f:(function `Case (x, y) -> Some (x, y) | _ -> None)
      cases
  in
  let cond x = Infix.(exp = x) in
  match cases with
  | [] -> mips_fail "empty switch"
