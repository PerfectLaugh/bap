...
let duration x y = Int.of_float_unchecked Float.(y - x)
...
let task_second { created } = duration created @@ Unix.gettimeofday ()
...
let render_times line { created; updated; active } =
  ...
  render_duration line
  @@ if active then time_elapsed created else Float.(updated - created)

let render_ping { created; updated; active } =
  if active then
    pp "( %c )"
      ping_frames.[duration created updated mod String.length ping_frames]
  else pp "done"
...
let render_stage_only s = pp "( %d )" (s + 1)
...
let render_level line level _ = pp "%a" Ansi.pp_pos (level + 12, line + 1)
let render_name level path _ = pp "%s: " (List.nth_exn path level)
let render_endline _ = pp "%t\n" Ansi.pp_reset

let render_slot line level path slot =
  List.iter
    ~f:(fun render -> render slot)
    [
      render_times line;
...
let render_progress tick =
  if not rendering.contents then (
    rendering := true;
...
let update_slots = function
  | Event.Log.Progress { task; note; stage; total } ->
      ...
      Hashtbl.update tasks task ~f:(function
        | None ->
            { note; stage; total; created = now; updated = now; active = true }
        | Some slot ->
            {
              slot with
              stage;
      ...
      Hashtbl.mapi_inplace tasks ~f:(fun ~key:name ~data:task' ->
          if String.is_prefix ~prefix:name task && String.(task <> name) then
            { task' with updated = now; active = true }
          else task');
      render_progress ()
  | _ -> ()
...
let sample ~interval stream =
  Stream.parse stream ~init:0. ~f:(fun last_updated _ ->
      let now = Unix.gettimeofday () in
      if duration last_updated now >= interval then (Some (), now)
...
let enable () =
  ...
  let ticks =
    sample ~interval @@ Stream.merge majors Event.stream ~f:(fun () _ -> ())
  in
...
let () =
  Config.declare_extension ~doc:"reports program status using progress bars"
    ~provides:[ "report"; "visualization" ] (fun { Config.get = ( !! ) } ->
