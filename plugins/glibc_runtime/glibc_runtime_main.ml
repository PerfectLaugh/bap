...
let enable =
  Extension.Configuration.flag "enable"
    ~doc:"Override the glib detection heuristic and enable the runtime fixup."

let references_glib_start_main doc =
  let open Ogre.Syntax in
  let query =
    Ogre.request Image.Scheme.external_reference ~that:(fun (_, name) ->
        Poly.(name = "__libc_start_main"))
    >>| Option.is_some
  in
  match Ogre.eval query doc with
  | Ok r -> r
  | Error err ->
      warning "unable to determine the presence of glibc runtime: %a" Error.pp
        err;
      false
...
let find_by_name prog name =
  ...
  |> Seq.find ~f:(fun sub -> String.equal (Sub.name sub) name)
...
let find_libc_start_main prog =
  ...
  | Call call -> (
      ...
      | Indirect _ -> insert_call prog start entry jmp)
...
let detect_main_address prog =
  ...
  | Bil.Var reg ->
      Seq.find defs ~f:(fun def -> Var.same (Def.lhs def) reg)
      >>| Def.rhs >>= proj_int
  | Bil.Load (_, addr, _, _) ->
      Seq.find_map defs ~f:(fun def ->
...
let rename_main abi prog =
  if is_sub_absent prog "main" then (
    match detect_main_address prog with
    | None -> prog
    | Some addr ->
...
let rename_libc_start_main prog =
  if is_sub_absent prog "__libc_start_main" then
    match find_libc_start_main prog with
    | None -> prog
    | Some (tid, prog) -> (
        Term.change sub_t prog tid @@ function
        | None -> None
        | Some sub ->
            Option.some @@ reinsert_args_for_new_name sub "__libc_start_main")
  else prog

module Main = struct
  let proto : C.Type.proto =
    match
      C.Type.function_ ~return:(C.Type.basic `sint)
        [
          ("argc", C.Type.basic `sint);
          ("argv", C.Type.(pointer (pointer (basic `char))));
        ]
    with
    | `Function { t = proto } -> proto
    | _ -> assert false
  ...
  let abi proj main =
    ...
    | None -> main
    | Some proc -> C.Abi.apply proc (C.Abi.model t) [] proto main
end

let fix_main proj = Project.map_program proj ~f:(rename_main (Main.abi proj))

let is_enabled ctxt proj =
  Extension.Configuration.get ctxt enable || is_glibc proj

let recover_main ctxt proj =
  if is_enabled ctxt proj then fix_main proj else proj

let discover_libc_start_main ctxt proj =
  if is_enabled ctxt proj then
    Project.map_program proj ~f:rename_libc_start_main
  else proj
...
let refine_glibc_internal_aliases () =
  KB.Rule.(
    ...
    |> comment
         "computes aliases for names starting with the __GI_ (Glibc Internal) \
          prefix");
  ...
  let init =
    ...
    | None | Some _ -> String.Set.empty
  in
  Set.fold aliases ~init ~f:(fun acc s ->
      if is_glibc_internal s then Set.add acc @@ drop_glibc_internal s else acc)
  |> Set.filter ~f:(Fn.non String.is_empty)
  |> Set.union aliases

let () =
  Extension.declare ~doc ~provides:[ "abi"; "api" ] @@ fun ctxt ->
