...
let perm_of_flags = function
  ...
  | xs -> (
      let perm =
        List.fold xs ~init:None ~f:(fun perm flag ->
            Option.merge perm (perm_of_flag flag) ~f:(fun p1 p2 -> Or (p1, p2)))
      in
      match perm with
...
let section_data data s : string Or_error.t =
  ...
  try
    ...
    Bigstring.To_bytes.blit ~src:data ~src_pos:offset ~dst ~dst_pos:0 ~len:size;
...
let create_symtab data endian elf =
  ...
  let create name s =
    ...
    | Ok data -> Some (name, Buffer.create data)
  in
  let sections =
    Seq.filter_map elf.e_sections ~f:(fun s ->
        let name = Elf.section_name data elf s in
        match name with
  ...
  let seq =
    Sequence.mapi (Dwarf.Fbi.functions dff) ~f:(fun i (name, fn) ->
        let pc_lo = Dwarf.Fn.pc_lo fn in
        let size =
          match Dwarf.Fn.pc_hi fn with
          | None -> return None
          | Some pc_hi ->
        ...
        size >>= fun size ->
        let location =
          Location.{ addr = pc_lo; len = Option.value size ~default:1 }
        in
        return
          {
            Symbol.name;
...
(** @return
      [None] - if segment should be skipped as non interesting, [Some error] -
      if an error has occurred when we have tried to load segment,
...
let create_segment make_addr i es : Segment.t Or_error.t option =
  if not ([%compare.equal: p_type] es.p_type PT_LOAD) then None
    ...
    let segment =
      ...
      return { Segment.name; Segment.perm; Segment.off; Segment.location }
    in
    match segment with
    | Error _ as err -> Some (tag_arg err "skipped segment" i sexp_of_int)
    | ok -> Some ok
...
let img_of_elf data elf : Img.t Or_error.t =
  let endian =
    match elf.e_data with
    | ELFDATA2LSB -> LittleEndian
    | ELFDATA2MSB -> BigEndian
  in
  let addr_size =
    match elf.e_class with ELFCLASS32 -> `r32 | ELFCLASS64 -> `r64
  in
  let addr = addr_maker addr_size in
  let entry = addr elf.e_entry in
  let arch =
    match (elf.e_machine, endian, addr_size) with
    | EM_386, _, _ -> Ok `x86
    ...
    | _ -> errorf "can't load file, unsupported platform"
  in
  let segments, errors =
    Seq.filter_mapi elf.e_segments ~f:(create_segment addr)
    |> Seq.to_list
    |> List.partition_map ~f:(function Ok s -> First s | Error e -> Second e)
  in
  let symbols, _ =
    match create_symtab data endian elf with
    | Ok syms -> (syms, errors)
    | Error err -> ([], Error.tag err ~tag:"failed to read symbols" :: errors)
  in
  arch >>= fun arch ->
  let sections =
    Seq.filter_map elf.e_sections ~f:(fun s ->
...
let of_data (data : Bigstring.t) : Img.t option =
  ...
  | Ok img -> Some img
  | Error err ->
      eprintf "Elf_backend: failed with exn: %s" @@ Error.to_string_hum err;
      None

let () =
  Config.declare_extension ~doc:"provides an alternative ELF image loader"
    ~provides:[ "elf"; "dwarf"; "loader" ]
  @@ fun _ ->
  match Image.register_backend ~name of_data with
  | `Ok -> ()
  | `Duplicate -> eprintf "Elf_backend: name «%s» is already used\n" name
