...
type problem = Unknown_loader of string | Loader_error of Error.t
...
let formats = [ ("sexp", `Sexp); ("yaml", `Yaml) ]

let format =
  Extension.Command.parameter
    (Extension.Type.enum formats)
    "format" ~aliases:[ "o" ] ~doc:"The output format"
...
let reader =
  Data.Read.create () ~of_bigstring:(Binable.of_bigstring (module Ogre.Doc))

let writer =
  Data.Write.create () ~to_bigstring:(Binable.to_bigstring (module Ogre.Doc))

let print fmt spec =
  let pp = match fmt with `Yaml -> Ogre.Doc.pp_yaml | `Sexp -> Ogre.Doc.pp in
  Format.printf "%a@\n%!" pp spec

let () =
  Extension.Command.(
    declare "specification"
      (args $ format $ input $ loader)
      ~doc ~requires:[ "loader" ])
  @@ fun format input loader ctxt ->
  let digest = Data.Cache.Digest.create ~namespace:"specification" in
  let digest =
    Data.Cache.Digest.add digest "%s%s%s" input loader
  ...
  | None -> (
      ...
      | Some (module Load) -> (
          match Load.from_file input with
          | Ok (Some spec) ->
...
let string_of_problem = function
  | Unknown_loader name ->
      sprintf "The loader `%s' is not registers, known loaders are: %s." name
        (Image.available_backends () |> String.concat ~sep:", ")
  | Loader_error err ->
...
let () =
  Extension.declare ~doc:"provides the specification command"
    ~provides:[ "command"; "specification"; "ogre" ] (fun _ -> Ok ())
