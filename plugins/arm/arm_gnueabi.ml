...
let data model =
  object (self)
    ...
    method! enum elts =
      if Int64.(C.Size.max_enum_elt elts < 1L lsl 32) then self#integer `uint
      else self#integer `ulong_long

    method! real = function `float -> `r32 | `double | `long_double -> `r64
  end
  ...
  let define t =
    ...
    sequence
      [
        ...
        (always, choose [ Arg.split_with_memory ~rev iargs; Arg.memory ]);
      ]
end

module Aapcs64 = struct
  let is_composite t = C.Type.(is_structure t || is_union t)
  ...
  let define t =
    ...
    let* x8 =
      Arg.Arena.create [ Option.value_exn (Theory.Target.var t "X8") ]
    in

    let return ~alignment size =
      select
        [
          ( C.Type.is_floating,
            choose [ Arg.registers ~rev frets; Arg.reference x8 ] );
          ( otherwise,
            sequence
              [
                ...
                ( always,
                  select
                    [
                      ...
                      ( always,
                        choose [ Arg.registers ~rev irets; Arg.reference x8 ] );
                    ] );
              ] );
        ]
    in
    describe ~return @@ fun ~alignment size ->
    select
      [
        ( C.Type.is_floating,
          choose [ Arg.registers ~rev fargs; Arg.reference iargs ] );
        ( otherwise,
          sequence
            [
                    ...
                    ( otherwise,
                      choose
                        [
                          ...
                          combine [ const (Arg.deplet iargs); Arg.memory ];
                        ] );
                  ] );
            ] );
      ]
...
let is_our_abi abi =
  List.exists ~f:(Theory.Abi.equal abi)
    Theory.Abi.[ unknown; gnu; eabi; gnueabi ]
