...
module Std = struct
  ...
  let rename_dot_to_rec old = String.(chop_suffix_exn old ~suffix:"o" ^ "_rec")

  let register_dot name lifter =
    List.iter
      ~f:(fun name -> register name (dot lifter))
      [ name; rename_dot_to_rec name ]
  ...
  let lift addr_size endian mem insn =
    ...
    let lift lifter =
      try lifter cpu (Insn.ops insn) |> bil_of_rtl |> Result.return
      with Failure str -> Error (Error.of_string str)
    in
