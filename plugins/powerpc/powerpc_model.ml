...
let make_regs_i typ prefix range =
  List.fold ~init:Int.Map.empty
    ~f:(fun regs i -> Map.set regs ~key:i ~data:(make_var_i typ prefix i))
    range
...
module Vars (B : Bitwidth) = struct
  ...
  let float_less = flag "FL"

  (** Greater Than or Positive *)
  let float_equal = flag "FE"

  (** Floating-Point Equal or Zero *)
  let float_greater = flag "FG"

  (** Floating-Point Unordered or NaN *)
  let float_unordered = flag "FU"
  ...
  (** carry of low-order 32 bit result *)
  let ca32 = flag "CA32"

  (** overflow of low-order 32 bit result *)
  let ov32 = flag "OV32"
end

let of_vars vars = Map.map vars ~f:(fun v -> Exp.of_var v)
let of_vars_i vars = Map.map vars ~f:(fun v -> Exp.of_var v)
...
module Exps (B : Bitwidth) = struct
  ...
  let crn =
    Map.fold Vars.cri ~init:String.Map.empty ~f:(fun ~key:_ ~data:var acc ->
  ...
  let cr_fields =
    Map.map cr_fields ~f:(fun (b3, b2, b1, b0) ->
        Exp.of_vars [ b0; b1; b2; b3 ])

  let cri_fields =
    Map.map cri_fields ~f:(fun (b3, b2, b1, b0) ->
        Exp.of_vars [ b0; b1; b2; b3 ])
end
...
module Make_cpu (P : PowerPC) : CPU = struct
  ...
  let gpr =
    ...
    List.fold data ~init:Var.Set.empty ~f:(fun regs v -> Set.add regs v)
  ...
  let flags = Var.Set.of_list [ so; ca; ov; cf; nf; zf; ca32; ov32 ]
  let is = Var.same
