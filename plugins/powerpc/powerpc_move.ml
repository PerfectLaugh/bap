...
(** Move To Special Purpose Cpu.Register Page 116 of IBM Power ISATM Version 3.0
    B example: 7c 20 4b a6 mtspr 9, r1 *)
let mtspr cpu ops =
...
(** Move From Special Purpose Cpu.Register Page 116 of IBM Power ISATM Version
    3.0 B example: 7c 20 42 a6 mfspr r1,8 *)
let mfspr cpu ops =
...
(** Move To CR Fields Page 121 of IBM Power ISATM Version 3.0 B example: 7c 20
    81 20 mtcrf 8, r1 *)
let mtcrf cpu ops =
...
(** Move From CR Page 122 of IBM Power ISATM Version 3.0 B example: 7c 20 00 26
    mfcr r1 *)
let mfcr cpu ops =
  let rt = unsigned cpu.reg ops.(0) in
  RTL.[ rt := cpu.cr ]

(** Move To CTR mtspr extended mnemonic. example: 7c 09 03 a6 mtctr r0 *)
let mtctr cpu ops =
  let rs = unsigned cpu.reg ops.(0) in
  RTL.[ cpu.ctr := rs ]

(** Move To LR mtspr extended mnemonic. example: 7c 08 03 a6 mtlr r0 *)
let mtlr cpu ops =
  let rs = unsigned cpu.reg ops.(0) in
  RTL.[ cpu.lr := rs ]

(** Move From CTR mfspr extended mnemonic. example: 7c 09 02 a6 mfctr r0 *)
let mfctr cpu ops =
  let rt = unsigned cpu.reg ops.(0) in
  RTL.[ rt := cpu.ctr ]

(** Move From LR mfspr extended mnemonic. example: 7c 08 02 a6 mflr r0 *)
let mflr cpu ops =
  let rt = unsigned cpu.reg ops.(0) in
  RTL.[ rt := cpu.lr ]
