...
let var_bitwidth v =
  ...
  | Type.Imm w -> w
  | _ -> ppc_fail "variable %s doesn't has notion of bitwidth" (Var.name v)

let width_of_vars vs = List.fold ~init:0 ~f:(fun x v -> x + var_bitwidth v) vs

let var_of_exp e =
  match e.body with Vars (v, _) -> v | _ -> ppc_fail "variable expected"
...
module Exp = struct
  ...
  let derive_op x y op_u op_s =
    match derive_sign x.sign y.sign with Signed -> op_s | Unsigned -> op_u
  ...
  let rshift x y =
    let op = if equal_sign x.sign Signed then Bil.arshift else Bil.rshift in
    binop_with_cast op x y
...
module Translate = struct
  ...
  let partial_assign v (hi_var, lo_var) rhs (hi_exp, lo_exp) =
    ...
    let rhs =
      ...
      else Exp.extract hi_exp lo_exp rhs
    in
    ...
    let left =
      ...
      else Some (Bil.extract ~hi:(width - 1) ~lo:(hi_var + 1) var)
    in
    let middle = bil_exp (Exp.body rhs) in
    let right =
      if width_right = 0 then None
      else Some (Bil.extract ~hi:(lo_var - 1) ~lo:0 var)
    in
    ...
    let rec assign es assigned vars_lo = function
      ...
      | v :: vars ->
          ...
          if var_in_bounds then
            ...
            let es =
              partial_assign v (hi_var, lo_var) rhs (hi_exp, lo_exp) @ es
            in
            assign es (assigned + bits_to_assign) (vars_lo + width) vars
          else assign es assigned (vars_lo + width) vars
    in
    assign [] 0 0 vars

  (** valid forms of assignment: 1) var := exp 2) var1 ^ var2 ... varN := exp 3)
      [var1;var2; ...] := exp - equivalent to 2) 4) extract hi lo var := exp -
      change only certain bits of var 5) extract hi lo (var1 ^ var2 ... varN) :=
      exp *)
  let move lhs rhs =
