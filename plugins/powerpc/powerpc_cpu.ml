...
let find m reg =
  ...
  let reg_searches =
    [ find_gpr; find_cr_bit; find_cr_field; find_fpr; find_vr ]
  in
...
let make_cpu addr_size endian memory =
  ...
  let word_width = match addr_size with `r32 -> word | `r64 -> doubleword in
  {
    load;
    store;
    jmp;
