  ...
  let return ~alignment:_ _ =
    select
      [
        ...
        (otherwise, choose [ Arg.registers ~rev irets; Arg.reference iargs ]);
      ]
  in
  declare ~return @@ fun ~alignment:_ size ->
  select
    [
      (C.Type.is_floating, choose [ Arg.register fargs; Arg.memory ]);
      (is (size > 32), choose [ Arg.pointer iargs; Arg.memory ]);
      (otherwise, choose [ Arg.pointer iargs; Arg.memory ]);
    ]
