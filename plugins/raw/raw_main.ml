...
module Spec = struct
  ...
  let addr_t =
    Type.define ~parse:Bitvec.of_string ~print:Bitvec.to_string Bitvec.zero

  let arch =
    Configuration.parameter ~doc:"Specifies the ISA of raw bytes"
      Type.(string =? "x86-64")
      "arch"
  ...
  let bits =
    Configuration.parameter
      ~doc:
        "The number of bits in the machine word. If not specified then it will \
         be derived from the architecture or default to 32 if the architecture \
         is not known."
      Type.(int =? 32)
      "bits"

  let entry_points =
    Configuration.parameter ~doc:"Address (or addresses) of entry points"
      Type.(list addr_t)
      "entry-point"

  let base_address =
    Configuration.parameter ~doc:"The address of the first byte" addr_t "base"

  let offset =
    Configuration.parameter ~doc:"The offset of the first byte" Type.int64
      "offset"

  let length =
    Configuration.parameter ~doc:"The length of the code section"
      Type.(some int64)
      "length"
end
...
let register_loader ctxt =
  Image.register_loader ~name:"raw"
    (module struct
      let generate measure input =
        ...
        Buffer.contents buf |> Ogre.Doc.from_string |> Or_error.ok_exn
      ...
      let length_of_data str = Int64.of_int (Bigstring.length str)

      let from_file name =
        Or_error.try_with @@ fun () -> Some (generate length_of_file name)

      let from_data data =
        Or_error.try_with @@ fun () -> Some (generate length_of_data data)
    end)

let () =
  Extension.declare ~provides:[ "loader" ] ~doc @@ fun ctxt ->
