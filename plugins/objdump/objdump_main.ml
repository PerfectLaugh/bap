...
let demangler =
  Extension.Configuration.parameter
    ...
    "demangler"
    ~doc:
      "Specify the demangler name. Set to $(i,disabled) to disable demangling."
...
let parse_func_start input accept init =
  try
    ...
    let addr =
      Regexp.parse_addr input ~start:(Re.Group.start groups 1)
...
let agent =
  KB.Agent.register ~package:"bap" "objdump-symbolizer"
    ~desc:"extracts symbols objdump" ~reliability:KB.Agent.doubtful
...
module Repository : sig
  ...
  val create :
    (string -> (info -> Bitvec.t -> string -> info) -> info -> info) -> t

  val name :
    t -> ?size:int -> ?bias:Bitvec.t -> path:string -> Bitvec.t -> string option
  ...
  let create parse = { parse; files = Hashtbl.create (module String) }

  let string_of_addrs addrs =
    String.concat ~sep:", " @@ List.map addrs ~f:Bitvec.to_string

  let string_of_names names = String.concat ~sep:", " names
    ...
    let add_aliases names t addr =
      {
        t with
        aliases =
          Map.update t.aliases addr ~f:(function
  ...
  let lookup { parse; files } path =
    ...
    | None ->
        ...
        if Bap_relation.is_empty info then warning "failed to obtain symbols";
  ...
  let to_real size = function
    | None -> Fn.id
    | Some bias -> fun addr -> Bitvec.((addr - bias) mod modulus size)
...
let create_repo ctxt =
  ...
  Repository.create (fun file accept init ->
      with_objdump_output demangler ~file ~init ~f:(fun info line ->
          parse_func_start line accept info))

let provide_function_starts_and_names repo =
  let declare name input output =
    KB.Rule.(
      declare ~package:"bap" name
      |> dynamic [ "objdump" ] |> require input |> provide output
      |> comment @@ sprintf "extracts %s from objdump" name)
  in
  let property promise slot key_slot f =
    ...
    f (Repository.name repo ~size ?bias ~path key)
  in
  let is_known = function None -> None | Some _ -> Some true in
...
let provide_aliases repo =
  let open Theory.Label in
  KB.Rule.(
    declare ~package:"bap" "aliases"
    |> dynamic [ "objdump" ] |> require addr |> provide aliases
...
let () = Extension.declare main ~doc ~provides:[ "symbolizer"; "rooter" ]
