...
let () =
  Config.declare_extension ~doc:"provides PPC Primus support package"
    ~provides:[ "primus"; "powerpc"; "ppc" ]
  @@ fun _ ->
  let module Component (Machine : Primus.Machine.S) = struct
    ...
    let init32 vars = initialize_regs vars
  ...
  Primus.Components.register_generic "powerpc-init"
    (module Component)
    ~package:"bap" ~desc:"Initializes PPC registers."
