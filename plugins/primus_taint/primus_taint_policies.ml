...
struct
  ...
  let name = Value.Symbol.to_value Id.name >>= Policy.of_value
...
module Computation (Machine : Primus.Machine.S) = struct
  ...
  let stored (x, y) =
    ...
    Taint.detach x indirect ts >>= fun () -> (direct --> indirect) [ y ] x
...
let components : (string * Primus.Machine.component * string) list =
  [
    ( "propagate-taint-exact",
      (module Exact),
      "Enables the exact taint propagation policy. In this policy, a taint is \
       only propagated between values when one value is derived from another \
       using some transparent (not changing the value computation), e.g., \
       store, load, cast, concat, extract." );
    ( "propagate-taint-by-computation",
      (module Computation),
