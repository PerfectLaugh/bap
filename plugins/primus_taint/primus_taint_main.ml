...
module Pre (Machine : Primus.Machine.S) = struct
  ...
  let optional value f k x =
    f k x >>= function None -> nil | Some x -> value x
...
module IntroDirect (Machine : Primus.Machine.S) = struct
  ...
  let run [ k; v ] = Kind.of_value k >>= Tracker.new_direct v >>= taint
end
...
module IntroIndirect (Machine : Primus.Machine.S) = struct
  ...
  let run [ k; v; n ] =
    Kind.of_value k >>= Tracker.new_indirect ~addr:v ~len:n >>= taint
end
...
module Get (Machine : Primus.Machine.S) = struct
  ...
  let run rel [ k; v ] =
    ...
    Tracker.lookup v rel >>= optional taint first_of_kind k
end
...
module Sanitize (Machine : Primus.Machine.S) = struct
  ...
  let run rel [ k; v ] =
    Kind.of_value k >>= fun k ->
    Tracker.sanitize v rel k >>| fun () -> v
end
...
module TaintKind (Machine : Primus.Machine.S) = struct
  ...
  let run [ t ] = Object.of_value t >>= Object.kind >>= Kind.to_value
end
...
module PolicySetDefault (Machine : Primus.Machine.S) = struct
  ...
  let run [ p ] = Policy.of_value p >>= Policy.set_default >>= fun () -> nil
end
...
module Signals (Machine : Primus.Machine.S) = struct
  ...
  let init () =
    ...
    Machine.List.all [ Object.to_value t; Value.of_bool live ]
end
...
let policy_selector name : Primus.component =
  let module Init (Machine : Primus.Machine.S) = struct
    ...
    let init () =
      ...
      Policy.of_value v >>= Policy.set_default
  end in
  (module Init)
...
let register_policy_selector name =
  Primus.Components.register_generic
    ("select-" ^ name ^ "-policy")
    (policy_selector name) ~package:"bap"
    ~desc:
      (sprintf
         "Selects the %s as the default taint propagation policy. This policy \
          could be overwritten per each taint kind individually."
         name)
...
manpage
  [
    ...
    `P
      "It is possible to track the liveness of a taint, i.e., whether\n\
...
let collectors = [ ("none", false); ("conservative", true) ]

let enable_gc =
  param (enum collectors) ~default:false "gc"
    ~doc:"Picks a taint garbage collector"

let () =
  declare_extension ~doc:"manages the Primus Taint Analysis framework"
    ~provides:[ "dataflow"; "pass"; "taint"; "primus"; "primus-lisp" ]
  @@ fun { get = ( !! ) } ->
  ...
  Primus.Components.register_generic ~package:"bap" "taint-signals"
    (module Signals)
  ...
  if !!enable_gc then
    Primus.Machine.add_component (module Taint.Gc.Conservative) [@warning "-D"]
