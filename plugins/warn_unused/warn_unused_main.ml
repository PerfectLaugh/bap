...
let taint prog =
  (object (self)
     ...
     method! map_def def =
       ...
       | None -> def
       | Some tid -> (
           match Program.lookup jmp_t prog tid with
           | None -> def
           | Some jmp -> (
     ...
     method private process def taint call =
       match Call.target call with
       | Indirect _ -> def
       | Direct callee -> (
           match Term.find sub_t prog callee with
           | None -> def
           | Some sub ->
               if
                 Term.enum arg_t sub
...
let marker unchecked =
  object
    inherit Term.mapper as super

    method! map_term cls t =
      if Set.mem unchecked (Term.tid t) then Term.set_attr t Term.dead ()
      else super#map_term cls t
  end

let printer unchecked =
  object
    inherit [unit] Term.visitor

    method! enter_jmp jmp () =
      if Set.mem unchecked (Term.tid jmp) then printf "%a" Jmp.pp jmp
  end
...
let mark unchecked proj = Project.map_program proj ~f:(marker unchecked)#run
let taint proj = Project.map_program proj ~f:taint
...
module Cmdline = struct
  let man =
    [
      ...
      `S "DESCRIPTION";
      `P
        "If a subroutine has GNU attribute $(b,warn_unused_result) and\n\
      ...
      `I
        ( "$(b,--)$(mname)$(b,-taint)",
          ...
          \     dependency." );
      `I ("$(b,--)$(mname)$(b,-print)", "Print all calls that weren't checked.");
      `I
        ( "$(b,--)$(mname)$(b,-mark)",
          "Mark all unchecked calls with $(b,Term.dead) attribute" );
      `I
        ( "$(b,--)$(mname)",
          "Same as $(b,--)$(mname)$(b,-taint) $(b,--propagate-taint \
           --)$(mname)$(b,-print)" );
      `S "SEE ALSO";
      `P
        "$(b,bap-api)(1), $(b,bap-plugin-propagate-taint)(1), \
         $(b,bap-plugin-taint)(1)";
    ]
  ...
  let () =
    Config.manpage man;
    Config.declare_extension ~doc:"the legacy warn-unused analysis"
      ~provides:[ "pass"; "analysis" ]
    @@ fun { Config.get = ( ! ) } ->
    ...
    Project.register_pass' ignore
      ~deps:[ name ^ "-taint"; "propagate-taint"; name ^ "-print" ]
end
