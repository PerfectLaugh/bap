...
module Region = struct
  ...
  type t = { lower : point; upper : point }
  [@@deriving compare, fields, sexp_of]
end

module Regions = Interval_tree.Make (Region)

type t = { regions : unit Regions.t Primus.Value.Map.t }

let state =
  Primus.Machine.State.declare ~uuid:"0FC4AF7F-7237-4CE1-874A-4971801C6D67"
    ~name:"region" (fun _ -> { regions = Primus.Value.Map.empty })
...
module Pre (Machine : Primus.Machine.S) = struct
  ...
  let bool = function true -> nil | false -> Value.one 1_
...
module Create (Machine : Primus.Machine.S) = struct
  ...
  let run [ reg; lower; upper ] =
    Machine.Local.update state ~f:(fun { regions } ->
        {
          regions =
            Map.update regions reg ~f:(function
...
module Lower (Machine : Primus.Machine.S) = struct
  ...
  let run [ reg; addr ] =
    ...
    | None -> nil
    | Some map -> (
        match Seq.hd (Regions.lookup map addr) with
        | None -> nil
...
module Move (Machine : Primus.Machine.S) = struct
  ...
  let run [ dst; src; addr ] =
    ...
    | Some reg ->
        let reg' =
          match Map.find regions dst with
          | None -> Regions.empty
          | Some reg' -> reg'
        in
        let reg, reg', n =
        ...
        in
        Machine.Local.put state { regions } >>= fun () -> Value.of_bool (n > 0)
end
...
let () =
  ...
  Primus.Components.register_generic "lisp-regions"
    (module Main)
    ~package:"bap" ~desc
