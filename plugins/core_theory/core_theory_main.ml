...
let domain = KB.Domain.optional "cst" ~equal:Sexp.equal ~inspect:Fn.id

let eslot =
  KB.Class.property Theory.Semantics.cls "eff" ~package:"core" ~public:true
    domain

let pslot =
  KB.Class.property Theory.Value.cls "val" ~package:"core" ~public:true domain
...
module Herbrand : Theory.Core = struct
  ...
  let list = function
    | [ Sexp.Atom op; List (Atom opx :: xs); List (Atom opy :: ys) ]
      when String.(op = opx && op = opy) ->
        Sexp.List (Atom op :: List.append xs ys)
    | [ Sexp.Atom op; List (Atom opx :: xs); y ] when String.(op = opx) ->
        Sexp.List ((Sexp.Atom op :: xs) @ [ y ])
    | [ Sexp.Atom op; x; List (Atom opy :: ys) ] when String.(op = opy) ->
  ...
  let ( >>=>? ) x f =
    x >>= fun v ->
    let s = esort v in
    match KB.Value.get eslot v with None -> f s (list []) | Some x -> f s x

  let empty = Theory.Value.empty

  let unary_s s op x =
    x >>|> fun _ -> function
    | None -> empty s | Some v -> pure s @@ app op [ v ]
  ...
  module Minimal = struct
    ...
    let let_ v x y =
      ...
      match (x, y) with
      | Some x, Some y -> pure s @@ app "let" [ atom name; x; y ]
      | _ -> empty s
    ...
    let ite c x y =
      ...
      x >>->? fun s x ->
      y >>|>? fun _ y ->
      match c with None -> empty s | Some c -> pure s @@ app "ite" [ c; x; y ]
    ...
    let genshift name fill x off =
      ...
      | Some fill, Some x, Some off -> pure s @@ app name [ fill; x; off ]
      | _ -> empty s
    ...
    let cast s fill exp =
      ...
      match (fill, x) with
      | Some fill, Some x ->
          if Theory.Value.Sort.same s s' then pure s x
          else pure s @@ list [ atom "cast"; atom ct; fill; x ]
      | _ -> empty s

    let concat s xs =
      List.map xs ~f:(fun x -> x >>|> fun _ -> Fn.id)
      |> KB.List.all >>| Option.all
      >>| function
      | None -> empty s
    ...
    let store m p x =
      ...
      x >>|> fun _ x ->
      match (m, p, x) with
      | Some m, Some p, Some x -> pure s @@ app "store" [ m; p; x ]
      | _ -> empty s
    ...
    let set v x =
      ...
      match x with
      | None -> nil
      | Some x -> data @@ app "set" [ atom (Theory.Var.name v); x ]

    let jmp x =
      x >>|> fun _ x ->
      match x with None -> nil | Some x -> ctrl @@ app "goto" [ x ]

    let goto dst =
      KB.collect Theory.Label.addr dst >>= function
      | Some dst -> ret @@ ctrl @@ app "goto" [ atom (Bitvec.to_string dst) ]
      | None ->
    ...
    let both s xs ys =
      match (xs, ys) with
      | None, None -> ret nil
      | Some r, None | None, Some r -> ret @@ eff s r
      | Some xs, Some ys -> ret @@ eff s @@ list [ xs; ys ]

    let seq xs ys =
      xs >>=> fun s xs ->
      ys >>=> fun _ ys -> both s xs ys

    let blk _ xs ys =
      xs >>=> fun _ xs ->
      ys >>=> fun _ ys -> both Theory.Effect.Sort.top xs ys
    ...
    let repeat cnd body =
      ...
      match cnd with
      | None -> ret @@ nil
      | Some cnd -> ret @@ eff s @@ app "while" [ cnd; body ]
    ...
    let branch cnd yes nay =
      ...
      match cnd with
      | None -> ret @@ nil
      | Some cnd -> ret @@ eff s @@ app "if" [ cnd; yes; nay ]
  end
  ...
  let mk_cast name s x =
    ...
    | Some x ->
        if Theory.Value.Sort.same s s' then pure s x
        else pure s @@ app name [ atom @@ sprintf "%d" (Theory.Bitv.size s); x ]
  ...
  let extract s lo hi x =
    ...
    x >>|> fun _ x ->
    match (lo, hi, x) with
    | Some lo, Some hi, Some x -> pure s @@ app "extract" [ lo; hi; x ]
    | _ -> empty s
  ...
  let loadw s dir mem ptr =
    ...
    | Some dir, Some mem, Some ptr ->
        pure s
        @@ app "loadw"
             [ atom @@ sprintf "%d" (Theory.Bitv.size s); dir; mem; ptr ]
    | _ -> empty s
  ...
  let mk_shift name x m =
    x >>->? fun s x ->
    m >>|> fun _ -> function
    | None -> empty s | Some m -> pure s @@ app name [ x; m ]
  ...
  let asort s =
    atom
    @@ Format.asprintf "%a" Theory.Value.Sort.pp (Theory.Value.Sort.forget s)
  ...
  let format s =
    ...
    | Some s -> (
        List.find_map wellknown ~f:(fun (par, name) ->
            ...
            if Theory.Value.Sort.same s s' then Some (atom name) else None)
        |> function
        | Some s -> s
        | None -> asort s)
  ...
  let fbits x =
    ...
    match x with None -> empty s | Some x -> pure s @@ app "fbits" [ x ]
  ...
  let monoid_f name m x y =
    ...
    y >>->? fun _ y ->
    m >>|> fun _ m ->
    match m with None -> empty s | Some m -> pure s @@ app name [ m; x; y ]

  let unary_f name m x =
    x >>->? fun s x ->
    m >>|> fun _ m ->
    match m with None -> empty s | Some m -> pure s @@ app name [ m; x ]
  ...
  let ternary_f name m x y z =
    ...
    z >>->? fun _ z ->
    m >>|> fun _ m ->
    match m with None -> empty s | Some m -> pure s @@ app name [ m; x; y; z ]
  ...
  let fconvert s m x =
    ...
    x >>|> fun _ x ->
    match (m, x) with
    | Some m, Some x -> pure s @@ app "fconvert" [ format s; m; x ]
    | _ -> empty s
...
let herbrand_enabled =
  Extension.Configuration.parameter
    Extension.Type.(bool =? false)
    "syntax" ~as_flag:true ~aliases:[ "herbrand"; "debug" ]

let enable_herbrand () =
  Theory.declare ~provides:herbrand_provides
    ~desc:"provides syntactic (herbrand) interpreation" ~package:"core"
    ~name:"syntax"
    (KB.return (module Herbrand : Theory.Core))

let () =
  Extension.declare ~doc:"provides core theory rules"
    ~provides:[ "semantics"; "core-theory" ]
  @@ fun ctxt ->
  if Extension.Configuration.get ctxt herbrand_enabled then enable_herbrand ();
  Ok ()
