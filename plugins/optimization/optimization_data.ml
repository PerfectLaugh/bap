...
type jmp_update = { cond : exp; kind : jmp_kind }
type update = Rhs of exp | Jmp of jmp_update
type t = { deads : Tid.Set.t; updates : update Tid.Map.t }

let updated_term =
  Value.Tag.register
    (module Unit)
    ~name:"updated-term" ~uuid:"d21d76fa-12dd-470f-902e-f0e890e382d3"
...
let updates_of_sub sub =
  ...
  let update_rhs updates d =
    ...
    Map.set updates ~key:(Term.tid d) ~data:(Rhs rhs)
  in
  let update_jmp updates j =
    let j = Jmp.map_exp ~f:drop_index j in
    let data = Jmp { cond = Jmp.cond j; kind = Jmp.kind j } in
    Map.set updates ~key:(Term.tid j) ~data
  in
  let add_if add updates t = if is_updated t then add updates t else updates in
...
let dead_jmps_of_blk b =
  ...
  |> Seq.fold
       ...
       ~f:(fun (deads, is_unreachable) jmp ->
         if is_unreachable then (Set.add deads (Term.tid jmp), is_unreachable)
         else
...
let remove_dead_edges g dead_jmps =
  ...
  Seq.fold (G.edges g) ~init:g ~f:(fun g edge ->
      if Set.mem dead_jmps (Graphs.Tid.Edge.label edge) then
...
let dead_blks g =
  ...
  @@ Graphlib.depth_first_search
       ...
       g
       ~init:(Set.empty (module Tid), false)
       ~start_tree:(fun node (deads, _) -> (deads, Tid.equal node G.start))
...
let find_unreachable sub t =
  ...
  let dead_blks = remove_dead_edges (Sub.to_graph sub) dead_jmps |> dead_blks in
...
let filter_map_alive deads cls ?(f = Fn.id) x =
  Term.filter_map cls x ~f:(fun t ->
      if Set.mem deads (Term.tid t) then None else Some (f t))

let remove_dead_code sub { deads } =
  let update_blk b =
    filter_map_alive deads def_t b |> filter_map_alive deads jmp_t
  in
...
let apply sub { deads; updates } =
  let update_blk b =
    ...
    |> filter_map_alive deads jmp_t ~f:(update_jmp updates)
  in
  filter_map_alive deads blk_t sub ~f:update_blk
