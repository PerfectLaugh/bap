...
type property = { extract : project -> addr -> string option; explain : string }

let find_in_memory extract element proj addr =
  let memory = Project.memory proj in
  Memmap.lookup memory addr
  |> Seq.find_map ~f:(fun (_, v) -> Value.get element v >>| extract)
    ...
    ( "symbol",
      {
        ...
        explain =
          "name of the enclosing symbol, where the symbol is looked up in the \
           file symbol table or debuging information, if any";
      } );
    ( "section",
      {
...
let print_bir patterns sema ppf proj =
  let pp =
    match sema with None -> Program.pp | Some cs -> Program.pp_slots cs
  in
...
module Adt = struct
  ...
  module Tid = struct
    let pp ppf tid =
      pr ppf "Tid(%#Ld, %S)" (Int63.to_int64 (KB.Object.id tid)) (Tid.name tid)
  end
  ...
  let pp_seq pp_elem ch seq =
    let rec pp ch = function
      | [] -> ()
      | [ x ] -> pp_elem ch x
      | x :: xs -> pr ch "%a,@ %a" pp_elem x pp xs
    in
  ...
  let pp_attrs ch dict = pr ch "Attrs(%a)" (pp_seq pp_attr) (Dict.data dict)

  let pp_term name pp_self ch t =
    let tid = Term.tid t and dict = Term.attrs t in
    pr ch "%s(%a,@, %a,@, %a)" name Tid.pp tid pp_attrs dict pp_self t
  ...
  let pp_phi_value ch (tid, exp) = pr ch "(%a,%a)" Tid.pp tid pp_exp exp

  let pp_phi_self ch t =
    let lhs = Phi.lhs t and values = Phi.values t in
    pr ch "%a,@ Values(%a)" pp_var lhs (pp_seq pp_phi_value) values
  ...
  let pp_call ch c =
    pr ch "(%a,%s)" pp_label (Call.target c)
      (Option.value_map (Call.return c) ~default:"" ~f:(fun l ->
           Format.asprintf "%a" pp_label l))
  ...
  let pp_blk_self ch t =
    pr ch "Phis(%a),@ Defs(%a),@ Jmps(%a)" (pp_seq pp_phi) (Term.enum phi_t t)
      (pp_seq pp_def) (Term.enum def_t t) (pp_seq pp_jmp) (Term.enum jmp_t t)
  ...
  let pp_sub_self ch t =
    pr ch "%S,@, Args(%a),@ Blks(%a)" (Sub.name t) (pp_seq pp_arg)
      (Term.enum arg_t t) (pp_seq pp_blk) (Term.enum blk_t t)
  ...
  let pp_byte ppf byte = fprintf ppf "\\x%02x" (ok_exn (Word.to_int byte))
  ...
  let pp_memmap ppf memmap =
    let pp_region ppf mem =
      pr ppf "Region(%a,%a)" pp_word (Memory.min_addr mem) pp_word
        (Memory.max_addr mem)
    in
    let pp_binding ppf (mem, attr) =
      pr ppf "Annotation(%a,@ %a)" pp_region mem pp_attr attr
    in
    fprintf ppf "Memmap(%a)" (pp_seq pp_binding) (Memmap.to_sequence memmap)

  let pp_project ppf proj =
    fprintf ppf "Project(%a,@ %a,@ %a,@ %a)" pp_attrs (Project.storage proj)
      pp_sections (Project.memory proj) pp_memmap (Project.memory proj)
      pp_program (Project.program proj)
end

let print_callgraph patterns ppf proj =
  let prog = extract_program patterns proj in
  fprintf ppf "%a@." Graphs.Callgraph.pp (Program.to_graph prog)
...
module Ir_pp = struct
  ...
  let succ_tid_of_jmp jmp : tid option =
    ...
    | Int (_, tid) -> Some tid
    | Call t -> (
        Option.(Call.return t >>= function Direct tid -> Some tid | _ -> None))
    | _ -> None

  let node_label blk =
    let phis = Term.enum phi_t blk |> Seq.map ~f:Phi.to_string in
    let defs = Term.enum def_t blk |> Seq.map ~f:Def.to_string in
    let jmps =
      Term.enum jmp_t blk
    ...
    let body =
      String.concat ~sep:"\n" lines
      |> String.concat_map ~f:(function '\n' -> "\\l" | c -> Char.to_string c)
    in
    sprintf "\\%s\n%s" (Term.name blk) body

  let string_of_node b = sprintf "\\%s" (Tid.to_string (Term.tid b))
end
...
let pp_bir_cfg ~labeled name ppf g =
  ...
  let edge_label e =
    match Cfg.Edge.cond e g with
    | Bil.Int w when Bitvector.is_one w -> ""
    | exp -> Exp.to_string exp
  in
  let nodes_of_edge e =
    Cfg.(Node.label (Edge.src e), Node.label (Edge.dst e))
  in
  let node_label, edge_label =
    if labeled then (Some node_label, Some edge_label) else (None, None)
  in
  Graphlib.Dot.pp_graph ~name ~subgraph:true ~cluster:true ~string_of_node
    ?node_label ?edge_label ~nodes_of_edge ~nodes ~edges ppf
  ...
  let entries =
    ...
    |> Seq.fold
         ...
         ~f:(fun entries sub ->
           ...
           | None -> entries
           | Some entry ->
               Map.add_exn entries ~key:(Term.tid sub) ~data:(Term.tid entry))
  in
  fprintf ppf "digraph {@\nnode[shape=box];@\n";
  Term.enum sub_t prog
  |> Seq.iter ~f:(fun sub ->
...
let sorted_blocks nodes =
  ...
  Seq.fold nodes ~init ~f:Set.add |> Set.to_sequence

let sort_fns fns =
  let fns = Array.of_list_rev fns in
  Array.sort fns ~compare:(fun (_, b1, _) (_, b2, _) -> Block.compare b1 b2);
  Seq.of_array fns
...
let pp_bil fmt _ _ ppf (mem, insn) =
  ...
  fprintf ppf "%a: %s@\n%a@\n" pp_addr addr (Insn.asm insn) pp_bil
    (Insn.bil insn)
...
let jmp_dst insn =
  let rec find =
    List.find_map ~f:(function
      ...
      | Bil.If (_, yay, nay) -> Option.first_some (find yay) (find nay)
      | _ -> None)
  in
  find (Insn.bil insn)
...
let print_jmp_dst tab blk ppf insn =
  ...
  | Some dst, None -> Format.fprintf ppf " # %s" (Addr.string_of_value dst)
  | None, Some name -> Format.fprintf ppf " # <%s>" name
  | None, None -> ()
...
let pp_insn fmt tab blk ppf (mem, insn) =
  ...
  if phys_equal insn (Block.terminator blk) then print_jmp_dst tab blk ppf insn;
  fprintf ppf "@\n"

let pp_knowledge ppf _ = KB.pp_state ppf @@ Toplevel.current ()
...
let main attrs ansi_colors demangle symbol_fmts subs secs patterns doms =
  ...
  let ver = version in
  let pp_syms =
    Data.Write.create ~pp:(print_symbols patterns demangle symbol_fmts) ()
  in
  Project.add_writer ~desc:"print symbol table" ~ver "symbols" pp_syms;
  ...
  Project.add_writer ~desc:"print program in IR" ~ver "bir" pp_bir;
  Project.add_writer ~desc:"print program IR in ADT format" ~ver "adt" pp_adt;
  let pp_callgraph = Data.Write.create ~pp:(print_callgraph patterns) () in
  Project.add_writer ~ver "callgraph"
    ~desc:"print program callgraph in DOT format" pp_callgraph;
  let pp_cfg =
    Data.Write.create ~pp:(print_bir_graph ~labeled:true patterns) ()
  in
  let pp_calls =
    Data.Write.create ~pp:(print_bir_graph ~labeled:false patterns) ()
  in
  let pp_spec = Data.Write.create ~pp:print_spec () in
  let pp_disasm_bil =
    Data.Write.create ~pp:(print_disasm (pp_bil "pretty") patterns) ()
  in
  let pp_disasm_bil_adt =
    Data.Write.create ~pp:(print_disasm (pp_bil "adt") patterns) ()
  in
  let pp_disasm_bil_sexp =
    Data.Write.create ~pp:(print_disasm (pp_bil "sexp") patterns) ()
  in
  let pp_disasm_asm =
    Data.Write.create ~pp:(print_disasm (pp_insn "asm") patterns) ()
  in
  let pp_disasm_adt =
    Data.Write.create ~pp:(print_disasm (pp_insn "adt") patterns) ()
  in
  let pp_disasm_decoded =
    Data.Write.create ~pp:(print_disasm (pp_insn "pretty") patterns) ()
  in
  let pp_disasm_sexp =
    Data.Write.create ~pp:(print_disasm (pp_insn "sexp") patterns) ()
  in

  let pp_knowledge = Data.Write.create ~pp:pp_knowledge () in

  Project.add_writer ~ver "knowledge" ~desc:"dumps the knowledge base"
    pp_knowledge;
  Project.add_writer ~ver "cfg" ~desc:"print rich CFG for each procedure" pp_cfg;
  Project.add_writer ~ver "graph" ~desc:"print unlabeled CFG for each procedure"
    pp_calls;
  Project.add_writer ~ver "asm" ~desc:"print assembly instructions"
    pp_disasm_asm;
  ...
  Project.add_writer ~ver "bil" ~desc:"print BIL instructions" pp_disasm_bil;
  Project.add_writer ~ver "bil.adt" ~desc:"print BIL instructions in ADT format"
    pp_disasm_bil_adt;
  Project.add_writer ~ver "bil.sexp"
    ~desc:"print BIL instructions in Sexp format" pp_disasm_bil_sexp;
  Project.add_writer ~desc:"print the file specification in the OGRE format"
    ~ver "ogre" pp_spec
...
let () =
  ...
  let demangle : string option Config.param =
    ...
    Config.(param (some string) "demangled-with" ~doc)
  in
  let bir_attr : string list Config.param =
    let doc = "When printing IR emit an attribute $(docv)" in
    Config.(param_all string "bir-attr" ~docv:"NAME" ~doc)
  in
  let ansi_colors : bool Config.param =
    let doc = "Allow coloring output with ansi color escape sequences" in
    let default = Caml_unix.(isatty stdout) in
    Config.(param bool ~default "with-colors" ~doc)
  in
  let print_symbols : _ list Config.param =
    let opts =
      [ ("name", `with_name); ("addr", `with_addr); ("size", `with_size) ]
    in
    let doc =
      sprintf
        "Print found symbols. Optional value defines output format, and can be \
         %s. You can specify this parameter several times, if you want both, \
         for example."
      @@ Config.doc_enum opts
    in
    let default = [ `with_name ] in
    Config.(param_all (enum opts) ~default "symbol-format" ~doc)
  in
  let subs : string list Config.param =
    let doc = "same as $(b,--print-matching=symbol:)$(docv)" in
    Config.(param_all string "symbol" ~docv:"NAME" ~doc)
  in
  let secs : string list Config.param =
    let doc = "same as $(b,--print-matching=section:)$(docv)" in
    Config.(param_all string "section" ~docv:"NAME" ~doc)
  in
  ...
  let patterns : string list Config.param =
    ...
    in
    Config.(param_all string "matching" ~doc)
  in
  ...
  let semantics : string list option Config.param =
    ...
    Config.(
      param
        (some (list string))
        ~as_flag:(Some []) ~doc ~docv:"SEMANTICS-LIST" "semantics")
  in
  Config.declare_extension ~doc:"prints the project in various formats"
    ~provides:[ "printer" ] (fun { Config.get = ( ! ) } ->
      main !bir_attr !ansi_colors !demangle !print_symbols !subs !secs !patterns
        !semantics)
