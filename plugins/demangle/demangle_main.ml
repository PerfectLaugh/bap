...
let provides = [ "symbolizer"; "demangling"; "demangler"; "symbolizer" ]
...
module Internal = struct
  ...
  let demangle_internal str =
    ...
    let extract_number pos_ref =
      lfindi str ~pos:!pos_ref ~f:(fun _ c -> Char.is_digit c) >>= fun s1_p0 ->
      lfindi str ~pos:s1_p0 ~f:(fun _ c -> not (Char.is_digit c))
      >>= fun s1_p1 ->
      let len = s1_p1 - s1_p0 in
      let str = Bytes.of_string str in
      let n =
        Substring.create ~pos:s1_p0 ~len str
        |> Substring.to_string |> Int.of_string
      in
      pos_ref := s1_p0 + len;
      Some n
    in
    let extract_name pos_ref =
      ...
      let name =
        Substring.create ~pos:!pos_ref ~len str |> Substring.to_string
      in
      pos_ref := !pos_ref + len;
      Some name
    in
    ...
    let rec extract_names acc =
      ...
      | Some name -> extract_names (name :: acc)
    in
    match extract_names [] |> String.concat ~sep:"::" with "" -> str | s -> s
  ...
  let install () = Demangler.declare ~package:"bap" "internal" run
end

let decide_name_from_possible_name ?override () : unit =
  let open KB.Syntax in
  KB.Rule.(
    declare ~package:"core" "name-of-possible-names"
  ...
  let demangler =
    ...
    | Some name -> Demanglers.get ~package:"bap" name
  in
