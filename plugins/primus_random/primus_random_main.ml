...
let is_numeral s = String.length s > 0 && Char.is_digit s.[0]
...
module Generator = struct
  ...
  type predicate = Any | Var of string list | Mem of region
  type name = Static | Random [@@deriving sexp]
  ...
  let of_sexp : Sexp.t -> generator = function
    | Atom ("static" | "random") as dis ->
        { predicate = Any; distribution = name_of_sexp dis; parameters = [] }
  ...
  let pp_predicate ppf = function
    ...
    | Mem (Address str | Section str) -> Format.fprintf ppf "(mem %s)" str
    | Mem (Interval (lower, upper)) ->
        Format.fprintf ppf "(mem %s %s)" lower upper

  let pp ppf { predicate = p; distribution = d; parameters = ps } =
    Format.fprintf ppf "(%a %a %a)" pp_predicate p pp_name d pp_strings ps

  let to_string = Format.asprintf "%a" pp
  let from_file filename = Sexp.load_sexps_conv_exn filename of_sexp
  let parse str = Sexp.of_string str |> of_sexp
  let default = { predicate = Any; distribution = Random; parameters = [] }
  let t = Extension.Type.define ~name:"GEN" ~parse ~print:to_string default
  let list = Extension.Type.(list t)

  let create_uniform ?(seed = 0) ?width ps =
    let min, max, local_seed =
      match ps with
      | [] -> (None, None, 0)
      | [ max ] -> (None, Some (int_of_string max), seed)
      | [ min; max ] -> (Some (int_of_string min), Some (int_of_string max), 0)
      | [ min; max; seed ] ->
          ( Some (int_of_string min),
            Some (int_of_string max),
            int_of_string seed )
      | _ -> invalid_argf "Generator Uniform expects less than 3 arguments" ()
    in
    Primus.Generator.Random.lcg (seed + local_seed) ?width ?min ?max
  ...
  let create_const ?width = function
    ...
    | xs ->
        ...
        Primus.Generator.of_iterator
          ...
          0 ~to_bitvec:Fn.id ?width

  let create ?seed ?width n ps =
    match n with
    | Random -> create_uniform ?seed ?width ps
    | Static -> create_const ?width ps

  let first_match ?seed ?width matches =
    List.find_map ~f:(fun s ->
        if matches s.predicate then
          Some (create ?seed ?width s.distribution s.parameters)
        else None)

  let for_var ?seed ?width v =
    first_match ?seed ?width @@ function
    | Var [] | Any -> true
...
let generators =
  Extension.Configuration.parameter Generator.list "generators"
    ~doc:
      "A list of generator specifications. The generators are\n\
...
let seed =
  Extension.Configuration.parameter Extension.Type.int "seed"
    ~doc:"The seed that will be used to initialize all generators."

let keep_protected =
  Extension.Configuration.flag "preserve-protection"
    ~doc:
      "Preserves page protection flags when a new page is swapped in. When \
       this flag is not set the $(b,bap:mem-randomizer) will map missing \
       memory pages as writable."

type arg_generators = { args : Primus.Generator.t Var.Map.t }

let debug_msg, post_msg =
  Primus.Observation.provide "random-debug" ~inspect:sexp_of_string
...
let main ctxt =
  ...
  let args =
    Primus.Machine.State.declare ~uuid:"2d9a70fb-8433-4a53-a750-3151f9366cb6"
      ~name:"generators-for-arguments"
    @@ fun proj ->
  ...
  let module TrapPageFault (Machine : Primus.Machine.S) = struct
    ...
    let exec = Code.unlink (`symbol Primus.Interpreter.pagefault_handler)
  end in
  let module RandomizeMemory (Machine : Primus.Machine.S) = struct
    ...
    let allocate ?upper ~width lower generator =
      let eval s = Eval.exp (parse_exp ~width s) >>| Primus.Value.to_word in
    ...
    *)
    ...
    let map_page ?generator already_mapped addr =
      let rec map len =
        ...
        | true -> map (len / 2)
        | false -> Memory.allocate ?generator addr len
      in
    ...
    let init () =
      Machine.gets Project.target >>| Theory.Target.bits >>= fun width ->
      let generator =
        Generator.first_match ~seed ~width
          (function Mem Default | Any -> true | _ -> false)
          generators
      in
      Machine.sequence
        [
  ...
  let module PertubeOutputs (Machine : Primus.Machine.S) = struct
    ...
    let pertube gen : exp -> unit Machine.t = function
      ...
      | Load (_, ptr, endian, size) ->
          ...
          Gen.word gen (Size.in_bits size) >>= Val.of_word >>= fun value ->
  ...
  let module HandleUnresolvedCalls (Machine : Primus.Machine.S) = struct
    ...
    let init () = Linker.link ~name (module PertubeOutputs)
  end in
  Primus.Components.register_generic "var-randomizer"
    (module RandomizeEnvironment)
    ~package:"bap" ~desc:"Randomizes registers.";

  Primus.Components.register_generic "mem-randomizer"
    (module RandomizeMemory)
    ~package:"bap" ~desc:"Randomizes process memory.";
...
let () = Extension.declare ~doc main ~provides:[ "primus" ]
