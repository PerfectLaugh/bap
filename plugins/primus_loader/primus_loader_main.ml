...
let init s b =
  ...
  Primus.Components.register_generic ~package:"bap" "program-loader"
    (module Loader)
    ~desc:
      "Initializes the runtime environment. This is a generic loader that \
       should work with most of the ABIs. It loads memory segments, including \
       virtual,  sets up [brk], [end], [edata], [etext], and [environ] \
       variables (see any unix man page for the description of these symbols). \
       Note that ([edata] and [etext] are not guaranteed, while [end] and \
       [brk] are). The loader also initializes all CPU registers to zero and \
       setups and alloocates the main stack frame (copies arguments from the \
       ctxt to the stack), and points the stack-pointer register to the \
       command line arguments. (The environment variables follows the \
       arguments)."
;;
...
let stack_base =
  Config.(
    param int64
      ~default:Int64.(1L * 1024L * 1024L * 1024L)
      "stack-base" ~doc:"default address of the stack base")

let () =
  Config.declare_extension ~doc:"generic Primus dynamic loader"
    ~provides:[ "abi"; "loader"; "primus"; "emulator" ]
    (fun { Config.get = ( !! ) } -> init !!stack_size !!stack_base)
