...
type t = { dicts : dict Primus.Value.Map.t }
...
*)
let state =
  Primus.Machine.State.declare ~uuid:"F050079C-7FDB-4A8F-8A59-72C9693ADBCE"
    ~name:"lisp-dictionaries" (fun _ -> { dicts = Primus.Value.Map.empty })
...
module Add (Machine : Primus.Machine.S) = struct
  ...
  let run [ dic; key; data ] =
    Machine.Local.update state ~f:(fun s ->
        {
          dicts =
            Map.update s.dicts dic ~f:(function
              | None -> pair key data
...
module Get (Machine : Primus.Machine.S) = struct
  ...
  let run [ dic; key ] =
    ...
    | None -> nil
    | Some { dict } -> (
        match Map.find dict key with None -> nil | Some x -> Machine.return x)
end
  ...
  let run = function
    ...
    | [ dic; key ] ->
        Machine.Local.update state ~f:(fun s ->
            {
              dicts =
                Map.change s.dicts dic ~f:(function
                  | None -> None
                  | Some { dict } -> Some { dict = Map.remove dict key });
            })
        >>= fun () -> nil
end
...
let () =
  ...
  Primus.Components.register_generic "lisp-dictionary"
    (module Main)
    ~package:"bap" ~desc
