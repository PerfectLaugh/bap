...
module Param = struct
  ...
  manpage
    [
      `S "DESCRIPTION";
      `P
        "Populates and run the Primus Jobs queue.\n\
  ...
  let entry =
    param (list string) "entry-points"
      ~doc:
        "Can be a list of $(i,entry points) or one of the following\n\
  ...
  let in_isolation =
    flag "in-isolation"
      ~doc:
        "Run each entry point as new system.\n\n\
  ...
  let with_repetitions =
    flag "with-repetitions"
      ~doc:
  ...
  let systems =
    param_all (list string) "systems" ~default:[ [ "bap:legacy-main" ] ]
      ~doc:
        "Runs the specified Primus systems. If several systems are specified \
         then runs all entry points for each specified system."

  let until_visited =
    flag "until-visited-all"
      ~doc:
        "Runs Primus until all subroutines and blocks are visited.\n\
...
let name_of_entry target entry =
  ...
  if String.is_empty entry then
    invalid_arg "An entry point should be a non-empty string"
  else
    match entry.[0] with
...
let entry_point_collector =
  object
    inherit [tid list] Term.visitor

    method! enter_term _ t entries =
      if Term.has_attr t Sub.entry_point then Term.tid t :: entries else entries
  end

let entry_points prog = entry_point_collector#run prog []

let callgraph start prog =
  let mark_as_root n g =
    if Tid.(n = start) then g else Callgraph.Edge.(insert (create start n []) g)
  in
  ...
  let connect_unreachable_scc g =
    Graphlib.depth_first_search
      (module Callgraph)
      g ~start ~init:g ~start_tree:mark_as_root
  in
  Program.to_graph prog |> connect_inputs |> connect_unreachable_scc
...
let all_subroutines ?(marked = false) prog =
  ...
  let unmarked =
    if not marked then Tid.Set.empty
    else
  ...
  @@ Graphlib.reverse_postorder_traverse (module Graphs.Callgraph) ~start
...
let exec x =
  ...
  info "Fork %a: starting from the %s entry point" pp_id cid (string_of_name x);
  Machine.catch (Linker.exec x) (fun exn ->
      info "execution from %s terminated with: %s " (string_of_name x)
        (Primus.Exn.to_string exn);
      Machine.return ())

let visited =
  Primus.Machine.State.declare ~uuid:"c6028425-a8c7-48cf-b6d9-57a44ed9a08a"
    ~name:"visited-subroutines" (fun _ -> Set.empty (module Tid))
...
let run need_repeat entries =
  ...
  let rec loop stage = function
    ...
    | x :: xs ->
        ...
        Machine.fork () >>= fun () ->
        Machine.current () >>= fun cid ->
        if Id.(pid = cid) then loop (stage + 1) xs
        else
          is_visited x >>= fun is_visited ->
          if is_visited && not need_repeat then Eval.halt >>= never_returns
          else
            exec x >>= fun () ->
...
let enqueue_super_job need_repeat envp args sys xs =
  Primus.Jobs.enqueue sys ~envp ~args ~name:"multi-task"
    ~desc:"runs all entries in one system" ~start:(run need_repeat xs)
...
let enqueue_separate_jobs need_repeat envp args sys xs =
  List.iter xs ~f:(fun p ->
      Primus.Jobs.enqueue sys ~args ~envp
        ~name:(Primus.Linker.Name.to_string p)
        ~start:
          ( Machine.get () >>= fun proj ->
            if (not need_repeat) && is_visited proj p then Machine.return ()
            else exec p ))
...
let on_success job status _ result : Primus.Jobs.action =
  ...
  | Primus.Exn e ->
      error "The job `%s' finished abnormally with an exception:@\n%s"
        (Primus.Job.name job) (Primus.Exn.to_string e);
      Continue
...
let main { Config.get = ( ! ) } proj =
  ...
  let enqueue_jobs =
    if !in_isolation then enqueue_separate_jobs else enqueue_super_job
  in
  ...
  let enqueue_term p =
    ...
    List.iter systems ~f:(fun sys ->
        Primus.Jobs.enqueue sys ~args:!argv ~envp:!envp
          ~name:(Primus.Linker.Name.to_string p)
          ~start:(exec p))
  in
  let rec run proj =
    ...
    let prog = Project.program proj in
    if run_until_visited then
      match find_first_unvisited_sub prog with
      | Some sub ->
          enqueue_term sub;
          run proj
      | None -> (
          match find_first_unvisited_blk prog with
          | Some blk ->
              enqueue_term blk;
              run proj
          | None -> proj)
...
let deps = [ "trivial-condition-form" ]

let () =
  Config.declare_extension ~doc:"emulates the program using Primus"
    ~provides:[ "pass"; "primus"; "emulator"; "microexecution" ] (fun conf ->
      Project.register_pass ~deps (main conf);
      Primus.Machine.add_component (module Visited) [@warning "-D"];
      Primus.Components.register_generic "records-visited"
        (module Visited)
        ~internal:true ~package:"run")
