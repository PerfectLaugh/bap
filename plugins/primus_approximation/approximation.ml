      ...
      ( "sin",
        floats
          [|
            ...
            1.0000000200128274;
            -3.0302439009453449e-7;
            -1.6666487069033565e-1;
            -5.4940340462839474e-6;
            8.3432342039387503e-3;
            -1.1282260464920005e-5;
            -1.8998652204112551e-4;
            -4.1639895115337191e-6;
            4.0918788476303817e-6;
...
let parse_op : string -> op option =
 fun s -> Option.try_with (fun () -> op_of_sexp (Sexp.of_string s))

let size_of_var var =
  let size = Bap.Std.Var.typ var in
  match size with Type.Imm size -> size | _ -> assert false
...
let with_fresh_var exp body =
  ...
  Var.scoped sort @@ fun v -> CT.let_ v !!a (body v)
...
module Range_Reduction = struct
  ...
  let fast_and_dirty_is_fpos x = and_ (inv (msb (fbits x))) (non_zero (fbits x))
  ...
  let fast_and_dirty_floor rm x =
    x >>-> fun s x -> cast_float s rm @@ cast_int (Floats.size s) rm !!x
  ...
  let fmod r x y =
    fmul r x y >>>= fun d ->
    floor r (var d) >>>= fun c ->
    fmul r y (var c) >>>= fun z -> fsub r x (var z)
...
module Range_Reconstruction = struct
  let sign_flip rm sign x = CT.fmul rm sign x
end
...
module Sin = struct
  ...
  let build ?(rm = rne) x c poly_eval =
    range_reduce rm x @@ fun n needs_corr ->
    poly_eval c n >>>= fun p -> ite needs_corr (fneg (var p)) (var p)
end
...
module Horner : sig
  ...
  let approximate ~coefs ?(rm = rne) x =
    let rank = Array.length coefs - 1 in
    let rec sum i y =
      if i >= 0 then
        fmul rm x y >>>= fun y ->
        fadd rm (var y) coefs.(i) >>>= fun y -> sum (i - 1) (var y)
      else y
    in
  ...
  let build func var coefs =
    ...
    let formula =
      ...
      | Cos ->
          let shifted_var =
            CT.(fadd rne (var v) (Reduction_Constant.pi_div_2 fsort))
          in
          Sin.build shifted_var c polynomial_evaluaton
    in
    exp formula
end
...
module Approximate (Machine : Primus.Machine.S) = struct
  ...
  let int_of_value x = Primus.Value.to_word x |> Word.to_int_exn
  ...
  let run [ name; size; x ] =
    ...
    let elementry_function =
      match parse_op name with Some op -> op | _ -> assert false
    in
    let coefficients =
      let name = match elementry_function with Sin | Cos -> "sin" in
...
let main () = Primus.Machine.add_component (module Main)
