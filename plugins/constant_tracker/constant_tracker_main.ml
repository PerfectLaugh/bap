...
type state = { consts : Primus.Value.Id.Set.t }

let state =
  Primus.Machine.State.declare ~uuid:"caf680aa-ef54-4468-9a24-4a125de6ace9"
    ~name:"constant-tracker" (fun _ -> { consts = Primus.Value.Id.Set.empty })

let is_const x { consts } = Set.mem consts (Primus.Value.id x)
let all_consts xs s = List.for_all xs ~f:(fun x -> is_const x s)
let declare_const v { consts } = { consts = Set.add consts (Primus.Value.id v) }
...
module IsConstBuffer (Machine : Primus.Machine.S) = struct
  ...
  let points_to_static_data s ptr len =
    let rec check ptr len =
      if Word.is_zero len then Value.b1
      else
        Memory.get ptr >>= fun v ->
        if is_const v s then check (Addr.succ ptr) (Word.pred len) else Value.b0
    in
  ...
  let run [ ptr; len ] =
    Machine.Local.get state >>= fun s -> points_to_static_data s ptr len
end
...
module Primitives (Machine : Primus.Machine.S) = struct
  ...
  let init () =
    Machine.sequence
      [
        Lisp.define "all-static-constant"
          (module IsConst)
          ~types:(all any @-> bool)
          ~docs:
            "(all-static-constant X Y ..) is true if X,Y,... are static \
             constants.\n\
            \    A value is a static constant if it was initialized from a \
             constant\n\
            \    value or computed from static constant values. ";
        Lisp.define "points-to-static-data"
          (module IsConstBuffer)
...
module Tracker (Machine : Primus.Machine.S) = struct
  ...
  let propagate inputs output =
    ...
    if all_consts inputs s then Machine.Local.put state (declare_const output s)
    else Machine.return ()
...
let () =
  Config.declare_extension ~doc:"a constant tracking analysis based on Primus"
  ...
  Primus.Components.register_generic "constant-tracker"
    ...
    ~package:"bap"
    ~desc:
      "Tracks constants, using the following policy. A value is a static \
       constant if it was initialized from a constant value or computed from \
