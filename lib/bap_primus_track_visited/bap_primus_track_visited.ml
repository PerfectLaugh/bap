...
type state = { visited : Tid.Set.t; total : int }
...
let state =
  Primus.Machine.State.declare ~name:"primus-track-visited"
...
let live =
  {
    mark = (fun t -> Term.set_attr (Term.del_attr t Term.dead) Term.visited ());
  }

let mark_block { mark } t =
  mark t |> Term.map def_t ~f:mark |> Term.map jmp_t ~f:mark

let marker p marker =
  object
    inherit Term.mapper
    method! map_blk t = if not (p t) then t else mark_block marker t
  end
...
let progress, report_progress =
  Primus.Observation.provide "visited-blocks" ~inspect:[%sexp_of: int * int]
    ~desc:
      "visited-blocks (visited,total) is posted every time a new basic block \
       is visited, where [visited] is the number of already visited blocks and \
...
module Tracker (Machine : Primus.Machine.S) = struct
  ...
  let visit t =
    ...
    let s = { s with visited = Set.add s.visited (Term.tid t) } in
    Machine.Observation.post report_progress ~f:(fun k ->
        k (Set.length s.visited, s.total))
    >>= fun () -> Machine.Global.put state s

  let visit_stub (name, _) =
    Linker.resolve_tid (`symbol name) >>= function
    | None -> Machine.return ()
    | Some tid -> (
        Machine.gets Project.program >>= fun prog ->
        match Term.find sub_t prog tid with
        | None -> Machine.return ()
        | Some sub -> Term.enum blk_t sub |> Machine.Seq.iter ~f:visit)
...
let init () =
  Primus.Components.register_generic "mark-visited"
    (module Tracker)
    ~package:"bap"
    ~desc:
      "Marks visited (by Primus) program terms with the [Term.visited] \
       attribute and unvisited with the [Term.dead] attribute. Note, that the \
       attributes are attached only when the system exits";
