...
module Name : sig
  ...
  module Id : sig
    ...
    let intern name =
      ...
      | None ->
          Hashtbl.add_exn registry ~key:id ~data:name;
          id
      | Some name' ->
  ...
  let register ?(desc = "no description provided") ?package
    ...
    if Hashtbl.mem agents agent then
      failwithf
        ...
        Name.str name ();
    Hashtbl.add_exn agents ~key:agent ~data:{ desc; name; rcls = reliability };
    agent
...
module Domain = struct
  ...
  let powerset (type t o)
      (module S : Base.Comparator.S
        with type t = t
         and type comparator_witness = o) ?(inspect = S.comparator.sexp_of_t)
      name =
  ...
  let mapping (type k o d)
      (module K : Base.Comparator.S
        with type t = k
         and type comparator_witness = o) ?(inspect = sexp_of_opaque) ?join
      ~equal name =
...
module Registry = struct
  ...
  let public_class cls = Hashtbl.add_exn public ~key:cls ~data:[]

  let update_class ~cls ~slot =
    if is_public cls then Hashtbl.add_multi public ~key:cls ~data:slot
...
module Knowledge = struct
  ...
  let map_update_objects { Class.name } f =
    ...
    in
    f objs @@ fun objs res ->
    put { state with classes = Map.set state.classes ~key:name ~data:objs }
    >>| fun () -> res
  [@@specialise]
    ...
    *)
    ...
    let do_intern =
      ...
      let createsym ~public name classes clsid objects s =
        ...
        put { s with classes = Map.set classes ~key:clsid ~data:objects }
        >>| fun () -> obj
      in
      ...
      fun ?(public = false) ?desc:_ name { Class.name = id } ->
        ...
        | Some obj ->
            if is_public name obj objects then unchanged obj
              ...
              put { s with classes = Map.set classes ~key:id ~data:objects }
              >>| fun () -> obj
        ...
        include
          Binable.Of_binable_with_uuid
            ...
            (struct
              ...
              let caller_identity =
                Bin_shape.Uuid.of_string "97c4dd90-34db-4ebb-a8b9-43b75164144e"
            end)
  ...
  let dequeue_waiting : ('a, 'p) slot -> 'a obj -> _ Knowledge.t =
   fun s x ->
    ...
    | Env.Work { waiting } ->
        ...
        in
        let info =
          { info with comp = Map.set works ~key:(uid s) ~data:no_work }
        in
  ...
  module Symbol = struct
    ...
    let intern_symbol name obj cls =
      Knowledge.return
        Env.{ cls with objs = Map.add_exn cls.objs ~key:name ~data:obj }
      ...
      Knowledge.List.fold ~init:s.classes imports ~f:(fun classes name ->
          ...
          |> Knowledge.Seq.fold ~init:classes
               ~f:(fun classes (clsid, objects) ->
                 ...
                 >>| fun objects -> Map.set classes ~key:clsid ~data:objects))
  ...
  module Io = struct
    ...
    let expand_comp comp =
      List.fold comp
        ...
        ~f:(fun works slot -> Map.add_exn works ~key:slot ~data:Env.Done)
    ...
    let add_object ({ Env.vals; objs } as self) { key; sym; data; comp } =
      ...
      match sym with
      | None -> self
      | Some s -> { self with objs = Map.add_exn objs ~key:s ~data:key }
    ...
    let load path =
      ...
      try
        let data =
          ...
          @@ Unix.map_file fd Bigarray.char Bigarray.c_layout ~shared:false
               [| -1 |]
        in
    ...
    let save state path =
      ...
      let size = String.length magic + bin_size_canonical bin_size_v2 repr in
      let fd =
        Unix.openfile path ~mode:Unix.[ O_RDWR; O_CREAT; O_TRUNC ] ~perm:0o660
      in
      try
        let dim = [| size |] in
        let buf =
          Bigarray.array1_of_genarray
          @@ Unix.map_file fd Bigarray.char Bigarray.c_layout ~shared:true dim
        in
