...
let pp ppf e = pp_print_string ppf (string_of_event e)
...
module Log = struct
  ...
  let message level ~section fmt =
    ...
    kfprintf
      (fun ppf ->
        ...
        send (Message { level; section; message }))
      ppf fmt
  ...
  let progress ?note ?stage ?total task =
    send @@ Progress { task; note; stage; total }
  ...
  module Create () = struct
    ...
    let manifest =
      try Bundle.manifest bundle with _exn -> Manifest.create main
    ...
    let is_verbose = has_var ("DEBUG_" ^ name) || has_var "DEBUG"
