...
let max_logs =
  match getenv "BAP_BACKLOG" with
  | None -> 99
  | Some n -> (
      try Int.of_string n
      with _ -> invalid_argf "BAP_BACKLOG expect a number" ())

let rec rotate max_logs file =
  let next =
    try
      Scanf.sscanf file "%s@~%d" (fun name num ->
          Option.some_if (num < max_logs) (sprintf "%s~%d" name (num + 1)))
    with
    | _ when max_logs > 0 -> Some (file ^ "~1")
    | _ -> None
  in
  match next with
...
let print_message ppf msg =
  ...
  match msg.level with Error -> eprintf "%s@\n%!" msg.message | _ -> ()
...
let open_log_channel user_dir =
  try
    ...
    protect
      ~f:(fun () ->
        mkdir log_folder;
        if Sys.file_exists file then rotate max_logs file)
...
let log_plugin_events () =
  ...
  Stream.observe Plugins.events (function
    | `Loaded p ->
        message Info ~section "Loaded %s from %S" (Plugin.name p)
          (Plugin.path p)
    | `Loading p ->
        message Debug ~section "Loading %s from %S" (Plugin.name p)
          (Plugin.path p)
    | `Opening p -> message Debug ~section "Opening bundle %s" p
    | `Errored (path, err) ->
        message Error ~section "Failed to load plugin %S: %a" path Error.pp err
    | `Linking lib -> message Debug ~section "Linking library %s" lib)
...
let in_directory ?logdir () = process_events (open_log_channel logdir)
