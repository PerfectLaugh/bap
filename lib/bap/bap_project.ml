...
let query doc attr =
  ...
  | Error err ->
      invalid_argf "Malformed ogre specification: %s" (Error.to_string_hum err)
        ()
  | Ok bias -> bias
...
let union_memory m1 m2 =
  Memmap.to_sequence m2
  |> Seq.fold ~init:m1 ~f:(fun m1 (mem, v) -> Memmap.add m1 mem v)

let memory_slot =
  KB.Class.property Theory.Unit.cls "unit-memory" ~package:"bap" ~public:true
    ~desc:"annotated memory regions of the unit" Memmap.domain

let with_filename spec target _code memory path f =
  let open KB.Syntax in
  let width = Theory.Target.code_addr_size target in
  let bias =
    query spec Image.Scheme.bias
    |> Option.map ~f:(fun x -> Bitvec.(int64 x mod modulus width))
  in
  ...
  >>= fun () ->
  KB.promising Theory.Label.unit ~promise:(fun _ -> !!(Some unit)) f
...
module State = struct
  ...
  type t = { disassembly : Dis.state; subroutines : Sub.t } [@@deriving bin_io]

  let empty = { disassembly = Dis.init; subroutines = Sub.empty }
  ...
  let partition self =
    let self = { self with disassembly = Dis.forget_debt self.disassembly } in
  ...
  let cfg { disassembly } = Disasm_expert.Recursive.global_cfg disassembly
  let disassembly { disassembly = d } = d
  let subroutines { subroutines = s } = s
  let set_length set = Sexp.Atom (string_of_int @@ Set.length set)

  let inspect { disassembly; subroutines } =
    Sexp.List
      [
        List
          [
            Atom ":number-of-basic-blocks"; set_length (Dis.blocks disassembly);
          ];
        List
          [
            Atom ":number-of-subroutines"; set_length (Sub.entries subroutines);
          ];
      ]

  let slot =
    KB.Class.property Theory.Unit.cls ~package:"bap" "disassembly"
      ~persistent:
  ...
  module Toplevel = struct
    let compute_target ?file spec : Theory.target =
      ...
      let create_unit =
        ...
        | Some file -> Theory.Unit.for_file file
      in
      Toplevel.put result
...
type library = { unit : Theory.Unit.t; info : unit_info }
...
module Input = struct
  ...
  let custom ?(finish = Fn.id) ?(filename = "") ?(code = Memmap.empty)
  ...
  let create ?(finish = Fn.id) arch file ~code ~data () =
    let spec =
      match arch with
  ...
  let register_loader name loader = Hashtbl.set loaders ~key:name ~data:loader
  let is_code v = Option.is_some @@ Value.get Image.code_region v
  ...
  let load ?target ?loader ?(libraries = []) main =
    match loader with
    | None -> from_image ?target main ~libraries
    | Some name -> (
        match Hashtbl.find loaders name with
        | None -> from_image ?target ?loader main ~libraries
        | Some load ->
            fun () -> List.bind (main :: dedup libraries) ~f:(Fn.flip load ()))
  ...
  let raw ?(target = Theory.Target.unknown) ?(filename = "") ?base arch big () =
    ...
    let addr_size =
      if Theory.Target.is_unknown target then
        Arch.addr_size arch |> Size.in_bits
      else Theory.Target.code_addr_size target
    in
    let endian =
      if Theory.Target.is_unknown target then Arch.endian arch
      else if Theory.Endianness.(Theory.Target.endianness target = le) then
        LittleEndian
      else BigEndian
    in
  ...
  let from_bigstring ?base target data = raw ~target ?base `unknown data

  let from_string ?base target data =
    from_bigstring ?base target (Bigstring.of_string data)

  let available_loaders () = Hashtbl.keys loaders @ Image.available_backends ()
end
...
let roots rooter =
  match rooter with None -> [] | Some r -> Rooter.roots r |> Seq.to_list
...
let pp_mem ppf mem =
  fprintf ppf "%s" (Addr.string_of_value (Memory.min_addr mem))
...
let pp_disasm_error ppf = function
  ...
  | `Failed_to_lift (_mem, insn, err) ->
      fprintf ppf "<%s>: %a" (Disasm_expert.Basic.Insn.asm insn) Error.pp err

let set_package package =
  match package with
  | None -> KB.return ()
  ...
  List.iter
    ~f:
      (Option.iter ~f:(fun name ->
           warning
             "Project.create parameter %S is deprecated, please consult the \
              documentation for the proper alternative"
             name))
...
let compute_unit ?package ?state input =
  ...
  let compute_target target spec =
    if Theory.Target.is_unknown target then target_of_spec spec else !!target
  in
  ...
  let prog, libs =
    List.fold libs ~init:(prog, []) ~f:(fun (p, l) (info, prog, unit) ->
        ( Term.enum sub_t prog |> Seq.fold ~init:p ~f:(Term.append sub_t),
          { unit; info } :: l ))
  in
  set_package package >>| fun () -> (main, List.rev libs, prog)

let create ?package ?state ?disassembler:p1 ?brancher:p2 ?symbolizer:p3
    ?rooter:p4 ?reconstructor:p5 (read : input) =
  try
    unused_options
      [
        "disassembler" =? p1;
  ...
  | Toplevel.Conflict err ->
      ...
      let msg =
        String
          (Format.asprintf "Knowledge Base Conflict: %a" KB.Conflict.pp err)
      in
      Error (to_info msg)
...
let restore_state _ =
  failwith
    "Project.restore_state: this function should no be used.\n\
    \    Please use the Toplevel module to save/restore the state."

let set t tag x = with_storage t @@ Dict.set t.storage tag x
...
let addr which mem =
  let take =
    match which with `min -> Memory.min_addr | `max -> Memory.max_addr
  in
...
let substitute project mem tag value : t =
  ...
  let find_symbol mem =
    ...
    |> List.hd
    |> Option.map ~f:(fun (name, entry, _) -> (Block.memory entry, name))
  in
  let find_block mem =
  ...
  let subst_disasm mem out =
    ...
    | Ok dis ->
        Disasm.insns dis
        |> Seq.map ~f:(fun (_, insn) -> inj insn)
        |> Seq.to_list |> String.concat ~sep:"\n"
  in

  let apply_subst find mem subst spec value =
    match find mem with Some thing -> subst thing spec | None -> value
  in
  let sub mem x =
    let buf = Buffer.create (String.length x) in
    Buffer.add_substitute buf
      (fun x ->
        match subst_of_string x with
        | Some (`section spec) ->
        ...
        | Some (`block spec) -> apply_subst find_block mem subst_block spec x
...
let register_pass ?(autorun = false) ?(runonce = autorun) ?(deps = []) ?name
    main : unit =
  let pref = name_of_bundle () in
  let name = match name with None -> pref | Some name -> pref ^ "-" ^ name in
  let starts, started = Stream.create () in
  let successes, succeded = Stream.create () in
  let failures, failed = Stream.create () in
  let finishes =
    Stream.either successes failures |> Stream.map ~f:Either.value
  in
...
let register_pass' ?autorun ?runonce ?deps ?name v : unit =
  register_pass ?autorun ?runonce ?deps ?name (fun p ->
      v p;
      p)
...
module Registry (T : T) (I : T) = struct
  ...
  type info = { name : Knowledge.Name.t; desc : string option; extra : I.t }
  ...
  let register ?desc ?package name extra entity =
    ...
    if Hashtbl.mem registry name then
      invalid_argf
        "An element with name %s is already registered please choose a unique \
         name"
...
module Collator = struct
  ...
  let apply (Collator { prepare; collate; summary }) projects =
    ...
    | [ base ], rest ->
        summary @@ Seq.foldi ~init:(prepare base) rest ~f:collate
    | _ -> ()

  let register ?desc ?package name ~prepare ~collate ~summary =
    register ?desc ?package name () @@ Collator { prepare; collate; summary }
end
...
module Analysis = struct
  ...
  type ('a, 'r) args = { run : 'a -> 'r arg; grammar : string list }
  type problem = No_input | Bad_syntax of string | Trailing_input
  type parse_error = { ctxt : ctxt; problem : problem }
  type Knowledge.conflict += Fail of parse_error

  let fail ctxt problem = Knowledge.fail (Fail { ctxt; problem })
  ...
  let string_of_parse_error { ctxt; problem } =
    sprintf "Syntax error: when parsing rule %s of argument %d - %s" ctxt.rule
      (ctxt.pos + 1)
      (string_of_problem problem)

  let () =
    Knowledge.Conflict.register_printer @@ function
  ...
  let pull_keyword kw inputs =
    let rec loop searched = function
      | [] -> None
      | [ k ] when String.equal k kw && List.is_empty searched -> Some []
      | k :: x :: xs when String.equal k kw ->
          Some (x :: List.rev_append searched xs)
      | x :: xs -> loop (x :: searched) xs
    in
    loop [] inputs
  ...
  let filter_flag kw inputs =
    let rec loop searched = function
      | [] -> None
      | k :: xs when String.equal k kw -> Some (List.rev_append searched xs)
      | x :: xs -> loop (x :: searched) xs
    in
    loop [] inputs
  ...
  let keyword key arg =
    {
      ...
      parse =
        (fun ctxt ->
          ...
          | Some inputs ->
              arg.parse { ctxt with inputs } >>| fun (x, ctxt) -> (Some x, ctxt));
    }
  ...
  let apply_until_exhausted ctxt arg =
    let rec loop rs ctxt =
      match ctxt.inputs with
      | [] -> KB.return (List.rev rs, ctxt)
      | _ -> arg.parse ctxt >>= fun (r, ctxt) -> loop (r :: rs) ctxt
    in
    loop [] ctxt
  ...
  let string =
    argument "<string>" ~parse:parse_string
  ...
  let parse_bitvec ~fail str =
    try !!(Bitvec.of_string str) with Invalid_argument msg -> fail msg

  let bitvec =
    argument "<bitvec>" ~parse:parse_bitvec
    ...
    let apply ({ parse = f } as lhs) ({ parse = x } as rhs) =
      {
        ...
        parse =
          (fun ctxt ->
            ...
            x { ctxt with rule = rhs.rule } >>| fun (x, ctxt) -> (f x, ctxt));
      }
  end
  ...
  let args a = { grammar = [ a.rule ]; run = (fun f -> Arg.map ~f a) }
  let apply ~f args = args.run f

  let run inputs args code =
    let ctxt = { rule = ""; pos = 0; parsed = []; inputs } in
    (apply args ~f:code).parse ctxt >>= fun (f, _ctxt) -> f
...
let () =
  ...
  |> dynamic [ "loader" ] |> require Theory.Label.addr
