...
module Create () = struct
  ...
  let has_verbose = Array.exists ~f:(function "--verbose" | _ -> false)

  let report_progress ?task ?note ?stage ?total () =
    let task =
      match task with
      | None -> name
      | Some subtask -> sprintf "%s/%s" name subtask
    in
    let task =
      if String.(name = main) then task else sprintf "%s/%s" main task
    in
  ...
  let filter_args name =
    ...
    Array.fold (Plugin.argv ()) ~init:([], `drop) ~f:(fun (args, act) arg ->
        ...
        if String.equal arg Sys.argv.(0) then (name :: args, `drop)
          ...
          match (String.chop_prefix arg ~prefix, act) with
  ...
  let argv = if String.equal name main then Sys.argv else filter_args name

  let has_var v =
    match Sys.getenv ("BAP_" ^ String.uppercase v) with
    | exception Stdlib.Not_found -> false
    | "false" | "0" -> false
    | _ -> true

  let is_verbose =
    has_verbose argv || has_var ("DEBUG_" ^ name) || has_var "DEBUG"
  ...
  module Config = struct
    ...
    let converter parser printer default =
      Type.define default
        ~parse:(fun x ->
          match parser x with `Ok x -> x | `Error err -> failwith err)
        ~print:(fun x -> Format.asprintf "%a" printer x)

    let prepend_deprecation dep doc =
      match dep with None -> doc | Some dep -> sprintf "%s %s" dep doc

    let param t ?deprecated ?default:(d = Type.default t) ?as_flag
    ...
    let param_all t ?deprecated ?default ?as_flag ?(docv = Type.name t)
    ...
    let flag ?deprecated ?docv:_ ?(doc = "Undocumented.") ?synonyms name =
      let doc = prepend_deprecation deprecated doc in
      Configuration.flag ~doc ?aliases:synonyms name |> Configuration.determined
