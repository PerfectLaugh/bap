...
module Repr = struct
  ...
  type t = { endian : endian; addr : addr; data : bigstring }
  [@@deriving bin_io]
end
  ...
  Binable.Of_binable_with_uuid
    ...
    (struct
      ...
      let to_binable { endian; addr; data; off; size } : Repr.t =
        { endian; addr; data = Bigstring.sub ~pos:off ~len:size data }

      let of_binable { Repr.endian; addr; data } : t =
        { endian; data; addr; off = 0; size = Bigstring.length data }

      let caller_identity =
        Bin_shape.Uuid.of_string "61de198b-c547-4250-ab5f-80c7d2518159"
    end)
...
(** [create_getters endian addr offset size data] creates a getters class
    according to the specified parameters. All parameters will be encapsulated
    inside a closure (cf, [getter] type). [addr] is a virtual address of the
    byte located at the [offset] from the beginning of the [data] *)
let getter { endian; addr; off; size; data } =
  ...
  let make inj bytes read =
    ...
    let safe ~pos_ref : word or_error =
      ...
      if pos < off then errorf "segfault: addr < min_addr"
      else if pos >= off + size then errorf "segfault: addr > max_addr"
      else if pos + bytes > off + size then
  ...
  let concat_int64X2 =
    make Word.(fun (a, b) -> concat (of_int64 a) (of_int64 b))
  in
  let concat_int64X4 =
    make
      Word.(
        fun (a, b, c, d) ->
          concat
            (concat (of_int64 a) (of_int64 b))
            (concat (of_int64 c) (of_int64 d)))
  in
  let open Bigstring in
  if [%compare.equal: endian] endian BigEndian then
    let get_int64_beX2 t ~pos =
      (unsafe_get_int64_t_be t ~pos, unsafe_get_int64_t_be t ~pos:(pos + 8))
    in
    let get_int64_beX4 t ~pos =
      ( unsafe_get_int64_t_be t ~pos,
    ...
    let get_int64_leX2rev t ~pos =
      (unsafe_get_int64_t_le t ~pos:(pos + 8), unsafe_get_int64_t_le t ~pos)
    in
...
let make_byte mem addr off : t = { mem with addr; off; size = 1 }
...
let create ?(pos = 0) ?len endian addr data : t Or_error.t =
  ...
  let v =
    Validate.(
      name_list "Bap_memory.create"
        [
          name "size"
          @@ Int.validate_bound size ~min:(Excl 0) ~max:(Incl data_len);
          name "pos"
          @@ Int.validate_bound pos ~min:(Incl 0) ~max:(Excl data_len);
          name "pos+size"
          @@ Int.validate_ubound (pos + size) ~max:(Incl data_len);
        ])
  in
  Validate.result v >>= fun () -> return { endian; data; addr; off = pos; size }
...
let first_byte mem : t = make_byte mem mem.addr mem.off
let last_byte mem : t = make_byte mem (max_addr mem) (mem.off + mem.size - 1)
let contains mem = Addr.between ~low:(min_addr mem) ~high:(max_addr mem)

let compare_with mem addr =
  let low = min_addr mem and high = max_addr mem in
  if Addr.between ~low ~high addr then `addr_is_inside
  else if Addr.(addr < low) then `addr_is_below
  else `addr_is_above
...
let view_exn ?(word_size = `r8) ?from ?words t =
  ...
  let amax =
    match words with
    | None -> max_addr t
    | Some w -> Addr.(amin ++ Int.((w * Size.in_bytes word_size) - 1))
  in
  if Word.(amax < amin) then invalid_arg "out-of-bounds"
    ...
    if off >= t.off && off < t.off + t.size then
      match size with
...
let to_buffer { data; off; size } = Bigsubstring.create ~pos:off ~len:size data

let merge m1 m2 =
  let m1, m2 =
    Addr.(if min_addr m1 < min_addr m2 then (m1, m2) else (m2, m1))
  in
  let m1_max = max_addr m1 in
  if Addr.(min_addr m2 > succ m1_max) then errorf "blocks doesn't intersect"
  else if not ([%compare.equal: endian] (endian m1) (endian m2)) then
    errorf "blocks has different sex"
  else if not (phys_equal m1.data m2.data) then
    errorf "blocks doesn't share base"
  else
    let pos = m1.off in
...
let folder step ?(word_size = `r8) t ~(init : 'a) ~f : 'a =
  ...
  let rec loop init =
    if pos_ref.contents <= finish then loop (step base ~pos_ref init f read)
    else init
  in
  loop init
...
let without_address _base ~pos_ref init f read = f (read ~pos_ref) init
let foldi ?word_size t ~init ~f = folder with_address ?word_size t ~init ~f
let fold ?word_size t ~init ~f = folder without_address ?word_size t ~init ~f
...
let iter ?word_size t ~f = fold ?word_size t ~init:() ~f:(fun w () -> f w)
let iteri ?word_size t ~f = foldi ?word_size t ~init:() ~f:(fun a w () -> f a w)

let exists ?word_size t ~f =
  with_return (fun s ->
      iteri ?word_size t ~f:(fun a w -> if f a w then s.return true);
      false)

let for_all ?word_size t ~f =
  with_return (fun s ->
      iteri ?word_size t ~f:(fun a w -> if not (f a w) then s.return false);
      true)
...
let find_map ?word_size t ~f =
  with_return (fun s ->
      iteri ?word_size t ~f:(fun a w ->
          match f a w with None -> () | some -> s.return some);
      None)
...
module Make_iterators (M : Monad.S) = struct
  ...
  let count ?word_size t ~f =
    foldi ?word_size t ~init:0 ~f:(fun a w s ->
        f a w >>| function true -> s + 1 | false -> s)
  ...
  let find_map ?word_size t ~f =
    with_return (fun s ->
        iteri ?word_size t ~f:(fun a w ->
            f a w >>= function
            | None -> return ()
            | some -> s.return (return some))
        >>| fun () -> None)

  let find_if ?word_size t ~f =
    find_map ?word_size t ~f:(fun a w ->
        f a w >>| function true -> Some w | false -> None)
end
...
let pp_hex fmt t =
  let print_char c =
    let c =
      match Char.of_int c with Some c when Char.is_print c -> c | _ -> '.'
    in
  ...
  in
  let chars =
    foldi t ~init:[] ~f:(fun addr char chars ->
        ...
        if newline then [ char ] else char :: chars)
  in
...
module Trie = struct
  ...
  struct
    ...
    module R8 = Trie.Make (Key (struct
      let size = `r8
    end))

    module R16 = Trie.Make (Key (struct
      let size = `r16
    end))

    module R32 = Trie.Make (Key (struct
      let size = `r32
    end))

    module R64 = Trie.Make (Key (struct
      let size = `r64
    end))
  end
...
include Printable.Make (struct
  ...
  let print_word fmt word =
    ...
    fprintf fmt "%0*Lx" width (Word.(to_int64 word) |> ok_exn)
  ...
  let pp fmt t = if length t < 16 then pp_small fmt t else pp_hex fmt t
end)

let hexdump t = Format.asprintf "%a" pp_hex t

let domain =
  KB.Domain.optional "mem"
    ~equal:(fun x y -> Addr.equal x.addr y.addr && Int.equal x.size y.size)
    ~inspect:(fun x -> Sexp.Atom (String.strip (to_string x)))

let slot =
  KB.Class.property ~package:"bap" Theory.Program.cls "mem" domain ~public:true
