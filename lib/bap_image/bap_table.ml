...
module Bound = struct
  type t = Unbound | Bounded of addr * addr [@@deriving sexp_of]

  let empty = Unbound

  let update bound mem =
    match bound with
  ...
  let is_unbound bound mem =
    match bound with
    | Unbound -> true
    | Bounded (x, y) ->
        Addr.(Mem.max_addr mem < x) || Addr.(Mem.min_addr mem > y)
...
type 'a t = { map : 'a map; bound : Bound.t } [@@deriving sexp_of]
...
let empty = { map = Mem.Map.empty; bound = Bound.empty }

let singleton k v =
  { map = Mem.Map.singleton k v; bound = Bound.(update empty k) }
...
let has_intersections tab (x : mem) : bool =
  ...
  | Some _ -> true
  | None -> (
      match prev_key tab.map x with
      | Some (p, _) when intersects p x -> true
      | _ -> (
          match next_key tab.map x with
          | None -> false
          | Some (n, _) -> intersects x n))

(** [left_bound t x ] returns a key to a largest interval in [t] that has
    intersections with [].

    since we can't get the matching key (only lesser or greater ones) we will
    return key [x] itself on a perfect match case. That means, that this
    function shouldn't ever be exposed to a user.

    Otherwise, the only solution (other then just reimplementing our own tree)
    is to sequence all keys and return the head.

    TODO: it looks like that a new `closest_key` function is able to find the
    matching key. So we can reimplement this function more efficiently. *)
let left_bound tab x =
  let rec search_left p =
    match prev_key tab.map p with
    | Some (p, _) when intersects p x -> search_left p
    | _ -> p
  in
  if has_intersections tab x then
    match prev_key tab.map x with
...
let intersections tab (x : mem) : 'a seq =
  ...
  let init = return () in
  let m =
    fold_intersections tab x ~init ~f:(fun addr x gen ->
        gen >>= fun () -> yield (addr, x))
  in
  run m
...
let change tab mem ~f =
  ...
  | (`update _ | `rebind _ | `remove) as cmd ->
      let map =
        Seq.fold ins ~init:tab.map ~f:(fun m (k, _) -> Map.remove m k)
      in
      let map, bound =
        match cmd with
        | `remove -> (map, tab.bound)
        | `update f ->
            Seq.fold ins ~init:(map, tab.bound) ~f:(fun (map, bnd) (mem, x) ->
                (Map.set map ~key:mem ~data:(f (mem, x)), Bound.update bnd mem))
        | `rebind (mem, data) ->
            (Map.set map ~key:mem ~data, Bound.update tab.bound mem)
      in
      { map; bound }

let add tab mem x =
  if has_intersections tab mem then
    error "memory has intersections" mem sexp_of_mem
  else
    Ok
      {
...
let remove tab x =
  ...
  if Map.mem tab.map x then { tab with map = Map.remove tab.map x } else tab
...
let first_some user our = Option.first_some user (Option.map ~f:fst our)

let foldi ?start ?until (tab : 'a t) ~(init : 'b) ~f : 'b =
  let f_labeled = fun ~key ~data s -> f key data s in
  if Option.is_none start && Option.is_none until then
    Map.fold tab.map ~init ~f:f_labeled
  else
    let from = first_some start (Map.min_elt tab.map) in
    let last = first_some until (Map.max_elt tab.map) in
    match (from, last) with
    | Some from, Some last ->
        if Mem.(from <= last) then
          Map.fold_range_inclusive tab.map ~init ~min:from ~max:last
            ~f:f_labeled
        else
          let last, from = (from, last) in
          let seq =
            Map.to_sequence tab.map ~order:`Decreasing_key
              ~keys_less_or_equal_to:from
            |> Seq.take_while ~f:(fun (m, _) -> Mem.(m >= last))
          in
...
let to_sequence ?start ?until tab =
  let open Seq.Generator in
  let m =
    foldi ?start ?until tab ~init:(return ()) ~f:(fun addr v gen ->
        gen >>= fun () -> yield (addr, v))
  in
  run m

let elements ?start ?until tab = to_sequence ?start ?until tab |> Seq.map ~f:snd
let regions ?start ?until tab = to_sequence ?start ?until tab |> Seq.map ~f:fst
let fold ?start ?until tab ~init ~f = foldi tab ~init ~f:(fun _ x s -> f x s)
...
let for_alli ?start ?until tab ~f =
  with_return (fun cc ->
      iteri ?start ?until tab ~f:(fun m x ->
          if not (f m x) then cc.return false);
      true)

let exists ?start ?until tab ~f = existsi ?start ?until tab ~f:(fun _ x -> f x)
...
let find_mapi ?start ?until tab ~f =
  with_return (fun cc ->
      iteri ?start ?until tab ~f:(fun a x ->
          match f a x with Some x -> cc.return (Some x) | None -> ());
      None)
...
(** TODO: this very inefficient implementation uses O(N) time, but we should
    provide this, or users will create their own implementations. Later, this
    should be optimized using interval tree. *)
let find_addr tab (addr : addr) : (mem * 'a) option =
  with_return (fun cc ->
      iteri tab ~f:(fun mem x ->
          match Mem.compare_with mem addr with
...
let make_map map add ?start ?until tab ~f =
  if Option.is_none start && Option.is_none until then
    { bound = tab.bound; map = map tab.map ~f:(fun ~key ~data -> f key data) }
  else
    let map, bound =
      foldi ?start ?until tab ~init:(Mem.Map.empty, Bound.empty)
        ~f:(fun addr x (map, bound) ->
          (add map ~key:addr ~data:(f addr x), Bound.update bound addr))
    in
    { map; bound }

let mapi ?start ?until tab ~f = make_map Map.mapi Map.set ?start ?until tab ~f
let map ?start ?until tab ~f = mapi ?start ?until tab ~f:(fun _ x -> f x)
...
let filter ?start ?until tab ~f = filteri ?start ?until tab ~f:(fun _ x -> f x)

let make_mapping t map =
  let size = Map.length map in
  let table =
    Hashtbl.Using_hashable.create ~growth_allowed:false ~size ~hashable:t ()
  in
...
let link_one t t1 t2 =
  ...
  fun x ->
    ...
    | None ->
        let err =
          Error.create "link_exn: unbound value " x (sexp_of_hashable t)
        in
        Error.raise err
...
let link_many t t1 t2 =
  ...
  fun x -> match Hashtbl.find table x with None -> Seq.empty | Some x -> x
...
let link_at_least_one t t1 t2 =
  ...
  fun x ->
    ...
    | _ ->
        let err =
          Error.create "at_least_one invariant failed.element is not mapped" x
            (sexp_of_hashable t)
        in
        Error.raise err
...
let link : type b c. one_to:(b, c) r -> 'a hashable -> 'a t -> b t -> 'a -> c =
 fun ~one_to:r ->
  match r with
  | One -> link_one
...
let make_rev_map add find t tab =
  ...
  let table =
    Hashtbl.Using_hashable.create ~growth_allowed:false ~hashable:t ~size ()
  in
  Map.iteri tab.map ~f:(fun ~key:mem ~data:x -> add t table ~key:x ~data:mem);
  find t table
...
let add_inj t tab ~key ~data =
  ...
  | `Duplicate ->
      let err =
        Error.create "mapping is non-injective, %s occurs more than once" key
          (sexp_of_hashable t)
      in
      Error.raise err

let add_sur t tab ~key ~data = Hashtbl.add_multi tab ~key ~data

let find_sur t tab key =
  match Hashtbl.find tab key with Some x -> Seq.of_list x | None -> Seq.empty

let find_sur_plus t tab key =
  match Hashtbl.find tab key with
  | Some (x :: xs) -> (x, Seq.of_list xs)
  | Some [] | None ->
      Error.(raise @@ create "value %s is unmapped" key (sexp_of_hashable t))

let rev_map_exn : type c. (mem, c) r -> 'a hashable -> 'a t -> 'a -> c =
  function
...
let pp pp_elem ppf tab =
  let pp_elem ppf (k, v) = Format.fprintf ppf "%a => %a" Mem.pp k pp_elem v in
