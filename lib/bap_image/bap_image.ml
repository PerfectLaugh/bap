...
let backends : loader String.Table.t = String.Table.create ()
...
type location = { addr : addr; size : int } [@@deriving bin_io, compare, sexp]

type 'a region = { name : string; locn : location; info : 'a }
...
type symbol_info = { kinds : kind list; extra_locns : location list }
[@@deriving bin_io, compare, sexp]
...
module Scheme = struct
  ...
  let relocation () = declare "relocation" (scheme fixup $ addr) Tuple.T2.create
...
module Spec = struct
  ...
  let from_arch arch =
    ...
    | Error err ->
        failwithf "got a malformed ogre document: %s" (Error.to_string_hum err)
          ()
    | Ok doc -> doc
end
...
let segment =
  Value.Tag.register
    (module Segment)
    ~name:"segment" ~uuid:"a0eec123-5937-4283-b141-58d579a9b0df"

let symbol =
  Value.Tag.register
    (module String)
    ~name:"symbol" ~uuid:"768bc13d-d4be-43fc-9f7a-c369ebab9c7e"

let function_start =
  Value.Tag.register
    (module Unit)
    ~name:"function-start" ~uuid:"1c1809ec-a38a-4aee-a46c-3d49127ba85a"

let symbol_info =
  Value.Tag.register
    (module Symbol)
    ~name:"symbol-info" ~uuid:"706e78cf-07de-4417-a422-effa7821dd02"

let section =
  Value.Tag.register
    (module String)
    ~name:"section" ~uuid:"4014408d-a3af-488f-865b-5413beaf198e"

let code_region =
  Value.Tag.register
    (module Unit)
    ~name:"code-region" ~uuid:"5296b7e9-d5c3-40ce-98ba-7655ec68fad7"

let file =
  Value.Tag.register
    (module String)
    ~name:"file" ~uuid:"c119f700-4069-47ad-ba99-fc29791e0d47"

let specification =
  Value.Tag.register
    (module Bap_ogre)
    ~name:"image-specification" ~uuid:"a0c98f1f-3693-412a-a11a-2b6c3f6935a7"

let mem_of_locn mem { addr; size } : mem Or_error.t =
  match Memory.view ~from:addr ~words:size mem with
  | Error err ->
      Result.failf "region %a+%d can't be mapped into memory, %a" Addr.pp addr
        size Error.pp err ()
  | ok -> ok

let tag mem tag value memmap = Memmap.add memmap mem (Value.create tag value)

let map_region data { locn = { addr }; info = { off; len; endian } } =
  Memory.create ~pos:off ~len endian addr data

let static_view segments = function
  | { addr } as locn -> (
      match Table.find_addr segments addr with
      | Some (segmem, _) -> mem_of_locn segmem locn
      | None ->
          Result.failf "region %a is not mapped to memory" Sexp.pp_hum
            (sexp_of_location locn) ())
...
let add_sym segments memory (symtab : symtab)
  ...
  Result.List.fold (entry :: locns) ~init:(memory, symtab)
    ~f:(fun (memory, symtab) locn ->
      ...
      let memory = tag mem symbol name memory |> tag mem symbol_info sym in
...
let add_sections_view segments sections memmap =
  List.fold sections ~init:(memmap, []) ~f:(fun (memmap, ers) { name; locn } ->
...
(** [words_of_table word_size table] maps all memory mapped by [table] to words
    of size [word_size]. If size of mapped region is not enough to fit the word,
    then it is ignored. We can ignore errors from [Table.add] since precondition
    that memory regions doesn't overlap is enforced by [Table.fold] and
    [Memory.fold]. *)
let words_of_table word_size tab =
  let words_of_memory mem tab =
    Memory.foldi ~word_size mem ~init:tab ~f:(fun addr word tab ->
        match Memory.view ~word_size ~from:addr ~words:1 mem with
        | Error _ -> tab
        | Ok mem -> ok_exn (Table.add tab mem word))
  in
...
let create_segment_of_symbol_table syms secs =
  let tab =
    Symbol.Table.create () ~growth_allowed:false ~size:(Table.length syms)
  in
  Table.iteri syms ~f:(fun mem sym ->
      match Table.find_addr secs (Memory.min_addr mem) with
      | None -> ()
      | Some (_, sec) -> Hashtbl.add_exn tab ~key:sym ~data:sec);
...
let from_spec query base doc =
  ...
  | { Spec.segments; symbols; sections; code } as spec ->
      ...
      let memory_of_symbol () : symbol -> mem * mem seq =
        ok_exn Table.(rev_map ~one_to:at_least_one Symbol.hashable syms)
      in
      let symbols_of_segment () : segment -> symbol seq =
        Table.(link ~one_to:many Segment.hashable segs syms)
      in
      let segment_of_symbol () : symbol -> segment =
        create_segment_of_symbol_table syms segs
      in
      Result.return
        ( {
            doc;
...
module Derive = struct
  ...
  let arch =
    Fact.collect Ogre.Query.(select (from arch)) >>| Seq.hd >>| function
    | None -> `unknown
    | Some a -> ( match Arch.of_string a with None -> `unknown | Some a -> a)
  ...
  let endian =
    Fact.request is_little_endian >>| function
  ...
  let location ~addr ~size : location Fact.t =
    ...
    int_of_int64 size >>| fun size -> { addr; size }
      ...
      Ogre.Query.(
        select
          ...
          ~join:
            [
              [ field addr ];
              [ field size ~from:segment; field size ~from:named_region ];
            ])
      ~f:(fun
  ...
  let sections =
    Fact.foreach
      Ogre.Query.(
        select
          (from section $ named_region)
          ~join:
            [
              [ field addr ];
              [ field size ~from:section; field size ~from:named_region ];
            ])
      ~f:(fun { addr; size } { info = name } ->
        location ~addr ~size >>| fun locn -> { locn; name; info = () })
    >>= Fact.Seq.all

  let symbol_locations start =
    Fact.foreach
      Ogre.Query.(
        select ~where:(symbol_chunk.(root) = int start) (from symbol_chunk))
      ~f:(fun { addr; size } -> location ~addr ~size)
    >>= Fact.Seq.all
  ...
  let symbols =
    Fact.foreach
      Ogre.Query.(select ~join:[ [ field addr ] ] (from code_start))
      ~f:(fun start ->
                  ...
                  (Some
                     {
                       name;
                       locn;
                       info = { kinds = [ `Code ]; extra_locns = locns };
                     })))
  ...
  let code =
    ...
    Fact.foreach
      Ogre.Query.(
        select (from code_region $ named_region) ~join:[ [ field addr ] ])
      ~f:(fun (addr, size, off) { info = name } ->
        location ~addr ~size >>= fun locn ->
        int_of_int64 off >>= fun off ->
        int_of_int64 size >>| fun len ->
        {
          locn;
          name;
          info = { endian; off; len; r = true; w = false; x = true };
        })
    >>= Fact.Seq.all
...
module Legacy = struct
  ...
  let location_repr { Location.addr = x; len } = (addr x, Int64.of_int len)

  let vsize secs { Segment.name; location = { Location.addr; len } } =
    List.find_map secs
      ~f:(fun { Section.name = n; location = { Location.addr = a; len } } ->
        if String.equal name n && Addr.equal addr a then Some len else None)
    |> function
    | None -> len
    | Some len -> len
  ...
  let provide_symbol s =
    if Symbol.is_function s then provide_function s else Fact.return ()

  let provide_image { Img.arch = a; entry; segments = s, ss; sections; symbols }
      =
...
let register_backend ~name load =
  let module Loader = struct
    let from_data input =
      ...
      | None ->
          Result.failf "A legacy %s loader was unable to process input" name ()
      | Some img -> (
          match Fact.exec (Legacy.provide_image img) Ogre.Doc.empty with
          | Error err ->
              Result.failf "A legacy %s loader failed to provide data - %a" name
                Error.pp err ()
          | Ok spec -> Ok (Some spec))

    let from_file filename = from_data (Bap_fileutils.readfile filename)
  end in
  if Hashtbl.mem backends name then `Duplicate
  else (
    register_loader ~name (module Loader);
    `Ok)

module Metaloader () = struct
  let merge_docs d1 d2 =
    match Ogre.Doc.merge d1 d2 with
    | Ok d3 -> Ok d3
    | Error _ ->
        if Ogre.Doc.declarations d1 > Ogre.Doc.declarations d2 then Ok d1
        else if Ogre.Doc.definitions d1 > Ogre.Doc.definitions d2 then Ok d1
        else Ok d2
...
let get_loader = function
  | None -> merge_loaders ()
  | Some name when Sys.file_exists name && Fn.non Filename.is_implicit name ->
      let local _ =
        match Ogre.Doc.from_file name with
        | Error err -> Error err
        | Ok doc -> Ok (Some doc)
      in
      (module struct
  ...
  | Some name -> (
      match Hashtbl.find backends name with
      | Some loader -> loader
      | None ->
...
let invoke load data arg =
  match load arg with
  | Ok (Some doc) -> from_spec (data arg) doc
  | Ok None ->
      Result.failf "input file format is not supported by the specified loader"
        ()
  | Error _ as err -> err

let result_with_filename file (t, warns) = ({ t with name = Some file }, warns)
