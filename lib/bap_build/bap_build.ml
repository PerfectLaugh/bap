module Plugin_rules = struct
  ...
  let is_installed pkg =
    try
      ignore (Fl.package_directory pkg);
      true
    with Fl.No_such_package _ -> false

  let default_packages =
    List.filter ~f:is_installed
      [
        "bap";
        "bap-main";
        "bap-core-theory";
  ...
  let infer_thread_predicates ~predicates pkg =
    if needs_threads ~predicates pkg then "mt" :: "mt_posix" :: predicates
    else predicates
  ...
  let interns () =
    topological_closure
      ~predicates:(bap_predicates ~native:true)
      default_packages

  let findlibs ?(native = true) ?(predicates = pkg_predicates ~native) ~dynamic
      pkg =
    try
      let preds = if dynamic then "plugin" :: predicates else predicates in
      let arch, preds = Fl.package_property_2 preds pkg "archive" in
      let base = Fl.package_directory pkg in
      if dynamic && not (List.mem ~set:preds (`Pred "plugin")) then
        raise Not_found;
      String.split_on_char ' ' arch |> List.map ~f:(Fl.resolve_path ~base)
    with Not_found -> []
  ...
  let link_shared_bytecode ~src ~dst =
    Cmd (S [ !Options.ocamlc; A "-linkall"; A "-a"; P src; A "-o"; Px dst ])
  ...
  let bundle env =
    ...
    in
    let provides =
      Sys.readdir Pathname.current_dir_name
      |> Array.to_list |> List.filter ~f:is_cmx
      |> List.map ~f:Filename.chop_extension
      |> make_list_option "-provides"
    in
    Cmd
      (S
         [
  ...
  let register_cmxs_of_cmxa_rule () =
    rule "bap: cmxa & a -> cmxs" ~prods:[ "%.cmxs" ]
      ~deps:[ "%.cmxa"; "%" -.- !Options.ext_lib ]
      (fun env _ -> link_shared_native ~src:(env "%.cmxa") ~dst:(env "%.cmxs"))

  let register_collect_bundle_rule () =
    rule "bap: cmxs & packages -> bundle" ~deps:[ "%.cmxs" ] ~stamp:"%.bundle"
      (fun _ _ -> Seq (generate_plugins_for_packages ()))

  let register_plugin_rule () =
    rule "bap: cmxs & cma & bundle -> plugin" ~prods:[ "%.plugin" ]
      ~deps:[ "%.bundle"; "%.cmxs"; "%.cma" ] (fun env _ ->
        Seq [ bundle env; symlink env ])
