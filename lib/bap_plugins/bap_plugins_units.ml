...
module Findlib = struct
  ...
  let has_meta_information pkg =
    try
      ignore (Findlib.package_meta_file pkg : string);
      true
    with Findlib.No_such_package _ -> false

  let report_missing_meta pkg =
    failwithf
      "Package %s was used to build the host program, but its meta information \
       is not available at runtime. To be able to use plugins without \
       installed META files, either update the version of OCaml to 4.08 or \
       newer or provide them at the compilation time via the `used_MOD` \
       predicates for each `MOD` linked into the\n\
      \    host binary."
      pkg ()

  (* this function requires working Findlib infrastructure. *)
  let unit_of_package ~findlib_is_required pkg =
    if findlib_is_required && not (has_meta_information pkg) then
      report_missing_meta pkg;
    let preds = Findlib.recorded_predicates () in
    try Findlib.package_property preds pkg "archive" |> Filename.chop_extension
    with _ -> pkg
  ...
  let init () =
    if not (Hashtbl.is_empty units) then
      failwith "the plugin system is already initialized";
    extract_units_from_predicates ();
    extract_units_from_packages ~findlib_is_required:(Hashtbl.is_empty units)

  let record name reason = Hashtbl.add_exn units ~key:name ~data:reason
...
module Dynlink = struct
  ...
  let init_dynlink () = try Dynlink.loadfile "" with _ -> ()
  ...
  let record name reason =
    match Hashtbl.add units ~key:name ~data:reason with
    | `Ok -> ()
    | `Duplicate ->
        failwithf "bap-plugins: internal error - the unit %s is already loaded"
          name ()
  ...
  let handle_error name reason = function
    | Dynlink.Module_already_loaded _ ->
        ...
        Ok ()
    | other -> Or_error.error_string (Dynlink.error_message other)
end

let is_toplevel = Stdlib.Sys.interactive.contents

module Make () =
  (val if is_toplevel then (module Findlib : S) else (module Dynlink : S))
