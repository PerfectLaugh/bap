...
let requires str = try Some (sscanf str "requires = %S" Fn.id) with _ -> None
let parse_meta path = In_channel.read_lines path |> List.find_map ~f:requires
...
module Plugins = struct
  ...
  let load_all () = list () |> List.iter ~f:load
end
