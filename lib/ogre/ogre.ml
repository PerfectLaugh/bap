...
type entry = { fields : string String.Map.t } [@@deriving bin_io, compare, sexp]
...
module Type = struct
  ...
  type 'a t = { parse : string -> 'a option; pack : 'a -> string; typ : typ }
  type 'a field = { t : 'a t; name : string }

  type header = { fname : string; ftype : typ }
  ...
  type ('f, 'k) scheme = {
    ...
    constraint 'f = 'a -> 'b constraint 'k = 'c -> 'd

  type ('r, 'a, 'b) sealed = ('k, 'f) scheme
    constraint 'f = 'a -> 'r constraint 'k = 'b -> entry

  let pack sexp x = Sexp.to_string (sexp x)
  let atom parse s = try Some (parse (Sexp.Atom s)) with exn -> None
  let header_equal x y = compare_header x y = 0
  let int = { parse = atom int64_of_sexp; pack = pack sexp_of_int64; typ = Int }
  let bool = { parse = atom bool_of_sexp; pack = pack sexp_of_bool; typ = Bool }
  let str = { parse = atom string_of_sexp; pack = Fn.id; typ = Str }

  let float =
    { parse = atom float_of_sexp; pack = pack sexp_of_float; typ = Float }
  ...
  let scheme field =
    {
      ...
      read =
        (fun ent k ->
          match get ent field with None -> None | Some x -> Some (k x));
    }

  let ( $ ) { read; save; signature } fld =
    {
      signature = signature @ [ { fname = fld.name; ftype = fld.t.typ } ];
      save = (fun k -> save (fun ent x -> k (add fld x ent)));
      read =
        (fun ent k ->
          match read ent k with
          | None -> None
          | Some k -> (
              match get ent fld with None -> None | Some x -> Some (k x)));
    }
  ...
  let signature_mismatch ~expect:s1 ~got:s2 =
    Or_error.errorf
      "Failed to parse an OGRE document, got a signature mismatch. Expected:\n\
       %s\n\
       Got:\n\
       %s\n"
...
module Attribute = struct
  ...
  let declare ~name { Type.signature; read; save } cons =
    { name; sign = signature; read = (fun entry -> read entry cons); save }
...
module Doc = struct
  ...
  let empty = { scheme = String.Map.empty; entries = String.Map.empty }
  ...
  let update_scheme name sign doc =
    ...
    | _ -> Ok { doc with scheme = Map.set doc.scheme ~key:name ~data:sign }
  ...
  let typecheck_entry name { fields } { scheme } =
    ...
    | Some sign ->
        Error.List.fold sign ~init:0 ~f:(fun fnum { Type.fname; ftype } ->
            ...
            | None ->
                errorf "attribute %S didn't provide a value for the field %S"
                  name fname
            | Some value when Type.check ftype value -> Ok (fnum + 1)
            | Some bad ->
                errorf "value %S is not in a domain of values of field '%s.%s'"
                  bad name fname)
        >>= fun checked ->
        if checked = Map.length fields then Ok ()
        else
          errorf
            "attribute %S has an arity %d, while a value with arity %d was \
             provided"
            name checked (Map.length fields)

  let normalize_entry name { fields } { scheme } =
    let sign = Map.find_exn scheme name in
    Map.mapi fields ~f:(fun ~key ~data ->
        let { Type.ftype } =
          List.find_exn sign ~f:(fun { Type.fname } -> String.equal fname key)
        in
  ...
  let update_entries name packed doc =
    ...
    let entries =
      Map.update doc.entries name ~f:(function
        | None -> [ packed ]
        | Some data ->
            if List.mem ~equal data packed then data else packed :: data)
    in
    { doc with entries }
  ...
  let put k attr =
    ...
    save (fun packed ->
        k (fun doc ->
            Or_error.(
              update_scheme name sign doc >>= update_entries name packed)))
  ...
  let parse_fname name =
    if
      Char.is_alpha name.[0]
      && String.for_all name ~f:(fun c -> Char.is_alphanum c || Char.(c = '-'))
    then Ok name
  ...
  let parse_fdecl = function
    ...
    | _, Sexp.List [ Sexp.Atom fname; Sexp.Atom tname ] ->
        ...
        parse_tname tname >>| fun ftype -> { Type.ftype; fname }
    | _ ->
        errorf "expected <decl> ::= <field-type> | (<field-name> <field-type>)"
  ...
  let parse_fdefn = function
    ...
    | _ ->
        errorf
          "expected <attribute-value> ::= <field-value> | (<field-name> \
           <field-value>)"
  ...
  let named hdr name x =
    List.find hdr ~f:(fun (_, { Type.fname }) -> String.equal name fname)
    |> function
  ...
  let parse_entry doc = function
    ...
    | Sexp.List (Sexp.Atom name :: flds) ->
        Error.List.map flds ~f:parse_fdefn >>= nest_defn doc.scheme name
        >>= fun fields -> update_entries name { fields } doc
    | _ ->
        errorf
          "expected <ogre-entry> ::= | (declare <attribute-name> \
           <field-decls>)| (<attribute-name> <attribute-value>)"

  let of_sexps = Error.List.fold ~init:empty ~f:parse_entry

  let sexp_of_header { Type.fname; ftype } =
    let tname = Type.string_of_typ ftype in
    if String.for_all fname ~f:Char.is_digit then Sexp.Atom tname
    else Sexp.(List [ Atom fname; Atom tname ])

  let sexp_of_decl (name, s) =
    Sexp.(List (Atom "declare" :: Atom name :: List.map ~f:sexp_of_header s))
  ...
  let sexps_of_value scheme name { fields } =
    ...
    | Some sign ->
        List.map sign ~f:(fun { Type.fname } ->
            ...
            | None ->
                invalid_argf
                  "malformed data base - attribute %s misses field %s" name
                  fname ()
  ...
  let pp_body ppf { scheme; entries } =
    ...
    Map.iteri entries ~f:(fun ~key:n ~data:vs ->
        List.iter vs ~f:(fun v ->
            fprintf ppf "@[%a@]@;" Sexp.pp_hum (sexp_of_attr scheme n v)));
    fprintf ppf "@]"

  let pp ppf t = fprintf ppf "%a@\n%a" pp_scheme t pp_body t

  module Yaml = struct
    let pp_typ ppf typ = pp_print_string ppf (Type.string_of_typ typ)
  ...
  let to_file doc name = Out_channel.with_file name ~f:(save doc)

  let from_string str =
    Or_error.try_with_join ~backtrace:true (fun () ->
        Sexp.scan_sexps (String.strip str |> Lexing.from_string) |> of_sexps)

  let sexps_of_t { scheme; entries } =
    let attrs =
      Map.fold entries ~init:[] ~f:(fun ~key:n ~data:vs sexps ->
          List.fold vs ~init:sexps ~f:(fun sexps v ->
              sexp_of_attr scheme n v :: sexps))
    in
...
type doc = Doc.t
type 'a column = { attr : 'a; field : Type.header } [@@deriving compare, sexp]
...
module Exp = struct
  ...
  let string_of_name = function Name s -> s | Pos n -> sprintf "#%d" n
  let bool = function true -> True | false -> Uop (Not, True)
  ...
  let rec simpl = function
    | Bop (And, Uop (Not, True), _) | Bop (And, _, Uop (Not, True)) ->
        Uop (Not, True)
    | Bop (And, True, e) | Bop (And, e, True) -> simpl e
    | Bop (Or, Uop (Not, True), e) | Bop (Or, e, Uop (Not, True)) -> simpl e
    | Bop (Or, True, _) | Bop (Or, _, True) -> True
    ...
    | Bop (Add, (Int 0L | Flt 0.), x) | Bop (Add, x, (Int 0L | Flt 0.)) ->
        simpl x
    ...
    | Bop (Or, Bop (And, x, y), z) | Bop (Or, z, Bop (And, x, y)) ->
  ...
  let rec n_cross_product xs =
    ...
    | Some (row, rows) ->
        row >>= fun elt -> n_cross_product rows >>| append (singleton elt)

  let has_all_attributes names data = List.for_all names ~f:(Map.mem data)
  ...
  let parsers =
    [
      ...
      parser Type.bool (function true -> True | false -> Uop (Not, True));
    ]
  ...
  let unify_var columns row =
    ...
    |> function
    | None -> True
    | Some (r, es) -> Seq.fold es ~init:True ~f:(fun e p -> Syntax.(e && p = r))
  ...
  let names_index names =
    ...
    function Name s -> Map.find_exn offs s | Pos n -> n
  ...
  let vars_of_join names scheme = function
    ...
    | { attr = None; field } ->
        List.filter_mapi names ~f:(fun pos name ->
            ...
            | None -> None
            | Some sign ->
                if List.mem sign field ~equal:Type.header_equal then
                  Some { attr = Pos pos; field }
                else None)
  ...
  let unify_class names scheme cls =
    ...
    | x :: y :: vs ->
        List.fold vs
          ~init:Syntax.(var x = var y)
          ~f:(fun cs z -> Syntax.(cs && var x = var z))
  ...
  let eval names joins exp { Doc.scheme; entries } : row seq =
    ...
    let exp = normalize_vars names Syntax.(cs && exp) |> simpl |> lambda in
...
module Query = struct
  ...
  type 'f tables = { arity : int; names : string list; read : row -> 'f }
  type 'f t = { where : exp; join : join list list; tables : 'f tables }

  let get_exn ({ row } as x) attr pos =
    let { Attribute.name; read } = attr () in
    match read (Array.get row pos) with
    | None ->
        failwithf "can't parse attribute %s of %s" name
  ...
  let field ?from fld =
    {
      attr =
        Option.map from ~f:(fun attr ->
            let { Attribute.name } = attr () in
            name);
      field = header_of_field fld;
    }

  let select ?(where = Exp.True) ?(join = []) tables = { where; join; tables }
...
module Make (B : Monad.S) = struct
  ...
  let foreach { Query.where; join; tables = { Query.read; names } } ~f :
      'b seq t =
    M.get () >>| fun doc ->
    Sequence.map (Exp.eval names join where doc) ~f:(fun row -> read row f)
  ...
  let require ?(that = fun _ -> true) attr : 'a t =
    ...
    M.get () >>= fun doc ->
    match Doc.get doc attr with
    | Error err -> M.lift (B.return (Error err))
    | Ok [] -> failf "%s is not provided" name ()
    | Ok xs -> (
        match List.filter ~f:that xs with
