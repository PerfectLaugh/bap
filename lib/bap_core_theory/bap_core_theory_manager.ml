...
let joinN p q xs = KB.List.map ~f:KB.return xs >>= fun xs -> p xs ++ q xs
...
let slot =
  Knowledge.Class.property theory "instance" domain ~package:"core"
    ~desc:"The theory structure"

let current =
  Knowledge.Context.declare ~package:"core" "*current-theory*"
    !!(Knowledge.Object.null theory)

module Theory = (val Knowledge.Object.derive theory)

let theories = KB.Context.declare ~package:"core" "*theories*" !!None
...
module Documentation = struct
  module Theory = struct
    ...
    let provides (self, { provides = fs }) = Set.to_list (fs - Empty.name - self)
  end
