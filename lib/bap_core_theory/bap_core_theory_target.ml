...
let sexp_of_mem (Var v) = Sexp.Atom (Format.sprintf "%s" (Var.name v))

type alignment = { code : int; data : int } [@@deriving sexp_of]
...
let mem name k v =
  let k = Bitv.(define k) and v = Bitv.(define v) in
...
let unpack (Var var) =
  ...
  let k = Bitv.size @@ Mem.keys s and v = Bitv.size @@ Mem.vals s in
...
let unknown =
  {
    ...
    aliasing =
      { subs = Map.empty (module Var.Top); sups = Map.empty (module Var.Top) };
...
let targets = Hashtbl.of_alist_exn (module Self) [ (unknown.parent, unknown) ]
...
module Alias = struct
  ...
  let regsize v =
    ...
    | Some s -> Bitv.size s
    | None -> (
        match Val.Bool.refine s with Some _ -> 1 | None -> assert false)

  let init =
    { sups = Map.empty (module Var.Top); subs = Map.empty (module Var.Top) }

  let is_solved { subs; sups } v = Map.mem subs v || Map.mem sups v
  let solved_variables { subs; sups } = Map.length subs + Map.length sups
  ...
  let add_sup sol lhs rhs =
    let rhs = Map.of_alist_exn (module Int) rhs in
    {
      sol with
      sups =
        Map.update sol.sups lhs ~f:(function
          | None -> rhs
          | Some rhs' ->
              Map.merge rhs rhs' ~f:(fun ~key:_ -> function
                | `Left x | `Right x -> Some x
                | `Both (x, y) ->
                    if not (Var.Top.equal x y) then failwith "invalid equation";
                    Some x));
    }
  ...
  let substitute_one sol lhs (n, r) =
    ...
    | Some (m, x) -> [ (n + m, x) ]
    | None -> (
        match Map.find sol.sups r with
        | None -> [ (n, r) ]
        | Some parts ->
            let parts = Map.to_alist parts in
            let partsize = regsize @@ snd (List.hd_exn parts) in
            if regsize lhs > partsize then
              List.map parts ~f:(fun (m, r) -> (m + n, r))
            else
              List.concat_map parts ~f:(fun (m, x) ->
                  if n >= m && n + regsize lhs <= m + regsize x then
                    [ (n - m, x) ]
                  else []))
  ...
  let pp_aliases ppf { subs; sups } =
    ...
    Map.iteri sups ~f:(fun ~key:alias ~data:parts ->
        let parts =
          Map.data parts |> List.rev_map ~f:Var.name |> String.concat ~sep:"."
        in
        Format.fprintf ppf "%s = %s@\n" (Var.name alias) parts)

  let pp_lhs () spec =
    List.map ~f:fst spec |> List.map ~f:Var.name |> String.concat ~sep:" "

  let report_unsolved spec =
    invalid_argf
      "Failed to solve register aliasing. Unable to resolve the following \
       registers: (%a). Please, provide extra constraints or mark them as \
       non-aliases."
      pp_lhs spec ()

  type equation = Var.Top.t * (int * Var.Top.t) list [@@deriving equal]
  type system = equation list [@@deriving equal]

  let no_progress = equal_system
  let has_progress x y = not (no_progress x y)

  let string_of_vars vars =
    Set.to_list vars |> List.map ~f:Var.name |> String.concat ~sep:" "

  let solve regs spec =
    let aliases =
      match Map.find regs Role.Register.alias with
    ...
    let invert sol spec =
      List.concat_map spec ~f:(fun (lhs, rhs) ->
          match rhs with
          | [ (off, rhs) ] when is_base lhs || is_solved sol lhs ->
              [ (rhs, [ (off, lhs) ]) ]
          | [ _ ] -> [ (lhs, rhs) ]
          | rhs -> List.map rhs ~f:(fun (off, sub) -> (sub, [ (off, lhs) ])))
    in
    let reduce spec sol =
    ...
    let rec loop input sol steps =
      ...
      if solved_variables sol = solved then
        match spec with
        | [] -> sol
        | spec when steps > 0 -> loop spec sol (steps - 1)
        | spec -> report_unsolved spec
      else loop spec sol (steps - 1)
    in
    loop spec init 10000

  let error def =
    Format.kasprintf @@ fun details ->
    invalid_argf "%s: bad aliasing defintion of %s - %s" "Theory.Alias.def"
      (Var.name def) details ()
  ...
  let check var = function
    ...
    | parts ->
        ...
        List.iter parts
          ~f:
            (Option.iter ~f:(fun v ->
                 if regsize v <> width then
                   error var "the size of %s must be %d divided by %d"
  ...
  let def var parts : t =
    ...
    ( var,
      List.filter_mapi parts ~f:(fun i -> function
        | None -> None
        | Some reg -> Some (total - (i * width) - width, Var.forget reg)) )
...
module Origin = struct
  ...
  let cast_sub = function Any (Sub _ as x) -> Some x | Any _ -> None
  let cast_sup = function Any (Sup _ as x) -> Some x | Any _ -> None
  let cast_set = function Any (Set _ as x) -> Some x | Any _ -> None
  let reg (Sub { base = v }) = v
  let bit (Sub { base = v }) = v
  let hi (Sub { hi }) = hi
  let lo (Sub { lo }) = lo
  let is_alias (Sub { hi; lo; base }) = hi - lo + 1 = Bitv.size (Var.sort base)
...
let extend parent ?(bits = parent.bits) ?(byte = parent.byte)
    ?(data = unpack @@ parent.data) ?(code = unpack @@ parent.code)
    ?(data_alignment = parent.alignment.data)
    ?(code_alignment = parent.alignment.code) ?vars ?regs ?aliasing
    ?(endianness = parent.endianness) ?(system = parent.system)
    ?(abi = parent.abi) ?(fabi = parent.fabi) ?(filetype = parent.filetype)
    ?(options = parent.options) ?nicknames name =
  let code = pack code
  and data = pack data
  and vars =
    Option.value_map vars ~default:parent.vars ~f:(Set.of_list (module Var.Top))
  and regs = Option.value_map regs ~default:parent.regs ~f:make_roles in
  let aliasing =
    Option.value_map aliasing ~default:parent.aliasing ~f:(Alias.solve regs)
  in
  let ( + ) s (Var v) = Set.add s (Var.forget v) in
  {
    ...
    aliasing;
    options;
    alignment = { code = code_alignment; data = data_alignment };
    names =
      Option.value_map nicknames ~default:parent.names
        ~f:String.Caseless.Set.of_list;
  }

let declare ?(parent = unknown.parent) ?bits ?byte ?data ?code ?data_alignment
    ?code_alignment ?vars ?regs ?aliasing ?endianness ?system ?abi ?fabi
    ?filetype ?options ?nicknames ?package name =
  let t = Self.declare ?package name in
  if Hashtbl.mem targets t then
    failwithf
      "A target with name %s already exists in the package %s, please choose \
       another name or package"
      (Name.unqualified (Self.name t))
      (Name.package (Self.name t))
      ();
  let p = Hashtbl.find_exn targets parent in
  let info =
    extend ?bits ?byte ?data ?code ?data_alignment ?code_alignment ?vars ?regs
      ?aliasing ?endianness ?system ?abi ?fabi ?filetype ?options ?nicknames p
      parent
  in
...
let lookup ?package name =
  try Some (Self.read ?package name) with _exn -> None

let get ?package name =
  match lookup ?package name with
  | None ->
      invalid_argf
        "Unknown target %s. Use `bap list targets' for the list of targets" name
        ()
  | Some t -> t
...
let info name =
  match Hashtbl.find targets name with None -> unknown | Some t -> t
...
let has_role roles var role =
  match Map.find roles role with None -> false | Some vars -> Set.mem vars var

let is_excluded exclude info =
  match exclude with
  | None -> fun _ -> false
  | Some excluded -> fun var -> List.exists excluded ~f:(has_role info.regs var)

let is_included roles info =
  match roles with
  | None -> fun _ -> true
  | Some included ->
...
let regs ?exclude ?roles t =
  let info = info t in
  let pred =
    match (exclude, roles) with
    | None, None -> None
    | _ ->
        Some
          (fun v ->
            is_included roles info v && not (is_excluded exclude info v))
  in
...
let reg ?exclude ?(unique = false) t role =
  ...
  | Some vars -> (
      let vars =
        Set.filter vars ~f:(fun v -> not (is_excluded exclude info v))
      in
...
let unalias t reg : ('a, unit) origin option =
  ...
  let cast refine v =
    Option.map (refine (Var.sort v)) ~f:(fun s -> Var.resort v s)
  in
  ...
  | None -> (
      ...
      | Some parts -> (
          ...
          | None -> (
              match parts Bool.refine with
              | Some parts -> Some Origin.(forget (Set parts))
              | None -> assert false)))

let data_addr_size, code_addr_size =
  let keys v = Bitv.size @@ Mem.keys @@ Var.sort v in
  ((fun t -> keys @@ data t), fun t -> keys @@ code t)
...
let parents target =
  let rec closure ps p =
    if Self.equal unknown.parent p then List.rev (p :: ps)
    else closure (p :: ps) (parent p)
  in
  closure [] (parent target)

let is_unknown c = Self.equal c unknown.parent
let is_known c = not @@ is_unknown c
let rec belongs p c = Self.equal p c || (is_known c && belongs p (parent c))
...
let declared () = Hashtbl.keys targets |> List.filter ~f:is_known

let sort_family_by_order =
  List.sort ~compare:(fun t1 t2 ->
      match order t1 t2 with KB.Order.NC | EQ -> 0 | LT -> -1 | GT -> 1)
...
let family t = declared () |> List.filter ~f:(belongs t) |> sort_family_by_order
...
let partition xs =
  ...
  let rec grandest t =
    let p = parent t in
    if is_known p && Set.mem universe p then grandest p else t
  in
...
let rec matching t name =
  if matches_name t name || Set.mem (info t).names name then Some t
  else if is_known t then matching (parent t) name
  else None

let matches t name = Option.is_some (matching t name)
...
let print_constraints system abi fabi filetype options parent ppf =
  Format.fprintf ppf
    "@[<v2>Constraints:@;\
     ...
     Options:  %a@;\
     @]"
    (pp_option Self.pp) parent (pp_option System.pp) system (pp_option Abi.pp)
    abi (pp_option Fabi.pp) fabi (pp_option Filetype.pp) filetype
    (pp_option Options.pp) options

let report_no_matches system abi fabi filetype options parent =
  invalid_arg
  @@ Format.asprintf "Target.select - no matching target@\n%t"
...
let report_non_unique system abi fabi filetype options parent targets =
  let targets = String.concat ~sep:", " @@ List.map targets ~f:Self.to_string in
  invalid_arg
  @@ Format.asprintf
       "Target.select - more than one target matches the constraints: {%s}@\n%t"
       targets
       (print_constraints system abi fabi filetype options parent)
...
let filter ?(strict = false) ?(parent = Self.unknown) ?(system = System.unknown)
    ?(abi = Abi.unknown) ?(fabi = Fabi.unknown) ?(filetype = Filetype.unknown)
    ?(options = Options.empty) () =
  let order (type t) (module E : Orderable with type t = t) x y =
    if strict then E.equal x y
    else is_greater_or_equal @@ KB.Domain.order E.domain x y
  in
  let result =
    family parent
    |> List.filter ~f:(fun t ->
...
let select ?(unique = false) ?strict ?parent ?system ?abi ?fabi ?filetype
    ?options () =
  filter ?strict ?parent ?system ?abi ?fabi ?filetype ?options () |> function
  | [] -> report_no_matches system abi fabi filetype options parent
  | [ t ] -> t
  | ts when unique ->
      report_non_unique system abi fabi filetype options parent ts
  | t :: _ -> t
...
let are_separators x y = match (x, y) with S _, S _ -> true | _ -> false
...
let field name x = F (KB.Name.unqualified (name x))

let register ?(systems = [ System.unknown ]) ?(abis = [ Abi.unknown ])
    ?(fabis = [ Fabi.unknown ]) ?(filetypes = [ Filetype.unknown ])
    ?(options = [ Options.empty ]) ?package parent =
  ignore
  @@
  ...
  then []
    ...
    let package =
      Option.value package ~default:(KB.Name.package (name parent))
    in
    ...
    let name =
      generate_name
        [
          F base;
          S "-";
          field System.name system;
          S "-";
          field Abi.name abi;
          S "";
          field Fabi.name fabi;
          S "-";
          field Filetype.name filetype;
          S "";
          F (options_to_string options);
        ]
    in
    [ declare ~parent ~package name ~system ~filetype ~abi ~fabi ~options ]
