...
let size = Bitv.size
let ( >>-> ) = fun x f -> x >>= fun x -> f (KB.Class.sort (Value.cls x)) x
...
module Make (L : Minimal) = struct
  ...
  include struct
    ...
    let eq x y =
      x >>= fun x ->
      y >>= fun y -> ule !!x !!y && ule !!y !!x

    let neq x y = not (eq x y)

    let slt x y =
      x >>= fun x ->
      y >>= fun y -> sle !!x !!y && not (sle !!y !!x)

    let ult x y =
      x >>= fun x ->
      y >>= fun y -> ule !!x !!y && not (ule !!y !!x)
  ...
  let is_zero x = x >>-> fun s x -> eq !!x (zero s)
  let non_zero x = inv (is_zero x)
  let nsucc x n = x >>-> fun s x -> add !!x (small s n)
  let npred x n = x >>-> fun s x -> sub !!x (small s n)
  ...
  let bind exp body =
    exp >>-> fun s exp ->
    Var.scoped s @@ fun v -> let_ v !!exp (body v)
  ...
  let loadw out dir mem key =
    ...
    let rec loop chunks loaded =
      ...
      else ite !!dir (concat out (List.rev chunks)) (concat out chunks)
    in
    loop [] 0
  ...
  let storew dir mem key data =
    ...
    let nth stored =
      let shift_amount =
        ite dir (small data_t stored) (small data_t (needed - stored))
      in
      cast chunks b0 (shiftr b0 !!data shift_amount)
    in
    let rec loop key stored mem =
      if stored < needed then
        loop (succ key) (stored + chunk_len) (store mem key (nth stored))
      else mem
    in
    loop key 0 !!mem
