...
(* see IEEE754 3.6 *)
type parameters = { base : int; bias : int; k : int; p : int; w : int; t : int }
...
let prec k =
  let k = float k in
  Int.of_float @@ (k -. round (4. *. log2 k) +. 13.)

let ebits k =
  let k = float k in
  Int.of_float @@ (round (4. *. log2 k) -. 13.)

let bias k p = (2 ^ (k - p - 1)) - 1

let binary k =
  let p = prec k and w = ebits k in
  { base = 2; k; w; p; t = k - w - 1; bias = bias k p }
...
let binary16 = { base = 2; bias = 15; k = 16; p = 11; w = 5; t = 10 }
let binary32 = { base = 2; bias = 127; k = 32; p = 24; w = 8; t = 23 }
let binary80 = { base = 2; bias = 16383; k = 80; p = 64; w = 15; t = 64 }
...
module Sort = struct
  ...
  let format { base; w; t = x; k } =
    Float.Format.define
      Sort.(int base @-> int w @-> int x @-> sym ieee754)
      (Bitv.define k)

  let define p : (('b, 'e, 't) ieee754, 's) Float.format Float.t sort =
    Float.define (format p)

  let spec e =
    let fmt = Float.format e in
    let k = Bitv.size (Float.Format.bits fmt) in
    let base = Sort.(value @@ hd (Float.Format.exp fmt)) in
    match base with 2 -> binary k | 10 -> decimal k | _ -> assert false
...
let decimal n = if n > 0 && n mod 32 = 0 then Some (decimal n) else None
