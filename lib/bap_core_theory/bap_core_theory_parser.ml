...
type ('a, 'e, 'r) bitv_parser =
  (module Grammar.Bitv with type t = 'a and type exp = 'e and type rmode = 'r) ->
  'e ->
  'a
...
type ('a, 'e, 'r) float_parser =
  (module Grammar.Float with type t = 'a and type exp = 'e and type rmode = 'r) ->
  'e ->
  'a
...
let ( >>-> ) x f = x >>= fun x -> f (KB.Class.sort (Value.cls x)) x
...
module Make (S : Core) = struct
  ...
  let mkvar sort name = Var.create sort (Var.Ident.of_string name)
  ...
  let rec expw : type s b e r. context -> (e, r, b) parser -> e -> s bitv =
   fun ctxt self ->
    self.bitv
      (module struct
        ...
        let append x y =
          ...
          x >>-> fun sx x ->
          y >>-> fun sy y -> append (join sx sy) !!x !!y

        let cast rs bit x = cast (bits rs) (expb bit) (expw x)

        let concat xs =
          Knowledge.List.fold ~init:([], 0) xs ~f:(fun (xs, s) x ->
              expw x >>| fun x ->
              (!!x :: xs, s + Bitv.size (KB.Class.sort (Value.cls x))))
          >>= fun (xs, sz) -> concat (bits sz) (List.rev xs)
        ...
        let extract sz hi lo x = extract (bits sz) (expw hi) (expw lo) (expw x)
  ...
  and expm : type k x b e r. context -> (e, r, b) parser -> e -> (k, x) mem =
   fun ctxt self ->
    self.mem
      (module struct
        open Knowledge.Syntax
        ...
        let store_word d m k x = storew (expb d) (expm m) (expw k) (expw x)
  ...
  and expb : type s b e r. context -> (e, r, b) parser -> e -> bool =
   fun ctxt self ->
    self.bool
      (module struct
        open Knowledge.Syntax
  ...
  and expf : type s b e r k n i g a.
      context -> (e, r, b) parser -> e -> ((i, g, a) IEEE754.t, s) format float
      =
   fun ctxt self ->
    self.float
      (module struct
        ...
        let ieee754_cast s m x = cast_float (floats s) (expr m) (expw x)
        let ieee754_cast_signed s m x = cast_sfloat (floats s) (expr m) (expw x)
        let ieee754_convert s m x = fconvert (floats s) (expr m) (expf x)
  ...
  and expr : type b e r. context -> (e, r, b) parser -> r -> rmode =
   fun _ctxt self ->
    self.rmode
      (module struct
        type nonrec t = rmode
  ...
  and stmts : type e s r. context -> (e, r, s) parser -> s list -> unit eff =
   fun ctxt self -> function
    | [] -> blk null pass skip
    | x :: xs ->
        self.stmt
          (module struct
            ...
            let error = Knowledge.fail Error
            let special _ = seq (blk null pass skip) (next xs)
            ...
            let while_ cnd ys =
              seq
                (blk null
                   (repeat (expb ctxt self cnd) (stmtd ctxt self ys))
                   skip)
                (next xs)

            let if_ cnd yes nay =
              seq
                (branch (expb ctxt self cnd) (bil ctxt self yes)
                   (bil ctxt self nay))
                (next xs)
            ...
            let call name =
              ...
              Knowledge.provide Label.is_subroutine dst (Some true)
              >>= fun () -> seq (blk null pass (goto dst)) (next xs)

            let goto addr =
              Label.for_addr addr >>= fun dst ->
              seq (blk null pass (goto dst)) (next xs)

            let move eff = seq (blk null eff skip) (next xs)
            ...
            let let_gen t var exp body =
              seq
                (bind (t ctxt self exp) (fun r ->
                     stmts ((var, Var.ident r) :: ctxt) self [ body ]))
                (next xs)
  ...
  and set_bit : type e s r.
      context -> (e, r, s) parser -> string -> e -> data eff =
   fun ctxt self v x -> set (mkvar bool v) (expb ctxt self x)

  and set_reg : type e s r.
      context -> (e, r, s) parser -> string -> int -> e -> data eff =
   fun ctxt self v s x -> set (mkvar (bits s) v) (expw ctxt self x)

  and set_mem : type e s r.
      context -> (e, r, s) parser -> string -> int -> int -> e -> data eff =
  ...
  and set_ieee754 : type e s r.
      ...
      e ->
      data eff =
   fun ctxt self v fs x ->
    set (mkvar (IEEE754.Sort.define fs) v) (expf ctxt self x)

  and set_rmode : type e s r.
      context -> (e, r, s) parser -> string -> r -> data eff =
   fun ctxt self v x -> set (mkvar Rmode.t v) (expr ctxt self x)

  and stmtd : type e s r. context -> (e, r, s) parser -> s list -> data eff =
   fun ctxt self -> function
    | [] -> pass
    | x :: xs ->
        self.stmt
          (module struct
            ...
            let bind exp body =
              exp >>-> fun s exp ->
              Var.fresh s >>= fun v -> seq (set v !!exp) (body v)
            ...
            let while_ cnd ys =
              seq (repeat (expb ctxt self cnd) (next ys)) (next xs)

            let if_ cnd yes nay =
              seq
                (branch (expb ctxt self cnd) (stmtd ctxt self yes)
                   (stmtd ctxt self nay))
                (next xs)
            ...
            let let_gen t var exp body =
              seq
                (bind (t ctxt self exp) (fun r ->
                     stmtd ((var, Var.ident r) :: ctxt) self [ body ]))
                (next xs)
