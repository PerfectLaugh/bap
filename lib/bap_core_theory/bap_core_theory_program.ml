...
let (cls : (cls, unit) Knowledge.cls) =
  Knowledge.Class.declare ~package "program" () ~public:true
    ~desc:"a class of program instances"

let program = cls

let word =
  Knowledge.Domain.optional "word" ~equal:Bitvec.equal
    ~inspect:Bitvec_sexp.sexp_of_t

let hexcode =
  Knowledge.Domain.optional "hexcode" ~equal:String.equal
    ~inspect:
...
let path =
  Knowledge.Domain.optional "path" ~equal:String.equal ~inspect:sexp_of_string

let format =
  Knowledge.Domain.optional "path" ~equal:String.equal ~inspect:sexp_of_string

let names =
  Knowledge.Domain.powerset (module String) "names" ~inspect:sexp_of_string

let name_choices =
  Knowledge.Domain.opinions ~empty:None
...
module Source = struct
  ...
  let language =
    Knowledge.Class.property cls ~package "source-language" Language.domain
      ~persistent:Language.persistent ~public:true
      ~desc:"the language of the unit's source code"

  let code =
    Knowledge.Class.property cls ~package "source-code" Knowledge.Domain.string
      ~persistent:Knowledge.Persistent.string ~public:true
      ~desc:"the units source code text"

  let file =
    string_property ~domain:path cls "source-path"
...
module Compiler = struct
  ...
  let create ?(specs = []) ?(version = []) ?(options = []) name =
    { name; version; options; specs = String.Map.of_alist_exn specs }
  ...
  let domain = Knowledge.Domain.optional ~inspect:sexp_of_t ~equal "compiler"
...
module Unit = struct
  ...
  let cls : (cls, unit) Knowledge.cls =
    Knowledge.Class.declare ~package "unit" () ~public:true
      ~desc:"a unit of code"

  let path =
    string_property ~domain:path cls "unit-path"
      ~desc:"a filesystem name of the file that contains the program"

  let bias =
    Knowledge.Class.property ~package cls "unit-bias" word
      ~persistent:
        (Knowledge.Persistent.of_binable
           (module struct
             type t = Bitvec_binprot.t option [@@deriving bin_io]
           end))
      ~public:true ~desc:"the value by which all addresses of the unit a biased"

  let for_file name =
    Knowledge.Symbol.intern ~package:"file" name cls ~public:true >>= fun obj ->
    Knowledge.provide path obj (Some name) >>| fun () -> obj

  let for_region ~lower ~upper =
    let to_symbol addr =
      Knowledge.Symbol.intern (Bitvec.to_string addr) program
      >>= Knowledge.Object.repr program
    in
  ...
  let target =
    Knowledge.Class.property ~package cls "unit-target" Target.domain
      ~persistent:Target.persistent ~public:true
      ~desc:"the target system for the unit"

  let source =
    Knowledge.Class.property ~package cls "unit-source" Source.domain
      ~persistent:Source.persistent ~public:true ~desc:"the source of the unit"

  let compiler =
    Knowledge.Class.property ~package cls "unit-compiler" Compiler.domain
      ~persistent:Compiler.persistent ~public:true
...
module Label = struct
  let int =
    Knowledge.Domain.optional "ivec" ~equal:Int.equal ~inspect:sexp_of_int

  let unit =
    Knowledge.Domain.optional "unit" ~equal:Unit.equal ~inspect:Unit.sexp_of_t

  let attr name desc =
    let bool_t =
      Knowledge.Domain.optional ~inspect:sexp_of_bool ~equal:Bool.equal "bool"
    in
  ...
  let is_valid = attr "is-valid" "is the program valid or not"

  let is_subroutine =
    attr "is-subroutine" "is the program a subroutine entry point"

  let encoding =
    Knowledge.Class.property ~package cls "encoding" Language.domain
      ~persistent:Language.persistent ~public:true
      ~desc:"the language of the code"

  let unit =
    Knowledge.Class.property ~package cls "label-unit" unit
      ~persistent:
        (Knowledge.Persistent.of_binable
           (module struct
             type t = Unit.t option [@@deriving bin_io]
           end))
      ~public:true ~desc:"the program unit"

  let addr =
    Knowledge.Class.property ~package cls "label-addr" word
      ~persistent:
        (Knowledge.Persistent.of_binable
           (module struct
             type t = Bitvec_binprot.t option [@@deriving bin_io]
           end))
      ~public:true ~desc:"the program virtual address"

  let name = string_property cls "label-name" ~desc:"the program linkage name"

  let possible_name =
    Knowledge.Class.property ~package cls "possible-name" name_choices
      ~public:true ~desc:"a unique name associated with the program"
  ...
  let ivec =
    Knowledge.Class.property ~package cls "label-ivec" int
      ...
      ~public:true ~desc:"the program interrupt vector"
  ...
  let aliases =
    Knowledge.Class.property ~package cls "label-aliases" names
      ...
      ~public:true ~desc:"the set of known program names"
...
module Semantics = struct
  ...
  let slot =
    Knowledge.Class.property ~package program "semantics" Self.domain
      ~persistent:(Knowledge.Persistent.of_binable (module Self))
      ~public:true ~desc:"the program semantics"

  let value =
    Knowledge.Class.property ~package cls "value" Value.Top.domain
      ~persistent:(Knowledge.Persistent.of_binable (module Value.Top))
      ~public:true ~desc:"the program semantics"

  let code =
    Knowledge.Class.property ~package cls "insn-code" hexcode
      ~persistent:
        (Knowledge.Persistent.of_binable
           (module struct
             type t = string option [@@deriving bin_io]
           end))
      ~public:true ~desc:"the program memory contents"
