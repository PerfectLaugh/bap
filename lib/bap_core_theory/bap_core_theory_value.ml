...
module Sort : sig
  ...
  let cls =
    KB.Class.declare ~package:"core" "value" () ~public:true
  ...
  module Name = struct
    ...
    let declare ?(package = "user") name =
      ...
      if Hash_set.mem names key then
        failwithf
          "Type name `%s' is already defined for package `%s'. Please, pick a \
           unique name or a different package."
          name package ();
    ...
    let to_string name =
      ...
      if Hashtbl.find_exn short_names short = 1 then short
  ...
  let int s = Int s
  let ( @-> ) = app
  let name = function Sym s -> s | _ -> assert false
  let value = function Int s -> s | _ -> assert false
  let hd = function App { args = x :: _ } -> x | _ -> assert false

  let tl = function
    | App { args = _ :: xs; name } -> App { args = xs; name }
    | _ -> assert false
  ...
  let rec pp ppf = function
    ...
    | App { args = xs } ->
        let f, args =
          ...
          (List.hd_exn sx, List.(rev @@ tl_exn sx))
        in
        fprintf ppf "%a(%a)" pp f (pp_print_list ~pp_sep pp) args

  let forget = Fn.id

  let refine witness t =
    match t with
    | (App { name = Some name } | Sym name)
      when [%compare.equal: name] name witness ->
        Some t
    | _ -> None
    ...
    include
      Binable.Of_binable_with_uuid
        ...
        (struct
          ...
          let caller_identity =
            Bin_shape.Uuid.of_string "7afb196e-fb95-47d9-892d-043460269e1b"
        end)
...
let resort : ('a sort -> 'b sort option) -> 'a t -> 'b t option =
 fun refine v -> Option.(refine (sort v) >>| KB.Value.refine v)

let forget : 'a t -> unit t =
 fun v -> KB.Value.refine v @@ Sort.forget @@ sort v
...
module Match : sig
  ...
  let can cast x action k =
    match resort cast x with None -> k () | Some x -> action x
