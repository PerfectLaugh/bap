...
let valid_first_char = function '0' .. '9' | '#' | '$' -> false | _ -> true
...
let mangle_if_necessary name =
  ...
  else if not (valid_first_char name.[0]) then "_" ^ name
  else name
...
let pp_ver ppf = function 0 -> () | n -> fprintf ppf ".%d" n

let pp_ident ppf ident =
  match ident with
  | Reg { name; ver } -> Format.fprintf ppf "%s%a" name pp_ver ver
  | Let { num } -> Format.fprintf ppf "$%a" Int63.pp num
  | Var { num; ver } -> Format.fprintf ppf "#%a%a" Int63.pp num pp_ver ver

let pp ppf (_, v) = Format.fprintf ppf "%a" pp_ident v
let name (_, v) = Format.asprintf "%a" pp_ident v
let ident (_, v) = v
let sort (s, _) = s
let is_virtual v = match ident v with Let _ | Var _ -> true | Reg _ -> false
let is_mutable v = match ident v with Let _ -> false | Reg _ | Var _ -> true

let nat1 =
  Knowledge.Domain.total "nat1" ~empty:0 ~inspect:sexp_of_int ~order:Int.compare

let versioned (s, v) ver =
  match v with
  | Let _ -> (s, v)
...
let version v =
  match ident v with Let _ -> 0 | Reg { ver } | Var { ver } -> ver
...
let scoped : 'a sort -> ('a t -> 'b pure) -> 'b pure =
 fun s f ->
...
module Ident = struct
  ...
  let num s =
    try Int63.of_string s
    with _ -> failwithf "`%s' is not a valid temporary value" s ()

  let split_version s =
    match String.Escaping.rindex s ~escape_char '.' with
    | None -> (s, 0)
    | Some n ->
        (String.subo ~len:n s, Int.of_string (String.subo ~pos:(n + 1) s))

  let sub1 = String.subo ~pos:1

  let of_string x =
    let n = String.length x in
    if n = 0 then invalid_arg "a variable identifier can't be empty";
    match x.[0] with
    | '$' -> Let { num = num (sub1 x) }
    | '#' ->
