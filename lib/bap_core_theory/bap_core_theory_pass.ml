...
let no_such_pass name =
  invalid_argf "Unknown core theory pass %s" (KB.Name.show name) ()
...
let lookup name =
  match Hashtbl.find passes name with None -> no_such_pass name | Some p -> p
...
module Scope = struct
  let vars =
    KB.Context.declare ~package:"core" "desugar-scope-vars"
      !!(Map.empty (module Var.Ident))

  let update slot obj f = KB.collect slot obj >>| f >>= KB.provide slot obj

  let push var =
    KB.Context.update vars @@ fun vars ->
    Map.update vars (Var.ident var) ~f:(function None -> 1 | Some n -> n + 1)
  ...
  let mem var = KB.Context.get vars >>| fun vars -> Map.mem vars (Var.ident var)
end

module Desugar (CT : Core) : Core = struct
  module Delta = struct
    let target = KB.Object.scoped Program.cls Program.Label.target
    let pass = Effect.empty Effect.Sort.bot
    let take what bits x = what (Val.Bitv.define bits) (CT.var x)
    ...
    let assign_sub dst src off =
      ...
      | _, _ ->
          CT.concat s [ take CT.high ulen dst; !!src; take CT.low off dst ]
    ...
    let set v x =
      ...
      | false -> (
          let* t = target in
          if Target.has_roles t [ Target.Role.Register.constant ] v then !!pass
          else
            match Target.unalias t v with
            | None -> CT.set v x
            | Some origin -> (
                let x = cast_val x in
                match Origin.cast_sub origin with
                | Some s when Origin.is_alias s -> CT.set (Origin.reg s) x
                | Some s -> assign_sub (Origin.reg s) x (Origin.lo s)
                | None -> (
                    match Origin.cast_sup origin with
    ...
    let const role value ret t r s =
      if Target.has_roles t [ role ] r then
        match Val.Bitv.refine (Val.Sort.forget s) with
        | None -> Some (CT.unk s)
          ...
          | None -> (
              ...
              | Some origin -> (
                  ...
                  | Some sub ->
                      ...
                      ret
                      @@ CT.extract bs (pos hi) (pos lo)
                           (CT.var (Origin.reg sub))
                  | None -> (
                      match Origin.cast_sup origin with
                      | Some sup ->
                          ...
                          let total =
                            List.sum
                              (module Int)
                              regs
                              ~f:(fun r -> Val.Bitv.size (Var.sort r))
                          in
...
let () =
  register "desugar-variables"
    (module Desugar)
    ~package:"core" ~desc:"desugars assignments and access to register aliases"
