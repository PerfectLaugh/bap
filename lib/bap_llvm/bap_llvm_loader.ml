...
module Primitive = struct
  external llvm_load : Bigstring.t -> string -> string = "bap_llvm_load_stub"
  ...
  let () = Callback.register_exception "Llvm_loader_fail" (Llvm_loader_fail 0)
end
...
module LLVM = struct
  ...
  (** entry point *)
  let entry_point () = Ogre.declare ~name:"llvm:entry-point" (scheme addr) Fn.id
  ...
  let segment_cmd () =
    Ogre.declare ~name:"llvm:segment-command"
      (scheme name $ off $ size)
      Tuple.T3.create
  ...
  let macho_symbol () =
    Ogre.declare ~name:"llvm:macho-symbol" (scheme name $ value) Tuple.T2.create

  (** elf program header as it is in file *)
  let elf_program_header () =
    Ogre.declare ~name:"llvm:elf-program-header"
      (scheme name $ off $ size)
      Tuple.T3.create
...
let foreach query f = Ogre.foreach query ~f >>= Ogre.Seq.iter ~f:Ogre.sequence
...
let provide_if cond code = if cond then code else []
...
let provide_base_and_bias ?file new_base =
  ...
  | Some b ->
      ...
      Ogre.sequence
        [ Ogre.provide bias b; Ogre.provide base_address Int64.(real + b) ]
  | None -> (
      Ogre.sequence
      @@
      match new_base with
      | None -> [ Ogre.provide bias 0L; Ogre.provide base_address real ]
      | Some base ->
          [
  ...
  provide_if
    ...
    [
      ...
      ( Ogre.request LLVM.code_entry ~that:(fun (n, o, s) ->
            Poly.(o = off && n = name && s = size))
      >>= fun entry ->
        if Option.is_some entry then Ogre.provide code_start addr
        else Ogre.return () );
    ]
  @ [
      Ogre.provide symbol_value addr value; Ogre.provide named_symbol addr name;
    ]

let provide_generic_sections =
  iter_rows LLVM.section_entry @@ fun bias (name, addr, size, _) ->
  let addr = Int64.(addr + bias) in
  [ Ogre.provide section addr size; Ogre.provide named_region addr size name ]
...
let provide_generic_code_regions =
  foreach
    Ogre.Query.(
      select
        (from named_region $ LLVM.code_entry)
        ~join:
          [
            [ field name ];
            [ field size ~from:named_region; field size ~from:LLVM.code_entry ];
          ])
  @@ fun { addr; size } (_, off, _) ->
...
let provide_macho_segments =
  ...
  foreach_by_name
    Ogre.Query.(
      from LLVM.segment_cmd $ LLVM.segment_cmd_flags $ LLVM.virtual_segment_cmd)
...
let derive_code_regions_of_segments =
  ...
  foreach_by_name
    Ogre.Query.(
      from LLVM.elf_program_header
      $ LLVM.elf_virtual_program_header $ LLVM.elf_program_header_flags)
  @@ fun (_, off, size) (_, addr, _) (_, _, _, _, x) ->
  provide_if x [ Ogre.provide code_region Int64.(addr + bias) size off ]
...
let map_sections_to_segments =
  Ogre.require bias >>= fun bias ->
  foreach_by_name Ogre.Query.(from LLVM.section_entry $ LLVM.section_flags)
...
let use_when_missing missing_entry derivers =
  ...
  if Seq.is_empty entries then Ogre.sequence derivers else Ogre.return ()

let derive_missing_sections =
  use_when_missing LLVM.section_entry
    [ derive_code_regions_of_segments; map_segments_to_sections ]

let derive_missing_segments =
  use_when_missing segment [ map_sections_to_segments ]

let derive_missing =
  Ogre.sequence [ derive_missing_sections; derive_missing_segments ]
...
let provide_coff_segmentation =
  [
    ( Ogre.require bias >>= fun bias ->
      foreach_by_name
        Ogre.Query.(
          from LLVM.section_entry $ LLVM.coff_virtual_section_header
          $ LLVM.section_flags)
      ...
      @ provide_if x [ Ogre.provide code_region addr vsize start ] );
  ]

let overload file_type info =
  Ogre.require format >>= fun file ->
  Ogre.sequence @@ provide_if (String.equal file file_type) info

module ElfDyn = struct
  type dyn = { tag : word; off : word }
  type img = { data : Bigstring.t; endian : Word.endian; width : int }

  let required attr k =
    Ogre.request attr >>= function None -> Ogre.return () | Some x -> k x
  ...
  let section_by_name { data; endian; width } sec_name =
    ...
    | None -> None
    | Some (_, addr, len, pos) -> (
        let len = Int64.to_int_trunc len and pos = Int64.to_int_trunc pos in
  ...
  let dynamic_contents img =
    ...
    | None -> []
    | Some mem -> (
        Memory.fold ~word_size:(word_size img) mem
          ~f:(fun data (acc, dyn) ->
            match dyn with
            | None -> (acc, Some data)
  ...
  let read ~strtab pos =
    ...
    let rec loop off =
      if Char.equal (Bigsubstring.get strtab off) '\x00' then
        Bigsubstring.sub strtab ~pos ~len:(off - pos)
      else loop (off + 1)
    in
...
let pdb_path ~pdb filename =
  if Sys.file_exists pdb then
    if Sys.is_directory pdb then
      let pdb_file =
        sprintf "%s.pdb" Filename.(remove_extension @@ basename filename)
      in
...
let load_doc ~pdb filename data =
  ...
  with Primitive.Llvm_loader_fail n -> (
    match n with
...
let map_file path =
  ...
  with exn ->
    Unix.close fd;
    Or_error.errorf "unable to process file %s: %s" path (Exn.to_string exn)
...
let init ?base ?(pdb_path = Sys.getcwd ()) () =
  Image.KB.register_loader ~name:"llvm"
    (module struct
