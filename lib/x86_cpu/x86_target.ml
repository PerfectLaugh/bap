...
module M16 = struct
  let main = [ reg r16 "AX"; reg r16 "BX"; reg r16 "CX"; reg r16 "DX" ]
  let lregs = [ reg r8 "AL"; reg r8 "BL"; reg r8 "CL"; reg r8 "DL" ]
  let hregs = [ reg r8 "AH"; reg r8 "BH"; reg r8 "CH"; reg r8 "DH" ]
  let index = [ reg r16 "SI"; reg r16 "DI"; reg r16 "BP"; reg r16 "SP" ]
  let index8 = [ reg r8 "SIL"; reg r8 "DIL"; reg r8 "BPL"; reg r8 "SPL" ]
  let segment = [ reg r16 "CS"; reg r16 "DS"; reg r16 "ES"; reg r16 "SS" ]
  let cf = reg bool "CF"
  let pf = reg bool "PF"
  let af = reg bool "AF"
  let zf = reg bool "ZF"
  ...
  let aliasing =
    lo_bits_of main are lregs @ hi_bits_of main are hregs
...
module M32 = struct
  let main = [ reg r32 "EAX"; reg r32 "EBX"; reg r32 "ECX"; reg r32 "EDX" ]
  let index = [ reg r32 "ESI"; reg r32 "EDI"; reg r32 "EBP"; reg r32 "ESP" ]
  ...
  let i486regs =
    i386regs @ Theory.Role.Register.[ ([ general; floating ], untyped stx) ]

  let i586regs =
    i486regs @ Theory.Role.Register.[ ([ general; floating ], untyped mmx) ]

  let i686regs =
    i586regs @ Theory.Role.Register.[ ([ general; floating ], untyped xmmx) ]

  let aliasing =
    M16.aliasing @ lo_bits_of main are M16.main @ lo_bits_of index are M16.index
end

module M64 = struct
  let main = [ reg r64 "RAX"; reg r64 "RBX"; reg r64 "RCX"; reg r64 "RDX" ]
  let index = [ reg r64 "RSI"; reg r64 "RDI"; reg r64 "RBP"; reg r64 "RSP" ]
  let segment = [ reg r16 "CS"; reg r16 "DS"; reg r16 "ES"; reg r16 "SS" ]
  let rx = array r64 "R" 8 ~index:(fun i -> string_of_int (i + 8))
  let stx = M32.stx
  let mmx = M32.mmx
  ...
  let vars =
    main @< index @< segment @< rx @< stx @< mmx @< ymmx @< flags @< [ data ]
    @< [ mxsr ]
...
let parent = Theory.Target.declare ~package "x86" ~bits:16 ~byte:8

let i86 =
  Theory.Target.declare ~package "i86" ~parent ~nicknames:[ "8086" ]
    ~data:M16.data ~code:M16.data ~vars:M16.vars ~regs:M16.regs
    ~endianness:Theory.Endianness.le

let i186 =
  Theory.Target.declare ~package "i186" ~parent:i86
    ~nicknames:[ "80186"; "186" ]

let i286 =
  Theory.Target.declare ~package "i286" ~parent:i186
    ~nicknames:[ "80286"; "286" ]

let i386 =
  Theory.Target.declare ~package "i386" ~parent:i286
    ~nicknames:[ "386"; "80386" ] ~bits:32 ~data:M32.data ~code:M32.data
    ~vars:M32.i386 ~regs:M32.i386regs ~aliasing:M32.aliasing

let i486 =
  Theory.Target.declare ~package "i486" ~parent:i386
    ~nicknames:[ "486"; "80486" ] ~vars:M32.i486 ~regs:M32.i486regs
    ~aliasing:M32.aliasing

let i586 =
  Theory.Target.declare ~package "i586" ~parent:i486
    ~nicknames:[ "586"; "80586"; "p5" ] ~vars:M32.i586 ~regs:M32.i586regs
    ~aliasing:M32.aliasing

let i686 =
  Theory.Target.declare ~package "i686" ~parent:i586
    ~nicknames:[ "686"; "80686"; "p6" ] ~vars:M32.i686 ~regs:M32.i686regs
    ~aliasing:M32.aliasing

let amd64 =
  Theory.Target.declare ~package "x86_64" ~parent:i686
    ~nicknames:[ "amd64"; "x64"; "x86-64" ]
    ~bits:64 ~data:M64.data ~code:M64.data ~vars:M64.vars ~regs:M64.regs
...
module Abi = struct
  ...
  let is_integer = any C.Type.[ is_integer; is_pointer; is_function ]
  let is_composite = any C.Type.[ is_structure; is_union ]
  ...
  let ia16 memory t =
    ...
    let return ~alignment:_ _ = choose [ Arg.registers irets; memory ] in
    ...
    let return ~alignment:_ size =
      select
        [
          ...
          (is (size > 64), combine [ Arg.reference irets; Arg.hidden ]);
  ...
  let fastcall t =
    ...
    let pass arg =
      ...
      select
        [
          (any [ is_big size; C.Type.is_floating ], Arg.memory);
          (otherwise, choose [ Arg.register iregs; Arg.memory ]);
        ]
        arg
    in
    override ~pass ()
  ...
  let watcomregs t =
    ...
    let pass arg = Arg.choice [ Arg.register iregs arg; Arg.memory arg ] in
    override ~pass ()
  ...
  let ms64 t =
    ...
    let iregs = (iregs, [ vregs ])
    and vregs = (vregs, [ iregs ])
    and irets = (irets, [])
    and vrets = (vrets, []) in
    let use pass (arena, coarena) arg =
      Arg.sequence [ pass arena arg; Arg.List.iter coarena ~f:Arg.discard ]
    in
    let pass how arena = choose [ use how arena; Arg.memory ] in
    let return ~alignment:_ size =
      select
        [
    ...
    in
    describe ~return @@ fun ~alignment:_ size ->
    select
      [
  ...
  let partition fields =
    List.fold fields ~init:(`Nil, 0, []) ~f:(fun (k, s, words) (k', s') ->
        if s + s' <= 64 then (merge_kinds k k', s + s', words)
        else (k', s', k :: words))
  ...
  let rec classify : C.Type.t -> [ `Nil | `Int | `Sse ] = function
    ...
    | `Structure { t = { fields } } | `Union { t = { fields } } ->
        classify_fields fields
    | `Array { t = { element } } -> classify element
    | _ -> `Nil

  and classify_fields fields =
    List.fold ~init:`Nil fields ~f:(fun k (_, t) -> merge_kinds k (classify t))
  ...
  let sysv t =
    ...
    let union_fields size fields =
      Arg.return [ (classify_fields fields, size) ]
    in
    let rec fields { C.Type.Compound.fields = xs } =
      Arg.List.fold ~init:(0, []) xs ~f:(fun (off, acc) (_, t) ->
          ...
          let off = off + size in
          match t with
          | t when is_integer t -> Arg.return (off, [ (`Int, size) ] :: acc)
          ...
          | _ -> Arg.reject ())
      >>| fun (_, acc) -> List.rev acc |> List.concat
    in
    ...
    in
    describe ~return @@ fun ~alignment:_ bits ->
    select
      [
  ...
  let calling_conventions =
    [
      ...
      ( i286,
        [
          (Abi.cdecl, cdecl16); (Abi.pascal, pascal16); (Abi.fortran, pascal16);
        ] );
      (* 32-bit ABI  *)
      ( i386,
        [
      ...
      (* 64-bit ABI *)
      (amd64, [ (Abi.ms, ms64); (Abi.sysv, sysv) ]);
    ]
  ...
  let default_calling_conventions =
    [
      ([ i86 ], cdecl16); ([ i386; i486; i586; i686 ], cdecl); ([ amd64 ], sysv);
    ]
  ...
  let install_demanglers () =
    List.iter demanglers ~f:(fun (abi, filetype, demangler) ->
        ...
        |> List.iter ~f:(fun target -> Demanglers.install target demangler))
...
let enable_loader ~abi () =
  ...
  KB.Rule.(
    declare ~package "x86-target"
    |> require Image.Spec.slot |> provide Theory.Unit.target
    |> comment "computes target from the OGRE specification");

  let make_target parent abi' filetype =
    let abi = if Theory.Abi.is_unknown abi then abi' else abi in
    Theory.Target.select ~strict:true ~parent ~abi ~filetype ()
  in
  ...
  let request =
    ...
    Ogre.request Image.Scheme.format >>= fun fmt -> Ogre.return (arch, bits, fmt)
  in
  ...
  KB.collect Image.Spec.slot unit >>| get_info >>| fun (arch, bits, fmt) ->
...
let enable_arch () =
  let open KB.Syntax in
  KB.Rule.(
    declare ~package "x86-arch"
    |> require Theory.Unit.target |> provide Arch.unit_slot
    |> comment "computes Arch.t from the unit's target");
  KB.promise Arch.unit_slot @@ fun unit ->
  KB.collect Theory.Unit.target unit >>| fun t ->
  if Theory.Target.belongs amd64 t then `x86_64
  else if Theory.Target.belongs i386 t then `x86
  else `unknown

let llvm_x86_encoding = Theory.Language.declare ~package "llvm-x86"
let llvm_x86_64_encoding = Theory.Language.declare ~package "llvm-x86_64"
let pcode = Theory.Language.declare ~package "pcode-x86"
...
let register_sleigh_disassembler () =
  ...
  let target =
    if Theory.Target.belongs amd64 target then "x86:LE:64:default"
    else "x86:LE:32:default"
  in
  Disasm_expert.Basic.create ~backend:"ghidra" target

let unit_encoding =
  KB.Class.property ~persistent:Theory.Language.persistent ~package:"bap"
...
let compute_unit_encoding backend =
  ...
  if Theory.Target.belongs parent t then
    if String.equal backend "llvm" then
      if Theory.Target.belongs amd64 t then llvm_x86_64_encoding
      else if Theory.Target.belongs parent t then llvm_x86_encoding
