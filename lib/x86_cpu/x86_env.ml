...
let gv mode { v32; v64 } = match mode with X86 -> v32 | X8664 -> v64
...
let ymms =
  Array.init 16 ~f:(fun i -> Var.create (Printf.sprintf "YMM%d" i) reg256_t)
...
module type ModeVars = sig
  ...
  (* r8 -> r15 *)
  val r : var array
  val nums : var array [@@deprecated "[since 2018-01] use `r` instead"]
  val ymms : var array
end
...
module R64 = struct
  ...
  (* r8 -> r15 *)
  let r =
    Array.init 8 ~f:(fun i -> Var.create (Printf.sprintf "R%d" (i + 8)) reg64_t)
...
let vars_of_mode mode =
  let module R =
    (val match mode with X86 -> (module R32) | X8664 -> (module R64)
        : ModeVars)
  in
  (module R : ModeVars)
