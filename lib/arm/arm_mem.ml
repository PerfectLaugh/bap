...
let lift_r ~(dst1 : Var.t) ?(dst2 : Var.t option) ~(base : Var.t)
  ...
  let address =
    ...
    | PreIndex, _, _, _ | Offset, _, _, _ -> Bil.(var base + offset)
  in
  ...
  let pre_write_back =
    ...
    | PreIndex, Ld, W, d when is_pc d ->
        [
          Bil.move o_base Bil.(var base); Bil.move base Bil.(var base + offset);
        ]
  ...
  let write_back =
    ...
    | PostIndex, Ld, W, d when is_pc d -> []
    | Offset, _, _, _ -> []
    | _ -> [ Bil.move base Bil.(var base + offset) ]
  in

  let typ = match size with B -> `r8 | H -> `r16 | W | D -> `r32 in

  let store mem addr v = Bil.(store ~mem ~addr v LittleEndian typ) in
  let load mem addr = Bil.(load ~mem ~addr LittleEndian typ) in

  let temp = match size with B -> tmp reg8_t | H -> tmp reg16_t | _ -> dst1 in
  ...
  | Ld ->
      ...
      let extend =
        match size with B | H -> [ Bil.move dst1 rhs ] | W | D -> []
      in
      let loads =
        ...
        else [ assn temp (load mem address) ]
      in
      List.concat
        [
          pre_write_back;
      ...
      let stores =
        ...
        | D ->
            [
              ...
              Bil.move m
                (store v Bil.(address + four) Bil.(var (Option.value_exn dst2)));
            ]
        | B | H | W -> [ Bil.move m (store v address Bil.(var temp)) ]
      in
      List.concat
        [ trunc; (* truncate the value if necessary *) stores; write_back ]
...
let lift_m dest_list base mode update operation =
  ...
  in
  let writeback =
    let dest_len = Word.of_int ~width:32 (4 * List.length dest_list) in
    match update with
    | NoUpdate -> []
    | Update ->
        let ( +- ) =
          match mode with IB | IA -> Bil.( + ) | DB | DA -> Bil.( - )
        in
        [ Bil.move base Bil.(var base +- int dest_len) ]
  in
  let create_access i dest =
    ...
    | St ->
        Bil.move Env.mem
          Bil.(store ~mem:Env.(var mem) ~addr (var dest) LittleEndian `r32)
  in
