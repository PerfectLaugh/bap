...
let repair_imm (src : word) ~sign_mask ~imm_mask rtype : exp =
  let bit_set = Word.(Z.(word sign_mask land src) = word sign_mask) in
  let negate =
...
let repair_reg reg imm ~sign_mask rtype =
  let bit_set = Word.(Z.(word sign_mask land imm) = word sign_mask) in
  let negate =
    (bit_set && [%compare.equal: repair] rtype `NEG)
    || ((not bit_set) && [%compare.equal: repair] rtype `POS)
  in
...
let lift_r_op ~dest1 ?dest2 ?shift ~base ~offset mode sign size operation =
  ...
  let (offset : exp) =
    ...
    | `Imm w ->
        ...
        if Word.(w = min_32) then Bil.(int Word.(zero width)) else Bil.(int w)
  in
  ...
  | `Reg (#gpr_reg as d1), Some (`Reg (#gpr_reg as d2)) ->
      Mem.lift_r ~dst1:(Env.of_reg d1) ~dst2:(Env.of_reg d2) ~base ~offset mode
        sign size operation
  | `Reg (#gpr_reg as d), None ->
      Mem.lift_r ~dst1:(Env.of_reg d) ~base ~offset mode sign size operation
  | op1, op2 ->
      fail [%here] "Unexpected arguments: %s, %s"
...
let lift_r_exp ~dest1 ?dest2 ~base ~offset mode sign size operation =
  ...
  | Some dest2 ->
      ...
      Mem.lift_r ~dst1:dest1 ~dst2:dest2 ~base ~offset mode sign size operation
  | None -> Mem.lift_r ~dst1:dest1 ~base ~offset mode sign size operation

let lift_m dest_list base mode update operation =
  let base = assert_reg [%here] base in
  let dest_list =
    List.map dest_list ~f:(fun d -> assert_reg [%here] d |> Env.of_reg)
  in
...
let mem_offset_reg_or_imm_neg reg_off imm_off =
  match reg_off with
  | `Reg #nil_reg -> repair_imm imm_off ~sign_mask:0x100 ~imm_mask:0xff `NEG
  | `Reg (#gpr_reg as reg) ->
