...
let lift_move ~encoding mem ops (insn : move_insn) : stmt list =
  ...
  | `MVNsr, [| dest; src; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src `MVN ~sreg:shift_reg ~simm:shift_imm mem cond
        ~wflag
  ...
  | `ANDrsr, [| dest; src1; src2; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `AND ~sreg:shift_reg ~simm:shift_imm mem
        cond ~wflag
  | `ANDrsi, [| dest; src1; src2; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `AND ~simm:shift_imm mem cond ~wflag
  ...
  | `BICrsr, [| dest; src1; src2; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `BIC ~sreg:shift_reg ~simm:shift_imm mem
        cond ~wflag
  | `BICrsi, [| dest; src1; src2; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `BIC ~simm:shift_imm mem cond ~wflag
  ...
  | `EORrsr, [| dest; src1; src2; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `EOR ~sreg:shift_reg ~simm:shift_imm mem
        cond ~wflag
  | `EORrsi, [| dest; src1; src2; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `EOR ~simm:shift_imm mem cond ~wflag
  ...
  | `ORRrsr, [| dest; src1; src2; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `ORR ~sreg:shift_reg ~simm:shift_imm mem
        cond ~wflag
  | `ORRrsi, [| dest; src1; src2; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `ORR ~simm:shift_imm mem cond ~wflag
  | `TEQri, [| src1; src2; cond; _ |] | `TEQrr, [| src1; src2; cond; _ |] ->
      lift ~encoding src1 ~src2 `EOR mem cond ~wflag:(`Reg `CPSR)
  | `TEQrsr, [| src1; src2; shift_reg; shift_imm; cond; _ |] ->
      lift ~encoding src1 ~src2 `EOR ~sreg:shift_reg ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  | `TEQrsi, [| _dest; src1; src2; shift_imm; cond; _ |] ->
      lift ~encoding src1 ~src2 `EOR ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  | `TSTri, [| src1; src2; cond; _ |] | `TSTrr, [| src1; src2; cond; _ |] ->
      lift ~encoding src1 ~src2 `AND mem cond ~wflag:(`Reg `CPSR)
  | `TSTrsr, [| src1; src2; shift_reg; shift_imm; cond; _ |] ->
      lift ~encoding src1 ~src2 `AND ~sreg:shift_reg ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  | `TSTrsi, [| src1; src2; shift_imm; cond; _ |] ->
      lift ~encoding src1 ~src2 `AND ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  ...
  | `ADDrsr, [| dest; src1; src2; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `ADD ~sreg:shift_reg ~simm:shift_imm mem
        cond ~wflag
  | `ADDrsi, [| dest; src1; src2; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `ADD ~simm:shift_imm mem cond ~wflag
  ...
  | `SUBrsr, [| dest; src1; src2; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `SUB ~sreg:shift_reg ~simm:shift_imm mem
        cond ~wflag
  | `SUBrsi, [| dest; src1; src2; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `SUB ~simm:shift_imm mem cond ~wflag
  ...
  | `ADCrsr, [| dest; src1; src2; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `ADC ~sreg:shift_reg ~simm:shift_imm mem
        cond ~wflag
  | `ADCrsi, [| dest; src1; src2; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `ADC ~simm:shift_imm mem cond ~wflag
  ...
  | `SBCrsr, [| dest; src1; src2; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `SBC ~sreg:shift_reg ~simm:shift_imm mem
        cond ~wflag
  | `SBCrsi, [| dest; src1; src2; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `SBC ~simm:shift_imm mem cond ~wflag
  ...
  | `RSBrsr, [| dest; src1; src2; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `RSB ~sreg:shift_reg ~simm:shift_imm mem
        cond ~wflag
  | `RSBrsi, [| dest; src1; src2; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `RSB ~simm:shift_imm mem cond ~wflag
  ...
  | `RSCrsr, [| dest; src1; src2; shift_reg; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `RSC ~sreg:shift_reg ~simm:shift_imm mem
        cond ~wflag
  | `RSCrsi, [| dest; src1; src2; shift_imm; cond; _; wflag |] ->
      lift ~encoding ~dest src1 ~src2 `RSC ~simm:shift_imm mem cond ~wflag
  | `CMPri, [| src1; src2; cond; _ |] | `CMPrr, [| src1; src2; cond; _ |] ->
      lift ~encoding src1 ~src2 `SUB mem cond ~wflag:(`Reg `CPSR)
  | `CMPrsr, [| src1; src2; shift_reg; shift_imm; cond; _ |] ->
      lift ~encoding src1 ~src2 `SUB ~sreg:shift_reg ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  | `CMPrsi, [| src1; src2; shift_imm; cond; _ |] ->
      lift ~encoding src1 ~src2 `SUB ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  | `CMNri, [| src1; src2; cond; _ |] | `CMNzrr, [| src1; src2; cond; _ |] ->
      lift ~encoding src1 ~src2 `ADD mem cond ~wflag:(`Reg `CPSR)
  | `CMNzrsr, [| src1; src2; shift_reg; shift_imm; cond; _ |] ->
      lift ~encoding src1 ~src2 `ADD ~sreg:shift_reg ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  | `CMNzrsi, [| src1; src2; shift_imm; cond; _ |] ->
      lift ~encoding src1 ~src2 `ADD ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  ...
  | insn, ops ->
      fail [%here] "ops %s doesn't match move insn %s" (string_of_ops ops)
        (Arm_insn.to_string (insn :> insn))
...
let lift_bits mem ops (insn : bits_insn) =
      ...
      exec
        Bil.
          [
            ...
            Env.mem :=
              store ~mem:(var Env.mem) ~addr:src2 (extract ~hi:7 ~lo:0 src1)
                LittleEndian `r8;
  ...
  | `PKHTB, [| `Reg dest; src1; src2; shift; cond; _ |] ->
      (* shift is always asr *)
      let shifted, _ =
        Shift.lift_c ~src:(exp_of_op src2) `ASR ~shift:(exp_of_op shift) reg32_t
      in
      exec
        [
          assn (Env.of_reg dest)
            Bil.(
              extract ~hi:31 ~lo:16 (exp_of_op src1)
  ...
  | `REV, [| `Reg dest; src; cond; _ |] ->
      ...
      let rev =
        Bil.(
          extract ~hi:7 ~lo:0 s ^ extract ~hi:15 ~lo:8 s
          ^ extract ~hi:23 ~lo:16 s ^ extract ~hi:31 ~lo:24 s)
      in
  ...
  | `REV16, [| `Reg dest; src; cond; _ |] ->
      ...
      exec [ assn (Env.of_reg dest) rev ] cond
  | insn, ops ->
      fail [%here] "ops %s doesn't match bits insn %s" (string_of_ops ops)
        (Arm_insn.to_string (insn :> insn))
...
let lift_mult ops insn =
  ...
  | `UMLAL, [| lodest; hidest; src1; src2; _loadd; _hiadd; cond; _; wflag |]
  | `UMLAL, [| lodest; hidest; src1; src2; _loadd; _hiadd; cond; wflag |] ->
      lift_mull ~lodest ~hidest ~src1 ~src2 Unsigned ~addend:true ~wflag cond
  | `SMLAL, [| lodest; hidest; src1; src2; _loadd; _hiadd; cond; _; wflag |]
  | `SMLAL, [| lodest; hidest; src1; src2; _loadd; _hiadd; cond; wflag |] ->
  ...
  | insn, ops ->
      fail [%here] "ops %s doesn't match mult insn %s" (string_of_ops ops)
        (Arm_insn.to_string (insn :> insn))

let lift_mem_multi ops insn =
  match (insn, Array.to_list ops) with
  | `STMDA, base :: cond :: _wr_flag :: dest_list ->
      let insns = Mem_shift.lift_m (List.rev dest_list) base DA NoUpdate St in
      exec insns cond
  | `STMDA_UPD, base :: _unknown :: cond :: _wr_flag :: dest_list ->
      let insns = Mem_shift.lift_m (List.rev dest_list) base DA Update St in
      exec insns cond
  ...
  | `LDMDB, base :: cond :: _wr_flag :: dest_list ->
      let insns = Mem_shift.lift_m (List.rev dest_list) base DB NoUpdate Ld in
      exec insns cond
  | `LDMDB_UPD, base :: _unknown :: cond :: _wr_flag :: dest_list ->
      let insns = Mem_shift.lift_m (List.rev dest_list) base DB Update Ld in
      exec insns cond
  | `STMDB, base :: cond :: _wr_flag :: dest_list ->
      let insns = Mem_shift.lift_m (List.rev dest_list) base DB NoUpdate St in
      exec insns cond
  | `STMDB_UPD, base :: _unknown :: cond :: _wr_flag :: dest_list ->
      let insns = Mem_shift.lift_m (List.rev dest_list) base DB Update St in
      exec insns cond
  ...
  | `LDMDA, base :: cond :: _wr_flag :: dest_list ->
      let insns = Mem_shift.lift_m (List.rev dest_list) base DA NoUpdate Ld in
      exec insns cond
  | `LDMDA_UPD, base :: _unknown :: cond :: _wr_flag :: dest_list ->
      let insns = Mem_shift.lift_m (List.rev dest_list) base DA Update Ld in
      exec insns cond
  | _ ->
      fail [%here] "ops %s doesn't match multi arg insn %s" (string_of_ops ops)
        (Arm_insn.to_string (insn :> insn))
...
let lift_mem ops insn =
  ...
  | `STRD, [| dest1; dest2; base; reg_off; `Imm imm_off; cond; _ |] ->
      ...
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~dest2 ~base ~offset Offset Unsigned D St
      in
      exec insns cond
  | `LDRD, [| dest1; dest2; base; reg_off; `Imm imm_off; cond; _ |] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~dest2 ~base ~offset Offset Unsigned D Ld
      in
      exec insns cond
  | ( `STRD_POST,
      [| dest1; dest2; base; _unknown; reg_off; `Imm imm_off; cond; _ |] ) ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~dest2 ~base ~offset PostIndex Unsigned D St
      in
      exec insns cond
  | ( `LDRD_POST,
      [| dest1; dest2; base; _unknown; reg_off; `Imm imm_off; cond; _ |] ) ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~dest2 ~base ~offset PostIndex Unsigned D Ld
      in
      exec insns cond
  | ( `STRD_PRE,
      [| _unknown; dest1; dest2; base; reg_off; `Imm imm_off; cond; _ |] ) ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~dest2 ~base ~offset PreIndex Unsigned D St
      in
      exec insns cond
  | ( `LDRD_PRE,
      [| dest1; dest2; _unknown; base; reg_off; `Imm imm_off; cond; _ |] ) ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~dest2 ~base ~offset PreIndex Unsigned D Ld
      in
      exec insns cond
  | `STRH, [| dest1; base; reg_off; `Imm imm_off; cond; _ |] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset Offset Unsigned H St
      in
      exec insns cond
  | `LDRH, [| dest1; base; reg_off; `Imm imm_off; cond; _ |] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset Offset Unsigned H Ld
      in
      exec insns cond
  | `STRH_PRE, [| _unknown; dest1; base; reg_off; `Imm imm_off; cond; _ |] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PreIndex Unsigned H St
      in
      exec insns cond
  | `LDRH_PRE, [| dest1; _unknown; base; reg_off; `Imm imm_off; cond; _ |] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PreIndex Unsigned H Ld
      in
      exec insns cond
  | `STRH_POST, [| _unknown; dest1; base; reg_off; `Imm imm_off; cond; _ |] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PostIndex Unsigned H St
      in
      exec insns cond
  ...
  | `STRrs, [| dest1; base; offset; shift; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset ~shift Offset Unsigned W St
      in
      exec insns cond
  | `LDRrs, [| dest1; base; offset; shift; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset ~shift Offset Unsigned W Ld
      in
      exec insns cond
  | `STRBrs, [| dest1; base; offset; shift; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset ~shift Offset Unsigned B St
      in
      exec insns cond
  | `LDRBrs, [| dest1; base; offset; shift; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset ~shift Offset Unsigned B Ld
      in
      exec insns cond
  ...
  | `STR_PRE_REG, [| _unknown; dest1; base; offset; shift; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset ~shift PreIndex Unsigned W St
      in
      exec insns cond
  | `LDR_PRE_REG, [| dest1; _unknown; base; offset; shift; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset ~shift PreIndex Unsigned W Ld
      in
      exec insns cond
  | `STRB_PRE_REG, [| _unknown; dest1; base; offset; shift; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset ~shift PreIndex Unsigned B St
      in
      exec insns cond
  | `LDRB_PRE_REG, [| dest1; _unknown; base; offset; shift; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset ~shift PreIndex Unsigned B Ld
      in
      exec insns cond
  (* Exclusive access, we may later want to do something special to these *)
  | `LDREX, [| dest1; base; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base
          ~offset:(`Imm (word 0))
  ...
  | `LDREX, [| dest1; `Reg _ as dest2; base; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~dest2 ~base
          ~offset:(`Imm (word 0))
          Offset Unsigned W Ld
      in
      exec insns cond
  ...
  | `LDREXD, [| multidest; base; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1:multidest ~base
          ~offset:(`Imm (word 0))
  ...
  | `STREX, [| `Reg dest1; src1; base; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1:src1 ~base
          ~offset:(`Imm (word 0))
  ...
  | `STREX, [| `Reg dest1; src1; `Reg _ as src2; base; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1:src1 ~dest2:src2 ~base
          ~offset:(`Imm (word 0))
          Offset Unsigned W St
      in
      let result = [ Bil.move (Env.of_reg dest1) (int32 0) ] in
  ...
  | `STREXD, [| `Reg dest1; multisrc; base; cond; _ |] ->
      let insns =
        Mem_shift.lift_r_op ~dest1:multisrc ~base
          ~offset:(`Imm (word 0))
  ...
  | insn, ops ->
      fail [%here] "ops %s doesn't match mem access insn %s" (string_of_ops ops)
        (Arm_insn.to_string (insn :> insn))
...
let lift_branch mem ops insn =
  ...
  match (insn, ops) with
  | `Bcc, [| offset; cond; _ |] -> Branch.lift offset ~cond addr
  | `BL, [| offset; cond; _ |] | `BL_pred, [| offset; cond; _ |] ->
      Branch.lift offset ~cond ~link:true addr
  | `BL, [| offset |] -> Branch.lift offset ~link:true addr
  | `BX_RET, [| cond; _ |] -> Branch.lift (`Reg `LR) ~cond ~x:true addr
  | `BX, [| target |] -> Branch.lift target ~x:true addr
  | `BX_pred, [| target; cond; _ |] -> Branch.lift target ~cond ~x:true addr
  | `BLX, [| target |] -> Branch.lift target ~link:true ~x:true addr
  | `BLX_pred, [| target; cond; _ |] ->
      Branch.lift target ~cond ~link:true ~x:true addr
  | `BLXi, [| offset |] -> Branch.lift offset ~link:true ~x:true addr
  | insn, ops ->
      fail [%here] "ops %s doesn't match branch insn %s" (string_of_ops ops)
        (Arm_insn.to_string (insn :> insn))
...
let lift_special ops insn =
  ...
  | `MRS, [| `Reg dest; cond; _ |] ->
      let get_bits flag src lsb =
        Bil.(src lor (cast unsigned 32 (var flag) lsl int32 lsb))
      in
  ...
  | `MSR, [| `Imm imm; `Reg src; cond; _ |] ->
      ...
      let s1 =
        if Word.(Int_exn.(imm land word 0x8) = word 0x8) then
          [
            Env.nf := 31; Env.zf := 30; Env.cf := 29; Env.vf := 28; Env.qf := 27;
          ]
        else []
      in
      let s2 =
  ...
  | insn, ops ->
      fail [%here] "ops %s doesn't match special insn %s" (string_of_ops ops)
        (Arm_insn.to_string (insn :> insn))
...
module CPU = struct
  ...
  let non_gpr = Var.Set.of_list [ pc; spsr; cpsr; itstate ]
  let gpr = Set.diff regs non_gpr
  let perms = Var.Set.of_list [ r4; r5; r6; r7; r8; r9; r10; r11 ]
  let flags = Var.Set.of_list @@ [ nf; zf; cf; qf; vf ] @ Array.to_list ge
  let nf = nf
  let zf = zf
  let cf = cf
  let vf = vf
...
let resolve_pc mem =
  Stmt.map
    (object
       ...
       method! map_var var =
         if Var.(equal var CPU.pc) then Bil.int (CPU.addr_of_pc mem)
         else super#map_var var

       method! map_move v x =
         if Var.(equal v CPU.pc) then super#map_jmp x else super#map_move v x
    end)

let insn_exn mem insn : bil Or_error.t =
  let encoding =
    Theory.Language.read ~package:"bap" (Basic.Insn.encoding insn)
  in
  Memory.(Addr.Int_err.(!$(max_addr mem) - !$(min_addr mem))) >>= Word.to_int
  >>= fun s ->
  Size.of_int ((s + 1) * 8) >>= fun scale ->
  Memory.get ~scale mem >>= fun word ->
  match Arm_insn.of_basic insn with
  | None -> Ok []
  | Some arm_insn -> (
      match arm_ops (Basic.Insn.ops insn) with
