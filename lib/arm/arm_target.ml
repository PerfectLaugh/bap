...
let of_bil v = CT.Var.define (Var.sort v) (Var.name v)
...
let regs32 =
  Theory.Role.Register.
    [
      ...
      ([ thumb ], regs Arm_env.[ r0; r1; r2; r3; r4; r5; r6; r7; sp; lr ]);
      ([ function_argument ], regs Arm_env.[ r0; r1; r2; r3 ]);
      ([ function_return ], regs Arm_env.[ r0; r1 ]);
      ([ caller_saved ], regs Arm_env.[ r0; r1; r2; r3; r12; lr ]);
      ([ callee_saved ], regs Arm_env.[ r4; r5; r6; r7; r8; r9; r10; r11; sp ]);
    ]
  @ status_regs

let vfp2regs =
  Theory.Role.Register.[ ([ general; floating ], untyped @@ array r64 "D" 16) ]

let vfp3regs =
  Theory.Role.Register.[ ([ general; floating ], untyped @@ array r64 "D" 32) ]
...
let flagsv8 = [ reg bool "NF"; reg bool "ZF"; reg bool "CF"; reg bool "VF" ]
let ( .$() ) = List.nth_exn

let aliases =
  xs @< ws @< qs @< ds @< ss @< hs @< bs @< [ fp64; lr64; sp64; zr64 ]
  @< [ sp32; zr32 ]
...
module Family (Order : Endianness) = struct
  ...
  let def ?code_alignment ~parent name =
    if CT.Target.is_unknown parent then CT.Target.unknown
    else
      CT.Target.declare ~package (ordered name) ~parent ?code_alignment
        ~nicknames:[ name ]

  let ( <: ) parent name = def ~parent name

  let v4 =
    if is_bi_endian then CT.Target.unknown
    else
      CT.Target.declare ~package (ordered "armv4") ~parent ~code_alignment:32
        ~nicknames:[ "armv4" ] ~bits:32 ~byte:8 ~endianness ~code:data ~data
        ~vars:vars32 ~regs:regs32
  ...
  let v7 =
    if not is_bi_endian then v6t2 <: "armv7"
    else
      CT.Target.declare ~package (ordered "armv7") ~parent
        ~nicknames:[ "armv7" ] ~code_alignment:16 ~bits:32 ~byte:8 ~endianness
        ~code:data ~data ~vars:vars32 ~regs:regs32

  let v7m = v7 <: "armv7-m"

  let v7fp =
    CT.Target.declare ~package (ordered "armv7+fp") ~parent:v7
      ~nicknames:[ "armv7+fp" ] ~vars:vars32_fp ~regs:(regs32 @ vfp3regs)

  let v7a = v7 <: "armv7-a"

  let v7afp =
    CT.Target.declare ~package (ordered "armv7-a+fp")
      ~nicknames:[ "armv7-a+fp" ] ~parent:v7a ~vars:vars32_fp
  ...
  let aarch32 =
    if is_bi_endian then Theory.Target.unknown
      ...
      Theory.Target.declare ~package
        ...
        ~parent:v7afp
        ~nicknames:[ (if is_little then "aarch32" else "aarch32eb") ]

  let v8 =
    CT.Target.declare ~package (ordered "armv8")
      ~parent:(if is_bi_endian then parent else aarch32)
      ~nicknames:[ "aarch64" ] ~aliasing ~code_alignment:32 ~bits:64
      ~code:datav8 ~data:datav8 ~vars:varsv8 ~regs:regsv8
  ...
  let v8a32 =
    Theory.Target.declare ~package
      (ordered "armv8-a+aarch32")
      ~nicknames:[ "armv8-a+aarch32" ] ~parent:v7

  let v8m32 =
    Theory.Target.declare ~package
      (ordered "armv8-m+aarch32")
      ~nicknames:[ "armv8-m+aarch32" ] ~parent:v7m

  let v8r32 =
    Theory.Target.declare ~package
      (ordered "armv8-r+aarch32")
      ~nicknames:[ "armv8-r+aarch32" ] ~parent:v7
  ...
  let aarch64 =
    if is_bi_endian then Theory.Target.unknown
      ...
      Theory.Target.declare ~package
        ...
        ~parent:v86a
        ~nicknames:[ (if is_little then "arm64" else "arm64eb") ]
...
module LE = Family (struct
  let endianness = CT.Endianness.le
end)

module Bi = Family (struct
  let endianness = CT.Endianness.bi
end)

module EB = Family (struct
  let endianness = CT.Endianness.eb
end)

let family_of_endian is_little : (module ARM) =
  match is_little with
  | None -> (module Bi)
  | Some true -> (module LE)
...
let in_family = function
  | None -> false
  | Some x -> List.exists prefixes ~f:(fun prefix -> String.is_prefix ~prefix x)

let drop_end s =
  Option.value ~default:s
  @@ List.find_map suffixes ~f:(fun suffix -> String.chop_suffix ~suffix s)

let split s =
  List.find_map_exn prefixes ~f:(fun prefix ->
...
let enable_loader () =
  let open Bap.Std in
  KB.Rule.(
    declare ~package "arm-target"
    |> require Image.Spec.slot |> provide CT.Unit.target
  ...
  let request_info doc =
    ...
    let request =
      ...
      Ogre.return (arch, sub, little, filetype)
    in
  ...
  >>| fun (arch, sub, is_little, filetype) ->
  if not (in_family arch) then CT.Target.unknown
    ...
    let parent =
      ...
      | "thumb", _ -> Family.v7m
      | ( "arm",
          ( "v4" | "v4t" | "v5" | "v5t" | "v5te" | "v5tej" | "v6" | "v6z"
          | "v6k" | "v6t2" | "v7" | "v7fp" | "v7a" | "v7afp" ) ) ->
          Family.aarch32
      | ( "arm",
    ...
    let filetype, system, abi =
      ...
      | Some "coff" ->
          (Theory.Filetype.coff, Theory.System.windows, Theory.Abi.eabi)
      | Some "macho" ->
          (Theory.Filetype.macho, Theory.System.darwin, Theory.Abi.eabi)
      | _ -> (Theory.Filetype.unknown, Theory.System.unknown, Theory.Abi.unknown)
    in
    Theory.Target.select ~strict:true ~system ~parent ~filetype ~abi ()

type arms = [ Arch.arm | Arch.armeb | Arch.thumb | Arch.thumbeb | Arch.aarch64 ]
...
let install_subtargets () =
  List.iter subtargets ~f:(fun (family, systems, abis, fabis, filetypes) ->
      List.iter family
        ~f:(Theory.Target.register ~systems ~abis ~fabis ~filetypes))

let enable_arch () =
  let open KB.Syntax in
  KB.Rule.(
    declare ~package "arm-arch"
    |> require CT.Unit.target |> provide Arch.unit_slot
  ...
  if is_arm t then
    ...
    | None -> (
        match (is_64bit t, is_big t, is_little t) with
...
let register ?attrs encoding triple =
  ...
  let triple =
    if Theory.Endianness.(eb = Theory.Target.endianness t) then triple ^ "eb"
    else triple
  in
...
let symbol_values doc =
  ...
  let symbols =
    let* symtab = Ogre.(collect Query.(select (from symbol_value))) in
    let+ entry = Ogre.request entry_point in
    match entry with
    | None -> symtab
    | Some entry ->
        let mask = Int64.(-1L lsl 1) in
        Seq.cons Int64.(entry land mask, entry) symtab
  in
...
module Encodings = struct
  ...
  let slot =
    ...
    @@ KB.Domain.mapping
         (module Bitvec_order)
         "encodings" ~equal:CT.Language.equal
  ...
  let set_encoding label x y =
    ...
    let* lang =
      ...
      | ":t32" | ":T32" -> !!llvm_t32
      | ":a32" | ":A32" -> !!llvm_a32
      | other -> Sigma.failp "unknown encoding %s, expects :T32 or :A32" other
    in
    let* encodings = unit --> slot in
    let res = Map.set encodings ~key:addr ~data:lang in
    KB.catch (KB.provide slot unit res) (fun _ -> KB.return ()) >>| fun () ->
    Sigma.Effect.pure Sigma.Value.nil

  let provide_primitive () =
    let types = Lambda.Type.Spec.(tuple [ int; sym ] @-> sym) in
    let docs =
      "(arm-set-encoding ADDR ENC) specifies the encoding of instruction at \
       ADDR as ENC, where ENC is either, :T32 or :A32"
    in
    Sigma.declare ~types ~docs ~package:"bap" "arm-set-encoding" ~body:(fun _ ->
        KB.return @@ fun lbl args ->
        match args with
...
let compute_encoding_from_symbol_table label =
  let ( >>=? ) x f =
    x >>= function None -> !!Theory.Language.unknown | Some x -> f x
  in
  ...
  if not (is_word_aligned addr) then llvm_t32
    ...
    match Map.find encodings addr with
    | Some x -> x
...
let ( < ) t p = t >= p && not (p >= t)
let ( <= ) t p = t = p || t < p
let m_profiles = [ LE.v7m; EB.v7m; Bi.v7m; LE.v8m32; EB.v8m32; Bi.v8m32 ]
let is_thumb_only t = List.exists m_profiles ~f:(fun p -> p <= t)

let register_pcode () =
  Dis.register pcode @@ fun t ->
  let triple =
    match (is_64bit t, is_little t, is_big t) with
    | true, true, _ -> "AARCH64:LE:64:v8A"
    | true, _, _ -> "AARCH64:BE:64:v8A"
    | false, true, _ -> "ARM:LE:32:v7"
    | false, _, true -> "ARM:BE:32:v7"
    | false, _, _ -> "ARM:LEBE:32:v7LEInstruction"
  in
...
let enable_pcode () =
  ...
  if is_arm t then pcode else Theory.Language.unknown

let guess_encoding interworking label target mode =
  if is_arm target then
    if is_64bit target then !!llvm_a64
    else if is_thumb_only target then !!llvm_t32
    else
      ...
      | Some true ->
          from_mode_or @@ fun () -> compute_encoding_from_symbol_table label
      | Some false -> !!llvm_a32
      | None -> (
          from_mode_or @@ fun () ->
...
let v8aversions =
  List.init max_v8a_version ~f:(fun i -> sprintf "+v8.%da" (i + 1))

let is_normal_feature s =
  String.length s > 0 && match s.[0] with '+' | '-' -> true | _ -> false

let normalize_features xs =
  List.map xs ~f:(fun x -> if is_normal_feature x then x else "+" ^ x)
  |> String.concat ~sep:","
...
let load ?features ?interworking ?(backend = "llvm") () =
  ...
  enable_arch ();
  Encodings.provide ();
  if String.equal backend "llvm" then enable_llvm ?features ?interworking ()
  else enable_pcode ()
