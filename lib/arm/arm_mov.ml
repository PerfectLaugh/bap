...
let lift ?(encoding = Theory.Language.unknown) ?dest src1 ?src2 itype ?sreg
    ?simm raw ~wflag cond =
  let dest : var =
    match dest with
    | None -> tmp reg32_t
    | Some (`Reg reg) -> Env.of_reg reg
    | Some (`Imm _) -> fail [%here] "dest is not a reg"
  in
  ...
  let s1, s2, stmts, carry =
    ...
    | `MOV, Some sreg, Some simm | `MVN, Some sreg, Some simm ->
        let shifted, carry =
          Shift.lift_r
            ~src:Bil.(var unshifted)
            simm ~shift:(exp_of_op sreg) reg32_t
        in
    ...
    | `MOV, None, Some simm | `MVN, None, Some simm ->
        let shifted, carry =
          Shift.lift_i ~src:Bil.(var unshifted) simm reg32_t
        in
        (shifted, s2, [ Bil.move unshifted s1 ], carry)
    | _, None, Some simm ->
        let shifted, carry =
  ...
  let stmts, flags =
    ...
    | (#arth as itype1), _, _ ->
        ...
        let flags =
          ...
          | `RSC -> set_sbc v2 v1 vd reg32_t
        in
        (stmts @ [ Bil.move orig1 s1; Bil.move orig2 s2 ], flags)
  in
  ...
  let oper =
    ...
    | `RSC -> Bil.(lnot s1 + s2 + cast unsigned 32 vcf)
  in
