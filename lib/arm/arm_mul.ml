...
let lift_mull ~lodest ~hidest ~src1 ~src2 sign ?addend ~wflag cond =
  ...
  let opn =
    match addend with
    | Some _ ->
        Bil.((s1_64 * s2_64) + concat (exp_of_reg hidest) (exp_of_reg lodest))
    | None -> Bil.(s1_64 * s2_64)
  in
  let insns =
    [
      Bil.move result opn;
...
let lift_smul ~dest ?hidest ~src1 ~src2 ?accum ?hiaccum ?q size cond =
  ...
  let result =
    ...
    | WT -> top32 (cast signed 64 (src1 * top src2))
  in
  let result =
    let open Bil in
    ...
    | _ -> fail [%here] "Cannot specify only a hi accumulator"
  in
  let qflag =
    match q with
    | Some true -> [ Bil.move Env.qf Bil.(excast 31 0 (var res) <> var res) ]
    | _ -> []
  in
  let instr =
    match hidest with
    ...
    | _ -> fail [%here] "unexpected operand type"
  in
