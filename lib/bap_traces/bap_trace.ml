...
type step = [ `Stop | `Skip | `Fail | `Make of event ] [@@deriving variants]
...
module Reader = struct
  type t = { tool : tool; meta : dict; next : unit -> event Or_error.t option }
end
...
let protocols_of_uri uri =
  Hashtbl.fold protos ~init:[] ~f:(fun ~key ~data acc ->
      let module A = (val data : P) in
      if A.probe uri then key :: acc else acc)
...
let save uri t : 'a result =
  ...
  match write uri t with Ok () -> Ok () | Error e -> Error (e :> error)

let create ?(monitor = Monitor.fail_on_error) tool next =
  let reader = { tool; meta = Dict.empty; next } in
  ...
  let set t =
    {
      t with
      meta =
        (match Dict.add t.meta attr v with
        | `Ok meta -> meta
        | `Duplicate -> Dict.change t.meta attr (fun _ -> Some v));
    }
  in
...
let set_meta t meta = { t with reader = { t.reader with meta } }
...
let read_events t : event seq =
  let map e =
    match t.mapper e with
    | None -> Seq.Step.Skip { state = () }
    | Some e -> Seq.Step.Yield { state = (); value = e }
  in
  Seq.unfold_step ~init:() ~f:(fun () ->
      ...
      | Some (Ok e) -> map e
      | Some (Error e) -> (
          match t.monitor e with
          | `Stop -> Seq.Step.Done
          | `Fail -> Error.raise e
          | `Skip -> Seq.Step.Skip { state = () }
          | `Make e -> Seq.Step.Yield { state = (); value = e }))

let read_all t tag = read_events t |> Seq.filter_map ~f:(Value.get tag)
...
let filter_map t ~f =
  {
    t with
    mapper = (fun ev -> match t.mapper ev with None -> None | Some ev -> f ev);
  }
...
let register_tool : (module S) -> tool =
 fun s ->
  let module A = (val s : S) in
  add tools A.name s;
  A.name

let register_proto : (module P) -> proto =
 fun p ->
