...
module Binary = struct
  ...
  let pp ppf t =
    fprintf ppf
      ...
      t.path
      (Stdlib.Digest.to_hex t.md5sum)
      Args.pp t.args Envp.pp t.envp
end

let pp_time fmt t =
  let open Unix in
  let tm = gmtime t in
  Format.fprintf fmt "%02d:%02d:%02d %02d.%02d.%04d GMT" tm.tm_hour tm.tm_min
    tm.tm_sec tm.tm_mday tm.tm_mon (1900 + tm.tm_year)
...
module Trace_stats = struct
  ...
  let pp ppf t =
    fprintf ppf "@[Trace created on %s by %s at %a@]" t.host t.user pp_time
      t.time
end
...
let tracer =
  Value.Tag.register
    (module Tracer)
    ~name:"tracer" ~uuid:"6e36c74e-580c-4496-9bf6-32cc0a09ef04"

let binary =
  Value.Tag.register
    (module Binary)
    ~name:"binary" ~uuid:"391bb6c3-2c93-4592-b292-43e8ec52e786"

let arch =
  Value.Tag.register
    (module Arch)
    ~name:"arch" ~uuid:"4bffe6d7-c554-45ec-9fb3-102f18e8f390"

let binary_file_stats =
  Value.Tag.register
    (module File_stats)
    ~name:"file-stats" ~uuid:"f7cde0f9-633d-4a9a-8e71-28fef521bee5"

let trace_stats =
  Value.Tag.register
    (module Trace_stats)
    ~name:"trace-stats" ~uuid:"c33eea1b-bca3-4cfe-91b5-52949e8f3c6b"
