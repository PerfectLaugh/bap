...
module Std : sig
  ...
  (** Trace is a stream of events plus meta data.
      ...
      It can be viewed as an input channel. In fact, [Trace.t] is an abstract
      data type usually inhabited with codata, i.e., some entity with hidden
      state. The [Trace] may be an interface to a remote server, virtual machine
      or just a file. So treat the trace as something that works as an input
      channel.

      Since it is worthwhile to know whether a particular event is not the trace
      because it didn't occur during program execution, but not because it
      wasn't detected by a trace tool or dropped by a given transport, we
      provide [supports] function, to query whether the given event type is
      really supported (and thus might occur) by a given trace.

      The meta information is also represented using [value] type, and thus can
      contain virtually any data. Meta information is indexed with [tag] value.
      Unlike the events, that are codata, meta is a regular data, obtained from
      a trace source at the time of trace creation and is not changed magically
      in the trace lifetime. *)
  module Trace : sig
    ...
    (** {2 Serialization}
        ...
        destination. URI contains enough information to uniquely designate data
        format and transporting options. *)

    val load : ?monitor:monitor -> Uri.t -> (t, error) Result.t
    (** [load ~monitor uri] fetches trace from a provided [uri]. [monitor] is
        fail_on_error by default. *)
    ...
    (** [set_attr trace attr value] updates [trace] meta attribute [attr] with a
        provided value. *)
    ...
    val tool : t -> tool
    (** [tool trace] returns a descriptor of a tool that was used to create the
        [trace]. *)
    ...
    (** [supports trace feature] is [true] if a tool that was used to generate
        the trace, as well as transporting protocol and underlying format
    ...
    (** [read_all_matching trace matcher] reads all events matching with a
        provided [matcher]. *)
    ...
    (** [next trace tag] reads and discards events until an event with a given
        tag is found. *)
    ...
    (** [next_matching trace matcher] reads and discards trace events until an
        event matching with the [matcher] is found.*)

    (** {2 Transformations} *)

    val filter_map : t -> f:(event -> event option) -> t
    (** [filter_map t ~f] will return a trace where all events a filter-mapped
    ...
    (** {2 Extension mechanism}
        ...
        For example, a tools is an instrumented qemu-user, a transport can be
        just a file, and protocol can be Google protobuf.

        Ideally, this three instances should be totally orthogonal, so that one
        can match them. In real life, we will strive to support only specific
        combinations.

        The extension mechanism allows a user to add support for new transports
        and protocols. The separation between transport and protocol is left
        beyond the scope of this interface. A user is welcome to built its own
        protocol stacks and reify them into explicit API.

        The interface is designed to support both static and dynamic trace
        tools. Although, the interface to control a dynamic tool is also left
        outside of the trace interface.

        In order to establish a correspondence between a concrete trace instance
        and a trace generator a unique id is used. Each time a trace is opened a
        fresh new unique id is generated that is passed to both sides: to the
        trace (accessible via [id] function) and to the trace reader (passed as
        a parameter). This [id] can be used from the client side to dynamically
        control a trace tool. *)
    ...
    module Monitor : sig
      ...
      (** [warn_on_error on_error] same as [ignore_errors] but calls [on_error]
      ...
      (** [pack_errors pack] will transform any occurred error into event using
          [pack] function. *)

      val create : (Error.t -> step) -> t
      (** [create filter] creates a user defined monitor from function [filter]
          that is applied to a sequence of events or errors, and returns a
          sequence of events. *)
    end
  end
  ...
  (** Loaded traces.
      ...
      This is a static container for the traces. Frontends and plugins can load
      traces and add it to the repository. *)
  module Traces : sig
    ...
    (** [remove trace] removes a [trace] from the repository, do nothing if it
        wasn't registered. *)
  end

  (** {2 Trace Events}

      The trace may contain arbitrary events. The events below is a good
      starting point. Other libraries may add more event. *)
  ...
  module Event : sig
    ...
    (** CPU loaded this memory chunk for execution. This event occurs just
        before the execution. All side effects of the code execution occurs
        after this event. *)

    val context_switch : int tag
    (** operating system has performed context switching to a provided thread
        (process) id. *)
