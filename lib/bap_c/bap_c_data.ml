...
type value = Top | Set of word list [@@deriving bin_io, compare, sexp]
...
let rec pp_layout ppf : layout -> unit =
 fun { layout = datum } -> pp_datum ppf datum

and pp_datum ppf : (layout obj, int) datum -> unit = function
  | Imm (sz, v) -> fprintf ppf "@[<2>[%a : %d]@]" pp_obj v sz
  | Seq objs ->
      fprintf ppf "@[@[<hv2>{@ ";
      pp_print_list ~pp_sep:(fun ppf () -> fprintf ppf ",@ ") pp_datum ppf objs;
      fprintf ppf "@]@;}@]"
  | Ptr t -> fprintf ppf "*%a" pp_datum t

and pp_obj ppf : layout obj -> unit = function
  | Basic t -> Bap_c_type.(pp ppf (basic t))
  | Field (name, layout) -> fprintf ppf "@[<2><%s : %a>@]" name pp_layout layout
  | Undef -> fprintf ppf "<undef>"
  | Union xs ->
      fprintf ppf "@[<hv>";
      pp_print_list ~pp_sep:(fun ppf () -> fprintf ppf "@;| ") pp_layout ppf xs;
      fprintf ppf "@]"
