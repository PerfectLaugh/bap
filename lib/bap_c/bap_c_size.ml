...
class base (m : model) =
  object (self)
    ...
    method pointer : addr_size =
      match m with #model32 -> `r32 | #model64 -> `r64

    method enum _ = self#integer `uint (* approximation *)

    method real (v : real) : real_size =
      match v with `float -> `r32 | `double -> `r64 | `long_double -> `r128

    method private double_size : real_size -> size =
      function `r32 -> `r64 | `r64 -> `r128 | `r128 -> `r256
    ...
    method array : _ -> Int.t option =
      fun { Spec.t = { Array.element = t; size } } ->
        match size with
        | None -> None
        | Some n -> (
            match self#bits t with None -> None | Some x -> Some (n * x))
    ...
    method union : compound unqualified -> Int.t option =
      fun { Spec.t = { Compound.fields } } ->
        ...
        | None -> None
        | Some ss -> (
            List.max_elt ~compare:Int.compare ss |> function
            | None -> None
            | Some s -> Some s)

    method structure : compound unqualified -> Int.t option =
      fun { Spec.t = { Compound.fields } } ->
        List.fold fields ~init:(Some 0) ~f:(fun sz (_, field) ->
            match sz with
            | None -> None
            | Some sz -> (
                match self#bits field with
                | None -> None
                | Some sz' -> Some (sz + sz' + padding (self#alignment field) sz)
                ))
  end
