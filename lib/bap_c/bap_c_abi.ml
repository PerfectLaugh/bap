...
let arg_intent : ctype -> intent = function
  ...
  | `Function _ -> In
  | `Union _ | `Structure _ -> In

type error = [ `Unknown_interface of string | `Parser_error of string * Error.t ]
[@@deriving sexp_of]
...
let data (size : #Bap_c_size.base) (t : Bap_c_type.t) =
  let open Data in
  let sizeof t =
    match size#bits t with None -> Size.in_bits size#pointer | Some s -> s
  in
  let padding pad : Data.t =
    ...
    | None ->
        ...
        Seq (List.init (pad / 8) ~f:(Fn.const data))
  in
  let rec data = function
    ...
    | `Function _ -> Ptr (Imm ((size#pointer :> size), Top))
  in
  data t
...
let layout (size : #Bap_c_size.base) (t : Bap_c_type.t) =
  ...
  let rec layout t : Data.layout =
    ...
    | `Structure { Spec.t = { Compound.fields = fs } } ->
        List.fold fs ~init:(0, 0, []) ~f:(fun (off, total, seq) (name, t) ->
            ...
            ( off + fsize + pad,
              ...
              :: (match pad with 0 -> seq | _ -> padding pad :: seq) ))
        |> fun (_, total, ss) ->
        let fullsize = sizeof t in
        let pad = max 0 (fullsize - total) in
        let ss = if pad = 0 then ss else padding (fullsize - total) :: ss in
    ...
    | `Union { Spec.t = { Compound.fields = fs } } ->
        ...
        let variants =
          List.map fs ~f:(fun (name, t) ->
              let fsize = sizeof t in
              let pad = max 0 (total - fsize) in
              let field = imm fsize @@ Field (name, layout t) in
              match pad with 0 -> field | _ -> seq [ field; padding pad ])
        in
        imm total (Union variants)
    | `Function _ -> ptr (imm (Size.in_bits size#pointer) Undef)
  in
  layout t
...
let coerce ltyp rtyp exp =
  match (ltyp, rtyp) with
  | Type.Mem _, _ | _, Type.Mem _ | Type.Unk, _ | _, Type.Unk -> exp
  | Imm m, Imm n -> (
      match Int.compare m n with
      | 0 -> exp
...
let register_model target model =
  if Hashtbl.mem models target then
    invalid_argf "A data model for target %s is already set"
      (Theory.Target.to_string target)
      ();
  Hashtbl.add_exn models ~key:target ~data:(model :> Bap_c_size.base)

let model target =
  match Hashtbl.find models target with
  | Some m -> m
  | None ->
      if Theory.Target.bits target = 32 then new Bap_c_size.base `LP32
...
let register name abi =
  let target =
    ...
    | None ->
        invalid_argf
          "The name of the abi should be a valid name. Got %s. See `bap list \
           targets` for the list valid names"
          name ()
  in
  ...
  | `Duplicate ->
      invalid_argf
        "The processor for ABI %s is already registered. Please pick a unique \
         name"
        name ()
                  ...
                  `Basic
                    {
                      qualifier =
                        Qualifier.
                          { const = false; volatile = false; restrict = () };
                      attrs = [];
                      t = `sint;
                    };
              };
...
let apply_args abi size attrs t sub =
  ...
  | Some { return; hidden; params } -> (
      ...
      | Unequal_lengths ->
          error
            "The ABI processor generated an incorrect number of argument terms \
             for the subroutine %s: %d <> %d"
            (Sub.name sub) (List.length params) (List.length t.args);
          sub
      | Ok args ->
          let ret =
            match return with
            ...
            | Some ret ->
                ...
                [ create_arg size 0 Out "result" t ret sub ]
          in
          let hid =
            List.mapi hidden ~f:(fun i (t, a) ->
                let n = "hidden" ^ if i = 0 then "" else Int.to_string i in
                create_arg size 0 Both n t a sub)
          in
...
let create_api_processor size abi : Bap_api.t =
  let stage1 gamma =
    object (self)
      ...
      method! map_sub sub =
        if Term.has_attr sub Attrs.proto then sub else self#apply_proto sub
...
module Stack = struct
  let create ?(growsup = false) arch off =
    ...
    let addr =
      if Word.is_zero off then Bil.(var sp)
      else if growsup then Bil.(var sp - int off)
      else Bil.(var sp + int off)
    in
...
module Arg = struct
  ...
  module Stack : sig
    ...
    (** [slots] returns a list of [(offset,datum,size_in_bits)] slots, where
        offset is properly aligned. *)

    val add : C.Type.t -> C.Data.t -> int -> t -> t
    (** [add datum bits] adds [bits] representing [datum] to the next available
        stack slot. The stack is growing downwards. *)

    val push : C.Type.t -> C.Data.t -> int -> t -> t
    (** [push datum bits] prepends [bits] representing [datume] to the beginning
    ...
    type info = { datum : C.Data.t; ctype : C.Type.t }
    ...
    let create (ruler : #C.Size.base) target =
      let min_alignment =
        max
          ...
          (Theory.Target.data_addr_size target / 8)
      in
      let align = function
        | None -> C.Size.next_multitude_of ~n:min_alignment
        | Some { ctype } ->
            let m = Size.in_bytes (ruler#alignment ctype) in
            C.Size.next_multitude_of ~n:(max min_alignment m)
      in
      match Theory.Target.reg target Theory.Role.Register.stack_pointer with
      | None -> None
      | Some sp ->
          Some { base = Var.reify sp; data = Map.empty (module Int); align }
    ...
    let append (info, size) stack =
      match Map.max_elt stack.data with
      | None -> singleton (info, size) stack
      | Some (k, (_, s)) ->
          let k' = stack.align info (k + bytes s) in
          {
            stack with
            data = Map.add_exn stack.data ~key:k' ~data:(info, size);
          }
    ...
    let push ctype datum size stack =
      Map.fold stack.data
        ~f:(fun ~key:_ ~data stack -> append data stack)
  ...
  module File = struct
    type t = { args : exp Map.M(Int).t; bits : int }

    let bits self = self.bits
    let deplet self = { self with args = Map.empty (module Int) }

    let pop self =
      match Map.min_elt self.args with
      | None -> None
      | Some (k, x) -> Some ({ self with args = Map.remove self.args k }, x)

    let popn n self =
      match Map.min_elt self.args with
      | None -> None
      | Some (k, _) -> (
          match Map.split self.args (k + n - 1) with
          | _, None, _ -> None
          | lt, Some (_, x), rt ->
    ...
    let align n self =
      ...
      | Some (k, _) -> (
          ...
          if k = k' then Some (self, ())
            ...
            match Map.split self.args k' with
            | _, None, _ -> None
    ...
    let of_list =
      List.foldi
        ~f:(fun pos regs reg -> Map.add_exn regs ~key:pos ~data:reg)
        ~init:Int.Map.empty
    ...
    let of_exps args =
      {
        ...
        bits =
          (match args with
          | [] -> -1
          | r :: _ -> ( match Type.infer_exn r with Imm x -> x | _ -> -1));
      }
    ...
    let of_roles roles t =
      let regs =
        Theory.Target.regs t ~roles |> Set.to_list |> List.map ~f:Var.reify
      in
      create regs
  end
  ...
  module Arena = struct
      ...
      let s =
        {
          s with
          files =
            (match Map.max_elt s.files with
            | None -> Map.singleton (module Int) 0 file
            | Some (k, _) -> Map.add_exn s.files ~key:(k + 1) ~data:file);
        }
      in
      let+ () = Arg.put s in
    ...
    let iargs = of_roles Theory.Role.Register.[ function_argument; integer ]
    let irets = of_roles Theory.Role.Register.[ function_return; integer ]
    let fargs = of_roles Theory.Role.Register.[ function_argument; floating ]
    let frets = of_roles Theory.Role.Register.[ function_return; floating ]
    let get { files } n = Map.find_exn files n

    let update s n f =
      match f (get s n) with
      | None -> Arg.reject ()
      | Some (arena, res) ->
          Arg.put { s with files = Map.set s.files ~key:n ~data:arena }
          >>= fun () -> Arg.return res
  ...
  let size t =
    let* s = Arg.get () in
    match s.ruler#bits t with None -> Arg.reject () | Some x -> Arg.return x
  ...
  let push_arg t exp =
    Arg.update @@ fun s ->
    match s.where with
    | `Return -> { s with return = Some (data s.ruler t, exp) }
    | `Hidden -> { s with hidden = (t, (data s.ruler t, exp)) :: s.hidden }
    | `Inputs -> { s with inputs = (data s.ruler t, exp) :: s.inputs }
  ...
  let discard ?(n = 1) file =
    Arg.get () >>= fun s ->
    Arena.popn ~n s file >>| fun _ -> ()

  let registers_for_bits file bits =
    let+ s = Arg.get () in
    let regs = Arena.get s file in
    let abits = File.bits regs in
    if abits > 0 then Some (((bits - 1) / abits) + 1) else None
  ...
  let needs_some f =
    f >>= function None -> Arg.reject () | Some x -> Arg.return x

  let registers_needed file bits = needs_some @@ registers_for_bits file bits
  ...
  let reference file t =
    with_hidden @@ fun () -> register file (C.Type.pointer t)

  let pointer file t = register file (C.Type.pointer t)

  let update_stack f =
    let* s = Arg.get () in
    match s.stack with
    | None -> Arg.reject ()
    | Some stack -> Arg.put { s with stack = Some (f stack) }
  ...
  let hidden t = with_hidden @@ fun () -> memory (C.Type.pointer t)
  ...
  let load t bits sp base =
    ...
    let addr = function
      | 0 -> Bil.(var sp)
      | off -> Bil.(var sp + int (Word.of_int ~width off))
    in
    let endianness =
    ...
    in
    let load_byte off =
      Bil.(load ~mem:(var mem) ~addr:(addr off) endianness `r8)
    in
    let rec load_bytes bytes loaded off =
      if loaded < bits then
        ...
        in
        Bil.(cast low) bits (concat bytes)
    in
  ...
  let stack =
    let* s = Arg.get () in
    match s.stack with None -> Arg.reject () | Some stack -> Arg.return stack
  ...
  let split f1 f2 typ =
    ...
    require (is_even bits) >>= fun () -> push_arg typ @@ concat (regs1 @ regs2)
  ...
  let define ?(return = Arg.return ()) inputs =
    Arg.sequence [ switch `Return; return; switch `Inputs; inputs ]
    ...
    let abi_processor =
      {
        ...
        insert_args =
          (fun _ attrs proto -> reify target ruler (pass attrs proto));
      }
    in
    register_abi abi_name abi_processor;
    register_model target ruler;
    Bap_abi.register_pass @@ fun proj ->
    if Theory.Target.equal (Project.target proj) target then (
    ...
    module V1 : V1 = struct
      ...
      let select options arg =
        List.find_map options ~f:(fun (cnd, action) ->
            if cnd arg then Some (action arg) else None)
        |> function
        | Some action -> action
        | None -> Arg.reject ()
      ...
      let choose options arg = choice (List.map options ~f:(fun f -> f arg))
      ...
      let install :
       ...
       fun target data k ->
        install target data
        @@ fun _ { Bap_c_type.Proto.return = r; args = xs } ->
        ...
        let return =
          ...
          | _ ->
              ...
              return ~alignment size r
        in
        let inputs =
          Arg.List.iter xs ~f:(fun (_, t) ->
              let* size = size t in
              let* alignment = alignment t in
              args ~alignment size t)
        in
        Arg.sequence [ switch `Return; return; switch `Inputs; inputs; finish ]
