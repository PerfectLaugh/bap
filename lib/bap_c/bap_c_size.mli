...
(** [next_multitude_of ~n x] returns [y >= x] so that [y] is a multitude of [n],
    i.e., [y = n * k].

    @since 2.5.0 *)

val padding : Size.t -> int -> int
(** [padding alignment offset] computes the required padding at [offset] to
...
(** The base class for computing sizes and aligments of C data types.
    ...
    The algorithm is implemented as a class to allow a particular implementation
    to fine tune the calculation.

    The [model] argument defines the default sizes for integral data types. If
    no suitable model is available for your architecture then use the closest
    model and override the specific methods to fine-tune the data model of your
    target.
    ...
    Using the LP64 data model, in which integers are 32 bit long and char is 8
    bit. The size of the structure is 12 bytes, due to the 3 bytes of padding
    before [v2] and six bytes of trailing padding.
...
class base : model -> object
  ...
  (** returns a size of the data type representation in bits.
      ...
      For incomplete types returns [None]. The size is always a multitude of the
      data type alignment and includes the paddings necessary for preserving the
  ...
  (** [array spec] if array [spec] is complete, i.e., the number of elements is
      known, then returns a product of the array size and the array's element
      type in bits, otherwise returns [None] *)

  method union : (no_qualifier, compound) spec -> bits option
  (** if spec is complete then returns a size in bits of the biggest element,
      including the padding between fields, but excludeing the trailing padding.
  *)

  method structure : (no_qualifier, compound) spec -> bits option
  (** if spec is complete then returns a total size of the structure, including
      the padding between fields, but excluding the trailing padding. *)
