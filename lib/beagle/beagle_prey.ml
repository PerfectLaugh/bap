...
module Words = struct
  ...
  let pp ppf set =
    ...
    let words =
      if String.length words < max then words else String.subo ~len:max words
    in
...
let chars =
  Value.Tag.register
    (module Words)
    ~uuid:"ff83ee29-1f58-4dc4-840c-4249de04a977" ~name:"beagle-chars"

let words =
  Value.Tag.register
    (module Words)
    ~uuid:"08e1ca88-eab9-4ac3-8fa8-3b08735a30e5" ~name:"beagle-words"

let strings =
  Value.Tag.register
    (module Words)
    ~uuid:"386efa37-85b0-4b48-b04d-8bafd5160670" ~name:"beagle-strings"

let statics =
  Value.Tag.register
    (module Statics)
    ~uuid:"eab82922-2c46-47bf-94ac-1ccb5de5daca" ~name:"static-strings"

type t = { terms : tid seq; chars : string }
...
let inspect (prey, words) =
  Sexp.List [ inspect_prey prey; String.Set.sexp_of_t words ]

let caught, catch =
  Primus.Observation.provide ~inspect "beagle"
