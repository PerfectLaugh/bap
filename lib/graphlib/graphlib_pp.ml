...
(** [next_char c] returns next a lowercase character following the [c], if it
    exists *)
...
(** [next_sym s] generates new fresh symbol, assuming that [s] is the highest
    generated symbol. The function will at first use all lowercase symbols from
    [[a-z]], and then fall back to a simple scheme [node_n], where n goes from
    zero to max_int.*)
