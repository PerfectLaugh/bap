...
let create_ancestors empty find : ('a, 'a set) map =
  let rec walk set x =
    match find x with None -> set | Some p -> walk (Set.add set p) p
  in
  object
...
let create_descendants children : ('a, 'a set) map =
  let rec walk set x =
    match Map.find children x with
    | None -> set
    | Some cs -> Set.union cs @@ Set.fold cs ~init:set ~f:walk
  in
  object
    method find x = Some (create_set (walk (empty_set children) x))
  end
...
let create_tree parents children =
  {
    ...
    cld =
      object
        method find x = Option.(Map.find children x >>| create_set)
      end;
  }

let enum = function Some x -> x#enum | None -> Seq.empty
...
module Tree = struct
  ...
  let mem table t p =
    match (table t)#find p with None -> fun _ -> false | Some xs -> xs#mem
...
module Frontier = struct
  type 'a t = { all : 'a list; map : ('a, 'a set) map }

  let create all map = { all; map }
  let enum t x = t.map#find x |> enum

  let mem t x =
    match t.map#find x with None -> fun _ -> false | Some xs -> xs#mem
...
module Group = struct
  ...
  type 'a t = { top : 'a; set : 'a set; ord : equiv }
  ...
  let pp pp_elt ppf t =
    fprintf ppf "{%d: %a => (%s)}" t.ord pp_elt t.top
      (string_of_set ~sep:" " pp_elt t.set)
...
module Partition = struct
  ...
  let create (type a) (type c)
    ...
    let roots, groups =
      ...
      |> Map.to_alist |> List.unzip
    in
    let roots = Array.of_list roots in
    let groups =
      Array.of_list_map groups ~f:(fun xs ->
          create_set (Set.of_list comparator xs))
    in
    let module M = (val comparator) in
    let { Comparator.compare } = M.comparator in
    let find_root x = Array.binary_search roots ~compare `First_equal_to x in
  ...
  *)
  let refine (type elt) t ~equiv ~cmp =
    ...
    let refine_group g =
      let rec insert elt output input =
        match input with
        | [] -> Set.singleton (module T) elt :: output
        | group :: input ->
            if equiv (Set.choose_exn group) elt then
              List.rev_append (Set.add group elt :: output) input
            else insert elt (group :: output) input
      in
      Seq.fold g#enum ~init:[] ~f:(fun groups elt -> insert elt [] groups)
      |> List.rev_map ~f:create_set
    in
    let groups_list =
      Array.fold t.groups ~init:[] ~f:(fun seqs g -> refine_group g @ seqs)
    in
      ...
      | Some i_x, Some i_y ->
          ...
          let find x =
            Option.Monad_infix.(
              ...
              fun n -> if n < j then n else if n = j then i else n - 1)
          in
          { roots; groups; find }
  ...
  let of_equiv t i =
    if i >= 0 && i < Array.length t.roots then Some (nth_group t i) else None
...
module To_ocamlgraph (G : Graph) = struct
  ...
  let out_degree g n = checked "out_degree" (G.Node.degree ~dir:`Out) n g
  let in_degree g n = checked "in_degree" (G.Node.degree ~dir:`In) n g
  let mem_vertex g n = G.Node.mem n g
  let mem_edge g x y = G.Node.has_edge x y g
  let succ g n = checked "succ" G.Node.succs n g |> Seq.to_list_rev
  let pred g n = checked "pred" G.Node.preds n g |> Seq.to_list_rev
  let succ_e g n = checked "succ_e" G.Node.outputs n g |> Seq.to_list_rev
  let pred_e g n = checked "pred_e" G.Node.inputs n g |> Seq.to_list_rev
  ...
  let fold_edges f g init =
    G.edges g
    |> Seq.fold ~init ~f:(fun init e -> f (G.Edge.src e) (G.Edge.dst e) init)

  let fold_edges_e f g init =
    G.edges g |> Seq.fold ~init ~f:(fun init e -> f e init)

  let map_vertex f g =
    G.nodes g |> Seq.fold ~init:G.empty ~f:(fun g v -> G.Node.insert (f v) g)

  let find_edge g x y =
    match G.Node.edge x y g with Some e -> e | None -> raise Stdlib.Not_found

  let find_all_edges g x y =
    G.Node.edge x y g |> function None -> [] | Some x -> [ x ]
  ...
  let add_edge g n = invalid_arg "add_edge operation is not supported"
  let add_edge_e g e = G.Edge.insert e g

  let remove_edge g x y =
    match G.Node.edge x y g with None -> g | Some e -> G.Edge.remove e g
...
module Of_ocamlgraph (G : Graph.Sig.P) = struct
  ...
  module Node = struct
    ...
    let update n l g =
      if G.mem_vertex g n then G.add_vertex (G.remove_vertex g n) (create l)
      else g
  ...
  module Edge = struct
    ...
    include Opaque.Make (struct
      ...
      let hash e = Hashtbl.hash (src e) lxor Hashtbl.hash (dst e)

      let compare x y =
        match Node.compare (src x) (src y) with
  ...
  include Printable.Make (struct
    ...
    let pp ppf graph =
      ...
      let string_of_node =
        by_natural_order symbols Node.compare (nodes graph)
      in
      Dot.pp_graph ~string_of_node
        ~nodes_of_edge:(fun e -> Edge.(src e, dst e))
        ~nodes:(nodes graph) ~edges:(edges graph) ppf
  end)
  ...
  include Opaque.Make (struct
    ...
    let hash g =
      Seq.fold (edges g) ~init:0 ~f:(fun hash x -> hash lxor Edge.hash x)
    ...
    let cmp (type t) (module C : Comparable with type t = t) enum x y =
...
let create (type t) (type a) (type b)
    (module G : Graph
      with type t = t
       and type Node.label = a
       and type Edge.label = b) ?(nodes = []) ?(edges = []) () =
  let g =
    List.fold edges ~init:G.empty ~f:(fun g (src, dst, data) ->
        G.Edge.insert
          (G.Edge.create (G.Node.create src) (G.Node.create dst) data)
          g)
  in
...
let compare (type e) (type g) (type n)
  ...
  let edges edges src dst empty g =
    edges g |> Seq.map ~f:(fun e -> (src e, dst e)) |> set empty
  in
...
module Ordering (G : Graph) = struct
  ...
  let enter t u =
    ( {
        ...
        numbers =
          Map.change t.numbers u ~f:(fun _ ->
              Some { pre = t.nodes_entered; rpost = 0 });
      },
      t.nodes_entered )
  ...
  let leave t u =
    ( {
        ...
        numbers =
          Map.change t.numbers u ~f:(function
            | None -> assert false
...
let pass _ _ s = s
let is_tree = function `Tree -> true | _ -> false

let depth_first_search (type g) (type n) (type e)
    (module G : Graph with type t = g and type node = n and type edge = e)
    ?(rev = false) ?start ?(start_tree = fun _ s -> s) ?(enter_node = pass)
    ?(leave_node = pass) ?(enter_edge = pass) ?(leave_edge = pass) g ~init =
  ...
  let init =
    ...
    | None -> (ord, init)
    | Some s ->
        if G.Node.mem s g then visit ord s (start_tree s init) Fn.id
        else (ord, init)
  in
  G.nodes g
...
let depth_first_visit graph ?rev ?start g ~init vis =
  depth_first_search graph ?rev ?start g ~init ~start_tree:vis#start_tree
    ~enter_node:vis#enter_node ~leave_node:vis#leave_node
    ~enter_edge:vis#enter_edge ~leave_edge:vis#leave_edge
...
let union (type t) (type n) (type e)
    (module G : Graph with type t = t and type node = n and type edge = e) g1 g2
    =
  depth_first_search
    (module G)
    ~init:g1 g2
...
let inter (type t) (type n) (type e)
  ...
  let insert_edge other e g =
    if G.Edge.mem e other then G.Edge.insert e g else g
  in
  let insert_node other n g =
    if G.Node.mem n other then G.Node.insert n g else g
  in
  let insert g g1 g2 =
    depth_first_search
      (module G)
      ...
      ~enter_edge:(fun _ -> insert_edge g2)
  in
  insert (insert G.empty g1 g2) g2 g1

let reverse_postorder_traverse graph ?rev ?start g =
  depth_first_search graph ?rev ?start g ~init:[] ~leave_node:(fun _ n ns ->
      n :: ns)
  |> Seq.of_list
...
let postorder_traverse graph ?rev ?start g =
  ...
  depth_first_search ?rev ?start graph g ~init:(return ())
    ~leave_node:(fun _ n ns -> ns >>= fun () -> yield n)
  |> run
...
let create_namer (type t) (type n)
  ...
  let namer =
    depth_first_search
      (module G)
      g ~init:G.Node.Map.empty
      ~leave_node:(fun rpost n names ->
        Map.set names ~key:n ~data:(Int.to_string rpost))
  in
  Map.find_exn namer

let nil _ = []

let to_dot (type t) (type n) (type e)
    (module G : Graph with type t = t and type node = n and type edge = e)
    ?(graph_attrs = nil) ?(node_attrs = nil) ?(edge_attrs = nil) ?string_of_node
    ?string_of_edge ?channel ?formatter ?filename g =
  let string_of_node =
    match string_of_node with
    | Some namer -> namer
    | None -> create_namer (module G) g
  in
  let module Dottable = struct
    ...
    let edge_attributes e =
      ...
      List.find attrs ~f:(function `Label _ -> true | _ -> false) |> function
      | Some _ -> attrs
      | None -> (
          match string_of_edge with
          | None -> attrs
  ...
  Option.iter filename
    ~f:(Out_channel.with_file ~f:(fun chan -> Dot.output_graph chan g))

exception Unreachable

(** Immediate dominators. This algorithm implements «A simple, fast dominance
    algorithm» [1], with some modifications, that allows it to run on arbitrary
    graphs.

    If provided with a graph and an entry node it will span only connected part
    of the graph, ignoring unreachable nodes. It will return a parent node as an
    immediate dominator of any unreachable node

    [1]: Cooper, Keith D., Timothy J. Harvey, and Ken Kennedy. "A simple, fast
...
let idom (type t) (type n) (type e)
  ...
  with_return (fun { return } ->
      depth_first_search ~rev
        ...
        ~leave_node:(fun _ n i ->
          ...
          i + 1)
      |> (ignore : int -> _));
  let rec lift x y : int =
    match (x, y) with
    | -1, _ | _, -1 -> len - 1
    | _ -> if x < y then lift doms.(x) y else x
  in
  ...
  `idom
    (fun n ->
      ...
      with Unreachable -> if G.Node.mem n g then Some node.(len - 1) else None)

let dominators (type t) (type n) (type e)
    (module G : Graph with type t = t and type edge = e and type node = n) ?rev
    g entry =
  let (`idom parent) = idom ?rev (module G) g entry in
  let init =
    G.nodes g
    |> Seq.fold ~init:G.Node.Map.empty ~f:(fun t n -> Map.set t ~key:n ~data:[])
  in
  let children =
    G.nodes g
  ...
  let compute ?(rev = false) g tree =
    ...
    let rec walk top dfs r =
      if Option.equal G.Node.equal (Some r) top then dfs
        ...
        match idom r with
        | None -> Set.add dfs r
        | Some p -> walk top (Set.add dfs r) p
    in
    G.nodes g
...
let dom_frontier_generic (type t) (type n) (type e)
    ...
    (module G : Graph with type t = t and type node = n and type edge = e) ?rev
    g idom : n Frontier.t =
  let module Algo = Algo (G) in
  let frontier = Algo.compute ?rev g idom in
  let frontier = Map.map frontier ~f:create_set in
  Frontier.create (Map.keys frontier)
    (object
       method find = Map.find frontier
    end)

let dom_frontier g = dom_frontier_generic (module Dom_frontier_cooper) g

let strong_components (type t) (type n) (type e)
    (module G : Graph with type t = t and type node = n and type edge = e) g =
  ...
  depth_first_search
    ...
    ~enter_node:(fun t v stack ->
      Hashtbl.add_exn roots ~key:v ~data:(t, v);
      v :: stack)
...
module Path = struct
  type 'a t = { edges : 'a array; weight : int }
  ...
  let pp pp_elt ppf t =
    ...
    Seq.range 1 (length t)
    |> Seq.iter ~f:(fun i -> fprintf ppf ", %a" pp_elt t.edges.(i));
    fprintf ppf "}@]"
...
let shortest_path (type t) (type n) (type e)
    (module G : Graph with type t = t and type node = n and type edge = e)
...
let fold_reachable (type t) (type n) (type e)
  ...
  with_return (fun { return } ->
      depth_first_search
        (module G)
        g ?rev ~start ~init
        ~start_tree:(fun n s -> if G.Node.(n = start) then s else return s)
...
struct
  ...
  module Has = struct
    let node = Has.node
    let edge e = Has.edge e && node (G.Edge.dst e) && node (G.Edge.src e)
  end
  ...
  module Node = struct
    ...
    let edge n m g =
      match edge n m g with Some e when Has.edge e -> Some e | _ -> None

    let has_edge n m g = match edge n m g with None -> false | _ -> true
...
let filtered (type t) (type n) (type e)
    (module G : Graph with type t = t and type node = n and type edge = e)
    ?(skip_node = fun _ -> false) ?(skip_edge = fun _ -> false) () :
    (module Graph with type t = t and type node = n and type edge = e) =
  let module R =
    Filtered
      (G)
      (struct
...
struct
  ...
  module Node = struct
    ...
    let edge n m g = Option.(edge (N.backward n) (N.backward m) g >>| E.forward)
  ...
  module Edge = struct
    ...
    include Opaque.Make (struct
      ...
      let compare x y = G.Edge.compare (E.backward x) (E.backward y)
...
let view (type t) (type n) (type e) (type a) (type b) (type m) (type f) (type c)
    (type d)
    (module G : Graph
      with type t = t
       and type node = n
       and type edge = e
       and type Node.label = a
       and type Edge.label = b) ~node:(nf, nb) ~edge:(ef, eb)
    ~node_label:(nlf, nlb) ~edge_label:(elf, elb) :
    (module Graph
       with type t = t
        and type node = m
        and type edge = f
...
module Fixpoint = struct
  ...
  let create constraints default =
    Solution { steps = Some 0; iters = 0; approx = constraints; default }

  let iterations (Solution { iters }) = iters
  let default (Solution { default }) = default

  let get (Solution { approx; default }) n =
    match Map.find approx n with None -> default | Some x -> x
  ...
  let is_fixpoint (Solution { steps; iters }) =
    match steps with None -> iters > 0 | Some steps -> iters < steps
  ...
  *)
  let compute (type g n d) (module G : Graph with type t = g and type node = n)
      ?steps ?start ?(rev = false) ?step
      ~init:(Solution { approx = init; iters; default }) ~equal ~merge ~f g :
      (n, d) t =
    let nodes =
      reverse_postorder_traverse (module G) ~rev ?start g |> Sequence.to_array
    in
    let rnodes =
      Array.foldi nodes ~init:G.Node.Map.empty ~f:(fun i rnodes n ->
          Map.set rnodes ~key:n ~data:i)
    in
    let succs =
    ...
    let user_step =
      ...
      | Some step ->
          fun visits n x x' ->
            let i =
              match Map.find visits n with None -> 1 | Some x -> x + 1
            in
            let visits = Map.set visits ~key:n ~data:i in
            (visits, step i nodes.(n) x x')
    in
    let get approx n : d =
      match Map.find approx n with Some x -> x | None -> default
    in
      ...
      | Some n ->
          ...
          |> Set.fold ~init:(visits, works, approx)
               ~f:(fun (visits, works, approx) n ->
                 ...
                 else (visits, Set.add works n, Map.set approx ~key:n ~data:ap'))
          |> continue
    in
    let can_iter iters =
      match steps with None -> true | Some steps -> iters < steps
    in
