...
module Make (Node : Opaque.S) (Label : T) = struct
  ...
  type edge = { src : Node.t; dst : Node.t; data : edge_label }
  [@@deriving fields]

  let compare_edge x y =
    match Node.compare x.src y.src with 0 -> Node.compare x.dst y.dst | n -> n

  type arrows = edge_label Node.Map.t

  let compare_arrows x y = Map.compare_direct (fun _ _ -> 0) x y

  type node_info = { inc : arrows; out : arrows } [@@deriving compare, fields]
  type graph = node_info Node.Map.t [@@deriving compare]
  ...
  module Node = struct
    ...
    let insert n g =
      Map.change g n ~f:(function None -> Some empty_node | other -> other)
    ...
    let edge src (dst : Node.t) g =
      match Map.find g src with
      | None -> None
      | Some a -> (
          match Map.find a.out dst with
          | None -> None
          | Some data -> Some { src; dst; data })

    let has_edge src dst g = Option.is_some @@ edge src dst g

    let degree select n g =
      match Map.find g n with None -> 0 | Some s -> Map.length (select s)
  ...
  module Edge = struct
    ...
    let upsert_arrow src dst field e g =
      Map.change g (src e) ~f:(function
        | None ->
            Some
              (Field.fset field empty_node (Node.Map.singleton (dst e) e.data))
        | Some ns ->
            let map = Field.get field ns in
            Some (Field.fset field ns (Map.set map ~key:(dst e) ~data:e.data)))

    let remove_arrow field arr src g =
      Map.change g src ~f:(function
        | None -> None
        | Some ns ->
  ...
  let number_of_edges g =
    Map.fold g ~init:0 ~f:(fun ~key:_ ~data:{ out } sum -> sum + Map.length out)
  ...
  include Opaque.Make (struct
    ...
    let hash g =
      nodes g |> Seq.fold ~init:0 ~f:(fun hash n -> Node.hash n lxor hash)
  end)
...
module Printable_graph (Graph_pp : Printable_graph) = struct
  ...
  include Printable.Make (struct
    ...
    let pp_label ppf lab =
      match asprintf "%a" pp_edge lab with
      | "" -> ()
      | l -> fprintf ppf "[label=%S]" l

    let pp_edge ppf e =
      Format.fprintf ppf "\"%a\" -> \"%a\"%a" pp_node (Edge.src e) pp_node
        (Edge.dst e) pp_label (Edge.label e)
