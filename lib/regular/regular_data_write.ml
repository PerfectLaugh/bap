...
let not_sufficient () = invalid_arg "Writable class definition is not complete"
let bytes_to_bytes ~dst ~src dst_pos = Bytes.blito ~src ~dst ~dst_pos ()
...
let create ?to_bytes ?to_bigstring ?dump ?pp ?size ?blit_to_string:copy
    ?blit_to_bigstring:(blit : ('a, bigstring) copy option) () =
  let to_bytes =
    match (to_bytes, to_bigstring, pp) with
    | Some f, _, _ -> Some f
  ...
  in
  let size =
    match (size, to_bytes, to_bigstring) with
    | None, None, None -> not_sufficient ()
    | Some f, _, _ -> f
    | _, Some f, _ -> fun x -> Bytes.length (f x)
    | _, _, Some f -> fun x -> Bigstring.length (f x)
  in
  let copy =
    match (copy, to_bytes, blit, to_bigstring) with
    ...
    | _, _, _, Some f -> fun dst x -> bigstring_to_bytes ~dst ~src:(f x)
  in
  let blit : ('a, bigstring) copy =
    match (blit, to_bytes, to_bigstring) with
    | Some f, _, _ -> f
    | _, Some f, _ -> fun dst x -> bytes_to_bigstring ~dst ~src:(f x)
    | _, _, Some f -> fun dst x -> bigstring_to_bigstring ~dst ~src:(f x)
    | _ -> fun dst x -> blit_via_copy size copy ~dst x
  in
  let to_bytes =
    match to_bytes with Some f -> f | None -> bytes_via_copy size copy
  in
  let pp =
    match pp with
    | Some f -> f
    | None ->
        fun ppf x -> Format.fprintf ppf "%s" (Bytes.to_string (to_bytes x))
  in
  let to_bigstring =
    match to_bigstring with
    | Some f -> f
    | None -> fun x -> Bigstring.of_bytes (to_bytes x)
  in
  let dump =
    match dump with
    | Some f -> f
