...
module Binable = Bin_prot.Utils.Make_binable_with_uuid (struct
  ...
  let caller_identity =
    Bin_shape.Uuid.of_string "19071993-d15a-49be-b3a2-4ef4bacd3c0a"
end)
...
module To_string = struct
  ...
  let subo ?(pos = 0) ?len src =
    let len =
      match len with None -> Bytes.length src - pos | Some len -> len
    in
    sub src ~pos ~len
end
...
module Unsafe = struct
  ...
  external blit : T.t -> int -> T.t -> int -> int -> unit
    = "caml_blit_string" "noalloc"

  external fill : T.t -> int -> int -> char -> unit
