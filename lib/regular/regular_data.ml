...
let pretty_writer (type t) (module T : Pretty_printer.S with type t = t) =
...
type 'a with_desc = { cls : 'a; desc : string option }
...
module Class = struct
  type 'a key = { key : 'a Type_equal.Id.t }
  type 'a t = 'a key
  ...
  let define key data = registry := Registry.add_exn !registry ~key ~data

  let refine cls ~f =
    registry :=
      Registry.update !registry cls ~f:(function
        | None ->
            invalid_argf "Data class %s is not defined yet"
              (Type_equal.Id.name cls.key)
              ()
        | Some x -> f x)

  let update (get, set) cls ~f =
    refine cls ~f:(fun meta -> set meta (f (get meta)))

  let add_method field cls ?desc ~ver name met : unit =
    update field cls ~f:(fun mets ->
        Map.change mets name ~f:(function
  ...
  let find_with_info from ?ver name =
    ...
    | Some (ver, { cls; desc }) -> { ver; serializer = cls; name; desc }
  ...
  let get (get, _) key = get (Registry.find_exn !registry key)
  let get_method fld cls ?ver name = find_cls (get fld cls) ?ver name
  let get_method_with_info fld cls ?ver name = find_info (get fld cls) ?ver name

  module Fields = struct
    let writer_field =
      ((fun { writers } -> writers), fun s writers -> { s with writers })

    let reader_field =
      ((fun { readers } -> readers), fun s readers -> { s with readers })
  ...
  let collect_info (get, _) def =
    let name =
      match def.module_name with None -> "unknown" | Some name -> name
    in
    ( name,
  ...
  type collector = { collect : 'a. 'a definition -> string * info list }
  ...
  let info { collect } =
    ...
    |> List.map ~f:(fun (Registry.Packed.T (_, data)) -> collect data)
...
module Extend (T : sig
  ...
  let default_printer () =
    Class.get default_printer_field cls |> Option.map ~f:info

  let default_reader () = info @@ Class.get default_reader_field cls
  ...
  let default_writer () = info @@ Class.get default_writer_field cls
  ...
  let get_default ?ver ?fmt field default_field =
    ...
    | Some name -> (Class.get_method_with_info field cls ?ver name).serializer

  let writer ver fmt = get_default ?ver ?fmt writer_field default_writer_field
  let reader ver fmt = get_default ?ver ?fmt reader_field default_reader_field
  ...
  let blit_to_bytes ?ver ?fmt = Write.blit_to_string @@ writer ver fmt
  let of_bigstring ?ver ?fmt = Read.of_bigstring @@ reader ver fmt
  let to_bigstring ?ver ?fmt = Write.to_bigstring @@ writer ver fmt
  let blit_to_bigstring ?ver ?fmt = Write.blit_to_bigstring @@ writer ver fmt
  ...
  module Io = struct
    ...
    let scan ?ver ?fmt ch =
      let next () = try Some (load ?ver ?fmt ch) with End_of_file -> None in
      next
    ...
    let with_printer ?ver ?fmt f =
      ...
      | None -> (
          match Class.get default_printer_field cls with
          | None -> ()
          | Some writer -> f writer.serializer)

    let show ?ver ?fmt x =
      with_printer ?ver ?fmt (fun writer -> Write.to_channel writer stdout x)

    let print ?ver ?fmt ppf x =
      with_printer ?ver ?fmt (fun writer -> Write.to_formatter writer ppf x)
  end
...
let set_module_name cls name =
  Class.update module_name_field cls ~f:(fun _ -> Some name)
