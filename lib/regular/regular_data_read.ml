...
let not_sufficient () = invalid_arg "Readable class definition is not complete"
...
let create ?of_channel:fc ?of_lexbuf:fl ?of_scanbuf:fs ?of_bigstring:fB
    ?of_bytes:fb () =
  let of_bytes =
    match (fb, fs, fl, fB) with
    | None, None, None, None -> not_sufficient ()
    | Some f, _, _, _ -> f
    | _, Some f, _, _ -> Bytes.of_scanbuf f
    | _, _, Some f, _ -> Bytes.of_lexbuf f
    | _, _, _, Some f -> Bytes.of_bigstring f
  in
  let of_channel =
    match (fc, fs, fl) with
    ...
    | _, _, Some f -> Channel.of_lexbuf f
  in
  let of_bigstring =
    match fB with
