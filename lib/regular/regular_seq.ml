...
let of_array arr = init (Array.length arr) ~f:Array.(unsafe_get arr)
let sexp_of_seq sexp_of_elt t = [%sexp_of: list] sexp_of_elt (to_list t)
let seq_of_sexp elt_of_sexp t = of_list ([%of_sexp: list] elt_of_sexp t)
...
module Binable = Bin_prot.Utils.Make_binable1_with_uuid (struct
  ...
  let caller_identity =
    Bin_shape.Uuid.of_string "57e090bd-189d-4bd1-a619-53e70d0baa16"
end)
...
let pp pp_elt ppf xs =
  if Sys.interactive.contents then pp_some pp_elt ppf xs
