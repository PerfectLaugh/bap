...
let empty (type key) (type data) compare_key compare_data =
  ...
  Rel { vals = Map.empty (module K); keys = Map.empty (module D) }

let add (Rel { vals; keys }) key value =
  Rel
    {
      vals =
        Map.update vals key ~f:(function
          | Some vals -> Set.add vals value
          | None -> Set.singleton (Map.comparator_s keys) value);
      keys =
        Map.update keys value ~f:(function
...
let find_multi xs x =
  match Map.find xs x with None -> [] | Some xs -> Set.to_list xs
...
let matching (Rel { vals; keys }) ?(saturated = skips) ?(unmatched = skipu) init
  ...
  Map.fold ~init vals ~f:(fun ~key ~data:vals init ->
      ...
      | [] -> assert false
      | _ :: _ :: _ as vals -> unmatched (Non_injective_bwd (vals, key)) init
      | [ s ] -> (
          match find_multi keys s with
