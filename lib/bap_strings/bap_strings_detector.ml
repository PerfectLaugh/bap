...
let empty =
  { chars = []; data = []; n = 0; m = 0; cut = 0; len = 0; stage = Working }

let log1p = Float.log1p

let create ?(alpha = 0.05) ?(beta = 0.001) ?(p1 = 0.5) ?(ps = 0.05) ?len_pdf
    alphabet =
  let s = 245. in
...
let decide t p =
  if Float.(p <= t.t0) then Decline
  else if Float.(p >= t.t1) then Accept
  else Undecided
...
let update t x char =
  ...
  in
  if e1 then { t with h = push h x char } else { t with h }
...
let step t x char =
  if equal_stage t.h.stage Finished then step_char { t with h = empty } x char
  else step_char t x char

let when_decided t ~f init =
  if equal_stage t.h.stage Finished then f t.h else init

let decision t = when_decided t ~f:(fun x -> Some x) None

let abort t =
  if equal_stage t.h.stage Accepted then Some { t.h with cut = 0 } else None
...
let run t =
  ...
  Sequence.unfold_with ~init:t ~f:(fun t (x, c) ->
      ...
      when_decided t
        (Skip { state = t })
        ~f:(fun d -> Yield { value = d; state = t }))

let rev_zip_skip skip xs ys =
  let rec loop xs ys zs r =
    match (xs, ys) with
    | x :: xs, y :: ys ->
        if r > 0 then loop xs ys zs (r - 1) else loop xs ys ((x, y) :: zs) 0
    | [], [] -> zs
    | _ -> invalid_arg "data and chars should be equal in size"
  in
  loop xs ys [] skip
...
let chars { chars; cut; len } =
  ...
  List.iteri chars ~f:(fun i c ->
      if i >= cut then Bytes.set data (len - 1 - (i - cut)) c);
  Bytes.to_string data

let data ?(rev = false) { data; cut } =
  if rev then List.drop data cut else List.drop data cut |> List.rev

let pp_stats ppf t =
  fprintf ppf "%.2f < p < %.2f where p = %.2f = %d*%.2f - %d * %.2f - %.2f" t.t0
    t.t1 (hprop t) t.h.n t.w1 t.h.m (-.t.w0) (-.t.p1)
...
let pp_decision ppf h = fprintf ppf "%s: %S" (string_of_stage h.stage) (chars h)
