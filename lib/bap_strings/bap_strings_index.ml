...
module Persistent = struct
  ...
  module Make (Key : Key) = struct
    ...
    type key = Key.t
    type t = { strings : string Index.t; keys : key String.Map.t }

    let empty = { strings = Index.empty; keys = String.Map.empty }

    let string { strings } key =
      match Map.find strings key with Some x -> x | None -> ""

    let key { keys } str =
      match Map.find keys str with Some x -> x | None -> Key.null
    ...
    let register idx str =
      if Map.mem idx.keys str then idx
        ...
        let key =
          Key.succ
          @@
          match Map.max_elt idx.strings with
          | None -> Key.null
          | Some (k, _) -> k
        in
        {
