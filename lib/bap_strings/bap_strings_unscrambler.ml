...
module Ascii = struct
  ...
  module Alpha = struct
    ...
    let length = 26 * 2
    let index c = if Char.is_uppercase c then c - 'A' else c - 'a' + 26
  end
  ...
  module Alphanum = struct
    module Caseless = struct
      ...
      let index c =
        if Char.is_alpha c then Alpha.Caseless.index c else Digits.index c + 26
    end

    let length = (26 * 2) + 10

    let index c =
      if Char.is_alpha c then Alpha.index c else Digits.index c + (26 * 2)
  end
...
module Make (Alpha : Alphabet) = struct
  type t = { dict : t Int.Map.t; data : string list }
      ...
      if i < Array.length count then
        {
          data;
          dict =
            Map.update dict count.(i) ~f:(function
              | None -> add empty (i + 1)
              | Some sub -> add sub (i + 1));
        }
      else { empty with data = word :: data }
    in
    add dict 0
  ...
  let of_files files = List.fold files ~init:empty ~f:add_from_file
end
