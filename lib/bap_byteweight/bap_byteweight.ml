...
struct
  ...
  let train ~max_length test set pass trie =
    let rec outer = function 0 -> () | n -> inner n 0
    and inner length m =
      match (Corpus.look set ~length m, pass) with
  ...
  let next_if (trie : t) ~length ~f set n =
    ...
    let rec loop n =
      ...
      match Trie.longest_match trie key with
      | None -> loop (n + 1)
      | Some (len, stats) -> if f key len stats then Some n else loop (n + 1)
    in
    loop n
...
module Make2 (Corpus : Corpus) (Trie : Trie.V2.S with type key = Corpus.key) =
struct
...
module Bytes = struct
  module Self =
    Make2
      ...
      (Memory.Trie.Stable.V1.R8)

  let t =
    Bap_byteweight_signatures.Data.declare "bytes"
      ~load:(fun bytes ->
        Binable.of_string (module Self) (Stdlib.Bytes.unsafe_to_string bytes))
      ~save:(fun data ->
        Stdlib.Bytes.unsafe_of_string @@ Binable.to_string (module Self) data)
  ...
  let find_using_bayes_factor sigs ~min_length ~max_length threshold =
    let s1, s0 =
      fold sigs ~init:(0, 0) ~f:(fun (s1, s0) key (h1, h0) ->
          let length = List.length key in
          if length >= min_length && length <= max_length then (h1 + s1, h0 + s0)
          else (s1, s0))
    in
    let ph1 = float s1 /. float (s1 + s0) in
    let ph0 = 1. -. ph1 in
    let ratio m n =
      let r = p1 m n /. p0 m n and q = ph1 /. ph0 in
      r *. q
    in
    find_if sigs ~length:max_length ~f:(fun _ length (h1, h0) ->
        length >= min_length && Float.(ratio h1 h0 > threshold))

  let find_using_threshold sigs ~min_length ~max_length threshold =
    find_if sigs ~length:max_length ~f:(fun _ length (h1, h0) ->
        length >= min_length && Float.(p1 h1 h0 > threshold))

  let pp_byte ppf x = Format.fprintf ppf "%02x" @@ Word.to_int_exn x

  let pp ppf t =
    fold t ~init:() ~f:(fun () words (a, b) ->
