...
let update_jmp jmp ~f =
  ...
  let jmp' =
    Ir_jmp.reify ~tid:(Term.tid jmp) ?cnd:(Ir_jmp.guard jmp) ?dst ?alt ()
  in
...
let intra_fall cfg block =
  KB.Seq.find_map (Cfg.Node.outputs block cfg) ~f:(fun e ->
      ...
      | `Fall ->
          ...
          Theory.Label.for_addr addr >>| fun tid -> Some (Ir_jmp.resolved tid)
      | _ -> !!None)
...
let maybe_new_tid = function None -> Theory.Label.fresh | Some tid -> !!tid
...
module IrBuilder = struct
  ...
  let set_attributes ?addr insn blks =
    let set_attributes k b =
      Term.map k b ~f:(fun t ->
          ...
          Option.value_map addr ~f:(Term.set_attr t address) ~default:t)
    in
    List.map blks ~f:(fun blk ->
        set_attributes jmp_t blk |> set_attributes def_t)
  ...
  let lift_insn ?addr insn blks =
    append blks @@ set_attributes ?addr insn (ir_of_insn insn)

  let with_first_blk_addressed addr = function
    | [] -> []
    | b :: bs -> Term.set_attr b address addr :: bs

  let is_unconditional jmp =
    match Ir_jmp.cond jmp with Bil.Int _ -> true | _ -> false
  ...
  let with_landing_pads return bs =
    match bs with
    | [] -> !![]
    | b :: bs as blks ->
        KB.List.fold ~init:[] blks ~f:(fun pads b ->
            ...
            |> KB.Seq.fold ~init:pads ~f:(fun pads jmp ->
                   ...
                   | Some pad -> pad :: pads
                   | None -> pads))
        >>| fun pads -> b :: List.rev_append pads bs
  ...
  let has_call blk =
    Term.enum jmp_t blk
    |> Seq.exists ~f:(fun jmp -> Option.is_some (Ir_jmp.alt jmp))

  let fall_if_possible dst blk =
    if is_last_jump_unconditional blk then !!blk
    else
      Theory.Label.fresh >>| fun tid ->
      Term.append jmp_t blk @@ Ir_jmp.reify ~tid ~dst ()

  let concat_map_fst_and_rev xs f =
    match xs with [] -> !![] | x :: xs -> f x >>| fun x -> List.rev (x @ xs)
  ...
  let maximize ?(is_call = false) ?(is_barrier = false) ?fall = function
    ...
    | x -> (
        ...
        | Some (`Intra dst) ->
            let blk =
              if is_call || has_call x then turn_into_call (Some dst) x else x
            in
        ...
        | Some (`Inter dst) ->
            if is_call || has_call x then
              ...
              turn_into_call (Some fall) x |> fall_if_possible fall
              >>| fun blk -> [ next; blk ]
  ...
  let insns ?fall ?addr insns =
    ...
    KB.List.fold insns ~init ~f:(fun (blks, _) insn ->
        lift_insn insn blks >>| fun blks -> (blks, Some insn))
    >>= function
    | blks, None -> !!blks
    | blks, Some x ->
        pack blks ?fall ?addr
  ...
  let fall_of_block symtab cfg block =
    ...
    | None -> (
        inter_fall symtab block >>| function
  ...
  let blk ?symtab cfg block : blk term list KB.t =
    ...
    Block.insns block
    |> KB.List.fold ~init ~f:(fun blks (mem, insn) ->
           let addr = Memory.min_addr mem in
           lift_insn ~addr insn blks)
    >>= fun blks ->
...
let lift_sub ?symtab ?tid entry cfg =
  ...
  |> Seq.to_list
  |> KB.List.map ~f:(IrBuilder.blk ?symtab cfg)
  >>| fun blks -> (Block.addr entry, List.concat blks, tid)
...
let alternate_nonlocal sub jmp =
  let needs_alternation =
    Option.is_none (Ir_jmp.alt jmp)
    &&
    match Ir_jmp.dst jmp with
    | None -> false (* already nonlocal *)
    | Some dst -> (
        match Ir_jmp.resolve dst with
        | Second _ -> true (* all unresolved are potentially calls *)
        | First dst -> Option.is_none (Term.find blk_t sub dst))
  in
  if needs_alternation then
    update_jmp jmp ~f:(fun dst _ jmp -> jmp ~dst:None ~alt:dst)
  else jmp
...
let link_call symtab addr sub_of_blk jmp =
  let resolve dst =
    Option.(
      dst jmp >>| Ir_jmp.resolve >>= function
      | Second _ -> None
      | First tid -> Some tid)
  in
  let sub_of_dst dst =
    Option.(resolve dst >>= Hashtbl.find sub_of_blk >>| Ir_jmp.resolved)
  in
  let external_callee () =
    match addr with
    | None -> !!None
    | Some addr -> (
        match Symtab.explicit_callee symtab addr with
        | None -> !!None
        | Some name ->
            Theory.Label.for_name name >>| fun tid -> Some (Ir_jmp.resolved tid)
        )
  in
  match (sub_of_dst Ir_jmp.dst, sub_of_dst Ir_jmp.alt) with
  | _, (Some _ as alt) -> !!(update_jmp jmp ~f:(fun dst _ jmp -> jmp ~dst ~alt))
  | (Some _ as alt), None ->
      !!(update_jmp jmp ~f:(fun _ _ jmp -> jmp ~dst:None ~alt))
  | None, None -> (
      match resolve Ir_jmp.alt with
      | None -> !!jmp
      | Some (_ : tid) -> (
          external_callee () >>| function
          | Some alt ->
              update_jmp jmp ~f:(fun dst _ jmp -> jmp ~dst ~alt:(Some alt))
          | None -> jmp))
...
let make_name ?name tid =
  ...
  | None -> (
      ...
      | Some name -> !!name
      | None -> (
          KB.collect Theory.Label.ivec tid >>| function
...
let create_synthetic ?blks ?name tid =
  ...
  let tags =
    List.filter_opt
      [
        Some Term.synthetic; Option.some_if (is_intrinsic sub) Ir_sub.intrinsic;
      ]
  in
  List.fold tags ~init:sub ~f:(fun sub tag -> Term.set_attr sub tag ())

let has_sub prog tid = Option.is_some (Term.find sub_t prog tid)
                       ...
                       | Some dst -> (
                           ...
                           | First dst ->
                               if has_sub prog dst then !!prog
                                 ...
                                 create_synthetic dst >>| Term.append sub_t prog
                           ))))
...
let lift_insn_nonempty insn =
  if KB.Value.is_empty insn then !!None else lift_insn insn >>| Option.some
...
let reify_externals symtab prog =
  ...
  |> KB.Seq.fold ~init:prog ~f:(fun prog (tid, insn) ->
         if has_sub prog tid then !!prog
           ...
           lift_insn_nonempty insn >>= fun blks ->
...
let mangle_name addr tid name =
  match addr with
  | Some a -> sprintf "%s@%s" name @@ Bap_bitvector.string_of_value ~hex:true a
...
let fix_names prog =
  let is_new tid name = Theory.Label.for_name name >>| Fn.non (Tid.equal tid) in
