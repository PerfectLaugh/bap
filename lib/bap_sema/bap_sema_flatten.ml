...
let get_direct_typ (e : exp) : Type.t =
  match e with
  | Bil.Var v -> Var.typ v
  | Bil.Unknown (_, t) -> t
...
class substituter (x : var) (x' : var) =
  object
    inherit Exp.mapper as super
    method! map_var v = if Var.equal x v then Var x' else super#map_var v
...
let new_def var e = Theory.Label.fresh >>| fun tid -> Ir_def.create ~tid var e

let flatten_exp ?(before : tid option = None) (exp : exp) (blk : blk term) :
    (exp * blk term) KB.t =
  let insert blk def =
    match before with
    | None -> Term.append def_t blk def
    | Some before -> Term.prepend def_t ~before blk def
  in
  let rec aux (exp : exp) (blk : blk term) =
    match exp with
    | Bil.Load (x, y, endian, s) ->
        ...
        new_def var e >>| fun def -> (Bil.Var var, insert blk def)
  ...
  let rec flatten_elts (elts : Ir_blk.elt seq) (blk : blk term) =
    let rec flatten_jmp (jmp : Ir_jmp.t) (expseq : exp seq) (blk : blk term) =
      ...
      | Some (hd, tl) ->
          ...
          Ir_jmp.substitute jmp hd exp
          |> Term.update jmp_t blk |> flatten_jmp jmp tl
      | None -> !!blk
    in
    ...
    | Some (hd, tl) -> (
        ...
        | `Def def ->
            ...
            Ir_def.with_rhs def exp |> Term.update def_t blk |> flatten_elts tl
