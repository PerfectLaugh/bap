...
let reg =
  Value.Tag.register ~name:"tainted-reg"
    ~uuid:"1ab9a363-db8f-4ab4-9fb4-5ff54de97c5c"
    (module Tid)

let ptr =
  Value.Tag.register ~name:"tainted-ptr"
  ...
  include Regular.Make (struct
    ...
    let pp_list pp ppf xs =
      let rec pp_rest ppf = function
        | [] -> ()
        | [ x ] -> pp ppf x
        | x :: xs -> fprintf ppf "%a,@;%a" pp x pp_rest xs
      in
      pp_rest ppf xs

    let pp_taints = pp_list Taint.pp
    let pp_taint_set ppf t = fprintf ppf "@[<1>[%a@]]" pp_taints (Set.to_list t)
    let pp_binding ppf (v, ts) = fprintf ppf "%a => %a" Var.pp v pp_taint_set ts
    let pp_vars ppf t = pp_list pp_binding ppf (Map.to_alist t)
    let pp ppf t = fprintf ppf "@[<1>{%a@]}" pp_vars t
    let hash = Hashtbl.hash
  end)
end

let regs : map tag =
  Value.Tag.register ~name:"tainted-regs"
    ~uuid:"03c90a60-e19f-43cc-8049-fdeb23973396"
    (module Taint_map)

let ptrs : map tag =
  Value.Tag.register ~name:"tainted-ptrs"
    ~uuid:"ecf96df5-f706-4f95-a421-3fa9b91ad8bd"
    (module Taint_map)

let create = Fn.id

let get_taints from key =
  match Map.find from key with None -> Taints.empty | Some ts -> ts

let collect_taints =
  Map.fold ~init:Taints.empty ~f:(fun ~key:_ ~data:ts ts' -> Set.union ts ts')
...
class context =
  object (self)
    ...
    method taint_reg r ts =
      ...
      if Set.is_empty ts then {<tvs = Map.remove tvs key>}
      else {<tvs = Map.set tvs ~key ~data:ts>}

    method taint_ptr a (s : size) ts =
      if Set.is_empty ts then self
      else if Size.equal s `r8 then {<tas = Map.set tas ~key:a ~data:ts>}
      else
        let addrs = Seq.init (Size.in_bytes s) ~f:(fun n -> Addr.(a ++ n)) in
        let tas' =
          Seq.fold addrs ~init:tas ~f:(fun tas a -> Map.set tas ~key:a ~data:ts)
        in
        {<tas = tas'>}
...
let merge : map -> map -> map = union_map ~f:Set.union

let pp_set ppf (set : set) =
  let rec pp ppf = function
    | [] -> ()
    | [ x ] -> Taint.pp ppf x
    | x :: xs -> Format.fprintf ppf "%a;@ %a" Taint.pp x pp xs
  in
...
let pp_map ppf (map : map) =
  ...
  Map.iteri map ~f:(fun ~key ~data ->
      Format.fprintf ppf "@[<2>%a => %a@]}" Var.pp key pp_set data)
...
module Make (SM : Monad.State.S2) = struct
  ...
  class ['a] propagator =
    object (self)
      ...
      method! eval_unop op e = super#eval_unop op e >>= self#eval e
      method! eval_cast ct n e = super#eval_cast ct n e >>= self#eval e
      method! eval_concat e1 e2 = super#eval_concat e1 e2 >>= self#eval2 e1 e2
      method! eval_extract n1 n2 e = super#eval_extract n1 n2 e >>= self#eval e
      ...
      method private eval2 e1 e2 r3 =
        ...
        self#propagate [ r1; r2 ] r3 >>= fun () -> SM.return r3

      method private eval e rr =
        self#eval_exp e >>= fun re ->
        self#propagate [ re ] rr >>= fun () -> SM.return rr

      method private propagate args rr =
        SM.update @@ fun ctxt ->
        List.map args ~f:ctxt#reg_taints
        |> Taints.union_list |> ctxt#taint_reg rr
    end
end
