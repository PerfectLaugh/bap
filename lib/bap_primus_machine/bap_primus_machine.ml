...
type outcome = Continue | Stop | Info of Info.t

module Observer = struct
  type ('f, 'm) t = { id : int; observe : (outcome -> 'm) -> 'f }
end

module Inspector = struct
  type 'm t = { id : int; inspect : Info.t -> 'm }
end
...
module Primus = struct
  ...
  type exit_status = Normal | Exn of Exception.t
  ...
  let exn_raised =
    Observation.declare ~package "machine-exception" ~inspect:(fun k exn ->
        k (Info.of_string (Exception.to_string exn)))

  let forked =
    Observation.declare ~package "machine-fork" ~inspect:(fun k pid cid ->
        k @@ Info.create "machine-fork" (pid, cid) [%sexp_of: Id.t * Id.t])

  let switched =
    Observation.declare ~package "machine-switch"
      ~inspect:(fun k parent child ->
        k
        @@ Info.create "machine-switch" (parent, child) [%sexp_of: Id.t * Id.t])
  ...
  let with_global_context (f : unit -> 'a t) =
    ...
    f () >>= fun r ->
    lifts (SM.switch id) >>| fun () -> r

  let get_local () : _ t = lifts (SM.gets @@ fun s -> s.local)

  let get_global () : _ t =
    with_global_context @@ fun () -> lifts (SM.gets @@ fun s -> s.global)

  let set_local local = lifts @@ SM.update @@ fun s -> { s with local }

  let set_global global =
  ...
  module Observation : sig
    ...
    let observations { observer } : _ observer list m =
      lifts @@ SM.gets @@ fun s -> Univ_map.Multi.find s.observations observer

    let set_observations { observer } obs =
      lifts @@ SM.update
      @@ fun s ->
      { s with observations = Univ_map.Multi.set s.observations observer obs }

    let set_inspectors { Observation.name } data =
      lifts @@ SM.update
      @@ fun s -> { s with inspectors = Map.set s.inspectors ~key:name ~data }

    let inspectors { name } =
      lifts @@ SM.gets @@ fun { inspectors } -> Map.find_multi inspectors name
    ...
    let provide observation ~f:k =
      ...
      else kill_observers obs kill |> set_observations observation
    ...
    let monitor { observer } ~f =
      ...
      @@ fun s ->
      {
        ...
        key = s.key + 1;
        observations =
          Univ_map.Multi.add s.observations observer
            Observer.{ id = s.key + 1; observe = f };
      }
    ...
    let inspect { name } ~f =
      ...
      @@ fun s ->
      {
        ...
        key = s.key + 1;
        inspectors =
          Map.add_multi s.inspectors ~key:name
            ~data:Inspector.{ id = s.key + 1; inspect = f };
      }
  ...
  struct
    ...
    let get : 'a state -> 'a machine =
     fun { State.key; init } ->
      S.get () >>= fun states ->
      match Univ_map.find states key with
      | Some s -> return s
      | None -> project >>= fun proj -> liftk (init proj)

    let put { State.key } x =
      S.get () >>= fun states -> S.set (Univ_map.set states key x)

    let update data ~f = get data >>= fun s -> put data (f s)
  end
  ...
  let notify_fork pid =
    current () >>= fun cid ->
    Observation.provide forked ~f:(fun observe -> observe pid cid)

  let sentence_to_death id =
    with_global_context (fun () ->
        lifts @@ SM.update (fun s -> { s with deathrow = id :: s.deathrow }))
  ...
  let switch id : unit c =
    C.call ~f:(fun ~cc:k ->
        ...
        Observation.provide switched ~f:(fun observe -> observe pid id)
        >>= fun () -> s.curr ())
  ...
  let kill id =
    if Id.(id = global) then return ()
      ...
      current () >>= fun cid ->
      if Id.(id = cid) then sentence_to_death id else lifts @@ SM.kill id

  let die next =
    current () >>= fun pid ->
    switch_state next >>= fun () ->
    lifts (SM.get ()) >>= fun s ->
    lifts (SM.kill pid) >>= fun () -> s.curr ()

  let raise exn =
    Observation.provide exn_raised ~f:(fun observe -> observe exn) >>= fun () ->
    fail exn
  ...
  let finished =
    Observation.declare ~package "fini" ~inspect:(fun k () ->
        k @@ Info.of_string "fini")

  let inited =
    Observation.declare ~package "init" ~inspect:(fun k () ->
        k @@ Info.of_string "init")
  ...
  let run : type a. a t -> a e =
   fun comp proj ->
    ...
    Knowledge.ignore_m @@ SM.run (C.run comp finish) state
  ...
  include (CM : Monad.S with type 'a t := 'a t and module Syntax := Syntax)
end
