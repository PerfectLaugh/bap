...
module Std = struct
  ...
  type name = ..
  type language = { name : name; repr : string }
  type 'a key = 'a Tid.t
  ...
  module Key = struct
    ...
    let create { repr; name } =
      (name_to_string := fun n -> if n = name then repr else !name_to_string n);
    ...
    include Printable.Make (struct
      ...
      let pp ppf { repr } = pp_print_string ppf repr
    end)
    ...
    module Name = struct
      ...
      include Printable.Make (struct
        ...
        let pp ppf n = pp_print_string ppf (!name_to_string n)
      end)
    end
