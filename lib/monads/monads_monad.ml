...
module Choice = struct
  ...
  module Make (M : Basic) : S with type 'a t := 'a M.t = Make2 (struct
    type ('a, 'e) t = 'a M.t
...
module Monad = struct
  ...
  module Make2 (M : B2) : S2 with type ('a, 'e) t := ('a, 'e) M.t = struct
    ...
    module Fn = struct
      ...
      let apply_n_times ~n f x =
        let rec loop n x = if n <= 0 then return x else f x >>= loop (n - 1) in
        loop n x
    ...
    module Collection = struct
      ...
      module type Base = sig
        ...
        val foldl :
          'a t -> init:'b -> f:('b -> 'a -> ('b, 'e) M.t) -> ('b, 'e) M.t

        val foldr :
          'a t -> f:('a -> 'b -> ('b, 'e) M.t) -> init:'b -> ('b, 'e) M.t

        val find_map :
          'a t -> f:('a -> ('b option, 'e) M.t) -> ('b option, 'e) M.t
      end
      ...
      module Eager_base (T : Types.Collection.Eager) = struct
        ...
        let foldl xs ~init ~f =
          T.fold xs
            ~init:(fun k -> k init)
            ~f:(fun k x k' -> k (fun a -> f a x >>= k'))
            M.return

        let foldr xs ~f ~init =
          T.fold xs ~init:M.return ~f:(fun k x -> fun a -> f x a >>= k) init

        let find_map xs ~f =
          foldl xs ~init:None ~f:(fun r x ->
              match r with None -> f x | r -> M.return r)
      end
      ...
      module Delay_base (T : Types.Collection.Delay) = struct
        ...
        let fold xs ~init ~f = T.fold xs ~init ~f:(fun a x k -> k (f a x)) ident
        ...
        let find_map xs ~f =
          T.fold xs ~init:()
            ~f:(fun () x k -> f x >>= function None -> k () | x -> M.return x)
            (fun () -> M.return None)
      end
      ...
      module Make (T : Base) :
        ...
        let reduce xs ~f =
          fold xs ~init:None ~f:(fun acc y ->
              match acc with
              | None -> return (Some y)
              | Some x -> f x y >>| fun z -> Some z)

        let exists xs ~f =
          T.find_map xs ~f:(fun x ->
              f x >>| function true -> Some () | false -> None)
          >>| Option.is_some
        ...
        let count xs ~f =
          fold xs ~init:0 ~f:(fun n x ->
              f x >>| function true -> n + 1 | false -> n)

        let map_reduce (type a) (module M : Monoid.S with type t = a) xs ~f =
          fold xs ~init:M.zero ~f:(fun x y -> f y >>| M.plus x)

        let find xs ~f =
          let f x = f x >>| function true -> Some x | false -> None in
          find_map xs ~f

        let filter_map xs ~f =
          let empty = T.zero () in
          fold_right xs ~init:empty ~f:(fun x ys ->
              f x >>| function None -> ys | Some y -> prepend ys y)

        let filter xs ~f =
          filter_map xs ~f:(fun x ->
              f x >>| function true -> Some x | false -> None)
      end
    ...
    module List = Collection.Delay (struct
      ...
      let fold xs ~init ~f =
        let rec loop xs k =
          match xs with
          | [] -> k
          | x :: xs -> fun k' -> k (fun a -> loop xs (f a x) k')
        in
    ...
    module Seq = Collection.Delay (struct
      ...
      let fold xs ~init ~f finish =
        Sequence.delayed_fold xs ~init ~f:(fun a x ~k -> f a x k) ~finish
  ...
  module Make (M : B) : S with type 'a t := 'a M.t = Make2 (struct
    type ('a, 'e) t = 'a M.t
...
module Ident : Monad.S with type 'a t = 'a = struct
  type 'a t = 'a
  ...
  module Collection = struct
    ...
    module Eager (T : Types.Collection.Eager) : S with type 'a t := 'a T.t =
    struct
    ...
    module Delay (T : Types.Collection.Delay) : S with type 'a t := 'a T.t =
    struct
  ...
  module Make2 (M : Monad.S2) :
    S2
      with type ('a, 'e) t := ('a, 'e) T2(M).t
       ...
       and type ('a, 'e) e := ('a, 'e) T2(M).e = struct
    open M.Syntax
    ...
    module Basic = struct
      ...
      let bind m f =
        M.bind m ~f:(function Some r -> f r | None -> M.return None)
      [@@inline]
    ...
    let run = ident
    let catch m f = m >>= function None -> f () | other -> M.return other

    let plus m1 m2 =
      m1 >>= function Some m1 -> M.return (Some m1) | None -> m2
  ...
  module Make (M : Monad.S) :
    S
      with type 'a m := 'a T1(M).m
       ...
       and type 'a e := 'a T1(M).e = Make2 (struct
    type ('a, 'e) t = 'a M.t
    type 'a error = unit
...
module ResultT = struct
  ...
  module Makep (T : T1) (M : Monad.S) :
    ...
    module Base = struct
      ...
      let bind m f : ('a, 'e) t =
        m >>= function Ok r -> f r | Error err -> M.return (Error err)
      [@@inline]
      ...
      let catch m f =
        m >>= function Error err -> f err | other -> M.return other
  ...
  module Error = struct
    ...
    module type S = sig
      ...
      val failf :
        ('a, Stdlib.Format.formatter, unit, unit -> 'b t) format4 -> 'a
    end
  ...
  module Self :
    S2
      with type ('a, 'e) t = ('a, 'e) result
       and type 'a m = 'a
       and type ('a, 'e) e = ('a, 'e) result = struct
    include T2 (Ident)
    include Make2 (Ident)
  end
    ...
    module Base = struct
      ...
      let bind xsm f =
        ...
        List.fold xs ~init:!![] ~f:(fun ysm x ->
            ysm >>= fun ys ->
            f x >>| fun xs -> List.rev_append xs @@ ys)
        >>| List.rev
...
module Seq = struct
  ...
  module Make2 (M : Monad.S2) :
    ...
    module Base = struct
      ...
      let bind xsm f =
        ...
        Sequence.fold xs ~init:!!Sequence.empty ~f:(fun ysm x ->
            ysm >>= fun ys ->
            f x >>| fun xs -> Sequence.append xs ys)
    ...
    let plus xsm ysm =
      xsm >>= fun xs ->
      ysm >>= fun ys -> M.return (Sequence.append xs ys)
  ...
  module SeqM :
    S
      with type 'a t = 'a Sequence.t
       and type 'a m = 'a
       and type 'a e = 'a Sequence.t = struct
    include T1 (Ident)
    include Make (Ident)
  end
...
module Writer = struct
  ...
  module Make2 (T : Monoid.S) (M : Monad.S2) :
    S2
      with type ('a, 'e) m := ('a, 'e) T2(T)(M).m
       and type ('a, 'e) t := ('a, 'e) T2(T)(M).t
       and type ('a, 'e) e := ('a, 'e) T2(T)(M).e
       and type state := T2(T)(M).state = struct
    open M.Syntax
    ...
    module Base = struct
      ...
      let bind m f =
        m >>= fun (Writer (x, a)) ->
        f x >>| fun (Writer (x, b)) -> writer (x, a + b)
  ...
  module Make (T : Monoid.S) (M : Monad.S) :
    S
      with type 'a m := 'a T1(T)(M).m
       ...
       and type 'a e := 'a T1(T)(M).e
       and type state := T1(T)(M).state = struct
    module M = struct
      type ('a, 'e) t = 'a M.t
  ...
  module Makep (T : T1) (M : Monad.S) :
    Sp
      with type ('a, 'e) t := ('a, 'e) Tp(T)(M).t
       and type 'a m := 'a Tp(T)(M).m
       and type ('a, 'e) e := ('a, 'e) Tp(T)(M).e
       and type 'a env := 'a Tp(T)(M).env = struct
    module Base = struct
      include Tp (T) (M)
      open M.Monad_infix
...
module State = struct
  ...
  module Makep (T : T1) (M : Monad.S) :
    ...
    let lift m = make @@ fun s -> M.bind m ~f:(fun x -> M.return { x; s })
    [@@inline]
  ...
  module Multi = struct
    ...
    module Makep (T : T1) (M : Monad.S) :
      ...
      let alive k id : id = if Map.mem k.forks id then id else alive_parent k id
      ...
      let context k =
        ...
        if id = global then k.init else Map.find_exn k.forks k.current
      ...
      let put ctxt =
        SM.update @@ fun k ->
        let id = alive k k.current in
        if id = global then { k with init = ctxt }
        else { k with forks = Map.set k.forks ~key:id ~data:ctxt }

      let parent () = SM.get () >>| fun k -> alive_parent k k.current

      let status id =
        SM.get () >>| fun k ->
        if id = k.current then `Current
        else if Map.mem k.forks id then `Live
        else `Dead

      let remove_dead_parents k =
        let parents =
          Map.fold k.forks ~init:Id.Map.empty ~f:(fun ~key ~data:_ ps ->
              Map.set ps ~key ~data:(alive_parent k key))
        in
        let children = Map.filter_keys k.children ~f:(Map.mem parents) in
        { k with children; parents }

      let gc k = if k.created mod 1024 = 0 then remove_dead_parents k else k
      ...
      let modify m f =
        m >>= fun x ->
        update f >>= fun () -> SM.return x

      let run m =
       fun ctxt ->
        M.bind (SM.run m (init ctxt)) ~f:(fun (x, cs) -> M.return (x, cs.init))
...
module LazyT = struct
  ...
  module Make2 (M : Monad.S2) :
    ...
    module Base = struct
      ...
      let bind m f = lazy (Lazy.force m >>= fun x -> Lazy.force (f x))
      let map = `Define_using_bind
      let run = Lazy.force
      let lift x = lazy x
    end
  ...
  module Self :
    S with type 'a t = 'a Lazy.t and type 'a m = 'a and type 'a e = 'a = struct
    type 'a t = 'a Lazy.t
    type 'a m = 'a
    type 'a e = 'a
...
module Cont = struct
  ...
  module Makep (T : T1) (M : Monad.S) :
    Sp
      with type ('a, 'e) t := ('a, 'e) Tp(T)(M).t
       and type ('a, 'e) e := ('a, 'e) Tp(T)(M).e
       and type 'a m := 'a Tp(T)(M).m
       and type 'a r := 'a Tp(T)(M).r = struct
    open M.Syntax
    ...
    module Base = struct
      ...
      let call ~f =
        cont @@ fun k -> run (f ~cc:(fun x -> cont @@ fun _ -> k x)) k
  ...
  module Self :
    S2
      with type ('a, 'e) t = ('a, 'e) cont
       and type 'a m = 'a
       and type ('a, 'e) e = ('a -> 'e) -> 'e = struct
