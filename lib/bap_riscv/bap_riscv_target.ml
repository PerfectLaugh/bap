...
let parent =
  Theory.Target.declare ~package "riscv" ~endianness:Theory.Endianness.le
...
let riscv t =
  ...
  Theory.Target.declare ~package name ~parent ~bits ~code:mem ~data:mem ~vars
    ~regs:
      Theory.Role.Register.
        [
          ...
          ( [ caller_saved ],
            ...
            @ f (28 -- 31) );
          ( [ callee_saved ],
            x [ 2 ] @ x (8 -- 9) @ x (18 -- 27) @ f (8 -- 9) @ f (18 -- 27) );
        ]
