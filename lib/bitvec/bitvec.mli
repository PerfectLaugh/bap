...
(** [(x <op> y) mod m] applies operation [<op>] modulo [m].
    ...
    Note: the [mod] function is declared as a primitive to enable support for
    inlining in non flambda versions of OCaml. Indeed, underneath the hood the
    ['a m] type is defined as the reader monad [modulus -> 'a], however we don't
    want to create a closure every time we compute an operation over bitvectors.
    With this trick, all versions of OCaml no matter the optimization options
    will inline [(x+y) mod m] and won't create any closures, even if [m] is not
...
module type S = sig
  ...
  (** [abs x mod m] absolute value of [x] modulo [m].
      ...
      The absolute value of [x] is equal to [neg x] if [msb x] and to [x]
      otherwise. *)
  ...
  (** [div x y mod m] is [x / y] modulo [m],
      ...
      where [/] is the truncating towards zero division, that returns [ones m]
      if [y = 0]. *)

  val sdiv : t -> t -> t m
  (** [sdiv x y mod m] is signed division of [x] by [y] modulo [m],

      The signed division operator is defined in terms of the [div] operator as
      follows:
      {v
                        /
  ...
  (** [srem x y mod m] is the signed remainder [x / y] modulo [m].
      ...
      This version of the signed remainder where the sign follows the dividend,
      and is defined via the [rem] operation as follows
  ...
  (** [smod x y mod m] is the signed remainder of [x / y] modulo [m].
      ...
      This version of the signed remainder where the sign follows the divisor,
      and is defined in terms of the [rem] operation as follows:
  ...
  *)
  ...
  (** [arshift x y mod m] shifts [x] right by [y] with [msb x] filling.
      ...
      Returns [ones mod m] if [y >= m /\ msb x mod m] and [zero] if
      [y >= m /\ msb x mod m = 0] *)

  val gcd : t -> t -> t m
  (** [gcd x y mod m] returns the greatest common divisor modulo [m]

      [gcd x y] is the meet operation of the divisibility lattice, with [0]
      being the top of the lattice and [1] being the bottom, therefore
      [gcd x 0 = gcd x 0 = x]. *)

  val lcm : t -> t -> t m
  (** [lcm x y mod] returns the least common multiplier modulo [m].

      [lcm x y] is the meet operation of the divisibility lattice, with [0]
      being the top of the lattice and [1] being the bottom, therefore
      [lcm x 0 = lcm 0 x = 0] *)
  ...
  (** [(g,a,b) = gcdext x y mod m], where
      ...
      The operation is well defined if one or both operands are equal to [0], in
      particular:
      - [(x,1,0) = gcdext(x,0)],
      - [(x,0,1) = gcdext(0,x)]. *)
...
val compare : t -> t -> int
(** [compare x y] compares [x] and [y] as unsigned integers, i.e., [compare x y]
...
(** [pp ppf x] is a pretty printer for the bitvectors.
    ...
    Could be used standalone or as an argument to the [%a] format specificator,
    e.g.,
...
(** [to_string x] returns a textual (human readable) representation of the
    bitvector [x]. *)
...
(** [bigint_unsafe x] directly interprets a zarith value as bitvector.
    ...
    The function is safe only if [x] is the result of [to_bigint], otherwise the
    resulting value is unpredictable.

    @since 2.5.0 *)

val of_substring : ?pos:int -> ?len:int -> string -> t
(** [of_substring ~pos ~len s] is a bitvector that corresponds to a substring of
    [s] designated by the start position [pos] and length [len].

    The result is the same as [of_string (String.sub s pos len)], except that no
    intermediate substring is created.

    @param pos is the starting position of the substring (defaults to [0])
    @param len is the length of the substring (defaults to the length of [s].

    @raise Invalid_argument
      if [pos] and [len] together do not define a valid substring of [s] or if
      the substring is not a sequence of numbers of the base [b]. *)

val of_string_base : int -> string -> t
(** [of_string_base b x] is a bitvector that corresponds to [s] represented in
    base [b].

    The base [b] must be between [2] and [16]. The textual representation shall
    not contain a prefix that designates the base and must be a sequence of
    digits that are less than [b].
...
(** [of_substring_base b x] is a bitvector that corresponds to a substring of
    [s] designated by the start position [pos] and length [len] that is a
    sequence of digits in base [b].

    The result is the same as [of_string_base b x (String.sub s pos len)],
...
val fits_int : t -> bool
(** [fits_int x] is [true] if [x] could be represented with the OCaml [int]
    type.

    Note: it is not always true that [fits_int (int x mod m)], since depending
    on [m] a negative number might not fit into the OCaml representation. For
    positive numbers it is true, however. *)
...
(** [fits_int32 x] is [true] if [x] could be represented with the OCaml [int]
    type.

    Note: it is not always true that [fits_int32 (int32 x mod m)], since
    depending on [m] the negative [x] may not fit back into the [int32]
    representation. For positive numbers it is true, however. *)

val to_int32 : t -> int32
(** [to_int32 x] returns an OCaml integer that has the same representation as
    [x].

    The function is undefined if [not (fits_int32 x)]. *)

val fits_int64 : t -> bool
(** [fits_int64 x] is [true] if [x] could be represented with the OCaml [int]
    type.

    Note: it is not always true that [fits_int64 (int64 x mod m)], since
    depending on [m] the negative [x] might not fit back into the [int64]
    representation. For positive numbers it is true, however. *)

val to_int64 : t -> int64
(** [to_int64 x] returns an OCaml integer that has the same representation as
    [x].
...
(** [select bits x] builds a bitvector from [bits] of [x].
    ...
    Returns a bitvector [y] such that [nth] bit of it is equal to
    [List.nth bits n] bit of [x].

    Returns [zero] if [bits] are empty. *)

val append : int -> int -> t -> t -> t
(** [append m n x y] takes [m] bits of [x] and [n] bits of [y] and returns their
    concatenation. The result has [m+n] bits.
...
(** [concat m xs] concatenates [m] bits of each [x] in [xs].
    ...
    The operation is the reduction of the [append] operation with [m=n]. The
    result has [m * List.length xs] bits and is equal to [0] if [xs] is empty.
*)
...
(** [module Mx = Make(Modulus)] produces a module [Mx] which implements all
    operation in [S] modulo [Modulus.modulus], so that all operations return a
    bitvector directly. *)
...
(** [M1] specializes [Make(struct let modulus = m1 end)]
    ...
    The specialization relies on a few arithmetic equalities and on an efficient
    implementation of the modulo operation as the [even x] aka [lsb x]
    operation. *)

module M8 : D
(** [M8] specializes [Make(struct let modulus = m8 end)]

    This specialization relies on a fact, that 8 bitvectors always fit into
    OCaml integer representation, so it avoids calls to the underlying arbitrary
    precision arithmetic library. *)

module M16 : D
(** [M16] specializes [Make(struct let modulus = m16 end)]

    This specialization relies on a fact, that 16 bitvectors always fit into
    OCaml integer representation, so it avoids calls to the underlying arbitrary
    precision arithmetic library.
    @since 2.5.0 *)
...
(** [M32] specializes [Make(struct let modulus = m32 end)]
    ...
    This specialization relies on a fact, that 32 bitvectors always fit into
    OCaml integer representation, so it avoids calls to the underlying arbitrary
    precision arithmetic library. *)
...
(** [M64] specializes [Make(struct let modulus = m64 end)]
    ...
    This specialization tries to minimize calls to the arbitrary precision
    arithmetic library whenever, it is known that the result will not overflow
    the OCaml int representation. *)

val modular : int -> (module D)
(** [modular n] returns a module [M], which implements all operations in [S]
