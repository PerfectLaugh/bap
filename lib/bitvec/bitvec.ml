...
*)
let norm { m } x = if Z.sign x < 0 || Z.geq x m then Z.(x land m) else x
[@@inline]
...
let m32 = modulus 32
let m64 = modulus 64
let compare x y = if x == y then 0 else Z.compare x y [@@inline]
...
let div x y m = if Z.(y = zero) then ones m else norm m @@ Z.div x y [@@inline]
...
let rem x y m = if Z.(y = zero) then x else norm m @@ Z.rem x y [@@inline]
...
let lshift x n m =
  shift n m ~overshift:zero ~in_bounds:(fun _ n -> norm m @@ Z.shift_left x n)
[@@inline]

let rshift x n m =
  shift n m ~overshift:zero ~in_bounds:(fun _ n -> norm m @@ Z.shift_right x n)
[@@inline]
...
let gcdext x y m =
  if Z.(equal x zero) then (y, zero, one)
...
let nonnegative r =
  if Z.sign r < 0 then
    invalid_arg "invalid string representation, sign is not expected";
  r
[@@inline]

let defaults_to_length ?len x =
  match len with None -> String.length x | Some n -> n
...
let of_substring ?(pos = 0) ?len x =
  nonnegative @@ Z.of_substring x ~pos ~len:(defaults_to_length ?len x)

let of_substring_base ?(pos = 0) ?len b x =
  nonnegative @@ Z.of_substring_base b x ~pos ~len:(defaults_to_length ?len x)
...
let doesn't_fit r x = failwith (to_string x ^ " doesn't fit the " ^ r ^ " type")

let convert tname size convert max_signed max_unsigned x =
  if Z.(x <= max_signed) then convert x
  else if Z.(x <= max_unsigned) then convert (Z.signed_extract x 0 size)
  else doesn't_fit tname x

let to_int x = convert "int" Sys.int_size Z.to_int max_sint max_uint x
[@@inline]

let to_int32 x = convert "int32" 32 Z.to_int32 max_sint32 max_uint32 x
[@@inline]

let to_int64 x = convert "int" 64 Z.to_int64 max_sint64 max_uint64 x [@@inline]
