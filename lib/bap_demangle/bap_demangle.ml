...
module Std = struct
  type demangler = { name : KB.Name.t; run : string -> string }
  ...
  module Demangler = struct
    ...
    let create ?package name run =
      ...
      if Hashtbl.mem registry name then
        failwithf
          "The demangler %s is already registered, please pick a unique name"
          (KB.Name.show name) ();
    ...
    let strip_leading_underscore =
      ...
      match String.chop_prefix s ~prefix:"_" with None -> s | Some s -> s
  end
  ...
  module Demanglers = struct
    ...
    let install target demangler =
      ...
      | `Duplicate ->
          ...
          failwithf
            "Failed to install demangler %s to the target %s, which already \
             has the demangler %s."
    ...
    let bad_name name =
      ...
      in
      invalid_argf
        "Failed to find a demangler named %s, the list of available \
         demanglers: %s"
    ...
    let select target =
      match Hashtbl.find selected target with
      | None -> Demangler.id
      | Some d -> d

    let lookup ?package name = try Some (get ?package name) with _ -> None
  end
end
