...
(** execution context.
    ...
    @param max_loop
      the maximum number of loop iterations (note only loops inner to a function
      are detected, so recursive loops may stall the interpreter. *)
class context : ?max_steps:int -> ?max_loop:int -> program term -> object ('s)
  [@@@warning "-D"]
...
(** BIR interpreter, that executes a program in a given context.
    ...
    @param deterministic
      if is [true] then only feasible execution path is taken, otherwise, the
      interpreter, will execute all linearly independent paths. *)
