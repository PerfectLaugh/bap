...
let visit vis tid =
  Map.change vis tid ~f:(function None -> Some 1 | Some n -> Some (n + 1))
...
class context ?(max_steps = Int.max_value) ?(max_loop = min 10 (max_steps / 10))
  p =
  object (self : 's)
    [@@@warning "-D"]
    ...
    method merge runner = {<vis = runner#visited; cps = runner#checkpoints>}
    ...
    method leave_sub (_ : sub term) =
      match callstack with
      | _ :: top -> {<callstack = top>}
      | [] -> {<callstack = []>}

    method step =
      if steps_left > 0 then Some {<steps_left = steps_left - 1>} else None

    method visit_term tid = {<vis = visit vis tid>}

    method will_loop tid =
      match callstack with
      | [] -> false
      | sub :: _ -> (
          match Map.find vis tid with
          | None -> false
          | Some n -> n > max_loop && (Option.is_some @@ Term.find blk_t sub tid)
          )

    method will_return tid =
      match callstack with
      | cur :: _ ->
...
class ['a] main ?(deterministic = false) p =
  object (self)
    ...
    method private return =
      SM.update @@ fun ctxt ->
      match ctxt#backtrack with None -> ctxt#return | Some next -> next
    ...
    method private next_of_jmp jmp =
      ...
      let next =
        ...
        | Call dst -> goto (Call.target dst)
      in
      SM.put ctxt >>= fun () -> next
