...
type param = { name : string; defl : string; desc : string }
...
type env = { vars : string Map.M(String).t; paths : string list }
type workspace = { path : string; temp : bool; main : string }
...
let empty_spec = { cmd = None; uses = []; opts = []; pars = [] }
...
let item_of_sexp = function
  | Sexp.List [ Sexp.Atom "extend"; Sexp.Atom x ] -> Ok (Use x)
  | Sexp.List
      [ Sexp.Atom "parameter"; Sexp.Atom name; Sexp.Atom defl; Sexp.Atom desc ]
    ->
      Ok (Par { name; defl; desc })
  | Sexp.List (Sexp.Atom "option" :: Sexp.Atom name :: args) ->
      atoms args >>| fun args -> Opt (name, args)
  | Sexp.List (Sexp.Atom "parameter" :: Sexp.Atom s :: _) -> Error (Bad_param s)
...
let spec_of_items =
  List.fold ~init:empty_spec ~f:(fun spec -> function
...
let input file parse root =
  ...
  if Sys.file_exists path then parse (In_channel.read_all path)
...
let get_descr w =
  match input "descr" parse_descr w with
...
let make_subst root pars vars arg =
  if String.equal arg "prefix" then root.path
    ...
    | Some x -> x
    | None -> (
        List.find_map pars ~f:(fun { name; defl } ->
            if String.equal name arg then Some defl else None)
        |> function
        | Some x -> x
        | None -> raise (Bad_substitution arg))

let linearize =
  List.map ~f:(fun (name, xs) ->
      match xs with
      | [] -> name
      | xs -> String.concat ~sep:"=" [ name; String.concat ~sep:"," xs ])

let apply_subst_exn root pars vars opts =
  let subs = List.map ~f:(subst_string (make_subst root pars vars)) in
  let args =
    linearize @@ List.map opts ~f:(fun (name, args) -> ("--" ^ name, subs args))
  in
  args
...
let mkdtemp ?(mode = 0o0700) ?tmp_dir ?(prefix = "") ?(suffix = "") () =
  ...
  let rec create name =
    let tmp =
      ...
      | Some tmp -> tmp
    in
    let path = String.concat [ tmp; Filename.dir_sep; prefix; name; suffix ] in
    match Unix.mkdir path mode with
    | () -> path
    | exception Unix.Unix_error (Unix.EEXIST, _, _) -> genname () |> create
  in
  genname () |> create
...
let is_zip path =
  try
    Zip.close_in (Zip.open_in path);
    true
  with Zip.Error _ -> false

let target_format t =
  if Sys.is_directory t then Dir else if is_zip t then Zip else Raw

let read t =
  match target_format t with
  | Zip ->
  ...
  | Raw ->
      { path = Filename.dirname t; temp = false; main = Filename.basename t }
  ...
  List.find_map paths ~f:(fun p ->
      if Sys.file_exists p && Sys.is_directory p then
        List.find ~f:Sys.file_exists
          [ p / name; (p / name) ^ ".scm"; (p / name) ^ ".recipe" ]
      else None)
...
and load env name =
  match which env.paths name with
...
let load_exn ?(paths = []) ?env name =
  let name, vars =
    ...
    | name :: vars -> (String.strip name, vars)
  in
  parse_vars ?env vars >>= fun vars -> load { paths; vars } name

let load ?paths ?env name =
  try load_exn ?paths ?env name with
  | Zip.Error (name, entry, msg) -> Error (Malformed_recipe (name, entry, msg))
  | Unix.Unix_error (err, _, _) -> Error (System_error (Unix.error_message err))
  | Sys_error s -> Error (System_error s)

let rec args t = List.concat_map t.loads ~f:args @ t.args

let prepend_before_dash_dash argv args =
  match Array.findi argv ~f:(fun _ -> String.equal "--") with
  | None -> Array.append argv args
  | Some (p, _) ->
      Array.concat [ Array.subo ~len:p argv; args; Array.subo ~pos:p argv ]
  ...
  | Some cmd ->
      let argv =
        ...
        | self :: arg :: rest when String.is_prefix ~prefix:arg cmd ->
            self :: cmd :: rest
        | self :: arg :: rest -> self :: cmd :: arg :: rest
      in
...
let rec close t =
  if t.root.temp then
    FileUtil.rm ~force:FileUtil.Force ~recurse:true [ t.root.path ];
  List.iter t.loads ~f:close

let pp_error ppf = function
  | Expect_atom sexp ->
      fprintf ppf "Parse error: expected an atom got (%a)"
        (pp_print_list Sexp.pp_hum)
        sexp
  | Expect_var s ->
      fprintf ppf "Parse error: bad variable definition, got `%s'" s
  | Bad_item s ->
      fprintf ppf "Parse error: unknown recipe item - %a" Sexp.pp_hum s
  | Missing_entry s -> fprintf ppf "The required file `%s' is missing" s
  | Bad_sexp err ->
      fprintf ppf "Parse error: %s" (Parsexp.Parse_error.message err)
  | No_recipe r -> fprintf ppf "Can't find the required recipe `%s'" r
  | Bad_subst v -> fprintf ppf "Don't know how to substitute $%s" v
  | Malformed_recipe (x, y, z) ->
      fprintf ppf "Malformed recipe: %s, %s, %s" x y z
  | Duplicate_var s -> fprintf ppf "Variable %s was specified twice" s
  | Bad_param s ->
      fprintf ppf
        "Syntax error in the parameter `%s' specification. Expecting: \
         (parameter <name> <value> <desc>)"
        s
  | Unbound_param s -> fprintf ppf "Unknown parameter `%s'" s
  | System_error s -> fprintf ppf "System error: %s" s
