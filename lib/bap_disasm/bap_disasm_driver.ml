...
type encoding = { coding : Theory.Language.t; target : Theory.Target.t }
[@@deriving bin_io, compare, equal]

let unknown =
  { coding = Theory.Language.unknown; target = Theory.Target.unknown }
...
module Machine : sig
  ...
  *)
  ...
  and slot = private Ready of task option | Delay [@@deriving bin_io]
  ...
  and slot = Ready of task option | Delay [@@deriving bin_io]
  ...
  let wrong_encoding s addr { coding } =
    ...
    | None -> false
    | Some other ->
        Result.is_error @@ KB.Domain.join Theory.Language.domain other coding

  let has_valid s dsts =
    dsts.indirect
    || Set.exists dsts.resolved ~f:(fun dst -> not (Set.mem s.data dst))

  let pp_task ppf = function
    | Dest { dst; parent = None } -> Format.fprintf ppf "root-%a" Addr.pp dst
    | Dest { dst } -> Format.fprintf ppf "dest-%a" Addr.pp dst
    | Fall { dst } -> Format.fprintf ppf "fall-%a" Addr.pp dst
    | Jump { src; age = 0 } -> Format.fprintf ppf "jump-%a" Addr.pp src
    | Jump { src; age } -> Format.fprintf ppf "del%d-%a" age Addr.pp src

  let sexp_of_task t = Sexp.Atom (Format.asprintf "%a" pp_task t)

  (** [revert t s] reverts the chain that produced the task [t].*)
  let rec revert task s =
    match task with
    | Dest { parent = None } -> s
    | Dest { parent = Some parent } | Fall { parent } | Jump { parent } -> (
        ...
        | Jump _ -> (
            match task with
            | Fall { delay = Ready (Some _) | Delay } -> cancel parent s
            | _ -> s))
  ...
  and revert_incoming s addr =
    match Map.find s.begs addr with
    | None -> s
    | Some tasks ->
        List.fold_right tasks ~f:revert
          ~init:{ s with begs = Map.remove s.begs addr }
  ...
  let rec step s =
    ...
    | [] when Set.is_empty s.usat ->
        { s with stop = true; code = Map.empty (module Addr) }
    | [] -> restart s
    ...
    | (Fall { dst = next; encoding } as curr) :: work ->
        if is_code s next then
          if is_slot s next then step @@ revert curr { s with work }
          else if wrong_encoding s next encoding then
            step @@ revert curr { s with work }
          else
            step
              { s with begs = Map.add_multi s.begs ~key:next ~data:curr; work }
        else if is_data s next then step @@ revert curr { s with work }
        else { s with work; addr = next; curr }
    | (Jump { src; dsts } as jump) :: ([] as work)
    | (Jump { src; dsts; age = 0 } as jump) :: work ->
        if Set.mem s.data src then step @@ revert jump { s with work }
        else
          let resolved = Set.diff dsts.resolved s.data in
          if Set.is_empty resolved && not dsts.indirect then
            step @@ revert jump { s with work }
          else
            let dsts = { dsts with resolved } in
            let init =
              { s with jmps = Map.add_exn s.jmps ~key:src ~data:dsts; work }
            in
            step
    ...
    | (Jump jmp as self) :: Fall ({ dst = next } as slot) :: work ->
        ...
        let jump = Jump { jmp with age = jmp.age - 1; src = next } in
        step
          {
            s with
    ...
    | Jump jmp :: work ->
        step { s with work = Jump { jmp with age = 0 } :: work }
  ...
  let decoded s mem encoding =
    ...
    {
      s with
      code = Map.add_exn s.code ~key:addr ~data:encoding.coding;
      usat = Set.remove s.usat addr;
    }
  ...
  let jumped s encoding mem dsts delay =
    ...
    let next =
      if dsts.barrier && delay = 0 then
        Dest { dst; parent = None; encoding = unknown }
      else Fall { dst; parent = jump; delay = Ready None; encoding }
    in
    step { s with work = jump :: next :: s.work }

  let insert_delayed t = function x :: xs -> x :: t :: xs | [] -> [ t ]
  ...
  let moved s encoding mem =
    ...
    let next =
      ...
      | Jump { dsts = { barrier = true } } ->
          Dest { dst = next; parent = None; encoding = unknown }
      | parent -> Fall { dst = next; parent; delay = Ready None; encoding }
    in
    let work =
      match s.curr with
  ...
  let skipped s addr = step { s with usat = Set.remove s.usat addr }
  let stopped s _ = step @@ cancel s.curr s
  ...
  let switch s encodings = { s with curr = with_encoding encodings s.curr }
  ...
  let start mem ~debt ~code ~data ~init =
    let init =
      if Set.is_empty init then
        Set.singleton (module Addr) (Memory.min_addr mem)
      else init
    in
...
let new_insn mem insn =
  ...
  KB.provide Dis.Insn.slot code (Some insn) >>| fun () -> code

let get_encoding label =
  Theory.Label.target label >>= fun target ->
  KB.collect Theory.Label.encoding label >>| fun coding -> { coding; target }
...
let delay code =
  KB.collect Theory.Semantics.slot code >>| fun insn ->
  KB.Value.get Insn.Slot.delay insn |> function None -> 0 | Some x -> x
...
let disassemble ~code ~data ~funs debt base : Machine.state KB.t =
  ...
  let rec next_encoding state current mem f =
    ...
    | true, true ->
        if equal_encoding current encoding then f state current mem
        else fail state encoding addr f
  and fail state encoding addr f =
    warning "ambiguous encoding for %a, rejecting" Addr.pp addr;
    Machine.view (Machine.failed state encoding addr) base ~empty:KB.return
      ~ready:(fun state current mem -> next_encoding state current mem f)
  and skip state addr f =
    warning "encoding for %a is unknown, skipping" Addr.pp addr;
    Machine.view (Machine.skipped state addr) base ~empty:KB.return
      ~ready:(fun state current mem -> next_encoding state current mem f)
  in
  let step d s =
    if Machine.is_ready s then KB.return s
      ...
      Machine.view s base
        ~ready:(fun s encoding mem ->
          next_encoding s encoding mem @@ fun s encoding mem ->
          Dis.jump (switch encoding d) mem @@ Machine.switch s encoding)
        ~empty:KB.return
  in
  Machine.start base ~debt ~code ~data ~init:funs
    ~ready:(fun init encoding mem ->
      next_encoding init encoding mem @@ fun init encoding mem ->
      match create_disassembler encoding with
      | Error _ ->
          warning "unable to disassemble instruction at %a with encoding %a"
            Addr.pp (Memory.min_addr mem) pp_encoding encoding;
          KB.return init
      | Ok disasm ->
          Dis.run disasm mem ~stop_on:[ `Valid ] ~return:KB.return
            ~init:(Machine.switch init encoding)
            ...
            ~hit:(fun d mem insn s ->
              KB.catch
                ( new_insn mem insn >>= fun label ->
                  ...
                  KB.provide Theory.Label.encoding label encoding.coding
                  >>= fun () ->
                  collect_dests encoding label >>= fun dests ->
                  if Set.is_empty dests.resolved && not dests.indirect then
                    step d @@ Machine.moved s encoding mem
                  else
                    delay label >>= fun delay ->
                    step d @@ Machine.jumped s encoding mem dests delay )
              @@ fun problem ->
              warning "rejecting %a due to a conflict %a" Memory.pp mem
                KB.Conflict.pp problem;
              step d (Machine.failed s encoding s.addr))
            ~invalid:(fun d mem s ->
              info "rejecting %a as an invalid instruction" Memory.pp mem;
              step d (Machine.failed s encoding s.addr)))
    ~empty:KB.return
...
let equal x y =
  phys_equal x y
  || (Set.equal x.begs y.begs && Map.equal equal_jump x.jmps y.jmps)
...
let commit_externals =
  fold_calls unresolved
    ~init:(Set.empty (module Theory.Label))
    ~f:(fun exts dst ->
      KB.return @@ if KB.Object.is_null dst then exts else Set.add exts dst)
...
let valid_dsts data dsts =
  ...
  if Set.is_empty resolved && not dsts.indirect then None
...
let scan_step ?(code = empty) ?(data = empty) ?(funs = empty) s mem =
  ...
  let jmps =
    Map.merge s.jmps jmps ~f:(fun ~key:_ -> function
      | `Left dsts -> valid_dsts data dsts
      | `Right dsts -> Some dsts
      | `Both (d1, d2) -> valid_dsts data (merge_dests d1 d2))
  in
...
let already_scanned mem s =
  ...
  List.exists s.mems ~f:(fun mem -> Memory.contains mem start)

let scan mem s =
  let open KB.Syntax in
  if already_scanned mem s then KB.return s
  else
    classify mem >>= fun (code, data, funs) ->
    scan_step ~code ~data ~funs s mem >>= fun s ->
    KB.List.fold s.mems
      ~f:(fun s mem -> if owns mem s then scan_step s mem else !!s)
...
let merge t1 t2 =
  {
    ...
    jmps =
      Map.merge t1.jmps t2.jmps ~f:(fun ~key:_ -> function
        | `Left dsts | `Right dsts -> Some dsts
        | `Both (d1, d2) -> Some (merge_dests d1 d2));
  }

let list_insns ?(rev = false) insns = if rev then insns else List.rev insns

let rec insert pos x xs =
  if pos = 0 then x :: xs
  else match xs with x' :: xs -> x' :: insert (pos - 1) x xs | [] -> [ x ]
...
let with_disasm beg blocks cfg f =
  Theory.Label.for_addr (Word.to_bitvec beg) >>= get_encoding
  >>= fun encoding ->
  match create_disassembler encoding with
  | Error _ ->
      warning "unable to explore block %a with encoding %a" Word.pp beg
        pp_encoding encoding;
      KB.return (blocks, cfg, None)
  | Ok dis -> f encoding dis

let explore ?entries ?entry:start ?(follow = always) ~block ~node ~edge ~init
    ({ begs; jmps; data; mems } as state) =
  let find_base addr =
    if Set.mem data addr then None
    else List.find mems ~f:(fun mem -> Memory.contains mem addr)
  in
  let edge_insert cfg src dst =
    match dst with None -> KB.return cfg | Some dst -> edge src dst cfg
  in
              ...
              | Some base -> (
                  Dis.run dis (view beg base) ~stop_on:[ `Valid ]
                    ...
                    ~hit:(fun s mem insn (curr, len, insns) ->
                      ...
                      let is_terminator =
                        Set.mem begs next || Map.mem jmps curr
                        || Set.mem data next
                      in
                      if is_terminator then KB.return (curr, len, insn :: insns)
                      else Dis.jump s (view next base) (next, len, insn :: insns))
                  >>= fun (fin, len, insns) ->
                  ...
                  | Some { resolved = dsts; barrier } ->
                      let dsts = if barrier then dsts else Set.add dsts fall in
                      Set.to_sequence dsts
                      |> KB.Seq.fold ~init:(blocks, cfg)
                           ~f:(fun (blocks, cfg) dst ->
                             build blocks cfg dst >>= fun (blocks, cfg, dst) ->
                             edge_insert cfg block dst >>| fun cfg ->
                             (blocks, cfg))
                      >>| fun (blocks, cfg) -> (blocks, cfg, Some block))))
  in
  let entries =
    match (start, entries) with
    | None, None -> Set.to_sequence state.begs
    | Some beg, None -> Seq.singleton beg
    | None, Some begs -> begs
    | Some beg, Some begs -> Seq.cons beg begs
  in
  let empty = Map.empty (module Addr) in
  KB.Seq.fold entries ~init:(empty, init) ~f:(fun (blocks, cfg) beg ->
      build blocks cfg beg >>| fun (blocks, cfg, _) -> (blocks, cfg))
  >>| snd
