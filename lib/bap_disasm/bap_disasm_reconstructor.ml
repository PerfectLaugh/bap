...
let is_start roots cfg blk =
  Set.mem roots (Block.addr blk) || Cfg.Node.degree ~dir:`In blk cfg = 0
...
let entries_of_block cfg roots blk =
  let entries =
    if is_start roots cfg blk then Block.Set.singleton blk else Block.Set.empty
  in
  if is_call blk then
    Seq.fold ~init:entries (Cfg.Node.outputs blk cfg) ~f:(fun entries e ->
        if is_fall e then entries else Set.add entries (Cfg.Edge.dst e))
  else entries

let terminator_addr blk =
  List.rev (Block.insns blk) |> List.hd_exn |> fst |> Memory.min_addr

let is_unresolved blk cfg =
  let deg = Cfg.Node.degree ~dir:`Out blk cfg in
  deg = 0 || (deg = 1 && is_fall (Seq.hd_exn (Cfg.Node.outputs blk cfg)))
...
let reachable cfg from =
  let rec loop nodes node =
    Seq.fold (Cfg.Node.outputs node cfg) ~init:(Set.add nodes node)
      ~f:(fun nodes edge ->
        if Set.mem nodes (Cfg.Edge.dst edge) then nodes
        else loop nodes (Cfg.Edge.dst edge))
  in
...
let sub roots prog start =
  ...
  let update_inputs node init =
    Seq.fold ~init (Cfg.Node.inputs node prog) ~f:Set.add
  in
  let rec loop cfg inputs node =
    Seq.fold
      (Cfg.Node.outputs node prog)
      ...
      ~f:(fun (cfg, inputs) edge ->
        if is_call edge then (cfg, inputs)
          ...
          else loop cfg' inputs (Cfg.Edge.dst edge))
  in
...
let reconstruct name initial_roots prog =
  ...
  let update_symtab syms cfg entry inputs =
    ...
    Set.fold inputs ~init:syms ~f:(fun syms e ->
        ...
        Symtab.insert_call ~implicit syms (Cfg.Edge.src e) name)
  in
...
let of_blocks syms =
  let reconstruct (cfg : cfg) =
    ...
    Seq.iter syms ~f:(fun (name, b, _) ->
        match Hashtbl.find blocks b with
        | None -> ()
    ...
    Hashtbl.fold symtab ~init:Symtab.empty ~f:(fun ~key ~data symtab ->
        ...
        | entry :: _ as blocks ->
            let g =
              List.fold blocks ~init:Cfg.empty ~f:(fun g x ->
                  List.fold blocks ~init:g ~f:(fun g y ->
                      match Cfg.Node.edge x y cfg with
                      | None -> g
                      | Some e -> Cfg.Edge.insert e g))
            in
            if Cfg.Node.mem entry g then Symtab.add_symbol symtab (key, entry, g)
            else symtab)
  in
  create reconstruct
