...
type t = { path : string option; find : addr -> string option; biased : bool }
type symbolizer = t

let name_of_addr addr = sprintf "sub_%s" @@ Addr.string_of_value addr

module Name = struct
  let is_empty name = String.is_prefix name ~prefix:"sub_"
...
let create find = { path = None; find; biased = false }
let path s = s.path
let set_path s p = { s with path = Some p }
let run { find } a = find a

let resolve sym addr =
  match run sym addr with Some name -> name | None -> name_of_addr addr

let empty = create (fun _ -> None)
let chain ss = create (fun addr -> List.find_map ss ~f:(fun s -> run s addr))

let string_of_addr addrs =
  List.map addrs ~f:Addr.to_string |> String.concat ~sep:", "
...
let of_image img =
  ...
  Table.foldi symtab ~init ~f:(fun mem sym rels ->
      let name = Image.Symbol.name sym and addr = Memory.min_addr mem in
      if not (Name.is_empty name) then Bap_relation.add rels addr name else rels)
  |> fun rels ->
...
let of_blocks seq =
  let names =
    ...
    Bap_relation.matching rels Addr.Map.empty
      ...
      ~unmatched:(fun reason names ->
        report_broken reason;
        names)
  in
  create @@ Map.find names

module Factory = Factory.Make (struct
  type nonrec t = t
end)
...
let provide_symbolizer s label =
  ...
  if Context.is_applicable ctxt s.path then
    s.find @@ Context.create_addr ctxt ~unbiased:(not s.biased) addr
  else None

let provide =
  KB.Rule.(
    declare ~package:"bap" "reflect-symbolizers"
    |> dynamic [ "symbolizer" ] |> require Theory.Label.addr
  ...
  fun agent s ->
    KB.propose agent Theory.Label.possible_name @@ provide_symbolizer s

let providing agent s =
  KB.proposing agent Theory.Label.possible_name ~propose:(provide_symbolizer s)
