...
let compare t1 t2 = Addr.Map.compare Fn.compare t1.addrs t2.addrs
...
let merge m1 m2 =
  ...
  |> Seq.fold ~init:m1 ~f:(fun m1 (mem, x) -> Memmap.add m1 mem x)
...
let filter_calls name cfg calls =
  ...
  Cfg.nodes cfg
  |> Seq.fold ~init ~f:(fun calls node -> Map.remove calls (Block.addr node))
...
let fns_of_seq seq =
  Seq.map seq ~f:snd |> Seq.to_list |> Fn.Set.of_list |> Set.to_list
...
let to_sequence t = Map.to_sequence t.addrs |> fns_of_seq |> Seq.of_list
...
let insert_call ?(implicit = false) symtab block data =
  ...
  if implicit then { symtab with icalls = Map.set symtab.icalls ~key ~data }
  else { symtab with ecalls = Map.set symtab.ecalls ~key ~data }

let explicit_callee { ecalls } = Map.find ecalls
let implicit_callee { icalls } = Map.find icalls

let callee tab src =
  match explicit_callee tab src with
  | Some dst -> Some dst
...
let add_node calls graphs node =
  update_graph calls graphs node @@ Cfg.Node.insert node

let add_edge calls graphs edge =
  update_graph calls graphs (Cfg.Edge.src edge) @@ Cfg.Edge.insert edge
  ...
  Disasm.explore disasm
    ...
    ~block:(fun mem insns ->
      ...
      KB.List.filter_map insns ~f:(fun label ->
          ...
          | None -> None
          | Some mem -> Some (mem, s))
      >>| fun insns -> Block.create mem insns)
    ~node:(fun n (tab, graphs) -> KB.return (tab, add_node calls graphs n))
    ~edge:(fun src dst (tab, graphs) ->
      ...
      if Callgraph.siblings calls (Block.addr src) dest then
        ...
        if Addr.equal next dest then
          ({ tab with icalls = Map.set tab.icalls ~key:from ~data:name }, graphs)
        else
          ({ tab with ecalls = Map.set tab.ecalls ~key:from ~data:name }, graphs))

let collect_externals disasm =
  Disasm.externals disasm |> Set.to_sequence
  |> KB.Seq.fold
       ~init:(Map.empty (module Theory.Label))
       ~f:(fun extern label ->
