...
(** Set of node parents.
    ...
    A node [n] is a parent of node [m] iff [n <> m /\ n <> Entry] and if every
    path from the [entry] node to [m] contains [n]. In other words, if it [n]
    strictly dominates [m] and is not the entry node.

    We compute parents using descending fixed point computation, with lattice of
    subsets. In our representation we use [Top] to denote the set of all nodes,
    and [Set xs] to denote a set of known nodes.

    The merge (meet) operator is the set intersection. The transfer function is
    defined as
    {v
    ...
    Usually, dominators are computed using a transfer function that is defined
    as [transfer node ps = (node) U ps], which our third clause, however we
    specialized our tranfer function for two reasons

    1) efficiency - the first clause removes an explicit entry node, which by
    dominates all nodes. So we can save some space, as well as implement the
    [is_root] as little bit more efficient.

    2) robustness - usually, the dominators are computed for graphs where all
    nodes are dominated by the entry node. However, our case is not general, and
    we have components which are not reachable from the entry node. We do not
    want such componenets to change the parent propery of any node, since there
    is no path from the [entry] node to any other node, that will contain a node
    from an unconnected component. *)
module Parent = struct
  ...
  let is_root = function Top -> false | Set x -> Set.is_empty x
  ...
  let transfer self parents =
    if Word.equal self Callgraph.entry then parents
    else
      match parents with
      | Top -> Top
  ...
  include
    Binable.Of_binable_with_uuid
      ...
      (struct
        ...
        let of_binable xs =
          let init = ok_exn @@ Map.of_increasing_sequence (module Word) xs in
          Solution.create init Parent.unknown

        let caller_identity =
          Bin_shape.Uuid.of_string "cb921c64-00a1-4cd3-a219-a0e73f841169"
      end)
end

type input = Driver.state
type output = { parents : Parents.t; entries : Addr.Set.t } [@@deriving bin_io]
type t = output [@@deriving bin_io]

let string_of_node n =
  sprintf "%S"
  @@ if Callgraph.is_entry n then "entry" else Addr.string_of_value n

let pp_callgraph ppf graph =
  Graphlib.to_dot (module Callgraph) graph ~formatter:ppf ~string_of_node

let pp_roots ppf graph =
  Graphlib.to_dot
    (module Callgraph)
    graph ~formatter:ppf
    ~string_of_node:(fun s -> sprintf "%S" (Addr.string_of_value s))
...
let empty =
  let root = Map.singleton (module Addr) Callgraph.entry Parent.none in
  {
...
let siblings { parents } x y =
  ...
  | Top, _ | _, Top -> false
  | Set p1, Set p2 ->
      if Set.is_empty p1 then Set.mem p2 x
      else if Set.is_empty p2 then Set.mem p1 x
      else not @@ Set.is_empty (Set.inter p1 p2)
...
let update { parents } disasm =
  ...
  Graphlib.fixpoint
    (module Callgraph)
    graph ~init:parents ~start:Callgraph.entry ~equal:Parent.equal
    ~merge:Parent.merge ~f:Parent.transfer
  |> fun parents -> { parents; entries = entries graph parents }
