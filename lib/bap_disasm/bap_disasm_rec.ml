...
type error =
  [ `Failed_to_disasm of mem | `Failed_to_lift of mem * full_insn * Error.t ]
[@@deriving sexp_of]
...
module Node = struct
  ...
  include Opaque.Make (struct
    ...
    let addr = Block.addr
    let compare x y = Addr.compare (addr x) (addr y)
...
let follows_after m1 m2 =
  Addr.equal (Addr.succ (Memory.max_addr m1)) (Memory.min_addr m2)

let has_conditional_jump blk =
  let insn = Block.terminator blk in
  Insn.(is jump insn) && Insn.(is conditional insn)

let global_cfg disasm =
  Driver.explore disasm ~init:Cfg.empty
    ~block:(fun mem insns ->
    ...
    ~edge:(fun src dst g ->
      let k =
        if follows_after (Block.memory src) (Block.memory dst) then `Fall
        else if has_conditional_jump src then `Cond
        else `Jump
      in
...
let with_unit =
  KB.Rule.(
    declare ~package:"bap" "unit-for-mem"
    |> dynamic [ "arch"; "mem" ]
    |> require Theory.Label.addr |> provide Theory.Label.unit
...
let scan arch mem state = with_unit arch mem @@ fun () -> Driver.scan mem state

let run ?backend:_ ?(brancher = Brancher.empty) ?(rooter = Rooter.empty) arch
    mem =
  Result.return @@ extract
  @@ Brancher.providing brancher
  @@ fun () ->
  Rooter.providing rooter @@ fun () ->
  with_unit arch mem @@ fun () -> Driver.scan mem Driver.init >>= global_cfg
