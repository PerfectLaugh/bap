...
type error = [ `Failed of Error.t | `Errors of error * error | Rec.error ]
[@@deriving sexp_of]
...
type disasm = { cfg : cfg; err : Rec.error list }
...
let empty = { cfg = Rec.Cfg.empty; err = [] }
let of_rec d = { cfg = Rec.cfg d; err = Rec.errors d }
...
module Disasm = struct
  ...
  let of_image ?backend ?brancher ?rooter image =
    ...
    let rooter = Option.value rooter ~default:(Rooter.of_image image) in
  ...
  let insn =
    Value.Tag.register
      (module Insn)
      ~name:"insn" ~uuid:"8e2a3998-bf07-4a52-a791-f74ea190630a"
end
