...
module Context = struct
  ...
  type t = { bias : Bitvec.t option; path : string option; size : int }
  ...
  let for_label label =
    ...
    | Some unit ->
        ...
        KB.collect Theory.Unit.path unit >>| fun path -> { bias; path; size }

  let create_addr { bias; size } ~unbiased addr =
    let bias =
      Option.value ~default:Bitvec.zero @@ if unbiased then bias else None
    in
