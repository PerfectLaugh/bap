...
module Props = struct
  ...
  module Bits = struct
    ...
    let caller_identity =
      Bin_shape.Uuid.of_string "650d7d34-27fc-46c5-b678-080321d2ef84"
  end

  let empty = Z.zero
  let ( + ) flags (flag, _) = Z.logor flags flag
  let ( - ) flags (flag, _) = Z.logand flags (Z.lognot flag)
  let has flags (flag, _) = [%compare.equal: t] (Z.logand flags flag) flag
  let set_if cond flag = if cond then fun flags -> flags + flag else Fn.id
  ...
  let assoc_of_props props =
    List.map !known_properties ~f:(fun p -> (name p, has props p))

  let domain =
    KB.Domain.flat "props" ~empty:Z.one ~equal:Z.equal ~inspect:(fun props ->
        [%sexp_of: (string * bool) list] (assoc_of_props props))

  let persistent = KB.Persistent.of_binable (module T)

  let slot =
    KB.Class.property ~package:"bap" Theory.Semantics.cls "insn-properties"
      domain ~persistent ~public:true
...
module Slot = struct
  ...
  let text =
    KB.Domain.flat "text" ~inspect:sexp_of_string ~empty ~equal:String.equal

  let delay_t =
    KB.Domain.optional "delay_t" ~inspect:sexp_of_int ~equal:Int.equal

  let name =
    KB.Class.property ~package:"bap" Theory.Semantics.cls "insn-opcode" text
      ~persistent:KB.Persistent.string ~public:true ~desc:"instruction opcode"

  let asm =
    KB.Class.property ~package:"bap" Theory.Semantics.cls "insn-asm" text
      ~persistent:KB.Persistent.string ~public:true ~desc:"an assembly string"
  ...
  let ops_domain =
    KB.Domain.optional "insn-ops" ~equal:[%compare.equal: Op.t array]
  ...
  let ops =
    KB.Class.property ~package:"bap" Theory.Semantics.cls "insn-ops" ops_domain
      ~persistent:ops_persistent ~public:true
      ~desc:"an array of instruction operands"

  let delay =
    KB.Class.property ~package:"bap" Theory.Semantics.cls "insn-delay" delay_t
      ~persistent:
        (KB.Persistent.of_binable
           (module struct
             type t = int option [@@deriving bin_io]
           end))
      ~public:true ~desc:"the length of the delay slot"
  ...
  let dests =
    ...
    let join x y =
      ...
      | None, Some x | Some x, None ->
          if Set.is_empty x then Ok None else Error Jump_vs_Move
      | Some x, Some y -> Ok (Some (Set.union x y))
    in
    ...
    KB.Class.property ~package:"bap" Theory.Semantics.cls "insn-dests" data
      ~persistent ~public:true
  ...
  let subs =
    ...
    let order x y =
      ...
      | xs, ys ->
          if Array.length xs <> Array.length ys then NC
            ...
            Array.fold2_exn xs ys ~init:EQ ~f:(fun result x y ->
                match result with
                | NC -> NC
                | _ -> (
                    match (result, KB.Domain.order dom x y) with
                    | EQ, GT -> GT
                    | EQ, LT -> LT
                    | r, r' -> if Poly.equal r r' then r' else NC))
    in
    let join xs ys =
      match (xs, ys) with
      | [||], x | x, [||] -> Ok x
      | xs, ys -> (
          try
            if Array.length xs <> Array.length ys then Error Different_sizes
            else
              Result.return
    ...
    in
    let inspect xs =
      Sexp.List (List.map ~f:(KB.Domain.inspect dom) (Array.to_list xs))
    in
    let domain =
      KB.Domain.define ~empty:[||] "instructions" ~order ~join ~inspect
    in
    KB.Class.property Theory.Semantics.cls "subinstructions" ~package:"bap"
      ~public:true ~desc:"a sequence of subinstructions"
      ~persistent:
        (KB.Persistent.of_binable
           (module struct
...
module Analyzer = struct
  ...
  type vis = { jump : bool; cond : bool; indirect : bool }
  ...
  let analyzer =
    ...
    object
      ...
      method! enter_jmp ex (effs, jumps) =
        ( effs,
          match ex with
      ...
      method! enter_stmt s (effs, jumps) =
        match Bil.(decode call s) with
...
let derive_props ?bil insn =
  let bil_effects =
    ...
    | None -> Analyzer.Effects.empty
  in
  let is = Insn.is insn in
  let is_bil = if Option.is_some bil then Set.mem bil_effects else is in
  ...
  let may_affect_control_flow = is_jump || is `May_affect_control_flow in
  ...
  Props.empty |> Props.set_if is_jump jump
...
let write init ops = List.fold ~init ops ~f:(fun init f -> f init)
...
let of_basic ?bil insn : t =
  let eff =
    KB.Value.put Bil.slot
      ...
      (Option.value bil ~default:[])
  in
  write (set_basic eff insn) Slot.[ Props.slot <-- derive_props ?bil insn ]
...
let ops s = match KB.Value.get Slot.ops s with None -> [||] | Some ops -> ops
...
module Adt = struct
  ...
  let pp ppf insn =
    ...
    if String.equal name Slot.empty then pr ppf "Undefined()"
    else
      pr ppf "%s(%a, Props(%s))" (String.capitalize name) pp_ops
        (Array.to_list (ops insn))
        (props insn)
end
...
module Trie = struct
  ...
  module Normalized = Trie.Make (struct
    ...
    let hash_ops =
      Array.fold ~init:0 ~f:(fun h x -> h lxor Op.Normalized.hash x)

    let hash (x, xs) = x lxor hash_ops xs
  end)
...
include Regular.Make (struct
  ...
  let string_of_ops ops =
    Array.map ops ~f:Op.to_string |> Array.to_list |> String.concat ~sep:","

  let pp fmt insn =
    let name = name insn in
    if String.equal name Slot.empty then Format.fprintf fmt "%s" name
    else Format.fprintf fmt "%s(%s)" name (string_of_ops (ops insn))
end)

let pp_asm ppf insn = Format.fprintf ppf "%s" (asm insn)

module Seqnum = struct
  type t = int

  let slot =
    KB.Class.property Theory.Program.cls "seqnum" ~public:true
      ~persistent:
        (KB.Persistent.of_binable
           (module struct
             type t = Int.t option [@@deriving bin_io]
           end))
      ~desc:"the sequential number of a subinstruction" ~package:"bap"
    @@ KB.Domain.optional "int" ~inspect:Int.sexp_of_t ~equal:Int.equal

  let freshnum =
    let cls = KB.Class.declare "seqnum-generator" () ~package:"bap" in
  ...
  let label ?package num =
    ...
    KB.provide slot obj (Some num) >>| fun () -> obj
...
let () =
  ...
  |> add_writer ~desc:"Abstract Data Type pretty printing format" ~ver:version
       "adt";
