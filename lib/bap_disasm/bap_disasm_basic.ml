...
type pred = [ `Valid | Kind.t ] [@@deriving sexp, compare]

type 'a oper = { oper : int; insn : int; data : 'a }
[@@deriving bin_io, compare, sexp]

type reg_info = { reg_code : int; reg_name : string } [@@deriving bin_io, sexp]

let compare_reg_info { reg_code = x } { reg_code = y } = Int.compare x y

type imm_info = { imm_small : int; imm_large : int64 option }
[@@deriving bin_io, compare, sexp]
...
module Table = struct
  type t = { data : Bigstring.t; size : int; cache : string Int.Table.t }

  let create data =
    { data; size = Bigstring.length data; cache = Int.Table.create () }

  let lookup t pos =
    Hashtbl.find_or_add t.cache pos ~default:(fun () ->
        if pos < 0 || pos >= t.size then "ERROR"
        else
          match Bigstring.find ~pos '\x00' t.data with
          | None -> "ERROR"
          | Some fin ->
              let len = fin - pos in
              let dst = Bytes.create len in
              Bigstring.To_bytes.blit ~src:t.data ~src_pos:pos ~dst ~dst_pos:0
                ~len;
              Bytes.to_string dst)
end
...
type dis = { name : string; enc : string; asm : bool; kinds : bool }
[@@deriving sexp_of]
...
let get { name } =
  ...
  | Some d -> d
  | None -> invalid_argf "Trying to access a closed disassembler %s" name ()
...
module Reg = struct
    ...
    let data =
      ...
      let reg_name =
        if reg_code = 0 then "Nil"
          ...
          if reg_code < 0 then sprintf "%c%d" (Char.of_int_exn off) ~-reg_code
          else Table.lookup (get dis).reg_table off
      in
      { reg_code; reg_name }
    in
    { insn; oper; data }
  ...
  module T = struct
    type t = reg [@@deriving bin_io, sexp, compare]
...
module Imm = struct
  let fits x = not (x = Int.max_value || x = Int.min_value)

  let create dis ~insn ~oper =
    let data =
      let imm_small = C.insn_op_imm_small_value !!dis ~insn ~oper in
      let imm_large =
        if fits imm_small then None
        else Some (C.insn_op_imm_value !!dis ~insn ~oper)
      in
      { imm_small; imm_large }
    in
    { insn; oper; data }
  ...
  let to_int64 { data = n } =
    match n.imm_large with None -> Int64.of_int n.imm_small | Some x -> x
  ...
  module T = struct
    type t = imm [@@deriving bin_io, sexp, compare]
    ...
    let pp fmt t =
      let x = to_int64 t in
      if Int64.is_negative x then Format.fprintf fmt "-0x%Lx" (Int64.abs x)
      else Format.fprintf fmt "0x%Lx" x

    let hash { data = n } =
      if fits n.imm_small then n.imm_small
...
module Fmm = struct
  let create dis ~insn ~oper =
    { insn; oper; data = C.insn_op_fmm_value !!dis ~insn ~oper }

  let to_float x = x.data

  module T = struct
    type t = fmm [@@deriving bin_io, sexp, compare]

    let module_name = Some "Bap.Std.Fmm"
    let version = "1.0.0"
    let hash t = Float.hash (to_float t)
    let pp fmt t = Format.fprintf fmt "%a" Float.pp (to_float t)
  end
      ...
      let compare_ops xs ys =
        ...
        let rec loop = function
          ...
          | n ->
              ...
              if r = 0 then loop (n - 1) else r
        in
        loop len

      let hash = function Imm _ | Fmm _ -> 0 | Reg x -> Reg.hash x
    end
  end
...
module Insn = struct
  ...
  let rec sexp_of_t ins =
    ...
    let parent =
      Sexp.List
        (Sexp.Atom name
        :: List.map ops ~f:(fun op -> Sexp.Atom (Op.to_string op)))
    in
    if Array.length ins.subs > 0 then
  ...
  let normalize_asm asm =
    String.substr_replace_all asm ~pattern:"\t" ~with_:" " |> String.strip
    ...
    let opers =
      Array.init (C.insn_ops_size !!dis ~insn) ~f:(fun oper ->
          ...
          | Insn ->
              ...
              Op.Imm { v with data = { imm_small = pos; imm_large = None } })
    in
    let subs = Array.init (Hashtbl.length subs) ~f:(Hashtbl.find_exn subs) in
    let code = C.insn_code !!dis ~insn in
    let fullname =
      let off = C.insn_name !!dis ~insn in
      KB.Name.read ~package:dis.enc @@ Table.lookup (get dis).insn_table off
    in
    ...
    let kinds =
      if kinds then
        List.fold Kind.all ~init:[] ~f:(fun ks k ->
            ...
            if C.is_supported !!dis p && C.insn_satisfies !!dis ~insn p then
              k :: ks
            else ks)
      else []
    in
  ...
  let domain =
    KB.Domain.optional ~inspect:sexp_of_t "insn" ~equal:(fun x y ->
        Int.equal x.code y.code)

  let slot =
    KB.Class.property ~package:"bap" Theory.Program.cls "insn" domain
      ~public:true ~desc:"a decoded machine instruction"
end

type ('a, 'k) insn = ('a, 'k) Insn.t

let compare_insn (i1 : ('a, 'b) insn) (i2 : ('a, 'b) insn) = Insn.compare i1 i2
...
let compare_full_insn i1 i2 =
  ...
  if r1 <> 0 then String.compare i1.asm i2.asm else r1
...
type step = { mem : mem; off : int; preds : preds } [@@deriving sexp_of]

type ('a, 'k) maybe_insn = (mem * ('a, 'k) insn option[@sexp.opaque])
[@@deriving sexp_of]

let sexp_of_maybe_ins (_, insn) = [%sexp_of: insn option] insn
...
type (+'a, +'k, 's, 'r) state = {
  ...
  hit : (('a, 'k, 's, 'r) state -> mem -> (asm, kinds) insn -> 's -> 'r) option;
      [@sexp.opaque]
}
[@@deriving sexp_of]

let create_state ?(backlog = 0) ?(stop_on = []) ?stopped ?invalid ?hit dis mem
    ~return =
  {
    backlog;
    dis;
...
let update_state s current =
  { s with history = List.take (s.current :: s.history) s.backlog; current }

let memory s = s.current.mem

let push_pred s p =
  let p = cpred_of_pred p in
  if C.is_supported !!(s.dis) p then C.predicates_push !!(s.dis) p
...
let with_preds s (ps : pred list) =
  ...
  if Set.is_empty drop then
    Set.iter (Set.diff ps s.current.preds) ~f:(push_pred s)
  else reset_predicates s ps;
  { s with current = { s.current with preds = ps } }

let insns s = List.init Array.(length s.insns) ~f:(fun i -> s.insns.(i))
...
let step s data =
  ...
  let rec loop s data =
    ...
    let insns =
      Array.init n ~f:(fun insn ->
          let is_valid = not (C.insn_satisfies !!(s.dis) ~insn Is_invalid) in
          ( insn_mem s ~insn,
            Option.some_if is_valid (Insn.create ~asm ~kinds s.dis ~insn) ))
    in
    let s = { s with insns } in
    if stop then
      match s.stopped with Some f -> f s data | None -> s.return data
    else if C.insn_satisfies !!(s.dis) ~insn Is_invalid then
      match s.invalid with
...
let back s data =
  let current, history =
    match s.history with [] -> (s.current, s.history) | x :: xs -> (x, xs)
  in
...
let register encoding construct =
  if Hashtbl.mem constructors encoding then
    invalid_argf
      "A disassembler backend for the encoding %s is already provided. Please, \
       disable the old one before registering a new one."
      (Theory.Language.to_string encoding)
      ();
...
let lookup target encoding =
  ...
  | None -> (
      match Hashtbl.find constructors encoding with
      | None ->
          errorf "no disassembler for encoding %s"
            (Theory.Language.to_string encoding)
      | Some create -> create target)

let create ?(debug_level = 0) ?(cpu = "") ?(attrs = "") ?(backend = "llvm")
    triple =
  let parts =
    [ backend; triple; cpu ] |> List.filter ~f:(Fn.non String.is_empty)
  in
  ...
  | None -> (
      match Prim.create ~backend ~triple ~cpu ~attrs ~debug_level with
      | n when n >= 0 ->
...
let run ?backlog ?(stop_on = []) ?invalid ?stopped ?hit dis ~return ~init mem =
  let state = create_state ?backlog ?invalid ?stopped ?hit ~return dis mem in
...
module Trie = struct
  ...
  module Normalized_key = struct
    ...
    let compare_token (x, xs) (y, ys) =
      ...
      if r <> 0 then r else Op.Normalized.compare_ops xs ys

    let ops_hash xs =
      Array.fold ~init:0 ~f:(fun h x -> Op.Normalized.hash x lxor h) xs

    let token_hash (x, xs) = x lxor ops_hash xs
  end
