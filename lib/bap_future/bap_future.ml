...
module Std = struct
  ...
  module Variadic = struct
    ...
    module Make (T : Applicable.S) : Variadic with type 'a arg = 'a T.t = struct
      include Make_args (T)
  ...
  module Future = struct
    type 'a waiters = { mutable waiters : ('a -> unit) list }
    ...
    let is_decided = function { cell = Full _ } -> true | _ -> false

    let rec unlink future : ('a, direct) cell =
      match future.cell with
      | Full _ as c -> c
      | Wait _ as c -> c
      | Link f -> unlink f
    ...
    let upon v f =
      match cell v with Full x -> f x | Wait w -> w.waiters <- f :: w.waiters
    ...
    let peek v = match cell v with Full x -> Some x | Wait _ -> None
  ...
  module Stream = struct
    ...
    let wait t = List.iter t.waiters ~f:(fun f -> f ())
    let on_wait t f = t.waiters <- f :: t.waiters
    let on_subscribe t f = t.on_subs <- f :: t.on_subs
    let on_unsubscribe t f = t.on_unsubs <- f :: t.on_unsubs
    ...
    let link s t f =
      let k = ref None in
      on_subscribe t (fun _ ->
          match !k with Some _ -> () | None -> k := Some (subscribe s f));
      on_unsubscribe t (fun _ ->
          if not (has_subscribers t) then
            match !k with
            | None -> ()
            | Some key ->
                unsubscribe s key;
    ...
    let filter_map s ~f =
      ...
      map' s ~f:(fun x ->
          match f x with None -> empty | Some x -> Queue.singleton x)

    let map s ~f = map' s ~f:(fun x -> Queue.singleton (f x))
    let filter s ~f = filter_map s ~f:(fun x -> Option.some_if (f x) x)

    let ratio x y =
      let x, y = (max x y, min x y) in
      x / y
    ...
    let merge s1 s2 ~f =
      ...
      let drop () =
        let drop : type a. a Queue.t -> unit =
         fun q -> ignore (Queue.dequeue_exn q : a)
        in
        drop q1;
        drop q2
      in
    ...
    let of_list xs =
      unfold_until ~init:xs ~f:(function [] -> None | x :: xs -> Some (x, xs))
    ...
    let upon finished s : 'a future =
      ...
      Future.upon finished (fun () ->
          match !result with None -> () | Some x -> Promise.fulfill promise x);
      future
    ...
    let before event xs =
      ...
      Future.upon event (fun () -> on_result xs id (Queue.to_list buf) promise);
      future

    let last_before e xs n =
      let buf = Queue.create () in
      let add_value x =
        if Queue.length buf >= n then ignore (Queue.dequeue buf : 'a option);
        Queue.enqueue buf x
      in
      let id = subscribe xs add_value in
      let future, promise = Future.create () in
      Future.upon e (fun () -> on_result xs id (Queue.to_list buf) promise);
      future
    ...
    let take xs n =
      ...
      let add_value id x =
        ...
        if Queue.length buf = n then on_result xs id (Queue.to_list buf) promise
      in
      watch xs add_value;
      future
    ...
    let nth xs n =
      ...
      let future, promise = Future.create () in
      let f id x =
        if !cnt = n then on_result xs id x promise;
        Int.incr cnt
      in
      watch xs f;
      future
    ...
    let find xs ~f =
      let future, promise = Future.create () in
      let f id x = if f x then on_result xs id x promise in
      watch xs f;
      future
    ...
    let find_map xs ~f =
      let future, promise = Future.create () in
      let f id x =
        match f x with None -> () | Some y -> on_result xs id y promise
      in
      watch xs f;
      future
    ...
    let tl xs =
      let s, Signal publish = create () in
      let ign = ref true in
      let f x = if !ign then ign := false else publish x in
      link xs s f;
      s

    (** [ndrop queue num] - drops up to [num] elements from [queue] and returns
        a number of actually dropped elements. *)
    let ndrop q num =
      let rec drop n =
        if n < num then
    ...
    let foldw ?(stride = 1) xs width ~init ~f =
      ...
      let estimate_drop p =
        ...
        | stride, `Not_pushed -> Int.pred stride
      in
      let drop n =
        match ndrop buf n with n' when n = n' -> 0 | n' -> n - n'
      in
      let publish d =
        let r = Queue.fold ~f ~init buf in
        (Some r, drop (estimate_drop d))
      in
      let add x =
        Queue.enqueue buf x;
        if is_enough () then publish `Pushed else (None, 0)
      in
      let try_push x = if is_enough () then publish `Not_pushed else add x in
      let fold drop x =
        match drop with 0 -> try_push x | n -> (None, n - 1)
      in
    ...
    let sample ~clk dat =
      let s = sync ~clk dat in
      let s', Signal push = create () in
      let f = function [] -> push None | hd :: _ -> push (Some hd) in
      link s s' f;
      s'
    ...
    let concat_merge ss ~f =
      ...
      parse (concat ss) ~init:None ~f:(fun state x ->
          match state with
          | None -> pair x
          | Some y ->
              let z = f x y in
