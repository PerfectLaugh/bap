...
module Rule = struct
  ...
  type hypot = { rule : rule; work : rule list }
  type t = hypot
  ...
  type hint = Expect_list of range | Expect_pair
  ...
  let sexp_of_t { rule } = Sexp.List [ List rule.patterns; List rule.template ]
  ...
  let sexp_fold_atoms ~get_pos ~init ~f sexp =
    let rec loop init s =
      ...
      | Sexp.List xs -> List.fold ~init ~f:loop xs
    in
    loop init sexp
  ...
  let vars_of_sexp ~get_pos init xs =
    sexp_fold_atoms ~get_pos ~init xs ~f:(fun atoms loc atom ->
        if is_variable atom then Map.set atoms ~key:atom ~data:loc else atoms)
  ...
  let check_rule ~get_pos lhs rhs =
    ...
    | None -> (
        Map.merge def use ~f:(fun ~key -> function
          | `Both _ -> None
          | `Left _ -> None
          | `Right p -> Some p)
        |> Map.min_elt
  ...
  let of_sexp ~get_pos sexp =
    ...
    | List [ List patterns; List template ] -> (
  ...
  let of_sexps ~get_pos xs = List.map xs ~f:(of_sexp ~get_pos) |> Result.all
  ...
  let empty_range =
    Parsexp.Positions.
      { start_pos = beginning_of_file; end_pos = beginning_of_file }
  ...
  let parse (type t) (module P : Parser with type r = t) str =
    ...
    | Ok r ->
        ...
        let get_pos sub =
          match P.ops.find_pos pos data ~sub with
          | None -> empty_range
          | Some x -> x
        in
  ...
  and concat_bindings xs ys =
    match (xs, ys) with
    | [], [] -> Some empty
    | [], _ :: _ | _ :: _, [] -> None
    | x :: xs, y :: ys -> (
        match bindings x y with
        | None -> None
        | Some bs -> (
            match concat_bindings xs ys with
            | None -> None
            | Some bs' -> merge bs bs'))
  ...
  let apply { bindings; template } = List.map template ~f:(apply bindings)

  let split_map xs ~f =
    let rec loop rev_prefix = function
      | [] -> None
      | x :: suffix -> (
          match f ~rev_prefix x ~suffix with
          | Some x -> Some x
          | None -> loop (x :: rev_prefix) suffix)
    in
    loop [] xs
  ...
  let report_error ?(filename = "//dynamic//") ppf err =
    ...
    | Wildcard r ->
        fprintf ppf
          ...
          pp_range r
    | Not_a_sexp perr -> Parsexp.Parse_error.report ppf ~filename perr
