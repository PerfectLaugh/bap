...
let flags =
  List.map ~f:(reg bool)
    [ "SO"; "CA"; "OV"; "CA32"; "OV32"; "C"; "FL"; "FE"; "FG"; "FU" ]
  @ crflags
...
let define ?(parent = parent) ?nicknames name bits endianness =
  ...
  let vars =
    array bits "R%d" 32 @< array r64 "F%d" 32 @< array r128 "V%d" 32 @< flags
    @< [ reg bits "CTR"; reg bits "LR"; reg bits "TAR" ]
    @< [ data ]
  in
  Theory.Target.declare ~package name ~parent ?nicknames ~bits:size ~endianness
    ~vars ~code:data ~data
    ~regs:
      Theory.Role.Register.
        [
          ...
          ( [ overflow_flag ],
            untyped @@ List.([ "SO"; "OV"; "OV32" ] >>| reg bool) );
          ( [ status; floating ],
            untyped @@ List.([ "FL"; "FE"; "FG"; "FU" ] >>| reg bool) );
          ( [ caller_saved ],
            untyped [ reg bits "R0" ]
...
let powerpc32le =
  define "powerpcle" r32 Theory.Endianness.le
    ~nicknames:
      [
        "powerpc32+le";
        "ppcle";
...
let powerpc64bi =
  define "powerpc64bi" r64 Theory.Endianness.bi
    ~nicknames:[ "powerpc64+bi"; "power64bi" ]

let powerpc64eb =
  define "powerpc64" r64 Theory.Endianness.eb
    ~nicknames:
      [
        "powerpc64+bi";
        "ppc64";
...
let powerpc64le =
  define "powerpc64le" r64 Theory.Endianness.le
    ~nicknames:
      [
        "powerpc64el";
        "powerpc64+le";
...
let enable_loader () =
  ...
  let request_info doc =
    ...
    let request =
      ...
      Ogre.return (arch, little, format)
    in
...
let map_powerpc () =
  ...
  if Theory.Target.belongs parent t then
    match
      (Theory.Target.bits t, Theory.Endianness.(Theory.Target.endianness t = le))
    with
    | 32, false -> `ppc
    | 64, false -> `ppc64
    | 64, true -> `ppc64le
...
let enable_llvm_decoder () =
  ...
  if Theory.Target.belongs parent t then
    if Theory.Target.belongs powerpc32bi t then llvm_powerpc32
    else llvm_powerpc64
...
let enable_pcode_decoder () =
  ...
  ( Dis.register pcode @@ fun t ->
    let endian =
      if Theory.(Endianness.(Target.endianness t = eb)) then "BE" else "LE"
    in
    let triple =
      sprintf "PowerPC:%s:%d:default" endian (Theory.Target.bits t)
    in
