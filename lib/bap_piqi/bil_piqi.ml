...
let unop_of_piqi = function `uneg -> NEG | `unot -> NOT
...
let rec exp_to_piqi : exp -> Stmt_piqi.expr = function
  | Load (m, i, e, s) ->
  ...
  | Var v -> `var (var_to_piqi v)
  | Int v -> `inte { P.Inte.int = Bitvector.to_string v }
  | Cast (ct, size, e) ->
...
let rec exp_of_piqi = function
  ...
  | `unop { P.Unop.op; exp } ->
      ...
      UnOp (uop, e)
  | `var v -> Var (var_of_piqi v)
  | `inte { P.Inte.int = s } -> Int (Bitvector.of_string s)
