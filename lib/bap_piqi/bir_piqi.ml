...
module Get = struct
  ...
  let intent = function `in_ -> In | `out -> Out | `both -> Both
  ...
  let jump = function
    ...
    | `call { R.Call.target = t; return = r } ->
        Call (Call.create ?return:(r >>| label) ~target:(label t) ())
...
module Put = struct
  ...
  let intent = function In -> `in_ | Out -> `out | Both -> `both
  ...
  let jump = function
    ...
    | Call c ->
        `call
          R.Call.
            { target = label (Call.target c); return = Call.return c >>| label }
  ...
  let def d =
    R.Def.
      { tid = tid (Term.tid d); lhs = var (Def.lhs d); rhs = exp (Def.rhs d) }
  ...
  let list t f term = Term.enum t term |> Seq.map ~f |> Seq.to_list
  ...
  let program t = R.Program.{ tid = tid (Term.tid t); subs = list sub_t sub t }
end
