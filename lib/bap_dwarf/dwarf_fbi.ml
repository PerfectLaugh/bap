...
module Value = struct
  type hi_addr = Abs of addr | Rel of int64 [@@deriving sexp, bin_io, compare]

  type t = Id of string | Lo of addr | Hi of hi_addr
...
module Fn = struct
  module T = struct
    type t = { pc_hi : addr option; pc_lo : addr }
    [@@deriving sexp, bin_io, compare, fields]

    let int_of_int64 off : int Or_error.t =
      Result.of_option
        ~error:Error.(create "hi pc offset is too big" off sexp_of_int64)
        (Int64.to_int off)
    ...
    let create ?pc_hi ~pc_lo () =
      let pc_hi =
        ...
        | Some (Value.Rel off) ->
            int_of_int64 off >>| fun off -> Some Addr.(pc_lo ++ off)
      in
...
let fn_of_values vs : (string * fn) Or_error.t =
  ...
  | Id name :: Lo pc_lo :: _ -> Fn.create ~pc_lo () >>| fun fn -> (name, fn)
  | Id name :: vs ->
      errorf "'%s' is degenerate" name |> fun err ->
      tag_arg err "attrs" vs (sexp_of_list sexp_of_value)
  | vs ->
      errorf "got anonymous function" |> fun err ->
      tag_arg err "attrs" vs (sexp_of_list sexp_of_value)
...
let read_scheme s str_sec action str ~pos_ref =
  ...
  let rec emit attr form =
    ...
    | Attr.High_pc, Form.Const len ->
        action
          (Input.map (Input.const len s.endian) ~f:(fun x -> Value.(hi (Rel x))))
    | Attr.Name, Form.Strp ->
        let reader str ~pos_ref =
          number str ~pos_ref >>= fun off ->
          let pos_ref = ref (Buffer.pos str_sec + off) in
          name (Buffer.data str_sec) ~pos_ref
        in
        action reader
    | _ -> (
...
let create_table s str_sec str ~pos_ref =
  ...
  let rec fill () =
    ...
    | code -> (
        ...
        Input.skip str ~pos_ref ~bytes:1 >>= fun () ->
        (* children flag *)
        let action =
          let open Tag in
          match tag with
          ...
          | _ -> Input.drop
        in
        read_scheme s str_sec action str ~pos_ref >>= fun scheme ->
        match Hashtbl.add abbrs ~key:code ~data:scheme with
        | `Ok -> fill ()
        | `Duplicate -> errorf "Duplicate entry code: %d" code)
  in
  fill () >>| fun () -> abbrs

let lookup table code =
  let error = Error.create "unknown code" code sexp_of_int in
...
let run_scheme scheme str ~pos_ref : value list Or_error.t =
  List.fold scheme ~init:(Ok []) ~f:(fun vs step ->
      vs >>= fun vs ->
      step str ~pos_ref >>| function Some v -> v :: vs | None -> vs)
...
let create data : t Or_error.t =
  ...
  let from_info read = read info ~pos_ref:info_pos in
  ...
  let read_unit () =
    ...
    Sequence.unfold_step ~init ~f:(fun (abbrs, cu_end) ->
        ...
        | Ok (Yield { value = fn; state = () }) ->
            Yield { value = fn; state = (abbrs, cu_end) }
