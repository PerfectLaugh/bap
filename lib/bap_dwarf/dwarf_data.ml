...
type 'a t = { sections : 'a buffer Section.Map.t; endian : endian }

let create endian ss : 'a t Or_error.t =
  let open Or_error.Monad_infix in
  Section.Map.of_alist_or_error ss >>| fun sections -> { sections; endian }

let section t section =
  match Map.find t.sections section with
  | Some buf -> Ok buf
  | None ->
      Or_error.errorf "section %s is not available" @@ Section.to_string section
