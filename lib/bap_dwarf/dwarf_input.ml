...
let read_word reader ~size str ~pos_ref =
  try_with ~backtrace:true (fun () -> read_word_exn reader size str ~pos_ref)
...
let int = { of_int64 = ok_some Int64.to_int; of_int32 = ok_some Int32.to_int }
let int32 = { of_int64 = ok_some Int64.to_int32; of_int32 = ok_any Fn.id }
let int64 = { of_int64 = ok_any Fn.id; of_int32 = ok_any Int64.of_int32 }
let addr = { of_int64 = ok_any Addr.of_int64; of_int32 = ok_any Addr.of_int32 }
...
let get_int16 ~byte_order ~buf =
  unpack_signed_16 ~byte_order ~buf:(of_string buf)

let get_int32 ~byte_order ~buf =
  unpack_signed_32 ~byte_order ~buf:(of_string buf)

let get_int64 ~byte_order ~buf =
  unpack_signed_64 ~byte_order ~buf:(of_string buf)

(** [make_reader inj endian size] will read a word of [size], with byte order
    specified with [ending] and then it will try to inject it into ['a] type
    using [inj] function. *)
...
let map read ~f = fun str ~pos_ref -> read str ~pos_ref >>| fun x -> f x

let pair read1 read2 str ~pos_ref =
  read1 str ~pos_ref >>= fun w1 ->
  read2 str ~pos_ref >>= fun w2 -> return (w1, w2)
...
let offset = read_int
let char : int reader = read_word get_uint8 ~size:1

let skip ~bytes str ~pos_ref =
  let rec loop n =
    if n > 0 then char str ~pos_ref >>= fun (_ : int) -> loop (n - 1)
    else return ()
  in
  loop bytes
...
let version str ~pos_ref =
  pair char char str ~pos_ref >>| function 0, n | n, 0 -> n | _, n -> n
...
let read_big_leb128 str ~pos_ref =
  Leb128.read ~signed:false str ~pos_ref >>= Leb128.to_int64
