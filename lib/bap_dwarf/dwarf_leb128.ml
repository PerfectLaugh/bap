...
type t = { negative : bool; data : int list } [@@deriving sexp, bin_io, compare]
type 'a encoder = ?signed:bool -> 'a -> t
type 'a decoder = t -> 'a Or_error.t
type cursor = { bit : int; pos : int; len : int }
...
let pack7 ~negative (s : Bytes.t) =
  ...
  Sequence.unfold ~init:(Cursor.init n) ~f:(fun cur ->
      ...
      if cur.bit >= n * 8 then None
        ...
        let word =
          if negative && (Cursor.next cur).bit >= n * 8 then
            word lor (0b0111_1111 land (lnot 0 lsl ((n * 8) - cur.bit)))
          else word
        in
        Some (word, Cursor.next cur))

(** accepts a sequence of bytes (in little endian) and performs the last step of
    conversion to LEB128 format, i.e., sets MSB to 1 for all bytes except the
    last one and drops the tail *)
let fix ~negative (s : int Sequence.t) : t =
  {
    negative;
    data =
      List.fold ~init:[] (Sequence.to_list_rev s) ~f:(fun acc x ->
          match (acc, x) with
          | [], 0 when not negative -> []
          | [], 127 when negative -> []
          | [], x -> [ x ]
...
(** [decode leb] transforms a number in a LEB128 representation to a binary form
*)
let decode (leb : t) buf ~off ~len : unit =
  let m = List.length leb.data in
  let (_ : Cursor.t) =
    List.foldi leb.data ~init:(Cursor.init len) ~f:(fun i cur x ->
        let bit0 = Cursor.bit0 cur in
        let word = Bits.get_uint16 ~buf ~pos:cur.pos in
        let word = word lor ((x land 0b0111_1111) lsl bit0) in
        let word =
          if leb.negative && i = m - 1 then word lor (lnot 0 lsl (bit0 + 7))
          else word
        in
        ...
        Cursor.next cur)
  in
  if leb.negative then
    for i = m to len - 1 do
      Bytes.set buf i '\255'
    done

let encode ~negative bits : t = fix ~negative (pack7 ~negative bits)

let read_exn ?(signed = false) bits ~pos_ref =
  let bits = Bytes.of_string bits in
  let s =
    Sequence.unfold ~init:`Continue ~f:(function
      | `Stop -> None
      | `Continue ->
          let x = Bits.get_uint8 ~buf:bits ~pos:!pos_ref in
          incr pos_ref;
          if x land (1 lsl 7) = 0 then Some (x, `Stop) else Some (x, `Continue))
  in
  let data = Sequence.to_list_rev s in
  let negative =
    match data with x :: _ -> signed && x land (1 lsl 6) <> 0 | [] -> false
  in
  { negative; data = List.rev data }

let read ?signed bits ~pos_ref =
  Or_error.try_with ~backtrace:true (fun () -> read_exn ?signed bits ~pos_ref)

let size t = max 1 @@ List.length t.data

let write t bits ~pos : unit =
  List.iteri t.data ~f:(fun i x -> Bits.set_int8 ~buf:bits ~pos:(pos + i) x)
...
let int =
  ...
  let open Bits in
  let safe cast x = Option.value_exn ~message:"cast failed" (cast x) in
  let read, write =
    ...
    | W64 ->
        ( (fun buf -> Int64.to_int (get_int64 ~buf ~pos:0)),
          fun buf x -> set_int64 ~buf ~pos:0 @@ Int64.of_int x )
  in
  { size = num_bits word_size / 8; read; is_negative = Int.is_negative; write }
...
let decoder (repr : 'a repr) x : 'a Or_error.t =
  ...
  match if Bytes.length bits > repr.size then None else repr.read bits with
  | Some n -> Ok n
  | None ->
      Or_error.errorf "number doesn't fit: %d -> %d" (Bytes.length bits)
        repr.size
