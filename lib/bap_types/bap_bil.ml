...
type var = Bap_var.t [@@deriving bin_io, compare, sexp]
...
type cast = Cast.cast [@@deriving bin_io, compare, sexp]
...
type binop = Binop.binop [@@deriving bin_io, compare, sexp]

(** Unary operations implemented in the IR *)
module Unop = struct
  type unop = NEG  (** Negate. (2's complement) *) | NOT  (** Bitwise not. *)
  [@@deriving bin_io, compare, sexp]
end

type unop = Unop.unop [@@deriving bin_io, compare, sexp]

module Exp = struct
  type exp =
    | Load of exp * exp * endian * size  (** Load (mem, idx, endian, size) *)
    ...
    | Var of var
    | Int of word
    | Cast of cast * nat1 * exp  (** Cast to a new type *)
    | Let of var * exp * exp
