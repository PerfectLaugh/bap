...
module Const = struct
  ...
  type t = Immediate of word | Undefined of string * typ | Top
...
module Propagate (SM : Monad.State.S2) = struct
  ...
  class exp_propagator =
    object (self)
      method update const v : unit r =
        SM.get () >>= fun ctxt -> SM.put (update ctxt ~key:v ~data:const)
      ...
      method eval_var v : exp r =
        ...
        | Some c -> (
            match Const.exp_of_const c with
            | None -> SM.return (Var v)
            | Some e -> SM.return e)

      method eval_load ~mem ~addr e s =
        self#eval_exp mem >>= fun mem ->
        self#eval_exp addr >>= fun addr -> SM.return (load ~mem ~addr e s)

      method eval_store ~mem ~addr data e s =
        self#eval_exp mem >>= fun mem ->
        self#eval_exp addr >>= fun addr ->
        self#eval_exp data >>= fun data -> SM.return (store ~mem ~addr data e s)

      method eval_unop op e = self#eval_exp e >>= fun e -> SM.return (unop op e)

      method eval_binop op x y =
        self#eval_exp x >>= fun x ->
        self#eval_exp y >>= fun y -> SM.return (binop op x y)
      ...
      method eval_cast c s e =
        self#eval_exp e >>= fun e -> SM.return (cast c s e)
      ...
      method eval_extract hi lo e =
        self#eval_exp e >>= fun e -> SM.return (extract ~hi ~lo e)

      method eval_concat x y =
        self#eval_exp x >>= fun x ->
        self#eval_exp y >>= fun y -> SM.return (concat x y)
  ...
  let join =
    Map.merge ~f:(fun ~key:_ -> function
      | `Both (c1, c2) -> if Const.equal c1 c2 then Some c1 else Some Const.top
      | _ -> Some Const.top)
  ...
  let has_unconditional_jmps bil =
    List.exists bil ~f:(function Jmp _ -> true | _ -> false)

  class bil_propagator =
    object (self)
      ...
      method eval (bil : bil) : bil r =
        SM.List.fold bil ~init:[] ~f:(fun acc s ->
            self#eval_stmt s >>= fun s -> SM.return (s :: acc))
        >>= fun bil -> SM.return (List.rev bil)

      method eval_move v e : stmt r =
        self#eval_exp e >>= fun e ->
        self#update_var v e >>= fun () -> SM.return (move v e)
      ...
      method eval_if cond yes no =
        ...
        SM.get () >>= fun ctxt_no ->
        let ctxt =
          match (has_unconditional_jmps yes, has_unconditional_jmps no) with
          | false, false -> join ctxt_yes ctxt_no
          | true, true -> ctxt
          | false, _ -> ctxt_yes
          | _ -> ctxt_no
        in
        SM.put ctxt >>= fun () -> SM.return (if_ cond yes no)
      ...
      method eval_while cond body =
        ...
        SM.get () >>= fun ctxt' ->
        let ctxt =
          if has_unconditional_jmps body then ctxt else join ctxt ctxt'
        in
        SM.put ctxt >>= fun () -> SM.return (while_ cond body)

      method eval_jmp e : stmt r =
        self#eval_exp e >>= fun e -> SM.return (jmp e)
    end
end
...
let propagate_consts bil =
  Monad.State.eval ((new P.bil_propagator)#eval bil) P.new_ctxt
...
let prune_dead_virtuals bil =
  ...
  let is_dead var live = Var.is_virtual var && not (Set.mem live var) in
  let rec loop init bil =
    List.fold (List.rev bil) ~init:([], init) ~f:(fun (bil, live) s ->
        match s with
        ...
        | While (cond, body) ->
            let live' =
              List.fold body ~init:live ~f:(fun live s ->
                  live ++ Stmt_helpers.free_vars s)
            in
