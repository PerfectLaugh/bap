...
(** This module is included into [Bap.Std], you need to open it specifically if
    you're developing inside BAP *)
module Std = struct
  module Integer = Integer
  (** A definition for a regular type, and a handy module, that can create
      regular types out of thin air. *)
  ...
  module Legacy = struct
    ...
    [@@alert deprecated "[since 2017-04] use new monads library"]
  end
  [@@alert
  ...
  include Type.Export
  (** This module exports first-class type definitions, like [reg8_t], or
      [mem32_t]. Look at [Bap_type], for more. *)
  ...
  (** Bitvector is an ubiquitous module, that represents bitstrings and
      arbitrary sized numbers at once. It is used to represent addresses,
      imidiate operands and memories. It is suggested, that depending on a
      context, one should use [Addr] module and corresponding type to represent
      addresses, and [Word] module to denote other values. Although they all
      will point to the same type, it will be a little more explicit, and more
      understandable. *)

  (** Address representation. This module not only includes [Bit_vector]
      implementation, but also adds some helpful functions, that makes good
      sense for addresses. Look at [Bap_addr] for more information. *)
  module Addr = struct
    include Bitvector
    include Bap_addr
  end
  ...
  (** A fancy abbreviation for bitvectors, that are supposed to represent data
  *)

  module Value = Bap_value
  module Dict = Value.Dict

  (** Byte endian. This is the only not first class type in a bap-types. Sorry,
      no maps and tables for this type. *)
  type endian = Bap_common_types.endian = LittleEndian | BigEndian
  [@@deriving sexp, bin_io, compare]

  (** {2 Type abbreviations}
      In this section there is a type abbreviation for all main types and
      auxiliary types. *)

  type 'a size_p = 'a Size.p [@@deriving bin_io, compare, sexp]
  (** polymorphic sizes allows one to specify explicitly a set of acceptable
      sizes, e.g., [type mmx = [`r128 | `r256] poly_size] *)

  type nonrec addr_size = Bap_common_types.addr_size
  [@@deriving bin_io, compare, sexp]
  (** [addr_size] is a subset of sizes that contains only two instances [`r32]
      and [`r64] *)
