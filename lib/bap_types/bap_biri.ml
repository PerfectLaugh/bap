...
class context ?main p =
  object
    ...
    method main =
      match main with Some main -> Some main | None -> Term.first sub_t p
...
type 'a s = Empty | First of 'a term | Ready of ('a term * 'a term)

let bad_phi phi = Ir_phi.select_or_unknown phi (Tid.create ())
...
module Make (SM : Monad.State.S2) = struct
  ...
  let update_context f = SM.get () >>= fun c -> SM.put (f c)

  let set_next term : #context u =
    update_context (fun c ->
        match term with
        | None -> c#set_next None
        | Some term -> c#set_next (Some (Term.tid term)))

  (** [eval_terms f cls c c' blk] eval terms of block [blk] by applying function
      [f] to each term of class [cls].

      The following invariants are preserved:

      1. if term [t] is not next, then [f] is not applied and [t] is skipped 2.
      if [f] is applied to [t], then [next] is set to the successor of [t], just
      before the application, where successor of t is
      - next element in sequence if [t] is not the last one
      - [t'] if [t] is last and [c] is not [None], and [t'] is the first element
        of non empty sequence of terms of class [c]
      - [t'] if [t] is last and [c] is not [None] and sequence of terms of class
        [c] is empty and [t'] is first element of non empty sequence of term of
        class [c']
      - None otherwise *)
  let eval_terms eval c1 c2 c3 blk : #context u =
    let set_next next =
      ...
      | None -> (
          ...
          | None -> SM.return ()
          | Some c2 -> (
              match Seq.hd (Term.enum c2 blk) with
              | Some t -> set_next (Some t)
              | None -> (
                  match c3 with
                  | None -> SM.return ()
                  | Some c3 -> set_next (Seq.hd (Term.enum c3 blk)))))
    in
    let step t1 next =
      SM.get () >>= fun c ->
      match c#next with
      ...
      | _ -> set_next next >>= fun () -> eval c1 t1
    in
    let terms = Term.enum c1 blk in
    set_next (Seq.hd terms) >>= fun () ->
    SM.Seq.fold terms ~init:Empty ~f:(fun m t ->
        match m with
        | Empty -> SM.return (First t)
        | First t1 -> set_next (Some t1) >>| fun () -> Ready (t1, t)
        | Ready (t1, t2) ->
            step t1 (Some t2) >>= fun () -> SM.return (Ready (t2, t)))
    >>= function
    | Empty -> SM.return ()
    | First t -> step t None
    | Ready (t1, t2) -> step t1 (Some t2) >>= fun () -> step t2 None
  ...
  class ['a] t =
    object (self)
      ...
      method private do_enter_term : 't 'p. ('p, 't) cls -> 't term -> 'a u =
        fun cls t ->
      ...
      method eval : 't 'p. ('p, 't) cls -> 't term -> 'a u =
        fun cls t ->
          ...
          Term.switch cls t
            ...
            ~sub:self#eval_sub ~arg:self#eval_arg ~blk:self#eval_blk
            ~phi:self#eval_phi ~def:self#eval_def ~jmp:self#eval_jmp
          >>= fun () -> self#leave_term cls t

      method eval_sub sub : 'a u =
        set_next (Term.first blk_t sub) >>= fun () ->
        eval_args `enter sub (self#eval arg_t) >>= fun () ->
        self#eval_fun sub >>= fun () -> eval_args `leave sub (self#eval arg_t)
      ...
      method private eval_fun sub : 'a u =
        ...
        | Some p -> (
            ...
            | Some blk -> self#eval blk_t blk >>= fun () -> self#eval_fun sub
            | None -> (
                match Term.find sub_t c#program p with
                | None -> SM.return ()
                | Some calee ->
                    self#eval sub_t calee >>= fun () -> self#eval_fun sub))
      ...
      method eval_arg arg : 'a u =
        self#eval_exp (Ir_arg.rhs arg) >>= self#update (Ir_arg.lhs arg)

      method eval_def def : 'a u =
        self#eval_exp (Ir_def.rhs def) >>= self#update (Ir_def.lhs def)

      method eval_phi phi : 'a u =
        self#do_enter_term phi_t phi >>= fun () ->
        SM.get () >>= fun c ->
        self#eval_exp (resolve_phi phi c#trace) >>= fun v ->
        self#update (Ir_phi.lhs phi) v >>= fun () -> self#leave_term phi_t phi
