...
module Make (Interval : Interval) = struct
  ...
  *)
  ...
  let bal l x d r =
    ...
    if hl > hr + 2 then
      ...
      | Some t -> (
          ...
          | Some rhs ->
              create
                (create t.lhs t.key t.data rhs.lhs)
    ...
    let rec go = function
      ...
      | Some t ->
          if is_inside t.key a then
            go t.lhs >>= fun () ->
            yield (t.key, t.data) >>= fun () -> go t.rhs
          else go t.lhs >>= fun () -> go t.rhs
    in
    start |> go |> run
  ...
  let has_intersections t r =
    let open Interval in
    let open Point in
    let p, q = (lower t.key, upper t.key) and x, y = (lower r, upper r) in
    if p <= x then x <= q && p <= y else p <= y && x <= q
  ...
  let query ~skip_if ~take_if start r =
    ...
    let rec go = function
      ...
      | Some t when take_if t r ->
          go t.lhs >>= fun () ->
          yield (t.key, t.data) >>= fun () -> go t.rhs
      | Some t -> go t.lhs >>= fun () -> go t.rhs
    in
    start |> go |> run

  let dominators m r =
    query m r ~skip_if:can't_be_dominated ~take_if:is_dominated

  let intersections m r =
    query m r ~skip_if:can't_be_in_tree ~take_if:has_intersections
  ...
  let rec map m ~f =
    Option.map m ~f:(fun m ->
        { m with lhs = map m.lhs ~f; data = f m.data; rhs = map m.rhs ~f })
  ...
  let remove_if ~leave_if ~remove_if map mem =
    let rec remove = function
      ...
      | Some t when remove_if t mem -> splice (remove t.lhs) (remove t.rhs)
      | Some t -> bal (remove t.lhs) t.key t.data (remove t.rhs)
    in
    remove map

  let remove_intersections map mem =
    remove_if map mem ~leave_if:can't_be_in_tree ~remove_if:has_intersections

  let remove_dominators map mem =
    remove_if map mem ~leave_if:can't_be_dominated ~remove_if:is_dominated

  let filter_mapi map ~f =
    let rec fmap = function
      | None -> None
      | Some t -> (
          match f t.key t.data with
          | None -> splice (fmap t.lhs) (fmap t.rhs)
          | Some data -> bal (fmap t.lhs) t.key data (fmap t.rhs))
    in
    fmap map
  ...
  module C = Container.Make (struct
    ...
    let rec fold m ~init ~f =
      Option.fold m ~init ~f:(fun acc m ->
          fold m.rhs ~init:(f (fold m.lhs ~init:acc ~f) m.data) ~f)

    let rec iter m ~f =
      Option.iter m ~f:(fun m ->
          iter m.lhs ~f;
          f m.data;
          iter m.rhs ~f)
...
module Make_binable (Interval : Interval_binable) = struct
  ...
  include (
    Base : S with type key := key and type point := point and type 'a t := 'a t)
end
