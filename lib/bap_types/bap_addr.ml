...
module Make (Int : Core_int) = struct
  ...
  (** [create_modulo_width x] takes a number represented as a [n]-bits length
      bitvector, casts it to an int modulo [n].

      Since [Int.t] should be either [int32] or [int64] we can be sure that
      number of bits is less then maximum number represantable by OCaml's [int]
      type. *)
