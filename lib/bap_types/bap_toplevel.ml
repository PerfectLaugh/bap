...
let eval slot exp =
  try_eval slot exp |> function Ok v -> v | Error x -> raise (Conflict x)
...
let this = Knowledge.Symbol.intern ~package "main" main
...
let exec stmt =
  try_exec stmt |> function Ok () -> () | Error err -> raise (Conflict err)

let put slot exp =
  exec
  @@ ( exp >>= fun v ->
       this >>= fun x -> Knowledge.provide slot x (Some v) )

let get slot =
  eval slot this |> function None -> raise Not_found | Some x -> x

let () =
  Stdlib.Printexc.register_printer @@ function
  | Conflict err -> Some (Format.asprintf "%a" Knowledge.Conflict.pp err)
  | _ -> None
