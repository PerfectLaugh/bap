...
module Make (Key : Key) = struct
  ...
  type 'a t = { mutable data : 'a option; subs : 'a t Tokens.t }
  ...
  let change trie k f { return } =
    ...
    and found parent c =
      Hashtbl.change parent c ~f:(function
        | None -> f None >>| init
    ...
    and find parent n c =
      ...
      | None -> (
          ...
          | Some _ ->
              let sub = create () in
              Hashtbl.add_exn parent ~key:c ~data:sub;
              loop (n + 1) sub)
    in
    if len > 0 then loop 0 trie else trie.data <- f trie.data
  ...
  let walk root k ~init ~f =
    ...
    let rec lookup best n trie =
      ...
      | Some sub ->
          ...
          if n + 1 = len then best else lookup best (n + 1) sub
    in
  ...
  let length trie =
    let rec count trie =
      ...
      Hashtbl.fold ~init:n trie.subs ~f:(fun ~key:_ ~data:trie n ->
          n + count trie)
    in
    count trie
  ...
  let find trie k =
    match longest_match trie k with
  ...
  let fold t ~init ~f =
    let rec fold init rprefix t =
      let init =
        match t.data with
        | None -> init
        | Some data -> f init (List.rev rprefix) data
      in
    ...
    and fold_table init rprefix cs =
      Hashtbl.fold cs ~init ~f:(fun ~key ~data init ->
          ...
          fold init rprefix data)
    in
    fold init [] t

  let iter t ~f = fold t ~init:() ~f:(fun () tokens data -> f tokens data)

  let make_printer pp_prefix pp_val ppf t =
    let rec pp_trie ppf rprefix t =
      Option.iter t.data ~f:(fun data ->
          fprintf ppf "@[<2>%a@ %a@]@;" pp_val data pp_prefix (List.rev rprefix));
      pp_table ppf rprefix t.subs
    and pp_table ppf rprefix cs =
      Hashtbl.iteri cs ~f:(fun ~key ~data ->
          let rprefix = key :: rprefix in
          pp_trie ppf rprefix data)
    in
    pp_trie ppf [] t
...
module String = struct
  ...
  module Suffix = Make (struct
    ...
    let nth_token s n = s.[String.length s - n - 1]
  end)
end
