...
let is_assigned ?(strict = false) x =
  exists
    (object
       inherit [unit] bil_finder

       method! enter_move y _ cc =
         if Bap_var.(x = y) && not (strict && under_condition) then
           cc.return (Some ());
         cc
    end)

class exp_reference_finder x =
  object
    inherit [unit] exp_finder

    method! enter_var y cc =
      if Bap_var.(x = y) then cc.return (Some ());
      cc
  end

class reference_finder x =
  object (self)
    inherit [unit] bil_finder
    ...
    method! visit_move y rhs goto =
      ...
      if Bap_var.(x = y) then goto.return None else goto

    method! enter_var y cc = exp_reference#enter_var y cc
  end

let is_referenced x = exists (new reference_finder x)

let prune_unreferenced ?(such_that = fun _ -> false) ?(physicals = false)
    ?(virtuals = false) stmt =
  let prune x =
    Bap_var.(
      (physicals && is_physical x) || (virtuals && is_virtual x) || such_that x)
  in
...
class negative_normalizer =
  object
    inherit bil_mapper as super

    method! map_binop op e1 e2 =
      match (op, e2) with
      | Binop.PLUS, Exp.Int arg when Word.(is_negative (signed arg)) ->
...
let rec addr_intersection (x, xs) (y, ys) =
  ...
  let ze = min xe ye in
  if x <= y then if ze > y then Some (y, ok_exn (to_int (ze - y))) else None
  else addr_intersection (y, ys) (x, xs)

class substitution x y =
  object (self)
    inherit bil_mapper as super

    method! map_let z ~exp ~body =
      if Bap_var.(z = x) then Let (z, self#map_exp exp, body)
      else super#map_let z ~exp ~body

    method! map_var z =
      match super#map_var z with Exp.Var z when Bap_var.(z = x) -> y | z -> z
  end
...
module Type = struct
  ...
  let check xs = match check xs with None -> Ok () | Some err -> Error err
...
module Eff = struct
  ...
  module Nz = struct
    ...
    and cast t _ x =
      match t with SIGNED | UNSIGNED -> bits x | HIGH | LOW -> narrow x

    and extract hi lo x = if hi - lo + 1 >= width x then bits x else narrow x
    and binop op x y = match op with OR -> join x y | _ -> Maybe

    and concat x y =
      match (bits x, bits y) with Exists, _ | _, Exists -> Exists | _ -> Maybe

    and narrow x = match bits x with Exists -> Maybe | x -> x
    and bitneg x = match bits x with Forall -> Exists | b -> b

    and bitnot x =
      match bits x with Forall -> Empty | Empty -> Forall | x -> x

    and join x y =
      match (bits x, bits y) with
  ...
  *)
  ...
  let rec eff = function
    ...
    | BinOp ((DIVIDE | SDIVIDE | MOD | SMOD), x, y) ->
        all [ div y; eff x; eff y ]
    | BinOp (_, x, y) | Concat (x, y) -> all [ eff x; eff y ]
    | Ite (x, y, z) -> all [ eff x; eff y; eff z ]
    | UnOp (_, x) | Cast (_, _, x) | Extract (_, _, x) -> eff x
    | Let (_, x, y) -> all [ eff x; eff y ]

  and div y = match Nz.bits y with Nz.Maybe | Nz.Empty -> raise | _ -> none
...
module Simpl = struct
  ...
  let exp ?(ignore = []) =
    ...
    and let_ v x y =
      ...
      | r -> Let (v, r, exp y)
    and concat x y =
      match (exp x, exp y) with
      | Int x, Int y -> Int (Word.concat x y)
      | x, y -> Concat (x, y)
    and cast t s x =
      match exp x with Int w -> Int (Apply.cast t s w) | x -> Cast (t, s, x)
    and extract hi lo x =
      match exp x with
    ...
    and binop op x y =
      let width =
        match Type.infer_exn x with Type.Imm s -> s | _ -> failwith "binop"
      in
      ...
      | op, BinOp (op', x, Int p), Int q when op_eq op op' && is_associative op
        ->
  ...
  let bil ?ignore =
    ...
    and if_ c ts fs =
      match exp c with
      | Int x -> if Word.is_zero x then bil fs else bil ts
...
let fix compare f x =
  let rec loop slow fast =
    if compare slow fast = 0 then fast
      ...
      let fast' = f fast in
      if compare fast' fast = 0 then fast else loop (f slow) (f fast')
  in
  let slow = x and fast = f x in
  if compare slow fast = 0 then fast else loop slow fast
...
module Trie = struct
    ...
    map
      (object
         ...
         method! map_sym var =
           ...
           Hashtbl.find_or_add vars name ~default:(fun () ->
               Bap_var.create name ty)
  ...
  let prune x = prune_unreferenced ~virtuals:true x

  let simplify =
    [ prune; normalize_negatives; Simpl.bil ] |> List.reduce_exn ~f:Fn.compose
...
module Constant_folder = struct
  ...
  class main =
    object
      ...
      method! map_let var ~exp ~body = super#map_let var ~exp ~body |> Simpl.exp
      method! map_ite ~cond ~yes ~no = super#map_ite ~cond ~yes ~no |> Simpl.exp
      method! map_if ~cond ~yes ~no = super#map_if ~cond ~yes ~no |> Simpl.bil
      method! map_while ~cond bil = super#map_while ~cond bil |> Simpl.bil
    end
end
...
module Normalize = struct
  ...
  (** substitutes let bound variables with theirs definitions. May change the
      semantics in general, as the function will copy expressions that are
      non-generative. However, the semantics is preserved wrt to the official
      BIL semantics, that states that all expressions have no side effects. *)
  let reduce_let exp =
    let rec ( / ) d x =
      match x with
      ...
      | Let (v, x, y) -> ((v, d / x) :: d) / y
      | Var x as var -> (
          match List.Assoc.find ~equal:Var.equal d x with
          | None -> var
          | Some exp -> exp)
    in
    [] / exp

  (** [nth n x] projects [n]'th byte from a word. requires that a word is at
      least [n] bytes long.*)
  ...
  let make_succ m =
    let int n =
      ...
      Exp.Int (Word.of_int ~width:(Size.in_bits size) n)
    in
  ...
  *)
  let expand_store m a x e s =
    ...
    let nth i =
      if [%compare.equal: endian] e BigEndian then nth (n - i - 1) else nth i
    in
    let rec expand i =
      if i >= 0 then
        Exp.Store (expand (i - 1), a ++ i, nth i x, LittleEndian, vs)
      else m
    in
    if Size.equal vs s then Exp.Store (m, a, x, e, s) else expand (n - 1)
  ...
  *)
  let expand_load m a e s =
    ...
    let ( ++ ) = make_succ m in
    let cat x y =
      if [%compare.equal: endian] e LittleEndian then Exp.Concat (y, x)
      else Exp.Concat (x, y)
    in
    let load a = Exp.Load (m, a, e, vs) in
    let rec expand a i =
      if i > 1 then cat (load a) (expand (a ++ 1) (i - 1)) else load a
    in
    if Size.equal vs s then load a else expand a (Size.in_bytes s)
  ...
  *)
  let assume_exp (Assume (c, yes)) =
    ...
    @@ object
         ...
         method! map_ite ~cond ~yes:x ~no:y =
           if Bap_exp.equal c cond then if yes then x else y
  ...
  let find_cond_in_exp =
    find
    @@ object
         inherit [exp] exp_finder
         method! enter_ite ~cond ~yes:_ ~no:_ { return } = return (Some cond)
       end

  let find_cond_in_stmt =
    find_stmt
    @@ object
  ...
  let find_non_generative_mem =
    find
    @@ object
         inherit [exp] exp_finder

         method! enter_load ~mem ~addr:_ _e _s ({ return } as continue) =
           match mem with Var _ -> continue | exp -> return (Some exp)
         (* always non-generative in well-typed programs *)
       end
  ...
  *)
  let hoist_non_generative_expressions =
    ...
    let rec exp : exp -> exp hoister = function
      ...
      | BinOp (op, x, y) ->
          exp x >>= fun x ->
          exp y >>= fun y -> return (BinOp (op, x, y))
      | Load (m, a, e, s) ->
          hoist m >>= fun m ->
          hoist a >>= fun a -> return (Load (m, a, e, s))
      | Store (m, a, v, e, s) ->
          exp m >>= fun m ->
          exp a >>= fun a ->
          exp v >>= fun v ->
          hoist a >>= fun a ->
          hoist v >>= fun v -> return (Store (m, a, v, e, s))
      | Cast (t, s, x) -> exp x >>= fun x -> return (Cast (t, s, x))
      | Let (v, x, y) ->
          exp x >>= fun x ->
          exp y >>= fun y ->
          hoist x >>= fun x -> return (Let (v, x, y))
      | Ite (x, y, z) ->
          exp x >>= fun x ->
          exp y >>= fun y ->
          exp z >>= fun z ->
          hoist x >>= fun x -> return (Ite (x, y, z))
      | Extract (n, m, x) -> exp x >>= fun x -> return (Extract (n, m, x))
      | Concat (x, y) ->
          exp x >>= fun x ->
          exp y >>= fun y -> return (Concat (x, y))
    in
    let rec stmt : stmt -> stmt list = function
      | (Special _ | CpuExn _) as s -> [ s ]
      | Move (v, x) -> exp x >>| fun x -> Move (v, x)
      | Jmp x -> exp x >>| fun x -> Jmp x
      | If (c, xs, ys) -> exp c >>| fun c -> If (c, stmts xs, stmts ys)
      | While (c, xs) -> exp c >>| fun c -> While (c, stmts xs)
  ...
  *)
  ...
  (** [hoist_whiles ~has_anomality : bil -> bil] generic while condition
      hoister. hoists an offending expression from a conditional of a while
      statement into two separate assignments, e.g.,

      [while (c) prog] if [has_anomality c] is transformed to
      [v := c; while (v) {prog; v := c}]

      [has_anomality c : bool] is applied to the conditional expression of a
      while statement and should return [true] if a conditional must be hoisted.

      requires: [has_anomality (Var _) = false] provides: [has_anomality c] is
      false forall while conditionals. *)
  ...
  let normalize_if_conditionals =
    ...
    @@ object (self)
         ...
         method! map_if ~cond ~yes ~no =
           ...
           if not_allowed_in_conditional cond then
             let v = new_hoisted_var () in
             [ Move (v, cond); If (Var v, yes, no) ]
           else [ If (cond, yes, no) ]
       end

  let normalize_conditionals =
    Fn.compose normalize_while_conditionals normalize_if_conditionals
  ...
  let rec split_ite bil =
    map
      (object (self)
         ...
         method private split stmt =
           match find_cond_in_stmt stmt with
           | None -> [ stmt ]
           | Some x ->
               split_ite [ If (x, [ with_true x stmt ], [ with_false x stmt ]) ]
      end)
      bil

  let rec hoist_stores bil =
    map
      (object (self)
         inherit bil_mapper
         ...
         method private hoist e stmt =
           match find_non_generative_mem e with
           | None -> [ stmt ]
           | Some store ->
               let v = memory_of_store store in
               hoist_stores
                 (Move (v, store) :: substitute store (Var v) [ stmt ])
      end)
      bil
...
module Exp = struct
  ...
  let find (finder : 'a #finder) es : 'a option = finder#find es
  let exists finder ss = Option.is_some @@ finder#find ss
    ...
    @@ fold ~init:(VS.empty, [])
         (object
            ...
            method! enter_var var (vars, stack) =
              if List.exists stack ~f:(fun x -> Bap_var.(x = var)) then
                (vars, stack)
              else (Set.add vars var, stack)
...
module Stmt = struct
  ...
  let find (finder : 'a #finder) s : 'a option = finder#find [ s ]
  let exists finder ss = Option.is_some @@ finder#find [ ss ]
