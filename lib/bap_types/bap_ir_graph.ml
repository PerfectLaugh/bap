...
module Pred = struct
  ...
  (** [remove src dst preds] remove a predecessor [src] from a set of
      predecessors of [dst] *)
  ...
  let insert dst preds = Map.set preds ~key:dst ~data:Tid.Set.empty
end
...
type t = { preds : Pred.t; sub : sub term } [@@deriving bin_io, compare, sexp]
type graph = t [@@deriving bin_io, compare, sexp]
type node = blk term [@@deriving bin_io, compare, sexp]

type edge = { src : blk term; dst : blk term; pos : int }
[@@deriving bin_io, sexp]

let compare_edge x y =
  match Blk.compare x.src y.src with 0 -> Blk.compare x.dst y.dst | n -> n
...
let succs_of_blk blk = Term.enum jmp_t blk |> Seq.filter_map ~f:succ_tid_of_jmp
...
let jmp_set b =
  Term.enum jmp_t b |> Seq.map ~f:Term.tid
  |> Seq.fold ~init:Tid.Set.empty ~f:Set.add

(** [control_flow_difference bx by] computes a changes in control flow produced
    by blocks [bx] and [by]. Only changes, that affects graph toplogy are
    computed, i.e., change in a condition expression, or change of the order
    doesn't affect the topology. If jump kind is changed form local (goto) to
    some non-local, then it will be removed from the set of edges. *)
let control_flow_difference bx by =
  ...
  let to_changes kind b js =
    ...
    Set.to_list js |> List.filter_map ~f:succ |> List.map ~f:kind
  in
  ...
  Set.fold coms ~init:(news @ dels) ~f:(fun changes id ->
      ...
      | Some tx, Some ty ->
          if Tid.(tx = ty) then changes else target_change tx ty :: changes)
...
module Node = struct
  ...
  let inputs dst t =
    match Map.find t.preds (Term.tid dst) with
    | None -> Seq.empty
    | Some ps ->
  ...
  let update b blk' t =
    ...
    | Some blk -> (
        ...
        | [] -> { t with sub }
        | ds ->
            List.fold ~init:{ t with sub } ds ~f:(fun t -> function
              | New_jmp dst -> { t with preds = Pred.update bid dst t.preds }
              | Del_jmp dst -> { t with preds = Pred.remove bid dst t.preds }
              | Target_change (tx, ty) ->
                  t.preds |> Pred.remove bid tx |> Pred.update bid ty
                  |> fun preds -> { t with preds }))
  ...
  let insert blk t : graph = if mem blk t then t else do_insert blk t
...
module Edge = struct
  ...
  let insert_jmp src pos dst =
    ...
    let src = Fn.apply_n_times ~n (fun b -> Term.append jmp_t b dummy) src in
  ...
  let create src dst pos : edge =
    ...
    | Some jmp -> (
        ...
        | _ ->
            let src =
              Term.change jmp_t src (Term.tid jmp) (fun _ ->
                  Some (Jmp.create_goto (Label.direct tid)))
            in
            { src; dst; pos })
  ...
  let mem e g =
    match Map.find g.preds (Term.tid e.dst) with
    | None -> false
    | Some ps -> Set.mem ps (Term.tid e.src)

  let jmps_before e src = Seq.take (Term.enum jmp_t src) e.pos
  let jmps_after e src = Seq.drop (Term.enum jmp_t src) (e.pos + 1)
  ...
  let do_update e l t =
    ...
    Node.(
      insert e.dst t |> insert e.src |> update e.src e.src |> update e.dst e.dst)

  let update e l t = if mem e t then do_update e l t else t

  let cut_tail pos blk =
    let b = Blk.Builder.init ~copy_phis:true ~copy_defs:true blk in
    Term.enum jmp_t blk
    |> Seq.iteri ~f:(fun i jmp -> if i < pos then Blk.Builder.add_jmp b jmp);
  ...
  let remove e t =
    ...
    | Some src ->
        ...
        let src =
          match Term.nth jmp_t src e.pos with
          | None -> src
          | Some jmp -> make_dummy jmp src
        in
  ...
  include Regular.Make (struct
    ...
    let pp ppf x = Option.iter (Term.nth jmp_t x.src x.pos) ~f:(Jmp.pp ppf)
  end)
end

let empty = { sub = Sub.create (); preds = Tid.Map.empty }
let create ?tid ?name () = { empty with sub = Sub.create ?tid ?name () }
let nodes t = Term.enum blk_t t.sub
let edges t = nodes t |> Seq.concat_map ~f:(fun src -> Node.outputs src t)

let number_of_edges t =
  Seq.(Map.to_sequence t.preds >>| snd |> sum (module Int) ~f:Set.length)
...
let pp_full ppf g =
  let node_label blk =
    let open Bap_ir in
    let phis = Term.enum phi_t blk |> Seq.map ~f:Ir_phi.to_string in
    let defs = Term.enum def_t blk |> Seq.map ~f:Ir_def.to_string in
    let jmps =
      Term.enum jmp_t blk
  ...
  let edge_label e =
    match Edge.cond e g with
    | Exp.Int w when Bitvector.is_one w -> ""
    | exp -> Bap_exp.to_string exp
  in
  let nodes_of_edge e = Edge.(src e, dst e) in
  Graphlib.Dot.pp_graph ~name:(Ir_sub.name g.sub) ~attrs:[ "node[shape=box]" ]
    ~string_of_node ~node_label ~edge_label ~nodes_of_edge ~nodes:(nodes g)
    ~edges:(edges g) ppf
