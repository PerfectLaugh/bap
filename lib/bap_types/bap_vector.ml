...
type 'a t = { mutable size : int; mutable data : 'a array; default : 'a }
[@@deriving bin_io, compare, sexp]

let create ?(capacity = 16) default =
  let capacity = max capacity 1 in
  { size = 0; data = Array.create ~len:capacity default; default }

let resize vec =
  let n = Array.length vec.data in
  let data =
    Array.init (vec.size * 2) ~f:(fun i ->
        if i < n then Array.unsafe_get vec.data i else vec.default)
  in
  vec.data <- data

let append vec x =
  if Array.length vec.data = vec.size then resize vec;
  Array.unsafe_set vec.data vec.size x;
  vec.size <- vec.size + 1

let to_array vec = Array.sub vec.data ~pos:0 ~len:vec.size

let map_to_array vec ~f =
  Array.init vec.size ~f:(fun i -> f (Array.unsafe_get vec.data i))
...
let nth vec n =
  if n < vec.size then Some (Array.unsafe_get vec.data n) else None
...
include struct
  ...
  let fold_until vec ~init ~f ~finish =
    let rec loop i init =
      if i < vec.size then
        match f init (Array.unsafe_get vec.data i) with
        | Stop x -> x
...
let foldi vec ~init ~f =
  let rec loop i init =
    if i < vec.size then loop (i + 1) (f i init (Array.unsafe_get vec.data i))
    else init
  in
  loop 0 init
...
let find_mapi vec ~f =
  with_return (fun { return } ->
      iteri vec ~f:(fun i x ->
          match f i x with None -> () | hay -> return hay);
      None)
...
let to_list vec = List.init vec.size ~f:(fun i -> Array.unsafe_get vec.data i)
...
let pp pp_elem ppf vec =
  let n = length vec in
  fprintf ppf "{@[<2>";
  iteri vec ~f:(fun i x -> if i < n - 1 then fprintf ppf "%a;@ " pp_elem x);
