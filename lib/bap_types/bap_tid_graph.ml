...
let connect_with_exit n =
  if Tid.equal n exit then Fn.id else G.Edge.insert (G.Edge.create n exit exit)

let connect_with_start n =
  if Tid.equal n start then Fn.id
  else G.Edge.insert @@ G.Edge.create start n start
...
let create sub =
  ...
  Graphlib.depth_first_search
    (module G)
    g ~init:g ~start ~start_tree:connect_with_start
  |> fun g ->
  Graphlib.depth_first_search
    (module G)
    g ~rev:true ~init:g ~start:exit ~start_tree:connect_with_exit
