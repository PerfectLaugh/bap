...
module PP = struct
  ...
  let pp_unop fmt op =
    fprintf fmt "%s" Unop.(match op with NEG -> "-" | NOT -> "~")

  let pp_edn fmt e =
    fprintf fmt "%s"
      Bap_bil.(match e with LittleEndian -> "el" | BigEndian -> "be")
  ...
  let rec pp fmt exp =
    ...
    let pfmt p c =
      if prec c >= prec p then format_of_string "%a"
      else format_of_string "(@[<2>%a@])"
    in
    let pr s = fprintf fmt s in
    let is_b0 x = Bitvector.(x = b0) in
    let is_b1 x = Bitvector.(x = b1) in
    match exp with
    | Load (mem, idx, _, `r8) -> pr "%a[%a]" pp mem pp idx
    | Load (mem, idx, edn, s) ->
        pr "%a[%a, %a]:%a" pp mem pp idx pp_edn edn Bap_size.pp s
    | Store (mem, idx, exp, _, `r8) ->
        pr "@[<4>%a@;with [%a] <- %a@]" pp mem pp idx pp exp
    | Store (mem, idx, exp, edn, s) ->
        pr "@[<4>%a@;with [%a, %a]:%a <- %a@]" pp mem pp idx pp_edn edn
          Bap_size.pp s pp exp
    | Ite (ce, te, fe) -> pr "@[if %a@;then %a@;else %a@]" pp ce pp te pp fe
    | Extract (hi, lo, exp) -> pr "%d:%d[%a]" hi lo pp exp
    | Concat (le, re) as p -> pr (pfmt p le ^^ "." ^^ pfmt p re) pp le pp re
    ...
    | UnOp (op, exp) as p -> pr ("%a" ^^ pfmt p exp) pp_unop op pp exp
    | Var var -> Bap_var.pp fmt var
    | Int bv -> pr "%a" Bap_bitvector.pp_hex bv
    | Cast (ct, n, exp) -> pr "%a:%d[%a]" pp_cast ct n pp exp
    | Let (var, def, body) ->
        pr "@[let %a =@ %a in@ %a@]" Bap_var.pp var pp def pp body
    | Unknown (s, typ) -> pr "unknown[%s]:%a" s Bap_type.pp typ
end
...
module Apply = struct
  ...
  let is_shift = function LSHIFT | RSHIFT | ARSHIFT -> true | _ -> false
  let unop op u = match op with NEG -> Word.neg u | NOT -> Word.lnot u
  ...
  let binop op u v =
    ...
    | _ -> (
        ...
        let u = extract_exn ~hi u and v = extract_exn ~hi v in
        match op with
        | PLUS -> u + v
        | MINUS -> u - v
        | TIMES -> u * v
...
module Type = struct
  ...
  and unify x y =
    ...
    if type_equal t1 t2 then t1 else Type_error.expect t1 ~got:t2

  and let_ v x y =
    let t = Var.typ v and u = infer x in
    if type_equal t u then infer y else Type_error.expect t ~got:u
  ...
  and unop x =
    match infer x with Type.Mem _ -> Type_error.expect_imm () | t -> t
  ...
  and shift x y =
    ...
    | Type.Mem _, _ | _, Type.Mem _ | Type.Unk, _ | _, Type.Unk ->
        Type_error.expect_imm ()
    | t, Type.Imm _ -> t

  and load m a r =
    match (infer m, infer a) with
  ...
  and store m a x _ =
    ...
    | (Type.Mem (s, _) as t), Type.Imm s', Type.Imm u ->
        let s = Size.in_bits s in
        if s <> s' then Type_error.expect (Type.Imm s) ~got:(Type.Imm s')
        else if is_error (Size.of_int u) then Type_error.wrong_cast ()
        else t
  ...
  and extract hi lo x =
    ...
    | Type.Imm _ ->
        ...
        if hi >= lo then Type.Imm (hi - lo + 1) else Type_error.wrong_cast ()
  ...
  let infer_exn = infer
  let infer x = try Ok (infer x) with Type_error.T err -> Error err
end
...
module Exp = struct
  ...
  (* we can't fail in smart constructors and should allow
     ...
     error reporting. *)
  let zero_or : exp -> exp =
   fun x -> match Type.infer x with Ok (Imm m) -> Int (Word.zero m) | _ -> x

  let binop : binop -> exp -> exp -> exp =
   fun op x y ->
    ...
    | PLUS, BinOp (PLUS, x, Int y), Int z | PLUS, BinOp (PLUS, Int y, x), Int z
      ->
  ...
  let cast t s x =
    ...
    | Cast (t', s', x) when equal_cast t t' && s' >= s -> Cast (t, s, x)
    | (Cast ((UNSIGNED as t'), s', x) | Cast ((SIGNED as t'), s', x))
      when equal_cast t t' && s' <= s ->
        Cast (t, s, x)
    | Extract (p, _, x) when equal_cast t HIGH -> Extract (p, p - s + 1, x)
  ...
  let ite ~if_:c ~then_:x ~else_:y =
    match c with Int c -> if Word.(c = b1) then x else y | _ -> Ite (c, x, y)

  let has_width n x = match Type.infer x with Ok (Imm m) -> m = n | _ -> false

  let extract ~hi ~lo x =
    match x with
    | Int w -> Int (Word.extract_exn ~hi ~lo w)
    | Extract (p, q, x) when hi <= p && lo >= q -> Extract (hi, lo, x)
...
let domain =
  Knowledge.Domain.flat "exp" ~equal
...
let slot =
  Knowledge.Class.property ~package:"bap" ~persistent Theory.Value.cls "exp"
    domain ~public:true ~desc:"semantics of expressions in BIL"
