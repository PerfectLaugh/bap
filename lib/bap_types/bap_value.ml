...
let register_slot (type a) ?uuid slot (module S : S with type t = a) : a tag =
  let slot_name = KB.Slot.name slot in
  let uuid =
    match uuid with None -> KB.Name.package slot_name | Some uuid -> uuid
  in
  let name =
    KB.Name.show
    @@ KB.Name.create ~package:uuid
    @@ KB.Name.unqualified slot_name
  in
  ...
  let pp ppf (Value.T (k, x)) =
    ...
    S.pp ppf x
  in
  let of_string str = Value.T (key, Binable.of_string (module S) str) in
  let to_string (Value.T (k, x)) =
    let T = Equal.proof k key in
    Binable.to_string (module S) x
  in
  let of_sexp str = Value.T (key, S.t_of_sexp str) in
  ...
  let collect obj dict =
    ...
    | None -> dict
    | Some x -> Univ_map.set dict ~key ~data:x
  in
  let info = { pp; of_sexp; to_sexp; of_string; to_string; collect; compare } in
...
let register (type a) ?public ?desc ?package ~name ~uuid
  ...
  let slot =
    KB.Class.property ?public ?desc ~persistent ?package Theory.Program.cls name
      domain
  in
  register_slot ~uuid slot (module S)

let key_name k = KB.Name.unqualified @@ KB.Name.read @@ Type_equal.Id.name k
let key_typeid k = Type_equal.Id.name k
let tagname (Value.T (k, _)) = key_name k
let typeid (Value.T (k, _)) = key_typeid k

let info typeid =
  Hashtbl.find_and_call types typeid ~if_found:Fn.id
    ~if_not_found:(fun typeid ->
      invalid_argf
        "Can't deserialize type %s, as it is no longer known to the system"
        typeid ())

let ops x = info (typeid x)
let compare_value x y = (ops x).compare x y
let compare = compare_value
let sexp_of_value x = Sexp.List [ Sexp.Atom (typeid x); (ops x).to_sexp x ]

let value_of_sexp = function
  | Sexp.List [ Atom typeid; repr ] -> (info typeid).of_sexp repr
...
module Univ = struct
  ...
  module Repr = struct
    type t = { typeid : string; data : string } [@@deriving bin_io]
  end
  ...
  include
    Binable.Of_binable_with_uuid
      ...
      (struct
        ...
        let to_binable x =
          Repr.{ typeid = typeid x; data = (ops x).to_string x }

        let of_binable { Repr.typeid; data } = (info typeid).of_string data
...
let get : type a. a tag -> Value.t -> a option =
 fun { key } (Value.T (k, x)) ->
...
let get_exn : type a. a tag -> Value.t -> a =
 fun { key } (Value.T (k, x)) ->
  let T = Equal.proof key k in
  x
...
module Tag = struct
  ...
  let same_witness t1 t2 =
    Option.try_with (fun () -> Type_equal.Id.same_witness_exn t1.key t2.key)

  let same_witness_exn t1 t2 = Type_equal.Id.same_witness_exn t1.key t2.key
  let same t1 t2 = Type_equal.Id.same t1.key t2.key
end

module Match = struct
  type 's t = { default : unit -> 's; handlers : (Value.t -> 's) Map.M(Uid).t }

  let empty = Map.empty (module Uid)
  let default default = { handlers = empty; default }

  let case t f (tab : 's t) =
    let h =
      Map.set tab.handlers ~key:(Tag.uid t) ~data:(fun v -> f (get_exn t v))
    in
    { tab with handlers = h }
...
module Dict = struct
  ...
  let data dict = Univ_map.to_alist dict |> Seq.of_list
  let to_sequence dict = Seq.map (data dict) ~f:(fun v -> (typeid v, v))
...
include struct
  type value = Univ.t [@@deriving bin_io]
end
