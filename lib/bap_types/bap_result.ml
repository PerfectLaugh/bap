...
type value = Imm of word | Mem of storage | Bot
...
module Id : sig
  ...
  include Regular.Make (struct
    ...
    let pp ppf id = Format.fprintf ppf "0x%LX" (Int63.to_int64 id)
    let hash = Int63.hash
...
include Printable.Make (struct
  ...
  let pp ppf r = Format.fprintf ppf "[%a] %a" Id.pp (id r) Value.pp (value r)
