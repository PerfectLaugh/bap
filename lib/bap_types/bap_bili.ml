...
module Make (SM : Monad.State.S2) = struct
  ...
  class ['a] t =
    object (self)
      ...
      method eval (ss : stmt list) : 'a u = List.iter ss ~f:self#eval_stmt

      method eval_stmt (s : stmt) : 'a u =
        match s with
      ...
      method eval_move v u : 'a u = self#eval_exp u >>= fun u -> self#update v u
      ...
      method eval_while ~cond ~body : 'a u =
        ...
        | Mem v -> self#type_error' TE.bad_imm
        | Imm r ->
            if Bitvector.(r = b0) then return ()
            else if Bitvector.(r = b1) then
              self#eval body >>= fun () -> self#eval_while ~cond ~body
      ...
      method eval_if ~cond ~yes ~no : 'a u =
        ...
        | Mem v -> self#type_error' TE.bad_imm
        | Imm r ->
            if Bitvector.(r = b0) then self#eval no
            else if Bitvector.(r = b1) then self#eval yes
            else self#type_error' @@ TE.bad_type ~exp:bool_t ~got:(imm_t r)
    end
end
