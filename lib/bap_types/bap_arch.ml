...
module T = struct
  ...
  let normalize s =
    ...
    | "x86" | "x86-32" | "x86_32" | "ia32" | "ia-32" | "i386" | "i486" | "i586"
    | "i686" ->
        "x86"
  ...
  let from_string s = match of_string s with None -> `unknown | Some s -> s
  ...
  let slot =
    KB.Class.property ~package:"bap" Theory.Program.cls "arch" domain
      ~persistent

  let unit_slot =
    KB.Class.property ~package:"bap" Theory.Unit.cls "unit-arch" domain
      ~persistent
end
