...
let fail exp ctx =
  error "ill-formed expression in %a ctxt: %a" Sexp.pp (sexp_of_context ctx)
    Bap_exp.pp exp
...
let bits_of_var v =
  match Var.typ v with Imm x -> x | _ -> failwith "not a bitv var"

let byte x = Exp.int (Word.of_int ~width:8 x)

let is_big e =
  Exp.int
  @@ if [%compare.equal: Word.endian] e BigEndian then Word.b1 else Word.b0

let is_reg v =
  match Var.typ v with Type.Imm 1 | Type.Mem _ -> false | _ -> true

let is_bit v = match Var.typ v with Type.Imm 1 -> true | _ -> false
...
let bitv : type t r. (t, exp, r) bitv_parser =
 fun (module S) -> function
  ...
  | UnOp (NEG, x) -> S.neg x
  | UnOp (NOT, x) -> S.not x
  | Load (m, k, e, s) -> S.load_word (Size.in_bits s) (is_big e) m k
  ...
  | (Let _ | Store (_, _, _, _, _) | Unknown (_, (Mem _ | Unk))) as exp ->
      fail exp `Bitv;
      S.error

let mem : type t. (t, exp) mem_parser =
 fun (module S) -> function
  | Unknown (_, Mem (k, v)) -> S.unknown (Size.in_bits k) (Size.in_bits v)
  | Store (m, k, v, e, _) -> S.store_word (is_big e) m k v
  | Var v ->
      let with_mem_types v f =
        match Var.typ v with
        | Mem (ks, vs) -> f (Size.in_bits ks) (Size.in_bits vs)
        | _ ->
            fail (Bil.Exp.Var v) `Mem;
            S.error
      in
...
let stmt : type t r. (t, exp, r, stmt) stmt_parser =
 fun (module S) ->
  let set v x =
    ...
    | Mem (ks, vs) -> S.set_mem n (Size.in_bits ks) (Size.in_bits vs) x
  in
  fun s ->
    match Stmt.(decode call s) with
    | Some dst -> S.call dst
    | None -> (
        match Stmt.(decode intrinsic s) with
