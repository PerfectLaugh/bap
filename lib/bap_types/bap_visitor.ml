...
class ['a] exp_visitor =
  object (self)
    ...
    method visit_unop op e x =
      self#enter_unop op e x |> self#visit_exp e |> self#leave_unop op e
    ...
    method visit_binop op e1 e2 x =
      ...
      |> self#visit_exp e1 |> self#visit_exp e2 |> self#leave_binop op e1 e2
    ...
    method visit_cast ct cs e x =
      self#enter_cast ct cs e x |> self#visit_exp e |> self#leave_cast ct cs e
    ...
    method visit_let v ~exp ~body x =
      ...
      |> self#visit_var v |> self#visit_exp exp |> self#visit_exp body
    ...
    method visit_ite ~cond ~yes ~no x =
      let x = self#enter_ite ~cond ~yes ~no x |> self#visit_exp cond in
      let self = {<under_condition = true>} in
      self#visit_exp yes x |> self#visit_exp no |> self#leave_ite ~cond ~yes ~no
    ...
    method visit_concat e1 e2 x =
      self#enter_concat e1 e2 x |> self#visit_exp e1 |> self#visit_exp e2
    ...
    method visit_int n x = self#enter_int n x |> self#leave_int n
    ...
    method visit_exp e x : 'a =
      ...
      let x =
        ...
        | Exp.Store (mem, addr, exp, e, s) ->
            self#visit_store ~mem ~addr ~exp e s x
        ...
        | Exp.Unknown (s, t) -> self#visit_unknown s t x
      in
      self#leave_exp e x
  end
...
class ['a] bil_visitor =
  object (self : 's)
    ...
    method run stmts x : 'a =
      match stmts with
      | [] -> x
      | s :: ss ->
          {<succs = ss>}#visit_stmt s x |> {<preds = s :: preds>}#run ss

    method enter_special s x = x
    method leave_special s x = x
    method visit_special s x = self#enter_special s x |> self#leave_special s
    method enter_jmp e x = x
    method leave_jmp e x = x

    method visit_jmp e x =
      self#enter_jmp e x |> {<in_jmp = true>}#visit_exp e |> self#leave_jmp e
    ...
    method visit_move v e x =
      ...
      self#visit_var v x |> self#visit_exp e |> self#leave_move v e
    ...
    method visit_while ~cond ss x =
      ...
      let self = {<under_condition = true; in_loop = true>} in
      self#run ss x |> self#leave_while ~cond ss

    method enter_if ~cond ~yes ~no x = x
    method leave_if ~cond ~yes ~no x = x

    method visit_if ~cond ~yes ~no x =
      let x = self#enter_if ~cond ~yes ~no x |> self#visit_exp cond in
      let self = {<under_condition = true>} in
      self#run yes x |> self#run no |> self#leave_if ~cond ~yes ~no

    method enter_cpuexn n x : 'a = x
    method leave_cpuexn n x = x
    method visit_cpuexn n x = self#enter_cpuexn n x |> self#leave_cpuexn n
    ...
    method visit_stmt stmt x =
      ...
      let x =
        ...
        | Stmt.CpuExn n -> self#visit_cpuexn n x
      in
...
class exp_mapper =
  object (self)
    inherit exp_state
    method map_unop op e = Exp.UnOp (op, self#map_exp e)
    method map_binop op e1 e2 = Exp.BinOp (op, self#map_exp e1, self#map_exp e2)

    method map_store ~mem ~addr ~exp e s =
      Exp.Store (self#map_exp mem, self#map_exp addr, self#map_exp exp, e, s)
    ...
    method map_extract ~hi ~lo e = Exp.Extract (hi, lo, self#map_exp e)
    method map_concat e1 e2 = Exp.Concat (self#map_exp e1, self#map_exp e2)
...
class bil_mapper =
  object (self : 's)
    ...
    method map_jmp e = [ Stmt.Jmp ({<in_jmp = true>}#map_exp e) ]
    ...
    method map_while ~cond ss =
      [
        Stmt.While
          (self#map_exp cond, {<in_loop = true; under_condition = true>}#run ss);
      ]
