...
module Make (State : Monad.State.S2) = struct
  ...
  let create_result f =
    ...
    State.put ctxt >>= fun () -> State.return v

  let undefined () = create_result (fun ctxt -> ctxt#create_undefined)
  let word w = create_result (fun ctxt -> ctxt#create_word w)
  let storage s = create_result (fun ctxt -> ctxt#create_storage s)
  ...
  class ['a] t =
    object (self)
      ...
      method private word_of_value v =
        match value v with
        | Imm x -> State.return (Some x)
        | _ -> State.return None

      method private storage_of_value v =
        match value v with
      ...
      method lookup v =
        State.get () >>= fun ctxt ->
        match ctxt#lookup v with
        | Some v -> State.return v
        | None -> self#undefined_var v

      method update var data = State.update @@ fun s -> s#update var data

      method load mem addr =
        match mem#load addr with
        | None -> self#undefined_addr addr
        | Some w -> word w

      method store mem addr word = storage (mem#save addr word)
    end
end
