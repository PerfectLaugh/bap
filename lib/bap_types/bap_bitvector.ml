...
type endian = LittleEndian | BigEndian [@@deriving bin_io, compare, sexp]
...
module Packed : sig
  ...
  val lift3 :
    t -> t -> t -> (Bitvec.t -> Bitvec.t -> Bitvec.t -> Bitvec.t Bitvec.m) -> t
end = struct
  ...
  let unsigned ({ packed } as x) =
    if is_signed x then { packed = Z.((packed asr 1) lsl 1) } else x
  ...
  module Stringable = struct
    ...
    let caller_identity =
      Bin_shape.Uuid.of_string "18adbffd-ea55-4ff0-a377-0a7dbe15d1b1"
  end
...
let pp_generic ?(case : [ `lower | `upper ] = `upper)
  ...
  let () =
    ...
    | `base -> pp_prefix ppf
    | `auto -> if Z.(geq x (min (word 10) base)) then pp_prefix ppf
  in
  let fmt =
    format_of_string
    @@
    match (format, case) with
    | `hex, `upper -> "%X"
    | `hex, `lower -> "%x"
    | _ -> "%d"
  in
  let rec print x =
    ...
    if Z.geq x base then print Z.(x / base);
    fprintf ppf fmt d
  in
  print x;
  match suffix with
...
let of_suffixed stem suffix =
  ...
  if sl = 0 then invalid_arg "Bitvector.of_string: an empty suffix";
...
let string_of_value ?(hex = true) x =
  if hex then asprintf "%a" (fun p -> pp_generic ~prefix:`none ~case:`lower p) x
...
let compare_unsigned x y = Bitvec.compare (payload x) (payload y) [@@inline]
...
let extract ?hi ?(lo = 0) t =
  ...
  let len = hi - lo + 1 in
  if len <= 0 then failwithf "Bitvector.extract: len %d is negative" len ();
  if is_signed t && msb t then
    pack Z.((minus_one lsl n) lor Z.extract z lo len) len
...
let convert cast_z cast_bitvec x =
  if Packed.is_signed x then
    cast_z
    @@ Z.signed_extract
         (Bitvec.to_bigint (payload x))
         0 (* pos *)
         (Packed.bitwidth x) (* len *)
  else cast_bitvec (payload x)
...
let of_binary ?width endian num =
  let num =
    match endian with LittleEndian -> num | BigEndian -> String.rev num
  in
...
let gcd a b = Or_error.try_with (fun () -> gcd_exn a b)
let lcm a b = Or_error.try_with (fun () -> lcm_exn a b)
let gcdext a b = Or_error.try_with (fun () -> gcdext_exn a b)
...
module Unsafe = struct
  module Base = struct
    ...
    let div x y =
      match (is_signed x, is_signed y) with
...
module Safe = struct
  ...
  let ( !$ ) v = Ok v
  let badwidth m x = Or_error.errorf "Word - wrong width, expects %d got %d" m x

  let lift m x =
    let w = bitwidth x in
    if m = w then Ok x else badwidth m w

  let lift1 op x = match x with Ok x -> Ok (op x) | Error _ as e -> e
  ...
  let i32 = lift 32
  let i64 = lift 64
  let of_word_size = function Word_size.W64 -> i64 | Word_size.W32 -> i32
...
let extract ?hi ?lo x = Or_error.try_with (fun () -> extract_exn ?hi ?lo x)
...
let validate check msg x =
  if check x then Validate.pass else Validate.fails msg x sexp_of_t

let validate_positive = validate is_positive "expects a positive number"

let validate_non_positive =
  validate is_non_positive "expects a non positive number"

let validate_negative = validate is_negative "expects a negative number"
...
let enum_chars t endian =
  ...
  if is_zero t then take (repeat '\x00') n
    ...
    let start, next, finish, cat, cut =
      match endian with
      | LittleEndian ->
          (0, Int.succ, len, Fn.flip append, Fn.flip take (len - len_diff))
      | BigEndian -> (len - 1, Int.pred, ~-1, append, Fn.flip drop len_diff)
    in
    let s =
      unfold ~init:start ~f:(fun i ->
          if i = finish then None else Some (bytes.[i], next i))
    in
...
let bits_of_byte byte =
  ...
  Sequence.unfold ~init:7 ~f:(fun n ->
      if n < 0 then None else Some (byte land (1 lsl n) <> 0, n - 1))
...
module Trie = struct
  ...
  module Little = struct
    module Bits = Bap_trie.Make (struct
      ...
      let length = bitwidth
      let nth_token v n = extract_exn ~hi:n ~lo:n v |> to_int |> ok_exn
    end)
...
include Regular.Make (struct
  ...
  let compare x y =
    if phys_equal x y then 0
      ...
      match Int.compare (bitwidth x) (bitwidth y) with
...
module V1 = struct
  module Bignum = struct
    module Repr = struct
      ...
      let caller_identity =
        Bin_shape.Uuid.of_string "32e96b23-cfee-470e-a99a-1be199b301d3"
    end
  ...
  type t = { z : Bignum.t; w : int; signed : bool } [@@deriving bin_io, sexp]
end
    ...
    include
      Binable.Of_binable_with_uuid
        ...
        (struct
          ...
          let caller_identity =
            Bin_shape.Uuid.of_string "1d938f5b-1fe7-4db4-97ec-d4de3a8e29da"
        end)
...
let () =
  ...
  let add name desc pp =
    add_writer ~desc ~ver:"2.0.0" name (Data.Write.create ~pp ())
  in
