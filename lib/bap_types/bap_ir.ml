...
module Tid = struct
  ...
  let for_ivec ?package s = generate ?package Theory.Label.for_ivec s

  let for_addr ?package s =
    generate ?package Theory.Label.for_addr @@ Bap_bitvector.to_bitvec s

  let set slot tid name = Toplevel.exec (KB.provide slot tid (Some name))
  ...
  let add_name tid name =
    Toplevel.exec
      (KB.provide Theory.Label.aliases tid @@ Set.singleton (module String) name)
  ...
  let intern n =
    Toplevel.put name
      ( KB.Symbol.intern n Theory.Program.cls >>= fun t ->
        KB.provide Theory.Label.name t (Some n) >>| fun () -> t );
    Toplevel.get name
  ...
  let parse name =
    Toplevel.put last (KB.Object.read Theory.Program.cls name);
    Toplevel.get last

  let create () =
    Toplevel.put last (KB.Object.create Theory.Program.cls);
    Toplevel.get last

  let to_string : t -> string =
   fun tid -> Format.asprintf "%%%08Lx" (Int63.to_int64 (KB.Object.id tid))

  let of_string : string -> t =
   fun str ->
    if String.is_empty str then intern str
    else
      match str.[0] with
      | '%' ->
          parse
...
type 'a term = { tid : tid; self : 'a; dict : dict }
...
module Rhs : sig
  ...
  let of_exp exp = KB.Value.put Exp.slot empty exp
  let with_exp exp x = KB.Value.put Exp.slot x exp
...
module Def : sig
  type t = { var : unit Theory.var; rhs : Rhs.t }
  [@@deriving bin_io, compare, sexp]

  val reify : 'a Theory.var -> 'a Theory.value -> t
  val of_bil : var -> exp -> t
end = struct
  type t = { var : Theory.Var.Top.t; rhs : Rhs.t }
  [@@deriving bin_io, compare, sexp]

  let reify lhs rhs = { var = Theory.Var.forget lhs; rhs = Rhs.of_value rhs }
  let of_bil var exp = { var = to_var var; rhs = Rhs.of_exp exp }
end

module Phi = struct
  type t = { var : Theory.Var.Top.t; map : Rhs.t Tid.Map.t }
  [@@deriving bin_io, compare, sexp]
end

module Cnd : sig
  type t = Theory.Bool.t Theory.value [@@deriving bin_io, compare, sexp]
...
module Jmp = struct
  type dst = Resolved of tid | Indirect of { vec : Rhs.t; len : int }
  [@@deriving bin_io, compare, sexp]

  type t = { cnd : Cnd.t option; dst : dst option; alt : dst option }
  [@@deriving bin_io, compare, sexp]
end
...
type arg = Def.t [@@deriving bin_io, compare, sexp]

type sub = { name : string; blks : blk term array; args : arg term array }
[@@deriving bin_io, compare, fields, sexp]

type path = int array [@@deriving bin_io, compare, sexp]

module Program : sig
  type t = private { subs : sub term array; paths : path Tid.Table.t }
  [@@deriving bin_io, compare, fields, sexp]

  val empty : unit -> t
  val update : t -> sub term array -> t
end = struct
  type t = { subs : sub term array; paths : path Tid.Table.t }
...
module Array = struct
  ...
  (** [insert xs x i] insert [x] into [xs] in a position before [i]. [i] can be
      in a range of [0 <= i <= length xs] *)
  let insert xs x i =
    init
      (length xs + 1)
      ~f:(fun j -> if j < i then xs.(j) else if j = i then x else xs.(j - 1))

  let push_back xs x = insert xs x (length xs)
  let push_front xs x = insert xs x 0

  (** [remove_if xs ~f] returns array that doesn't contain elements for which
      [f x] is true. Returns an unmodified array if no such elements are in the
      array. *)
  let remove_if xs ~f:is_victim =
    if Array.exists xs ~f:is_victim then Array.filter xs ~f:(Fn.non is_victim)
    else xs

  let update_if xs y ~f:is_target =
    if Array.exists xs ~f:is_target then
      Array.map xs ~f:(fun x -> if is_target x then y else x)
    else xs
  ...
  let pp ppx ppf xs =
    ...
    Array.iteri xs ~f:(fun i x ->
        ...
        if i <> last then Format.pp_print_cut ppf ())
end
...
let pp_attr ppf attr =
  Format.fprintf ppf "@[.%s %a@]" (Value.tagname attr) Value.pp attr

let pp_attrs ppf dict = Dict.data dict |> Seq.iter ~f:(pp_attr ppf)
...
let cls typ par nil field =
  { par; typ; nil; set = Field.fset field; get = Field.get field }
...
let undefined_variable = Theory.Var.(forget @@ define Void.t "undefined")
let undefined_semantics = Rhs.of_value @@ Theory.Value.empty Void.t
let empty self = { tid = Tid.nil; dict = Value.Dict.empty; self }

let nil_def : def term =
  empty Def.{ var = undefined_variable; rhs = undefined_semantics }

let nil_phi : phi term =
  empty Phi.{ var = undefined_variable; map = Map.empty (module Tid) }

let nil_jmp : jmp term = empty Jmp.{ cnd = None; dst = None; alt = None }
...
let term_pp ?(no_cut = false) ?(box = horizontal) pp_self ppf t =
  ...
  let pp_cut ppf =
    if not no_cut then Format.pp_print_break ppf 1 0 else fprintf ppf " "
  in
  box ppf;
  fprintf ppf "%08Lx:%t%a"
    (Int63.to_int64 (KB.Object.id t.tid))
    pp_cut pp_self t.self;
...
module Call = struct
  ...
  include Regular.Make (struct
    ...
    let pp_return ppf lab =
      match lab with
      | Some label -> Format.fprintf ppf "with return %a" Label.pp label
      | None -> Format.fprintf ppf "with noreturn"

    let pp ppf c =
      Format.fprintf ppf "call %a %a" Label.pp c.target pp_return c.return
...
module Ir_arg = struct
  ...
  module Intent = struct
    ...
    let slot =
      KB.Class.property Theory.Value.cls "arg-intent" domain ~package
        ~persistent

    let set intent x =
      match intent with None -> x | Some intent -> KB.Value.put slot x intent

    let get x = KB.Value.get slot x
  end

  let set_intent ({ self = { Def.rhs } as self } as t) intent : t =
    { t with self = { self with rhs = Intent.set (Some intent) rhs } }
  ...
  let warn_unused =
    Bap_value.Tag.register
      (module Unit)
      ~package ~name:"warn-unused" ~uuid:"7aa17c89-cc9b-4ed2-8700-620cb9e09491"

  let format =
    Bap_value.Tag.register
      (module String)
      ~package ~name:"format" ~uuid:"d864c411-73eb-48b2-a7e9-33b51fa540c9"

  let alloc_size =
    Bap_value.Tag.register
      (module Unit)
      ~package ~name:"alloc-size" ~uuid:"b29905b3-4fb5-486e-8064-9b63cadc6174"

  let restricted =
    Bap_value.Tag.register
      (module Unit)
      ~package ~name:"restricted" ~uuid:"5ee30262-aed9-4aa8-a8e3-34a061104420"

  let nonnull =
    Bap_value.Tag.register
      (module Unit)
      ~package ~name:"nonnull" ~uuid:"3c0a6181-9a9c-4cf4-aa37-8ceebd773952"

  let pp_sort ppf var =
    match Var.typ (Var.reify var) with
    | Unk -> Theory.Value.Sort.pp ppf (Theory.Var.sort var)
    | typ -> Bap_type.pp ppf typ

  let pp_self pp_rhs ppf { Def.var; rhs } =
    Format.fprintf ppf "%s :: %s%a = %a" (Theory.Var.name var)
      (string_of_intent @@ Intent.get rhs)
      pp_sort var pp_rhs rhs
...
module Ir_def = struct
  ...
  let reify ?(tid = Tid.create ()) lhs rhs = make_term tid @@ Def.reify lhs rhs
  ...
  let with_lhs ({ self = { Def.rhs } } as t) v =
    { t with self = Def.{ var = to_var v; rhs } }

  let with_rhs ({ self = { Def.var; rhs } } as t) exp =
    { t with self = Def.{ var; rhs = Rhs.with_exp exp rhs } }

  let map_exp def ~f : def term = with_rhs def (f (rhs def))
  ...
  let pp_self ppf { Def.var; rhs } =
    Format.fprintf ppf "%s@ :=@ %a" (Theory.Var.name var) Bap_exp.pp
      (Rhs.exp rhs)

  let pp_self_slots slots ppf { Def.var; rhs } =
    Format.fprintf ppf "%s@ :=@ %a" (Theory.Var.name var) (pp_value slots) rhs
...
module Ir_phi = struct
  ...
  let with_lhs ({ self } as t) lhs =
    { t with self = Phi.{ self with var = to_var lhs } }

  let reify ?(tid = Tid.create ()) var bs =
    let bs = List.map bs ~f:(fun (t, x) -> (t, Rhs.of_value x)) in
    make_term tid
      Phi.
        { var = Theory.Var.forget var; map = Map.of_alist_exn (module Tid) bs }

  let of_list ?(tid = Tid.create ()) var bs : phi term =
    let bs = List.map bs ~f:(fun (t, x) -> (t, Rhs.of_exp x)) in
    make_term tid
      Phi.{ var = to_var var; map = Map.of_alist_exn (module Tid) bs }

  let create ?tid var src exp : phi term = of_list ?tid var [ (src, exp) ]

  let values { self = { Phi.map } } : (tid * exp) Seq.t =
    Map.to_sequence map |> Seq.map ~f:(fun (t, x) -> (t, Rhs.exp x))
  ...
  let update ({ self = { Phi.map; var } } as t) tid exp : phi term =
    {
      t with
      self = Phi.{ var; map = Map.set map ~key:tid ~data:(Rhs.of_exp exp) };
    }

  let remove ({ self = { Phi.map; var } } as t) tid : phi term =
    { t with self = Phi.{ var; map = Map.remove map tid } }

  let select { self = { Phi.map } } tid : exp option =
    Option.map (Map.find map tid) ~f:Rhs.exp

  let select_or_unknown phi tid =
    match select phi tid with
    | Some thing -> thing
    | None ->
...
module Ir_jmp = struct
  ...
  let dst_of_lbl : label -> Jmp.dst option = function
    ...
    | Indirect exp -> (
        match Bap_helpers.Type.infer_exn exp with
  ...
  let create ?(tid = Tid.create ()) ?(cond = always) kind =
    ...
    @@
    match kind with
    | Goto lbl -> Jmp.{ cnd; dst = dst_of_lbl lbl; alt = None }
    | Ret lbl -> Jmp.{ cnd; dst = None; alt = dst_of_lbl lbl }
    | Int (int, ret) ->
        let alt = Tid.create () in
        Tid.set_ivec alt int;
        Jmp.{ cnd; dst = Some (Resolved ret); alt = Some (Resolved alt) }
    | Call t ->
        {
          cnd;
  ...
  let kind_of_jmp { Jmp.dst; alt } =
    ...
    | Some dst, Some alt -> (
        match (dst, ivec_of_dst alt) with
        | Resolved dst, Some vec -> Int (vec, dst)
        | _ ->
            Call
              (Call.create () ~return:(lbl_of_dst dst) ~target:(lbl_of_dst alt))
        )
  ...
  let with_guard jmp cnd = { jmp with self = Jmp.{ jmp.self with cnd } }
  let dst { self = { Jmp.dst } } = dst
  let alt { self = { Jmp.alt } } = alt
  let with_dst jmp dst = { jmp with self = Jmp.{ jmp.self with dst } }
  let with_alt jmp alt = { jmp with self = Jmp.{ jmp.self with alt } }
  ...
  let kind : jmp term -> jmp_kind = fun t -> kind_of_jmp t.self

  let cond_of_jmp { Jmp.cnd } =
    match cnd with None -> always | Some cnd -> KB.Value.get Exp.slot cnd

  let cond : jmp term -> exp = fun t -> cond_of_jmp t.self

  let with_cond t cnd =
    { t with self = Jmp.{ t.self with cnd = Some (Cnd.of_exp cnd) } }
  ...
  let exps (jmp : jmp term) : exp Sequence.t =
    ...
    let label label =
      match label with Indirect exp -> yield exp | Direct _ -> return ()
    in
    let call call =
      Option.value_map ~default:(return ()) (Call.return call) ~f:label
      >>= fun () -> label (Call.target call)
    in
    let r =
      match kind jmp with
      | Call t -> call t
  ...
  let eval jmp bili =
    ...
    in
    match kind jmp with Goto t -> eval_label t | _ -> assert false
  ...
  let pp_dst ppf = function
    ...
    | Int (n, t) -> Format.fprintf ppf "interrupt 0x%X return %%%a" n Tid.pp t

  let pp_cond ppf cond =
    if Exp.(cond <> always) then Format.fprintf ppf "when %a " Bap_exp.pp cond

  let pp_self ppf jmp =
    Format.fprintf ppf "%a%a" pp_cond (cond_of_jmp jmp) pp_dst (kind_of_jmp jmp)
...
module Term = struct
  ...
  let find t p tid = Array.find (t.get p.self) ~f:(fun x -> Tid.(x.tid = tid))
  ...
  let to_seq xs = Seq.init (Array.length xs) ~f:(Array.unsafe_get xs)
  ...
  let next t p tid =
    ...
    let ts = t.get p.self in
    findi t p tid >>= fun (i, _) ->
    if i + 1 >= Array.length ts then None else Some ts.(i + 1)

  let prev t p tid =
    let open Option.Monad_infix in
    let ts = t.get p.self in
    findi t p tid >>= fun (i, _) -> if i = 0 then None else Some ts.(i - 1)
  ...
  let after ?(rev = false) t p tid =
    ...
    Seq.(
      drop_eagerly
        (to_sequence ~rev t p |> run ~f:(fun x -> Tid.(x.tid <> tid)))
        cut)
  ...
  let before_or_after run ?rev t p tid =
    if Array.exists (t.get p.self) ~f:(fun x -> Tid.(x.tid = tid)) then
      run ?rev t p tid
    else Seq.empty
  ...
  let insert t where p x =
    ...
    let xs =
      ...
      | `after (Some this) | `before (Some this) -> (
          ...
          | Some (i, _), `after _ -> Array.insert xs x (i + 1))
    in
  ...
  let origin =
    Bap_value.Tag.register
      (module Tid)
      ~package ~public:true ~desc:"denotes an origin of a synthetic program"
      ~name:"origin" ~uuid:"fa804594-d2fc-4865-824a-3ad481963f54"

  let synthetic =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true ~desc:"denotes a synthetic program"
      ~name:"synthetic" ~uuid:"a83b26b5-902e-4aaf-bfa1-503e3ced0b1a"

  let live =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true ~desc:"marks program as live" ~name:"live"
      ~uuid:"4d3871ab-9481-4d41-97a3-cd3136acfa90"

  let dead =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true ~desc:"marks program as dead" ~name:"dead"
      ~uuid:"6009fb21-2a6c-4511-9aa4-92b2894debc7"

  let visited =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true ~desc:"marks program as dead" ~name:"visited"
      ~uuid:"0e162aa3-153f-44a3-88e7-6e42d876e760"

  let precondition =
    Bap_value.Tag.register
      (module Exp)
      ~package ~public:true ~desc:"the program precondition"
      ~name:"precondition" ~uuid:"f08c88e1-56f5-4148-822a-ac2dff34bda5"

  let invariant =
    Bap_value.Tag.register
      (module Exp)
      ~package ~public:true ~desc:"the program invariant" ~name:"invariant"
      ~uuid:"743d712b-7ee4-46da-b3b7-98d3ca5e618b"

  let postcondition =
    Bap_value.Tag.register
      (module Exp)
      ~package ~public:true ~desc:"the program postcondition"
      ~name:"postcondition" ~uuid:"f248e4c1-9efc-4c70-a864-e34706e2082b"

  let equal x y = compare_term compare_blk x y = 0
  let equal_tids x y = Tid.equal (tid x) (tid y)

  let domain =
    Knowledge.Domain.flat ~empty:[] "bir" ~equal:(List.equal equal_tids)
      ~inspect:(fun blks ->
        Sexp.List (List.map blks ~f:(fun b -> Sexp.Atom (name b))))
  ...
  let slot =
    Knowledge.Class.property Theory.Semantics.cls "bir" domain ~package
      ~persistent ~public:true ~desc:"BIL semantics in a graphical IR"
  ...
  let change t p tid f =
    ...
    | Some (i, x) -> (
        ...
        | Some c ->
            let xs =
              Array.mapi (t.get p.self) ~f:(fun n x -> if i = n then c else x)
            in
            { p with self = t.set p.self xs })
  ...
  let cata (type t) (cls : (_, t) cls) ~init:default
      ...
      ?(jmp : (jmp, 'a) cata = this default) (t : t term) : 'a =
    match cls.typ with
    | Nil -> assert false
    | Top -> program t
    | Sub -> sub t
  ...
  let switch (type t) (cls : (_, t) cls) ~(program : (program, 'a) cata)
      ~(sub : (sub, 'a) cata) ~(arg : (arg, 'a) cata) ~(blk : (blk, 'a) cata)
      ~(phi : (phi, 'a) cata) ~(def : (def, 'a) cata) ~(jmp : (jmp, 'a) cata)
      (t : t term) : 'a =
    match cls.typ with
    | Nil -> assert false
    | Top -> program t
  ...
  let map_term (type t) (cls : (_, t) cls) ?(program : program map = Fn.id)
      ?(sub : sub map = Fn.id) ?(arg : arg map = Fn.id) ?(blk : blk map = Fn.id)
      ?(phi : phi map = Fn.id) ?(def : def map = Fn.id) ?(jmp : jmp map = Fn.id)
      (t : t term) : t term =
    match cls.typ with
    | Nil -> assert false
    | Top -> program t
  ...
  class mapper =
    object (self)
      ...
      method map_term : 't 'p. ('p, 't) cls -> 't term -> 't term =
        fun cls t ->
          map_term cls t ~program:self#run ~sub:self#map_sub ~arg:self#map_arg
            ~blk:self#map_blk ~phi:self#map_phi ~def:self#map_def
            ~jmp:self#map_jmp

      method run p = map sub_t ~f:(fun t -> self#map_term sub_t t) p

      method map_sub sub =
        map arg_t ~f:(self#map_term arg_t) sub
        |> map blk_t ~f:(self#map_term blk_t)

      method map_blk blk =
        map phi_t ~f:(self#map_term phi_t) blk
  ...
  class ['a] visitor =
    object (self)
      ...
      method visit_phi phi x =
        self#enter_phi phi x |> self#visit_var (Ir_phi.lhs phi) |> fun x ->
      ...
      method visit_jmp jmp x =
        ...
        Seq.fold (Ir_jmp.exps jmp) ~init:x ~f:(fun x e -> self#visit_exp e x)
        |> self#leave_jmp jmp
    end
end
...
module Ir_blk = struct
  ...
  module Builder = struct
    ...
    let init ?(same_tid = true) ?(copy_phis = false) ?(copy_defs = false)
        ?(copy_jmps = false) blk =
      let tid = if same_tid then Term.tid blk else Tid.create () in
      let b =
        create ~tid () ~phis:(Term.length phi_t blk)
          ~defs:(Term.length def_t blk) ~jmps:(Term.length jmp_t blk)
      in
  ...
  let create ?(phis = []) ?(defs = []) ?(jmps = []) ?(tid = Tid.create ()) () :
      blk term =
    ...
    | _ ->
        let b =
          Builder.create ~tid () ~phis:(List.length phis)
            ~defs:(List.length defs) ~jmps:(List.length jmps)
        in
  ...
  let split_while blk ~f =
    let i =
      match Array.findi blk.self.defs ~f:(fun _ d -> not (f d)) with
      | Some (i, _) -> i
      | None -> Array.length blk.self.defs
    in
    split_at i blk
  ...
  let split_before blk def =
    split_while blk ~f:(fun { tid } -> Tid.(tid <> def.tid))

  let split_after blk def =
    let i =
      match
        Array.findi blk.self.defs ~f:(fun _ { tid } -> Tid.(tid = def.tid))
      with
      | Some (i, _) -> i + 1
      | None -> Array.length blk.self.defs
    in
  ...
  let apply_map name get map skip blk ~f =
    if List.mem ~equal:Poly.equal skip name then get blk.self
  ...
  let map_elts ?phi ?def ?jmp blk =
    let map get ~f =
      match f with
      | None -> get blk.self
      | Some f -> Array.map (get blk.self) ~f
    in
    {
      blk with
      self =
  ...
  let substitute ?skip blk x y = map_exp ?skip blk ~f:(Exp.substitute x y)
  ...
  let has_lhs cls lhs blk x =
    Term.to_sequence cls blk |> Seq.exists ~f:(fun t -> Var.(lhs t = x))

  let defines_var blk x =
    has_lhs phi_t Ir_phi.lhs blk x || has_lhs def_t Ir_def.lhs blk x
  ...
  let find_var blk var =
    ...
    |> Seq.find ~f:(fun d -> Var.equal (Ir_def.lhs d) var)
    |> function
    | Some def -> Some (`Def def)
    | None -> (
        Term.to_sequence phi_t blk
        |> Seq.find ~f:(fun p -> Var.equal (Ir_phi.lhs p) var)
        |> function
  ...
  let occurs b ~after:dominator id =
    Tid.(dominator = id)
    || Term.(
         after def_t b dominator |> Seq.exists ~f:(fun x -> Tid.(x.tid = id)))

  let pp_sep xs ys zs ppf =
    if
      (Array.length xs <> 0 && Array.length ys <> 0)
      || (Array.length xs <> 0 && Array.length ys = 0 && Array.length zs <> 0)
    then Format.pp_print_cut ppf ()

  let pp_self ppf self =
    Format.fprintf ppf "%a%t%a%t%a" (Array.pp Ir_phi.pp) self.phis
  ...
  let pp_self_slots ds ppf self =
    Format.fprintf ppf "%a%t%a%t%a"
      (Array.pp (Ir_phi.pp_slots ds))
      self.phis
      (pp_sep self.phis self.defs self.jmps)
      (Array.pp (Ir_def.pp_slots ds))
      self.defs
      (pp_sep self.defs self.jmps [||])
      (Array.pp (Ir_jmp.pp_slots ds))
      self.jmps

  let box ppf = Format.pp_open_vbox ppf 0
  let pp = term_pp ~box pp_self
  let pp_slots ds ppf x = term_pp ~box (pp_self_slots ds) ppf x
...
module Ir_sub = struct
  ...
  module Enum (T : Bap_value.S) = struct
    ...
    let pp ppf xs =
      ...
      |> String.concat ~sep:", " |> Format.fprintf ppf "%s"
  end
  ...
  let aliases =
    Bap_value.Tag.register
      (module Aliases)
      ~package ~public:true ~desc:"alternative names of the program"
      ~name:"aliases" ~uuid:"ed73c040-d798-4fc9-96a0-d3c12a870955"

  let const =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true ~desc:"the program is pure and closed term"
      ~name:"const" ~uuid:"b8795164-2e19-4469-9776-d41a6e6afe2e"

  let pure =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true ~desc:"the program is pure" ~name:"pure"
  ...
  let stub =
    Bap_value.Tag.register
      ...
      ~name:"stub" ~uuid:"c9eaf8a5-783d-4e01-9bf2-5d012602475f"

  let extern =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true
      ~desc:"the program is a part of the public interface" ~name:"extern"
      ~uuid:"7965a2ee-ae72-4eb6-88c1-6f2b6108915f"

  let leaf =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true ~desc:"the program is a leaf function" ~name:"leaf"
  ...
  let malloc =
    Bap_value.Tag.register
      ...
      ~name:"malloc" ~uuid:"b9237d20-07c2-462f-938d-91e4d438fd07"

  let noreturn =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true ~desc:"the program terminates the computation"
      ~name:"noreturn" ~uuid:"1f2941fb-d227-418d-a1d5-9f9d288a585f"

  let nothrow =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true ~desc:"the doesn't throw exceptions" ~name:"nothrow"
      ~uuid:"32058b19-95ca-46e3-bee0-d0a3694fd5b1"

  let returns_twice =
    Bap_value.Tag.register
      (module Unit)
      ~package ~public:true ~desc:"the program is non-determinstic"
      ~name:"returns-twice" ~uuid:"40166004-ea98-431b-81b0-4e74a0b681ee"
  ...
  let entry_point =
    Bap_value.Tag.register
      ...
      ~name:"entry-point" ~uuid:"d1eaff96-4ed4-4405-9305-63508440ccc1"
  ...
  let intrinsic =
    Bap_value.Tag.register
      ...
      ~name:"intrinsic" ~uuid:"68be4fa5-f8f4-4219-b89c-42718c59f09b"

  module Builder = struct
    type t = tid option * arg term vector * blk term vector * string option
    ...
    let result (tid, args, blks, name) : sub term =
      let tid = match tid with Some tid -> tid | None -> Tid.create () in
  ...
  let create ?(args = []) ?(blks = []) ?tid ?name () : t =
    ...
    | _ ->
        let b =
          Builder.create ?tid ?name () ~args:(List.length args)
            ~blks:(List.length blks)
        in
        List.iter args ~f:(Builder.add_arg b);
        List.iter blks ~f:(Builder.add_blk b);
        Builder.result b

  let pp_blks pp ppf blks = Array.iter blks ~f:(Format.fprintf ppf "@;%a@;" pp)

  let pp_self ppf self =
    Format.fprintf ppf "sub %s(%s)@;%a@;%a" self.name
      (String.concat ~sep:", " @@ Array.to_list
      @@ Array.map self.args ~f:Ir_arg.name)
      (Array.pp Ir_arg.pp) self.args (pp_blks Ir_blk.pp) self.blks

  let pp_self_slots ds ppf self =
    Format.fprintf ppf "sub %s(%s)@;%a@;%a" self.name
      (String.concat ~sep:", " @@ Array.to_list
      @@ Array.map self.args ~f:Ir_arg.name)
      (Array.pp (Ir_arg.pp_slots ds))
      self.args
      (pp_blks (Ir_blk.pp_slots ds))
      self.blks

  let pp = term_pp pp_self ~no_cut:true ~box:vertical
  let pp_slots ds = term_pp (pp_self_slots ds) ~no_cut:true ~box:vertical
...
module Ir_program = struct
        ...
        Array.iteri self.subs ~f:(fun i { self } ->
            Array.iteri self.blks ~f:(fun j blk ->
                Array.iteri (get blk.self) ~f:(fun k ent ->
                    if Tid.(ent.tid = tid) then
                      return (Some ([| i; j; k |], ent)))));
        None)
  ...
  type 'a entry = { get : 'a getter; of_path : program term -> path -> 'a term }
  ...
  let finder_of_type (type b) (t : b typ) : b finder =
    match t with
  ...
  let create ?(subs = []) ?(tid = Tid.create ()) () : t =
    ...
    | _ ->
        let b = Builder.create ~tid () ~subs:(List.length subs) in
        List.iter subs ~f:(Builder.add_sub b);
        Builder.result b

  let pp_self ppf self =
    Format.fprintf ppf "program@;%a" (Array.pp Ir_sub.pp) self.subs

  let pp_self_slots ds ppf self =
    Format.fprintf ppf "program@;%a" (Array.pp (Ir_sub.pp_slots ds)) self.subs
