...
module Ops (T : T) = struct
  ...
  module Unary = Registry (struct
    type t = string -> T.t
  end)
end

module Predicates = Ops (struct
  type t = bool Term.visitor
end)

module Mappers = Ops (struct
  type t = Term.mapper
end)
...
let marker parse tag x =
  ...
  object
    ...
    method! map_term cls t = Term.set_attr t tag x |> super#map_term cls
  end
...
module Cmp (T : Comparable) = struct
  ...
  let cmp test tag x cls t _ =
    match Term.get_attr t tag with None -> false | Some y -> test x y
...
module Markers = struct
  ...
  let () = unit "" "set" "has" mark
end

let expect exp got = raise (Parse_error (sprintf "Expected %s got %S" exp got))
...
module Color = struct
  ...
  let color_t s =
    match List.Assoc.find ~equal:String.equal colors s with
    | Some c -> c
...
module Comment = struct
  let () = Mappers.Unary.register "comment" @@ marker Fn.id comment
end

module Python = struct
  let () = Mappers.Unary.register "python" @@ marker Fn.id python
end
...
module Taint = struct
  ...
  let seed tag =
    object
      ...
      method! map_term cls t =
        Term.set_attr t tag (Term.tid t) |> super#map_term cls
    end
  ...
  let has_taint tag =
    has_attr
      (fun taints taint ->
        Map.exists taints ~f:(fun taints -> Set.mem taints taint))
      tag
