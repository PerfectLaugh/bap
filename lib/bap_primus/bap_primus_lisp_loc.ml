...
let merge p1 p2 =
  if String.(p1.file <> p2.file) then
    invalid_arg "Loc: can't merge locations from different files";
  Parsexp.Positions.
    {
      p1 with
      range =
...
let shift_pos p off =
  Parsexp.Positions.{ p with col = p.col + off; offset = p.offset + off }
