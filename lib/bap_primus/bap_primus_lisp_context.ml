...
let attr proj attr =
  match Project.get proj attr with Some x -> [ x ] | None -> []

let endian proj =
  Project.arch proj |> Arch.endian |> function
  | LittleEndian -> "little"
  | BigEndian -> "big"
...
let of_project proj =
  ...
  Name.Map.of_alist_exn
    [
      ...
      ( "abi",
        features @@ attr proj Bap_abi.name
        @ [ enum (module Theory.Abi) (Theory.Target.abi t) ] );
      ("fabi", features [ enum (module Theory.Fabi) @@ Theory.Target.fabi t ]);
      ( "filetype",
        features [ enum (module Theory.Filetype) @@ Theory.Target.filetype t ]
      );
      ...
      ( "target-system",
        features [ enum (module Theory.System) @@ Theory.Target.system t ] );
    ]
...
let sexp_of_context (name, values) =
  Sexp.List (List.map (name :: Set.to_list values) ~f:(fun x -> Sexp.Atom x))

let sexp_of (cs : t) =
  Sexp.List (Atom "context" :: (Map.to_alist cs |> List.map ~f:sexp_of_context))

let value = function { data = Atom x } -> x | s -> expect_atom s
...
let push cs name vs =
  Map.update cs name ~f:(function None -> vs | Some vs' -> Set.union vs vs')
...
let add cs cs' =
  Map.fold cs ~init:cs' ~f:(fun ~key:name ~data:vs cs' -> push cs' name vs)

let pp ppf ctxt = Sexp.pp_hum ppf (sexp_of ctxt)
...
let ( <= ) xs ys =
  Map.fold_symmetric_diff xs ys ~data_equal:String.Set.equal ~init:true
    ~f:(fun matches (_, diff) ->
      matches
      &&
      match diff with
...
let domain = KB.Domain.define "context" ~empty ~order ~join ~inspect:sexp_of
let t = Attribute.declare "context" ~package:"core" ~domain ~parse

let slot =
  KB.Class.property Theory.Unit.cls "primus-lisp-context" ~public:true
    ~persistent:
