...
module Variables = struct
  ...
  let var package t =
    ...
    | { data = Atom v; id; eq } -> (
        match Var.read ~package id eq v with
        | Ok v -> v
        | Error err -> fail (Var_error err) [ t ])

  let parse ~package xs = List.map xs ~f:(var package) |> Var.Set.of_list
  let global = Attribute.declare "global" ~package ~domain ~parse
  let static = Attribute.declare "static" ~package ~domain ~parse
end

type Attribute.error += Expect_atom | Unterminated_quote

let parse_name = function
  | { data = Atom x } as s ->
      let n = String.length x in
      if n < 2 then x
      else if Char.(x.[0] = '"') then
        if Char.(x.[n - 1] = '"') then String.sub ~pos:1 ~len:(n - 2) x
...
module External = struct
  ...
  let parse ~package:_ xs = List.map xs ~f:parse_name |> String.Set.of_list
  let t = Attribute.declare "external" ~package ~domain ~parse
end
...
module Visibility = struct
  ...
  let domain =
    KB.Domain.define "visibility-list" ~empty:[] ~order
      ~inspect:(function
        | [] | Public :: _ -> Sexp.Atom "public" | _ -> Sexp.Atom "private")
  ...
  let t = Attribute.declare "visibility" ~package ~domain ~parse
  let is_public = function Private :: _ -> false | _ -> true
end
...
module Advice = struct
  ...
  let join { methods = xs } { methods = ys } =
    Ok
      {
        methods =
          Map.merge xs ys ~f:(fun ~key:_ -> function
            | `Left xs | `Right xs -> Some xs
            | `Both (xs, ys) -> Some (Set.union xs ys));
      }

  let order xs ys : KB.Order.partial =
    match compare xs ys with 1 -> GT | 0 -> EQ | _ -> LT

  let domain = KB.Domain.define "methods" ~join ~order ~empty

  let methods =
    String.Map.of_alist_exn [ (":before", Before); (":after", After) ]

  let targets { methods } m =
    match Map.find methods m with
  ...
  let parse_targets met ss =
    ...
    | ss ->
        List.fold ss ~init:{ methods = Methods.empty } ~f:(fun { methods } t ->
            {
              methods =
                Map.update methods met ~f:(function
                  | None -> Set.singleton (module KB.Name) (parse_target t)
                  | Some ts -> Set.add ts (parse_target t));
            })

  let parse ~package:_ trees =
    match trees with
    | [] -> fail Empty trees
  ...
  let t = Attribute.declare "advice" ~package ~domain ~parse
end
