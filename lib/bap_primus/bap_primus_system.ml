...
module Repository = struct
  ...
  let add sys =
    if Hashtbl.mem self sys.name then
      invalid_argf "System named %s is already present in the repository"
        (Name.show sys.name) ();
  ...
  let get ?package name = require @@ Name.read ?package name
  ...
  let list () =
    Hashtbl.data self
    |> List.map ~f:(fun { desc; name } -> Info.create ~desc name)
end

let pp_names = Format.pp_print_list ~pp_sep:Format.pp_print_space Name.pp
...
let pp ppf { name; components; depends_on; desc } =
  Format.fprintf ppf
    "@[<v2>(defsystem %a@\n\
     ...
     :depends-on (@[<v>%a@])@\n\
     :components (@[<v>%a@]))@]"
    Name.pp name Format.pp_print_text desc pp_names depends_on pp_names
    components

let component = Name.create
let depends_on = Name.create

let define ?(desc = "") ?(depends_on = []) ?(components = []) ?package name =
  { name = Name.create ?package name; desc; components; depends_on }
...
module Components = struct
  ...
  let add_component ns table ?(internal = false) ?(desc = "")
      ?(package = "user") name init =
    let name = Name.create ~package name in
    if Hashtbl.mem table name then
      invalid_argf
        "A %s component named %s is already registered, please choose a unique \
         name"
        ns (Name.show name) ();
  ...
  module Generic (Machine : Machine) = struct
    ...
    let inited () = Machine.Observation.make inited ()
    let finish () = Machine.Observation.make finish ()
      ...
      let context =
        Context.create
          [
            ...
            ( "component",
              Set.to_list (components s) |> List.map ~f:Knowledge.Name.show );
          ]
      in
    ...
    let run_internal ~boot ~init ~fini ~start =
      Machine.run ~boot
        ...
        ~fini:(fini >>= finish) start

    let run ?envp ?args ?(init = Machine.return ()) ?(fini = Machine.return ())
        ?(start = Machine.return ()) sys proj =
      run_internal (Name.show sys.name) proj ?envp ?args ~boot:(init_system sys)
        ~init ~fini ~start
  end
  ...
  let init_system system =
    components system |> Set.to_list
    |> Machine.List.fold
         ~init:(Set.empty (module Name))
         ~f:(fun loaded name ->
           match Hashtbl.find analyses name with
  ...
  let result_t =
    Knowledge.Domain.optional "primus-computation-result"
      ~equal:(fun (_, p1) (_, p2) -> phys_equal p1 p2)

  let system = Knowledge.Class.declare "system" ~package:"primus" ()

  let result =
    Knowledge.Class.property system "result" result_t ~public:false
      ~package:"primus"

  let run ?envp ?args ?(init = Machine.return ()) ?(fini = Machine.return ())
      ?(start = Machine.return ()) sys proj state =
    let comp =
      let open Knowledge.Syntax in
      Knowledge.Object.create system >>= fun obj ->
      Generics.run_internal (Name.show sys.name) proj ?envp ?args
        ~boot:(init_system sys) ~init ~fini ~start
      >>= fun x ->
      Knowledge.provide result obj (Some x) >>| fun () -> obj
    in
    match Knowledge.run system comp state with
    | Ok (v, state) ->
        let status, proj = Option.value_exn (Knowledge.Value.get result v) in
...
module Parser = struct
  ...
  type error_with_loc = { file : string; system : string; error : error }

  let with_no_loc error = { file = "unknown"; system = "unknown"; error }
  let in_system system error = { file = "unknown"; system; error }
  let in_file file error = { error with file }
  ...
  let rec parse_specs push : t -> Sexp.t list -> (t, error) Result.t =
   fun sys -> function
    ...
    | other :: _ -> Error { expects = Component; got = other }

  let rec parse_items : t -> Sexp.t list -> (t, error) Result.t =
   fun sys -> function
    | [] -> Ok sys
    | (List _ as list) :: _ -> Error { expects = Keyword; got = list }
    ...
    | Atom ":components" :: List comps :: items ->
        Result.bind (parse_specs comp sys comps) ~f:(fun sys ->
            parse_items sys items)
    | Atom ":depends-on" :: List comps :: items ->
        Result.bind (parse_specs deps sys comps) ~f:(fun sys ->
            parse_items sys items)
  ...
  let of_sexps sexps = List.map sexps ~f:of_sexp |> Result.all

  let from_file name =
    Sexp.load_sexps name |> of_sexps |> Result.map_error ~f:(in_file name)
  ...
  let pp_error ppf = function
    | { expects = Defsystem; got } ->
        pr ppf
          "expected (defsystem <system-designator> <system-definition>)got %a"
          Sexp.pp got
    ...
    | { expects = List_of_components; got = atom } ->
        pr ppf "expects a list of components, got an atom %a(add parentheses)"
          Sexp.pp atom
    | { expects = List_of_systems; got = atom } ->
        pr ppf "expects a list of systems, got an atom %a(add parentheses)"
          Sexp.pp atom
    | { expects = Keyword; got = list } ->
        pr ppf "expects an option name, got list %a" Sexp.pp list
    | { expects = Known_keyword; got = unknown } ->
        pr ppf
          "expects option ::= :description | :components, got an unknown \
           option %a"
          Sexp.pp unknown
    | { expects = Component; got = problem } ->
        pr ppf "expects component ::= <ident>, got %a" Sexp.pp problem

  let pp_error_with_loc ppf { file; system; error } =
    pr ppf "In file %S, in the definition of the system %S, %a" file system
      pp_error error
end
...
module Job = struct
  ...
  let create ?(name = "unnamed") ?(desc = "") ?(envp = [||]) ?(args = [||])
      ?(init = Analysis.return ()) ?(fini = Analysis.return ())
      ?(start = Analysis.return ()) system =
    { name; desc; envp; args; init; fini; start; system }
end
...
module Jobs = struct
  ...
  let enqueue ?name ?desc ?envp ?args ?init ?fini ?start sys =
    Queue.enqueue jobs
    @@ Job.create ?name ?desc ?envp ?args ?init ?fini ?start sys
  ...
  let success job project state result =
    { result with project; jobs = job :: result.jobs; state }
  ...
  let run ?(on_failure = fun _ _ _ -> Continue)
      ?(on_success = fun _ _ _ _ -> Continue) =
    let rec process result
        ({ Job.envp; args; init; fini; start; system } as job) =
      Components.run system result.project result.state ~envp ~args ~init ~fini
        ~start
      |> function
      | Ok (status, proj, state) ->
          handle_success job status state @@ success job proj state result
      | Error problem ->
          handle_failure job problem @@ conflict job problem result
    ...
    and continue result =
      ...
      | None -> result
      | Some job -> process result job
    in
    fun project state -> continue { project; state; failures = []; jobs = [] }
end
...
let () =
  Components.register_generic "load-binary"
    (module Bap_primus_interpreter.LinkBinaryProgram)
    ~package:"bap"
    ~desc:
      "Links the binary program into the Primus machine. All symbols of the \
       binary program are linked weakly, i.e., if a symbol is already linked, \
       then this component will not override it."
