...
type 'a defs = 'a Def.t list Map.M(String).t
...
type package = {
  ...
  places : Def.place defs;
  defs : Def.func defs;
  mets : Def.meth defs;
  pars : Def.para defs;
  sigs : Def.signal defs;
}
[@@deriving fields]

type library = package Map.M(String).t
...
let nodefs = Map.empty (module String)
...
let reset_package program = with_package program default_package

let is_public def =
  Lisp.Attributes.Visibility.is_public
  @@ Lisp.Attribute.Set.get Lisp.Attributes.Visibility.t (Def.attributes def)

type def_filter = { visit : 'a. 'a Def.t -> bool }

let filter_defs defs { visit = f } =
  Map.filter_map defs ~f:(fun defs ->
      match List.filter defs ~f with [] -> None | xs -> Some xs)

let filter_package pkg f =
  {
    semas = filter_defs pkg.semas f;
    codes = filter_defs pkg.codes f;
    defs = filter_defs pkg.defs f;
    mets = filter_defs pkg.mets f;
    pars = filter_defs pkg.pars f;
    sigs = filter_defs pkg.sigs f;
    macros = filter_defs pkg.macros f;
    substs = filter_defs pkg.substs f;
    consts = filter_defs pkg.consts f;
    places = filter_defs pkg.places f;
  }

let get_package ?(public = false) program package =
  match Map.find program.library package with
  | None -> empty_package
  | Some pkg -> if public then filter_package pkg { visit = is_public } else pkg

let targets { exports } package =
  match Map.find exports package with
...
let fold_multi ~f =
  Map.fold ~f:(fun ~key:_ ~data x -> List.fold data ~init:x ~f)

let items xs = List.concat (Map.data xs)

let ( ++ ) : 'a defs -> 'a defs -> 'a defs =
 fun xs ys ->
  let remember set def =
    Map.update set (Def.name def) ~f:(function
      | None -> Set.singleton (module Id) def.id
      | Some ids -> Set.add ids def.id)
  in
  let is_presented set def =
    match Map.find set (Def.name def) with
    | None -> false
    | Some ids -> Set.mem ids def.id
  in
  let add ~key:name ~data:defs init =
    List.fold defs ~init ~f:(fun (set, defs) def ->
        if is_presented set def then (set, defs)
        else (remember set def, Map.add_multi defs ~key:name ~data:def))
  in
...
let pp_defs ppf =
  Format.pp_print_list
    (fun ppf x -> Format.fprintf ppf "%s" (Def.name x))
    ~pp_sep:Format.pp_print_space ppf
...
let use_package program ?(target = program.package) from =
  let program =
    {
      program with
      exports =
        Map.update program.exports from ~f:(function
  ...
  in
  {
    ...
    library =
      Set.fold
        (transitive_closure program from)
        ~f:(fun library target ->
...
let equal p1 p2 = Source.equal p1.sources p2.sources
...
let reexport program =
  Map.fold program.exports
    ~f:(fun ~key:from ~data:targets program ->
      Set.fold targets
        ~f:(fun program target -> use_package program ~target from)
        ~init:program)
    ~init:program
  ...
  reexport
    {
      ...
      exports =
        Map.merge_skewed p1.exports p2.exports ~combine:(fun ~key:_ ->
            Set.union);
    }

type full_access = [ `Read | `Set_and_create ]
type 'a item = (full_access, package, 'a defs) Fieldslib.Field.t_with_perm
...
let add prog fld elt =
  ...
  let packages = transitive_closure prog parent in
  {
    ...
    library =
      Set.fold packages
        ~f:(fun lib pkg ->
          if is_public || String.equal pkg parent then
...
let get ?name p (fld : 'a item) =
  match Map.find p.library p.package with
  | None -> []
  | Some pkg -> (
      let defs = Field.get fld pkg in
      match name with
      | None -> List.concat @@ Map.data defs
      | Some name -> (
          match Map.find defs name with None -> [] | Some xs -> xs))

let fold { library } fld ~init ~f =
  Map.fold library ~init ~f:(fun ~key:package ~data x ->
      Map.fold (Field.get fld data) ~init:x ~f:(fun ~key:_ ~data:defs x ->
          List.fold defs ~init:x ~f:(fun x def -> f ~package def x)))
...
let union init xs ~f = List.fold xs ~init ~f:(fun vs x -> vs ++ f x)
...
module Places = struct
  ...
  let collect_registers target =
    List.fold
      (target :: Theory.Target.parents target)
      ~f:(fun regs t ->
  ...
  let collect_globals target program =
    ...
    let var v =
      Theory.Var.forget @@ Lisp_var.reify v ~width:(Theory.Target.bits target)
    in
    let vars def =
      let get attr =
        Set.map (module Theory.Var.Top) ~f:var
        @@ Attribute.Set.get attr (Def.attributes def)
      in
      Set.union_list
        (module Theory.Var.Top)
        [ get Variables.global; get Variables.static ]
    in
    fold program Items.func
      ~init:(Set.empty (module Theory.Var.Top))
      ~f:(fun ~package:_ def acc -> Set.union acc (vars def))

  let add_registers target prog =
    let regs = collect_registers target in
    Map.fold regs
      ~f:(fun ~key:name ~data:r prog ->
        let package = KB.Name.package name in
        let name = KB.Name.unqualified name in
        add prog Items.place @@ Def.Place.create ~package name r)
      ~init:prog
  ...
  let add_globals target prog =
    ...
    Set.fold vars
      ~f:(fun prog v ->
        ...
        let package = KB.Name.package name in
        let name = KB.Name.unqualified name in
        add prog Items.place @@ Def.Place.create ~package name v)
      ~init:prog

  let add_vars ~package vars lisp =
    Seq.fold vars
      ~f:(fun prog v ->
        add prog Items.place @@ Def.Place.create ~package (Theory.Var.name v) v)
      ~init:lisp
  ...
  let add ?(globals = Seq.empty) lisp target =
    List.fold ~init:lisp ~f:( |> )
      [
        ...
        add_vars ~package:"target" @@ Seq.map ~f:snd
...
type node = Entry | Defun of Id.t | Exit [@@deriving compare]
...
module Callgraph = struct
  ...
  let union xs ~f = List.map xs ~f |> Set.union_list (module KB.Name)
  ...
  let compute_ids (defs : library) =
    let init = Map.empty (module KB.Name) in
    fold_library defs ~init ~f:(fun ~package { defs } ids ->
        fold_multi ~init:ids defs ~f:(fun ids def ->
            let name = KB.Name.create ~package (Def.name def) in
            Map.add_multi ids ~key:name ~data:def.id))

  let edge id id' = G.Edge.create (Defun id) (Defun id') ()

  let build_kernel defs =
    let ids =
      let ids = compute_ids defs in
      fun name -> match Map.find ids name with None -> [] | Some x -> x
    in
    Map.fold ~init:G.empty defs ~f:(fun ~key:_ ~data:{ defs } g ->
        fold_multi defs ~init:g ~f:(fun g def ->
            let g = G.Node.insert (Defun def.id) g in
            Set.fold
              (calls (Def.Func.body def))
              ~init:g
              ~f:(fun g name ->
  ...
  let connect_with_entry = function
    | Entry -> Fn.id
    | n -> G.Edge.insert @@ G.Edge.create Entry n ()

  let connect_with_exit = function
    | Exit -> Fn.id
    | n -> G.Edge.insert @@ G.Edge.create n Exit ()

  let close dir g =
    let fix =
      match dir with `In -> connect_with_entry | `Out -> connect_with_exit
    in
    G.nodes g
  ...
  let build library =
    ...
    Graphlib.depth_first_search
      (module G)
      g ~init:g ~start:Entry ~start_tree:connect_with_entry
    |> fun g ->
    Graphlib.depth_first_search
      (module G)
      g ~init:g ~start:Exit ~rev:true ~start_tree:connect_with_exit
...
module Ast = struct
  ...
  let pp_name ppf name =
    if String.equal (KB.Name.package name) !package then
      Format.fprintf ppf "%s" (KB.Name.unqualified name)
    else KB.Name.pp ppf name

  let pp_term pp_exp ppf = function
    | { data = { exp; typ = Any } } -> fprintf ppf "@[<v2>%a@]" pp_exp exp
    | { data = { exp; typ } } ->
  ...
  let rec pp ppf { data } = fprintf ppf "@[<v2>%a@]" pp_exp data

  and pp_exp ppf = function
    | Int x -> pp_word ppf x
    | Sym x -> pp_name ppf x.data
    | Var x -> pp_var ppf x
    | Ite (c, t, e) ->
        fprintf ppf "@[<2>(if@ %a@;<1 2>%a@ %a)@]" pp c pp t pp_prog e
    | Let (v, e, b) ->
        fprintf ppf "@[(let@;<1 2>@[<2>(%a@ %a)@]@ %a)@]" pp_var v pp e pp b
    | App (b, xs) -> fprintf ppf "@[<2>(%a@ %a)@]" pp_binding b pp_exps xs
    | Seq [] -> fprintf ppf "()"
    | Seq [ x ] -> pp ppf x
    | Seq xs -> fprintf ppf "@[<2>(prog@ @[<v>%a@])@]" pp_exps (concat_prog xs)
    | Set (v, x) -> fprintf ppf "@[<2>(set@ %a@ %a)@]" pp_var v pp x
    | Rep (c, b) ->
        fprintf ppf "@[<2>(while@;<1 2>%a@ @[<v>%a@])@]" pp c pp_prog b
    | Msg (f, es) -> fprintf ppf "@[<2>(msg@ \"%a\"@ %a)@]" pp_fmt f pp_exps es
    | Err msg -> fprintf ppf "@[<2>(error@ %s)@]" msg
  ...
  and pp_prog ppf = function
    | { data = Seq xs } -> fprintf ppf "%a" pp_exps (concat_prog xs)
    | exp -> pp ppf exp
end

let pp_def ppf d =
  fprintf ppf "@[<2>(defun %s @[<2>(%a)@]@ %a)@]@," (Def.name d)
    (pp_print_list ~pp_sep:pp_print_space Ast.pp_var)
    (Def.Func.args d) Ast.pp_prog (Def.Func.body d)

let pp_met ppf d =
  fprintf ppf "@[<2>(defmethod %s @[<2>(%a)@]@ %a)@]@," (Def.name d)
    (pp_print_list ~pp_sep:pp_print_space Ast.pp_var)
    (Def.Meth.args d) Ast.pp_prog (Def.Meth.body d)

let pp_par ppf d =
  fprintf ppf "@[<2>(defparamerter %s@,%a@,%S)@]" (Def.name d) Ast.pp_prog
    (Def.Para.default d) (Def.docs d)

let pp_package ppf { pars; mets; defs } =
  let pp_items pp xs = fprintf ppf "@[<v>%a@]" (pp_print_list pp) (items xs) in
...
module Use = struct
  ...
  let vars bound ast =
    let use bound { exp = v } id =
      if Set.mem bound v then empty else use v id
    in
    let rec free bound = function
      ...
      | { data = Let (v, x, y) } ->
          free bound x ++ free (Set.add bound v.data.exp) y
      | { data = Seq xs | App (_, xs) | Msg (_, xs) } ->
          union xs ~f:(free bound)
      | { data = Set (v, x); id } -> use bound v.data id ++ free bound x
      | { data = Rep (x, y) } -> free bound x ++ free bound y
    in
    free bound ast
  ...
  let collect { defs } =
    ...
    fold_multi ~init defs ~f:(fun s def ->
        let body = Def.Func.body def in
        let bound =
          Def.Func.args def
          |> List.map ~f:(fun { data = { exp } } -> exp)
          |> Set.of_list (module KB.Name)
        in
...
(** Assign fresh indices to trees that were produced my macros or that ** have
    no indices at all. ** ** We first scan through all meta definitions (i.e.,
    macros, substs, ** and consts) to obtain a set of indices that we shall
    rewrite, and ** then perform rewriting for all program definitions (defs,
    mets, ** and pars) ** ** The newly generated Ids are derived (i.e.,
    associated) with their ** base ids, so that if needed their origin can be
    always ** established. (except if their origin was the null identifier). **
    ** Motivation: since we identify an ast by its identifier, we want ** the
    trees produced by the term rewriting to have different ** identifiers.
    Otherwise, they could be unified, for example in the ** Type checker. **)
module Reindex : sig
  ...
  let rec ids_of_trees trees =
    List.fold trees ~init:Id.Set.empty ~f:(fun xs t ->
        match t with
        | { data = Atom _; id } -> Set.add xs id
        | { data = List ts; id } -> Set.union (Set.add xs id) (ids_of_trees ts))

  let ids_of_defs defs map reduce =
    Id.Set.union_list
      [
        Id.Set.of_list @@ List.map (items defs) ~f:(fun d -> d.id);
        map (items defs) ~f:reduce |> ids_of_trees;
      ]

  let macro_ids prog =
    fold_library prog
      ~f:(fun ~package:_ p macros ->
        Id.Set.union_list
          [
            macros;
  ...
  let derive from =
    ...
    State.put (Source.derived src ~from nextid) >>| fun () -> nextid

  let reindex (get, set) macros def =
    let rename t =
      if Set.mem macros t.id || Id.(null = t.id) then
        derive t.id >>| fun id -> { t with id }
      else State.return t
    in
    let rec map : ast -> ast m =
     fun t ->
      rename t >>= fun t ->
      match t.data with
      | Err _ -> State.return t
      | Int x -> rename x >>| fun x -> { t with data = Int x }
      | Sym s -> rename s >>| fun s -> { t with data = Sym s }
      | Var v -> rename v >>| fun v -> { t with data = Var v }
      | Ite (x, y, z) ->
          map x >>= fun x ->
          map y >>= fun y ->
          map z >>| fun z -> { t with data = Ite (x, y, z) }
      | Let (c, x, y) ->
          rename c >>= fun c ->
          map x >>= fun x ->
          map y >>| fun y -> { t with data = Let (c, x, y) }
      | Rep (x, y) ->
          map x >>= fun x ->
          map y >>| fun y -> { t with data = Rep (x, y) }
      | App (b, xs) -> map_all xs >>| fun xs -> { t with data = App (b, xs) }
      | Msg (f, xs) -> map_all xs >>| fun xs -> { t with data = Msg (f, xs) }
      | Seq xs -> map_all xs >>| fun xs -> { t with data = Seq xs }
      | Set (v, x) ->
          rename v >>= fun v ->
          map x >>| fun x -> { t with data = Set (v, x) }
  ...
  let reindex_package macros p =
    ...
    State.List.map (items p.defs) ~f:(reindex def macros) >>= fun defs ->
    State.List.map (items p.mets) ~f:(reindex met macros) >>= fun mets ->
    State.List.map (items p.pars) ~f:(reindex par macros) >>= fun pars ->
    State.return (defs, mets, pars)

  let repack defs =
    List.fold defs
      ~f:(fun defs def -> Map.add_multi defs ~key:(Def.name def) ~data:def)
      ~init:nodefs

  let package prog macros pkgname =
    let pkg = get_package prog pkgname in
    let (defs, mets, pars), sources =
      State.run (reindex_package macros pkg) prog.sources
    in
    {
      prog with
      sources;
      library =
        Map.set prog.library ~key:pkgname
          ~data:
            {
              pkg with
              defs = repack defs;
  ...
  let program prog =
    ...
    fold_library prog.library
      ~f:(fun ~package pkg prog ->
        let (defs, mets, pars), sources =
          State.run (reindex_package macros pkg) prog.sources
        in
        {
          ...
          sources;
          library =
            Map.set prog.library ~key:package
              ~data:
                {
                  pkg with
                  defs = repack defs;
...
module Typing = struct
  ...
  *)
  ...
  type tval = Tany | Tsym | Grnd of int [@@deriving compare, sexp_of]
  ...
  module Tvals : sig
    ...
    type t = { result : (tval, type_error) Result.t; origin : Id.t }
    [@@deriving compare, sexp_of]

    let fail where what = { result = Error what; origin = where }
    let create origin t = { result = Ok t; origin }
    let is_any = function { result = Ok Tany } -> true | _ -> false
    let is_bot x = Result.is_error x.result

    let meet_tvals x y t1 t2 =
      match (t1, t2) with
      | Tany, _ | _, Tany -> Ok Tany
      | _ ->
          if compare_tval t1 t2 = 0 then Ok t1
    ...
    let meet id x y =
      ...
      | _, Error _ -> y
      | Ok t1, Ok t2 ->
          { result = meet_tvals x.origin y.origin t1 t2; origin = id }
  ...
  type tvar = Name of string | Tvar of Id.t [@@deriving compare, sexp_of]
  ...
  (** Typing environment.
      ...
      Typing constraint is built as a composition of rules, where each rule is a
      function of type [gamma -> gamma], so the rules can be composed with the
  ...
  module Gamma : sig
    ...
    (** [fail x] marks [x] as an unsound expression, which violates an external
        [rule]. *)

    val get : t -> Id.t -> Tvals.t option
    (** [get gamma exp] returns a type of the expression [exp]. If [None] is
        returned, then the expression doesn't have any statical constraints, so
        its type is [Any]. If some set is returned, then this set denotes a
        disjunction of types, that a term can have during program evaluation. If
        this set is empty, then the term is ill-typed. *)
    ...
    (** [meet x y] types of [x] and [y] shall have the same type, which is the
        greatest lower bound of types [x] and [y]. *)
    ...
    (** [partition g] returns a partition of the set of expressions.
        ...
        Each class denotes a set of expressions that represent the same set of
        values. If the set of values of that group is empty, then those
        expressions are ill-typed. *)
    ...
    module Group : sig
      ...
      (** [values groups] is the set of values that all elements of the
          equivalence class share. [None] denotes a set of all sets, i.e.,
          absence of any constraints for the expressions in the group. *)
    end
    ...
    *)
    type t = { vars : tvar Id.Map.t; vals : Tvals.t Tvar.Map.t }
    [@@deriving compare, sexp_of]

    type rule = t -> t
    type group = { var : tvar; values : Tvals.t option; elements : Id.Set.t }

    let empty = { vars = Id.Map.empty; vals = Tvar.Map.empty }

    let get g x =
      match Map.find g.vars x with None -> None | Some v -> Map.find g.vals v

    let add_var x v g = { g with vars = Map.add_exn g.vars ~key:x ~data:v }
    let set_var x v g = { g with vars = Map.set g.vars ~key:x ~data:v }
    let set_val v t g = { g with vals = Map.set g.vals ~key:v ~data:t }
    let del_val v g = { g with vals = Map.remove g.vals v }
    ...
    *)
    ...
    let merge g1 g2 =
      ...
      let g =
        {
          vars =
            Map.merge g1.vars g2.vars ~f:(fun ~key:_ -> function
              | `Left u | `Right u -> Some u
              | `Both (u, v) when Tvar.equal u v -> Some u
              | `Both _ -> None);
          vals =
            Map.merge g1.vals g2.vals ~f:(fun ~key:_ -> function
    ...
    let with_tvar x g f =
      match Map.find g.vars x with
      | None -> f (Tvar x) (add_var x (Tvar x) g)
      | Some u -> f u g

    let fail x err g =
      with_tvar x g @@ fun u g -> set_val u (Tvals.fail x err) g

    let constr_name x n g =
      with_tvar x g @@ fun u g -> unify_types x u (Name n) g

    let constr_grnd x t g =
      with_tvar x g @@ fun u g ->
    ...
    let pp_exps = pp_print_list ~pp_sep:(fun ppf () -> fprintf ppf ", ") Id.pp
    ...
    let pp_vals_list =
      pp_print_list ~pp_sep:(fun ppf () -> fprintf ppf "+") pp_val

    let pp_vals ppf = function
      | None -> fprintf ppf "T"
      | Some t -> (
          match Tvals.result t with
          | Ok t -> pp_val ppf t
  ...
  let pp_args ppf args =
    pp_print_list Lisp.Type.pp ppf args ~pp_sep:(fun ppf () ->
        pp_print_char ppf ' ')

  let pp_signature ppf { args; rest; ret } =
    fprintf ppf "@[(%a" pp_args args;
    Option.iter rest ~f:(fun rest -> fprintf ppf "&rest %a" Lisp.Type.pp rest);
  ...
  let pp_tvals ppf tvals =
    if Set.is_empty tvals then fprintf ppf "nil"
    else
      fprintf ppf "%a"
  ...
  let apply_return appid ret g =
    match ret with Any -> Gamma.widen appid g | _ -> Gamma.constr appid ret g
  ...
  let apply_signature ?(allow_partial = false) appid ts g { args; rest; ret } =
    let rec apply g ts ns =
      ...
      | [], _ -> (None, [])
      | t :: ts, n :: ns -> apply (Gamma.constr t.id n g) ts ns
    in
    ...
    | Some g, ts -> (
        ...
        | ts -> (
            ...
            | Some typ ->
                Some
                  (List.fold ts ~init:g ~f:(fun g t -> Gamma.constr t.id typ g))
            ))
  ...
  let type_of_expr g expr : typ =
    ...
    | None -> Any
    | Some ts -> (
        match Tvals.result ts with
        | Ok Tsym -> Symbol
        | Ok (Grnd n) -> Type n
        | _ -> Any)

  let type_of_exprs gamma exprs = List.map exprs ~f:(type_of_expr gamma)
  ...
  let signatures glob id gamma name =
    ...
    | None -> (
        match Map.find glob.funcs name with
        | None -> []
        | Some funcs ->
            List.map funcs ~f:(fun def -> signature_of_gamma def gamma))

  let apply glob id name args gamma =
    let sigs = signatures glob id gamma name in
    List.filter_map sigs ~f:(fun s -> apply_signature id args gamma s)
    |> List.hd
    |> function
    | None ->
  ...
  let last xs = match List.rev xs with { id } :: _ -> id | _ -> assert false

  let constr_glob { globs; paras } vars var gamma =
    let name = var.data.exp in
    if Map.mem vars name then gamma
    else
      match Map.find globs name with
      | Some n -> Gamma.constr var.id (Type n) gamma
      | None ->
          if Set.mem paras name then gamma
          else
            Gamma.fail var.id
              (Unresolved_variable (var.id, KB.Name.show name))
              gamma

  let is_special var =
    let var = KB.Name.unqualified var in
    String.is_prefix ~prefix:"*" var && String.is_suffix ~suffix:"*" var

  let constr_special { paras } var gamma =
    let name = var.data.exp in
    if is_special name && not (Set.mem paras name) then
      Gamma.fail var.id (Unresolved_parameter (var.id, KB.Name.show name)) gamma
    else gamma

  let push vars { data; id } = Map.set vars ~key:data.exp ~data:id

  let varclass vars v =
    match Map.find vars v.data.exp with None -> v.id | Some id -> id

  let pp_binding ppf (v, id) = fprintf ppf "%s:%a" v Id.pp id
  let pp_sep ppf () = fprintf ppf ", "

  let pp_vars ppf vs =
    fprintf ppf "{%a}" (pp_print_list ~pp_sep pp_binding) @@ Map.to_alist vs
  ...
  let infer_ast glob bindings ast : Gamma.t -> Gamma.t =
    let rec infer vs expr =
      match expr with
      | { data = Sym _; id } -> Gamma.constr id Symbol
      | { data = Int x; id } -> Gamma.constr id x.data.typ
      | { data = Var v; id } ->
          Gamma.unify v.id (varclass vs v)
          ++ Gamma.unify id v.id ++ constr_glob glob vs v
          ++ Gamma.constr v.id v.data.typ
      | { data = Ite (x, y, z) } -> infer vs x ++ infer vs y ++ infer vs z
      | { data = Let (v, x, y); id } ->
          constr_special glob v ++ Gamma.unify y.id id ++ Gamma.unify x.id v.id
          ++ infer (push vs v) y
          ++ infer vs x
          ++ Gamma.constr v.id v.data.typ
      | { data = App (Dynamic name, xs); id } ->
          apply glob id name xs ++ reduce vs xs
      | { data = Seq [] } -> Fn.id
      | { data = Seq xs; id } -> Gamma.unify (last xs) id ++ reduce vs xs
      | { data = Set (v, x); id } ->
          Gamma.unify_all id [ v.id; x.id; varclass vs v ]
          ++ Gamma.constr v.id v.data.typ
          ++ constr_glob glob vs v ++ infer vs x
      | { data = Rep (c, x); id } ->
          Gamma.unify c.id id ++ infer vs c ++ infer vs x
      | { data = Msg (_, xs); id } -> Gamma.constr id (Type 1) ++ reduce vs xs
      | { data = Err _ } -> Fn.id
      | { data = App (Static _, _) } -> Fn.id
    and reduce vs = function
      | [] -> Fn.id
      | x :: xs -> infer vs x ++ reduce vs xs
    in
    infer bindings ast
  ...
  let transfer glob node gamma =
    ...
    | Entry | Exit -> gamma
    | Defun id -> (
        match find_func glob.funcs id with
        | None -> gamma
        | Some f ->
            ...
            let gamma =
              List.fold args ~init:gamma ~f:(fun gamma v ->
                  Gamma.constr v.id v.data.typ gamma)
            in
  ...
  let make_paras { library } =
    fold_library library
      ~f:(fun ~package { pars } init ->
        fold_multi pars ~init ~f:(fun pars par ->
  ...
  let applicable global =
    let filter elts =
      Map.filter_map elts ~f:(fun defs ->
          List.filter defs ~f:(fun def ->
              let def_ctxt =
                Lisp.Attribute.Set.get Lisp.Context.t (Def.attributes def)
              in
              Lisp.Context.(def_ctxt <= global))
          |> function
          | [] -> None
  ...
  let add_places prog =
    fold prog Items.place
      ~f:(fun ~package place vars ->
  ...
  let infer externals (p : program) : Gamma.t =
    ...
    let fp =
      Graphlib.fixpoint
        (module Callgraph)
        ~rev:true ~start:Exit ~equal ~merge:Gamma.merge ~init ~f:(transfer glob)
        g
    in
  ...
  module Type = struct
    type error = { sources : Source.t; problem : type_error }
    type env = { program : program; gamma : Gamma.t }

    let empty = { program = empty; gamma = Gamma.empty }
    ...
    let pp_exp ppf (src, exp) =
      ...
      fprintf ppf "%a:@\n%a@\n" Loc.pp loc (Source.pp_underline src) loc

    let pp_error ppf { sources; problem } =
      match problem with
      ...
      | Unresolved_signal (exp, name, Some signature) ->
          fprintf ppf
            "\n\
             The signal `%s' has signature `%a'. This signature doesn't match \
             with\n\
      ...
      | Unresolved_parameter (exp, name) ->
          fprintf ppf
            "The variable `%s' is treated as parameter, but there is no such \
             parameter:@\n\
             %a"
      ...
      | No_unification (x, t1, y, t2) ->
          fprintf ppf
            "\n\
             ...
             %a has type %a, but it is expected to have type %a after its \
             unification\n\
             with the expression:@\n\
             %a@\n"
            pp_val t1 pp_val t2 pp_exp (sources, x) pp_val t1 pp_val t2 pp_exp
            (sources, y)
  end
end
