...
module State = struct
  ...
  let var = KB.Context.declare ~package:"bap" "lisp-interpter-state" !!empty
...
type KB.Conflict.t += Unresolved_definition of string | User_error of string

let package = "bap"
let language = Theory.Language.declare ~package "primus-lisp"

let program =
  KB.Class.property Theory.Source.cls "primus-lisp-program" ~public:true
    ~package
  @@ KB.Domain.flat "lisp-program" ~empty:Program.empty ~equal:Program.equal
...
let pp_problem ppf = function
  ...
  | Uncallable -> Format.fprintf ppf "This item is not callable"
  | Unexpected Prim -> ()
  | Unexpected kind ->
      Format.fprintf ppf "internal error, unexpected %a" Sexp.pp
        (sexp_of_kind kind)

let unresolved name problem =
  let msg =
    Format.asprintf "Failed to find a definition for %a.@ %a@." KB.Name.pp name
      pp_problem problem
  in
...
module Property = struct
  let name =
    KB.Class.property Theory.Program.cls ~package "lisp-name"
    @@ KB.Domain.optional "lisp-name" ~equal:KB.Name.equal
  ...
  let args =
    KB.Class.property Theory.Program.cls ~package "lisp-args"
    @@ KB.Domain.optional "lisp-args" ~equal:equal_args ~inspect:sexp_of_args
         ~join:(fun xs ys ->
...
let definition =
  KB.Class.property Theory.Program.cls "lisp-definition" ~package ~public:true
    ~persistent:
      (KB.Persistent.of_binable
         (module struct
           type t = Theory.Label.t option [@@deriving bin_io]
         end))
  @@ KB.Domain.optional "label" ~equal:Theory.Label.equal
       ~inspect:Theory.Label.sexp_of_t

let dummy_type _ = Type.{ args = []; rest = Some any; ret = any }

let declare ?(types = dummy_type) ?(docs = "undocumented") ?package ?body name =
  let name = KB.Name.create ?package name in
  if Hashtbl.mem library name then
    invalid_argf
      "A semantic primitive `%s' already exists, please choose a different \
       name for your primitive"
      (KB.Name.show name) ();
  Hashtbl.add_exn library ~key:name ~data:{ docs; types; body; kind = `defn }

let signal ?params ?(docs = "undocumented") property reflect =
  let name = KB.Slot.name property in
  if Hashtbl.mem library name then
    invalid_argf "The signal `%s' is already reflected." (KB.Name.show name) ();
  ...
  let types t =
    ...
    | Some (`Gen (ts, r)) -> Type.signature ~rest:(r t) (app ts t) Any
  in
  ( KB.observe property @@ fun lbl x ->
  ...
  Hashtbl.add_exn library ~key:name
    ~data:{ docs; types; body = None; kind = `meth }
...
let create eff res = KB.Value.put Theory.Semantics.value eff (forget res)
...
let set_static r x =
  update_value r @@ fun v -> KB.Value.put static_slot v (Some x)
...
let sym str =
  let v = update_value empty @@ fun v -> KB.Value.put symbol v (Some str) in
  match str with
  | "t" -> KB.return @@ set_static v Bitvec.one
  | "nil" -> KB.return @@ set_static v Bitvec.zero
  | name -> intern name >>| set_static v
...
let static x = KB.Value.get static_slot (res x)

let reify_sym x =
  match static x with
  | Some _ -> KB.return x
  | None -> (
      match KB.Value.get symbol (res x) with
      | None -> KB.return x
      | Some name -> intern name >>| set_static x)

let is_machine_var t v = Set.mem (Theory.Target.vars t) (Theory.Var.forget v)
...
module Env = struct
  ...
  let set v x =
    State.update @@ fun s ->
    { s with binds = Map.set s.binds ~key:(Theory.Var.forget v) ~data:x }
  ...
  let var word { data = { exp = n; typ = t } } =
    let s =
      match t with Any | Name _ -> word | Symbol -> symsort | Type m -> m
    in
  ...
  let set_args ws bs =
    ...
    let binds, old =
      List.fold bs ~init:(s.binds, []) ~f:(fun (s, old) (v, x) ->
          let v = var ws v in
          (Map.set s ~key:v ~data:x, (v, Map.find s v) :: old))
    in
    State.set { s with binds } >>| fun () -> List.rev old
  ...
  let del_args bs =
    State.update @@ fun s ->
    {
      s with
      binds =
        List.fold bs ~init:s.binds ~f:(fun s (v, x) ->
            match x with
            | None -> Map.remove s v
...
module Scope = struct
    ...
    State.update (fun s ->
        {
          s with
          scope =
            Map.update s.scope orig ~f:(function
              | None -> [ v ]
              | Some vs -> v :: vs);
        })
    >>| fun () -> v
  ...
  let pop orig =
    State.update @@ fun s ->
    {
      s with
      scope =
        Map.change s.scope (forget orig) ~f:(function
  ...
  let clear =
    let* s = State.get in
    let+ () = State.set { s with scope = Map.empty (module Theory.Var.Top) } in
    s.scope

  let restore scope = State.update @@ fun s -> { s with scope }
end
...
module Prelude (CT : Theory.Core) = struct
  ...
  let pure res = res >>| fun res -> create empty res

  let bigint x m =
    let s = bits m in
    let m = Bitvec.modulus m in
    let x = Bitvec.(bigint x mod m) in
    pure @@ CT.int s x >>| fun r -> set_static r x

  let ( := ) v x = CT.set v x

  let full eff res =
    seq eff >>= fun eff ->
    res >>| fun res -> create eff res
  ...
  let blk lbl xs = seq [ CT.blk lbl pass skip; seq xs ]
  let nil = !!(Theory.Value.empty Theory.Bool.t)
  let undefined = full [] nil
  let purify eff = full [] !!(res eff)

  let coerce_bits s x f =
    let open Theory.Value.Match in
    let| () = can Theory.Bitv.refine x @@ fun x -> CT.cast s CT.b0 !x >>= f in
    let| () =
      can Theory.Bool.refine x @@ fun cnd ->
      CT.ite !cnd (CT.int s Bitvec.one) (CT.int s Bitvec.zero) >>= fun x -> f x
    in
    undefined
  ...
  let coerce_bool x f =
    ...
    let| () =
      can Theory.Bitv.refine x @@ fun x ->
      CT.non_zero !x >>= fun x -> f x
    in
    undefined
  ...
  let assign ?(local = false) target v eff =
    ...
    | Some _ when local || not (is_machine_var target v) ->
        Env.set v (res eff) >>= fun () -> purify eff
    | _ ->
        Env.del v >>= fun () ->
  ...
  let reify ppf program defn target name args =
    ...
    let rec eval : ast -> unit Theory.Effect.t KB.t = function
      ...
      | { data = Var { data = { exp = n; typ = Type t } } } ->
          lookup @@ var ~t n
      ...
      | { data = Seq xs } -> seq_ xs
      | { data = Set ({ data = { exp = n; typ = Type t } }, x) } ->
          set_ (var ~t n) x
    ...
    and ite cnd yes nay =
      ...
      | Some cnd -> if Bitvec.(equal cnd zero) then eval nay else eval yes
      | None ->
          coerce_bool (res cnd) @@ fun cres ->
          Theory.Var.fresh Theory.Bool.t >>= fun c ->
          let* yes = eval yes in
          let* nay = eval nay in
          full [ !!cnd; data [ c := !cres ]; CT.branch (CT.var c) !yes !nay ]
          @@ CT.ite (CT.var c) !(res yes) !(res nay)
    and rep cnd body =
      let* r = eval cnd in
      match static r with
      | Some x ->
          if Bitvec.(equal x zero) then !!r
          else eval body >>= fun _ -> rep cnd body
      | None ->
          ...
          full
            [
              ...
              blk loop [ !!body ];
              blk tail [ !!r; ctrl [ CT.branch !cres (CT.goto head) skip ] ];
            ]
            !!cres
    ...
    and msg fmt args =
      ...
      List.iter fmt ~f:(function
        ...
        | Pos n -> (
            ...
            | Some v -> (
                match KB.Value.get static_slot v with
    ...
    and lookup v =
      ...
      | None -> (
          ...
          | Some v -> pure !!v
          | None -> (
              match lookup_parameter prog v with
    ...
    and let_ v x b =
      ...
      if is_parameter prog orig then
        Env.set orig (res xeff) >>= fun () ->
        let* beff = eval b in
        Env.del orig >>= fun () -> full [ !!beff ] !!(res beff)
      else
        Scope.push orig >>= fun v ->
        let* aeff = assign ~local:true target v xeff in
        let* beff = eval b in
        Scope.pop orig >>= fun () -> full [ !!aeff; !!beff ] !!(res beff)
    and prim ?(package = "core") name args =
      call (KB.Name.read ~package name) args
    in
    match args with
...
module Unit = struct
  ...
  let slot =
    KB.Class.property Theory.Unit.cls "lisp-unit" ~package ~public:true
    @@ KB.Domain.optional "unit-name" ~inspect:sexp_of_string
         ~equal:equal_string
  ...
  let is_lisp obj = KB.collect slot obj >>| Option.is_some
...
let failp fmt = Format.kasprintf (fun msg -> KB.fail (Primitive_failed msg)) fmt
...
let primitive name defn args =
  ...
  >>= fun () ->
  KB.catch (KB.collect Theory.Semantics.slot obj) (function
    | Primitive_failed msg -> KB.fail (Failed_primitive (name, msg))
    | other -> KB.fail other)
...
let collect_names kind key prog =
  Program.fold prog key
    ~f:(fun ~package def names ->
      if Program.is_applicable prog def then
...
let merge_names names =
  List.reduce names ~f:(Map.merge_skewed ~combine:(fun ~key:_ _ v -> v))
  |> function
  | None -> Map.empty (module KB.Name)
  | Some r -> r
...
let obtain_typed_program unit =
  ...
  | None -> (
      ...
      let input =
        ...
        @@ Context.merge (Program.context init) context
      in
      let* prog = link_library target @@ Program.with_places input target in
      let tprog = Program.Type.infer prog in
      let prog = Program.Type.program tprog in
      let places =
        Program.fold prog Key.place
      ...
      | [] ->
          ...
          KB.provide Theory.Unit.source unit src >>| fun () -> program
...
let provide_semantics ?(stdout = Format.std_formatter) () =
  ...
  KB.Rule.(
    ...
    |> require Property.name |> require Property.args
    |> require Theory.Label.unit |> require Theory.Unit.source
    |> require Theory.Unit.target |> require program
  ...
  KB.Context.set State.var
    State.
      {
        binds = Map.empty (module Theory.Var.Top);
        scope = Map.empty (module Theory.Var.Top);
        arith =
          (module Bitvec.Make (struct
  ...
  | Some basic when Insn.(res <> empty) -> Insn.with_basic res basic
  | _ -> res

let provide_attributes () =
  let open KB.Syntax in
  let empty = Attribute.Set.empty in
  let ( >>=? ) x f = x >>= function None -> !!empty | Some x -> f x in
  ...
  let package = KB.Name.package name and name = KB.Name.unqualified name in
...
let () =
  ...
  | Property.Unequal_arity -> Some "The number of arguments is different"
  | User_error msg -> Some ("error: " ^ msg)
  | Illtyped_program errs ->
      let open Format in
      let msg =
        asprintf "%a"
          (pp_print_list ~pp_sep:pp_print_newline Program.Type.pp_error)
          errs
      in
      Some msg
  | _ -> None
