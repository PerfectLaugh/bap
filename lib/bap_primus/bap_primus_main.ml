...
let () =
  System.Repository.add
  @@ System.define "legacy-main" ~package
       ~components:[ System.component ~package "load-binary" ]
       ~desc:
         "The legacy Primus system that contains all components registered \
          with the Machine.add_component function."

let add_component component =
  incr components;
  let name = sprintf "legacy-main-component-%d" !components in
  System.Components.register_generic name component ~internal:true
