...
type ('m, 'a) observers = { last : int; subs : (int * ('a -> 'm)) list }
...
type 'a mstream = { providers : provider list }
...
let provide ?desc ?(inspect = sexp_of_opaque) ?package name =
  ...
  if Hashtbl.mem providers name then
    failwithf
      "Observation name `%s' is already used by another component. Please, \
...
let add_observer observers key obs =
  ...
  | Some { last; subs } ->
      ( Map.set observers ~key
          ~data:{ last = last + 1; subs = (last + 1, obs) :: subs },
        Subs (key, last + 1) )
...
let add_watcher observers { key } obs = add_observer observers key obs

let callbacks os key =
  match Map.find_exn os key with exception _ -> [] | { subs } -> subs
...
module Make (Machine : Monad.S) = struct
  ...
  let push_data p key os ws x =
    ...
    if Stream.has_subscribers p.data then
      Signal.send p.newdata (Lazy.force sexp);
    apply Fn.id x os >>= fun () -> apply Lazy.force sexp ws
  ...
  let notify_if_observed os key k =
    ...
    | os, ws -> k @@ fun x -> push_data p key os ws x
  [@@inline]
end

let empty = Map.empty
let list_providers () = Hashtbl.data providers
let list () = list_providers () |> List.map ~f:(fun { info } -> info)
