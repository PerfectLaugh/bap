...
type source = Cmdline | Module of Loc.t
...
let is_symbol s =
  String.length s > 1 && match s.[0] with '\'' | ':' -> true | _ -> false

let is_keyword s = String.length s > 1 && Char.(s.[0] = ':')

let unquote s =
  if is_quoted s then String.sub ~pos:1 ~len:(String.length s - 2) s else s
...
module Parse = struct
  ...
  let expand prog cs =
    List.concat_map cs ~f:(function
      ...
      | { data = Atom x } as atom -> (
          ...
          | Some (Ok (d, ())) ->
              Def.Subst.body d
              |> List.map ~f:(fun tree -> { tree with id = atom.id })))
  ...
  let fmt prog fmt tree =
    ...
    let fail err off =
      let pos = Loc.nth_char (loc (Program.sources prog) [ tree ]) (off + 1) in
      raise (Fail (Format_error (err, pos)))
    in
    let unescape off c =
      try Scanf.unescaped (sprintf "\\%c" c)
      with Scanf.Scan_failure _ -> fail (Illegal_escape c) off
    in
    let nil = `Lit [] in
    ...
    let rec parse off spec state =
      ...
      if Int.(off < String.length fmt) then
        ...
        | (`Esc | `Exp) as state -> fail (Unterminated state) off
    in
    parse 0 [] nil
    ...
    let rec exp : tree -> ast =
     fun tree ->
      ...
      let macro op args =
        ...
        | Some (Error err) -> fail (Unresolved (Macro, op, err)) tree
      in
      ...
      let sym ({ id; eq; data = r } as s) =
        if is_symbol r then cons (Sym { s with data = symbol ~package s.data })
        else
          match Var.read ~package id eq r with
          | Error e -> fail (Bad_var_literal e) tree
          | Ok v -> cons (Var v)
      in

      let lit ({ id; eq; data = r } as t) =
        match Word.read id eq r with
        | Ok x -> cons (Int x)
        | Error Not_an_int -> sym t
        | Error other -> fail (Bad_word_literal other) tree
      in
      ...
      let start : tree -> ast = function
        ...
        | { data = Atom ux } as t -> (
            ...
            | Some (Ok (const, ())) -> exp (Def.Const.value const))
      in
      start tree
  ...
  let ascii xs =
    let rec loop xs acc =
      ...
      | ({ data = Atom x } as s) :: xs when is_quoted x ->
          let x =
            try Scanf.unescaped @@ unquote x
            with Scanf.Scan_failure _ -> fail Bad_ascii s
          in
          String.fold x ~init:acc ~f:(fun acc x ->
              { data = x; id = s.id; eq = Eq.null } :: acc)
          |> loop xs
      | here :: _ -> fail Bad_ascii here
    in
    ...
    let rec loop xs acc =
      ...
      | ({ data = Atom x } as s) :: xs ->
          ...
          String.foldi x ~init:acc ~f:(fun i acc _ ->
              if is_odd i then
                { s with data = Atom (sprintf "0x%c%c" x.[i - 1] x.[i]) } :: acc
              else acc)
          |> loop xs
    in
    loop xs []
  ...
  let defun ?docs ?(attrs = []) name p body prog gattrs tree =
    ...
    Program.add prog func
    @@ Def.Func.create ?docs ~attrs name (params p)
         { data = Seq es; id = tree.id; eq = tree.eq }
         tree
  ...
  let defmethod ?docs ?(attrs = []) name' p body prog gattrs tree =
    ...
    Program.add prog meth
    @@ Def.Meth.create ?docs ~attrs name (params p)
         { data = Seq es; id = tree.id; eq = tree.eq }
         tree
  ...
  let defmacro ?docs ?(attrs = []) name ps body prog gattrs tree =
    ...
    @@ Def.Macro.create ?docs
         ~attrs:(parse_declarations prog gattrs attrs)
         name (metaparams ps) body tree

  let defparameter ?docs ?(attrs = []) name body prog gattrs tree =
    let attrs = parse_declarations prog gattrs attrs in
    Program.add prog para
    @@ Def.Para.create ?docs ~attrs name
         (parse (constrained prog attrs) body)
         tree

  let is_keyarg = function { data = Atom s } -> is_keyword s | _ -> false
  ...
  let declaration prog gattrs s =
    match s with
    | { data = List [ { data = Atom "in-package" }; { data = Atom package } ] }
      ->
  ...
  let stmt gattrs state s =
    match s with
    | {
     data = List ({ data = Atom "defpackage" } :: { data = Atom name } :: elts);
    } ->
    ...
    | {
     data =
       List ({ data = Atom "defun" } :: { data = Atom name } :: params :: body);
    } ->
    ...
    | {
     data =
       List
         ({ data = Atom "defmethod" } :: { data = Atom name } :: params :: body);
    } ->
        defmethod name params body state gattrs s
    | { data = List ({ data = Atom "defmethod" } :: _) } ->
        fail (Bad_def Meth) s
    | {
     data = List [ { data = Atom "defparameter" }; { data = Atom name }; body ];
    } ->
    ...
    | { data = List ({ data = Atom "defparameter" } :: _) } ->
        fail (Bad_def Para) s
    | _ -> state

  let meta gattrs state s =
    match s with
    | { data = List [ { data = Atom "in-package" }; { data = Atom package } ] }
      ->
        Program.with_package state package
    ...
    | {
     data =
       List [ { data = Atom "defmacro" }; { data = Atom name }; params; body ];
    } ->
    ...
    | {
     data = List ({ data = Atom "defsubst" } :: { data = Atom name } :: body);
    } ->
        defsubst name body state gattrs s
    | { data = List ({ data = Atom "defsubst" } :: _) } ->
        fail (Bad_def Subst) s
    | { data = List ({ data = Atom "defmacro" } :: _) } ->
        fail (Bad_def Macro) s
    | { data = List ({ data = Atom "defconst" } :: _) } ->
        fail (Bad_def Const) s
    | _ -> state

  let declarations prog trees =
    snd
    @@ List.fold trees
         ~f:(fun (prog, attrs) tree -> declaration prog attrs tree)
  ...
  let source constraints source =
    ...
    let state =
      Source.fold source ~init ~f:(fun _ trees state ->
          ...
          List.fold trees ~init:state ~f:(meta (declarations state trees)))
    in
...
module Load = struct
  ...
  let load_tree paths p feature loc =
    ...
    | None -> raise (Fail (Unresolved_feature (feature, loc)))
    | Some name -> (
        match Source.find p name with
        | Some _ -> p
        | None -> (
            match Source.load p name with
            | Ok p -> p
  ...
  let transitive_closure paths p =
    let rec fixpoint p =
      ...
      else fixpoint (load_trees paths p required)
    in
    fixpoint p
  ...
  let features ?(paths = [ Filename.current_dir_name ]) proj fs =
    let source =
      ...
      |> transitive_closure paths
    in
    try Parse.source (Context.of_project proj) source
    with Parse_error (err, trees) ->
      raise (Fail (Parse_error (err, loc source trees)))
end

let program ?paths proj features =
  try Ok (Load.features ?paths proj features) with Fail e -> Error e
...
let string_of_def_syntax = function
  ...
  | Subst ->
      "(defsubst <ident> [<docstring>] [<declarations>] [:<syntax>] <atom> ...)"
  | Inpkg -> "(in-package <ident>)"
  | Defpkg ->
      "(defpackage <ident> (:use <package-list>) (:documentation <docstring>))"

let pp_parse_error ppf err =
  match err with
  | Bad_var_literal e ->
  ...
  | Bad_form name ->
      fprintf ppf "bad %s syntax - expected %s" name
        (string_of_form_syntax name)
  | Bad_app ->
      fprintf ppf "head of the list in the application form shall be an atom"
  | Bad_let_binding -> fprintf ppf "expected a variable literal"
  | Bad_param_list -> fprintf ppf "expected a list of variables"
  | Bad_macro_param_list -> fprintf ppf "expected a list of atoms"
  | Bad_require -> fprintf ppf "expected (require <feature>)"
  | Unknown_toplevel -> fprintf ppf "unknown toplevel form"
  | Bad_toplevel -> fprintf ppf "bad toplevel syntax"
  | Bad_ascii | Bad_hex -> fprintf ppf "expected a list of atoms"
  | Unknown_subst_syntax -> fprintf ppf "unknown substitution syntax"
  | Unresolved (k, n, r) ->
      fprintf ppf "unable to resolve %s `%a', because %a" (string_of_defkind k)
        KB.Name.pp n Resolve.pp_resolution r
  | Bad_def def ->
      fprintf ppf "bad %s definition, expect %s" (string_of_defkind def)
        (string_of_def_syntax def)

let pp_request ppf req =
  match req with
  | Cmdline -> fprintf ppf "requested by a user"
  | Module loc -> fprintf ppf "requested in %a" Loc.pp loc

let pp_format_error ppf err =
  match err with
...
let pp_error ppf err =
  ...
  | Sexp_error (name, req, err) ->
      fprintf ppf "%a@\nOccured when parsing feature %s %a" Source.pp_error err
        name pp_request req
  | Unresolved_feature (name, req) ->
      fprintf ppf "Error: no implementation provided for feature `%s' %a" name
        pp_request req
