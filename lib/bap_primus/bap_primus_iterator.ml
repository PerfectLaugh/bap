...
type ('a, 'e) iterator = Fin of ('a, 'e) finite | Inf of ('a, 'e) infinite
...
module Finite = struct
  ...
  let enum : type a e. (a, e) t -> a -> e Sequence.t =
   fun (module G) gen ->
    Sequence.unfold ~init:(Some gen) ~f:(function
      | None -> None
      | Some gen -> Some (G.value gen, G.next gen))

  let create : type a e. (a, e) finite -> (a, e) iterator = fun g -> Fin g
end
...
module Infinite = struct
  ...
  let create : type a e. (a, e) infinite -> (a, e) iterator = fun g -> Inf g
end

let enum = function Fin g -> Finite.enum g | Inf g -> Infinite.enum g
