...
let () =
  Exn.add_printer (function
    | Undefined_var v -> Some (sprintf "undefined variable `%s'" (Var.name v))
    | _ -> None)

type t = { values : value Var.Map.t; random : Generator.t Var.Map.t }

let inspect_generated (v, x) = Sexp.List [ sexp_of_var v; sexp_of_value x ]

let generated, on_generated =
  Observation.provide "var-generated" ~inspect:inspect_generated ~package:"bap"
    ~desc:"Occurs when a new value is generated during the read operation"

let sexp_of_values values =
  Sexp.List
    (Map.to_sequence values
    |> Seq.map ~f:(fun (v, { value }) ->
        Sexp.List
          [
            Sexp.Atom "set-var";
...
let sexp_of_random map =
  Sexp.List
    ...
    |> Seq.map ~f:(fun (v, gen) ->
        Sexp.List
          [
            Sexp.Atom "gen-var"; Sexp.Atom (Var.name v); Generator.sexp_of_t gen;
          ])
    |> Seq.to_list_rev)

let sexp_of_env { values; random } =
  Sexp.List [ sexp_of_values values; sexp_of_random random ]

let state =
  Bap_primus_machine.State.declare ~inspect:sexp_of_env
    ~uuid:"44b24ea4-48fa-47e8-927e-f7ba65202743" ~name:"environment" (fun _ ->
      { values = Var.Map.empty; random = Var.Map.empty })

let inspect_environment { values; random } =
  let keys =
    Set.union
      (Var.Set.of_list (Map.keys values))
      (Var.Set.of_list (Map.keys random))
  in
  let sexp_of_var sexp_of_value var value =
    Sexp.(
      List
        [
          Atom (Var.name var);
  ...
  let bindings =
    Set.fold keys ~init:[] ~f:(fun acc var ->
        ...
        | None -> (
            match Map.find random var with
...
module Make (Machine : Machine) = struct
  ...
  let null = Machine.gets Project.target >>| Theory.Target.bits >>= Value.zero
    ...
    | None -> (
        ...
        | Type.Imm width -> (
            ...
            | Some gen ->
                ...
                set var x >>= fun () ->
                Machine.Observation.make on_generated (var, x) >>| fun () -> x))

  let is_set var = Machine.Local.get state >>| fun t -> Map.mem t.values var
