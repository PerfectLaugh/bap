...
module Index = struct
  ...
  include Strings.Index.Persistent.Make (struct
    ...
    let null = { id = Int63.zero; value = Word.zero key_width }
    let succ s = { s with value = Word.succ s.value }
  end)
end

let state =
  Bap_primus_machine.State.declare ~uuid:"873f2ba6-9adc-45bb-8ed1-a0f57337ca80"
    ~name:"value" (fun _ -> Int63.one)

let symbols =
  Bap_primus_machine.State.declare ~uuid:"2d2293e9-4c42-4b82-90a7-7e8e74fd01ed"
    ~name:"symbols" (fun _ -> Index.empty)
...
module Reg = Regular.Make (struct
  ...
  let pp ppf { id; value } = fprintf ppf "%a#%a" Word.pp_hex_full value Id.pp id
  ...
  let t_of_sexp = function
    ...
    | Sexp.Atom s -> (
        ...
        | [ w; id ] -> { value = Word.of_string w; id = Int63.of_string id }
...
module Make (Machine : Machine) = struct
  ...
  let of_word value =
    ...
    Machine.Global.put state (Int63.succ id) >>| fun () -> { id; value }
  ...
  module Symbol = struct
    let to_value sym =
      ...
      Machine.Global.put symbols index >>| fun () -> Index.key index sym

    let of_value value =
      Machine.Global.get symbols >>| fun index -> Index.string index value
  end
