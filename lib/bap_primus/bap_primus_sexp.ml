...
let rec sexp_of_exp = function
  ...
  | Bil.BinOp (op, x, y) ->
      Sexp.List
        [ Sexp.Atom (Bil.string_of_binop op); sexp_of_exp x; sexp_of_exp y ]
  | Bil.UnOp (op, x) ->
      Sexp.List [ Sexp.Atom (Bil.string_of_unop op); sexp_of_exp x ]
  | Bil.Var x -> sexp_of_var x
  | Bil.Int x -> sexp_of_word x
  | Bil.Cast (ct, n, x) ->
      Sexp.List
        [ Sexp.Atom (Bil.string_of_cast ct); sexp_of_int n; sexp_of_exp x ]
  | Bil.Let (v, x, y) ->
      Sexp.List
        [
          Sexp.Atom "let";
  ...
  | Bil.Ite (c, x, y) ->
      Sexp.List [ Sexp.Atom "ite"; sexp_of_exp x; sexp_of_exp y ]
  | Bil.Unknown (v, x) -> Sexp.Atom "bot"
  | Bil.Extract (hi, lo, x) ->
      Sexp.List
        [ Sexp.Atom "extract"; sexp_of_int hi; sexp_of_int lo; sexp_of_exp x ]
  | Bil.Concat (x, y) ->
      Sexp.List [ Sexp.Atom "concat"; sexp_of_exp x; sexp_of_exp y ]
