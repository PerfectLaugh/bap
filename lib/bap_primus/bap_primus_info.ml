...
type t = { name : Name.t; desc : string; long : unit -> string }
...
let unquote s =
  if String.is_prefix s ~prefix:{|"|} && String.is_suffix s ~suffix:{|"|} then
...
let normalize_text s = dedup_whitespace (unquote (String.strip s))

let create ?(long = fun () -> "") ?(desc = "") name =
  { name; long; desc = normalize_text desc }
