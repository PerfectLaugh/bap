...
type meta = { name : string; docs : string; attrs : attrs } [@@deriving fields]
type func = { args : var list; body : ast } [@@deriving fields]
type meth = func
type macro = { param : string list; subst : tree } [@@deriving fields]
type subst = { elts : tree list } [@@deriving fields]
...
type sema = { apply : semafun; types : Type.signature }
type para = { default : ast }
...
type prim = { lambda : closure; types : Type.signature option }
...
let create data tree = { data; id = tree.id; eq = tree.eq }

let rename def name : 'a t =
  { def with data = { def.data with meta = { def.data.meta with name } } }
...
module Func = struct
  ...
  let create ?(docs = "") ?(attrs = Attribute.Set.empty) name args body =
    create { meta = { name; docs; attrs }; code = { args; body } }

  let with_body t body =
    { t with data = { t.data with code = { t.data.code with body } } }
end
  ...
  let create ?(docs = "") ~types name apply : sema t =
    {
      data =
        {
          meta = { name = KB.Name.show name; docs; attrs = Attribute.Set.empty };
          code = { apply; types };
        };
      id = Source.Id.null;
      eq = Source.Eq.null;
...
module Signal = struct
  ...
  let create ?(docs = "") ~types name : t =
    {
      data =
        { meta = { name; docs; attrs = Attribute.Set.empty }; code = types };
      id = Source.Id.null;
      eq = Source.Eq.null;
    }
...
module Para = struct
  let create : 'a def =
   fun ?(docs = "") ?(attrs = Attribute.Set.empty) name default ->
    create { meta = { name; docs; attrs }; code = { default } }

  let default p = p.data.code.default

  let with_default t default =
    { t with data = { t.data with code = { default } } }
end
...
module Macro = struct
  ...
  let create ?(docs = "") ?(attrs = Attribute.Set.empty) name param subst =
    create { meta = { name; docs; attrs }; code = { param; subst } }

  let take_rest xs ys =
    let rec take xs ys zs =
      match (xs, ys) with
      | [], [] -> Some zs
    ...
    | Some [] -> Some (0, [])
    | Some ((_, rest) :: _ as bs) -> Some (List.length rest, List.rev bs)
    | None -> None
  ...
  let subst bs body =
    let rec sub : tree -> tree list = function
      ...
      | { data = Atom x } as atom -> (
          match find bs x with None -> [ atom ] | Some cs -> cs)
    in
    match body with
    | { data = List xs; id } ->
        { data = List (List.concat_map xs ~f:sub); id; eq = unknown }
...
module Const = struct
  let create ?(docs = "") ?(attrs = Attribute.Set.empty) name ~value =
    create { meta = { name; docs; attrs }; code = { value } }
...
module Subst = struct
  ...
  let create ?(docs = "") ?(attrs = Attribute.Set.empty) name elts =
    create { meta = { name; docs; attrs }; code = { elts } }
end
...
module Closure = struct
  let of_primitive prim lambda =
    { prim with data = { prim.data with code = { lambda; types = None } } }
