...
type refs = { term : tid option; name : string option; addr : addr option }
...
include struct
  ...
  let sexp_of_call (dst, args) = List (Atom dst :: sexp_of_args args)
end
...
module Trace = struct
  ...
  let exec, will_exec =
    Observation.provide ~inspect:sexp_of_name "linker-exec"
      ~desc:"Occurs before code with the given name is executed."

  let unresolved, will_fail =
    Observation.provide ~inspect:sexp_of_name "linker-unresolved"
...
let id_of_name name s =
  match name with
  | `symbol name -> Map.find s.names name
  | `addr addr -> Map.find s.addrs addr
  | `tid tid -> Map.find s.terms tid

let code_of_name name s = Option.bind (id_of_name name s) ~f:(Map.find s.codes)

let lookup_name k t1 names : string option =
  match Map.find t1 k with
  | None -> None
  | Some idx ->
      Map.to_sequence names
      |> Seq.find_map ~f:(fun (n, i) -> Option.some_if Int.(i = idx) n)
...
let state =
  Bap_primus_machine.State.declare ~uuid:"38bf35bf-1091-4220-bf75-de79db9de4d2"
    ~name:"linker" (fun _ -> empty)
...
module Make (Machine : Machine) = struct
  ...
  let run name (module Code : Code) =
    ...
    Machine.Observation.make will_exec name >>= fun () -> Code.exec
  ...
  let lookup name = Machine.Local.get state >>| code_of_name name

  let exec name =
    lookup name >>= function None -> fail name | Some code -> run name code
  ...
  let resolve f name =
    ...
    | None -> None
    | Some id -> (
        match Map.find s.alias id with None -> None | Some refs -> f refs)
...
module Name = struct
  ...
  include Regular.Make (struct
    ...
    let pp ppf n = Format.fprintf ppf "%s" (string_of_name n)
  end)
end
