...
let exn_raised, raise_exn =
  Observation.provide
    ...
    "exception" ~desc:"Occurs just before the Machine exception is raised."

let fork, forked =
  Observation.provide
    ~inspect:(fun (parent, child) ->
      Sexp.List
        [
          Monad.State.Multi.Id.sexp_of_t parent;
          Monad.State.Multi.Id.sexp_of_t child;
        ])
    "machine-fork" ~desc:"Occurs after a machine fork is created."
...
let switch, switched =
  Observation.provide
    ...
    "machine-switch" ~desc:"Occurs when machines are switched."

let kill, killed =
  Observation.provide "machine-kill" ~inspect:Monad.State.Multi.Id.sexp_of_t
    ~desc:
...
module Make (M : Monad.S) = struct
  ...
  and 'a machine = {
    run : reject:(exn -> state -> r) -> accept:('a -> state -> r) -> state -> r;
  }
  ...
  let get () = { run = (fun ~reject:_ ~accept s -> accept s s) } [@@inline]
  let gets f = { run = (fun ~reject:_ ~accept s -> accept (f s) s) } [@@inline]
  ...
  let with_context cid (f : unit -> 'a t) =
    {
      run =
        (fun ~reject ~accept s ->
          (f ()).run { s with self = cid } ~reject ~accept:(fun r s' ->
  ...
  let set_global global = update @@ fun s -> { s with global }
  ...
  module Observation = struct
    ...
    let unrestricted () =
      gets @@ fun s -> if s.restricted then None else Some s.observations
    ...
    let observe key observer = subscribe key observer >>| fun _ -> ()
    ...
    let cancel sub =
      observations () >>= fun os -> set_observations (Observation.cancel sub os)
    ...
    let make_even_if_restricted key obs =
      observations () >>= fun os -> Observation.notify os key obs

    let post key ~f =
      unrestricted () >>= function
      | None -> return ()
      | Some os ->
          Observation.notify_if_observed os key @@ fun k -> f (fun x -> k x)
  end

  let make_get states state =
    states () >>= fun states ->
    State.Bag.with_state states state ~ready:return ~create:(fun make ->
        get () >>= fun { proj } -> return (make proj))

  let make_put get set state x =
    get () >>= fun states -> set (State.Bag.set states state x)

  let make_update get put data ~f = get data >>= fun s -> put data (f s)
  ...
  module Other = struct
    let get_other pid state = with_context pid @@ fun () -> get_local state
    let put_other pid state = with_context pid @@ fun () -> set_local state
  ...
  let fork_state () =
    ...
    {
      s with
      local = Map.change s.local child ~f:(fun _ -> Map.find s.local s.self);
      self = child;
  ...
  let store_curr k =
    update @@ fun s -> { s with conts = Map.set s.conts ~key:s.self ~data:k }

  let get_curr =
    gets @@ fun { conts; self } ->
    match Map.find conts self with None -> return | Some k -> k

  let status x =
    gets @@ fun { self; conts } ->
    if Id.equal self x then `Current
    else if Map.mem conts x then `Live
    else `Dead
  ...
  let ancestor xs =
    gets @@ fun { parent = ps; conts } ->
    let is_alive = Map.mem conts in
    let rec parent i =
      match Map.find ps i with
      | None -> global
  ...
  let notify_fork pid =
    current () >>= fun cid -> Observation.make forked (pid, cid)

  let call : f:(cc:('a -> _ t) -> 'a t) -> 'a t =
   fun ~f ->
    {
      run =
        (fun ~reject ~accept s ->
  ...
  let restrict x = update @@ fun s -> { s with restricted = x }

  let sentence_to_death id =
    update @@ fun s -> { s with deathrow = id :: s.deathrow }

  let drop id s =
    { s with conts = Map.remove s.conts id; local = Map.remove s.local id }
  ...
  let switch id : unit t =
    ...
    | false ->
        call ~f:(fun ~cc:k ->
            ...
            Observation.make switched (pid, id) >>= fun () -> k ())
  ...
  let fork () : unit t =
    ...
    | false ->
        call ~f:(fun ~cc:k ->
            current () >>= fun pid ->
            store_curr k >>= fork_state >>= fun () ->
            execute_sentenced >>= fun () -> notify_fork pid)
    ...
    | false ->
        if Id.(id = global) then return ()
          ...
          if Id.(id = cid) then
            ...
            restrict false >>= fun () -> sentence_to_death id
          else
            switch id >>= fun () ->
            restrict true >>= fun () ->
            Observation.make_even_if_restricted killed id >>= fun () ->
            restrict false >>= fun () ->
            switch cid >>= fun () -> do_kill id

  let start sys = Observation.make started sys

  let stop sys =
    restrict true >>= fun () -> Observation.make_even_if_restricted stopped sys

  let fail exn = { run = (fun ~reject ~accept:_ s -> reject exn s) }

  let catch x except =
    {
      run =
        (fun ~reject ~accept s ->
          x.run s ~accept ~reject:(fun exn s ->
              (except exn).run ~reject ~accept s));
    }

  let lift : 'a m -> 'a t =
   fun x ->
    { run = (fun ~reject:_ ~accept s -> M.bind x ~f:(fun x -> accept x s)) }

  let raise exn = Observation.make raise_exn exn >>= fun () -> fail exn
  let catch = catch
  ...
  let run : 'a t -> 'a e =
   fun user ?(boot = return ()) ?(init = return ()) ?(fini = return ())
       ?(envp = [||]) ?(args = [||]) sys proj ->
    let machine =
      boot >>= fun () ->
      restrict false >>= fun () ->
      init >>= fun () ->
        ...
        (fun exn ->
          fini >>= fun () ->
          stop sys >>= fun () -> raise exn)
      >>= fun x ->
      fini >>= fun () ->
      stop sys >>= fun () -> return x
    in
  ...
  include (Machine : Monad.S with type 'a t := 'a t and module Syntax := Syntax)
end
