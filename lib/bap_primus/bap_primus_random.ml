...
module MCG = struct
  ...
  module Bitvec_M16 = Bitvec.Make (struct
    let modulus = Bitvec.modulus 16
  end)
  ...
  module Cat (Output : Size) (Sub : S) : S = struct
    ...
    let create seed : t =
      Array.init generators ~f:(fun salt -> Sub.create (seed + salt))

    let next = Array.map ~f:Sub.next

    let value xs =
      Array.fold
        ~f:(fun y x -> Z.((y lsl int Sub.size) lor Sub.value x))
        ~init:Z.zero xs
  end
  ...
  *)
  ...
  let create ?min ?max width =
    if Option.is_none min && Option.is_none max then make_for_width width
    else
      let module Z = Bitvec.Make (struct
        let modulus = Bitvec.modulus width
      end) in
      let module R = struct
        let min = match min with None -> Bitvec.zero | Some min -> min
        let max = match max with None -> Z.ones | Some max -> max
        let size = width
      end in
  ...
  let create_small ?min ?max width =
    ...
    let min = Option.map ~f:int min and max = Option.map ~f:int max in
