...
let char_of_string s =
  try Ok Char.(Z.of_int @@ to_int @@ of_string s) with _ -> Error Bad_literal

let read_char str = char_of_string (String.subo ~pos:1 str)

let read_int str =
  try Ok (Z.of_string (String.strip str)) with _ -> Error Bad_literal
...
let int ?typ id eq s =
  Result.bind (read_int s) ~f:(fun exp ->
      ...
      | Some s -> (
          match Type.read s with
          | None -> Error Bad_type
          | Some typ -> Ok { data = { exp; typ }; id; eq }))

let read id eq x =
  if String.is_empty x then Error Empty
  else if Char.(x.[0] = '?') then char id eq x
  else if
    Char.is_digit x.[0]
    || (String.length x > 1 && Char.is_digit x.[1] && Char.(x.[0] = '-'))
  then
