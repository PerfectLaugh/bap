...
let cls : (cls, unit) KB.cls =
  KB.Class.declare "attributes" () ~package:"primus" ~public:true
...
let parsers : (Name.t, parser) Hashtbl.t = Hashtbl.create (module Name)
...
module Parse = struct
  ...
  let atom = function { data = Atom s } -> Some s | _ -> None
  let list = function { data = List ts } -> Some ts | _ -> None
...
let declare ?desc ?package ~domain ~parse name =
  let slot = KB.Class.property cls name domain ?package ?desc ~public:true in
...
let expected_parsers () =
  Hashtbl.keys parsers |> List.map ~f:Name.show |> String.concat ~sep:" | "

let parse package s attrs name values =
...
module Set = struct
  ...
  let slot =
    KB.Class.property Theory.Program.cls "attributes" Self.domain ~public:true
      ~package:"primus"
...
let () =
  Stdlib.Printexc.register_printer (function
    | Failure (error, _) ->
        let msg =
          Format.asprintf "Attribute parse error: %s"
