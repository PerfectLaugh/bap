...
type name = [ `top | `sub | `arg | `blk | `phi | `def | `jmp ] [@@deriving sexp]
type invariant = { level : t; dst : name }
...
let () =
  Exn.add_printer (function
    | Broken_invariant { level; dst } ->
        Option.some
        @@ sprintf
             "Level transition - broken invariant: No transition is defined \
              from the %s level to the %s level"
             (to_string level)
...
let level name f x = Sexp.List [ Sexp.Atom name; Sexp.Atom (f x) ]
let ( >> ) = Fn.compose

let leaf name str t =
  Sexp.List [ Sexp.Atom name; Sexp.Atom (String.strip (str t)) ]
...
let next level cls t =
  ...
  Term.switch cls t
    ...
    ~blk:(fun me ->
      match level with
      | Def { up = { up } } | Jmp { up = { up } } | Sub up | Arg { up } ->
          accept blk { me; up }
      | Top ({ me = prog } as top) ->
          accept blk { me; up = { me = parent prog me; up = top } }
      | _ -> reject `blk)
    ~phi:(fun me ->
      match level with
    ...
    ~jmp:(fun me ->
      match level with
      | Blk up | Phi { up } | Def { up } | Jmp { up } -> accept jmp { me; up }
      | _ -> reject `jmp)
