...
type advisors = { after : Set.M(KB.Name).t; before : Set.M(KB.Name).t }

type bindings = { vars : int Var.Map.t; stack : (Var.t * value) list }
[@@deriving sexp_of]
...
let state =
  Bap_primus_state.declare ~inspect ~name:"lisp-env"
    ~uuid:"0360697d-febe-4982-a528-152ada72bf4a" (fun proj ->
      {
        env = { stack = []; vars = Var.Map.empty };
        cur = Id.null;
...
let lisp_primitive, primitive_called =
  Bap_primus_observation.provide ~inspect:Bap_primus_linker.sexp_of_call
    "lisp-primitive" ~desc:"Occurs when the Lisp primitive is invoked."
...
let merge_advisors x y =
  { before = Set.union x.before y.before; after = Set.union x.after y.after }
...
let collect_advisors prog =
  Lisp.Program.fold prog func
    ~f:(fun ~package def init ->
      if Lisp.Program.is_applicable prog def then
        let adv = Attribute.Set.get Advice.t (Lisp.Def.attributes def) in
        let advisor = KB.Name.create ~package (Lisp.Def.name def) in
        List.fold
          Advice.[ After; Before ]
          ~init
          ~f:(fun init cmethod ->
...
module Errors (Machine : Machine) = struct
  ...
  let error kind =
    Format.ksprintf (fun msg () ->
        with_loc msg >>= fun msg -> Machine.raise (kind msg))
...
module Locals (Machine : Machine) = struct
  ...
  let replace x w s =
    { s with env = { s.env with stack = update s.env.stack x ~f:(fun _ -> w) } }
      ...
      env =
        {
          ...
          vars =
            Map.update s.env.vars v ~f:(function None -> 1 | Some n -> n + 1);
        };
    }
   ...
   * anywhere.
   *)
  let push_frame bs s = List.fold bs ~init:s ~f:(fun s (v, w) -> push v w s)
...
let () =
  Exn.add_printer (function
    ...
    | Unresolved (name, res) ->
        let msg =
          asprintf "unable to resolve function %a, because %a" KB.Name.pp name
            Lisp.Resolve.pp_resolution res
        in
        Some msg
    | _ -> None)
...
let message, new_message =
  Bap_primus_observation.provide ~inspect:sexp_of_string "lisp-message"
    ~desc:"Occurs with X when (msg x) is evaluated."

type closure_frame = { name : string; args : value list }

let closure_context =
  Bap_primus_state.declare ~uuid:"b31c12fc-a131-4966-bcc9-e360ed76fb8e"
    ~name:"lisp-closure-context"
  @@ fun _ -> []
...
module Interpreter (Machine : Machine) = struct
  ...
  let say fmt = ksprintf (Machine.Observation.make new_message) fmt
  let width () = Machine.Local.get state >>| fun { width } -> width

  let is_zero x =
    Value.zero (Value.bitwidth x) >>= fun zero -> Eval.binop Bil.EQ x zero

  let is_external_call name def =
    let calls = Attribute.Set.get External.t (Lisp.Def.attributes def) in
    Set.mem calls name

  let notify_when ?rval cond obs name args =
    if cond then
      Machine.Observation.post obs ~f:(fun notify ->
          match rval with
  ...
  let lookup_parameter prog { data = { exp = name } } =
    ...
    Lisp.Program.get ~name:(KB.Name.unqualified name) prog para |> List.hd
  ...
  *)
  let rec eval_lisp fullname args : value Machine.t =
    ...
    Lisp.Resolve.defun Lisp.Check.value s.program func fullname args |> function
    | None -> eval_primitive fullname args
    | Some (Error resolution) ->
        Machine.raise (Unresolved (fullname, resolution))
    | Some (Ok (fn, bs)) ->
        ...
        notify_when is_external Trace.call_returned name args ~rval:r
        >>= fun () -> eval_advices Advice.After r fullname args
  ...
  and eval_primitive name args =
    ...
    | Some (Ok (code, ())) ->
        ...
        Machine.Observation.post primitive_called ~f:(fun k ->
            k (show name, args @ [ r ]))
        >>= fun () -> eval_advices Advice.After r name args
  ...
  and eval_exp exp =
    let int v t =
      ...
      Eval.const (Word.create bv width)
    in
    let sym v = Value.Symbol.to_value (KB.Name.unqualified v.data) in
    let var width places v =
      match Map.find places v.data.exp with
    ...
    and ite c e1 e2 = eval c >>= fun c -> Eval.branch c (eval e1) (eval e2)
    and let_ v e1 e2 =
      ...
      eval e2 >>= fun r ->
      Machine.Local.update state ~f:(Vars.pop 1) >>= fun () -> Machine.return r
    and lookup v' =
      Machine.Local.get state >>= fun { env; width; program; places } ->
      let v = var width places v' in
      if Map.mem env.vars v then
        Machine.return @@ List.Assoc.find_exn ~equal:Var.equal env.stack v
      else
        Env.has v >>= function
        | true -> Eval.get v
        | false -> (
            lookup_parameter program v' |> function
            | None -> Eval.get v
            | Some p ->
                eval (Lisp.Def.Para.default p) >>= fun x ->
                Eval.set v x >>| fun () -> x)
    and app n args =
      Machine.List.map args ~f:eval >>= fun args ->
      match n with Static _ -> assert false | Dynamic n -> eval_lisp n args
    and seq es =
      let rec loop = function
        | [] -> Eval.const Word.b0
        | e :: [] -> eval e
        | e :: es -> eval e >>= fun _ -> loop es
      in
      loop es
    and set v w =
      Machine.Local.get state >>= fun s ->
      let v = var s.width s.places v in
      if Map.mem s.env.vars v then
        Machine.Local.put state (Vars.replace v w s) >>| fun () -> w
    ...
    and msg fmt es =
      let str e =
        Machine.catch
          ...
          (fun err -> Machine.return (asprintf "<%s>" (Exn.to_string err)))
      in
      Machine.List.map fmt ~f:(function
        | Lit s -> Machine.return s
        | Pos n -> (
            match List.nth es n with
            | None -> failf "bad positional argument $%d" n ()
            | Some e -> str e))
      >>= fun contents ->
      let msg = String.concat contents in
      Machine.Observation.make new_message msg >>= fun () -> Eval.const Word.b0
    in
    Machine.Local.update state ~f:(fun s -> { s with cur = exp.id })
    >>= fun () -> eval exp

  let eval_signal name args : unit Machine.t =
    Machine.Local.get state >>= fun s ->
    let res = Lisp.Resolve.meth Lisp.Check.value s.program meth name args in
    match res with
    | None -> Machine.return ()
...
module Type = struct
  ...
  let error, notify_error =
    Bap_primus_observation.provide "lisp-type-error"
      ~desc:"Occurs when the Lisp type error is detected by the type checker"
      ~inspect:(fun err -> Sexp.Atom (Format.asprintf "%a" pp_error err))
  ...
  module Spec = struct
    ...
    let int t = Lisp.Type.word (Theory.Target.bits t)
    let bool = word 1
    let byte = word 8
    let a : t = var "a"
    let b : t = var "b"
...
module Make (Machine : Machine) = struct
  ...
  module Typechecker = struct
    ...
    let signature_of_sub sub =
      let lisp_type_of_arg arg =
        match Var.typ (Arg.lhs arg) with
        | Imm 1 -> Type.Spec.bool
        | Imm n -> Type.Spec.word n
        | Unk | Mem _ -> Type.Spec.any
      in
      if Term.length arg_t sub = 0 then Type.Spec.(all any @-> any)
      else
  ...
  let export_externals program =
    Lisp.Program.fold program func
  ...
  let link_global var =
    ...
    | false -> (
        ...
        | Imm m -> Value.zero m >>= Env.set var
        | _ -> Machine.return ())
  ...
  let add_places =
    ...
    Machine.Local.update state ~f:(fun s ->
        { s with program = Lisp.Program.with_places ~globals s.program target })
    ...
    Machine.Local.update state ~f:(fun s ->
        {
          s with
          places =
            Lisp.Program.fold s.program place
              ~f:(fun ~package r places ->
                Map.set places
  ...
  let link_places () =
    ...
    Map.to_sequence places |> Machine.Seq.iter ~f:(fun (_, v) -> link_global v)
  ...
  let find_sub prog name =
    ...
    | Some sub -> (
        ...
        | Some proto ->
            ...
            let args, ret =
              match proto.C.Type.Proto.return with
              | `Void -> (args, None)
              | _ -> List.(take args (List.length args - 1), last args)
            in
            (args, ret, tid, addr))
  ...
  let link_feature program def =
    ...
    Lisp.Resolve.extern Lisp.Check.arg program Lisp.Program.Items.func fullname
      args
    |> function
    | None -> Machine.return ()
    ...
    | Some (Ok (fn, bs)) ->
        ...
        let module Code (Machine : Machine) = struct
          ...
          let size_of_reg r =
            match Var.typ r with Imm x -> x | _ -> assert false
          ...
          let exec =
            ...
            Interp.notify_when true Trace.lisp_call_entered name args
            >>= fun () ->
            Interp.eval_exp (Lisp.Def.Func.body fn) >>= fun r ->
            Machine.Local.update state ~f:(Vars.pop frame_size) >>= fun () ->
            Interp.notify_when true Trace.lisp_call_returned name args ~rval:r
            >>= fun () ->
            Interp.notify_when true Trace.call_returned name args ~rval:r
            >>= fun () ->
  ...
  let copy_primitives ~src ~dst =
    ...
    |> List.fold ~init:dst ~f:(fun dst p -> Lisp.Program.add dst primitive p)
  ...
  let link_program program =
    Machine.sequence [ set_program program; link_features; link_advisors ]

  let typecheck = Machine.sequence [ link_places (); Typechecker.run ]
  let program = Machine.Local.get state >>| fun s -> s.program

  let link_primitive p =
    Machine.Local.update state ~f:(fun s ->
        { s with program = Lisp.Program.add s.program primitive p })

  let define ?types ?docs ?package name body =
    Machine.gets Project.target >>= fun arch ->
    let types = Option.map types ~f:(fun t -> t arch) in
    Lisp.Def.Closure.create ?types ?docs ?package name body |> link_primitive
  ...
  let signal ?params ?(doc = "undocumented") obs proj =
    ...
    let types =
      match params with
      | None -> default_types
      | Some (`All t) -> Lisp.Type.signature [] ~rest:(t arch) Any
      | Some (`Tuple ts) -> Lisp.Type.signature (specialize ts) Any
      | Some (`Gen (ts, t)) ->
          Lisp.Type.signature (specialize ts) ~rest:(t arch) Any
    in
    ...
    Machine.Local.update state ~f:(fun s ->
        {
          s with
          program = Lisp.Program.add s.program signal r;
    ...
    let known_methods =
      Lisp.Program.fold s.program meth
        ...
        ~f:(fun ~package met mets ->
          Set.add mets @@ KB.Name.create ~package (Lisp.Def.name met))
    in
    let useless_subscriptions =
      Map.fold s.signals ~init:[] ~f:(fun ~key:name ~data:sub subs ->
          if Set.mem known_methods name then subs else sub :: subs)
    in
    ...
    Machine.Local.update state ~f:(fun s ->
        {
          ...
          program =
            Lisp.Program.with_context s.program
              (Lisp.Context.merge ctxt (Lisp.Program.context s.program));
        })
end
...
module Doc = struct
  ...
  module Descr = struct
    type t = { desc : string; code : string option; loc : Lisp.Loc.t option }

    let normalize_location : loc -> loc =
     fun loc -> { loc with file = Filename.basename loc.file }
    ...
    let pp_location ppf { loc } =
      match loc with None -> () | Some loc -> Lisp.Loc.pp ppf loc

    let pp_source ppf { code } =
      match code with None -> () | Some code -> Format.fprintf ppf "%s" code
  ...
  let normalize xs =
    Map.of_alist_reduce (module Name) xs ~f:Descr.merge |> Map.to_alist
  ...
  let describe prog item =
    Lisp.Program.fold prog item ~init:[] ~f:(fun ~package def defs ->
        ...
        (name, Descr.create prog def) :: defs)
    |> normalize
  ...
  module Make (Machine : Machine) = struct
    ...
    let generate_index : index Machine.t =
      Machine.Local.get state >>| fun s -> create_index s.program
  end
end
...
module Semantics = struct
  ...
  let documentation unit = KB.Syntax.(typed_program unit >>| Doc.create_index)
end
